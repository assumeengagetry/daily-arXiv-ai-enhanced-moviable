...
TL;DR: 提出基于高斯分布的混合表示方法，将高斯分为描绘高频边缘的Sketch Gaussians和覆盖低频平滑区域的Patch Gaussians，实现分层渐进式流式传输，在保持视觉质量的同时大幅压缩模型大小。


<details>
  <summary>Details</summary>
Motivation: 观察到高斯分布与传统艺术技法类似，有些捕捉高频特征（如边缘轮廓），有些表示低频平滑区域（如画笔笔触）。基于此观察，希望开发一种能自然支持分层渐进式流式传输的3D场景表示方法，实现高效存储和自适应渲染。

Method: 提出混合表示方法，将高斯分为两类：Sketch Gaussians（表示高频边界特征）和Patch Gaussians（覆盖低频平滑区域）。采用分层自适应分类框架，基于多准则密度聚类和自适应质量驱动优化，直接在3DGS表示上操作，无需依赖外部3D线基元。

Result: 在多样化场景（人造和自然环境）评估中，相比均匀剪枝基线，在相同模型大小下PSNR提升达1.74 dB，SSIM提升6.7%，LPIPS提升41.4%。对于室内场景，仅需原始模型大小的0.5%即可保持视觉质量。

Conclusion: 提出的结构感知表示方法能够实现高效存储、自适应流式传输和渲染，适用于带宽受限网络和资源有限设备，为高保真3D内容的高效处理提供了有效解决方案。
...
### [2] [LayerGS: Decomposition and Inpainting of Layered 3D Human Avatars via 2D Gaussian Splatting](https://arxiv.org/abs/2601.05853)
*Yinghan Xu,John Dingliana*

Main category: cs.CV

TL;DR: 提出LayerGS框架，使用2D高斯表示分解人体为可动画的多层3D化身，通过扩散模型修复遮挡区域，实现高质量渲染和虚拟试穿


<details>
  <summary>Details</summary>
Motivation: 传统单层重建方法将服装锁定在特定身份上，而现有多层方法难以处理遮挡区域，限制了高质量3D人体资产的创建和虚拟试穿应用

Method: 1) 使用2D高斯表示编码每层几何和纹理；2) 通过预训练2D扩散模型和SDS修复遮挡区域；3) 三阶段训练：先单层重建粗粒度规范服装，再联合训练恢复内层身体和外层服装细节

Result: 在4D-Dress和Thuman2.0数据集上优于现有方法，实现更好的渲染质量、层分解和重组，支持新颖视角和姿态下的真实虚拟试穿

Conclusion: LayerGS框架成功解决了多层人体重建中的遮挡问题，为沉浸式应用创建高保真3D人体资产提供了实用方案，代码已开源
...
### [3] [Bi-Orthogonal Factor Decomposition for Vision Transformers](https://arxiv.org/abs/2601.05328)
*Fenil R. Doshi,Thomas Fel,Talia Konkle,George Alvarez*

Main category: cs.CV

TL;DR: BFD框架通过统计分解和奇异值分解，揭示了视觉Transformer中注意力机制如何通过位置和内容因子进行token交互，发现内容交互主导注意力，DINOv2在内容-位置耦合上分配更多能量并具有更丰富的模式谱。


<details>
  <summary>Details</summary>
Motivation: 尽管注意力机制是Vision Transformer的核心计算单元，但我们缺乏对其在token间交换信息的原理性理解。注意力图仅显示权重集中的位置，无法揭示查询和键之间交换的是位置信息、内容信息还是两者兼有。

Method: 提出双正交因子分解（BFD）框架：第一阶段使用基于ANOVA的分解将token激活统计解耦为正交的位置和内容因子；第二阶段对查询-键交互矩阵QK^T进行奇异值分解，揭示这些因子如何介导通信的双正交模式。

Result: 1. 注意力主要通过内容运作，内容-内容交互主导注意力能量，其次是内容-位置耦合。DINOv2比监督模型分配更多能量到内容-位置耦合，并在更丰富的模式谱上分布计算。
2. 注意力机制表现出专业化：头部分化为内容-内容、内容-位置和位置-位置算子，头内的奇异模式也显示类似专业化。
3. DINOv2的优越整体形状处理能力源于中间层同时保持位置结构并在上下文中丰富语义内容。

Conclusion: BFD揭示了token如何通过注意力机制交互，以及哪些信息因子（位置或语义）介导它们的通信，为视觉Transformer机制提供了实用见解。
...
### [4] [Coding the Visual World: From Image to Simulation Using Vision Language Models](https://arxiv.org/abs/2601.05344)
*Sagi Eppel*
...
<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型是否能够理解和模拟图像中描绘的真实世界系统，评估其构建心理模型的能力。

Method: 使用Im2Sim方法：给VLM输入真实世界系统的自然图像，要求其描述系统并编写生成模拟代码，执行代码产生合成图像，然后与原图比较。

Result: 领先的VLMs（GPT、Gemini）能够理解和建模跨多个抽象层次和广泛领域的复杂多组件系统，但在复制精细细节和低层模式排列方面能力有限。

Conclusion: VLMs展现出有趣的不对称性：结合了高层次、深度的视觉理解能力，但对精细细节的感知有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有轻量级神经网络通常在精度和延迟之间进行权衡，这限制了它们在微控制器和神经处理单元等边缘硬件上的应用。需要开发在资源受限平台上同时优化精度、效率和内存占用的模型。

Method: 提出了两个模型家族：STResNet用于图像分类，包含从Nano到Tiny的变体；STYOLO用于目标检测，包含Micro和Milli变体。这些模型专门针对资源受限平台进行联合优化。

Result: STResNet系列在400万参数预算内取得了有竞争力的ImageNet 1K准确率，其中STResNetMilli仅用300万参数就达到了70.0%的Top-1准确率，优于MobileNetV1和ShuffleNetV2。STYOLOMicro和STYOLOMilli在MS COCO数据集上分别达到30.5%和33.6%的mAP，超越了YOLOv5n和YOLOX Nano。

Conclusion: 提出的STResNet和STYOLO模型在资源受限的边缘设备上实现了精度、效率和内存占用的良好平衡，为在微控制器和神经处理单元上部署深度学习模型提供了有效的解决方案。
...
TL;DR: MOSAIC-GS：一种基于高斯溅射的单目视频动态场景重建方法，通过几何线索和刚性运动约束实现高效优化和实时渲染
...
Method: 1. 利用深度、光流、动态物体分割和点跟踪等多重几何线索；2. 结合基于刚性的运动约束，在初始化阶段估计初步的3D场景动态；3. 将场景分解为静态和动态组件；4. 为动态部分的高斯分配时间相关的Poly-Fourier曲线轨迹，实现参数高效的运动编码。

Result: 相比现有方法，MOSAIC-GS实现了显著更快的优化和渲染速度，同时在标准单目动态场景基准测试中保持了与最先进方法相当的重建质量。

Conclusion: MOSAIC-GS通过几何线索和高效运动编码，解决了单目动态场景重建中的病态问题，实现了高质量、高效率的重建和实时渲染能力。
...
TL;DR: 集成深度学习和影像组学的方法在乳腺癌筛查中表现最佳，AUC达到0.87，优于单独的深度学习方法（0.83）和影像组学方法（0.80）。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌早期诊断对提高生存率至关重要。放射组学和深度学习在辅助放射科医生进行早期癌症检测方面显示出巨大潜力，但需要评估这些方法及其组合在筛查乳腺X光片中的性能。

Method: 使用两个独立数据集：RSNA 2023乳腺癌检测挑战赛（11,913名患者）和墨西哥TecSalud队列（19,400名患者）。在RSNA数据集上训练ConvNeXtV1-small深度学习模型并在TecSalud数据集上验证；在TecSalud数据集上开发放射组学模型并使用留一年法验证；采用一致的集成方法组合和校准预测。

Result: 集成方法获得最高的曲线下面积（AUC）0.87，而ConvNeXtV1-small深度学习模型为0.83，放射组学模型为0.80。集成方法显著提升了乳腺癌诊断性能。

Conclusion: 结合深度学习和放射组学预测的集成方法能显著增强乳腺X光片的乳腺癌诊断能力，为临床实践提供了更优的辅助诊断工具。
...
<details>
  <summary>Details</summary>
Motivation: 边缘设备部署深度神经网络受限于内存带宽和密集线性运算成本。四元数神经网络通过Hamilton乘积提高参数效率，但权重仍为密集结构；结构化矩阵可实现快速计算但通常应用于实数域。需要结合两者的优势。

Method: 提出EdgeLDR框架，将四元数块循环线性层和卷积层结合，通过复数伴随表示实现基于FFT的评估。提供参考实现，比较FFT计算与空间域实现的性能。

Result: FFT评估相比朴素实现获得显著加速，且延迟随块大小增加保持稳定，使更大压缩因子计算可行。在紧凑CNN和Transformer骨干中集成EdgeLDR层，在多个数据集上评估准确率-压缩权衡，显示显著压缩同时保持竞争力准确率。

Conclusion: EdgeLDR层通过结合四元数通道混合与块循环参数结构，为边缘设备提供显著压缩和竞争性准确率，是实用的高效推理框架。
...
TL;DR: 提出一个多任务学习框架来微调BiomedCLIP，以改进胸部X光图像-文本检索性能，通过结合分类、对比和CLIP损失实现更平衡和临床相关的检索结果。


<details>
  <summary>Details</summary>
Motivation: CLIP和BiomedCLIP虽然提供强大的跨模态嵌入，但未针对细粒度医学检索任务（如使用胸部X光图像查询相关放射学报告）进行优化，需要专门的方法来提升临床检索性能。

Method: 以BiomedCLIP为骨干，加入轻量级MLP投影头，使用多任务复合损失函数训练：1）区分正常与异常CXR研究的二元交叉熵损失；2）增强类内一致性的监督对比损失；3）保持跨模态对齐的CLIP损失。

Result: 微调后的模型在图像到文本和文本到图像检索任务上相比预训练的BiomedCLIP和通用CLIP模型，实现了更平衡和临床有意义的性能。t-SNE可视化显示正常与异常病例的语义聚类更清晰，表明模型诊断敏感性增强。

Conclusion: 领域自适应的多任务学习对于推进生物医学应用中的跨模态检索具有重要价值，能够显著提升临床相关性和诊断敏感性。
...
TL;DR: 该论文提出了一种让大视觉语言模型具备"地图思维"能力的方法，通过代理在地图中的循环机制，结合强化学习和并行测试时扩展，显著提升了图像地理定位的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型方法虽然利用了世界知识、思维链推理和代理能力，但忽视了人类常用的策略——使用地图。图像地理定位任务需要预测图像在地球上的拍摄位置，而地图是解决这一问题的关键工具。

Method: 1. 赋予模型"地图思维"能力，将其表述为代理在地图中的循环机制；2. 采用两阶段优化方案：先通过代理强化学习增强模型的代理能力以提高采样效率，然后通过并行测试时扩展让模型在做出最终预测前探索多个候选路径；3. 构建MAPBench基准，包含完全由真实世界图像组成的全面地理定位训练和评估数据集。

Result: 该方法在大多数指标上优于现有的开源和闭源模型，特别是将Acc@500m从Gemini-3-Pro的8.0%提升到22.1%，性能提升了近三倍。

Conclusion: 通过赋予模型地图思维能力并采用两阶段优化策略，可以显著提升图像地理定位的准确性。MAPBench基准为这一领域提供了更贴近现实世界的评估标准。
...
Method: 提出轨迹感知Mamba传播网络(TAPM-Net)，包含两个核心组件：扰动引导路径模块(PGM)和轨迹感知状态块(TASB)。PGM从多级特征构建扰动能量场，提取梯度跟随的特征轨迹；TASB是基于Mamba的状态空间单元，沿轨迹建模动态传播，结合速度约束扩散和语义对齐的特征融合。
...
Method: 1. 使用自适应XY间隙树从复杂布局中提取分层阅读序列；2. 通过阅读顺序感知相对位置偏置将阅读顺序集成到注意力机制中；3. 引入文本令牌子块注意力先验自适应抑制视觉噪声并增强细粒度文本-文本交互。

Result: 在FUNSD和CORD基准测试中，ROAP持续改进了代表性骨干网络（包括LayoutLMv3和GeoLayoutLM）的性能。
...
TL;DR: 本文提出了一种用于地下植物根系成像的多图像超分辨率深度学习框架，通过合成数据集训练，利用多视角空间冗余重建高分辨率根系图像，显著提升了图像质量和根系性状分析能力。


<details>
  <summary>Details</summary>
Motivation: 地下植物根系成像面临遮挡、土壤湿度变化和低对比度等挑战，传统视觉方法效果有限，阻碍了土壤-植物相互作用、养分吸收和植物健康研究。

Method: 提出新型地下成像系统，捕获多重叠视角的根系图像，并集成深度学习多图像超分辨率框架；构建模拟真实地下成像场景的合成数据集进行训练和评估；利用多视角间的空间冗余重建高分辨率图像。

Result: 定量评估显示该方法优于现有超分辨率基线，BRISQUE指标降低2.3%，CLIP-IQA分数相同，表明图像质量提升；能够准确估计根系关键性状如根毛数量和密度。

Conclusion: 该框架为农业和生态研究提供了稳健的自动地下植物根系成像和性状量化方法，有望推动根系表型分析的进步。
...
TL;DR: 该研究使用CAT12/SPM12体素形态测量分析249名ADNI参与者的MRI数据，发现AD患者海马萎缩显著，海马体积对MCI向AD转化具有中等预测价值，APOE4状态对海马体积无显著影响。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病和轻度认知障碍与灰质损失相关，特别是在内侧颞叶结构。研究旨在通过MRI分析验证海马萎缩作为AD进展的关键特征，并探索其作为预测生物标志物的价值以及遗传因素的影响。

Method: 使用CAT12/SPM12体素形态测量分析249名ADNI参与者（CN=90，MCI=129，AD=30）的基线T1加权MRI扫描。采用一般线性模型分析灰质体积，以诊断组为主要预测因子，年龄和总颅内体积作为协变量。统计图阈值设为p<0.001（体素水平），并使用家族误差校正进行多重比较校正（p<0.05）。

Result: AD患者相对于CN和MCI表现出显著的海马萎缩（Cohen's d分别为2.03和1.61）。海马体积对MCI向AD转化具有中等预测价值（AUC=0.66）。按APOE4状态分层分析未发现遗传因素对海马体积有显著影响。

Conclusion: 研究结果支持内侧颞叶变性是AD进展的关键特征，为预测生物标志物和遗传影响提供了见解。海马体积可作为AD进展的生物标志物，但APOE4状态对海马体积无显著影响。
...
TL;DR: MMViR提出了一种新的多模态、多粒度结构化表示方法，用于解决长视频理解中的计算复杂性和内容冗余问题，通过关键转折点分割和三级描述实现高效检索和理解。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在处理长视频（从几分钟到几小时）时面临重大挑战，包括复杂事件、多样化场景和长距离依赖关系。直接编码计算成本过高，而简单的视频到文本转换往往导致内容冗余或碎片化。

Method: MMViR通过识别关键转折点来分割视频，并构建三级描述结构，将全局叙事与细粒度视觉细节相结合。这种设计支持基于查询的高效检索，并能很好地泛化到各种场景。

Result: 在三个任务（问答、摘要和检索）上的广泛评估表明，MMViR优于先前最强方法，在小时级视频理解上实现了19.67%的改进，同时将处理延迟降低到原始的45.4%。

Conclusion: MMViR为长视频理解提供了一种有效的多模态、多粒度结构化表示方法，显著提升了性能并降低了计算成本，为解决长视频分析中的挑战提供了有前景的解决方案。
...
TL;DR: 该研究提出了一种多任务深度学习框架，利用Segment Anything Model (SAM)视觉编码器的嵌入特征，联合进行乳腺超声图像的病灶分割和诊断分类，在PRECISE 2025数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 乳腺超声图像存在对比度低、斑点噪声和病灶形态多样等挑战，导致准确的肿瘤分割和分类困难。需要开发能够同时处理分割和分类任务的有效方法。

Method: 提出多任务深度学习框架：1) 使用SAM视觉编码器提取高维特征；2) 通过轻量级卷积头或UNet风格解码器进行像素级分割；3) 通过掩码引导注意力机制增强分类分支，聚焦病灶相关特征；4) 采用无提示的完全监督适应方法。

Result: 在PRECISE 2025乳腺超声数据集上（按类别80%训练，20%测试），Dice相似系数达到0.887，准确率达到92.3%，在PRECISE挑战排行榜上名列前茅。

Conclusion: SAM基础表示结合分割引导学习能显著改善乳腺超声图像中的病灶描绘和诊断预测，为乳腺超声分析提供了有效的多任务解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前的大型语言模型和多模态大语言模型在处理表意文字时存在结构盲区：LLMs将字符视为文本标记，MLLMs将其视为原始像素网格，两者都无法建模字符笔画的内在逻辑。现有的结构分析方法通常是特定于某种文字的且劳动密集型。

Method: 提出Hieroglyphic Stroke Analyzer (HieroSA)框架，将字符图像转换为归一化坐标空间中的显式、可解释的线段表示。该方法无需手工数据，能够实现跨语言泛化。

Result: 大量实验表明，HieroSA能有效捕捉字符内部结构和语义，无需语言特定的先验知识。实验结果突显了该工作作为字形分析工具的潜力，可用于更深入地理解象形文字系统。

Conclusion: HieroSA为多模态大语言模型提供了一种自动分析字符笔画结构的新方法，克服了现有方法的局限性，为表意文字的结构分析提供了通用且可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频人脸交换方法局限于像素级表示，生成的人脸只是一组无结构的像素，缺乏动画或交互操作能力。需要从传统的像素级视频生成转向创建高保真、可交互的人脸化身。

Method: 1. 预处理目标视频提取FLAME参数、相机姿态和分割掩码；2. 将3D高斯溅射绑定到FLAME模型上实现动态面部控制；3. 提出复合身份嵌入（结合三种最先进的人脸识别模型）进行化身微调；4. 在背景帧上渲染人脸交换后的化身生成最终视频。

Result: 实验结果表明，GaussianSwap在身份保持、视觉清晰度和时间一致性方面表现优异，同时实现了传统方法无法达到的交互应用能力。

Conclusion: GaussianSwap实现了从传统像素级视频生成到高保真可交互人脸化身创建的范式转变，为人脸交换技术带来了新的交互可能性。
...
<details>
  <summary>Details</summary>
Motivation: 远距离视频行人重识别面临严重挑战：分辨率严重退化、视角剧烈变化和不可避免的外观噪声，需要新的方法来提取更具判别性的特征表示。

Method: 提出三模块框架：1) 记忆增强视觉骨干(MEVB)，结合CLIP视觉编码器和多代理记忆；2) 多粒度时序建模(MGTM)，构建多时间粒度序列并自适应强调跨尺度运动线索；3) 先验正则化形状动力学(PRSD)，捕捉身体结构动态。

Result: 在VReID-XFD基准测试中验证了各模块的有效性，最终框架在VReID-XFD挑战排行榜上排名第一。

Conclusion: SAS-VPReID框架通过三个互补模块，能够获得更具判别性的特征表示，有效解决了远距离视频行人重识别的挑战。
...
Method: 提出DIFF-MF模型：1）利用模态间特征差异图指导特征提取；2）通道维度：通过交叉注意力双状态空间建模的通道交换模块增强通道间交互；3）空间维度：通过跨模态状态空间扫描的空间交换模块实现全面空间融合；4）保持线性计算复杂度同时捕获全局依赖

Result: 在驾驶场景和低空无人机数据集上的实验结果表明，该方法在视觉质量和定量评估方面均优于现有方法

Conclusion: DIFF-MF通过差异驱动的通道-空间状态空间模型，有效整合了互补的多模态特征，解决了现有方法在红外强度和可见光细节之间的平衡问题
...
TL;DR: MoGen：一种用户友好的多目标图像生成方法，通过区域语义锚定和自适应多模态引导，实现精确的数量一致性和细粒度控制


<details>
  <summary>Details</summary>
Motivation: 现有多目标图像生成方法难以实现语言描述与局部图像生成区域的精确对齐，常导致对象数量不一致和属性混淆。主流方法依赖外部控制信号来约束空间布局，但输入格式僵化，无法适应不同用户资源和约束需求。

Method: 1. 设计区域语义锚定（RSA）模块，在生成过程中将语言描述中的短语单元精确锚定到对应图像区域，实现遵循数量规范的多对象生成。2. 引入自适应多模态引导（AMG）模块，自适应解析和整合多源控制信号，形成结构化意图，指导场景布局和对象属性的选择性约束。

Result: 实验结果表明，MoGen在生成质量、数量一致性和细粒度控制方面显著优于现有方法，同时展现出更好的可访问性和控制灵活性。

Conclusion: MoGen通过区域语义锚定和自适应多模态引导，解决了多目标图像生成中的数量一致性和细粒度控制问题，提供了一种更灵活、用户友好的解决方案。
...
TL;DR: 提出VIB-Probe框架，利用变分信息瓶颈理论检测和缓解视觉语言模型的幻觉问题，通过分析注意力头信号实现更准确的幻觉识别和干预。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型(VLMs)容易产生幻觉（生成文本与视觉内容不符），当前检测方法主要依赖输出logits或外部验证工具，忽略了模型内部机制。作者认为特定注意力头携带真实生成的主要信号，但直接探测这些高维状态很困难，因为视觉-语言语法和噪声纠缠在一起。

Method: 提出VIB-Probe框架：1) 利用变分信息瓶颈(VIB)理论提取跨层和跨头的判别模式，同时通过信息瓶颈原则过滤语义噪声；2) 利用VIB探针的梯度识别对幻觉有强因果影响的注意力头；3) 引入推理时干预策略缓解幻觉。

Result: 在多个基准测试上的广泛实验表明，VIB-Probe在幻觉检测和缓解两方面都显著优于现有基线方法。

Conclusion: VIB-Probe通过分析内部注意力头信号，提供了一种有效的幻觉检测和缓解框架，为理解视觉语言模型的内部机制提供了新视角。
...
Method: 首先发现语言编码器在异常检测中并非必需，然后提出简单方法完全解耦分类和分割任务，解耦跨层级特征，为不同任务和层级特征学习独立的权重。仅需学习0.002M参数。

Result: 在14个真实世界异常检测基准测试（工业和医疗领域）上，大幅超越最先进的零样本/少样本方法，甚至首次超越全样本方法，展现出卓越的通用性和有效性。

Conclusion: UniADet通过极简设计实现了通用视觉异常检测，证明了简单解耦方法的有效性，为开放动态场景下的异常检测提供了高效解决方案。
...
TL;DR: 提出基于动态阈值调整和选择性负学习的半监督面部表情识别算法，在有限标注数据下取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 面部表情识别是HCI和情感计算的关键任务，但获取大量标注数据成本高昂，需要设计能充分利用标注和未标注数据的半监督算法

Method: 1) 特征提取阶段采用局部注意力增强和特征图随机丢弃策略；2) 动态阈值调整适应半监督学习框架；3) 选择性负学习策略从低置信度未标注样本中挖掘有用表情信息
...
### [25] [What's Left Unsaid? Detecting and Correcting Misleading Omissions in Multimodal News Previews](https://arxiv.org/abs/2601.05563)
*Fanxiao Li,Jiaying Wu,Tingchao Fu,Dayang Li,Herun Wan,Wei Zhou,Min-Yen Kan*

Main category: cs.CV

TL;DR: 该研究开发了检测社交媒体新闻预览中"解释漂移"的框架，通过构建MM-Misleading基准评估LVLM模型，并提出OMGuard方法提升误导性检测和修正能力。


<details>
  <summary>Details</summary>
Motivation: 社交媒体新闻预览（图像-标题对）即使事实正确，也可能通过选择性省略关键上下文导致"解释漂移"，使读者形成与完整文章不同的判断。这种隐蔽的危害比显性错误信息更难检测且研究不足。

Method: 开发多阶段管道，分离和模拟基于预览与基于上下文的理解，构建MM-Misleading基准。提出OMGuard方法，包含：(1)解释感知微调提升多模态误导性检测；(2)理性引导的误导内容修正，使用明确推理指导标题重写以减少误导印象。

Result: OMGuard将8B模型的检测准确率提升至与235B LVLM相当，并提供显著更强的端到端修正。分析显示误导性通常源于局部叙事转变（如缺少背景）而非全局框架改变，并识别出仅文本修正失败的图像驱动场景。

Conclusion: 该研究揭示了社交媒体新闻预览中解释漂移的危害，开发了有效的检测和修正方法，强调了视觉干预的必要性，为多模态误导性检测提供了新框架。
...
### [26] [Towards Generalized Multi-Image Editing for Unified Multimodal Models](https://arxiv.org/abs/2601.05572)
*Pengcheng Xu,Peng Tang,Donghao Luo,Xiaobin Hu,Weichu Cui,Qingdong He,Zhennan Chen,Jiangning Zhang,Charles Ling,Boyu Wang*

Main category: cs.CV

TL;DR: 提出了一种可扩展的多图像编辑框架，通过可学习的潜在分离器和正弦索引编码来区分图像身份，提升多模态模型在跨图像编辑中的视觉一致性和细节区分能力。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在多图像编辑中存在视觉一致性保持和视觉线索区分不足的问题，特别是在跨多个输入图像引用细节时难以准确区分图像身份。

Method: 1) 可学习的潜在分离器：在潜在空间中显式区分每个参考图像，实现准确解耦的条件控制；2) 正弦索引编码：为同一图像的视觉标记分配连续的正弦索引嵌入，提供显式图像身份标识，支持可变数量输入的泛化和外推。

Result: 实验表明，在多样化的多图像编辑任务中，相比先前基线方法，在语义一致性、视觉保真度和跨图像整合方面均有明显改进，验证了方法在一致性和泛化能力上的优势。

Conclusion: 提出的可扩展多图像编辑框架通过显式区分图像身份的技术创新，有效解决了统一多模态模型在多图像编辑中的视觉一致性和细节区分问题，建立了高质量的训练评估基准。
...
### [27] [Orient Anything V2: Unifying Orientation and Rotation Understanding](https://arxiv.org/abs/2601.05573)
...
<details>
  <summary>Details</summary>
Motivation: 现有方向估计方法通常假设物体有单一独特的前向面，但实际物体具有多样的旋转对称性，需要更通用的方法来处理不同对称类型并直接估计相对旋转。

Method: 通过四个关键创新：1) 使用生成模型合成可扩展的3D资产；2) 模型在环的高效标注系统识别0到N个有效前向面；3) 对称感知的周期性分布拟合目标；4) 直接预测相对物体旋转的多帧架构。

Result: 在11个广泛使用的基准测试中，Orient Anything V2在方向估计、6DoF姿态估计和物体对称性识别方面实现了最先进的零样本性能，展现出强大的泛化能力。

Conclusion: 该模型显著扩展了方向估计在多样化下游任务中的适用性，为处理具有不同旋转对称性的物体提供了统一且强大的解决方案。
...
### [28] [Generalizable and Adaptive Continual Learning Framework for AI-generated Image Detection](https://arxiv.org/abs/2601.05580)
...
Result: 离线检测器比最佳基线平均精度提升5.51%；持续学习策略平均准确率达92.20%，优于现有最先进方法；在包含GANs、深度伪造和扩散模型的27个生成模型基准上验证。

Conclusion: 提出的三阶段持续学习框架能有效适应不断演化的生成模型，解决了AI生成图像检测中的泛化和适应性问题，为实际应用提供了可行的解决方案。
...
### [29] [GS-DMSR: Dynamic Sensitive Multi-scale Manifold Enhancement for Accelerated High-Quality 3D Gaussian Splatting](https://arxiv.org/abs/2601.05584)
...
<details>
  <summary>Details</summary>
Motivation: 在3D动态场景重建领域，如何平衡模型收敛速度和渲染质量一直是关键挑战，特别是在复杂动态运动场景的高精度建模中。现有方法难以同时满足快速收敛和高质量渲染的需求。

Method: 1. 提出GS-DMSR方法，通过定量分析高斯属性的动态演化过程，实现自适应梯度聚焦机制，动态识别高斯模型运动状态的显著差异，并对不同重要性的高斯模型应用差异化优化策略；2. 集成多尺度流形增强模块，利用隐式非线性解码器和显式变形场的协同优化，提升复杂变形场景的建模效率。

Result: 在合成数据集上达到96 FPS的帧率，同时有效减少了存储开销和训练时间。实验结果表明该方法在收敛速度和渲染质量之间取得了良好平衡。
...
### [30] [Quantifying and Inducing Shape Bias in CNNs via Max-Pool Dilation](https://arxiv.org/abs/2601.05599)
*Takito Sawada,Akinori Iwata,Masahiro Okuda*

Main category: cs.CV

TL;DR: 提出量化数据集形状-纹理平衡的指标，并基于此设计高效的自适应方法，通过调整最大池化膨胀率来增强CNN的形状偏向性，在形状主导数据集上提升分类精度。


<details>
  <summary>Details</summary>
Motivation: CNN固有的纹理偏向性在处理形状主导数据（如插画、草图）时性能下降。现有形状偏向模型缺乏量化指标来判断哪些数据集真正需要这种修改。

Method: 1) 提出数据驱动指标：通过计算图像亮度通道与其L0平滑版本的结构相似性指数(SSIM)来量化数据集的形状-纹理平衡；2) 基于该指标设计高效自适应方法：冻结卷积权重，仅调整最大池化操作的膨胀率来增强形状偏向性。

Result: 该方法在形状主导数据集上持续提升分类精度，特别是在数据量少的场景下效果显著，仅需训练最后的分类层，计算效率高。

Conclusion: 提出的形状-纹理平衡量化指标能有效识别需要形状偏向模型的数据集，相应的自适应方法能在保持计算效率的同时提升形状主导数据上的性能。
...
### [31] [SceneAlign: Aligning Multimodal Reasoning to Scene Graphs in Complex Visual Scenes](https://arxiv.org/abs/2601.05600)
*Chuhan Wang,Xintong Li,Jennifer Yuntong Zhang,Junda Wu,Chengkai Huang,Lina Yao,Julian McAuley,Jingbo Shang*

Main category: cs.CV

TL;DR: SceneAlign：利用场景图进行可控结构干预，通过构造视觉基础错误的负例对，提升多模态大语言模型的推理忠实度


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视觉场景中经常出现推理不忠实的问题，表现为幻觉实体、错误基础关系、跳过步骤和过度指定推理。现有基于偏好的方法依赖文本扰动或答案条件化推理，无法解决这一问题，因为模型可以利用语言先验绕过视觉基础。

Method: 提出SceneAlign框架，利用场景图作为结构化视觉信息进行可控结构干预。通过识别推理关键节点，采用四种针对性策略进行扰动（模拟典型基础失败），构造语言合理但视觉基础错误的硬负例推理。使用这些对比对进行直接偏好优化，引导模型进行细粒度、结构忠实的推理。

Result: 在七个视觉推理基准测试中，SceneAlign持续提高了答案准确性和推理忠实度，证明了基于基础的对齐方法对多模态推理的有效性。

Conclusion: SceneAlign通过场景图的结构化干预和对比学习，有效解决了多模态大语言模型的视觉基础不忠实问题，为提升复杂视觉场景中的推理质量提供了新方法。
...
### [32] [Learning Geometric Invariance for Gait Recognition](https://arxiv.org/abs/2601.05604)
*Zengbin Wang,Junjie Li,Saihui Hou,Xu Liu,Chunshui Cao,Yongzhen Huang,Muyi Sun,Siye Wang,Man Zhang*

Main category: cs.CV

TL;DR: 提出RRS-Gait框架，将不同步态条件下的变化视为几何变换的组合，通过反射、旋转、缩放三种几何变换的等变性学习实现身份不变性特征提取


<details>
  <summary>Details</summary>
Motivation: 现有步态识别方法主要通过数据驱动方式隐式学习不同步态条件下的共同特征，但较少显式探索不同步态条件间的内在关系。作者认为不同步态条件的变化可近似视为几何变换的组合

Method: 提出RRS-Gait框架，探索反射、旋转、缩放三种几何变换，通过灵活调整卷积核实现特征等变性，然后将三种等变性感知特征分别输入全局池化操作进行最终的不变性学习

Result: 在四个流行步态数据集（Gait3D、GREW、CCPG、SUSTech1K）上的大量实验显示，在各种步态条件下都表现出优越性能

Conclusion: 通过将步态变化建模为几何变换组合，并实现几何不变性，可以自然地获得身份不变性，为步态识别提供了新的视角
...
### [33] [LatentVLA: Efficient Vision-Language Models for Autonomous Driving via Latent Action Prediction](https://arxiv.org/abs/2601.05611)
*Chengen Xie,Bin Sun,Tianyu Li,Junjie Wu,Zhihui Hao,XianPeng Lang,Hongyang Li*

Main category: cs.CV

TL;DR: LatentVLA：无需语言标注的自监督潜在动作预测框架，解决VLA模型在自动驾驶中的数值不精确、语言依赖和计算效率问题，实现实时高效驾驶


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型在大规模数据集上表现良好，但在罕见长尾场景中表现不佳。现有VLA模型虽然利用视觉语言预训练知识，但存在三个关键问题：轨迹预测数值不精确、过度依赖语言标注引入语言偏见、多步推理导致计算效率低下影响实时部署。

Method: 提出LatentVLA框架，采用自监督潜在动作预测方法，无需语言标注训练VLA模型，消除语言偏见，从未标注轨迹数据中学习丰富驾驶表示。通过知识蒸馏将VLA模型的泛化能力转移到高效视觉网络中。

Result: 在NAVSIM基准测试中达到92.4的PDMS分数，创下新的SOTA；在nuScenes基准测试中表现出强大的零样本泛化能力。

Conclusion: LatentVLA通过自监督潜在动作预测解决了VLA模型的关键限制，实现了鲁棒性能和实时效率的平衡，为自动驾驶系统提供了更实用的解决方案。
...
### [34] [Compressing image encoders via latent distillation](https://arxiv.org/abs/2601.05639)
*Caroline Mazini Rodrigues,Nicolas Keriven,Thomas Maugey*

Main category: cs.CV

TL;DR: 提出一种通过简化知识蒸馏来压缩图像压缩网络编码器的方法，在保持重建质量的同时减少模型大小和计算需求
...
### [35] [SGDrive: Scene-to-Goal Hierarchical World Cognition for Autonomous Driving](https://arxiv.org/abs/2601.05640)
*Jingyu Li,Junjie Wu,Dongnan Hu,Xiangkai Huang,Bin Sun,Zhihui Hao,Xianpeng Lang,Xiatian Zhu,Li Zhang*

Main category: cs.CV

TL;DR: SGDrive：基于场景-智能体-目标层次结构，为通用视觉语言模型提供结构化时空表示，提升自动驾驶规划能力


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶方法使用通用视觉语言模型（VLMs），但这些模型缺乏对驾驶特定3D时空推理的专业理解，难以建立捕捉几何关系、场景上下文和运动模式的结构化时空表示，影响安全轨迹规划

Method: 在预训练VLM骨干上构建SGDrive框架，将驾驶理解分解为场景-智能体-目标层次结构，模拟人类驾驶认知：先感知整体环境（场景上下文），再关注安全关键智能体及其行为，最后制定短期目标并执行动作

Result: 在NAVSIM基准测试中，SGDrive在仅使用摄像头的PDMS和EPDMS指标上均达到最先进性能，验证了层次化知识结构对通用VLM适应自动驾驶的有效性

Conclusion: 通过显式构建驾驶特定知识层次结构，SGDrive成功为通用视觉语言模型提供了结构化时空表示，显著提升了自动驾驶轨迹规划能力，证明了层次化知识分解在专业领域适应中的重要性
...
### [36] [SketchVL: Policy Optimization via Fine-Grained Credit Assignment for Chart Understanding and More](https://arxiv.org/abs/2601.05688)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的多模态大语言模型在信用分配方面存在挑战，其优势估计通常在轨迹层面进行，无法区分单个生成响应中正确和错误的推理步骤。图表作为复杂数据的高密度视觉载体，需要精确复杂的视觉推理，这对现有MLLMs构成了重大挑战。

Method: SketchVL采用FinePO算法进行训练，这是一种为轨迹内细粒度信用分配设计的新RL算法。模型通过在图像上绘制中间推理步骤作为标记，并将标注后的图像反馈给自己，创建鲁棒的多步推理过程。FinePO算法利用细粒度过程奖励模型对轨迹中的每个绘图动作进行评分，从而精确分配每个步骤的信用。

Result: 实验表明，SketchVL学会了将其步骤级行为与FinePRM对齐，在图表数据集、自然图像数据集和数学任务上，相比基础模型平均性能提升了7.23%。

Conclusion: SketchVL通过细粒度信用分配机制，为训练强大的推理模型提供了一个有前景的新方向，能够更精确地奖励正确标记并惩罚错误标记，从而提升复杂视觉推理任务的性能。
...
### [37] [Rotate Your Character: Revisiting Video Diffusion Models for High-Quality 3D Character Generation](https://arxiv.org/abs/2601.05722)
...
TL;DR: RCM是一个从单图像生成高质量3D角色的先进图像到视频扩散框架，通过将复杂姿态转换到规范姿态，实现高质量的新视角合成和3D角色生成。


<details>
  <summary>Details</summary>
Motivation: 从单图像生成高质量3D角色在数字内容创作中面临重大挑战，特别是由于复杂的身体姿态和自遮挡问题。现有方法难以处理这些挑战。

Method: RCM是一个图像到视频扩散框架，具有以下特点：1) 将任意复杂姿态的角色转换到规范姿态；2) 生成1024x1024高分辨率轨道视频；3) 支持不同初始相机姿态的可控观察位置；4) 支持最多4张输入图像的多视角条件。

Result: 大量实验表明，RCM在新视角合成和3D生成质量方面均优于现有最先进方法。

Conclusion: RCM通过创新的姿态转换和高分辨率视频生成能力，有效解决了从单图像生成高质量3D角色的挑战，为数字内容创作提供了强大的工具。
...
### [38] [TAGRPO: Boosting GRPO on Image-to-Video Generation with Direct Trajectory Alignment](https://arxiv.org/abs/2601.05729)
...
TL;DR: TAGRPO是一种针对图像到视频生成模型的鲁棒后训练框架，通过对比学习和基于相同初始噪声的rollout视频来优化模型性能，显著优于之前的DanceGRPO方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示GRPO在文本到图像和文本到视频生成中有效，但直接应用于图像到视频模型时往往无法获得一致的奖励提升，需要专门针对I2V模型的优化方法。

Method: 提出TAGRPO框架，基于对比学习思想，利用相同初始噪声生成的rollout视频作为优化指导，在中间潜在空间应用新颖的GRPO损失，并引入rollout视频记忆库来增强多样性和降低计算开销。

Result: TAGRPO在图像到视频生成任务中取得了显著改进，性能优于DanceGRPO方法。

Conclusion: TAGRPO是一个简单而有效的后训练框架，能够显著提升图像到视频生成模型的性能，通过对比学习和基于相同初始噪声的优化策略解决了现有方法的局限性。
...
### [39] [FeatureSLAM: Feature-enriched 3D gaussian splatting SLAM in real time](https://arxiv.org/abs/2601.05738)
*Christopher Thirgood,Oscar Mendez,Erin Ling,Jon Storey,Simon Hadfield*

Main category: cs.CV

TL;DR: 提出实时跟踪SLAM系统，结合3D高斯泼溅进行特征丰富的地图构建，通过密集特征栅格化实现超越RGB-D输入的语义理解，支持开放集分割等新下游任务。


<details>
  <summary>Details</summary>
Motivation: 传统语义SLAM通常嵌入预定义类别标签，限制了应用范围。需要一种既能实时跟踪又能构建富含特征地图的系统，支持开放集语义理解和新下游任务。

Method: 将密集特征栅格化集成到新视角合成中，与视觉基础模型对齐。使用3D高斯泼溅进行特征丰富的地图构建，实现实时相机跟踪与逼真地图生成的统一。

Result: 在标准基准测试中实现实时跟踪，姿态误差降低9%，地图精度提高8%。跟踪稳定性与地图保真度优于现有系统，语义和语言掩码结果与离线3DGS模型相当。

Conclusion: 实时特征嵌入SLAM不仅能支持新的下游应用，还能提升底层跟踪和地图构建子系统的性能，在保持实时性的同时提供高质量的语义理解和渲染结果。
...
### [40] [ViTNT-FIQA: Training-Free Face Image Quality Assessment with Vision Transformers](https://arxiv.org/abs/2601.05741)
*Guray Ozgur,Eduarda Caldeira,Tahar Chettaoui,Jan Niklas Kolf,Marco Huber,Naser Damer,Fadi Boutros*

Main category: cs.CV

TL;DR: 提出ViTNT-FIQA，一种无需训练的面部图像质量评估方法，通过分析Vision Transformer中间块中patch嵌入的演化稳定性来评估图像质量，仅需单次前向传播且无需反向传播或模型修改。


<details>
  <summary>Details</summary>
Motivation: 当前FIQA方法主要利用最终层表示，而无训练方法需要多次前向传播或反向传播。需要一种更高效、无需训练且能利用中间层信息的方法来评估面部图像质量。

Method: 通过测量Vision Transformer中间块之间patch嵌入演化的稳定性来评估图像质量。计算连续transformer块之间L2归一化patch嵌入的欧氏距离，并将其聚合为图像级质量分数。高质量图像在块间表现出稳定的特征细化轨迹，而退化图像则显示不稳定的变换。

Result: 在八个基准测试（LFW、AgeDB-30、CFP-FP、CALFW、Adience、CPLFW、XQLFW、IJB-C）上，ViTNT-FIQA实现了与最先进方法竞争的性能，同时保持计算效率，并能立即应用于任何预训练的ViT面部识别模型。

Conclusion: ViTNT-FIQA提供了一种高效、无需训练的面部图像质量评估方法，通过分析ViT中间块的特征演化稳定性，仅需单次前向传播即可获得准确的图像质量评估，具有实际应用价值。
...
### [41] [FlyPose: Towards Robust Human Pose Estimation From Aerial Views](https://arxiv.org/abs/2601.05747)
*Hassaan Farooq,Marvin Brenner,Peter St\ütz*

Main category: cs.CV

TL;DR: FlyPose是一个轻量级无人机视角人体姿态估计系统，通过多数据集训练提升检测精度，在边缘设备上实现实时推理，并发布了新的无人机视角数据集。


<details>
  <summary>Details</summary>
Motivation: 无人机在人群密集环境中的应用日益增多，需要从空中视角准确感知人体姿态和动作。现有方法面临低分辨率、陡峭视角和遮挡等挑战，特别是需要实时可行模型的应用场景。

Method: 训练和部署FlyPose，一个轻量级的自上而下人体姿态估计流程，通过多数据集训练提升性能。采用top-down架构，在Jetson Orin AGX开发套件上实现实时推理。

Result: 在多个测试集上平均提升6.8 mAP的人体检测精度，在UAV-Human数据集上提升16.3 mAP的2D人体姿态估计精度。推理延迟约20毫秒，包括预处理，并成功在四旋翼无人机上部署。

Conclusion: FlyPose系统有效解决了无人机视角下人体姿态估计的挑战，实现了实时性能，并发布了FlyPose-104数据集，为无人机视角人体姿态估计研究提供了有价值的资源。
...
### [42] [Adaptive Disentangled Representation Learning for Incomplete Multi-View Multi-Label Classification](https://arxiv.org/abs/2601.05785)
*Quanjiang Li,Zhiming Liu,Tianxiang Xu,Tingjin Luo,Chenping Hou*

Main category: cs.CV

TL;DR: ADRL是一种自适应解耦表示学习方法，通过跨模态特征传播、随机掩码重建、标签原型学习、互信息优化和伪标签引导的视图融合，解决多视图多标签学习中的特征缺失和标注不完整问题。
...
Method: ADRL采用双通道网络，通过邻域感知的跨模态特征传播实现鲁棒视图补全，利用随机掩码策略增强重建效果，通过标签分布传播学习相互依赖的标签原型，使用互信息目标优化共享和特定表示，并进行原型特定的特征选择和伪标签生成。

Result: 在公开数据集和实际应用中，ADRL表现出优越的性能，证明了其在处理多视图多标签学习问题上的有效性。

Conclusion: ADRL通过自适应解耦表示学习，有效解决了多视图多标签学习中的特征缺失和标注不完整问题，为这一复杂而实用的挑战提供了有效的解决方案。
...
TL;DR: SceneFoundry是一个语言引导的扩散框架，用于生成公寓规模的3D世界，包含功能性的铰接式家具和语义多样的布局，用于机器人训练。


<details>
  <summary>Details</summary>
Motivation: 现有生成方法往往无法捕捉真实室内环境的功能复杂性，特别是包含对机器人物体操作和导航至关重要的可移动部件的铰接式物体。需要能够自动生成大规模、交互式、物理真实的3D环境来推进机器人学习和具身智能。

Method: 使用语言引导的扩散框架：1）LLM模块控制楼层布局生成；2）基于扩散的后验采样从大规模3D资源库中高效填充场景中的铰接式资产；3）采用可微分指导函数确保物理可用性，包括调节物体数量、防止铰接碰撞、保持足够的机器人可通行空间。

Result: 实验表明，该框架能够生成结构有效、语义连贯、功能交互的环境，适用于多种场景类型和条件，实现了可扩展的具身AI研究。

Conclusion: SceneFoundry能够生成公寓规模的3D世界，包含功能性的铰接式家具和语义多样的布局，为机器人训练提供了可扩展的具身AI研究环境。
...
TL;DR: 提出Send-VAE，一种语义解耦的VAE，通过将潜在空间与预训练视觉基础模型的语义层次对齐，实现属性级解耦，显著提升图像生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用相同的对齐目标来优化VAE和LDM，忽略了它们根本不同的表示需求。LDM需要保留高级语义概念的潜在空间，而VAE应该擅长语义解耦，能够以结构化方式编码属性级信息。

Method: 提出语义解耦VAE（Send-VAE），通过非线性映射网络将VAE潜在空间与预训练视觉基础模型的语义层次对齐，桥接属性级解耦和高级语义之间的差距，有效指导VAE学习。

Result: 通过属性预测任务的线性探测评估语义解耦，显示与改进的生成性能强相关。使用Send-VAE训练基于流的transformer SiTs，显著加速训练，在ImageNet 256x256上达到SOTA的FID 1.21（有分类器自由引导）和1.75（无引导）。

Conclusion: Send-VAE通过显式优化语义解耦表示学习，为VAE提供了更合适的对齐目标，显著提升了图像生成模型的训练效率和最终性能。
...
TL;DR: GeoSurDepth：利用几何一致性作为主要线索的环视深度估计框架，通过基础模型提供几何先验，结合空间3D表面法线一致性和2D对象纹理一致性，实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有环视深度估计方法主要关注光度级别的跨视图约束，很少显式利用单目和环视设置中固有的丰富几何结构。需要更充分利用几何一致性来提升深度估计的鲁棒性。

Method: 1) 利用基础模型作为伪几何先验和特征表示增强工具，指导网络保持空间3D表面法线一致性并正则化2D对象纹理一致性深度估计；2) 引入新颖的视图合成流程，通过空间扭曲重建密集深度实现2D-3D提升，在时间、空间和时空上下文中提供额外光度监督；3) 提出自适应联合运动学习策略，自适应强调信息丰富的空间几何线索以改进运动推理。
...
### [46] [Goal Force: Teaching Video Models To Accomplish Physics-Conditioned Goals](https://arxiv.org/abs/2601.05848)
...
Method: 提出Goal Force框架，允许用户通过明确的力向量和中间动态定义目标。在合成的因果原语数据集（如弹性碰撞、多米诺骨牌倒下）上训练视频生成模型，教会模型在时空上传播力。

Result: 尽管在简单物理数据上训练，模型展现出显著的零样本泛化能力，能够处理复杂的现实场景，包括工具操作和多对象因果链。模型成为隐式神经物理模拟器。

Conclusion: 通过将视频生成基于基本物理交互，模型能够作为隐式神经物理模拟器，实现精确的物理感知规划，无需依赖外部引擎。这种方法为机器人学和规划提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统切片式方法的局限性，探索在腹部CT上使用弱监督进行3D异常检测的可行性，减少标注需求

Method: 结合DDPM、DDIM和VQ-GAN的潜在扩散管道，直接在3D图像体积上操作，仅使用病例级伪标签进行弱监督训练
...
TL;DR: 提出BCSI框架用于半监督医学图像分割，通过语义空间扰动、通道选择路由器和双向通道交互策略，解决现有方法错误累积、结构复杂和忽略数据流交互的问题。


<details>
  <summary>Details</summary>
Motivation: 现有半监督医学图像分割方法（如mean teacher和双流一致性学习）存在错误累积、模型结构复杂以及忽略标记与未标记数据流之间交互的问题，需要更有效的解决方案。

Method: 1. 语义空间扰动(SSP)机制：使用两种强增强操作扰动数据，利用弱增强的伪标签进行无监督学习，并通过强增强预测的一致性提高稳定性。2. 通道选择路由器(CR)：动态选择最相关通道进行信息交换，减少噪声干扰。3. 双向通道交互(BCI)策略：补充额外语义信息，增强重要通道表示。
...
TL;DR: Phase4DFD是一个相位感知的频域深度伪造检测框架，通过可学习的注意力机制显式建模相位-幅度交互，在CIFAKE和DFFD数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法主要依赖频谱幅度，对相位信息的探索不足。相位不连续性通常由合成生成引入，可以提供幅度无法提供的互补信息。

Method: 1) 将标准RGB输入与FFT幅度和LBP表示结合；2) 引入输入级相位感知注意力模块，利用相位不连续性引导模型关注最具有操作指示性的频率模式；3) 使用高效的BNext M主干网络处理，可选通道空间注意力进行语义特征细化。

Result: 在CIFAKE和DFFD数据集上的实验表明，Phase4DFD优于现有的空间和基于频率的检测器，同时保持较低的计算开销。消融研究证实显式相位建模提供了幅度无法提供的互补信息。

Conclusion: 显式相位建模为深度伪造检测提供了幅度无法提供的互补信息，相位感知注意力机制能有效引导模型关注最具有操作指示性的频率模式，实现更准确的检测。
...
TL;DR: 提出Relay Tokens方法，在Vision Transformer中并行处理局部高分辨率和全局低分辨率特征，通过可学习的中继令牌聚合传播特征，解决超高分辨率图像分割中局部细节与全局上下文难以兼顾的问题。


<details>
  <summary>Details</summary>
Motivation: 当前超高分辨率图像分割方法存在两难：滑动窗口会丢弃全局上下文，下采样会丢失细节。需要同时保留局部细节和全局感知能力。

Method: 并行处理局部尺度（高分辨率小裁剪）和全局尺度（低分辨率大裁剪），使用少量可学习的relay tokens在两个分支间聚合和传播特征。可直接集成到标准Transformer骨干网络（如ViT、Swin），仅增加不到2%的参数。

Result: 在三个超高分辨率分割基准（Archaeoscape、URUR、Gleason）和传统Cityscapes数据集上均取得一致提升，相对mIoU最高提升15%。

Conclusion: Relay Tokens方法简单有效，为Vision Transformer带来显式的多尺度推理能力，同时保留局部细节和全局感知，在多个分割任务上表现优异。
...
TL;DR: 基于Vision Transformer的深度学习分割模型在胰腺肿瘤EUS图像分割中表现良好，但存在数据集异质性和部分错误预测问题。


<details>
  <summary>Details</summary>
Motivation: 胰腺癌预后极差，内镜超声(EUS)是重要诊断工具但受操作者主观性限制，需要开发自动化分割模型提高诊断客观性。

Method: 使用Vision Transformer骨干网络的USFM框架分割模型，在17,367张EUS图像上进行5折交叉验证，并在独立数据集(350张图像)上进行外部验证，采用DSC、IoU、敏感性、特异性等指标评估。

Result: 5折交叉验证平均DSC 0.651±0.738，IoU 0.579±0.658，敏感性69.8%，特异性98.8%；外部验证DSC 0.657，IoU 0.614，敏感性71.8%，特异性97.7%，但9.7%病例出现错误多重预测。

Conclusion: Vision Transformer模型在胰腺肿瘤EUS分割中表现良好，但数据集异质性、有限的外部验证和错误预测问题表明需要进一步优化、标准化和前瞻性研究。
...
TL;DR: 论文提出一种无需训练的方法CEI，通过分析LVLM生成过程中的层间动态，发现真实token比幻觉token更早累积概率质量，并利用上下文嵌入作为接地信号来减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在开放任务（如图像描述、视觉推理）中产生的幻觉（与视觉输入不一致的响应）是一个关键限制，需要有效的缓解策略。

Method: 使用Logit Lens分析LVLM在解码器层间构建下一个token分布的动态，发现"承诺深度差距"现象；提出上下文嵌入注入（CEI）方法，利用最后一个输入token的隐藏状态作为接地信号，在解码过程中保持视觉保真度。

Result: 在CHAIR、AMBER和MMHal-Bench基准测试中（最大token长度为512），CEI在三个LVLM上优于现有最佳基线方法，其动态变体实现了最低的整体幻觉率。

Conclusion: 通过结合新的机制洞察和可扩展的干预措施，这项工作推进了LVLM中幻觉的缓解，为训练免费的幻觉缓解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视网膜血管分割在自动化眼科诊断中至关重要，但面临光照不均匀和对比度变化导致的域偏移问题。现有SAM方法依赖简单适配器微调，忽略了编码域不变特征的频域信息，且在直接上采样时会丢失细血管细节。

Method: 1. 频谱引导域调制器：结合小波分解和可学习域标记，分离光照鲁棒的低频结构和高频血管边界，促进域特定特征生成。2. 频率自适应域融合：通过小波频率相似性和软加权融合实现智能测试时域选择。3. 分层掩码提示细化器：通过长程依赖建模的粗到细细化克服SAM上采样限制。

Result: 在四个公共视网膜数据集上采用Leave-One-Domain-Out协议进行广泛实验，WaveRNet实现了最先进的泛化性能。

Conclusion: WaveRNet通过小波引导的频率学习框架有效解决了视网膜血管分割中的域泛化挑战，特别是在光照和对比度变化下保持了细血管结构的完整性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成领域由扩散模型和流匹配模型主导，虽然能产生高质量结果，但计算密集且难以扩展。需要开发更高效、可扩展的视频生成方法，缩小自回归模型与扩散模型之间的性能差距。

Method: 1. 结合多尺度下一帧预测与自回归建模；2. 通过3D多尺度分词器高效编码时空动态；3. 提出多尺度时序RoPE、跨帧错误校正和随机帧掩码来改善长期一致性；4. 采用多阶段预训练流程逐步对齐空间和时间学习。
...
<details>
  <summary>Details</summary>
Motivation: 多对比度图像配准面临不同成像对比度间复杂非线性强度关系的挑战。传统方法耗时，而现有学习方法的泛化性局限于训练所见对比度，需要能泛化到任意成像对比度的配准框架。

Method: 提出自适应条件对比度无关可变形图像配准框架(AC-CAR)：1) 基于随机卷积的对比度增强方案；2) 自适应条件特征调制器(ACFM)自适应调制特征；3) 对比度不变潜在正则化确保特征一致性；4) 集成方差网络提供对比度无关配准不确定性估计。

Result: 实验结果表明AC-CAR在配准精度上优于基线方法，并在未见成像对比度上表现出优越的泛化能力。代码已开源。
...
TL;DR: 该研究将DUMB/DUMBer方法应用于深度伪造检测，评估了在迁移攻击和跨数据集配置下检测器的对抗鲁棒性，发现对抗训练在分布内情况下能增强鲁棒性，但在跨数据集配置下可能降低鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度伪造检测系统面临对抗攻击威胁，现有对抗训练防御方法在攻击者知识有限和数据分布不匹配的真实场景下的有效性尚未充分探索。

Method: 采用DUMB/DUMBer方法论，评估5个SOTA检测器(RECCE、SRM、XCeption、UCF、SPSL)在3种攻击(PGD、FGSM、FPBA)和2个数据集(FaceForensics++、Celeb-DF-V2)下的鲁棒性，分析迁移攻击和跨数据集配置。

Result: 对抗训练策略在分布内情况下能增强鲁棒性，但在跨数据集配置下可能降低鲁棒性，具体效果取决于采用的策略。从攻击者和防御者角度分析不匹配场景。

Conclusion: 现实世界应用中需要针对具体情况的防御策略，对抗训练的效果高度依赖于数据分布匹配程度，不能一概而论。
...
<details>
  <summary>Details</summary>
Motivation: 支付网络产生大量交易数据，现有基础模型使用索引表示分类商户字段导致语义信息丢失，而LLM虽能解决但计算开销大，难以实时部署

Method: 混合框架：使用LLM生成的嵌入作为轻量级交易模型的语义初始化；多源数据融合丰富商户分类字段；单词约束原则确保跨LLM架构的嵌入一致性；系统处理数据质量（噪声过滤和上下文感知增强）

Result: 在大规模交易数据集上的实验显示，在多个交易理解任务上取得显著性能提升

Conclusion: 提出的混合框架有效平衡了语义理解与计算效率，为实时金融部署提供了可行方案
...
TL;DR: 该研究从关注媒体"左/右翼"立场转向分析具体语言操作，提出了包含38种基本偏见类型的句子级媒体偏见分类法，并展示了其在覆盖范围和减少歧义方面的优势。


<details>
  <summary>Details</summary>
Motivation: 传统关于"左翼"或"右翼"新闻的公共辩论忽略了偏见通常通过超越单一政治光谱的具体语言操作来传达。研究旨在从关注媒体立场转向分析个别句子中如何表达偏见。

Method: 收集26,464个句子（来自新闻语料库、用户提交和自行浏览），通过细读、跨学科理论和试点标注的迭代结合，开发精细的句子级媒体偏见和宣传分类法。

Result: 创建了两层分类模式，包含38种基本偏见类型，分为六个功能家族，可视化为"媒体偏见元素表"。对155个句子的随机样本进行定量调查显示流行度差异，与NLP和传播科学分类法的对比显示覆盖范围显著增加且歧义减少。

Conclusion: 该研究提供了一个精细的句子级媒体偏见分类法，超越了传统的左/右翼二分法，为媒体偏见分析提供了更准确、更全面的工具，在覆盖范围和减少歧义方面优于现有分类法。
...
TL;DR: 本文介绍了MLCL诊断基准，系统评估了多语言工具调用性能，发现参数值语言不匹配是主要失败模式，现有策略无法完全恢复英语水平性能。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在英语工具调用评估中表现良好，但在多语言用户交互下的鲁棒性尚未充分探索。需要评估LLM作为代理在多语言环境中的工具调用能力。

Method: 引入MLCL诊断基准，系统评估中文、印地语和低资源语言伊博语的多语言工具调用。通过细粒度错误分析识别失败模式，并评估多种推理时系统策略。

Result: 研究发现许多失败发生在正确理解意图和选择工具的情况下，参数值语言不匹配是主导失败模式。现有策略能显著减少语言引起的执行错误，但无法完全恢复英语水平性能。

Conclusion: 多语言工具调用面临语言不匹配挑战，现有方法不足以解决这一问题，需要进一步研究来提升LLM在多语言环境中的工具调用鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注直接提问或简化场景下的LLM偏见，缺乏对复杂真实金融环境和高风险、上下文敏感的多语言金融虚假信息检测任务的考虑。

Method: 与金融专家合作构建三种复杂金融场景：基于角色和人格、基于角色和地区、以及包含种族和宗教信仰的角色场景。开发涵盖英语、中文、希腊语和孟加拉语的多语言金融虚假信息数据集，并系统评估22个主流LLM。

Result: 研究发现，商业和开源模型中都存在显著的行为偏见，表明LLM在复杂金融决策中可能继承和放大人类偏见。

Conclusion: 需要更全面的基准来评估LLM在复杂金融环境中的行为偏见，MFMDScen为此提供了系统评估框架，有助于提高金融决策的稳定性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 行政和官僚文本通常难以理解，需要一种客观的方法来评估和改进其可读性。信息熵作为信息论中的概念，可以量化文本的信息密度和复杂度，从而为可读性评估提供理论基础。

Method: 提出一个可视化框架，使用多个语言模型来近似计算文本的信息熵，并将结果可视化。该方法通过分析文本的信息熵来估计其可读性。

Result: 开发了名为Glitter的开源工具集，可在GitHub上获取，实现了基于信息熵的文本可读性评估和可视化功能。

Conclusion: 信息熵可以作为评估文本可读性的有效指标，所提出的可视化框架和工具能够帮助分析和改进行政/官僚文本的清晰度和可读性。
...
TL;DR: 对11个前沿大语言模型进行大规模统计审计，发现它们在概率采样方面存在严重缺陷：批量生成仅有13%的中位通过率，独立请求几乎完全失效，采样保真度随分布复杂度和采样数量增加而下降。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型从聊天接口转变为教育评估和合成数据构建等领域的随机管道核心组件，从指定概率分布中忠实采样的能力已成为功能需求而非理论好奇。需要评估当前LLMs是否具备可靠的内部采样器。

Method: 采用双协议设计：批量生成（模型在单个响应中产生N=1000个样本）和独立请求（N=1000个无状态调用）。对11个前沿LLM在15种分布上进行大规模统计审计，分析采样保真度与分布复杂度和采样数量的关系。

Result: 发现明显的协议不对称性：批量生成仅达到中等统计有效性（13%中位通过率），而独立请求几乎完全失效（11个模型中有10个在所有分布上都失败）。采样保真度随分布复杂度和采样数量增加而单调下降，这些失败会传播到下游任务中。

Conclusion: 当前大语言模型缺乏功能性内部采样器，对于需要统计保证的应用，必须使用外部工具。模型的采样缺陷会严重影响需要概率约束的下游任务。
...
TL;DR: 研究发现LLMs通过语言统计规律自发形成与人类道德判断对齐的分布式、分层且部分解耦的道德概念结构，而非简单的道德模仿。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs产生类似人类道德判断时，是真正具有内在概念结构还是仅仅是表面上的"道德模仿"，以理解道德概念在模型中的编码机制。

Method: 采用多层次分析方法：1) 分层分析MFT概念表征与人类道德感知的对齐；2) 使用预训练稀疏自编码器识别支持道德概念的稀疏特征；3) 使用密集MFT向量和稀疏SAE特征进行因果导向干预。

Result: 两个模型都以结构化、分层依赖的方式表示和区分道德基础，与人类判断对齐；SAE特征显示与特定道德基础的语义联系；导向干预能预测性地改变道德相关行为。

Conclusion: LLMs中的道德概念是分布式、分层且部分解耦的，表明多元道德结构可以从语言统计规律中作为潜在模式自发涌现，而非简单模仿。
...
TL;DR: RL单独应用对韩语推理能力提升有限，需要通过微调对齐韩语输入与模型内部推理过程，特别是调整早期层的韩语特定神经元，才能有效提升韩语推理能力。


<details>
  <summary>Details</summary>
Motivation: LLMs在英语等高资源语言中表现出强大的推理和自校正能力，但在韩语等低资源语言中性能有限。本研究旨在探索强化学习是否能将韩语推理能力提升到与英语相当的水平。

Method: 1. 尝试单独应用强化学习；2. 探索多种微调策略；3. 通过调整早期层的韩语特定神经元来对齐模型内部推理过程与韩语输入；4. 引入自校正代码转换数据集来促进对齐。

Result: 研究发现：1. RL单独应用于缺乏固有韩语推理能力的模型时改进有限；2. 通过微调对齐韩语输入与内部推理过程（特别是调整早期层韩语特定神经元）能显著提升性能；3. 在数学推理和自校正任务中都观察到显著性能提升。
...
TL;DR: 本文提出了一个基于大语言模型的教育场景学生模拟的概念与方法框架，核心解决"能力悖论"问题，强调通过明确的认知状态规范来约束生成，而非追求表面真实。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的学生模拟存在"能力悖论"问题：能力强大的LLM被要求模拟知识不完整的学习者，导致不现实的错误模式和学习动态。需要建立更可靠的模拟框架来支持科学研究和教学应用。

Method: 1) 将学生模拟重构为受约束的生成问题，引入明确的认知状态规范来定义模拟学习者能访问什么、错误如何结构化、学习状态如何随时间演变；2) 提出目标-环境框架，根据行为目标和部署情境定位模拟学生系统；3) 综合现有文献，形式化关键设计维度。

Result: 提出了一个系统的概念框架和方法论，但没有提出新的系统或基准。形式化了学生模拟的关键设计维度，并阐明了与有效性、评估和伦理风险相关的开放挑战。

Conclusion: 基于大语言模型的学生模拟应将认知保真度置于表面真实之上，这是将LLM模拟学生作为可靠科学和教学工具的前提条件。需要关注有效性验证、评估方法和伦理风险。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs能够抵抗明显的错误信息，但在面对经过精心设计、难以证伪的误导性证据时存在根本性脆弱性。为了可靠地辅助人类决策，LLMs需要维持事实性内部信念，抵抗这种高级别的误导注入。

Method: 提出MisBelief框架，通过多角色LLMs之间的协作、多轮交互生成误导性证据，模拟微妙的、可辩驳的推理和渐进式优化过程。使用该框架生成了4,800个实例，分为三个难度级别，评估了7个代表性LLMs。为防御此类攻击，提出了Deceptive Intent Shielding（DIS）治理机制，通过推断证据背后的欺骗意图提供早期预警。

Result: 实验结果显示，虽然模型对直接错误信息具有鲁棒性，但对MisBelief生成的精细证据高度敏感：对虚假信息的信念评分平均增加了93.0%，从根本上损害了下游推荐质量。DIS机制能够持续减轻信念偏移，促进更谨慎的证据评估。

Conclusion: LLMs在面对精心设计的误导性证据时存在严重脆弱性，需要新的防御机制。Deceptive Intent Shielding提供了一种有效的治理方法，能够检测欺骗意图并增强模型对误导性证据的抵抗力。
...
<details>
  <summary>Details</summary>
Motivation: 长期对话中保持一致性是LLMs的根本挑战，标准检索机制难以捕捉历史状态的时序演变，现有记忆增强框架要么依赖闭源模型的静态提示，要么训练范式低效且奖励稀疏。

Method: 引入MemBuilder强化学习框架：1) 使用合成会话级问题生成提供密集中间奖励；2) 引入贡献感知梯度加权，根据各组件下游影响缩放策略更新，实现多维记忆构建的协调训练。

Result: MemBuilder使4B参数模型在长期对话基准测试中超越最先进的闭源基线，展现出强大的泛化能力。

Conclusion: MemBuilder通过密集奖励和贡献感知训练有效解决了长期对话记忆构建的挑战，为开源模型在复杂对话任务中提供了有前景的解决方案。
...
TL;DR: FlashMem通过计算重用从推理状态中蒸馏内在记忆，利用共享KV合并器直接关注冻结缓存来合成记忆，无需额外参数化，实现5倍推理加速同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的无状态架构缺乏动态上下文保持机制，导致智能体需要冗余地重新处理历史来维持长期自主性。现有潜在记忆方法存在架构隔离问题，依赖与推理主干解耦的辅助编码器。

Method: FlashMem框架通过计算重用从瞬态推理状态中蒸馏内在记忆：1）利用内部表示唯一编码输入轨迹的特性，将最后隐藏状态作为交互历史的充分统计量；2）共享KV合并器直接关注主干冻结缓存来合成记忆，避免冗余参数化；3）参数自由的认知监视器利用注意力熵自适应触发合并，仅在检测到高认知不确定性时执行。

Result: 实验表明FlashMem在匹配重型基线性能的同时，将推理延迟降低了5倍，有效弥合了效率与持久认知之间的差距。

Conclusion: FlashMem通过计算重用直接从推理状态中蒸馏内在记忆，避免了架构隔离和冗余参数化，实现了高效且持久的认知能力，为LLM的长期自主性提供了实用解决方案。
...
TL;DR: CHisAgent是一个多智能体LLM框架，用于构建中国古代历史事件分类法，通过分解为归纳、扩展和丰富三个阶段，利用《二十四史》构建大规模领域感知分类法。
...
Method: 提出CHisAgent多智能体框架，将分类法构建分解为三个阶段：自下而上的归纳器从原始历史语料推导初始层次，自上而下的扩展器利用LLM世界知识引入缺失的中间概念，证据引导的丰富器整合外部结构化历史资源以确保忠实性。

Result: 使用《二十四史》构建了覆盖中国古代政治、军事、外交和社会生活的大规模领域感知事件分类法。无参考和有参考评估显示结构连贯性和覆盖范围得到改善，进一步分析表明所得分类法支持跨文化对齐。

Conclusion: CHisAgent框架能有效构建高质量的中国古代历史分类法，解决LLM在历史推理方面的局限性，并为跨文化知识组织提供支持。
...
TL;DR: Double Retrieval Speculative Parallelism (Double) 是一种无需训练、无损的并行推测解码方法，通过双重检索机制突破传统推测解码的速度上限，解决早期错误导致的拒绝问题，在LLaMA3.3-70B上实现5.3倍加速。


<details>
  <summary>Details</summary>
Motivation: 传统并行推测解码(PSD)存在两个根本挑战：1) 由草稿模型与目标模型速度比决定的理论加速上限；2) 早期错误导致的中途令牌拒绝造成的高计算浪费和流水线停顿。

Method: 提出Double框架，通过双重检索机制：1) 草稿模型执行迭代检索推测以突破理论速度上限；2) 目标模型执行权威检索以生成多令牌指导，无需回滚即可减轻拒绝问题。

Result: 在LLaMA3.3-70B上实现5.3倍加速，在Qwen3-32B上实现2.8倍加速，显著优于需要大量模型训练的先进方法EAGLE-3。

Conclusion: Double通过解决检索的精度-效率困境，完全无需训练且无损，实现了最先进的加速性能，突破了传统推测解码的理论限制。
...
TL;DR: TARS：通过强化学习框架对齐语音和文本模态的推理轨迹，缩小语音大语言模型的模态推理差距
...
Method: 提出TARS强化学习框架，通过非对称奖励设计对齐文本条件和语音条件的推理轨迹。使用两种密集互补信号：1) 表征对齐：测量语音和文本条件轨迹的层间隐藏状态相似性；2) 行为对齐：评估生成输出与参考文本补全的语义一致性。

Result: 在MMSU和OBQA等具有挑战性的推理基准测试中，该方法显著缩小了模态推理差距，并在7B规模的语音LLM中实现了最先进的性能。

Conclusion: TARS框架通过强化学习对齐语音和文本模态的推理轨迹，有效解决了语音大语言模型的模态推理差距问题，提升了语音输入的推理能力。
...
TL;DR: 该研究揭示了现有自动作文评分系统和大型语言模型在识别和评分有害内容方面的缺陷，提出了有害作文检测基准，发现当前模型无法有效区分有害与论证性作文，且忽视伦理维度。


<details>
  <summary>Details</summary>
Motivation: 当前自动作文评分系统和大型语言模型在评分时经常忽略作文中的伦理和道德问题，错误地给传播有害观点的作文高分，存在明显的伦理盲区。

Method: 引入有害作文检测基准，包含涉及种族主义、性别偏见等敏感话题的作文，用于测试不同大型语言模型识别和评分有害内容的能力。

Result: 研究发现：1）大型语言模型需要进一步提升才能准确区分有害作文与论证性作文；2）现有自动作文评分系统和大型语言模型在评分时都未考虑内容的伦理维度。

Conclusion: 需要开发更强大的自动作文评分系统，使其能够敏感地识别所评分内容的伦理影响，确保评分过程不仅关注写作质量，也考虑内容的伦理安全性。
...
TL;DR: 提出KEEM数据集，通过动态生成整合记忆来增强长时会话系统的记忆更新，同时保留关键事实信息和情感上下文


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖简单的累积或基于操作的方法，容易导致信息冲突，难以准确跟踪用户当前状态，需要更有效的记忆更新机制

Method: 引入KEEM数据集，采用动态生成整合记忆的方法，同时保留关键事实信息并融入情感上下文和因果关系

Result: KEEM数据集能够无缝更新系统记忆，包含情感和关键数据，促进更深层次的共情，增强系统在开放域对话中的响应能力

Conclusion: KEEM数据集为长时会话系统提供了更有效的记忆更新方法，通过整合情感和关键信息，实现更细致入微的用户理解
...
TL;DR: ReasonAny：通过对比梯度识别解决推理与领域能力融合时的性能崩溃问题，实现"推理+X"的无训练模型合并


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在长链思维推理方面取得显著成功，但为领域专用模型赋予这种推理能力（"推理+X"）仍面临挑战。现有模型合并方法往往导致性能崩溃：既削弱推理深度又损害领域特定效用。研究发现推理能力主要存在于梯度敏感度低的参数区域，这与通常假设领域能力对应高幅度参数相矛盾。

Method: 提出ReasonAny框架，通过对比梯度识别解决推理-领域性能崩溃问题。该方法基于发现推理能力主要存在于低梯度敏感度参数区域的洞察，设计了一种创新的模型合并方法。

Result: 在安全、生物医学和金融领域的实验中，ReasonAny有效合成了"推理+X"能力，显著优于最先进的基线方法，同时保持了稳健的推理性能。

Conclusion: ReasonAny通过识别推理能力存在于低梯度敏感度参数区域的洞察，成功解决了模型合并中的推理-领域性能崩溃问题，为无训练实现"推理+X"能力提供了有效解决方案。
...
TL;DR: 评估大型语言模型在自动化解读群聊决策过程方面的潜力，使用日本聚餐活动作为案例研究，发现LLM能可靠提取显性因素但难以识别隐性因素


<details>
  <summary>Details</summary>
Motivation: 传统旅行调查难以观察群体活动的复杂决策过程，非结构化聊天数据为此提供了新视角，但人工标注耗时耗力，需要探索LLM自动化标注的可行性

Method: 设计基于知识获取过程的提示框架，顺序提取关键决策因素（群体餐厅选择集、结果、个体偏好、偏好驱动属性），将非结构化对话转换为结构化表格数据

Result: LLM能可靠捕捉显性决策因素，但在识别人类标注者容易发现的细微隐性因素方面存在困难，需要明确LLM提取可信与需要人工监督的具体情境

Conclusion: LLM在整合社会活动非传统数据源方面具有潜力，但存在局限性，需要结合人类监督来识别隐性因素，为LLM在社会科学研究中的应用提供指导
...
TL;DR: 提出自适应上下文重构框架(ACR)，通过动态监控和重构对话历史来解决多轮对话中的上下文惯性和状态漂移问题，显著提升性能并减少token消耗。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多轮对话中仍存在难以保持与先前对话对齐、跨多轮依赖跟踪困难、对话增长时事实漂移等问题。现有方法主要关注扩展上下文窗口、引入外部记忆或应用上下文压缩，但仍面临上下文惯性和状态漂移的限制。

Method: 提出自适应上下文重构框架(ACR)，包含上下文重构算子库和教师引导的自进化训练范式。该框架动态监控对话历史并主动重构，将上下文管理与推理过程解耦，学习何时干预以及如何重构。

Result: 在多轮对话的广泛实验中，该方法显著优于现有基线方法，同时减少了token消耗。

Conclusion: ACR框架通过动态监控和重构对话历史，有效解决了多轮对话中的上下文惯性和状态漂移问题，在提升性能的同时优化了计算资源使用。
...
<details>
  <summary>Details</summary>
Motivation: 法律领域信息抽取任务需要大量标注数据，人工标注成本高且耗时，需要一种高效的数据增强方法来减少人工标注工作量

Method: 提出基于大语言模型（LLMs）的数据增强流水线，利用LLM生成训练数据，方法简单有效且可泛化

Result: 显著减少信息抽取任务所需的人工标注工作量，同时增强信息抽取系统的鲁棒性

Conclusion: 基于LLM的数据增强方法在法律领域信息抽取中简单有效，可泛化到其他NLP任务，具有广泛应用前景
...
TL;DR: 该研究针对非洲语言缺乏有效在线安全工具的问题，开发了适用于isiXhosa和Yorùbá两种低资源非洲语言的自动文本去毒化系统，采用混合方法（TF-IDF+逻辑回归检测+规则重写），在保持非毒性句子100%不变的同时成功去毒所有检测到的毒性句子。


<details>
  <summary>Details</summary>
Motivation: 有毒语言是在线安全参与的主要障碍，但针对非洲语言的强大缓解工具稀缺。本研究旨在填补这一关键空白，为两种低资源非洲语言（isiXhosa和Yorùbá）开发自动文本去毒化系统。

Method: 采用混合方法：1）轻量级、可解释的TF-IDF和逻辑回归模型进行毒性检测；2）基于受控词典和令牌引导的重写组件。构建了包含习语用法、变音符号和语码转换的平行语料库用于训练和评估。

Result: 检测组件在分层K折交叉验证中达到61-72%（isiXhosa）和72-86%（Yorùbá）的准确率，ROC-AUC最高达0.88。重写组件成功去毒所有检测到的毒性句子，同时100%保留非毒性句子。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在数学和代码生成等正式学习任务上表现出色，但在"实践智慧"和泛化智能（如战略创造力和社交推理）方面仍存在不足，这源于缺乏基于交互反馈而非目标导向的非正式学习。

Method: 提出将游戏作为LLM非正式学习环境，利用其内在奖励信号和抽象复杂性。为解决多任务学习中的性能退化问题，引入嵌套训练框架，通过顺序任务组合强制执行明确的"AND"目标，要求模型同时掌握多种能力以获得最大奖励。使用基于GRPO的强化学习在矩阵游戏、井字棋和"谁是卧底"等游戏中实施。

Result: 集成基于游戏的非正式学习不仅能防止任务干扰，还能显著增强模型在广泛能力导向基准测试中的泛化能力。

Conclusion: 游戏作为非正式学习环境结合嵌套训练框架，能有效提升LLM的实践智慧和泛化智能，填补当前LLM与人类认知之间的差距。
...
TL;DR: 该论文研究多语言大语言模型的遗忘学习，在10种语言上评估数据和概念遗忘效果，发现高资源语言遗忘更稳定，句法相似性是跨语言遗忘行为的最佳预测指标。
...
Method: 使用Aya-Expanse 8B模型研究多语言遗忘，分为两种设置：(1) 数据遗忘和(2) 概念遗忘。通过翻译将事实知识和刻板印象基准扩展到10种语言：英语、法语、阿拉伯语、日语、俄语、波斯语、韩语、印地语、希伯来语和印尼语，涵盖5个语系和不同资源水平。
...
TL;DR: 提出一种上下文感知的用户画像框架，通过查询生成器和画像器优化利用用户历史活动来预测消息说服力


<details>
  <summary>Details</summary>
Motivation: 当前缺乏系统性框架来优化利用用户历史活动（如对话记录）来预测消息说服力，而说服力预测需要考虑目标说服对象的特征（价值观、经验、推理风格等）

Method: 提出包含两个可训练组件的上下文感知用户画像框架：1) 查询生成器 - 生成最优查询从用户历史中检索与说服相关的记录；2) 画像器 - 将这些记录总结成画像，有效为说服力预测模型提供信息

Result: 在ChangeMyView Reddit数据集上的评估显示，相比现有方法在多个预测模型上取得一致改进，F1分数最高提升+13.77个百分点。分析表明有效的用户画像是上下文依赖且预测器特定的，而非依赖静态属性或表面相似性

Conclusion: 任务导向、上下文依赖的用户画像对于个性化说服力预测至关重要，提出的框架能有效利用用户历史活动来提升说服力预测性能
...
<details>
  <summary>Details</summary>
Motivation: 现有逐步AI聊天系统通常将一次性生成分割成多个消息顺序发送，但缺乏主动等待机制，消息节奏不自然，无法模拟真实人类即时通讯的对话模式。

Method: 提出Stephanie2代理，在每一步明确决定发送或等待，将延迟建模为思考时间和打字时间的总和；引入基于时间窗口的双代理对话系统生成伪对话历史用于评估。

Result: 实验表明Stephanie2在自然度和参与度等指标上明显优于Stephanie1，在角色识别图灵测试的人类评估中获得更高的通过率。

Conclusion: Stephanie2通过主动等待和消息节奏适应机制，能够生成更自然、更吸引人的逐步对话，更好地模拟人类即时通讯的社交聊天模式。
...
TL;DR: Afri-MCQA是首个覆盖15种非洲语言的多语言文化问答基准，包含7.5k个Q&A对，由母语者创建。评估显示LLM在非洲语言和文化知识上表现不佳，突显了需要语音优先方法、文化基础预训练和跨语言文化迁移。
...
Method: 创建Afri-MCQA基准：包含7.5k个Q&A对，覆盖15种非洲语言（来自12个国家），提供文本和语音模态的平行英语-非洲语言Q&A对，全部由母语者创建。使用该基准评估大型语言模型，并设计控制实验分离语言能力和文化知识评估。

Result: 开源模型在所有评估的文化中表现不佳，在母语或语音查询的开放式VQA上准确率接近零。在语言能力控制实验中，观察到母语和英语之间存在显著的性能差距（文本和语音均如此）。

Conclusion: 研究结果强调了需要语音优先方法、文化基础预训练和跨语言文化迁移。为支持非洲语言的多模态AI发展，Afri-MCQA基准已发布在HuggingFace上。
...
TL;DR: 语音大语言模型通过多模态上下文学习能够有效处理未见语言，结合跨语言迁移学习和注意力机制分析，提出了一种改进低资源语言ASR性能的新方法。


<details>
  <summary>Details</summary>
Motivation: 当前自动语音识别系统仅覆盖世界上少数语言，主要受限于监督数据稀缺。虽然大语言模型的上下文学习能缓解此问题，但现有研究主要关注训练时覆盖的高资源语言和纯文本设置。本文旨在探索语音大语言模型是否能够通过多模态上下文学习处理未见语言，并利用这种学习改进ASR性能。

Method: 使用Phi-4和Qwen3-Omni两种语音大语言模型，在三种不同的濒危语言上进行实验。研究多模态上下文学习的有效性，分析跨语言迁移学习对目标语言效率的影响，通过注意力模式解释学习机制，并提出结合更强声学模型和语音大语言模型的ASR系统，通过MICL选择声学假设。

Result: 多模态上下文学习对未见语言有效，能同时利用语音和文本模态。跨语言迁移学习能提高目标语言效率而无需训练目标语言数据。注意力分析显示层依赖的音频和文本上下文偏好，整体偏向文本。基于提示的ASR在未见语言上表现不佳，但提出的ASR系统通过MICL选择声学假设能持续改进ASR性能，跨语言迁移学习匹配或优于使用目标语言数据训练的语料库语言模型。

Conclusion: 语音大语言模型通过多模态上下文学习能够有效处理未见语言，跨语言迁移学习提供了一种无需目标语言数据的高效方法。注意力机制分析揭示了模型处理多模态信息的内在机制。提出的ASR系统框架为低资源语言ASR提供了实用解决方案，代码已公开。
...
TL;DR: 提出一种新工具，通过将扩散张量成像（DTI）应用于词嵌入，分析和可视化自然语言表达中的信息流，揭示LLM如何表示实际自然语言表达。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅提取单个词的嵌入并通过点图可视化，忽略了词语使用的上下文，无法分析完整的自然语言表达。需要一种新方法来理解LLM如何表示实际的自然语言表达。

Method: 将扩散张量成像（DTI）技术应用于词嵌入，分析和可视化自然语言表达中的信息流。通过追踪LLM各层中的信息流，比较不同模型结构并识别可修剪的未充分利用层。

Result: DTI揭示了词嵌入之间的信息流动方式。该方法能够比较不同模型结构，识别可修剪的层，并揭示代词消解和隐喻检测等任务中信息流的差异。
...
TL;DR: 研究评估了13个大型语言模型在16种语言中生成说服性语言时的性别偏见，发现所有模型都存在显著的性别差异，反映了社会心理学和语言学中记录的性别刻板印象。
...
Method: 提出了一个评估框架，通过成对提示指令评估13个LLMs在16种语言中的表现，使用基于社会心理学和传播科学的LLM-as-judge设置对19个说服性语言类别进行评估。
...
TL;DR: AutoMonitor-Bench是首个系统性评估LLM误行为监控器可靠性的基准，包含3,010个标注样本，涵盖问答、代码生成和推理任务，评估了22个模型并揭示了监控性能的显著差异和安全-效用权衡。
...
Method: 构建包含3,010个标注样本的基准数据集，涵盖问答、代码生成和推理任务；使用漏报率(MR)和误报率(FAR)两个互补指标评估监控器；评估12个专有和10个开源LLM；构建153,581个样本的训练集，微调Qwen3-4B-Instruction模型研究训练效果。

Result: 监控性能存在显著差异，MR和FAR之间存在一致的权衡关系，揭示了固有的安全-效用张力；训练已知误行为数据集对未见和更隐式误行为的监控性能提升有限。

Conclusion: 可靠的、可扩展的误行为监控面临挑战，需要未来研究任务感知的设计和训练策略，以改善LLM监控器的性能。
...
TL;DR: 研究探讨了罗马化对多语言语言模型的影响，发现在音素文字中性能损失可忽略，但在语素音节文字（中文、日文）中性能下降，且高保真罗马化可缓解但无法完全恢复。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注罗马化最有利的场景（高资源拉丁文字到低资源非拉丁文字，或亲缘关系近但文字不同的语言），不清楚罗马化是否适合预训练通用多语言模型，特别是罗马化导致的信息损失是否会损害高资源语言的性能。

Method: 从零开始预训练编码器语言模型，对六种类型多样的高资源语言同时使用罗马化和原始文本，使用两种保真度不同的罗马化工具，比较单语模型与多语言模型，分析两个潜在性能下降来源：脚本特定信息损失和词汇重叠增加导致的负迁移干扰。

Result: 音素文字语言性能损失可忽略，语素音节文字（中文、日文）性能下降，高保真罗马化可缓解但无法完全恢复；未发现子词重叠增加导致负迁移干扰；罗马化提高了音素文字的编码效率（生育率）且性能成本可忽略。

Conclusion: 罗马化对音素文字是可行的表示选择，但对语素音节文字需谨慎；罗马化不会因增加词汇重叠而导致负迁移干扰；罗马化可提高编码效率，为多语言模型设计提供实用指导。
...
TL;DR: 比较微调与提示工程在文本简化任务中的表现，发现微调模型在结构简化上更优，而提示工程在语义相似度上得分更高但倾向于复制输入


<details>
  <summary>Details</summary>
Motivation: 在大语言模型时代，文本简化任务存在微调与提示工程两种主要方法，需要系统比较这两种范式的实际效果差异

Method: 引入Simplify-This研究，使用编码器-解码器大语言模型，在多个基准测试上评估微调和提示工程两种范式，采用多种评估指标

Result: 微调模型在结构简化方面表现更优，而提示工程方法在语义相似度得分上更高但倾向于复制输入；人工评估总体上更偏好微调模型的输出

Conclusion: 微调在文本简化任务中整体表现更好，研究发布了代码、清理后的数据集、微调模型检查点和提示模板以促进可重复性和未来研究
...
<details>
  <summary>Details</summary>
Motivation: 训练LLM作为真实环境中的智能体需要丰富多样的工具交互环境，但现有方法存在三个主要问题：1）访问真实系统受限；2）LLM模拟环境容易产生幻觉和不一致；3）手动构建的环境难以扩展。
...
Conclusion: EnvScaler通过程序合成自动创建可扩展的工具交互环境，有效解决了LLM智能体训练中的环境构建瓶颈，显著提升了模型在复杂环境中的任务解决能力。
...
TL;DR: 提出训练LLM模拟科学记者的框架，帮助早期研究者学习向公众传播研究成果
...
Method: 开发训练框架，使LLM能够模拟科学记者的角色，通过与研究者对话来学习如何有效传播研究成果

Result: 训练后的LLM记者能提出更相关的问题，关注研究的社会影响，促使研究者澄清和阐述发现；用户研究显示多数参与者更偏好与训练后的LLM记者互动
...
<details>
  <summary>Details</summary>
Motivation: 预分词是字节级BPE分词器的关键步骤，现有基于正则表达式的实现存在性能瓶颈。需要一种更高效、安全的预分词算法来提升整个分词流程的吞吐量。

Method: 提出Peek2算法，作为cl100k类预分词器的直接替代品。该算法完全在CPU上运行，采用无正则表达式的设计，具有稳定的线性时间复杂度O(n)，并能产生与原正则表达式预分词器相同的预分割结果。

Result: Peek2在整个字节级BPE编码过程中实现了1.11倍的总体吞吐量提升，同时保持与原始预分词器相同的输出结果，确保了兼容性和安全性。

Conclusion: Peek2是一种高效、安全的预分词算法替代方案，能够显著提升字节级BPE分词器的性能，同时保持与现有系统的完全兼容。
...
TL;DR: 研究发现大型语言模型在新闻摘要生成中存在政治框架中心化倾向，大多数模型倾向于产生中立框架，而Grok 4是意识形态表达最丰富的生成模型。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地用于文本生成和重写，特别是在新闻领域，人们担心其可能通过微妙的措辞选择影响政治框架，从而塑造读者对新闻事件的解读。

Method: 研究评估了9个最先进的LLM：首先通过少样本学习预测意识形态倾向（左/中/右）；然后在FAITHFUL、CENTRIST、LEFT、RIGHT四种提示下生成"引导"摘要；最后使用固定的意识形态评估器对所有输出进行评分。

Result: 发现普遍存在意识形态中心化现象，模型在文章级别评分和生成文本中都倾向于中立框架。Grok 4是意识形态表达最丰富的生成模型，Claude Sonnet 4.5和Llama 3.1分别在商业和开源模型中表现出最强的偏见评分性能。

Conclusion: 大型语言模型在新闻摘要生成中存在系统性中心化倾向，这可能影响新闻报道的政治框架多样性，需要进一步研究模型偏差对新闻传播的影响。
...
TL;DR: 提出一个语义NLP驱动的流水线，将非结构化电子健康记录转换为符合FHIR标准的数字孪生表示


<details>
  <summary>Details</summary>
Motivation: 从非结构化电子健康记录生成可互操作的患者数字孪生具有挑战性，主要由于临床文档的变异性以及缺乏标准化映射

Method: 使用语义NLP流水线：命名实体识别提取临床概念，概念规范化映射到SNOMED-CT或ICD-10，关系提取捕获结构化关联

Result: 在MIMIC-IV临床数据库演示集上评估，与MIMIC-IV-on-FHIR参考映射验证，实体和关系提取获得高F1分数，相比基线方法提高了模式完整性和互操作性

Conclusion: 该流水线能够有效将非结构化EHR笔记转换为符合FHIR标准的数字孪生表示，解决了临床文档变异性问题，提高了医疗数字孪生的互操作性
...
TL;DR: 本文提出多模态自动补全(MAC)任务，结合文本和视觉上下文预测实时聊天中的后续字符，相比传统文本自动补全(TAC)能更好理解用户意图。作者构建了基准数据集，评估了视觉语言模型(VLMs)与文本模型的性能权衡，并提出了Router-Suggest框架动态选择模型，在保持用户满意度的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 在数字助手、聊天机器人、设计工具和医疗咨询等场景中，用户输入通常依赖于共享的视觉上下文。传统的文本自动补全(TAC)无法利用多模态信息，导致对用户意图的理解有限。需要开发能够结合视觉线索的多模态自动补全系统，以提供更准确、用户友好的补全建议。

Method: 1) 提出多模态自动补全(MAC)任务，使用部分输入文本和视觉线索预测后续字符；2) 基于MMDialog和ImageChat构建基准数据集；3) 评估视觉语言模型(VLMs)与文本基线的性能权衡；4) 提出Router-Suggest框架，根据对话上下文动态选择文本模型或VLM；5) 开发轻量级版本用于资源受限环境。

Result: 1) VLMs在用户满意度方面显著优于文本模型，特别是在节省用户输入努力和提升多轮对话补全质量方面；2) Router-Suggest框架相比最佳VLM实现了2.3倍到10倍的速度提升；3) 用户研究表明，多模态上下文对于高质量的自动补全至关重要。

Conclusion: 多模态上下文对于智能自动补全系统至关重要，能够显著提升用户满意度和交互质量。Router-Suggest框架通过动态模型选择，在保持VLMs优势的同时解决了效率问题，为实现更智能、用户感知的助手系统提供了有效方案。
...
Method: 提出CLewR策略，将课程学习集成到各种最先进的偏好优化算法中。核心是带重启的课程学习，在训练过程中多次重复从易到难的课程，有效缓解对简单示例的灾难性遗忘

Result: 在多个模型家族（Gemma2、Qwen2.5、Llama3.1）和偏好优化技术上均获得了一致的性能提升

Conclusion: CLewR策略通过集成课程学习到偏好优化算法中，有效提升了多语言机器翻译性能，并缓解了灾难性遗忘问题，代码已公开
...
TL;DR: 自动口译质量评估方法无法考虑交际语境，不适合单独用于评估真实口译实践的质量


<details>
  <summary>Details</summary>
Motivation: 随着远程口译、计算机辅助口译、自动语音翻译和口译虚拟人等技术的发展，需要快速高效地测量口译质量。各种自动化质量评估方法被提出，但需要检验它们是否适合评估真实口译实践的质量。

Method: 文章审查了最近提出的各种口译质量测量方法，分析它们是否适合评估人类或机器提供的真实口译实践的质量。

Result: 研究发现，当前提出的自动度量方法无法考虑交际语境，因此单独使用时不能作为任何口译服务质量的可行测量标准。在口译研究中，口译发生的语境对最终分析至关重要。

Conclusion: 自动口译质量评估方法不能单独使用，因为它们无法捕捉交际语境这一口译质量评估的核心要素。任何口译质量测量都需要考虑口译发生的具体语境。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究常将引用幻觉归因于模型过度依赖参数知识，但本文挑战这一观点，认为需要更深入地理解模型内部机制如何导致这种欺骗性错误。

Method: 提出FACTUM框架，包含四个机制性评分，分别衡量模型的注意力路径、FFN路径的贡献以及两者之间的对齐程度，通过分析这些内部机制来识别引用幻觉。

Result: 发现正确引用有两个一致特征：更强的参数知识贡献和更多使用注意力汇进行信息合成。但正确引用的特征随模型规模演变：Llama-3.2-3B需要更高路径对齐，而Llama-3.1-8B需要更低对齐（路径贡献更正交的信息）。FACTUM在AUC上优于现有基线达37.5%。

Conclusion: 引用幻觉是复杂、规模依赖的内部机制相互作用的结果，FACTUM框架为构建更可靠、更细致的RAG系统提供了新途径。
...
TL;DR: 该论文提出使用基于词性标注的语码转换和重播适配器策略的持续学习方法，来解决多语言场景下小语言模型从大语言模型适应时的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 在多语言场景下，为低资源语言构建小语言模型时，通过适应大语言模型会遇到灾难性遗忘这一主要挑战。现有方法在从大语言模型训练小语言模型时存在灾难性遗忘的缺陷。

Method: 提出一种持续学习策略，结合基于词性标注的语码转换和重播适配器策略，以缓解从大语言模型训练小语言模型时的灾难性遗忘问题。

Result: 在视觉语言任务（如视觉问答）和语言建模任务上的实验表明，所提出的架构取得了成功。

Conclusion: 基于词性标注的语码转换与重播适配器策略相结合的持续学习方法，能有效解决多语言场景下小语言模型适应时的灾难性遗忘问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有自演化框架主要奖励最终结果，对中间推理过程约束较弱，而中间推理对于视觉基础决策至关重要。需要一种能够改进大型多模态模型隐式推理能力的方法。

Method: 提出iReasoner框架，在无标签图像上通过Proposer-Solver循环，除了结果层面的内在奖励外，还增加了基于中间推理步骤的轨迹感知信号，通过思维链的内部一致性提供学习信号。

Result: 从Qwen2.5-VL-7B开始，iReasoner在完全无监督的后训练下，在多样化的多模态推理基准测试中获得了最高+2.1分的提升。
...
TL;DR: 研究显示，主要大语言模型在家庭法律场景中存在性别偏见，不同模型对离婚案例中父母抚养权分配的建议存在系统性差异，存在性别依赖模式。


<details>
  <summary>Details</summary>
Motivation: 随着普通人越来越多地依赖大语言模型进行法律自助，但可能基于不完整、错误或有偏见的输出形成期望。本研究旨在检验领先的大语言模型在现实家庭法律场景中是否表现出性别偏见。

Method: 基于捷克家庭法设计专家离婚场景，评估GPT-5 nano、Claude Haiku 4.5、Gemini 2.5 Flash和Llama 3.3四种先进模型。使用两个版本场景：带性别化姓名和中性标签版本作为比较基线。引入九个法律相关因素改变案件事实情况，测试这些变化是否影响模型建议的共同抚养比例。

Result: 初步结果显示不同模型之间存在差异，某些系统生成的结果存在性别依赖模式。研究强调了普通人依赖大语言模型获取法律指导的风险，以及需要在敏感法律背景下对模型行为进行更稳健评估的必要性。

Conclusion: 研究提供了探索性和描述性证据，旨在识别系统性不对称而非建立因果关系。结果表明大语言模型在家庭法律应用中存在性别偏见问题，需要进一步研究和更严格的模型评估。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明偏好调优在训练领域外会降低性能，但适应策略如何缓解这种领域迁移问题尚未充分探索。本文旨在系统研究对齐方法在领域迁移下的泛化能力。

Method: 比较五种流行的对齐目标，以及从源领域到目标领域的各种适应策略，包括目标领域监督微调和伪标签方法，在摘要和问答帮助性任务上进行实验

Result: 发现不同对齐目标在领域迁移下存在系统性泛化差异，基于伪标签的适应策略能显著减少领域迁移带来的性能下降

Conclusion: 对齐方法的泛化能力在领域迁移下存在显著差异，伪标签适应策略是缓解领域迁移问题的有效方法，为实际应用中的对齐方法选择提供了指导
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM路由方法通常只关注选择LLM架构，而忽略了参数设置对任务性能的关键影响，需要同时考虑架构和参数的联合优化。

Method: 提出分层路由框架HAPS：高层路由器选择候选LLM架构，低层路由器为选定架构搜索最优参数；设计参数生成网络在两个路由器间共享参数以增强能力；使用奖励增强目标进行训练优化。

Result: 在两个常用基准测试中，HAPS始终优于强大的路由基线方法，证明了联合搜索架构和参数的有效性。

Conclusion: HAPS通过分层路由框架联合优化LLM架构和参数选择，显著提升了路由性能，为利用不同LLM的专业优势提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM评估主要依赖点状置信度（如Self-Consistency），无法检测信念在轻微上下文干扰下的脆弱性，而真实部署需要模型在上下文扰动下保持真实信念

Method: 1) 提出NCB作为信念鲁棒性的结构性度量，评估概念邻域内的响应一致性；2) 设计认知压力测试协议，探测上下文干扰下的输出稳定性；3) 提出SAT训练方法，优化上下文不变的信念结构

Result: 实验表明：1) 即使完美自一致的事实也会在轻微干扰下迅速崩溃；2) 高NCB数据在干扰下表现更稳定；3) SAT能将长尾知识脆弱性降低约30%

Conclusion: NCB能有效评估LLM信念鲁棒性，SAT能显著增强模型对上下文干扰的抵抗力，为可靠部署提供新方法
...
<details>
  <summary>Details</summary>
Motivation: 为法语开发一个能够无缝处理文本和语音输入的共享架构模型，通过特征空间自监督目标更有效地学习语言和声学表示，超越现有的法语基线模型。

Method: 使用大规模法语语料库预训练分离的模型：文本使用Wikipedia、OSCAR和CroissantLLM；语音使用MultilingualLibriSpeech、LeBenchmark和新引入的INA-100k（来自法国国家视听研究所的10万小时法语音频）。模型在特征空间学习上下文目标表示，而不是预测模态特定的目标。

Result: 在FLUE和LeBenchmark等标准法语基准测试的各种下游任务中，Pantagruel模型表现出与CamemBERT、FlauBERT和LeBenchmark2.0等强基线模型相当或更优的性能，同时保持能够无缝处理语音或文本输入的共享架构。

Conclusion: 结果证实了特征空间自监督目标对法语表示学习的有效性，并突出了Pantagruel作为多模态语音-文本理解的稳健基础模型的价值。
...
TL;DR: 提出FOREAGENT框架，通过内部化执行先验来替代昂贵的物理执行，使用预测-验证循环加速自主机器学习代理的收敛速度
...
Method: 1. 形式化数据为中心的解决方案偏好任务，构建包含18,438对比较的语料库；2. 使用已验证数据分析报告引导LLMs进行预测；3. 实现FOREAGENT代理，采用预测-验证循环

Result: 1. LLMs在验证数据分析报告引导下表现出显著预测能力，达到61.5%准确率并具有稳健置信度校准；2. FOREAGENT实现6倍收敛加速，超越基于执行的基线方法+6%

Conclusion: 通过内部化执行先验替代物理执行，预测-验证范式能显著加速自主机器学习代理的收敛，同时保持或超越传统执行方法的性能
...
TL;DR: 提出一个框架，通过文件记忆系统和智能体控制工具调用，将临时性批评转化为可检索的指导原则，从而分摊推理时的计算成本


<details>
  <summary>Details</summary>
Motivation: 减少推理时的计算成本，避免每次都需要重新进行复杂的推理过程，通过将一次性的批评建议转化为可重用的指导原则

Method: 使用基于文件的记忆系统和智能体控制的工具调用，将瞬时的批评转化为可检索的指导原则，在Rubric Feedback Bench数据集上进行评估

Result: 增强后的LLM能够快速达到测试时精炼管道的性能水平，同时大幅降低推理成本

Conclusion: 该方法通过摊销推理成本，在保持性能的同时显著提高了效率，为基于规则的学习任务提供了有效的解决方案
...
TL;DR: 论文提出长链思维推理的有效学习需要稳定的分子状结构，包含三种相互作用类型，并开发了Mole-Syn方法来合成这些结构以提升性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在从人类或非长链思维LLM模仿中学习有效的长链思维推理时经常失败，需要理解这种失败的原因并找到解决方案。

Method: 提出长链思维轨迹具有类似分子的稳定结构，包含深度推理（共价键状）、自我反思（氢键状）和自我探索（范德华力状）三种相互作用类型。引入有效语义异构体概念，并开发Mole-Syn方法，这是一种分布转移图方法，用于指导合成有效的长链思维结构。

Result: 分析发现这些结构来自长链思维微调而非关键词模仿，只有促进快速熵收敛的键才能支持稳定的长链思维学习。Mole-Syn方法在多个基准测试中提升了性能和强化学习稳定性。

Conclusion: 长链思维推理的有效学习依赖于特定的分子状结构相互作用，通过Mole-Syn方法可以合成这些结构，从而改善LLM的长链推理能力和训练稳定性。
...
TL;DR: 论文评估了三大LLM提供商在代理任务中的提示缓存效果，发现策略性缓存可降低45-80%成本并提升13-31%响应速度


<details>
  <summary>Details</summary>
Motivation: 尽管主流LLM提供商提供提示缓存以降低成本和延迟，但针对需要大量工具调用的多轮代理任务，缓存的实际效益在研究中尚未充分探索，缺乏对成本节省的量化分析和缓存策略的比较

Method: 在三大LLM提供商（OpenAI、Anthropic、Google）上评估三种缓存策略：全上下文缓存、仅系统提示缓存、排除动态工具结果的缓存。使用DeepResearchBench多轮代理基准测试，代理自主执行真实网络搜索工具调用来回答复杂研究问题，测量超过500个代理会话的API成本和首令牌时间

Result: 提示缓存可将API成本降低45-80%，首令牌时间提升13-31%。策略性缓存块控制（如将动态内容置于系统提示末尾、避免动态传统函数调用、排除动态工具结果）比朴素的全上下文缓存效果更稳定，后者可能反而增加延迟。不同提供商间存在细微差异

Conclusion: 提示缓存对代理工作负载具有显著效益，但需要策略性实施。研究提供了在生产代理系统中实施提示缓存的实用指导，强调应根据提供商特性和任务特点选择合适的缓存策略
...
TL;DR: CaRR提出细粒度奖励框架，通过分解复杂问题为可验证单跳rubrics，要求智能体识别隐藏实体、提供正确引用、构建完整证据链，结合C-GRPO优化策略，提升深度搜索智能体的推理全面性和事实基础。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的深度搜索智能体主要依赖二元结果奖励，无法捕捉推理过程的全面性和事实性，导致捷径利用和幻觉等不良行为。需要更细粒度的奖励框架来强调推理全面性、事实基础和证据连通性。

Method: 提出Citation-aware Rubric Rewards (CaRR)框架：将复杂问题分解为可验证的单跳rubrics，要求智能体通过明确识别隐藏实体、提供正确引用、构建完整证据链来满足这些rubrics。进一步提出Citation-aware Group Relative Policy Optimization (C-GRPO)，结合CaRR和结果奖励训练鲁棒的深度搜索智能体。

Result: C-GRPO在多个深度搜索基准测试中持续优于标准基于结果的RL基线。分析验证C-GRPO有效抑制捷径利用，促进全面、基于证据的推理，并在开放式深度研究任务中表现出强大的泛化能力。

Conclusion: CaRR和C-GRPO为深度搜索智能体提供了有效的细粒度奖励框架，解决了现有二元奖励的局限性，显著提升了推理质量和事实准确性。
...
TL;DR: AdaFuse提出了一种自适应融合解码框架，通过动态选择语义合适的融合单元，在生成过程中灵活调整融合粒度，解决了传统集成方法固定融合粒度的局限性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型具有互补优势，但现有集成方法存在固定融合粒度的局限性，缺乏生成过程中的自适应能力，无法适应不同任务的生成特性。

Method: 提出AdaFuse框架：1）基于不确定性的标准决定每个解码步骤是否应用集成；2）在置信状态下直接生成，在不确定状态下调用多样性感知缩放策略探索替代候选；3）建立自适应集成与测试时缩放的协同交互。

Result: 在开放域问答、算术推理和机器翻译任务上，AdaFuse持续优于强集成基线，平均相对改进达到6.88%。

Conclusion: AdaFuse通过动态调整融合行为，实现了更灵活的集成解码，在多个任务上显著提升了性能，为LLM集成提供了有效的自适应解决方案。
...
TL;DR: NAIAD是一个基于大语言模型的智能助手，利用地球观测数据为内陆水体监测提供端到端解决方案，通过单提示界面将自然语言查询转化为可操作见解，在基准测试中正确性和相关性分别超过77%和85%。


<details>
  <summary>Details</summary>
Motivation: 现有内陆水体监测方法通常孤立地处理蓝藻、叶绿素等子问题，缺乏整体解决方案。NAIAD旨在为专家和非专家用户提供一个统一的智能助手，利用地球观测数据进行全面的水体监测。

Method: 采用基于大语言模型的智能体架构，结合检索增强生成、外部工具编排、计算图执行和智能体反思等技术。系统集成天气数据、Sentinel-2影像、遥感指数计算、叶绿素-a估算以及CyFi等平台工具。

Result: 在专门基准测试中，系统在正确性和相关性指标上分别达到77%和85%以上，表现出强大的适应性和鲁棒性。消融研究发现Gemma 3 (27B)和Qwen 2.5 (14B)在计算效率和推理性能之间达到最佳平衡。

Conclusion: NAIAD证明了基于大语言模型的智能体方法能够为内陆水体监测提供全面解决方案，通过单提示界面有效满足不同专业水平用户的需求，在正确性和相关性方面表现优异。
...
Method: 1) 构建增材制造数学知识图谱(AM-MKG)，形式化编码方程、变量、假设及其语义关系；2) 基于MKG子图条件约束的LLM方程生成，确保物理意义；3) 引入置信感知外推评估，整合外推距离、统计稳定性和物理一致性

Result: 本体引导提取显著改善知识提取的结构连贯性和定量可靠性；子图条件方程生成相比无引导LLM输出产生更稳定、物理一致的外推结果
...
<details>
  <summary>Details</summary>
Motivation: 尽管已有研究表明为人格特质可以影响LLM行为，但在受控条件下人格引导如何影响合作行为仍不清楚。本研究旨在探索人格引导对LLM智能体在战略互动中合作行为的影响。
...
Conclusion: 人格引导更多是作为行为偏差而非确定性控制机制。宜人性是促进合作的关键因素，但人格引导的效果因模型代际而异，后期模型表现出更复杂的合作策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有酶-底物相互作用预测方法依赖专家标注的稀疏数据库，数据不足且维护成本高，限制了模型的泛化能力。化学反应方程式数据更易获取且更丰富，但涉及多化合物与酶的复杂关系，传统模型难以捕捉。

Method: 将化学反应方程式表示为(底物，酶，产物)三元组构建知识图谱，提出Hyper-Enz模型：结合超图transformer和知识图谱嵌入学习多底物/产物超边表示，并引入多专家范式指导酶-底物相互作用学习。

Result: 实验结果显示显著改进：酶检索准确率相对提升高达88%，配对级预测提升30%，证明该方法有效性。

Conclusion: 利用化学反应方程式的知识图谱表示和超图transformer能有效预测酶-底物相互作用，解决了传统方法数据稀疏和复杂关系建模的挑战。
...
TL;DR: 医疗角色设定对临床LLMs有系统性、情境依赖且非单调的影响：在重症任务中提升性能，但在初级护理中降低性能，交互风格影响风险倾向但高度模型依赖


<details>
  <summary>Details</summary>
Motivation: 研究角色设定（如急诊医生、护士）对大型语言模型在临床决策中的影响，因为通常认为角色设定能单调提升专业性和安全性，但其在高风险临床决策中的实际效果尚不明确

Method: 系统评估临床LLMs中基于角色的控制，考察专业角色和交互风格（大胆vs谨慎）在不同模型和医疗任务中的影响，使用多维评估方法衡量任务准确性、校准和安全相关风险行为

Result: 医疗角色在重症护理任务中提升性能（准确性和校准增益达~20%），但在初级护理环境中降低性能；交互风格调节风险倾向但高度模型依赖；人类临床医生对安全合规性有中等一致性但95.9%的推理质量回答信心较低

Conclusion: 角色设定作为行为先验引入情境依赖的权衡而非安全或专业性的保证，揭示了角色设定的复杂影响和局限性
...
TL;DR: 大型多模态语言模型作为AI代理在群体环境中表现出系统性从众偏差，符合社会影响理论，即使单独表现完美的AI代理也容易受到社会影响操纵，存在安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理在多代理环境中运行，理解它们的集体行为对于预测人工社会动态变得至关重要。研究需要探索AI代理作为社会行动者如何响应群体影响，特别是从众倾向。

Method: 通过改编社会心理学中的经典视觉实验，研究大型多模态语言模型作为AI代理在群体影响下的反应。实验考察了群体规模、一致性、任务难度和来源特征等因素对AI代理从众行为的影响。

Result: AI代理表现出系统性从众偏差，符合社会影响理论。单独表现近乎完美的AI代理在社会影响下变得高度易受操纵。虽然大型模型在简单任务上因能力提升而减少从众，但在能力边界上仍然脆弱。

Conclusion: AI代理决策中存在基本安全漏洞，可能被恶意操纵、错误信息传播和偏见传播利用，突显了在集体AI部署中迫切需要安全防护措施。
...
TL;DR: 评估国际象棋作弊的潜在性能增益，而非检测作弊


<details>
  <summary>Details</summary>
Motivation: 国际象棋中使用强大软件作弊已成为严重问题，但现有研究主要关注作弊检测，缺乏对作弊性能增益的量化评估

Method: 开发算法并在常用国际象棋引擎上测试，模拟比赛中有限次数的作弊行为

Result: 通过实验测量了有限次数作弊可能带来的性能提升效果
...
TL;DR: ART提出了一种分层论证树方法进行声明验证，通过树形结构组织支持/攻击论点，使用LLM作为裁判进行两两比较，实现透明、可争议的决策过程，相比CoT等方法更具解释性。


<details>
  <summary>Details</summary>
Motivation: LLM在复杂决策中表现出色，但其不透明性阻碍了在高风险环境中的应用。现有方法缺乏忠实解释和错误纠正机制，无法建立可信赖的决策系统。

Method: ART采用分层论证树结构：从根声明开始，分支为支持和攻击的子论点。通过LLM作为裁判进行子论点间的两两比较，自底向上确定论点强度，最终得出透明、可争议的裁决。

Result: ART在多个数据集上验证有效，分析不同论点生成器和比较策略。结果表明ART的结构化推理优于强基线，为可解释声明验证设立了新基准，提供更可靠、清晰的决策过程。

Conclusion: ART通过分层论证树结构解决了LLM决策的透明性问题，提供了可解释、可争议的声明验证方法，在高风险环境中具有重要应用价值，为可信AI决策系统奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的检索增强生成系统面临两个主要障碍：1) 检索崩溃 - 在大规模语料库上进行迭代检索时，缺乏推理引导的规划导致无法找到包含桥梁答案的中间证据；2) 学习不稳定 - 端到端轨迹训练在推理链上信用分配薄弱，模块间错误定位困难，容易过拟合到特定基准的启发式方法，限制了可迁移性和稳定性。

Method: 提出PRISMA框架，采用解耦的Plan-Retrieve-Inspect-Solve-Memoize架构。核心是推理引导的协作：检查器提供基于推理的反馈来改进规划器的分解和细粒度检索，同时在解决器中强制执行基于证据的推理。使用两阶段组相对策略优化进行优化：第一阶段将规划器和解决器校准为规划和推理专家；第二阶段使用观察感知残差策略优化增强检查器的上下文验证和针对性恢复能力。

Result: 在十个基准测试中实现了最先进的性能，并且能够在真实场景中高效部署。

Conclusion: PRISMA通过解耦的强化学习框架和推理引导的协作机制，有效解决了检索增强生成系统中的检索崩溃和学习不稳定问题，为复杂多跳问题的可靠部署提供了可行方案。
...
Result: 在纽约、香港和深圳的案例研究中成功识别了城市变化模式：纽约小型社区公园增加、香港跨区域水污染扩散、深圳露天垃圾场减少但夜间经济活动与不同类型垃圾的关联差异；相比最佳基线任务成功率提升46.7%，有效缓解幻觉问题

Conclusion: MMUEChange框架能够有效支持具有现实政策意义的复杂城市变化分析任务，展示了多模态智能体在城市环境变化研究中的应用潜力
...
TL;DR: 该论文提出了一种概率范式来处理医学AI评估中ground truth答案不确定性的问题，引入了期望准确率和期望F1分数，并建议按ground truth概率分层评估系统性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统（特别是医学领域）的基准测试通常忽略ground truth答案中的不确定性，这可能导致误导性的结论，例如非专家系统可能被误认为与专家表现相当。

Method: 提出概率范式理论，引入期望准确率和期望F1分数来估计在ground truth答案变异性的情况下专家或系统能达到的分数。建议按ground truth答案概率（通常通过专家间一致率衡量）进行分层评估。

Result: 理论分析表明，在ground truth高确定性的情况下专家才能获得高分，而在答案变异大的数据集中，随机标注者与专家可能差异不大。分层评估能在高确定性区间提供更可靠的性能比较。

Conclusion: 在评估AI系统能力时，应考虑ground truth答案的不确定性，特别是在总体性能低于80%时，必须按ground truth概率进行分层评估，以消除不确定性这一关键混杂因素的影响。
...
TL;DR: 该论文提出将可解释人工智能（XAI）与因果推理结合，通过"向学习者学习"的方式，从基础模型中提取因果机制，指导稳健设计和控制，支持高风险应用中的信任与问责。


<details>
  <summary>Details</summary>
Motivation: 尽管人工智能在科学和工程任务中已超越人类，但其内部表示通常不透明。需要开发方法使AI系统更可解释，以便人类能够理解和信任AI的决策过程，特别是在高风险应用中。

Method: 结合可解释人工智能（XAI）与因果推理，利用基础模型和可解释性方法，从AI模型中提取因果机制。重点关注发现、优化和认证三个领域。

Result: 展示了XAI与因果推理结合能够：1）从AI模型中提取因果机制；2）指导稳健的设计和控制；3）支持高风险应用中的信任和问责。

Conclusion: XAI可作为科学与工程中人机协作的统一框架，但仍面临解释的忠实性、泛化性和可用性等挑战。通过"向学习者学习"的方法，能够更好地理解AI的内部表示并建立可信赖的人机协作系统。
...
TL;DR: 该论文通过火灾疏散场景评估LLM在安全关键系统中的决策能力，发现即使99%准确率也意味着每百次执行可能造成灾难性后果，当前LLM不适合直接部署于安全关键系统。
...
Result: 结果揭示严重漏洞：多个模型在ASCII导航中成功率为0%；在模拟消防演习中，模型指示机器人向危险区域而非紧急出口移动。分析显示1%失败率在安全关键环境中会升级为灾难性后果。

Conclusion: 当前LLM尚未准备好直接部署于安全关键系统。99%准确率在机器人领域具有误导性，意味着每百次执行可能造成灾难性伤害。即使最先进模型也无法保证安全，绝对依赖它们会带来不可接受的风险。
...
Method: 1) 从同行评审文献自动合成领域特定科学问题，创建WildSci数据集；2) 将复杂科学推理任务转化为多项选择题格式；3) 应用强化学习对模型进行微调，并分析训练动态。

Result: 在一系列科学基准测试上的实验证明了数据集和方法的有效性。WildSci数据集已公开发布，支持科学推理的可扩展和可持续研究。

Conclusion: WildSci通过提供高质量的科学问题数据集和多项选择题框架，解决了科学领域LLM推理的数据稀缺和评估困难问题，为科学推理研究提供了可扩展的解决方案。
...
TL;DR: 论文提出Crisis-Bench基准，用于评估LLM在需要战略模糊性的专业领域（如公关危机管理）中的表现，揭示通用安全对齐与专业需求之间的冲突。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的安全对齐过于强调普遍性的诚实和帮助性（"童子军道德"），但在需要战略模糊和信息保留的专业领域（如公关、谈判、危机管理）中，这种刚性道德框架会带来"透明度税"，损害专业效用。

Method: 提出Crisis-Bench：基于多智能体部分可观察马尔可夫决策过程（POMDP）的评估框架，包含80个跨8个行业的危机情景。设置PR智能体在7天动态危机模拟中管理分离的私有和公共叙事状态，强制执行信息不对称。引入"仲裁者-市场循环"评估指标，将公众情绪转化为模拟股价，创建经济激励结构。

Result: 结果揭示关键二分现象：一些模型因伦理顾虑而妥协，另一些模型则展现出马基雅维利式的合法战略保留能力以稳定模拟股价。该基准首次为"声誉管理"能力提供了量化评估框架。

Conclusion: 需要从刚性道德绝对主义转向情境感知的专业对齐，Crisis-Bench为评估LLM在需要战略模糊性的专业领域中的表现提供了首个定量框架。
...
Method: 提出了一种新颖方法，使用强化学习对轻量级语言模型进行后训练，专门用于欺诈检测任务。采用Group Sequence Policy Optimization (GSPO)算法结合基于规则的奖励系统，在真实交易数据集上微调不同大小的语言模型。

Result: 实验结果表明该方法有效，后训练的语言模型在保留测试数据上取得了显著的F1分数提升。性能改进主要归因于强化学习固有的探索机制，使模型能够发现超越传统工程特征的新型欺诈指标。

Conclusion: 该方法成功弥合了传统机器学习局限性与LLMs在欺诈检测中未开发潜力之间的差距，证明了强化学习框架能够有效从原始交易数据中发现多样化的信任和风险信号。
...
TL;DR: 提出基于因果学习的新型排序框架，通过结构因果模型和信息论工具联合处理点击数据中的多种偏差（位置、选择、信任偏差），使用条件互信息衡量偏差泄漏并作为正则化项，结合双重稳健估计器提升风险估计可靠性。


<details>
  <summary>Details</summary>
Motivation: 网络搜索和推荐系统中广泛使用用户点击数据训练排序模型，但点击数据存在严重偏差：位置偏差（用户倾向点击高排名项）、选择偏差（只点击展示项）、信任偏差（更信任顶部结果）。现有无偏学习排序方法主要校正位置偏差并依赖倾向性估计，但无法衡量剩余偏差、提供风险保证或联合处理多种偏差源。

Method: 结合结构因果模型与信息论工具的新框架：1) SCMs指定点击生成机制，帮助从点击数据中识别真实相关性信号；2) 使用条件互信息衡量偏差泄漏到学习相关性估计中的程度；3) 将偏差泄漏作为正则化项加入模型训练以减少偏差；4) 结合因果推断估计器（双重稳健估计器）确保更可靠的风险估计。

Result: 在标准学习排序基准测试中，该方法持续减少测量的偏差泄漏并提升排序性能，特别是在位置偏差和信任偏差等多种偏差强烈交互的现实场景中表现优异。

Conclusion: 提出的因果学习排序框架通过SCMs和信息论工具有效处理多种点击偏差，提供偏差泄漏的严格度量并作为正则化项，结合双重稳健估计提升风险估计可靠性，在现实多偏差交互场景中显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识图谱补全方法在处理新兴实体时效果不佳，现有归纳KGC方法存在对噪声结构信息敏感和难以捕捉长距离依赖的问题。

Method: 提出CPSR框架：1) 查询依赖掩码模块自适应掩码噪声结构信息，保留与目标相关的关键信息；2) 全局语义评分模块评估推理路径中节点的个体贡献和集体影响。

Result: 实验结果表明CPSR达到了最先进的性能。
...
TL;DR: 提出理论框架分析生成模型的可控性，通过对话场景的算法估计可控集，提供样本复杂度的理论保证，实验显示模型可控性脆弱且依赖实验设置。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型普及，需要细粒度控制生成过程，但现有控制方法（从提示到微调）未回答根本问题：这些模型是否真正可控？需要理论框架分析模型可控性的基本限制。

Method: 将人机交互建模为控制过程，提出新算法估计对话场景中模型的可控集。提供分布无关的理论保证（PAC边界），仅需输出有界假设，适用于任何黑盒非线性控制系统（即任何生成模型）。

Result: 实验在对话控制任务中验证理论框架，涵盖语言模型和文生图模型。结果显示模型可控性出奇脆弱，高度依赖实验设置。
...
Conclusion: HAG框架通过分层方法解决了智能体初始化中的主题适应性问题，实现了宏观分布对齐和微观一致性的平衡，为基于智能体建模提供了高质量的人口初始化解决方案。
...
TL;DR: CHDP框架将混合动作空间视为合作博弈，使用离散和连续扩散策略的协作代理，通过顺序更新和低维嵌入解决表达能力和可扩展性问题。
...
Method: 将混合动作空间视为完全合作博弈，提出协作混合扩散策略框架：1）使用离散和连续扩散策略的协作代理；2）连续策略以离散动作表示为条件；3）顺序更新方案避免冲突；4）构建低维潜在空间的代码本；5）基于Q函数的引导机制对齐嵌入。

Result: 在具有挑战性的混合动作基准测试中，CHDP在成功率上比最先进方法提高了高达19.3%。

Conclusion: CHDP框架通过协作设计、顺序更新和低维嵌入，有效解决了混合动作空间的建模和优化挑战，在表达能力和可扩展性方面表现优异。
...
TL;DR: 论文提出"循环推理"这一新失败模式，开发LoopBench数据集分析数值循环和陈述循环，发现推理瓶颈触发循环开始，并利用CUSUM算法实现早期循环预测。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在测试时缩放时常遇到重复循环，导致计算浪费和推理失败。传统模型退化不同，这种"循环推理"现象表现为自我强化的陷阱，生成内容作为自身重复的逻辑前提。

Method: 1) 引入LoopBench数据集捕捉数值循环和陈述循环两种类型；2) 将循环推理表征为具有明确边界的状态崩溃；3) 揭示推理瓶颈触发循环开始，随后由自我强化的V形注意力机制维持；4) 使用累积和(CUSUM)算法捕捉循环前兆进行早期预测。

Result: 实验验证了CUSUM算法在不同大型推理模型中的准确性，阐明了长链推理的稳定性。该方法能有效预测循环开始，避免计算浪费。

Conclusion: 循环推理是大型推理模型的一种独特失败模式，通过分析其机制和开发早期检测方法，可以提高推理效率和稳定性，为长链推理的可靠性提供新见解。
...
TL;DR: 提出一个逻辑参数化框架，将逻辑形式作为可控组件，在神经符号自然语言推理中实现更鲁棒、模块化和自适应的推理。
...
Method: 使用LogiKEy方法将经典和非经典形式主义嵌入高阶逻辑，实现逻辑参数化框架。比较逻辑外部方法（通过公理编码规范要求）和逻辑内部方法（规范模式从逻辑内置结构中产生）。

Result: 逻辑内部策略能持续提升性能并产生更高效的混合证明。逻辑有效性具有领域依赖性：一阶逻辑擅长常识推理，道义和模态逻辑在伦理领域表现优异。

Conclusion: 将逻辑作为神经符号架构中的一等参数化元素，能实现更鲁棒、模块化和自适应的推理，逻辑选择对推理质量和解释细化有显著影响。
...
TL;DR: HSD是一种无损验证方法，通过分层平衡概率质量来克服联合不可处理性，显著提高接受令牌数量，在EAGLE-3中实现超过12%的性能提升。
...
Method: 提出分层推测解码（HSD），一种可证明无损的验证方法。通过平衡可访问分支上的超额和不足概率质量来克服联合不可处理性，显著提高预期接受令牌数量。

Result: 大规模实验表明，HSD在不同模型家族和基准测试中持续提高接受率。集成到EAGLE-3中实现超过12%的性能提升，在不损害分布保真度的情况下建立最先进的解码效率。

Conclusion: HSD具有强大的可解释性和通用性，易于集成到各种推测解码框架中，为提升推理效率提供了有效的无损验证解决方案。
...
TL;DR: 提出了PII-VisBench基准，评估视觉语言模型在不同在线可见度下的PII泄露风险，发现模型对高可见度主体的PII泄露更严重


<details>
  <summary>Details</summary>
Motivation: 现有VLM隐私评估将隐私视为静态提取任务，忽略了主体在线存在（数据可获取性）对隐私对齐的影响，需要更全面的评估框架

Method: 创建PII-VisBench基准，包含4000个独特探针，将200个主体按在线信息量分为高、中、低、零可见度四类，评估18个开源VLMs的拒绝率和条件PII泄露率

Result: 模型呈现一致模式：随着主体可见度降低，拒绝率增加，PII泄露减少（从高可见度的9.10%降至低可见度的5.34%）；模型更可能泄露高可见度主体的PII，存在模型家族异质性和PII类型差异

Conclusion: 需要基于可见度的安全评估和训练干预，重写和越狱式提示暴露了攻击和模型依赖的失败，强调了隐私评估中考虑在线存在的重要性
...
TL;DR: DynaDebate通过动态路径生成与分配、过程中心辩论和触发式验证代理三个机制，解决多智能体辩论中因初始化相同导致推理路径重复、辩论退化为简单多数投票的问题，显著提升了多智能体辩论的效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的多智能体辩论框架存在未引导的初始化问题，导致智能体采用相同的推理路径并犯相同错误，使得辩论效果受限，最终结果往往退化为简单的多数投票，无法充分发挥多智能体辩论的优势。

Method: 提出DynaDebate框架，包含三个核心机制：1) 动态路径生成与分配：使用专门的路径生成代理生成多样化、逻辑化的解决方案路径，并自适应分配冗余；2) 过程中心辩论：从关注表面结果投票转向严格的逐步逻辑批判，确保过程正确性；3) 触发式验证代理：在出现分歧时激活，使用外部工具客观解决僵局。

Result: 广泛的实验表明，DynaDebate在各种基准测试中实现了卓越的性能，超越了现有的最先进多智能体辩论方法。

Conclusion: DynaDebate通过创新的动态辩论机制有效解决了多智能体辩论中的路径同质化和辩论退化问题，显著提升了多智能体系统的协作决策和复杂问题解决能力。
...
TL;DR: BEPA方法通过双层专家轨迹对齐，将少量专家轨迹转化为策略对齐的指导，显著提升了端到端GUI操作策略在OSWorld等基准上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前GUI数据集规模有限，专家轨迹收集困难，而直接将专家轨迹与在线强化学习混合存在结构不匹配和分布偏移问题，需要更有效利用有限专家轨迹的方法。

Method: 提出BEPA（双层专家到策略同化）方法：第一层通过基础策略生成可达轨迹来对齐专家轨迹；第二层使用动态更新的任务特定缓存来指导强化学习。

Result: 在OSWorld-Verified上，BEPA将UITARS1.5-7B的成功率从22.87%提升到32.13%，在保留测试集上从5.74%提升到10.30%，在MMBench-GUI和Online-Mind2Web上也有一致性提升。

Conclusion: BEPA能够有效利用少量专家轨迹，通过双层对齐机制解决轨迹不匹配问题，显著提升端到端GUI操作策略的性能，为计算机使用代理的训练提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的集中式多智能体系统在处理复杂知识密集型任务时，中央智能体由于缺乏内存管理，导致上下文膨胀、错误累积和跨任务泛化能力差，影响了长时程协作的稳定性。
...
Conclusion: StackPlanner通过显式内存控制解决了多智能体系统中的内存效率问题和协调经验重用问题，为可靠的长时程协作提供了有效解决方案。
...
TL;DR: TowerMind是一个基于塔防游戏的轻量级多模态环境，用于评估LLM在实时策略游戏中的长期规划和决策能力，同时支持幻觉评估和高可定制性。
...
Method: 提出TowerMind环境，基于塔防游戏子类型，保留RTS游戏评估优势，具有低计算需求和多模态观察空间（像素、文本、结构化状态表示），支持幻觉评估和高可定制性。

Result: 在五个基准关卡上评估多种LLM，结果显示LLM与人类专家在能力和幻觉维度存在明显差距，揭示了LLM的关键局限性：规划验证不足、决策缺乏多终局性、动作使用效率低。

Conclusion: TowerMind作为轻量级多模态环境，补充了现有RTS游戏环境，为AI智能体领域引入了新基准，有助于评估LLM在实时策略游戏中的规划和决策能力。
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键领域，语言歧义可能导致严重后果，但现有具身AI研究大多忽视这一问题，假设指令清晰并专注于执行而非确认。为填补这一安全空白，需要开发能够检测3D场景中指令歧义的系统。

Method: 首先定义了开放词汇3D指令歧义检测任务，构建了包含700多个3D场景和约22k指令的Ambi3D基准数据集。然后提出了AmbiVer两阶段框架：第一阶段从多个视角收集显式视觉证据，第二阶段使用这些证据指导视觉语言模型判断指令歧义性。

Result: 分析发现当前最先进的3D大语言模型难以可靠判断指令是否歧义。AmbiVer框架在实验中表现出有效性，为解决这一挑战提供了可行方案。

Conclusion: 该研究填补了具身AI中的安全关键空白，提出的任务、基准和框架为开发更安全、更可信赖的具身AI系统铺平了道路。
...
### [142] [More Power to the Particles: Analytic Geometry for Partial Optimal Transport-based Fluid simulation](https://arxiv.org/abs/2601.05765)
*Cyprien Plateau--Holleville,Bruno Lévy*

Main category: cs.GR

TL;DR: 提出基于部分最优传输的解析几何构造方法，用于自由表面流体模拟和变形力学，通过计算广义拉盖尔单元（球与拉盖尔单元的交集）来精确计算体积和面积，并显著减少计算量。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如Gallouët-Mérigot方案或Power Particles方法）依赖经典凸单元裁剪算法进行离散化，导致计算成本高且对评估量的近似粗糙。需要更高效精确的几何构造方法。

Method: 提出解析算法高效计算广义拉盖尔单元（Laguerre cells与球体的交集），精确计算体积和面积，大幅减少几何构造所需的操作数量。同时提供基于计算体积结构的专用渲染框架。

Result: 新算法能够更精确地计算体积和面积，同时显著减少几何构造所需的操作数量，为自由表面流体模拟和变形力学提供更高效的几何支持。

Conclusion: 提出的解析几何构造方法通过计算广义拉盖尔单元，为基于部分最优传输的流体模拟和变形力学提供了更精确、更高效的几何计算方案，并配有专用渲染框架。
...
### [143] [DexterCap: An Affordable and Automated System for Capturing Dexterous Hand-Object Manipulation](https://arxiv.org/abs/2601.05844)
*Yutong Liang,Shiyi Xu,Yulong Zhang,Bowen Zhan,He Zhang,Libin Liu*

Main category: cs.GR

TL;DR: DexterCap是一个低成本的光学捕捉系统，用于灵巧的手部操作，使用密集字符编码标记点实现遮挡下的鲁棒跟踪，并构建了DexterHand数据集。


<details>
  <summary>Details</summary>
Motivation: 现有手部-物体交互捕捉方法面临挑战：昂贵的光学动捕系统需要大量手动后处理，而低成本视觉方法在遮挡下精度和可靠性不足。需要低成本、高精度的灵巧手部操作捕捉方案。

Method: 提出DexterCap系统：1）使用密集字符编码标记点实现严重自遮挡下的鲁棒跟踪；2）开发自动化重建流程，最小化人工干预；3）构建DexterHand数据集，涵盖从简单到复杂物体的多样化操作行为。

Result: 创建了DexterCap低成本光学捕捉系统和DexterHand数据集，数据集包含丰富的灵巧手部-物体交互数据，支持未来灵巧手部交互研究。

Conclusion: DexterCap解决了灵巧手部操作捕捉的挑战，提供了低成本、高精度的解决方案，并通过开源数据集和代码推动手部-物体交互研究的发展。
