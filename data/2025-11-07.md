...
Method: 采用稀疏体素化表示，设计双模式自编码器，支持任意网格的高分辨率表示，保留锐利特征和内部结构。

Result: 在表示精度方面达到10^-5级别的距离误差；在网格重建方面，Chamfer距离减少93%，F-score提高35%。
...
TL;DR: LoRA-Edge是一种参数高效微调方法，结合低秩适应和Tensor-Train分解，用于边缘设备上CNN的轻量级微调，显著减少可训练参数数量，同时保持接近全微调的精度。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上对CNN进行全微调不可行，因为存在严格的内存、计算和能耗限制。需要一种参数高效的微调方法来应对领域偏移问题，特别是在人类活动识别等应用中。

Method: 1. 对预训练卷积层应用Tensor-Train SVD分解
2. 选择性更新输出侧核心，使用零初始化保持辅助路径初始不活跃
3. 将更新融合回密集核，保持推理成本不变

Result: 1. 与全微调相比，可训练参数减少两个数量级
2. 在多种HAR数据集和CNN骨干上，精度与全微调相差不超过4.7%
3. 在Jetson Orin Nano上，收敛速度提高1.4-3.8倍
...
TL;DR: SILVI是一个开源标注软件，用于在视频数据中标注行为和互动，支持训练和验证计算机视觉模型，填补了现有工具在互动检测方面的空白。
...
Method: 开发SILVI软件，整合行为标注和个体定位功能，直接在视频数据中标注行为和互动，生成结构化输出。

Result: SILVI能够有效标注行为和互动，为训练计算机视觉模型提供合适的数据，促进精细行为分析的自动化方法开发。

Conclusion: SILVI填补了行为生态学与计算机视觉之间的工具空白，虽然主要针对动物行为开发，但也可用于标注人类互动等其他需要提取动态场景图的应用。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在图像识别中容易学习源特定的伪影而非合理的生物标志物，导致在分布外数据上泛化失败，特别是在COVID-19胸部X光检测中。

Method: 在训练过程中应用四种基础噪声注入技术：高斯噪声、斑点噪声、泊松噪声和椒盐噪声，以增强模型对分布变化的鲁棒性。

Result: 噪声注入技术显著减少了分布内和分布外评估之间的性能差距，从0.10-0.20降低到0.01-0.06，在AUC、F1、准确率、召回率和特异性等关键指标上平均十个随机种子的结果。

Conclusion: 基础噪声注入技术是提高COVID-19胸部X光检测模型在分布外数据上泛化能力的有效方法。
...
TL;DR: 该研究探索了在X射线引导的脊柱手术中应用模仿学习策略的可行性，开发了一个模拟环境来训练基于双平面X射线图像的插管对齐策略，在模拟环境中取得了68.5%的首试成功率，并能泛化到复杂解剖结构。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在视频机器人控制中重新受到关注，但尚不清楚这种方法是否适用于X射线引导的脊柱手术，因为多视图X射线的解读很复杂。研究旨在探索模仿策略学习在双平面引导插管插入中的机会和挑战。

Method: 开发了一个高真实度的X射线引导脊柱手术模拟沙盒，收集了正确轨迹和对应双平面X射线序列的数据集，训练了基于视觉信息的模仿学习策略，用于规划和开环控制，逐步对齐插管。

Result: 策略在68.5%的情况下首次尝试成功，在不同椎体水平上保持安全的椎弓根内轨迹，能够泛化到复杂解剖结构（包括骨折），对不同的初始化保持鲁棒性，在真实双平面X射线上的测试也显示出产生合理轨迹的能力。

Conclusion: 初步结果令人鼓舞，但也发现了局限性，特别是在入口点精度方面。完整的闭环控制需要更多关于如何提供足够频繁反馈的考虑。通过更鲁棒的先验知识和领域知识，这类模型可能为未来轻量级、无需CT的机器人术中脊柱导航奠定基础。
...
TL;DR: 提出了一种基于轻量化YOLOv12的实时目标检测框架，结合自对抗训练和专门的数据增强策略，用于沙漠环境中的垃圾检测，在精度和效率方面取得显著提升。
...
Method: 使用修剪后的轻量化YOLOv12版本，集成自对抗训练和专门的数据增强策略，在DroneTrashNet数据集上进行训练和测试。

Result: 在精度、召回率和平均精度方面取得显著改进，同时实现低延迟和紧凑模型尺寸，适合在资源受限的空中无人机上部署。与最先进的轻量化YOLO变体相比，在准确性和效率方面达到最佳平衡。
...
TL;DR: 提出基于类别的图像合成方法，通过将同类多张图像融合为合成输入图像来增强训练数据，显著提升小样本不平衡数据集的分类性能


<details>
  <summary>Details</summary>
Motivation: 解决小样本、不平衡数据集和低质量输入图像导致深度学习模型假阳性率高的问题，增强模型对细微疾病模式的区分能力

Method: Class-Based Image Composition方法，将同类多张图像融合为3x1布局的合成输入图像(CoImg)，构建类别平衡的Co-OCTDL数据集

Result: 在OCTDL数据集上，相比原始数据集，新方法达到99.6%准确率、0.995 F1分数和0.9996 AUC，假预测率显著降低

Conclusion: 该方法即使对于受类别不平衡或小样本影响的弱数据集也能产生高质量预测，显著改善诊断结果
...
TL;DR: 提出了一种无监督、无需专家标注的医学影像异常检测框架，通过增量扩展正常样本集，结合轻量级适配器更新和不确定性门控机制，在多个医学影像数据集上显著提升了异常检测性能。
...
Method: 使用冻结的预训练视觉骨干网络，添加微小卷积适配器进行快速领域适应。通过k近邻异常评分和双重概率门控机制（距离z-score阈值和SWAG认知不确定性边界）来安全扩展正常样本集。
...
Conclusion: 该框架在真实世界标签稀缺的医学影像应用中表现出高效性和有效性，能够随着未标注数据的到来持续改进正常性概念。
...
TL;DR: 提出了两种互补的时间动作定位方法：边界距离回归(BDR)通过有符号距离回归实现信息理论最优定位，自适应时间精炼(ATR)通过连续深度选择分配计算量，在降低计算成本的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前时间动作定位方法对所有边界应用统一计算，忽视了边界检测难度的显著差异，导致计算效率低下。

Method: BDR使用有符号距离回归替代分类，提供信息理论最优的边界定位；ATR通过连续深度选择τ∈[0,1]分配计算量，实现端到端可微分优化。

Result: BDR使边界峰值锐度提升43%，在多种架构上带来1.8-3.1%的mAP@0.7提升；ATR在THUMOS14上以162G FLOPs达到56.5% mAP@0.7，相比均匀处理的53.6%提升2.9%且减少18%计算量。

Conclusion: 该方法通过自适应计算分配和优化的边界回归，在降低计算成本的同时显著提升了时间动作定位性能，特别在边界异质性高的场景下效果更明显。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常在几何精度（多视图立体）和照片级真实感渲染（新视角合成）之间权衡，解耦几何和外观优化阻碍了下游编辑任务。

Method: 同时优化网格几何（顶点位置和面）和顶点颜色，利用高斯引导的网格可微分渲染，结合输入图像的光度一致性和法线深度图的几何正则化。
...
Method: 引入线性分数变换参数α解耦主镜头和微透镜阵列，采用基于最小二乘的解析解和非线性优化，并提出了从原始图像检测特征的方法。
...
Result: 开发了Room Envelopes数据集，支持训练模型同时预测可见表面和结构布局表面。

Conclusion: 该方法能够理解场景的完整范围以及物体的形状和位置，为场景结构元素重建提供了有效解决方案。
...
TL;DR: 人类通过3D姿态信息识别社交互动，而AI视觉模型缺乏这种能力。研究发现3D关节位置比现有AI模型更能预测人类社交判断，且简单的3D社交姿态特征就能达到相同效果。
...
Result: 3D关节位置优于大多数AI视觉模型，简单的3D社交姿态特征（面部位置和方向）与完整关节集具有相同的预测能力，并能提升现成AI模型的性能。
...
TL;DR: CaRF是一个完全可微分的框架，直接在3D高斯空间中操作，通过引入相机感知的高斯场编码和训练中的配对视图监督，实现了多视图一致的3D高斯分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖2D渲染伪监督和视图特定特征学习，导致跨视图一致性不足，需要提升3D高斯场中语言引导分割的视图一致性。

Method: 提出高斯场相机编码(GFCE)将相机几何融入高斯-文本交互，以及训练配对视图监督(ITPVS)对齐不同视图的高斯logits。

Result: 在三个基准测试上平均mIoU分别提升16.8%、4.3%和2.0%，显著优于现有方法。

Conclusion: CaRF实现了更可靠和视图一致的3D场景理解，对具身AI、AR/VR交互和自主感知具有潜在价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成模型虽然视觉质量出色，但经常违反物理规律，表现为物体动态不合理、交互不连贯和运动模式不真实，这限制了在具身AI、机器人等领域的应用。

Method: 提出PhysicsRM双维度奖励模型量化物体内部稳定性和物体间交互，开发PhyDPO直接偏好优化管道，利用对比反馈和物理感知重加权引导生成物理一致的输出。

Result: 在多个基准测试上的广泛实验表明，PhysCorr在物理真实性方面取得显著改进，同时保持视觉保真度和语义对齐。

Conclusion: 这项工作向物理基础和可信的视频生成迈出了关键一步，提出的框架模型无关且可扩展，可集成到各种视频扩散和基于Transformer的骨干网络中。
...
<details>
  <summary>Details</summary>
Motivation: 领域泛化要求视觉Transformer在未见过的领域上保持鲁棒性能，但标准微调成本高且可能损害泛化能力，需要更高效的适应方法

Method: 提出GNN-MoE框架，使用图神经网络（GCN、GAT、SAGE）作为路由器，在补丁间图上动态分配补丁给专家，利用补丁间关系进行上下文感知路由
...
TL;DR: MedDChest是一个专门为胸部影像优化的基础视觉Transformer模型，通过在120万张多模态胸部图像上从头预训练，结合新颖的内容感知数据增强策略，显著超越了基于ImageNet预训练的模型。


<details>
  <summary>Details</summary>
Motivation: 医学影像中视觉模型的性能受到在自然图像上预训练的主干网络领域差距的限制，需要专门针对医学影像领域优化的基础模型。

Method: 提出MedDChest模型，在120万张多模态胸部图像（包括X光和CT）上从头预训练，并开发了引导随机调整裁剪（Guided Random Resized Crops）的内容感知数据增强策略。

Result: 在多个下游诊断任务上的综合实验表明，MedDChest显著优于公开可用的ImageNet预训练模型。

Conclusion: 通过大规模领域内预训练结合领域特定数据增强，MedDChest为各种胸部诊断任务提供了更强大的特征提取器，模型权重将公开以促进未来研究。
...
TL;DR: 提出基于低帧率指尖视频提取PPG信号的轻量级生物认证框架，使用混合深度学习模型CVT-ConvMixer-LSTM，在46个受试者上达到98%的认证准确率。


<details>
  <summary>Details</summary>
Motivation: PPG信号因其无创采集、固有的活体检测能力和低成本可穿戴设备适用性而受到关注，但面临运动伪影、光照变化和个体生理差异等挑战，需要稳健的特征提取和分类方法。

Method: 使用CFIHSR数据集，对原始PPG信号进行预处理（基线漂移去除、PCA运动伪影抑制、带通滤波、傅里叶重采样和幅度归一化），通过连续小波变换将一维PPG段转换为二维时频谱图，开发CVT-ConvMixer-LSTM混合深度学习模型结合空间和时间特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要将神经活动直接与视觉嵌入对齐，但纯视觉表示往往无法捕捉潜在语义维度，限制了可解释性和深度鲁棒性。由于受试者变异性和视觉特征的纠缠性质，从神经信号中揭示视觉语义仍是一个基本挑战。
...
Conclusion: Bratrix通过语言锚定的多模态对齐框架，有效解决了神经信号中视觉语义提取的挑战，提高了对齐精度和模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究在无配对和自监督条件下的CT图像去噪，评估两种训练数据高效的方法，为缺乏干净配对数据的情况提供解决方案。

Method: 使用CycleGAN残差翻译器和Noise2Score分数匹配去噪器两种范式，通过配置扫描确定最佳参数设置（lambda_cycle=30，lambda_iden=2，ngf=ndf=64），并使用标准U-Net骨干网络。

Result: CycleGAN将输入从34.66 dB/0.9234 SSIM提升到38.913 dB/0.971 SSIM，得分1.9441；Noise2Score在绝对PSNR/SSIM上稍逊，但对高噪声输入有显著改善。

Conclusion: CycleGAN提供最强的最终图像质量，而Noise2Score是无配对情况下的稳健替代方案，两者在CT图像去噪中各有优势。
...
TL;DR: UKAST是一种结合KANs和Swin Transformer的U-Net架构，用于医学图像分割，在数据稀缺情况下表现优异，超越了CNN和Transformer基线方法。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临复杂解剖结构和标注数据有限的挑战。CNN擅长局部特征提取但难以建模长距离依赖，Transformer能有效捕捉全局上下文但数据需求大且计算昂贵。

Method: 提出UKAST架构，将基于有理函数的Kolmogorov-Arnold Networks集成到Swin Transformer编码器中，使用Group Rational KANs提高表达能力和数据效率。

Result: 在四个不同的2D和3D医学图像分割基准测试中达到最先进性能，在数据稀缺设置下获得更高精度，FLOPs减少且参数增加很小。
...
Conclusion: SpatialLock能够生成具有精确空间排列的对象，并提高生成图像的视觉质量，为文本到图像生成中的对象定位控制提供了有效解决方案。
...
TL;DR: THG是一种无需训练的扩散采样加速策略，通过将CFG ODE重构为多速率ODE系统，发现额外引导项对数值误差更鲁棒，从而在粗网格上计算引导项，在细网格上计算噪声估计，减少30%函数评估次数且保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散采样方法计算成本高，无法充分利用额外引导项的数值误差鲁棒性，存在计算冗余。

Method: 将CFG ODE重构为多速率ODE系统，噪声估计在细网格（乌龟方程）上计算，额外引导在粗网格（兔子方程）上计算，并引入误差边界感知时间步采样器和引导尺度调度器。

Result: 减少30%函数评估次数，生成质量几乎无损失（ΔImageReward ≤ 0.032），在相同计算预算下优于现有CFG加速器。

Conclusion: 多速率公式为扩散求解器提供了新潜力，为实现无需模型重训练的高质量实时图像合成铺平了道路。
...
TL;DR: 提出基于扩散模型的免训练框架M3S，通过文本提示和参考草图实现精确的草图风格控制，采用线性平滑和风格-内容引导机制减少内容泄漏，支持多风格生成。
...
Method: 基于扩散模型的免训练框架，将参考特征作为辅助信息结合线性平滑，使用风格-内容引导机制，通过联合AdaIN模块支持多风格生成。

Result: 实验表明该方法能实现高质量的草图生成，具有准确的风格对齐和更好的风格控制灵活性，在参考草图与目标草图结构相似度低时表现尤佳。

Conclusion: M3S框架通过创新的风格引导机制有效解决了草图风格控制的精确性问题，为草图生成提供了更灵活和高质量的风格控制方案。
...
TL;DR: 开发了一个完整的自动化网球比赛分析系统，整合多个深度学习模型进行实时球员和网球检测跟踪，并提供详细比赛分析指标。


<details>
  <summary>Details</summary>
Motivation: 为网球比赛提供自动化的全面分析解决方案，帮助教练、转播方和球员获得可操作的比赛洞察。

Method: 使用YOLOv8进行球员检测，定制训练的YOLOv5模型进行网球跟踪，基于ResNet50的架构进行球场关键点检测。

Result: 实验结果显示在不同球场条件和比赛场景下都具有鲁棒性能，系统输出带注释的视频和详细性能指标。

Conclusion: 该框架成功实现了自动化网球比赛分析，能够提供球员移动模式、球速、击球准确度和球员反应时间等详细分析。
...
TL;DR: 提出DMSORT方法，通过双分支并行跟踪器解决海上多目标跟踪中的相机运动问题，结合目标检测、重识别和相机运动补偿，在保持高身份一致性的同时实现最快运行速度。


<details>
  <summary>Details</summary>
Motivation: 复杂海洋环境导致相机运动和视觉退化，给多目标跟踪带来挑战，需要开发能够应对平台运动和视觉噪声的鲁棒跟踪方法。

Method: 采用双分支并行架构：一个分支包含可逆柱状检测网络和轻量Transformer外观提取器，另一个分支通过投影变换解耦平台和目标运动，在卡尔曼滤波中进行运动补偿。

Result: 在新加坡海事数据集上达到最先进性能，在基于ReID的MOT框架中运行速度最快，对抖动和遮挡具有高鲁棒性。

Conclusion: DMSORT通过有效的运动补偿和特征融合，在复杂海洋环境中实现了高效、鲁棒的多目标跟踪，为安全航行和海事监控提供了可靠解决方案。
...
TL;DR: 提出一个框架让计算机使用代理能够从在线视频中学习，通过检索教程视频、转换为结构化演示轨迹，并在执行时动态选择轨迹作为上下文指导，从而提升代理性能。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理在执行需要特定领域程序性知识的复杂任务时仍落后于人类，而人类可以通过观看视频教程来学习。本文旨在让代理也能从在线视频中有效学习。

Method: 使用视觉语言模型推断UI动作，将视频分割为短动作子序列并为每个子序列分配文本目标。采用两阶段选择机制在执行时动态选择最相关的轨迹作为上下文指导。

Result: 在两个广泛使用的基准测试中，该框架始终优于仅使用文本教程或转录的强基线代理和变体。

Conclusion: 在线视频可以系统地提炼为可操作的指导，通过轨迹分割与选择、动作过滤和视觉信息的重要性，显著提升计算机使用代理在推理时的性能。
...
Method: 构建了ORB基准测试（包括英文和11种印度语言的子集），并基于Phi-3.5-Vision的视觉编码器开发了带有动态图像裁剪的轻量级旋转分类管道，专门针对4类旋转任务进行微调。

Result: 旋转分类在英文和印度语言数据集上分别达到96%和92%的准确率；在模拟真实世界设置中，闭源OCR模型性能提升高达14%，开源模型性能提升高达4倍。

Conclusion: 该研究证明了旋转校正模块在提升OCR性能中的关键作用，特别是在处理多语言文档时，提出的方法在准确性和效率方面都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 数字病理学中，不同染色或成像模态图像的准确配准对于生物标志物分析和组织重建等应用至关重要，需要研究颜色变换技术如何影响多模态图像配准性能。

Method: 使用20对组织样本，应用多种预处理步骤（CycleGAN、Macenko、Reinhard、Vahadane颜色变换、反转、对比度调整、强度归一化、去噪），采用VALIS配准方法进行刚性和非刚性配准，通过相对目标配准误差(rTRE)评估性能。

Result: 在原始和反转多模态图像两种场景下，CycleGAN颜色变换均实现了最低的配准误差，其他方法显示出更高的误差。

Conclusion: 配准前应用颜色变换能够改善不同模态图像之间的对齐效果，支持数字病理学中更可靠的分析。
...
Conclusion: 研究证明了将协方差描述符与强大的预训练视觉编码器结合在医学图像分析中的潜力，为医学影像分类提供了有效的新方法。
...
TL;DR: 提出了一种新的自适应统计融合器(AStF)，通过引入偏度和峰度等更高阶统计量来改进运动风格迁移，相比传统仅使用均值和方差的方法能更好地捕捉运动数据的复杂动态模式和时空一致性。
...
Conclusion: 引入更高阶统计量(偏度和峰度)能够显著提升运动风格迁移的效果，为运动数据的风格建模提供了更全面的统计框架。
...
TL;DR: 该论文提出MedSapiens模型，通过将面向人体姿态估计的基础模型Sapiens适配到医学影像解剖标志点检测任务，在多个数据集上达到新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 探索利用面向人体的大规模预训练视觉基础模型进行医学影像解剖标志点检测，这类模型在空间姿态定位方面具有天然优势但尚未被充分利用。

Method: 通过多数据集预训练将人体姿态估计基础模型Sapiens适配到医学影像领域，构建MedSapiens模型。

Result: MedSapiens在多个数据集上相比通用模型提升5.26%，相比专业模型提升21.81%的平均成功检测率；在少样本设置下相比现有最优方法提升2.69%。

Conclusion: 面向人体的基础模型为医学影像解剖标志点检测提供了强大的先验知识，通过适当适配可显著提升检测性能。
...
TL;DR: Proto-LeakNet是一个基于信号泄漏的深度伪造图像溯源框架，通过在扩散模型的潜在空间中检测生成器特定的统计痕迹，实现可解释的源归属分析。
...
Method: 在扩散模型的潜在域中运行，通过部分前向扩散重新模拟来暴露残留的生成器特定线索。使用时间注意力编码器聚合多步潜在特征，特征加权原型头结构嵌入空间并实现透明归属。

Result: 仅使用封闭数据训练，Macro AUC达到98.13%，学习的潜在几何结构在后处理下保持鲁棒，超越现有最先进方法，在已知和未知生成器之间实现强可分性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注模态不变的外观特征，但忽略了步态特征在跨模态匹配中的重要性。步态特征不仅模态不变，还包含丰富的时序动态信息。

Method: 提出语义感知轮廓和步态学习模型(SASGL)，利用DINOv2生成增强的轮廓表示；开发渐进式双向多粒度增强模块(PBMGE)，在多个空间粒度上实现步态和外观流的双向交互。
...
TL;DR: FastGS是一个新颖的3D高斯溅射加速框架，通过基于多视图一致性的致密化和剪枝策略，显著提升训练速度，同时保持渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯溅射加速方法未能有效控制高斯数量，导致冗余计算时间开销。需要解决训练时间和渲染质量之间的权衡问题。

Method: 设计基于多视图一致性的致密化和剪枝策略，摒弃预算机制，充分考虑每个高斯的的重要性。

Result: 在多个数据集上显著优于现有方法，在Mip-NeRF 360数据集上实现3.32倍训练加速，在Deep Blending数据集上实现15.45倍加速，并在各种任务中展现2-7倍的训练加速。
...
TL;DR: 本文提出了一种针对除草剂试验的领域专用视觉基础模型，通过自监督学习在大型农业数据集上训练，显著提升了物种识别和损伤分类的性能，特别是在未见过的环境和无人机图像中表现更佳。


<details>
  <summary>Details</summary>
Motivation: 通用视觉基础模型在农业领域的细粒度物种和损伤类型识别方面性能有限，需要开发领域专用模型来满足除草剂试验的精确分析需求。

Method: 采用自监督学习方法在大型农业数据集上训练领域专用视觉基础模型，优化除草剂试验图像的表示学习。

Result: 领域专用模型在物种识别F1分数从0.91提升到0.94，损伤分类从0.26提升到0.33；在未见条件下提升更显著（物种识别从0.56到0.66，损伤分类从0.17到0.27）；在无人机图像中保持强性能（物种分类从0.49到0.60）。
...
Method: 结合合成和真实Sentinel-1卫星图像训练YOLOv10目标检测模型，并在三个未见区域（墨西哥湾、北海、波斯湾）进行地理迁移性评估。
...
TL;DR: RISE-T2V是一个创新的文本到视频生成框架，通过将提示重述和语义特征提取整合到单一流程中，解决了现有模型对简洁提示理解不足的问题。
...
Method: 提出重述适配器模块，使扩散模型能够利用LLM在下一个词预测过程中的文本隐藏状态作为视频生成条件，将基础提示隐式重述为更全面的表示。

Result: RISE-T2V是一个通用框架，适用于不同的视频扩散模型架构，显著提升了T2V模型生成与用户意图一致的高质量视频的能力。

Conclusion: 该框架通过集成提示重述和语义特征提取，有效解决了文本到视频生成中的语义对齐问题，增强了模型的实用性和扩展性。
...
TL;DR: 提出一种使用体素稀疏化和子流形稀疏卷积网络的两阶段方法，用于医学影像中的肿瘤自动分割，在保持高分辨率输入的同时显著降低计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 放射影像中肿瘤的精确分割是一项专业且耗时的任务，目前是阻碍定量分析在临床常规应用的瓶颈。传统方法处理3D扫描时面临计算资源不足的问题，通常需要降采样或使用图像块。

Method: 采用两阶段方法：体素稀疏化和子流形稀疏卷积网络，允许使用高分辨率输入和原生3D模型架构进行分割。

Result: 在KiTS23挑战赛的肾癌CT图像中，获得了与挑战赛优胜者相当的结果：肾脏+肿块Dice相似系数95.8%，肿瘤+囊肿85.7%，单独肿瘤80.3%。计算效率显著提升：推理时间减少60%，VRAM使用减少75%。

Conclusion: 该方法在保持最先进精度的同时，显著减少了GPU内存和时间的计算资源需求，为医学影像中的肿瘤自动分割提供了高效解决方案。
...
Result: ConvNeXt-Tiny表现最佳，马检测AP达95.53%，摩托车检测AP达89.12%。数据增强显著改善了少数类检测性能，特别是对深层架构帮助更大。

Conclusion: 研究为不平衡二分类任务的架构选择提供了指导，并量化了数据增强在缓解类别不平衡问题中的影响。
...
TL;DR: 研究分析了传感器遮挡对BEVFusion架构3D物体检测性能的影响，发现相机遮挡导致41.3% mAP下降，激光雷达在重度遮挡下下降47.3%，融合模型中激光雷达遮挡影响更大（26.8%下降）。


<details>
  <summary>Details</summary>
Motivation: 虽然BEV融合架构通过多模态集成表现出强大性能，但环境条件（如雾、霾或物理障碍）导致的传感器遮挡对3D检测精度的影响尚未充分探索。

Method: 使用BEVFusion架构在nuScenes数据集上评估相机和激光雷达遮挡的影响，检测性能使用mAP和NDS指标衡量。

Result: 相机遮挡导致mAP下降41.3%（35.6%→20.9%），激光雷达在重度遮挡下mAP下降47.3%（64.7%→34.1%）。融合设置中，相机遮挡仅导致4.1%下降（68.5%→65.7%），激光雷达遮挡导致26.8%下降（至50.1%）。

Conclusion: 模型对激光雷达的依赖更强，需要未来研究关注遮挡感知评估方法和改进的传感器融合技术，以在部分传感器失效或环境条件恶化时保持检测精度。
...
TL;DR: 本教程提供了使用开源深度学习模型从分析化学成像数据中提取空间信息的逐步指南，包括MATLAB代码演示。


<details>
  <summary>Details</summary>
Motivation: 分析化学中虽然常用成像技术获取材料空间信息，但传统化学计量学方法难以有效提取和分析这些信息，且深度学习在分析化学中的应用因缺乏结构化指导而受限。

Method: 使用现有开源深度学习模型从成像数据中提取深度特征，并将其与其他数据源（如光谱信息）集成，提供MATLAB代码教程演示。

Result: 提供了处理分析化学中常见成像模式数据的实用教程，读者可以使用提供的代码在自己的数据集上运行教程步骤。

Conclusion: 该教程填补了深度学习在分析化学应用中的指导空白，使研究人员能够利用现有开源模型从成像数据中提取有价值的空间信息。
...
TL;DR: 提出一个多任务框架，使用LoRA微调的Florence-2模型同时处理视觉问答、解释生成和视觉定位任务，在医学VQA应用中显著提升了答案准确性和视觉定位能力。


<details>
  <summary>Details</summary>
Motivation: 解决医学视觉问答中需要同时提供准确答案、解释和视觉定位的挑战，通过多任务学习提升模型的综合能力。

Method: 使用LoRA微调的Florence-2模型，整合三个数据集：Kvasir-VQA-x1用于问答学习、合成的解释数据集提供结构化医学推理、文本-区域对链接视觉特征与分割掩码。

Result: 广泛评估表明该方法在答案准确性和视觉定位方面显著优于单任务基线模型。

Conclusion: 基于多任务学习的方法在医学VQA应用中有效，能够产生准确且可解释的响应，突出了接地多任务学习的有效性。
...
TL;DR: 提出了一种轻量级单目深度估计模型BoRe-Depth，仅包含870万参数，在嵌入式系统上实现高效深度估计并显著改善边界质量。
...
TL;DR: DORAEMON是一个开源的PyTorch库，统一了不同尺度的视觉对象建模和表示学习，提供单一YAML驱动的工作流，包含1000多个预训练骨干网络，支持分类、检索和度量学习任务。
...
Method: 使用YAML驱动的工作流，通过timm兼容接口暴露预训练骨干网络，结合模块化损失函数、数据增强和分布式训练工具，支持一键导出到ONNX或HuggingFace。

Result: 在ImageNet-1K、MS-Celeb-1M和Stanford在线产品数据集上，可复现的配方达到或超过参考结果。

Conclusion: DORAEMON通过整合视觉识别和表示学习的各个组件，为快速实验提供了可扩展的基础平台，有效连接了研究和部署。
...
<details>
  <summary>Details</summary>
Motivation: 由于章鱼的伪装能力、皮肤纹理颜色快速变化、非刚性形变和频繁遮挡，加上水下光照和浑浊度变化，在自然栖息地分析章鱼具有挑战性。缺乏大规模标注数据集也限制了相关研究。

Method: 集成SAM2与自定义训练的YOLOv11目标检测器。用户提供点坐标生成初始分割掩码，作为YOLO训练数据，然后通过边界框提示实现全自动分割管道。引入时间一致性DICE_t和新组件计数NC_t两个无监督指标。

Result: HideAndSeg实现了令人满意的性能，相比手动提示方法减少了分割噪声。即使在完全遮挡后也能重新识别和分割章鱼，而手动提示模型在此场景下失败。

Conclusion: 该方法减少了真实场景中手动分析的需求，为野生头足类动物行为研究提供了实用工具，开辟了更高效研究途径。
...
TL;DR: 本文提出了一种解决凸多边形拼图的新方法，扩展了传统仅处理方形拼图的限制，结合几何和图像兼容性，并建立了首个此类拼图的基准数据集。


<details>
  <summary>Details</summary>
Motivation: 传统自动拼图求解器主要针对方形拼图，这严重限制了实际应用。本研究旨在显著扩展计算处理的拼图类型，专注于凸分区这一多边形拼图的主要子集。

Method: 利用几何和图像兼容性，引入贪婪求解器，并建立首个凸分区拼图的基准数据集。

Result: 报告了多种性能指标，并提供了此类拼图的第一个基准数据集。

Conclusion: 成功扩展了自动拼图求解器的应用范围，为凸多边形拼图提供了有效的计算方法。
...
TL;DR: V-Thinker是一个通用多模态推理助手，通过端到端强化学习实现交互式视觉中心思考，包含数据进化飞轮和视觉渐进训练课程，在视觉交互推理任务上优于现有LMM基线。
...
Result: 在VTBench基准测试中，V-Thinker在通用和交互推理场景中均优于强大的LMM基线模型。
...
TL;DR: 提出基于地理空间基础模型的三轴分析框架，用于滑坡测绘，在传感器、标签和领域适应性方面优于传统深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 滑坡对生命、基础设施和环境造成严重损害，传统深度学习模型在不同传感器、区域和有限训练数据条件下表现不佳，需要更鲁棒和可扩展的方法。

Method: 采用三轴分析框架（传感器、标签、域）来适配地理空间基础模型Prithvi-EO-2.0，基于全局预训练、自监督和适应性微调。

Result: 模型在光谱变化下保持稳健，在标签稀缺时维持准确性，在不同数据集和地理环境中具有更好的泛化能力，优于U-Net、Segformer等其他方法。

Conclusion: 地理空间基础模型是朝着更鲁棒和可扩展的滑坡风险减少和环境监测方法迈出的一步，但仍面临计算成本和可重用AI训练数据有限等挑战。
...
TL;DR: 提出了"Thinking with Video"新范式，利用视频生成模型（如Sora-2）在统一的时间框架中桥接视觉和文本推理，克服了纯文本和图像推理的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的"Thinking with Text"和"Thinking with Images"范式存在固有局限性：图像只能捕捉单一时刻，无法表示动态过程；文本和视觉作为分离模态阻碍了统一的多模态理解与生成。

Method: 引入"Thinking with Video"范式，使用视频生成模型Sora-2，并开发了VideoThinkBench基准测试，包含视觉中心任务（如Eyeballing Puzzles）和文本中心任务（如GSM8K、MMMU子集）。

Result: Sora-2在视觉中心任务上与SOTA VLMs相当，在Eyeballing Games等任务上甚至超越VLMs；在文本中心任务上，MATH准确率达92%，MMMU达75.53%。自一致性和上下文学习可进一步提升性能。

Conclusion: 视频生成模型具有成为统一多模态理解与生成模型的潜力，"thinking with video"可作为统一的多模态推理范式。
...
TL;DR: 提出一个新的评估框架，包含8个指标来评估说话头部视频生成的三个维度：质量、自然度和同步性，旨在解决现有评估指标不足的问题。


<details>
  <summary>Details</summary>
Motivation: 视频生成技术快速发展，但评估指标的发展滞后。目前说话头部生成的评估主要依赖有限的指标和用户研究，需要更全面的评估框架。

Method: 设计包含8个指标的新评估框架，关注效率以及与人类偏好的一致性，分析头部、嘴巴、眉毛的精细动态以及面部质量。
...
TL;DR: STC-Net是一个用于腹腔镜胆囊切除术中基于Parkland分级标准自动评估手术复杂度的框架，能够在完整手术视频中进行单时间点复杂度估计，无需手动剪辑。


<details>
  <summary>Details</summary>
Motivation: 准确评估腹腔镜胆囊切除术的手术复杂度至关重要，因为严重炎症与更长手术时间和更高并发症风险相关。虽然Parkland分级标准提供了临床验证的炎症严重程度分层框架，但其在手术视频中的自动化应用仍未被充分探索，特别是在需要分析完整视频而非手动剪辑片段的现实场景中。

Method: 提出了STC-Net框架，在弱时间监督下运行，通过定位、窗口提议和分级模块联合执行时间定位和分级。引入了结合硬性和软性定位目标以及背景感知分级监督的新损失函数。

Result: 在1,859个LC视频的私有数据集上评估，STC-Net达到62.11%的准确率和61.42%的F1分数，比非定位基线在两个指标上都高出10%以上，证明了弱监督在手术复杂度评估中的有效性。
...
TL;DR: UniSplat是一个用于自动驾驶动态场景重建的前馈框架，通过统一的潜在时空融合学习鲁棒的动态场景重建，解决了稀疏非重叠相机视图和复杂场景动态的联合挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈3D重建方法在处理稀疏非重叠相机视图和复杂场景动态时存在困难，需要一种能够有效整合空间视图和时间帧信息的方法。

Method: 构建3D潜在支架作为结构化表示，利用预训练基础模型捕获几何和语义场景上下文；引入在3D支架内直接操作的高效融合机制；设计双分支解码器，结合点锚定细化和基于体素的生成来生成动态感知高斯分布；维护静态高斯的持久记忆以实现流式场景补全。

Result: 在真实世界数据集上的广泛实验表明，UniSplat在新视角合成方面达到了最先进的性能，即使在原始相机覆盖范围之外的视点也能提供鲁棒和高质量的渲染。

Conclusion: UniSplat通过统一的潜在时空融合框架，成功解决了动态场景重建中的关键挑战，为自动驾驶等应用提供了有效的3D重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注增强视觉信息处理的粒度，而多模态大语言模型研究表明长文本描述训练能有效改善细粒度视觉语言对齐。但CLIP文本编码器的token长度限制阻碍了其处理更细粒度文本信息的能力。

Method: 1) 建立自动标注流水线生成像素级定位的长文本描述，构建LongGRIT数据集；2) 用LLM替换CLIP原始文本编码器，提出三分支像素-文本对齐学习框架，实现任意粒度的图像区域与文本对齐。
...
TL;DR: 提出了一个基于染色精度的自动化框架来评估虚拟IHC图像质量，发现传统图像保真度指标与染色精度相关性差，配对的图像翻译模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前基于纹理和分布的图像质量评估指标主要量化图像保真度而非IHC染色精度，缺乏可靠的虚拟IHC图像质量评估方法。

Method: 使用颜色反卷积生成IHC阳性像素掩码，通过计算真实和虚拟IHC的Dice、IoU、Hausdorff距离等染色精度指标来量化像素级标记准确性。

Result: 传统指标（FID、PSNR、SSIM）与染色精度相关性差；配对模型（PyramidPix2Pix、AdaptiveNCE）染色精度最高；全玻片图像评估显示基于补丁评估无法发现的性能下降。
...
TL;DR: 提出了一种可扩展的流式视频质量评估模型，该模型无需参考视频且不依赖人工标注，通过时间感知卷积架构直接预测全参考指标。


<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估方法存在局限性：全参考方法需要干净参考视频，大多数无参考方法依赖昂贵的人工标注，而大多数无监督方法基于图像忽略了视频中关键的时间上下文。

Method: 利用DAVIS数据集的合成退化，训练时间感知卷积架构直接从退化视频预测全参考指标（LPIPS、PSNR、SSIM），推理时无需参考视频。

Result: 流式方法优于基于图像的基线，能泛化到多种退化类型；与广泛使用的BRISQUE相比，与全参考指标的相关性更高。

Conclusion: 时间建模对于现实世界视觉系统中可扩展的视频质量评估具有重要价值，提出的时间感知无监督方法有效。
...
TL;DR: 偏振分辨近红外成像通过测量眼组织反射光的偏振状态，为眼动追踪提供了额外的光学对比机制，相比仅基于强度的系统能减少10-16%的注视误差。


<details>
  <summary>Details</summary>
Motivation: 传统的基于强度的眼动追踪系统在存在眼睑遮挡、眼距变化和瞳孔大小变化时性能受限，需要新的对比机制来提高鲁棒性。

Method: 使用偏振滤光片阵列相机配合线性偏振近红外照明器构建偏振眼动追踪系统，通过卷积神经网络分析偏振图像中的可追踪特征。

Result: 在346名参与者的测试中，PET系统在正常条件和各种干扰情况下，将95%分位绝对注视误差的中位数降低了10-16%。

Conclusion: 偏振眼动追踪是一种简单、鲁棒的传感方式，能够显著提升人机交互性能，适用于未来可穿戴设备。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准测试存在严重问题，模型可以通过利用偏置、语言先验和表面模式来获得高分，而不需要真正的视觉理解能力，这削弱了基准测试的有效性。

Method: 采用测试集压力测试(TsT)方法，通过仅使用测试集的文本输入对LLM进行k折交叉验证来识别可被利用的模式，并结合随机森林诊断；然后使用迭代偏置剪枝(IBP)过滤高偏置样本。

Result: 在四个基准测试(VSI-Bench、CV-Bench、MMMU、VideoMME)中发现普遍存在的非视觉偏置，并通过去偏处理创建了VSI-Bench-Debiased，显著降低了非视觉可解性。

Conclusion: 基准测试设计应采用主动诊断和去偏方法，通过"先攻击自己的基准"原则来确保评估的有效性，提出的框架能有效识别和缓解非视觉偏置问题。
...
TL;DR: SIMS-V框架利用3D模拟器生成空间丰富的视频训练数据，通过系统消融实验发现三种关键问题类型（度量测量、视角依赖推理和时间追踪）能最有效地培养可迁移的空间智能，仅用25K模拟样本训练的7B模型在真实世界空间推理基准上超越了72B基线模型。


<details>
  <summary>Details</summary>
Motivation: 多模态语言模型在时空空间推理方面存在困难，而获取具有精确空间标注的多样化真实视频数据存在瓶颈，因此需要利用模拟器的特权信息来生成空间丰富的训练数据。

Method: 提出SIMS-V系统数据生成框架，利用3D模拟器创建空间丰富的视频训练数据，通过系统消融实验分析问题类型、混合方式和规模对真实世界迁移效果的影响。

Result: 识别出三种最有效的问题类别，仅用25K模拟样本训练的7B参数视频LLM在真实世界空间推理基准上超越了72B基线模型，并与专有模型竞争，同时保持通用视频理解性能。

Conclusion: 模拟数据可以有效培养可迁移的空间智能，通过精心选择的问题类型可以实现高效训练，在保持通用能力的同时显著提升具身和真实世界空间任务的性能。
...
TL;DR: 论文提出了从反应式任务驱动系统转向空间超感知的新范式，包含语义感知、流式事件认知、隐式3D空间认知和预测性世界建模四个阶段，并开发了VSI-SUPER基准测试来推动这一方向的发展。
...
Method: 提出了空间超感知的四个阶段框架，开发了VSI-SUPER基准测试（包括VSR和VSC任务），训练了Cambrian-S模型，并探索了预测性感知方法。

Result: 通过数据扩展在VSI-Bench上获得了30%的绝对提升，但在VSI-SUPER上表现仍然有限；预测性感知方法显著优于领先的专有基线模型。

Conclusion: 仅靠规模扩展不足以实现空间超感知，需要能够预见、选择和组织经验的预测性感知模型。
...
Result: 在VBench上获得83.74分，大幅超越所有自回归模型，甚至超过HunyuanVideo等扩散模型；无需额外优化即可生成5秒720p视频，速度比领先的扩散方法快10倍。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的物体经常经历状态转换（如苹果被切成块、蝴蝶破茧而出），现有方法在物体外观发生显著变化时容易丢失跟踪目标，需要解决这一限制。

Method: TubeletGraph系统首先识别可能被忽略的轨迹，基于语义和邻近性先验决定是否整合；然后推理添加的轨迹并生成描述每个观察到的转换的状态图。

Result: TubeletGraph在状态转换下实现了最先进的跟踪性能，同时展示了对物体转换的深入理解，以及在时间定位和复杂物体转换语义推理方面的有前景能力。

Conclusion: 该方法成功解决了物体状态转换过程中的跟踪问题，提供了对物体状态演化的深度理解，并为复杂物体转换的语义推理和时间定位开辟了新途径。
...
TL;DR: 本文提出了一种自动图像裁剪方法，旨在生成多个具有美学吸引力的不同裁剪区域，并引入了一个包含277张相关图像和人工标签的数据集。


<details>
  <summary>Details</summary>
Motivation: 现代社交媒体应用需要生成多个不同的美学裁剪区域，而现有工作主要关注单一裁剪，缺乏对多裁剪问题的研究。

Method: 使用图像分割算法作为预处理步骤，评估了几种单裁剪模型在多裁剪场景下的效果。

Result: 构建了一个包含277张图像和人工标签的数据集，可用于多裁剪问题的研究。

Conclusion: 本文为多裁剪问题提供了数据集和初步评估方法，填补了该领域的研究空白。
...
TL;DR: 提出了一个新颖的管道，通过提取基于大五人格特质的隐藏状态激活，发现低秩子空间，并识别特质特定最优层，实现LLM人格对齐的精确控制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成中表现出隐含的人格特质，但可靠地控制或对齐这些特质以满足特定需求仍是一个开放挑战，需要有效的行为操纵机制。

Method: 使用大五人格特质框架提取transformer层的隐藏状态激活，应用低秩子空间发现方法，识别特质特定最优层，并通过灵活的控制框架实现人格对齐方向的精确控制。

Result: 发现人格特质占据低秩共享子空间，这些潜在结构可以通过精心设计的扰动转化为有效的控制机制，而不影响流畅性、方差和一般能力。
...
<details>
  <summary>Details</summary>
Motivation: TextGrad作为文本自动微分方法缺乏自我验证机制，无法确保文本推理的有效性。本研究旨在填补这一验证空白，提升文本决策的可靠性。

Method: 采用四阶段工作流：思维链分解、变体生成、多数投票和共识聚合。使用Gemini 1.5 Pro模型进行实验评估，包括独立验证和与TextGrad集成验证两个阶段。

Result: 在PRM800K上推理步骤有效性提升29%；与TextGrad集成后在GPQA-Diamond、MMLU-ML和MMLU-CP基准上分别获得8.08、10.71和3.92个百分点的改进；集成到TextGrad损失函数中使准确率从68.2%提升至70.4%。
...
TL;DR: 扩展希腊方言数据集GRDD+，新增6种希腊方言变体，总规模达637万词，是迄今最大最全的希腊方言数据集。通过微调实验验证高质量方言数据对多种大语言模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 现有希腊方言数据集覆盖范围有限，需要构建更大规模、更多样性的方言数据集来支持方言语言模型的研究和发展。

Method: 扩展GRDD数据集，新增克里特、塞浦路斯、庞蒂克和北希腊方言数据，并加入六种新变体：希腊-科西嘉语、格里科语、马尼奥语、赫普塔尼西亚语、察科尼亚语和卡萨雷武萨希腊语。对三种模型架构进行微调实验。

Result: 构建了包含10种希腊方言变体、总计6,374,939词的数据集，这是迄今规模最大、变体最丰富的希腊方言数据集。通过微调实验比较了不同模型在方言任务上的表现。

Conclusion: GRDD+数据集为希腊方言研究提供了重要资源，高质量方言数据能有效提升语言模型在方言任务上的性能，为方言语言技术的发展奠定了基础。
...
Conclusion: PLLuM为波兰语提供了专门的基础模型，通过负责任AI框架和严格数据治理，促进了开放研究和主权AI技术的发展。
...
TL;DR: STARS是一种解码时算法，通过迭代采样、评分和拒绝/接受固定大小的短标记段来引导模型生成，显著提高计算效率和对齐质量。
...
Method: STARS采用段级标记对齐与拒绝采样，在解码时对短固定大小的标记段进行迭代采样、评分和拒绝/接受，实现生成路径的早期修正。

Result: 在六个LLM上的测试表明，STARS在胜率上比监督微调高出14.9个百分点，比直接偏好优化高出4.3个百分点，与强大的Best-of-N基线保持高度竞争力。

Conclusion: STARS建立了细粒度、奖励引导的采样作为一种通用、鲁棒且高效的替代方案，用于对齐大语言模型，优于传统的微调和全序列排序方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在多标签分类任务中的效率问题，特别是针对短文本推理场景，同时保持分类准确性。

Method: 将多标签分类分解为独立的二分查询，采用前缀缓存机制优化推理效率，并通过LLM-to-SLM蒸馏方法，使用DeepSeek-V3生成标注数据来微调小型模型。

Result: 微调后的模型在训练维度上显著优于零样本基线，证明了该方法的有效性，特别是在情感文本分析任务中。
...
TL;DR: 本文分析了三种低资源语言（阿法尔奥罗莫语、阿姆哈拉语、提格里尼亚语）机器翻译数据集的质量问题，重点关注性别代表性，发现数据存在男性偏向、有害内容和领域不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 随着低资源语言被纳入NLP研究，大规模数据集收集过程中存在重数量轻质量的风险，可能导致技术表现不佳和产生有害内容，需要关注数据集质量特别是性别代表性。

Method: 研究三种低资源语言的机器翻译数据集，分析性别代表性，包括人名、动词语法性别和刻板印象描绘，以及有害内容的存在情况。

Result: 发现训练数据主要集中在政治和宗教领域，而基准数据集聚焦于新闻、健康、体育；数据存在严重的男性偏向，在动词语法性别、人名和刻板印象中都有体现；发现了针对女性的有害和有毒内容，且数据量最大的语言问题最突出。

Conclusion: 数量不能保证质量，需要进一步研究低资源语言数据集的质量问题，并及早缓解有害内容。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部知识源，但基于提示的接地方法脆弱且领域敏感，符号知识集成则带来高昂的检索和格式化成本。受知识图谱启发，作者希望开发一种轻量级方法，在不重新训练的情况下将生成过程基于语料库证据。

Method: GRAD通过单次前向传播在小型检索语料库上累积下一个令牌对数概率来构建稀疏令牌转移图。在解码时，图检索的对数概率经过最大归一化处理，并与模型对数概率自适应融合，以偏好高证据延续同时保持流畅性。

Result: 在三个模型和多个问答基准测试中，GRAD始终优于基线方法：内在准确率提高9.7%，幻觉率降低8.6%，正确性提高6.9%，在所有方法中获得了最高的真实性-信息性乘积分数。

Conclusion: GRAD提供了一种轻量级、即插即用的替代方案，证明了来自语料库级别令牌转移的统计证据可以有效引导生成朝向更真实和可验证的输出。
...
TL;DR: 论文研究了迭代参考游戏中人类和视觉语言模型的性能差异，发现在相关上下文条件下模型表现显著提升，但抽象指称的少样本参考游戏对机器学习模型仍是挑战


<details>
  <summary>Details</summary>
Motivation: 测试智能体在多轮语言环境中执行上下文敏感语用推理的能力，通过迭代参考游戏比较人类和模型的差异

Method: 使用迭代参考游戏测试人类和视觉语言模型，通过改变上下文的数量、顺序和相关性来评估性能

Result: 无相关上下文时模型表现高于随机但远差于人类；有相关上下文时模型性能随试验次数显著提升；抽象指称的少样本参考游戏对模型仍是难题

Conclusion: 上下文相关性对模型性能至关重要，但抽象指称的少样本参考游戏仍是机器学习模型面临的挑战
...
TL;DR: 开发了人类繁荣地理指数(HFGI)，通过分析26亿条地理定位的美国推文，使用微调的大型语言模型对48个繁荣指标进行分类，提供县级和州级的高时空分辨率繁荣测量。
...
Method: 分析2013-2023年约26亿条地理定位的美国推文，使用微调的大型语言模型对48个与哈佛全球繁荣研究框架一致的指标进行分类，包括对移民态度和腐败感知。
...
TL;DR: 该论文提出了一种通过向量翻译在多智能体系统中实现跨模型语义通信的方法，而不是传统的token传递方式。


<details>
  <summary>Details</summary>
Motivation: 在多智能体设置中，语言模型通过纯token传递消息会丢弃大部分潜在语义，限制了信息传输并增加了不必要的计算开销。

Method: 使用双编码器翻译器在Llama-2-7B和Mistral-7B-Instruct之间学习映射，实现表示空间之间的直接语义交换。

Result: 训练得到的翻译器平均余弦对齐度为0.538，以30%混合强度注入翻译向量可以引导目标模型生成而不破坏logits稳定性。双向评估显示2.01:1的传输不对称性。

Conclusion: 这种保守的注入方法保持了计算稳定性，证明了跨模型潜在通信是可行的，使协作AI系统能够共享意义而非token。
...
TL;DR: 提出了一个将溯因推理整合到检索增强LLMs中的框架，通过检测证据不足、生成候选缺失前提并进行验证，提高了RAG系统的准确性和推理可信度。


<details>
  <summary>Details</summary>
Motivation: RAG管道在检索证据不完整时经常失败，导致推理过程中存在空白。溯因推理可以生成合理的缺失前提来弥补这些空白。

Method: 检测证据不足，生成候选缺失前提，通过一致性和合理性检查进行验证。

Result: 在溯因推理和多跳QA基准测试中，该方法提高了答案准确性和推理可信度。
...
TL;DR: 提出了一种弱监督转换器（WST），能够在转录错误率高达70%的情况下保持性能，优于现有的基于CTC的弱监督方法。
...
Method: WST集成了一个灵活的训练图，能够鲁棒地处理转录错误，无需额外的置信度估计或预训练模型。

Result: 在合成和工业数据集上的实验表明，WST在高达70%的转录错误率下仍能有效保持性能，优于BTC和OTC等方法。

Conclusion: WST在实际ASR设置中具有实用性和鲁棒性，代码将公开提供。
...
TL;DR: 提出了T-FIX基准来评估LLM解释与专家判断的对齐程度，专注于知识密集型领域的专家级解释质量


<details>
  <summary>Details</summary>
Motivation: 当前LLM解释评估主要关注合理性和内部忠实性，但缺乏对解释内容是否真正符合专家直觉的评估，特别是在知识密集型领域

Method: 与领域专家合作开发了T-FIX基准，涵盖七个知识密集型领域，并设计了新的指标来衡量LLM解释与专家判断的对齐度

Result: 开发了T-FIX基准和相应的评估指标

Conclusion: 需要更关注LLM解释与专家判断的对齐性，特别是在知识密集型应用场景中
...
TL;DR: 提出了PoK框架，通过知识规划和对比时间检索器来增强LLMs在时间知识图谱问答中的表现，显著提升了检索精度和推理准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分理解时间约束的复杂语义信息，而LLMs在时间推理能力上存在局限性，容易出现幻觉和知识缺失问题。

Method: 提出知识规划模块将复杂时间问题分解为子目标序列，并构建时间知识存储库与对比检索框架，选择性检索语义和时间对齐的事实。

Result: 在四个基准TKGQA数据集上的实验表明，PoK显著提升了检索精度和推理准确性，最多超越最先进方法56.0%。

Conclusion: 通过结合结构化规划和时序知识检索，PoK有效增强了时间推理的可解释性和事实一致性。
...
TL;DR: 比较人类与大型语言模型在情感词汇联想上的表现差异，发现LLMs的联想与人类有中等重叠，但更可预测、缺乏创造性，且会放大情感负荷


<details>
  <summary>Details</summary>
Motivation: 探索人类与大型语言模型在词汇联想行为上的相似性，特别是对情感词汇的联想，以了解LLMs是否能像人类一样形成关联

Method: 通过比较人类参与者和大型语言模型对情感词汇的联想反应，分析其重叠度、可预测性和创造性

Result: 人类与LLMs的联想重叠度中等，但LLMs的联想更可预测、缺乏创造性，且会放大刺激词的情感负荷

Conclusion: 大型语言模型在词汇联想方面与人类存在显著差异，其联想行为更加模式化且缺乏人类特有的创造性关联能力
...
<details>
  <summary>Details</summary>
Motivation: 提升放射学报告的自动去标识化能力，通过扩展训练数据集和基准测试来改进受保护健康信息(PHI)的检测性能

Method: 在现有最先进的Transformer-based PHI去标识化流水线基础上，使用斯坦福大学两个大型注释放射学语料库进行微调，引入额外的PHI类别(AGE)，并评估模型在斯坦福和宾夕法尼亚大学测试集上的表现

Result: 模型在Penn数据集上总体F1得分0.973，在斯坦福数据集上0.996，优于或保持先前最先进模型性能；在合成PHI评估中表现稳定(总体F1:0.959)，在所有供应商系统中表现最佳

Conclusion: 基于Transformer的去标识化模型在多样化放射学数据集上训练后，在PHI检测方面超越了先前的学术和商业系统，为安全临床文本处理建立了新基准
...
TL;DR: 本文研究了语言识别问题，提出了k-列表识别的新框架，给出了精确的数学特征，并建立了统计学习环境下的收敛速率界限。


<details>
  <summary>Details</summary>
Motivation: 经典的语言识别在极限情况下对于大多数有趣的语言集合是不可能的。受最近语言生成问题的积极结果启发，本文重新审视了经典问题，通过赋予学习者每次输出k个猜测的能力来克服传统限制。

Method: 提出了k-列表识别框架，基于Angluin特征的递归版本给出了精确的数学特征，并建立了该特征与可分解为k个可识别语言集合的等价关系。

Result: 证明了语言集合可k-列表识别的充要条件是该集合可分解为k个可识别（列表大小为1）的语言集合。在统计设置下，建立了指数收敛速率的最优性。

Conclusion: k-列表识别为语言识别提供了新的可能性，通过放宽输出要求克服了经典限制，并建立了理论上的最优收敛速率。
...
TL;DR: 批处理提示不仅降低LLM推理成本，还能作为推理时正则化器，提高大型推理模型在多步推理中的准确性和效率，减少3-5倍的推理token使用，抑制过度思考并产生更果断的答案。


<details>
  <summary>Details</summary>
Motivation: 探索批处理在大型语言模型中的额外益处，特别是其在多步推理中对模型行为的正则化作用，而不仅仅是作为吞吐量优化手段。

Method: 在13个多样化基准测试上进行全面研究，通过详细的行为分析考察批处理对模型推理行为的影响，包括过度思考抑制、犹豫语言减少和集体效应观察。

Result: 批处理显著提高了准确性，同时大幅减少了3-5倍的推理token使用，抑制了过度思考，减少了重复自我修正等犹豫语言，并观察到模型在批次内从简单示例泛化模式解决困难示例的集体效应。

Conclusion: 批处理不仅是吞吐量优化工具，更是一个强大的推理时正则化器，能够实现更高效可靠的LLM推理。
...
TL;DR: RIDE是一个基于项目反应理论的对抗性问题重写框架，通过生成内在更难的数学问题变体来评估LLMs的真实数学推理能力，而不是表面模式匹配。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在数学推理上的高表现可能源于训练数据泄露或表面模式匹配，而非真正的推理能力。需要对抗性评估来测量真实的数学推理能力。

Method: 使用项目反应理论(IRT)严格测量问题难度，利用35个LLMs模拟学生构建难度排序器，通过强化学习指导问题重写模型生成不同难度级别的问题变体。

Result: 在竞赛级数学基准测试中，RIDE生成的扰动版本使先进LLMs性能平均下降21.73%，暴露出数学推理的有限鲁棒性。

Conclusion: RIDE验证了评估方法的有效性，揭示了LLMs在数学推理中的局限性，为更真实的数学推理能力评估提供了新框架。
...
TL;DR: CantoASR是一个协作式粤语语音识别框架，结合了强制对齐、LoRA微调的Whisper模型和指令调优的Qwen-Audio，通过整合声学线索和大型音频语言模型推理来提升低资源粤语ASR性能。


<details>
  <summary>Details</summary>
Motivation: 低资源粤语语音识别面临标注数据有限、六种声调、变调和口音变化等挑战，现有模型如Whisper存在高词错误率问题，需要更好的声调和韵律声学线索处理。

Method: 提出协作式ASR-LALM错误修正框架，包括：强制对齐提取声学特征、LoRA微调Whisper改进声调识别、指令调优Qwen-Audio进行韵律感知修正。

Result: 在自发粤语数据评估中，相比Whisper-Large-V3取得了显著的字符错误率提升。

Conclusion: 整合声学线索与LALM推理为低资源声调和方言ASR提供了可扩展策略。
...
TL;DR: 本文探索了三种多智能体LLM管道用于Text-to-SQL任务，发现多智能体讨论能提升小模型性能，其中LLM推理器-编码器管道表现最佳，可将准确率从52.4%提升至56.4%。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在从自然语言生成SQL时面临模式规模大和复杂推理的挑战，先前工作多关注复杂不实用的旗舰模型管道，而忽视了更小、更高效的模型。

Method: 提出三种多智能体LLM管道：(1)多智能体讨论管道，智能体迭代批评和优化SQL查询，法官合成最终答案；(2)规划器-编码器管道，思考模型规划器生成逐步SQL生成计划，编码器合成查询；(3)编码器-聚合器管道，多个编码器独立生成SQL查询，推理智能体选择最佳查询。
...
Conclusion: 多智能体方法能有效提升Text-to-SQL任务的性能，特别是对于较小模型，其中规划器-编码器管道表现最为出色，为高效SQL生成提供了实用解决方案。
...
TL;DR: LAAC提出将LLM作为智能通信中介的新范式，通过结构化对话捕捉发送者意图，促进真实知识交换，而非AI生成内容的膨胀-压缩循环。


<details>
  <summary>Details</summary>
Motivation: 解决AI生成内容泛滥导致的虚假沟通剧场问题，发送者用LLM将简单想法膨胀为冗长内容，接收者用LLM压缩回摘要，双方都不接触真实内容。

Method: 采用多智能体架构，通过结构化对话提取发送者意图，评估三个信任维度：信息捕获保真度、可重现性和查询响应完整性。

Result: 初步发现存在可测量的信任差距，需要在高风险通信场景部署前解决。

Conclusion: LAAC有潜力改变通信范式，但需解决信任问题才能可靠部署于高风险通信场景。
...
TL;DR: 本文提出了一个计算图灵测试框架来验证LLM生成文本的人类相似性，并系统比较了9个开源LLM在5种校准策略下的表现，发现即使经过校准，LLM输出仍与人类文本存在明显差异。


<details>
  <summary>Details</summary>
Motivation: 当前社会科学中广泛使用LLM模拟人类行为，但缺乏对LLM生成文本真实性的可靠验证方法，现有基于人类判断的评估方法存在局限性。
...
TL;DR: 当前大语言模型无法通过波兰国家上诉庭资格考试，尽管在知识测试中表现尚可，但在实践写作部分均未达标，且LLM作为评判者的评估与官方委员会存在分歧。


<details>
  <summary>Details</summary>
Motivation: 评估当前大语言模型是否能够通过波兰国家上诉庭的官方资格考试，探索LLM作为考生和自动评判者的可行性。

Method: 构建混合信息检索和提取管道，在闭卷和检索增强生成设置下测试多个LLM（包括GPT-4.1、Claude 4 Sonnet和Bielik-11B-v2.6），采用LLM作为评判者方法自动评估模型生成的答案。

Result: 模型在知识测试中取得满意分数，但在实践写作部分均未达到及格线，LLM作为评判者的评估与官方审查委员会的判断经常存在差异。
...
TL;DR: REMIND是一种新颖的机器学习遗忘评估方法，通过分析模型在输入微小变化时的损失模式来检测未学习数据的残余影响，比现有单点评估方法更敏感和可靠。
...
Method: REMIND通过分析模型在输入微小变化时的损失动态来检测残余记忆，发现未学习数据会产生更平坦、不陡峭的损失景观，而保留或无关数据则表现出更尖锐、更易变的模式。

Result: REMIND在仅需查询访问的情况下优于现有方法，在不同模型、数据集和改写输入上表现出鲁棒性，适合实际部署。

Conclusion: REMIND为语言模型中的遗忘评估提供了一个更敏感、可解释的可靠框架，为记忆和遗忘研究提供了新的视角。
...
TL;DR: 本文通过检索增强生成和测试时计算来量化预训练过程中未充分利用的数据集价值，发现在MMLU、Math-500和SimpleQA等任务上，预训练后从标准开源数据集检索能显著提升准确率，检索相当于5倍计算乘数。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注改进预训练数据集，但很少理解预训练机制从数据中提取知识和思想的效率如何，以及这种效率如何随模型规模变化。

Method: 使用检索增强生成和测试时计算作为量化工具，评估预训练后从标准开源数据集检索的价值，并通过去污染验证结果的可靠性。

Result: 在MMLU上检索相当于5倍计算乘数，对LLaMA 3.1 8B模型在MMLU上使用额外测试时计算解析检索内容可提升10个百分点准确率。
...
TL;DR: 提出一种基于图的方法进行主题标注，通过丰富主题词并分析其关系来生成有意义的标签，相比传统方法在BERTScore和余弦相似度上表现更好，与ChatGPT-3.5效果相当但计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 现有主题建模方法生成的主题词分布缺乏清晰可解释性，且大多数方法计算成本高昂。本文旨在开发一种计算效率高的主题标注方法，为自动发现的主题分配有意义的标签。

Method: 采用基于图的方法，通过丰富主题词的语义相关词汇并分析词汇间的关系，从图中推导出准确捕捉主题含义的合适标签。

Result: 在两个不同数据集上的比较研究表明，该方法在BERTScore和余弦相似度上持续优于传统基准方法，与ChatGPT-3.5效果相当，同时保持计算效率。

Conclusion: 基于图的主题标注方法在保持计算效率的同时实现了良好的标注效果，为提升主题建模的可解释性和自动化提供了有前景的研究方向。
...
TL;DR: SSPO提出句子级重要性比率，平衡GRPO和GSPO，避免训练崩溃和低数据利用率问题，在五个数据集上平均得分46.57，优于GRPO(43.01)和GSPO(44.42)。
...
Conclusion: SSPO通过吸收GSPO的优点但摒弃其缺点，有效利用了生成数据，在推理任务中表现出色。
...
TL;DR: 提出了一种基于可学习性评分的数据选择方法，用于机器翻译模型的微调，通过学习者模型与预训练参考模型的协同作用来提升训练效率。
...
Method: 定义可学习性评分来系统评估数据点对训练的价值，采用考虑数据点间相互依赖关系的批次选择策略，优化训练效率同时保持数据相关性。

Result: 在英译波斯等语言对上的实验表明，该方法相比iid基线实现了高达5倍的数据效率提升，使用缓存嵌入时计算效率提高24%，并获得了优于随机选择方法的翻译性能。

Conclusion: 该方法能够显著提高机器翻译模型的数据效率和计算效率，同时改善泛化能力，为构建高效的翻译系统提供了有效的数据选择方案。
...
TL;DR: 本研究测试LLMs的时间推理能力，使用1940年挪威书籍中的琐事问题，让LLMs以1940年的视角回答问题，并比较英文和挪威文提示的效果。
...
Method: 使用1940年挪威书籍中的琐事问题，让LLMs以1940年的视角用英文和挪威文回答问题，采用LLM-as-judge进行评估，并由母语者抽样检查。

Result: 英文提示始终比挪威文提示效果更好，这是一个意外结果；使用更大的LLMs能改善结果；测试了DeepSeek-R1、Gemma3、Qwen3、Llama3.1等模型家族以及专门为挪威语定制的大型LLM。

Conclusion: LLMs在时间推理方面表现出一定能力，但语言选择对性能有显著影响，英文提示优于挪威文提示，模型规模对结果有积极影响。
...
TL;DR: PTTSD是一个概率性文本时间序列抑郁症检测框架，通过临床访谈的逐句内容预测PHQ-8抑郁评分，同时建模时间不确定性，在多个数据集上达到最先进的性能。
...
Method: 提出PTTSD框架，包含序列到序列和序列到单值两种变体，结合双向LSTM、自注意力机制、残差连接，使用高斯或学生t分布输出头，通过负对数似然进行训练。

Result: 在E-DAIC和DAIC-WOZ数据集上，PTTSD在纯文本系统中达到最先进性能（MAE分别为3.85和3.55），产生良好校准的预测区间，消融实验确认了注意力和概率建模的价值。

Conclusion: 该框架通过三部分校准分析和定性案例研究，突显了不确定性感知预测的可解释性和临床相关性，为抑郁症检测提供了更可靠的工具。
...
TL;DR: ThaiOCRBench是首个评估视觉语言模型在泰语文本丰富视觉理解任务上的综合基准，包含2,808个样本和13个任务类别，揭示了专有模型优于开源模型的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注高资源语言，泰语在需要文档结构理解的任务中代表性不足，需要专门基准来评估VLMs在低资源、复杂文字环境下的表现。

Method: 构建了多样化、人工标注的数据集，在零样本设置下评估了广泛的专有和开源VLMs，并进行详细的错误分析。

Result: 专有模型（如Gemini 2.5 Pro）表现优于开源模型，特别是在细粒度文本识别和手写内容提取任务中开源模型性能下降最明显。

Conclusion: ThaiOCRBench为评估低资源、复杂文字环境下的VLMs提供了标准化框架，并为改进泰语文档理解提供了可行的见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有表格推理基准测试主要使用小型、统一的表格，无法反映真实世界数据的复杂性，不能全面评估大型语言模型的推理能力。真实表格通常很长、异构且领域特定，需要跨数千个token的多跳推理。

Method: 构建RUST-BENCH基准测试，包含来自NSF资助记录的科学领域和NBA统计数据的体育领域的2031个真实表格，共7966个问题，评估LLM在规模、异构性、领域特定性和推理复杂性四个维度上的表现。
...
TL;DR: 本文提出了基于LLM提示的OUNLP系统，用于TSAR-2025共享任务中的可读性控制文本简化。通过分析发现文本简化性能与源和目标CEFR级别差距相关，提出了两种多轮简化方法，在20个团队中排名第7。
...
Method: 提出了两种基于GPT-4o的多轮简化方法：基于规则的简化(MRS-Rule)和联合基于规则的LLM简化(MRS-Joint)。

Result: 提交的系统在20个团队中排名第7。后续改进显示，将LLM简化候选作为起点可以进一步提升多轮简化性能。

Conclusion: 多轮简化方法有效，特别是将LLM简化结果作为起点可以显著提升性能，证明了源-目标CEFR级别差距对文本简化效果的重要性。
...
TL;DR: 本文系统评估了6个大型语言模型在BFI-2人格框架下的表现，发现不同模型在四个主要人格维度上存在显著差异，神经质和外向性对温度调整敏感，架构特征影响人格稳定性。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在人类中心应用中的普及，理解其类人格行为对于负责任开发和部署AI系统至关重要。

Method: 应用BFI-2人格框架评估6个LLMs在不同采样温度下的人格特质表达，使用层次聚类分析模型特征。

Result: 发现四个主要人格维度存在显著差异，神经质和外向性对温度变化敏感，聚类分析显示不同架构模型具有稳定的人格特征模式。

Conclusion: 研究为理解LLMs中类人格模式的出现提供了新视角，对模型调优、选择和AI系统伦理治理具有重要意义。
...
TL;DR: RAGalyst是一个自动化、与人类对齐的代理框架，用于严格评估领域特定的检索增强生成(RAG)系统，通过生成高质量合成QA数据集和优化LLM-as-a-Judge指标来实现与人类判断的强相关性。
...
Method: 开发代理管道生成高质量合成QA数据集，包含代理过滤步骤确保数据保真度；通过提示优化精炼两个关键LLM-as-a-Judge指标(答案正确性和可回答性)，实现与人类标注的强相关性。
...
TL;DR: 该论文提出了一个处理放射学报告中不确定性的两阶段框架，量化显性不确定性并建模隐性不确定性，发布了增强版Lunguage++基准。


<details>
  <summary>Details</summary>
Motivation: 放射学报告对临床决策至关重要，但包含两种不确定性：显性不确定性（通过模糊表达体现）和隐性不确定性（因推理过程省略而产生），现有规则系统无法有效处理这些不确定性。

Method: 1. 量化显性不确定性：创建专家验证的LLM参考排名，将常见模糊短语映射到概率值；2. 建模隐性不确定性：通过扩展框架系统添加专家定义诊断路径中的特征性子发现。
...
TL;DR: 通过控制语言模型的隐藏激活来预测和控制其在思维链推理中的不确定性，发现模型在不确定性高时更容易被引导，表明模型隐式表示可能的推理路径。


<details>
  <summary>Details</summary>
Motivation: 语言模型在生成文本时，单个token的选择可能导致不同的推理路径，使得不确定性难以量化。本研究旨在探索语言模型是否表示其生成过程中可能采取的替代路径。

Method: 使用隐藏激活来控制并预测语言模型在思维链推理中的不确定性，通过激活干预实验分析模型的可操控性与不确定性之间的关系。

Result: 发现模型在不同token上的不确定性与模型通过激活控制被引导的容易程度存在明显相关性；隐藏激活可以预测模型的未来结果分布。

Conclusion: 激活干预在模型尚未确定最终答案时（即存在替代路径时）最有效，表明模型隐式表示可能的路径空间。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动评分系统缺乏对用户体验的关注，需要一种能更好理解议论文结构语义与用户理解之间差距的系统。

Method: 将议论文构建为论证图：节点表示论点，节点属性包含支持证据，边编码支持或攻击关系；使用LLM进行分类评分和可视化。

Result: 开发了IntelliProof系统，提供分类理由、连贯性量化指标，支持快速探索论证质量，同时保持人工监督。

Conclusion: IntelliProof通过论证图分析和自然语言工具，弥合了议论文结构语义与用户理解之间的差距，提供了更好的议论文分析体验。
...
TL;DR: LLM代码助手存在严重安全漏洞，最新开源模型在现实使用场景中仍然易受早期报告的漏洞攻击，安全与功能权衡阻碍了有效修复。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代码助手在软件开发中的关键作用增强，其生成的bug对网络安全的影响变得至关重要。现有安全基准和改进方法对主流编码LLM的实际影响尚不明确。

Method: 引入新的严重性指标Prompt Exposure (PE)，综合考虑漏洞严重性、生成概率和诱导漏洞代码的提示表述。基于PE定义Model Exposure (ME)评分，衡量模型生成漏洞的严重性和普遍性。

Result: 研究发现即使最新的开源模型在现实使用场景中仍然容易受到早期报告的漏洞攻击，表明安全功能权衡阻碍了漏洞的有效修复。
...
TL;DR: 开发了首个孟加拉语生物医学问答数据集BanglaMedQA和BanglaMMedBench，并应用多种RAG策略提升低资源语言医疗AI的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言生物医学问答系统开发困难的问题，促进医疗知识的公平获取，特别是在孟加拉语等资源有限的语言环境中。

Method: 结合基于教科书的检索和网络检索，应用多种RAG策略（传统、零样本回退、代理、迭代反馈和聚合RAG），并创新性地集成OCR处理的孟加拉语医学教科书语料库。

Result: 代理RAG方法在使用openai/gpt-oss-120b时达到最高准确率89.54%，优于其他配置，并展现出更优的推理质量。
...
TL;DR: DeReC是一个轻量级事实验证框架，使用通用文本嵌入替代自回归LLM方法，在保持高精度的同时显著提升效率，运行时间减少92-95%。
...
Method: 结合密集检索和专门分类的轻量级框架，用通用文本嵌入替代自回归LLM生成解释性理由的方法。

Result: 在RAWFC数据集上F1得分65.58%，超越L-Defense(61.20%)；运行时间大幅减少：RAWFC从454分钟降至23分钟，LIAR-RAW从1692分钟降至134分钟。
...
TL;DR: LEASH是一种无需训练的解码算法，通过监控token熵斜率和top-logit边际改善来自适应停止推理过程，在保持准确性的同时显著减少token生成和延迟。
...
Result: 在四个指令调优模型上的GSM8K和AQuA-RAT基准测试中，LEASH平均减少30-35%的token生成和27%的延迟，相对于CoT仅损失10个百分点的准确率。
...
TL;DR: DreamGym是一个统一的框架，通过合成多样化的经验数据来解决RL训练中的挑战，使用推理模型替代昂贵的环境交互，实现可扩展的智能体训练。


<details>
  <summary>Details</summary>
Motivation: 强化学习在实际应用中面临昂贵的环境交互、任务多样性有限、奖励信号不可靠和基础设施复杂等挑战，阻碍了可扩展经验数据的收集。

Method: DreamGym将环境动态提炼为基于推理的经验模型，通过逐步推理获得一致的状态转换和反馈信号；使用经验回放缓冲区初始化离线数据并持续丰富；自适应生成新任务进行课程学习。

Result: 在多样环境和智能体骨干上的实验表明，DreamGym显著改善RL训练。在WebArena等非RL就绪任务上超越所有基线30%以上；在RL就绪但昂贵设置中仅使用合成交互即可匹配GRPO和PPO性能；在模拟到真实迁移中，仅需少量真实交互即可获得显著性能提升。

Conclusion: DreamGym为通用RL提供了一个可扩展的预热启动策略，通过合成经验有效解决RL训练中的数据收集挑战。
...
<details>
  <summary>Details</summary>
Motivation: 汇编代码的分词在代码分析中至关重要，但目前仍是一个未被充分探索的领域，需要系统评估分词模型的内在特性和参数选择。

Method: 使用多种分词模型进行系统分析，包括Llama 3.2、BERT和BART等预训练模型，通过内在评估比较分词效率、词汇压缩和表示保真度。
...
<details>
  <summary>Details</summary>
Motivation: 探索文本和图像两种用户行为数据表示方式对多模态大语言模型性能的影响，确定哪种模态能最大化模型表现。

Method: 开发BehaviorLens基准框架，在6个MLLMs上测试三种数据表示方式：文本段落、散点图和流程图，使用真实购买序列数据集。

Result: 图像表示（散点图和流程图）相比等效文本表示，将MLLMs的下一购买预测准确率提高了87.5%，且无需额外计算成本。

Conclusion: 图像表示在多模态大语言模型的用户行为推理中显著优于文本表示，为优化MLLMs在序列数据分析中的应用提供了重要见解。
...
TL;DR: KnowThyself是一个基于聊天的LLM可解释性工具，通过整合现有功能、降低技术门槛，为用户提供交互式可视化和指导性解释。


<details>
  <summary>Details</summary>
Motivation: 现有LLM可解释性工具分散且代码密集，需要整合并降低使用门槛，使LLM解释更易访问。

Method: 使用编排器LLM重新表述用户查询，代理路由器将查询定向到专门模块，最后将输出情境化为连贯解释。

Result: 开发了一个基于聊天的界面，用户可以上传模型、提出自然语言问题并获得交互式可视化。

Conclusion: KnowThyself通过对话式工作流程为LLM可解释性提供了强大且可扩展的基础平台。
...
TL;DR: 本文挑战了深度知识追踪(DKT)模型性能源于建模知识组件间双向关系的传统解释，证明DKT的真正优势在于其隐式建模先决条件关系作为因果结构的能力。
...
Method: 通过将练习关系图修剪为有向无环图(DAG)，并在Assistments数据集的因果子集上训练DKT，分析其预测能力与因果结构的一致性。同时提出使用DKT学习表示提取练习关系DAG的新方法。

Result: 实验表明DKT的预测能力与因果结构高度一致，其学习表示可用于有效提取练习关系DAG，为DKT的有效性提供了新的解释。
...
TL;DR: LLMs对提示语言和文化框架有响应，但存在系统性文化偏见，偏向荷兰、德国、美国和日本的价值观，无法充分代表全球文化多样性。
...
Method: 使用霍夫斯泰德价值观调查模块和世界价值观调查的63个项目，翻译成11种语言，以不同文化视角制定提示，测试10个LLM。

Result: 提示语言和文化视角都会影响LLM输出，但模型系统性偏向特定国家的价值观；明确文化视角比针对性提示语言更能改善与人类价值观的一致性。

Conclusion: LLMs处于尴尬的中间地带：对提示变化敏感但过于固守特定文化默认值，无法充分代表文化多样性。
...
TL;DR: ArchPilot是一个多智能体系统，通过集成架构生成、代理评估和自适应搜索来解决LLM-based代理在自动化ML工程中计算开销大、可扩展性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM-based代理在自动化ML工程中严重依赖重复的完整训练来评估候选方案，导致计算开销大、搜索空间有限、迭代周期慢。

Method: ArchPilot包含三个专业代理：编排代理使用MCTS启发的新算法协调搜索过程；生成代理迭代生成、改进和调试候选架构；评估代理执行代理训练运行并生成优化的代理函数。

Result: 在MLE-Bench上的实验表明，ArchPilot超越了AIDE和ML-Master等SOTA基线方法。

Conclusion: ArchPilot的多智能体协作能够在最小化依赖昂贵完整训练的情况下优先考虑高潜力候选方案，在有限预算下实现高效的ML工程。
...
TL;DR: 该论文提出了多智能体AI系统中的异常检测任务，创建了两个包含4,275和894条轨迹的基准数据集，并展示了监督和半监督方法分别达到98%和96%的准确率。
...
Method: 提出了一个数据集构建流程，捕捉用户行为、智能体非确定性和LLM变化，并创建了两个基准数据集。使用XGBoost（监督）和SVDD（半监督）方法进行异常检测基准测试。

Result: 监督方法（XGBoost）准确率达到98%，半监督方法（SVDD）准确率达到96%，两种方法性能相当。
...
TL;DR: LLMs编码真实世界数值相关性但会系统性地放大它们，无关上下文会引起数值表示的偏移，这揭示了LLM决策中的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 尽管行为研究记录了大型语言模型在数值推理中的错误，但其底层表征机制仍不清楚。研究者假设数值属性占据共享的潜在子空间，并探讨LLM如何整合单个实体的多个数值属性，以及无关数值上下文如何干扰这些表征。

Method: 结合线性探测与偏相关分析，以及在不同规模模型上进行基于提示的脆弱性测试。

Result: LLMs编码真实世界的数值相关性但倾向于系统性地放大它们；无关上下文会引起数值表示的一致偏移，下游影响因模型规模而异。
...
TL;DR: 提出了Agentmandering框架，将选区重划重新构想为对立政治利益代表之间的回合制谈判，通过LLM智能体嵌入战略互动，显著减少党派偏见和不公平性。


<details>
  <summary>Details</summary>
Motivation: 现有计算方法主要生成合法选区重划方案，但忽略了选择过程中的战略动态，这为党派行为者挑选技术上合规但政治上有利的地图创造了机会。

Method: 基于博弈论思想，特别是'选择并冻结'协议，让代表对立政治利益的LLM智能体轮流从候选地图集中选择和冻结选区，通过受限且可解释的选择逐步划分州。

Result: 在2020年美国人口普查数据上的评估显示，Agentmandering显著减少了党派偏见和不公平性，方差比标准基线低2-3个数量级，在摇摆州场景中表现出公平性和稳定性。

Conclusion: Agentmandering框架通过将战略互动嵌入选区重划过程，能够产生更公平和稳定的结果，特别是在竞争激烈的政治环境中。
...
TL;DR: LLM-KGFR框架通过结合大语言模型与知识图谱基础检索器，解决LLM在知识密集型问题上的局限性，实现零样本泛化到未见过的知识图谱，并保持可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖微调的LLM或GNN检索器，存在数据集特定调优和在大型或未见图谱上可扩展性不足的问题。

Method: 提出LLM-KGFR协作框架，KGFR使用LLM生成的关系描述编码关系，基于问题中的角色初始化实体，采用非对称渐进传播处理大型图谱，通过节点、边和路径级接口支持迭代推理。
...
<details>
  <summary>Details</summary>
Motivation: 随着语音AI代理快速部署到生产环境，缺乏系统化的测试可靠性验证方法，组织无法客观评估测试方法是否有效，这在大规模部署时造成了关键的测量差距。

Method: 结合心理测量技术（成对比较产生Elo评分、bootstrap置信区间和置换检验）与严格的统计验证，提供可重复的指标，适用于任何测试方法。

Result: 对三个领先商业平台的实证评估显示，表现最佳的平台Evalion在评估质量上达到0.92（F1分数），而其他平台为0.73；在模拟质量上达到0.61，而其他平台为0.43。
...
TL;DR: 本文介绍了Disempower-Grid测试套件，发现针对单人类优化的赋能目标在多人类环境中会导致其他人类的环境影响力和奖励显著降低，这种现象被称为"去赋能"。


<details>
  <summary>Details</summary>
Motivation: 赋能作为AI代理的通用目标无关目标已被提出，但先前工作假设代理在孤立环境中协助单个人类。多人类环境（如家庭和医院）对AI辅助很有前景，需要研究赋能目标在这些环境中的表现。

Method: 引入开源的多人类网格世界测试套件Disempower-Grid，通过实验验证赋能RL代理在优化单人类赋能时对其他人类的影响。

Result: 实验表明，优化单人类赋能的辅助RL代理会显著降低其他人类的环境影响力和奖励，即产生"去赋能"现象。联合赋能可以缓解去赋能，但会以用户奖励为代价。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种系统化的方法来评估和比较工作流的质量和效率，将成功概率、资源使用和输出收益等概念整合到统一的数学模型中。

Method: 结合Opus工作流奖励（概率函数估计预期性能）和Opus工作流规范惩罚（测量结构和信息质量的函数），支持强化学习循环中的工作流发现和优化。

Result: 提出了一个统一的工作流评估框架，能够形式化工作流成功概率，定义可测量的结构和语义属性，并支持在奖励-惩罚权衡下识别和排名最优工作流。

Conclusion: 该框架为工作流质量评估和优化提供了数学基础，能够集成到现代自动化系统中，指导工作流的发现和改进过程。
...
TL;DR: 提出了一种多智能体预测编码框架，通过最小化智能体间的相互不确定性来实现协调，在带宽受限条件下表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体系统中由于部分可观测性和有限带宽导致的协调失败问题，建立一致的空间记忆共享机制。

Method: 基于信息瓶颈目标的多智能体预测编码框架，包含自监督运动预测产生的网格细胞状度量编码、带宽高效通信机制、社会位置细胞表示和分层强化学习策略。
...
TL;DR: RLVR训练存在RL过拟合问题，模型获得训练奖励但失去泛化能力。RLoop通过迭代策略初始化框架解决此问题，将训练过程转化为探索和利用的良性循环。
...
Method: RLoop框架：首先使用RL从给定策略探索解空间，然后筛选成功轨迹创建专家数据集，通过拒绝采样微调(RFT)优化初始策略，为下一次迭代创建更好的起点。

Result: RLoop显著改善泛化能力，平均准确率提升9%，pass@32指标提升超过15%，相比标准RL方法。

Conclusion: RLoop通过迭代策略重初始化将瞬态策略变化转化为稳健性能提升，有效缓解遗忘问题并大幅提升泛化性能。
...
TL;DR: GUI-360°是一个大规模数据集和基准测试套件，用于推进计算机使用代理(CUAs)的研究，包含超过120万执行动作步骤，支持GUI定位、屏幕解析和动作预测三个核心任务。


<details>
  <summary>Details</summary>
Motivation: 解决计算机使用代理面临的三个关键挑战：真实世界CUA任务稀缺、缺乏多模态轨迹的自动收集和标注流程、缺少统一评估GUI定位、屏幕解析和动作预测的基准。

Method: 使用LLM增强的自动化流程，包括查询来源、环境模板构建、任务实例化、批量执行和LLM驱动的质量过滤，在Windows办公应用中收集数千条轨迹数据。

Result: 基准测试显示现有视觉-语言模型在GUI定位和动作预测方面存在显著不足，监督微调和强化学习虽有改进但未能达到人类可靠性水平。
...
TL;DR: 论文指出CAV探测器的分类准确率不能可靠反映概念对齐度，提出了基于空间线性归因的新概念定位方法，并引入三类评估指标来量化概念对齐。


<details>
  <summary>Details</summary>
Motivation: 当前在可解释AI中，CAV通常通过训练线性分类器探测器获得，但仅凭探测器的高准确率无法可靠评估概念对齐程度，探测器更可能捕捉虚假相关性而非目标概念。

Method: 引入基于空间线性归因的新概念定位方法，与现有特征可视化技术进行比较，提出三类概念对齐评估指标：硬准确率、分割得分和增强鲁棒性。

Result: 研究表明，具有平移不变性和空间对齐的探测器能持续提高概念对齐度，故意构造的错位探测器也能达到接近标准探测器的准确率。

Conclusion: 需要基于对齐度的评估指标而非探测器准确率，探测器设计需同时考虑模型架构和目标概念特性。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM安全和鲁棒性研究生态系统存在碎片化、实现错误多、难以复现和比较的问题，阻碍了该领域的实质性进展。

Method: 设计了一个以可复现性、正确性和可扩展性为核心的框架，实现了12种对抗攻击算法，集成了7个基准数据集，支持通过Hugging Face访问多种开源LLM，并包含计算资源跟踪、确定性结果和分布评估等高级功能。

Result: 开发了AdversariaLLM工具箱及其配套的JudgeZoo评估包，为LLM安全研究提供了透明、可比较和可复现的基础设施。

Conclusion: AdversariaLLM为LLM安全研究建立了一个强大的基础，促进了该领域的透明性、可比较性和可复现性。
...
TL;DR: 提出了RxSafeBench框架，通过模拟临床咨询来评估LLMs的药物安全能力，包含6,725种禁忌症、28,781种药物相互作用和14,906种适应症-药物对，构建了2,443个高质量咨询场景的基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLMs的医疗系统在药物安全方面的研究有限，缺乏真实世界数据集，且在真实临床咨询环境下的评估不足，需要系统评估LLMs的药物安全能力。

Method: 构建包含禁忌症、药物相互作用和适应症-药物对的RxRisk DB数据库，采用两阶段过滤策略确保临床真实性和专业性，生成包含药物风险的咨询对话，使用结构化多选题评估LLMs推荐安全药物的能力。

Result: 当前LLMs在整合禁忌症和相互作用知识方面存在困难，特别是当风险是隐含而非明确时表现不佳，揭示了LLM系统在确保药物安全方面的关键挑战。

Conclusion: RxSafeBench提供了首个评估LLMs药物安全性的综合基准，通过改进提示和任务特定调优可提高可靠性，推动更安全可信的AI驱动临床决策支持系统发展。
...
Method: 将Flavell以及Nelson和Narens的元认知理论形式化为计算规范，在Generate-Verify范式前添加显式监控，捕获生成前的元认知体验并通过验证反馈优化未来监控。
...
TL;DR: 提出了一种名为迭代遗憾最小化微调(Iterative RMFT)的后训练方法，通过反复将低遗憾决策轨迹蒸馏回基础模型来增强LLM的决策能力。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地被部署为交互动态环境中的决策代理，但由于最初并非为决策设计，研究表明LLM即使在基本在线决策问题中也表现不佳，无法实现低遗憾或有效的探索-利用权衡。

Method: 在每次迭代中，模型生成多个决策轨迹，选择k个最低遗憾的轨迹，并在这些轨迹上微调自身。该方法利用遗憾指标来激发模型自身的决策能力和推理原理，避免了刚性输出工程。

Result: 实验结果表明，Iterative RMFT在不同模型上都能提升LLM的决策性能，包括Transformer、开源权重LLM和GPT-4o mini等闭源模型。该方法在输出和推理格式上的灵活性使其能够泛化到具有不同时间范围、动作空间、奖励过程和自然语言上下文的任务。

Conclusion: Iterative RMFT提供了一个原则性和通用的后训练框架，用于增强LLM的决策能力，理论分析表明在简化设置下单层Transformer可以充当无遗憾学习器。
...
TL;DR: CoRPO解决了GRPO在强化学习中处理序数奖励时的缺陷，通过自适应基线确保失败轨迹不会被正向强化，并在达到质量阈值后自动切换到相对偏好模式来寻找最优解。


<details>
  <summary>Details</summary>
Motivation: GRPO的简单性使其在处理非二元反馈时存在缺陷，特别是当使用序数奖励给予部分信用时，其组平均基线经常对失败轨迹分配正优势，从而强化错误行为。

Method: CoRPO采用自适应基线强制执行最低质量阈值，确保失败解决方案永远不会被正向强化。一旦策略持续满足该阈值，基线自动过渡到相对偏好模式，推动模型寻找最优解而非仅可接受的解。

Result: 在代码验证任务上的实证验证表明，CoRPO表现出更稳定的收敛性和更好的域外泛化能力。

Conclusion: 这项工作是在强化学习中使LLM学习真正新能力的关键步骤，通过从二元奖励扩展到序数奖励，为更密集的逐步监督铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 传统车辆路由系统只能优化单一指标（如时间或距离），无法理解人类驾驶员的复杂语义和动态上下文（如多步骤任务、情境约束、紧急需求），需要能集成上下文推理的混合方法。
...
<details>
  <summary>Details</summary>
Motivation: 虽然网络代理研究蓬勃发展，但其引发的可持续性问题仍未得到充分探索。为强调此问题的紧迫性，作者从理论和实证角度研究了网络代理的能源和二氧化碳成本。

Method: 采用理论估计和实证基准测试相结合的方法，分析不同网络代理设计理念对能耗的影响，并评估能耗与性能之间的关系。

Result: 结果显示不同网络代理设计理念会严重影响能耗，且更高能耗并不一定带来更好结果。同时发现某些网络代理在披露模型参数和过程方面缺乏透明度，限制了能耗估计的准确性。

Conclusion: 研究呼吁改变评估网络代理的方式，建议在基准测试中引入专门的能耗测量指标，以促进更可持续的网络代理开发。
...
TL;DR: 本文开发了博弈论实验的数字孪生和系统提示框架，发现不同LLM在模拟人类决策时表现出不同模式：Llama能高保真复制人类合作行为，Qwen则更接近纳什均衡预测，无需角色提示即可实现群体行为复制。


<details>
  <summary>Details</summary>
Motivation: 理解LLM与人类决策的相似性至关重要，因为差异可能导致实际应用中的有害结果，而无法复制人类行为会使LLM在社会模拟中无效。

Method: 开发博弈论实验的数字孪生，引入系统提示和探测框架进行机器行为评估，测试三个开源模型（Llama、Mistral和Qwen）。

Result: Llama能高保真复制人类合作模式，捕捉人类偏离理性选择理论的行为；Qwen则与纳什均衡预测紧密一致；无需角色提示即可实现群体行为复制。

Conclusion: 适当校准的LLM可以复制人类群体行为模式，并能系统探索未开发的实验空间，为社会和行为科学研究提供补充方法，生成关于人类社交决策的新实证预测。
...
TL;DR: 提出了一种数据驱动的稀疏传感框架，结合EPA-SWMM模型，通过优化传感器布置来重建雨水系统中的峰值流量，在资源受限条件下实现高精度流量监测。


<details>
  <summary>Details</summary>
Motivation: 城市地表水洪水日益频繁，但高时空分辨率的洪水预测和监测面临时间、预算和技术限制，如何在资源受限条件下监测城市排水网络和预测流量状况是主要挑战。

Method: 使用SWMM模型生成训练数据集，应用数据驱动稀疏传感框架，利用奇异值分解进行降维和QR分解进行传感器分配，识别最优监测节点。

Result: 在77个节点中仅需3个优化布置的传感器即可实现满意的重建性能，Nash-Sutcliffe效率值为0.92-0.95，模型对测量不确定性具有良好的鲁棒性。

Conclusion: 该框架平衡了计算效率和物理可解释性，能够以最少的传感器实现高精度流量重建，可进一步与预测模型集成，在有限的传感和监测资源下实现洪水预警和实时控制。
...
<details>
  <summary>Details</summary>
Motivation: 理解AI科学家系统的当前能力和风险对于确保可信赖和可持续的AI驱动科学进步至关重要，同时需要维护学术生态系统的完整性。

Method: 开发Jr. AI Scientist系统，模拟新手学生研究者的核心研究流程：分析基线论文局限性、提出改进假设、通过严格实验验证、撰写结果论文。利用现代编码代理处理复杂的多文件实现。

Result: Jr. AI Scientist生成的论文在AI评审员评估中获得比现有全自动系统更高的评分，但在作者评估和Agents4Science评审中仍存在重要局限性。

Conclusion: 当前AI科学家系统存在直接应用的风险，需要解决关键挑战。研究深入报告了开发过程中识别的各种风险，希望加深对AI科学家发展现状和风险的理解。
...
TL;DR: 本文提出将自然语言查询中的歧义性重新定义为协作交互的特征，开发了一个区分可协作查询与不可协作查询的框架，并分析了15个流行数据集中的查询类型分布。


<details>
  <summary>Details</summary>
Motivation: 传统方法将查询歧义视为缺陷，本文重新将其视为协作交互的特征，强调用户和系统共同承担查询规范的责任。

Method: 开发了一个原则性框架来区分可协作查询和不可协作查询，并将该框架应用于15个流行的表格问答和分析数据集的查询分析。

Result: 发现现有数据集中的查询类型混合不当，既不适合评估系统执行准确性，也不适合评估解释能力。

Conclusion: 该框架将视角从消除歧义转向在解决查询中拥抱协作，为表格数据的自然语言接口提供了更明智的设计和评估方向。
...
Method: 基于合理代表(JR)概念建立审计框架，开发O(mn log n)时间复杂度的审计算法，应用于历史审议数据比较不同问题选择方法的代表性。

Result: 比较了主持人选择、整数线性规划选择和LLM生成摘要问题三种方法的代表性，揭示了LLM在支持审议过程中的潜力和当前局限。
...
TL;DR: DR.WELL是一个去中心化的神经符号框架，用于协作多智能体规划。通过两阶段协商协议（角色提议和联合分配），智能体在符号层面而非轨迹层面进行协调，避免了脆弱的步级对齐问题。


<details>
  <summary>Details</summary>
Motivation: 协作多智能体规划面临部分信息和有限通信的挑战。轨迹级协调容易因微小偏差导致冲突，需要更高层次的抽象来支持同步和集体进展。

Method: 采用两阶段协商协议：智能体首先提出候选角色及推理，然后在共识和环境约束下承诺联合分配。承诺后，每个智能体独立生成和执行其角色的符号计划，通过共享世界模型将计划落地到执行结果。

Result: 在协作推块任务上的实验表明，智能体能够跨回合适应，动态世界模型捕获可重用模式，提高了任务完成率和效率。

Conclusion: 通过符号规划而非原始轨迹推理，DR.WELL避免了脆弱的步级对齐，实现了可重用、可同步且可解释的高层操作，以时间开销换取演化出更高效的协作策略。
...
TL;DR: VeriCoT是一个神经符号方法，通过将思维链推理形式化为一阶逻辑并使用自动求解器验证逻辑有效性，来识别和纠正LLM推理中的缺陷。
...
Method: VeriCoT从CoT推理中提取形式逻辑论证，将每个推理步骤形式化为一阶逻辑，识别基于源上下文、常识知识或先前推理步骤的前提，使用符号表示和自动求解器验证逻辑有效性。
...
Conclusion: VeriCoT通过形式化验证增强了LLM推理的可信度，为高风险应用提供了可靠的推理验证机制，并能通过微调进一步提升模型性能。
...
TL;DR: 提出了一个紧凑而高表达性的壳层超材料设计空间，结合GPU加速均质化管道实现快速评估和逆向设计，能够定制具有多样化几何形状和机械性能的结构。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够高效探索壳层超材料设计空间的方法，实现从简单平面结构到复杂三周期极小曲面的几何表示，并支持针对特定宏观性能的逆向设计。

Method: 使用少量自由度（几十个）表示几何设计空间，开发基于GPU的快速均质化管道（20ms评估结构，0.5s计算有效弹性张量），支持逆向设计方法。

Result: 设计的结构展现出几何多样性和广泛的机械响应，覆盖大范围材料性能，达到理论上限的91.86%，与最先进低体积壳层结构性能相当，并通过增材制造验证了可制造性。

Conclusion: 该紧凑设计空间结合快速计算管道能够有效探索壳层超材料，实现高性能定制化设计，具有实际工程应用潜力。
