...
Motivation: 现有方法对每张图像统一应用超分辨率会导致多视角不一致和模糊渲染，需要一种更智能的方法来选择性增强图像

Method: 基于相机姿态相对于场景几何的信息，选择性在欠采样区域应用超分辨率内容，利用近距离低分辨率视图中的高频信息来增强远距离视图

Result: 在Tanks & Temples、Deep Blending和Mip-NeRF 360数据集上，方法在保真度和感知质量上都超越了基线方法，特别是在需要更高细节的局部前景区域效果最显著

Conclusion: 选择性应用超分辨率内容比统一应用更有效，能够产生更清晰、更一致的渲染结果，特别是在需要高细节的区域
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究基于深度学习和高斯混合模型的部分到部分点集配准方法中特征向量在平移和旋转下的不变性问题，特别关注DeepGMR等现有方法在部分到部分配准场景中的局限性，旨在揭示问题根源并提出解决方案。

Method: 提出了一种基于注意力的参考点偏移层，该层使用经过充分研究的注意力模块来识别两个部分点集的共同参考点，而不是寻找重叠区域，从而获得变换不变的特征表示。

Result: ARPS层显著提升了DeepGMR及其最新变体UGMMReg的性能，这些扩展模型甚至超越了先前使用注意力块和Transformer提取重叠区域或共同参考点的深度学习方法。

Conclusion: 这些发现为使用深度学习和高斯混合模型的配准方法提供了更深入的见解，表明通过注意力机制识别共同参考点而非重叠区域，可以更有效地解决部分到部分点集配准中的变换不变性问题。
...
TL;DR: 研究通过微调TerraMind地理空间基础模型，结合多模态卫星数据（Sentinel-1 SAR和Sentinel-2光学影像），在85个全球洪水事件数据集上评估了洪水范围制图性能，发现基础模型配置在精度和计算成本间取得最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 洪水是最具破坏性的天气相关灾害之一，2024年极端洪水事件影响五大洲。虽然地球观测卫星提供了关键的洪水监测数据，但传统方法的准确性严重依赖标注数据集和模型泛化能力。新兴的地理空间基础模型（GFMs）通过大规模自监督预训练提高了泛化性，但其在全球多样化洪水事件中的性能尚不清楚。

Method: 使用FloodsNet数据集（包含85个全球洪水事件的Sentinel-1 SAR和Sentinel-2光学协同定位影像）微调ESA-IBM的TerraMind模型。测试了四种配置（基础vs大型模型；冻结vs解冻骨干网络），并与TerraMind Sen1Floods11示例以及在FloodsNet和Sen1Floods11上训练的U-Net进行比较。

Result: 基础-解冻配置在准确性、精确度和召回率之间取得最佳平衡，且计算成本显著低于大型模型。大型解冻模型获得最高召回率。在FloodsNet上训练的模型在召回率上优于Sen1Floods11训练的示例，总体准确性相似。U-Net比所有GFM配置获得更高召回率，但准确性和精确度略低。

Conclusion: 整合多模态光学和SAR数据并微调GFM可以增强近实时洪水制图能力。这是首次在全球范围内评估GFM用于洪水分割的研究之一，凸显了其在气候适应和灾害韧性方面的潜力和当前局限性。
...
TL;DR: 提出一种结合纹理映射的Gaussian Splatting方法，通过自适应纹理分辨率减少所需的高斯基元数量，提高效率


<details>
  <summary>Details</summary>
Motivation: 传统Gaussian Splatting方法需要大量小高斯基元来表示精细外观细节，这在几何和外观具有不同频率特性时效率低下。受纹理映射传统启发，希望用纹理来表示可能的外观细节

Method: 为2D高斯基元引入每个基元的纹理映射，根据图像采样频率限制纹理像素大小并适应输入图像内容。在优化过程中自适应调整纹理分辨率（上采样或下采样），并基于纹理分辨率控制基元数量

Result: 相比其他纹理化高斯基元解决方案，该方法在图像质量和总参数数量方面表现更优

Conclusion: 通过结合纹理映射的Gaussian Splatting方法，能够更高效地表示场景外观细节，减少不必要的基元数量，在保持图像质量的同时优化参数使用
...
TL;DR: 提出上下文增强对比损失函数，通过两个收敛目标同时提升学习效果并解决信息失真问题，在8个大规模基准数据集上优于16种SOTA对比学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在从文本生成场景内在属性时缺乏结构一致性和物理合理性，需要一种能够联合生成多种内在属性并保持物理一致性的框架。

Method: 提出LumiX结构化扩散框架，包含两个关键技术：1) 查询广播注意力机制，通过在所有内在属性图之间共享查询来确保结构一致性；2) 张量LoRA，通过张量化适配高效建模跨属性关系，实现稳定的联合扩散训练。

Result: 实验表明LumiX生成的结果具有更好的物理一致性和对齐性，比现有技术提高了23%的对齐度，偏好得分从-0.41提升到0.19，同时还能在同一框架内进行图像条件的内在属性分解。

Conclusion: LumiX通过结构化扩散框架成功实现了从文本到多种内在属性的联合生成，确保了物理一致性，为场景理解和编辑提供了更全面的内在属性表示。
...
TL;DR: 本文提出了一种联合评估文本到图像生成模型和视觉语言模型的结构化方法，通过测试VLM是否能识别T2I模型生成的图像中的27种特定错误模式，并创建了包含5个T2I模型和3个VLM的数据集。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型虽然能生成视觉上令人印象深刻的图像，但经常无法准确捕捉用户提示中的特定属性（如正确数量的对象和指定颜色）。同时，视觉语言模型的基准测试未能跟上VLM用于注释的复杂场景需求。需要一种分层评估框架来比较不同图像生成模型的提示遵循能力。

Method: 提出结构化方法联合评估T2I模型和VLM：1) 测试VLM是否能识别T2I模型在挑战性提示下生成的图像中的27种特定错误模式；2) 创建数据集包含5个T2I模型（Flux, SD3-Medium, SD3-Large, SD3.5-Medium, SD3.5-Large）生成的图像和3个VLM（Molmo, InternVL3, Pixtral）的注释；3) 使用LLM（Llama3）标注VLM是否正确识别了生成图像中的错误模式。

Result: 通过分析精心设计的提示集上的错误模式，揭示了属性保真度和对象表示方面的系统性错误。研究发现当前指标不足以捕捉这些细微错误，强调了针对性基准测试对于推进生成模型可靠性和可解释性的重要性。

Conclusion: 本文提出的联合评估框架和数据集为评估T2I模型和VLM提供了新的方法，揭示了现有模型的系统性缺陷，强调了需要更精细的基准测试来提升生成模型的可靠性和可解释性。
...
TL;DR: SurfFill：一种基于高斯面元的LiDAR补全方法，结合LiDAR和相机优势，通过分析光束发散造成的伪影，使用密度变化启发式识别模糊区域，并在这些区域优化高斯面元重建来补全点云。


<details>
  <summary>Details</summary>
Motivation: LiDAR在平坦区域精度高，但容易遗漏小几何结构和暗色吸收材料；而摄影测量能捕捉细节但精度不足。需要结合两者优势，解决LiDAR点云缺失问题。

Method: 1) 分析LiDAR光束发散导致的伪影；2) 使用点云密度变化启发式识别模糊区域；3) 在这些区域约束高斯面元重建进行优化和密集化；4) 提取高斯基元采样补全点云；5) 针对大规模重建采用分治策略。

Result: 在合成和真实场景的LiDAR点云补全任务中，该方法优于之前的重建方法。

Conclusion: SurfFill成功结合了LiDAR和相机优势，通过高斯面元重建有效补全了LiDAR点云中的缺失区域，特别是在薄结构和边缘处，并适用于大规模场景。
...
TL;DR: LLOST模型使用双变分自编码器，通过共享潜在空间将医学影像点云与体细胞突变计数关联，实现从影像预测基因突变，支持癌症早期诊断。


<details>
  <summary>Details</summary>
Motivation: 癌症治疗有效性依赖早期诊断，但当前临床实践中医学影像和基因信息分析是分离的。需要建立从医学影像直接预测体细胞突变的方法，以加速诊断和治疗决策。

Method: 提出LLOST模型：1) 将病灶图像转换为点云表示以实现模态不变性；2) 使用双变分自编码器分别处理点云和突变计数；3) 通过共享潜在空间连接两个域；4) 每个潜在空间使用条件归一化流先验处理不同分布。

Result: 在TCIA医学影像和TCGA Pan Cancer突变数据集上验证：模型能准确预测特定突变计数和突变发生概率，并发现影像与突变域之间的共享模式反映癌症类型。

Conclusion: LLOST成功建立了医学影像与体细胞突变的关联，为早期癌症诊断提供了新方法。未来可扩展到其他遗传领域，并进一步改进模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 肖像视频编辑需要灵活而精确的控制各种修改（如外观变化、表情编辑、添加对象），关键难点在于保持主体原始的时间行为，要求每个编辑帧与对应源帧精确同步

Method: 使用图像到视频扩散模型，通过修改第一帧定义编辑并传播到整个序列。训练上下文LoRA使用成对视频（相同运动轨迹但外观不同），通过同步过滤自动生成和筛选训练数据，使模型学会结合源视频运动线索和编辑第一帧的视觉变化

Result: 在紧凑、高度筛选的同步人像数据集上训练后，Sync-LoRA能够泛化到未见身份和多样化编辑（修改外观、添加对象、改变背景等），稳健处理姿势和表情变化，实现高视觉保真度和强时间连贯性

Conclusion: Sync-LoRA在编辑保真度和精确运动保持之间实现了稳健平衡，展示了高质量的视觉修改和帧级同步能力
...
TL;DR: RobustSurg：一种用于手术场景分割的鲁棒方法，通过利用风格和内容信息、实例归一化和特征协方差映射来提高对未见分布和模态的泛化能力，在多个数据集上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前手术场景分割方法通常在单中心、单模态数据上表现良好，但难以泛化到未见分布（其他中心）和未见模态。现有自然场景的域泛化方法不能直接应用于手术场景，因为手术场景视觉线索有限且场景极其多样化。

Method: 通过实例归一化和特征协方差映射技术来利用手术场景的风格和内容信息，减少外观变化的影响。在ResNet骨干网络中引入恢复模块，保留与任务相关的有用特征表示。同时提供了新的多类别多中心数据集。

Result: 在HeiCholSeg数据集上，RobustSurg相比基线DeepLabv3+提升了近23%，相比SOTA方法提升了10-32%（mIoU）。在EndoUDA息肉数据集上，相比基线提升了近22%，相比最近SOTA方法提升了近11%。

Conclusion: 通过利用风格和内容信息以及引入恢复模块，RobustSurg能够显著提高手术场景分割对未见分布和模态的泛化能力，为解决手术场景分割的域泛化问题提供了有效方案。
...
TL;DR: 该论文提出两种归纳先验方法（单分形和多分形重校准），利用分形指数概率质量与多分形谱之间的关系形成编码器嵌入的统计描述，作为卷积网络中的通道注意力函数，在医学图像分割任务中取得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 现有端到端多分形方法依赖重池化或强特征空间降采样，限制了语义分割等任务的应用。为了克服这些限制，需要开发新的多分形分析方法来更好地描述深度学习中的自相似现象。

Method: 提出两种归纳先验：单分形重校准和多分形重校准。这些方法利用分形指数概率质量与多分形谱之间的关系，形成编码器嵌入的统计描述，实现为卷积网络中的通道注意力函数。使用U-Net框架进行验证。

Result: 多分形重校准在使用U-Net框架时，相比使用其他基于高阶统计的通道注意力机制的基线模型，取得了显著性能提升。在三个公开医学影像数据集（ISIC18、Kvasir-SEG、BUSI）上验证了方法的有效性。

Conclusion: 多分形重校准为医学图像分割提供了有效的通道注意力机制。研究发现，在U-Net架构中，由于跳跃连接的存在，激励响应不会随着编码器深度增加而变得过度专门化，其有效性可能与实例变异的全局统计特性相关。
...
TL;DR: 提出Unified-VQA框架，通过诊断性混合专家模型解决通用视频质量评估问题，提供统一质量模型和可解释的多维伪影向量


<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估方法存在三个主要问题：1）单一质量分数无法提供诊断性反馈；2）多为格式特定的专用指标而非通用解决方案；3）从不同感知域学习妥协表示而非真正通用

Method: 提出Unified-VQA框架，采用诊断性混合专家方法：1）多个"感知专家"专注于不同感知域；2）新颖的多代理专家训练策略，使用排序损失优化每个专家；3）集成诊断性多任务头，生成全局质量分数和可解释的多维伪影向量；4）弱监督学习策略利用大规模训练数据库

Result: 在静态模型参数下（无需重新训练或微调），Unified-VQA在17个包含HD、UHD、HDR和HFR格式的数据库上，相比18个基准方法，在通用VQA和诊断伪影检测任务上均表现出一致且优越的性能

Conclusion: 该工作代表了向实用、可操作和可解释的视频质量评估迈出的重要一步，提供了统一的、可诊断的视频质量评估框架
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型需要联合理解视觉、音频和语言，但现有视频基准测试很少评估关于人类语音的细粒度推理。许多任务要么可以通过视觉解决，要么只粗略评估语音，无法深入了解模型是否能对齐说话人、说话内容和时间。

Method: 提出AV-SpeakerBench基准测试，具有三个特点：1) 以说话人为核心推理单元而非场景；2) 融合基础的问题设计，将视听依赖嵌入问题语义；3) 专家标注确保时间精度和跨模态有效性。

Result: Gemini系列模型表现最佳，Gemini 2.5 Pro达到最好结果。开源模型中，Qwen3-Omni-30B接近Gemini 2.0 Flash但仍远落后于Gemini 2.5 Pro，主要原因是视听融合能力较弱而非视觉感知。

Conclusion: AV-SpeakerBench为未来多模态系统的细粒度视听推理建立了严格的基础，有助于推动模型在说话人中心推理方面的发展。
...
TL;DR: 本文提出了一种用于图像去雨的视觉LIF神经元（VLIF）和相应的脉冲神经网络框架，在显著降低能耗的同时超越了现有SNN去雨方法的性能。
...
Method: 提出了视觉LIF（VLIF）神经元来克服传统脉冲神经元的空间上下文理解障碍；设计了脉冲分解增强模块和轻量级脉冲多尺度单元，用于层次化多尺度表示学习，解决频域饱和问题。
...
TL;DR: 提出SPF（时空金字塔流）方法，通过分层时空建模实现高效并行气候模拟，并构建ClimateSuite大规模数据集，为多时间尺度气候模拟提供新基础。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型依赖天气尺度自回归进行气候模拟，这种方法在长期气候模拟中速度慢，且在非平稳强迫条件下稳定性不足，需要更高效、稳定的气候模拟方法。

Method: 提出时空金字塔流（SPF）方法，将生成轨迹划分为时空金字塔结构，逐级增加空间分辨率以减少计算量，每阶段耦合特定时间尺度，可直接在金字塔任意时间层级采样，并结合物理强迫条件。

Result: 在ClimateBench上，SPF在年尺度和月尺度上优于强基线流匹配方法和预训练模型，采样速度快，特别是在较粗时间层级。构建的ClimateSuite包含33,000+模拟年，SPF模型在跨气候模型的保留场景中表现出良好泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型具有强大的时间生成能力，但它们在图像修复方面的潜力尚未充分探索。作者希望利用这些模型的时间建模能力来实现渐进式视觉修复。

Method: 通过微调CogVideo模型，使其生成修复轨迹而非自然视频。构建了超分辨率、去模糊和低光增强的合成数据集，每个样本展示从退化帧到干净帧的渐进过渡。比较了两种提示策略：统一的文本提示和通过LLaVA多模态LLM生成、ChatGPT优化的场景特定提示。

Result: 微调后的模型能够将时间进展与修复质量关联起来，生成的序列在PSNR、SSIM和LPIPS等感知指标上逐帧改善。模型有效恢复了空间细节和光照一致性，同时保持时间连贯性。在ReLoBlur数据集上无需额外训练就能泛化到真实场景，表现出强大的零样本鲁棒性和通过时间修复的可解释性。

Conclusion: 文本到视频模型可以成功重新用于渐进式视觉修复任务，通过时间建模实现高质量的图像修复，并在真实场景中展现出良好的泛化能力和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 現有SAR油污分割模型在不同區域間泛化能力差，特別是在秘魯海岸標註數據稀缺的情況下，模型性能大幅下降

Method: 兩階段合成增強框架：第一階段使用形態學區域擾動生成幾何變化；第二階段使用條件生成模型渲染SAR紋理

Result: 模型從地中海到秘魯的mIoU從67.8%降至51.8%，使用MORP-Synth後提升最多6% mIoU，少數類IoU顯著提升（油污+10.8，相似物+14.6）

Conclusion: MORP-Synth能有效改善SAR油污分割模型的跨區域泛化能力，特別是在標註數據稀缺的區域
...
TL;DR: 利用预训练视频扩散模型的内在运动表示进行自监督跟踪，显著提升视觉相似物体的区分能力


<details>
  <summary>Details</summary>
Motivation: 现有自监督跟踪器在视觉线索模糊时表现不佳，需要大量标注数据，限制了可扩展性和泛化能力。预训练视频扩散模型可能已学习到适合跟踪的运动表示。

Method: 发现视频扩散模型的去噪过程在早期高噪声阶段分离出运动信息，与后期的外观细化不同。利用这一发现构建自监督跟踪器，无需任务特定训练。

Result: 在现有基准测试和新引入的视觉相似物体跟踪测试中，相比近期自监督方法提升高达6个百分点。可视化证实扩散模型衍生的运动表示能鲁棒跟踪甚至完全相同的物体。

Conclusion: 预训练视频扩散模型内在学习到的运动表示可用于构建高效的自监督跟踪器，特别擅长区分视觉相似物体，解决了现有方法的未探索失败点。
...
TL;DR: 提出TALO框架，基于薄板样条进行高自由度长期对齐，解决3D视觉基础模型在时序预测中的不一致性问题
...
Conclusion: 提出的TALO框架有效解决了3D基础模型在时序预测中的对齐问题，通过高自由度薄板样条对齐和鲁棒子图配准设计，显著提升了跨时间一致性
...
<details>
  <summary>Details</summary>
Motivation: CNN在SAR任务中表现出色，但其内部机制复杂不透明，难以满足高可靠性要求，限制了在SAR中的应用。提升CNN的可解释性对SAR领域的发展部署至关重要。

Method: 提出多权重自匹配类激活映射(MS-CAM)方法，将SAR图像与CNN提取的特征图和梯度进行匹配，结合通道权重和元素权重来可视化模型在SAR图像中的决策依据。

Result: 在自建的SAR目标分类数据集上的实验表明，MS-CAM能更准确地突出网络的关注区域，捕获详细的目标特征信息，有效增强网络可解释性。同时验证了MS-CAM在弱监督目标定位中的可行性。

Conclusion: MS-CAM方法显著提升了CNN在SAR图像中的可解释性，为SAR应用提供了更可靠的决策依据，并展示了在弱监督目标定位中的潜力。文中还深入分析了像素阈值等影响定位精度的关键因素，为后续工作提供指导。
...
TL;DR: 本文分析了统一多模态模型的推理效率问题，发现理解组件具有较好的可压缩性，而生成组件对压缩高度敏感，为此提出了基于MoE的稀疏激活适配方法，在仅激活约一半参数的情况下达到与完整模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型虽然整合了理解和生成能力，但带来了推理效率问题。特定任务或样本可能不需要完整模型的知识或容量，然而目前对这些低效性在不同组件中的表现缺乏系统理解。

Method: 首先使用训练无关的剪枝作为探测方法，对统一多模态模型组件进行系统分析（包括深度剪枝和宽度缩减）。然后提出基于MoE的适配方法，将生成模块划分为多个专家并启用稀疏激活来恢复生成质量。

Result: 研究发现理解组件在理解和生成任务中都表现出显著的可压缩性，而生成组件对压缩高度敏感。提出的稀疏激活适配方法有效，经过适配的BAGEL模型在仅激活约一半参数的情况下，性能与完整模型相当。

Conclusion: 统一多模态模型存在组件效率差异，理解组件可压缩而生成组件敏感。通过MoE稀疏激活适配可以显著提升效率，在保持性能的同时大幅减少激活参数，为高效多模态模型设计提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要昂贵的标注（密度图、相机校准）或图像级人群标注。为降低部署成本，需要开发更弱监督、无需校准的方法。

Method: 1) 使用人群数量而非密度图作为单视角计数模块的监督；2) 利用多尺度先验的自监督排序损失增强感知能力；3) 利用语义信息实现更准确的视角匹配和场景级计数。

Result: 在三个广泛使用的多视角计数数据集上，该方法在弱监督设置下优于现有最优方法，表明更适合实际部署。

Conclusion: 提出的WSCF-MVCC方法通过弱监督和无需校准的方式，在降低标注成本的同时实现了优越的性能，更适合实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型主要依赖大规模真实数据或合成多样性，在基础感知任务上表现不佳，而重新训练成本高昂且收益有限，需要更高效的增强方法。

Method: 提出VACoT框架，在模型推理时动态调用图像增强（如去噪），采用结构化视觉增强集合，通过条件奖励机制的高效智能体强化学习减少训练复杂度和计算开销。

Result: 在13个感知基准测试中表现优异，特别是在OCR相关对抗场景中显著提升鲁棒性，引入AdvOCR基准展示后处理视觉增强在对抗场景中的泛化优势。

Conclusion: VACoT通过推理时动态图像增强有效提升视觉语言模型的鲁棒性，特别是在对抗性OCR场景中，为视觉增强在VLMs中的应用提供了新方向。
...
TL;DR: 本研究比较了ResNet-50和SqueezeNet两种深度学习模型在胸片结核病诊断中的性能，发现SqueezeNet在准确率、精确度、召回率等指标上均优于ResNet-50。


<details>
  <summary>Details</summary>
Motivation: 结核病诊断在资源有限地区面临挑战，传统方法如痰涂片镜检和培养效率低下，需要探索基于深度学习和计算机视觉的先进技术来改善诊断效率和准确性。

Method: 使用Kaggle的4,200张胸片数据集，对比预训练的ResNet-50模型和通用的SqueezeNet模型。预处理包括数据分割、增强和调整大小以提高训练效率，使用准确率、精确度、召回率和混淆矩阵等指标评估模型性能。

Result: SqueezeNet表现更优：损失32%，准确率89%，精确度98%，召回率80%，F1分数87%。ResNet-50表现较差：损失54%，准确率73%，精确度88%，召回率52%，F1分数65%。

Conclusion: 机器学习在结核病检测中具有潜力，特别是SqueezeNet模型表现优异，适合集成到移动设备中，在资源匮乏地区应用。但仍需开发更快、更小、更准确的模型来支持全球结核病防治工作。
...
TL;DR: 提出一种新颖的无地图轨迹预测算法，在时域、空域和频域进行轨迹预测，通过MoE机制自适应选择关键频率成分，使用选择性注意力模块过滤冗余信息，在Nuscences数据集上表现优异。
...
Method: 1. 使用时域信息处理的MoE机制自适应选择关键频率成分并集成多尺度时域特征；2. 提出选择性注意力模块过滤时域序列和空间交互中的冗余信息；3. 设计多模态解码器，在补丁级和点级损失监督下获得合理轨迹结果。

Result: 在Nuscences数据集上的实验证明了该算法的优越性，验证了其在处理复杂交互场景中的有效性。

Conclusion: 提出的无地图轨迹预测算法通过时域、空域和频域的综合处理，以及自适应频率选择和信息过滤机制，能够有效处理复杂交互场景中的轨迹预测问题，提高了预测精度和计算效率。
...
TL;DR: SAGE是一个针对隐私约束下语义分割域泛化的框架，通过合成视觉提示来对齐特征分布，无需修改模型权重。
...
Method: SAGE框架学习合成视觉提示来隐式对齐跨风格的特征分布。首先利用风格迁移构建源域的多样化风格表示，学习覆盖广泛视觉特征的风格特征集；然后模型根据每个输入的视觉上下文自适应融合这些风格线索，形成动态提示来协调图像外观而不触及模型内部。

Result: 在五个基准数据集上的广泛实验表明，SAGE在隐私约束下实现了与最先进方法相当或更优的性能，并且在所有设置中都优于完全微调的基线方法。

Conclusion: SAGE通过这种闭环设计，有效弥合了冻结模型不变性与未见域多样性之间的差距，为隐私约束下的域泛化问题提供了有效的解决方案。
...
TL;DR: 提出On-the-fly Feedback SfM框架，通过探索-利用策略实现实时无人机摄影测量，集成在线网格生成、质量评估和路径规划，提供可操作反馈以减少覆盖间隙和重飞成本。


<details>
  <summary>Details</summary>
Motivation: 传统离线无人机摄影测量无法满足灾害响应等时间关键应用需求，现有实时方法缺乏对3D重建质量的在线评估和采集指导反馈，需要更智能的自适应工作流程。

Method: 基于SfM on-the-fly，集成三个模块：1)在线增量粗网格生成用于动态扩展稀疏3D点云；2)带可操作指标的在线网格质量评估；3)用于实时轨迹优化的预测路径规划。

Result: 综合实验表明，该方法实现近实时现场重建和评估，提供可操作反馈显著减少覆盖间隙和重飞成本，支持从传统被动模式向智能自适应探索工作流程的转变。

Conclusion: 通过集成数据采集、处理、3D重建评估和在线反馈，On-the-fly Feedback SfM可作为传统被动工作模式向更智能自适应探索工作流程过渡的替代方案。
...
TL;DR: FDTA提出一个显式特征精炼框架，通过空间、时间和身份适配器增强目标嵌入的判别性，解决端到端多目标跟踪中关联准确率低的问题。
...
TL;DR: 该论文提出了一种无需人工标注的相机引导雷达标注方法，通过将雷达点云投影到相机语义分割结果并进行空间聚类，生成准确的雷达标签，解决了4D雷达语义分割中数据集稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 4D雷达在恶劣条件下具有强大的环境感知潜力，但雷达语义分割的进展受到开源数据集和标签稀缺的限制。现有的RaDelft数据集仅提供LiDAR标注且没有公开的雷达标签生成代码，限制了研究的可重复性和下游应用。

Method: 提出相机引导的雷达标注流程：将雷达点云投影到基于相机的语义分割结果中，然后应用空间聚类技术，无需依赖人工标注即可生成准确的雷达点云标签。

Result: 成功复现了RaDelft组的数值结果，并证明该方法能显著提高雷达标签的准确性。建立了可重复的框架，使研究社区能够训练和评估标注的4D雷达数据。此外，还研究和量化了不同雾度水平对雷达标注性能的影响。

Conclusion: 该工作提供了一种可重复的雷达标注框架，解决了4D雷达语义分割领域的数据集稀缺问题，为后续研究奠定了基础，并量化了恶劣天气条件对雷达标注性能的影响。
...
TL;DR: Skywork-R1V4是一个300亿参数的多模态智能体模型，通过监督微调实现图像操作与网络搜索的统一，在多项基准测试中超越Gemini 2.5 Flash，无需强化学习即可实现复杂多步任务推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法将图像操作和网络搜索视为分离能力，过度依赖昂贵的强化学习，且缺乏基于真实工具执行轨迹的规划。需要开发统一的多模态智能体系统。

Method: 构建30B参数多模态模型，统一多模态规划、主动图像操作、深度多模态搜索和交错推理。仅使用不到30,000个高质量、规划-执行一致的轨迹进行监督微调，并通过逐步一致性过滤验证。

Result: 在MMSearch上得分为66.1，在FVQA上得分为67.2，在全部11个指标上超越Gemini 2.5 Flash。能够协调超过10个工具调用来解决复杂的多步任务，展现出涌现的长时程推理能力。

Conclusion: 精心策划的监督学习足以实现复杂的多模态智能体智能，无需依赖强化学习。Skywork-R1V4展示了统一多模态能力在解决复杂任务方面的有效性。
...
TL;DR: Nav-R²：通过结构化思维链推理和相似性感知记忆，在开放词汇环境中实现目标导航，提升未见物体的定位成功率


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标导航方法存在决策过程不透明和定位未见物体成功率低的问题，需要更清晰的推理机制和更好的泛化能力
...
Conclusion: Nav-R²通过结构化推理和高效记忆机制，显著提升了开放词汇目标导航的透明度和成功率，为智能体在未知环境中定位新物体提供了有效解决方案
...
TL;DR: WISE是一个多智能体辩论框架，通过将智能体分为求解者和反思者，结合两阶段辩论模型和修改的Dawid-Skene算法，在视觉语言推理任务上比现有方法提升2-7%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在多模态任务中具有互补优势，但多智能体辩论主要应用于纯语言任务，在视觉语言推理问题上的效果尚未充分探索。需要开发能够整合单模态和多模态专家能力的通用辩论框架。

Method: 提出WISE框架：1) 将智能体分为求解者（生成解决方案）和反思者（验证正确性、分配权重、提供自然语言反馈）；2) 使用修改的Dawid-Skene算法进行后处理，整合两阶段辩论模型，考虑智能体响应差异和反馈权重；3) 支持异构专家（单模态和多模态能力）。

Result: 在SMART-840、VisualPuzzles、EvoChart-QA和新的SMART-840++数据集上评估，WISE在不同多模态任务和LLM配置中，比最先进的多智能体辩论设置和聚合方法持续提升2-7%的准确率。

Conclusion: WISE是一个通用且模块化的多智能体辩论框架，能够有效整合异构专家的能力，显著提升视觉语言推理任务的性能，证明了多智能体辩论在多模态问题中的潜力。
...
TL;DR: MitUNet是一个用于室内平面图墙体分割的混合神经网络架构，通过Mix-Transformer编码器和U-Net解码器结合scSE注意力块，采用Tversky损失优化策略，在CubiCasa5k数据集上表现出色，为3D重建提供高质量分割结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在墙体分割中存在两个主要问题：1）难以检测薄壁结构；2）生成的分割掩码边界不规则，缺乏几何精度，无法满足后续矢量化和3D重建的需求。

Method: 提出MitUNet混合神经网络架构：1）使用分层Mix-Transformer编码器捕捉全局上下文；2）U-Net解码器增强scSE注意力块实现精确边界恢复；3）采用基于Tversky损失函数的优化策略，通过调整超参数平衡精度和召回率，抑制边界假阳性噪声同时保持对薄结构的高敏感性。

Result: 在公开的CubiCasa5k数据集和专有区域数据集上的实验表明，该方法能够生成结构正确且边界精度高的掩码，优于标准的单任务模型。

Conclusion: MitUNet为自动化3D重建管道的数据准备提供了鲁棒工具，通过专门设计的架构和优化策略，有效解决了墙体分割中的薄结构检测和边界精度问题。
...
TL;DR: 提出GuiDG框架，通过训练多个参数高效的领域专家模型来提升视觉语言模型在领域泛化任务上的性能，避免单一通用模型在领域泛化能力上的妥协。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型微调面临领域特定性和领域泛化能力之间的权衡。传统方法在整个数据集上微调单一通用模型，可能会损害模型在未见领域上的泛化能力。需要一种既能保持领域特异性又能提升领域泛化的方法。

Method: 提出两阶段GuiDG框架：1）通过提示调优获得源领域专家模型；2）引入跨模态注意力模块，通过自适应专家集成来指导视觉编码器的微调。同时构建了ImageNet-DG数据集用于少样本领域泛化评估。

Result: 在标准领域泛化基准测试和ImageNet-DG数据集上的实验表明，GuiDG在保持效率的同时，优于当前最先进的微调方法。

Conclusion: 通过理论分析发现，在划分的源领域上训练多个参数高效的专家模型比微调单一通用模型具有更好的泛化能力。GuiDG框架有效解决了视觉语言模型微调中领域特定性与领域泛化之间的权衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型视觉语言模型的发展，GUI智能体任务从单屏任务转向复杂的屏幕导航挑战。然而，真实世界的GUI环境（如PC软件和移动应用）通常复杂且专有，难以获取全面的环境信息用于智能体训练和评估，这限制了智能体导航能力的系统研究和基准测试。

Method: 提出GUI Exploration Lab模拟环境引擎，支持灵活定义和组合屏幕、图标和导航图，为智能体训练和评估提供全面的环境信息访问。采用三阶段训练方法：1）监督微调用于记忆基础知识；2）单轮强化学习增强对未见场景的泛化能力；3）多轮强化学习通过交互试错鼓励探索策略发展。

Result: 在静态和交互式基准测试中验证了方法的有效性，证明研究结果能有效泛化到真实世界场景。强化学习方法在GUI导航中展现出优势，监督微调为后续训练奠定基础，单轮强化学习增强泛化能力，多轮强化学习进一步提升屏幕导航性能。

Conclusion: GUI Exploration Lab解决了GUI智能体导航研究的模拟环境需求，提出的训练方法为构建更强大、更可泛化的GUI智能体提供了实用指导，强化学习方法在GUI导航任务中具有显著优势。
...
TL;DR: WorldMM：一种多模态记忆代理，通过构建文本和视觉互补记忆，解决长视频理解中的上下文容量限制和视觉细节丢失问题，在五个长视频问答基准上平均提升8.4%


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在理解小时或天级长视频时面临挑战：上下文容量有限，抽象过程中丢失关键视觉细节。现有基于记忆增强的方法依赖文本摘要，无法利用视觉证据进行复杂场景推理，且固定时间尺度的检索限制了捕捉可变时长事件的灵活性。

Method: 提出WorldMM多模态记忆代理，构建三种互补记忆：1）情节记忆：跨多个时间尺度索引事实事件；2）语义记忆：持续更新高层概念知识；3）视觉记忆：保留场景的详细信息。推理时，自适应检索代理迭代选择最相关记忆源，基于查询利用多个时间粒度，直到收集足够信息。

Result: 在五个长视频问答基准上显著优于现有基线，平均性能比之前最先进方法提升8.4%，展示了其在长视频推理上的有效性。

Conclusion: WorldMM通过构建文本和视觉互补记忆系统，解决了长视频理解中的关键挑战，其自适应多尺度检索机制能够有效处理复杂的长视频推理任务。
...
TL;DR: 本文提出了一种名为LightHCG的新型因果表示驱动的青光眼检测模型，该模型基于卷积VAE，通过HSIC潜在空间解缠和图自编码器无监督因果表示学习，实现了轻量级且考虑青光眼真实因果关系的检测。


<details>
  <summary>Details</summary>
Motivation: 青光眼作为一种不可逆的致盲性眼病，传统AI检测方法存在可靠性不足、参数过多、可能存在虚假相关性、以及难以应用于干预分析和临床模拟等问题，需要开发更可靠、轻量且考虑真实因果关系的检测模型。

Method: 提出LightHCG模型，采用卷积VAE架构，结合HSIC（希尔伯特-施密特独立性准则）进行潜在空间解缠，并使用图自编码器进行无监督因果表示学习，以捕捉青光眼相关物理因素之间的真实因果关系。

Result: LightHCG在青光眼分类任务中表现出更高性能，同时模型参数量比现有先进视觉模型（如InceptionV3、MobileNetV2、VGG16）减少93~99%，并增强了AI驱动干预分析的可能性。

Conclusion: LightHCG作为一种轻量级因果表示驱动的青光眼检测模型，不仅提高了检测性能并大幅减少参数，还为AI驱动的干预分析和临床模拟提供了新的可能性，解决了现有AI方法在可靠性和应用范围方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 医疗领域标注数据稀缺，需要鲁棒的视觉语言模型。对比学习训练需要大批量，计算成本高，且医疗数据有限，需要从数据和模型中提取更多知识。

Method: 结合动量方法和蒸馏技术：1) 利用动量自蒸馏增强多模态学习；2) 将动量机制与梯度累积结合，在不增加资源消耗的情况下扩大有效批量大小。

Result: 在零样本分类中达到SOTA竞争性性能，少样本适应显著提升（AUC-ROC超过90%），检索任务提升2-3%，单GPU实现高效训练且保持合理训练时间。
...
TL;DR: BOLT：一种通过提取任务感知正交谱基，在共享子空间中进行低秩参数高效微调的框架，用于少样本迁移学习


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：元学习方法需要额外的元训练阶段，计算成本高且不稳定；同时大量任务特定预训练模型未被充分利用。需要一种能够重用现有微调模型、在有限数据和计算预算下高效适应新任务的方法。

Method: BOLT框架分为离线阶段和在线阶段：离线阶段从多个任务向量中提取主导奇异方向并按层正交化，形成可重用的正交基；在线阶段冻结这些基，仅训练每层的小型对角系数集，实现秩可控的参数高效更新。

Result: BOLT提供了强大的免训练初始化（通过池化源任务系数）和轻量级重缩放步骤，在参数高效微调路径中相比常见PEFT基线和代表性元学习初始化方法，实现了稳健的性能表现。

Conclusion: 将适应约束在任务感知的正交子空间中，为未见任务的迁移提供了一种有效的替代方案，能够重用现有模型知识，在有限计算资源下实现高效迁移学习。
...
TL;DR: 本文提出Temporal Dynamics Enhancer (TDE)来增强脉冲神经网络(SNN)的时间信息建模能力，包含Spiking Encoder生成多样化时间刺激和Attention Gating Module指导生成，并通过Spike-Driven Attention降低能耗。
...
Method: 提出TDE框架：1) Spiking Encoder (SE)生成跨时间步的多样化输入刺激；2) Attention Gating Module (AGM)基于时间间依赖性指导SE生成；3) Spike-Driven Attention (SDA)消除AGM引入的高能耗乘法操作。

Result: TDE可无缝集成到现有SNN检测器中，在静态PASCAL VOC数据集上达到57.7% mAP50-95，在神经形态EvDET200K数据集上达到47.6%，优于最先进方法。SDA能耗仅为传统注意力模块的0.240倍。

Conclusion: TDE有效增强了SNN的时间信息建模能力，同时通过SDA显著降低了注意力机制的能量消耗，为SNN在复杂视觉任务中的应用提供了高效解决方案。
...
Method: 深入分析nuScenes数据集的创建过程，包括其扩展版本nuImages和Panoptic nuScenes，总结未公开的技术细节。追踪nuScenes对后续数据集的影响及其定义的行业标准。综述基于nuScenes的官方和非官方任务。

Result: 提供了nuScenes数据集前所未有的创建细节，揭示了其作为首个包含雷达数据、跨两大洲城市驾驶场景、使用完全自动驾驶车辆收集的数据集的特点。展示了nuScenes对后续数据集和行业标准的深远影响。

Conclusion: nuScenes数据集在自动驾驶领域具有里程碑意义，推动了多模态传感器融合、标准化基准和广泛任务的发展。本文提供了全面的自动驾驶文献综述，特别聚焦于nuScenes数据集的影响和应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D布局估计模型主要基于单房间/单楼层合成数据训练，无法处理大型多楼层建筑，需要分割场景处理，这移除了楼梯等多楼层连接结构的全局空间上下文信息

Method: 提出HouseLayout3D真实世界基准数据集，支持全建筑尺度布局估计；提出MultiFloor3D训练免费基线方法，利用现有场景理解技术

Result: MultiFloor3D在HouseLayout3D基准和现有数据集上均优于现有3D布局估计模型，突显了该方向进一步研究的必要性

Conclusion: 需要推动全建筑尺度3D布局估计研究，HouseLayout3D基准和MultiFloor3D基线为此方向提供了基础
...
<details>
  <summary>Details</summary>
Motivation: 现有风格化运动生成模型虽然能从风格运动中理解特定风格信息并插入内容运动，但难以捕捉同一风格内的多样性（即单一风格对应多种运动变化）

Method: 提出聚类框架ClusterStyle，使用原型集建模同一风格类别内运动的多样风格模式；考虑全局级（同一类别风格运动间）和局部级（运动序列时间动态内）两种风格多样性；构建全局和局部两个结构化风格嵌入空间，通过与非学习原型锚点对齐优化；使用风格调制适配器（SMA）增强预训练文本到运动生成模型以集成风格特征

Result: 大量实验表明，该方法在风格化运动生成和运动风格迁移方面优于现有最先进模型

Conclusion: ClusterStyle通过原型建模风格多样性，有效解决了现有模型在捕捉同一风格内多样性方面的限制，在风格化运动生成任务上表现出色
...
TL;DR: STL是一个自训练框架，通过结构化推理模板（先提取视觉属性再推理）和负向原理增强，提升视觉语言模型的多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法提升视觉语言模型推理能力依赖高质量思维链数据，这些数据获取成本高（人工标注、专用模型）或忽略感知能力。需要一种成本效益高且能同时提升感知和推理的方法。

Method: 提出See-Think-Learn自训练框架：1）结构化推理模板，先"看"（提取视觉属性文本）再"思考"（基于属性推理）；2）自训练循环，模型生成并学习自己的结构化原理；3）添加负向原理（解释错误答案为何错误）增强区分能力。

Result: 跨多个领域的实验表明，STL始终优于仅基于答案或自生成推理的基线方法，定性分析确认其生成原理的高质量。

Conclusion: STL为提升视觉语言模型的多模态推理能力提供了一个成本效益高的解决方案，通过结构化自训练同时改进感知和推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 探索音频-视频联合去噪训练是否能在只关注视频质量的情况下提升视频生成效果，验证跨模态协同训练的价值

Method: 提出参数高效的AVFullDiT架构，利用预训练的文本到视频和文本到音频模块进行联合去噪训练，并与纯视频模型在相同设置下对比

Result: 音频-视频联合去噪训练在具有大运动和物体接触的挑战性子集上带来一致改进，音频预测作为特权信号帮助模型内化视觉事件与声学后果的因果关系
...
TL;DR: SEER-Bench：首个序列化具身探索与推理基准，包含EQA和EMN任务，并提出3DSPMR方法，首次将几何信息融入MLLM进行空间推理。


<details>
  <summary>Details</summary>
Motivation: 现有具身AI研究主要关注单任务场景，但现实应用中智能体常面临序列化任务，需要复用先前探索积累的空间知识来支持后续推理和探索。当前缺乏针对这一实际重要挑战的评估基准和方法。

Method: 提出SEER-Bench基准，包含序列化具身问答(EQA)和多模态导航(EMN)任务。开发3DSPMR方法，利用已探索区域的关系、视觉和几何线索来增强多模态大语言模型(MLLMs)，首次将几何信息显式融入MLLM的空间理解和推理中。

Result: 在SEER-Bench上的大量实验验证了3DSPMR方法在序列化EQA和EMN任务上均取得显著的性能提升。

Conclusion: 这是首个针对序列化具身探索与推理挑战的系统研究，提出的SEER-Bench基准和3DSPMR方法为这一实际重要问题提供了有效的评估框架和解决方案，首次实现了几何信息与MLLM的融合用于空间推理。
...
TL;DR: TGDD通过将分布匹配重新定义为沿模型训练轨迹的动态对齐过程，解决了传统方法忽略特征表示演化的问题，在保持高效的同时显著提升下游任务性能。
...
TL;DR: WorldPack提出了一种具有高效压缩记忆的视频世界模型，通过轨迹打包和记忆检索显著提升长时生成的空间一致性和质量，在Minecraft LoopNav基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型在长时建模中存在时空一致性问题，现有方法由于长上下文输入的计算成本过高而难以解决，需要更高效的记忆机制来保持长期一致性。

Method: 提出WorldPack模型，包含压缩记忆机制：1) 轨迹打包实现高上下文效率；2) 记忆检索保持rollout一致性并支持需要空间推理的长时生成。

Result: 在专门评估长时一致性的Minecraft LoopNav基准上，WorldPack显著优于现有最先进模型，在空间一致性、保真度和生成质量方面表现突出。

Conclusion: WorldPack通过高效的压缩记忆机制成功解决了视频世界模型的长时一致性问题，在更短的上下文长度下实现了更好的长时生成性能。
...
TL;DR: G-SHARP是一个用于微创手术的商业兼容实时手术场景重建框架，基于Apache-2.0许可的GSplat高斯光栅化器，实现可变形组织的快速准确3D建模。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的手术重建方法多依赖非商业衍生工具，限制了实际部署能力。需要开发商业兼容的实时手术重建系统，满足手术室实际应用需求。

Method: 基于GSplat（Apache-2.0）可微分高斯光栅化器构建原生手术管道，实现原理性变形建模、鲁棒遮挡处理和高保真重建。提供Holoscan SDK应用，可在NVIDIA IGX Orin和Thor边缘硬件上部署。

Result: 在EndoNeRF pulling基准测试中达到最先进的重建质量，具有适合术中使用的速度-精度平衡。实现了实时手术可视化。

Conclusion: G-SHARP是首个基于商业兼容高斯光栅化器的手术重建框架，为手术室环境中的实时3D组织建模提供了可行的解决方案。
...
TL;DR: UCAgents是一个分层多智能体框架，通过结构化证据审计实现单向收敛，解决医学VLM中的推理脱离问题，在提高诊断准确性的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学诊断中存在推理脱离问题，即语言解释与可验证的图像证据脱节，损害临床信任。现有的多智能体框架虽然能缓解单一模型偏见，但开放式讨论会放大文本噪声和计算成本，且未能将推理锚定在视觉证据上。

Method: 提出UCAgents分层多智能体框架，受临床工作流程启发，禁止立场改变并将智能体交互限制在有针对性的证据验证中。引入单轮询问讨论来揭示视觉-文本不对齐的潜在风险，通过信息论形式化双重噪声瓶颈。

Result: 在四个医学VQA基准测试中，UCAgents在PathVQA上达到71.3%的准确率（比最先进方法提高6.0%），同时降低87.7%的token成本，在揭示更多视觉证据和避免混淆性文本干扰之间取得平衡。

Conclusion: UCAgents在诊断可靠性和计算效率方面表现出色，这对于实际临床部署至关重要，证明了通过结构化证据审计实现单向收敛的有效性。
...
TL;DR: 提出3D-SLIM注意力掩码策略，用几何自适应掩码和指令感知掩码替代传统因果掩码，解决3D场景理解中的顺序偏见和受限注意力问题，无需架构修改即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景语言理解方法使用语言建模的标准解码器，依赖因果注意力掩码，这导致两个根本冲突：1) 顺序无关的3D对象之间存在顺序偏见；2) 对象-指令注意力受限，阻碍任务特定推理。

Method: 提出3D-SLIM（3D空间语言指令掩码），包含两个关键组件：1) 几何自适应掩码，基于空间密度而非标记顺序约束注意力；2) 指令感知掩码，使对象标记能直接访问指令上下文。该方法简单，无需架构修改，不增加额外参数。

Result: 在多种3D场景语言任务上带来显著性能提升，跨多个基准测试和LLM基线的广泛实验验证了其有效性，强调了解码器设计在3D多模态推理中的关键作用。

Conclusion: 3D-SLIM通过自适应注意力掩码策略有效解决了传统因果掩码在3D场景理解中的局限性，为3D多模态推理提供了简单而有效的解决方案，突显了注意力机制设计的重要性。
...
TL;DR: ReVSeg通过显式分解推理过程，将视频目标分割任务分解为语义解释、时序证据选择和空间定位三个步骤，利用预训练视觉语言模型进行多步推理，并通过强化学习优化推理链。


<details>
  <summary>Details</summary>
Motivation: 现有视频目标分割方法通常将复杂的动态、因果和时序交互推理折叠到潜在嵌入中，导致推理链不透明且难以追踪。需要一种显式分解的推理方法来解决这一根本问题。

Method: 采用显式分解视角，将推理过程分解为三个顺序操作：语义解释、时序证据选择和空间定位。利用预训练视觉语言模型的原生接口执行推理，并通过强化学习优化多步推理链，使模型能够从结果驱动的信号中自我改进决策质量。

Result: ReVSeg在标准视频目标分割基准测试中达到了最先进的性能，并产生了可解释的推理轨迹。

Conclusion: 通过显式分解推理过程并利用强化学习优化多步推理链，ReVSeg不仅提升了视频目标分割的性能，还提供了透明可解释的推理过程，为解决复杂推理任务提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 虽然音频驱动化身视频生成的扩散模型在合成具有自然音视频同步和身份一致性的长序列方面取得了显著进展，但具有摄像机运动的音乐表演视频生成仍未被充分探索。现有长视频生成方法缺乏明确的摄像机运动控制。

Method: 提出级联框架，包括：音频语义分析、可解释镜头规划模块(MV-Director)、时序感知扩散Transformer架构、长序列一致性建模。引入摄像机适配器模块将摄像机姿态嵌入潜在噪声，并提出基于音频嵌入的时间感知动态窗口范围策略来增强剪辑间的连续性。

Result: 构建了大规模Music-in-the-Wild数据集。综合基准测试表明，YingVideo-MV在生成连贯且富有表现力的音乐视频方面表现出色，实现了精确的音乐-动作-摄像机同步。

Conclusion: YingVideo-MV是首个音乐驱动长视频生成框架，能够自动合成高质量音乐表演视频，解决了摄像机运动控制和长序列连续性的关键问题，为音乐视频生成开辟了新方向。
...
TL;DR: 提出一种针对多模态大语言模型的新型图像中心化越狱攻击方法CIA，通过多智能体系统将有害查询嵌入看似良性的视觉上下文中，显著提高了攻击成功率。
...
Method: 提出Contextual Image Attack (CIA)方法，采用多智能体系统，通过四种不同的可视化策略将有害查询微妙地嵌入看似良性的视觉上下文中。系统还结合了上下文元素增强和自动毒性混淆技术来提升攻击效果。

Result: 在MMSafetyBench-tiny数据集上的实验结果显示，CIA对GPT-4o和Qwen2.5-VL-72B模型分别获得了4.73和4.83的高毒性分数，攻击成功率分别达到86.31%和91.07%，显著优于先前的工作。

Conclusion: 视觉模态本身是越狱先进多模态大语言模型的有效向量，CIA方法通过充分利用图像的复杂上下文信息承载能力，显著提升了攻击效果，揭示了MLLMs安全对齐的新脆弱性。
...
TL;DR: MedSeg-TTA是一个全面的医学图像分割测试时适应基准，系统评估了20种代表性方法在7种成像模态下的表现，发现不同范式在不同条件下各有优劣，强调临床部署需要基于原则的方法选择。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像分割的测试时适应评估存在模态覆盖有限、任务多样性不足和方法一致性缺乏的问题，需要建立一个全面、统一的基准来系统评估不同适应方法的可靠性和临床适用性。

Method: 提出了MedSeg-TTA基准，在完全统一的数据预处理、骨干网络配置和测试时协议下，评估20种代表性适应方法在7种成像模态（MRI、CT、超声、病理、皮肤镜、OCT、胸片）上的表现，涵盖输入级变换、特征级对齐、输出级正则化和先验估计四大范式。

Result: 没有单一范式在所有条件下表现最佳：输入级方法在轻微外观变化下更稳定；特征级和输出级方法在边界相关指标上优势更大；先验方法表现出强烈的模态依赖性。许多方法在大规模跨中心和跨设备变化下性能显著下降。

Conclusion: MedSeg-TTA为医学图像分割的测试时适应研究提供了标准化数据集、验证实现和公开排行榜，强调了临床部署需要基于原则的方法选择，并为未来开发稳健、临床可靠的测试时适应方法奠定了严格基础。
...
TL;DR: dots.ocr是一个统一的视觉语言模型，首次将文档布局解析的三个核心任务（布局检测、文本识别、关系理解）在端到端框架中联合学习，在多语言文档理解方面取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前文档布局解析方法依赖碎片化的多阶段流水线，存在错误传播问题，且无法利用联合训练的优势。需要一种统一框架来同时处理布局检测、文本识别和关系理解，以提升文档理解能力。

Method: 提出dots.ocr单一视觉语言模型，通过高度可扩展的数据引擎合成大规模多语言语料库，在统一的端到端框架中联合学习三个核心任务，支持多种语言、布局和领域。

Result: 在OmniDocBench上取得最先进性能；在新基准XDocParse（涵盖126种语言）上超越次优竞争者7.4个百分点，展现了卓越的多语言能力。

Conclusion: dots.ocr证明了统一端到端框架在文档布局解析中的优势，为全球文档智能研究提供了强大的新基准，展示了在多语言文档理解方面的卓越能力。
...
TL;DR: GeoDiT：首个面向地理空间领域的扩散视觉语言模型，通过并行细化过程实现结构化、对象中心的输出，在图像描述、视觉定位和多目标检测任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自回归模型与地理空间理解的并行本质存在结构性不匹配，强制将场景转化为刚性序列叙述，阻碍了结构化、连贯输出的生成。需要重新思考地理空间生成的范式。

Method: 将地理空间生成重构为并行细化过程，实现从粗到细的整体合成，同时解析所有语义元素。引入GeoDiT——首个面向地理空间领域的扩散视觉语言模型。

Result: GeoDiT在需要结构化、对象中心输出的基准测试中建立了新的SOTA。在图像描述、视觉定位和多目标检测任务上取得显著提升，这些正是自回归模型表现不佳的任务。
...
TL;DR: ViT-SR：一种基于Vision Transformer的两阶段训练方法，通过自监督着色预训练提升单图像超分辨率性能


<details>
  <summary>Details</summary>
Motivation: 单图像超分辨率（SISR）在计算机视觉中仍然是一个难题，传统方法可能无法充分学习到丰富的视觉表示。作者希望通过自监督预训练来提升模型性能。

Method: 采用两阶段训练策略：1）自监督预训练阶段，在着色任务上学习通用视觉表示；2）微调阶段，将预训练模型调整为4倍超分辨率任务，通过预测高频残差图像来简化残差学习。
...
TL;DR: SkyMoE是一个基于专家混合(MoE)的视觉语言模型，专门用于遥感多模态多任务解释，通过任务和粒度感知的路由机制实现局部细节感知与全局上下文理解的平衡。
...
Method: 1) 采用Mixture-of-Experts架构，通过自适应路由器生成任务和粒度感知的路由指令；2) 引入上下文解耦增强策略，创建局部与全局特征的对比对；3) 构建MGRS-Bench综合基准测试。

Result: 在21个公共数据集上的实验表明，SkyMoE在多个任务上达到最先进性能，验证了其在复杂场景中的适应性、可扩展性和多粒度理解能力。

Conclusion: SkyMoE通过专家混合架构和粒度感知机制，有效解决了遥感多任务解释中局部与全局理解的平衡问题，为遥感视觉语言模型提供了新的解决方案。
...
TL;DR: 这篇博士论文研究了零样本异常分类与分割的核心挑战，提出了基于理论和算法设计的解决方案，包括识别一致异常问题、开发CoDeGraph框架、扩展到3D医学影像，以及连接批处理和文本方法。


<details>
  <summary>Details</summary>
Motivation: 工业检测和医学影像中需要无需训练数据就能检测异常样本和区域的零样本异常分类与分割方法。论文旨在解决零样本AC/AS的核心挑战，特别是"一致异常"问题，即重复出现的相似异常会系统性地偏置基于距离的方法。

Method: 1. 分析预训练Vision Transformers中补丁表示的统计和几何行为，识别相似性缩放和邻居烧毁现象；2. 提出基于图的CoDeGraph框架，通过多阶段图构建、社区检测和结构化精炼来过滤一致异常；3. 扩展到3D医学影像，提出无需训练的计算高效体积标记化策略；4. 用CoDeGraph生成的伪掩码监督提示驱动的视觉语言模型。

Result: 1. 识别了描述正常补丁关系变化的关键现象；2. CoDeGraph能有效抑制一致异常的影响；3. 实现了无需3D训练样本的真正零样本3D异常检测；4. 成功连接了批处理和文本方法，展示了伪掩码监督视觉语言模型的可行性。

Conclusion: 该论文为零样本异常分类与分割问题提供了理论理解和实用解决方案，通过识别核心挑战、开发算法框架、扩展到3D领域以及连接不同方法，为工业检测和医学影像中的异常检测提供了全面的方法体系。
...
Method: 1) Noisy Query Tokens：通过端到端优化学习VLM与扩散模型之间的分布式表示空间；2) 引入带线性投影的VAE分支来恢复细粒度图像细节

Result: 实验结果表明该方法缓解了泛化崩溃问题，能够在多样化任务上实现稳定的持续学习

Conclusion: 提出的方法通过分布式表示学习和细节恢复机制，有效解决了多模态大语言模型中VLM与扩散模型桥接时的泛化崩溃问题
...
TL;DR: 提出一种无需训练的两步加速方案，通过分析VGGT和π³模型的全局注意力机制，将早期全局层转换为帧注意力，并对全局注意力进行子采样，实现8-10倍推理加速，同时保持或略微提升原始模型精度。


<details>
  <summary>Details</summary>
Motivation: 现有多视图3D模型如VGGT和π³虽然性能强大，但过度依赖全局自注意力导致计算成本高昂。现有稀疏注意力变体只能提供部分加速，且缺乏对全局注意力在多视图推理中作用的系统性分析。
...
Result: 在标准姿态和点云基准测试中，该方法实现了8-10倍的推理时间加速，同时匹配或略微提升了原始模型的精度。即使在极端密集的多视图设置下，该方法仍保持鲁棒性，而先前的稀疏注意力基线在此情况下会失败。

Conclusion: 通过系统性分析全局注意力在多视图推理中的作用，提出的训练免费加速方案能显著降低计算成本，同时保持模型性能，为高效多视图3D理解提供了实用解决方案。
...
Method: 提出OmniPerson统一生成模型：1)支持RGB/IR模态图像/视频生成，具有多参考图像、姿态和文本控制；2)设计Multi-Refer Fuser实现多参考图像的身份融合；3)创建PersonSyn数据集及其自动标注流程。

Result: OmniPerson在行人生成任务上达到SOTA，在视觉保真度和身份一致性方面表现优异。使用生成数据增强现有数据集能持续提升ReID模型性能。

Conclusion: OmniPerson解决了行人重识别数据不足问题，通过统一的身份保持生成框架实现了高质量可控的行人生成，为ReID任务提供了有效的数据增强方案。
...
TL;DR: Panel2Patch：从生物医学文献中挖掘层次化视觉语言对齐数据的新方法，通过解析多面板、标记丰富的图表构建从整体到局部的多粒度监督信号，显著提升预训练效率。


<details>
  <summary>Details</summary>
Motivation: 当前生物医学视觉语言预训练通常将丰富的科学图表和文本压缩为粗略的图表级配对，丢弃了临床医生实际依赖的细粒度对应关系。需要一种能保留局部语义的方法来构建更有效的监督信号。

Method: Panel2Patch数据管道：1）解析科学图表的布局、面板和视觉标记；2）构建图表、面板和补丁三个层次的视觉语言对齐对；3）开发粒度感知的预训练策略，统一从粗粒度教学描述到细粒度区域聚焦短语的异构目标。

Result: 仅使用少量文献图表，Panel2Pipeline就能提取比先前方法更有效的监督信号，在更少的预训练数据下实现显著更好的性能。

Conclusion: 通过挖掘生物医学文献中的层次结构并构建多粒度监督，Panel2Patch为开发强大的生物医学视觉语言模型提供了更有效的数据管道，能够更好地捕捉临床医生关注的局部语义信息。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用音频与运动特征距离或共享特征空间检索运动轨迹，但音频与手势之间存在多对多映射关系，一对一映射方法无法充分处理这种复杂性

Method: 1) 使用扩散模型生成手势动作，学习音频与运动的联合分布；2) 从输入音频提取低层和高层特征丰富扩散模型训练；3) 设计基于运动的检索算法，评估全局和局部运动相似性；4) 对检索路径中不连续节点进行无缝拼接

Result: 实验结果表明，该方法在同步准确性和生成手势自然度方面显著优于先前方法

Conclusion: 提出的框架通过扩散模型学习音频-手势联合分布，结合多级特征提取和智能检索拼接，有效解决了协同语音手势视频生成的挑战
...
TL;DR: RULER-Bench是一个评估视频生成模型推理能力的基准，专注于认知规则而非传统视觉感知指标，涵盖40个任务和622个实例，当前SOTA模型在规则一致性上仅得48.87%。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成评估主要关注视觉感知和理解（如美学质量、指令遵循、时间一致性），但缺乏对模型基于规则的推理能力的系统评估。尽管有初步探索视频模型作为零样本学习者的研究，但仍缺少对推理能力的细粒度分解和全面评估协议。

Method: 构建RULER-Bench基准，基于文本到视频和图像到视频两种基本范式，涵盖6个规则类别的40个代表性任务，包含622个高质量标注实例。通过包含4个指标的检查表评估生成视频，利用GPT-4o进行评分（与人工判断一致性达85%）。

Result: 实验表明，当前最先进的视频生成模型在规则一致性指标上仅达到48.87%，表明下一代视频模型在推理能力方面仍有显著改进空间。
...
TL;DR: PPTBench是一个用于评估多模态大语言模型在PowerPoint相关任务上表现的综合性基准，涵盖检测、理解、修改和生成四大类别，揭示当前模型在视觉布局推理方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅关注狭窄的子任务，忽视了布局理解这一核心挑战，而布局理解对于现实世界的幻灯片创建和编辑至关重要。PowerPoint结合了丰富的文本内容和结构化视觉布局，是评估多模态模型能力的理想测试平台。

Method: 基于958个PPTX文件的多样化来源，构建了包含4,439个样本的PPTBench基准，涵盖检测、理解、修改和生成四个任务类别。通过实验分析模型在语义理解和视觉布局推理方面的表现，并进行消融研究和案例分析。

Result: 实验显示当前多模态大语言模型在语义理解和视觉布局推理之间存在显著差距：模型能够解释幻灯片内容，但无法生成连贯的空间排列。模型难以将视觉线索与JSON布局结构相结合，也无法将视觉信息整合到API规划能力中。案例分析揭示了系统性布局错误，如错位和元素重叠。

Conclusion: PPTBench为评估视觉语言模型在PPT场景中的表现提供了新视角，突显了视觉结构推理和连贯幻灯片生成方面的挑战，为未来研究指明了方向。所有数据集和代码已完全开源以支持可复现性和后续研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在遥感图像融合中存在泛化能力差的问题，主要原因是真实训练数据有限且不同卫星传感器之间存在领域差异。需要探索基础模型潜力来解决这一挑战。

Method: 提出新颖的预训练策略：1）构建多样化模拟数据集，对ImageNet自然图像和SkyScript遥感图像应用各种退化操作（模糊、噪声、下采样）和数据增强（波段生成、通道混洗、高通滤波、颜色抖动等）；2）在这些模拟数据上预训练融合模型以学习可泛化的空间-光谱表示；3）在六个数据集上评估，采用零样本和单样本范式，使用全调优和冻结调优方法。

Result: 预训练策略显著提升了不同网络架构（CNN、Transformer、Mamba）在各种融合模型中的泛化性能。预训练模型在零样本场景中取得优异结果，在单样本设置中展现出卓越的适应能力。在跨传感器和成像条件下均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏能够同时捕获传统面部标志点和毛孔尺度关键点轨迹的基准数据集，限制了通过分析细微皮肤表面运动来研究精细面部表情的能力。

Method: 创建了包含超过440,000条面部轨迹的数据集，其中52,000多条超过10帧，68条手动审核的轨迹跨越整个150帧。系统评估了最先进的动态3D高斯泼溅方法，并建立了数据创建流程框架。

Result: 建立了该领域的首个性能基线，数据集公开可用，为高保真面部运动捕捉和动态3D重建建立了新框架。

Conclusion: PoreTrack3D是首个同时捕获传统面部标志点和毛孔尺度关键点轨迹的基准数据集，推动了通过分析细微皮肤表面运动来研究精细面部表情的进展。
...
TL;DR: 提出SelVA模型，通过文本提示作为目标声音源的显式选择器，从多对象视频中生成用户指定的单一声音，解决了现有方法只能生成混合声音的问题。
...
Method: 提出SelVA模型：1) 将文本提示作为目标声源的显式选择器；2) 调制视频编码器以提取与提示相关的视频特征；3) 使用补充令牌通过抑制文本无关激活来促进跨注意力；4) 采用自增强方案解决单声道音频监督数据不足的问题。

Result: 在VGG-MONOAUDIO基准测试中，SelVA在音频质量、语义对齐和时间同步方面表现出色，实验验证了其有效性。

Conclusion: SelVA成功实现了文本条件下的选择性视频到音频生成，能够从多对象视频中准确生成用户指定的单一声音，为多媒体制作提供了精确的音频控制能力。
...
TL;DR: 提出Snappy：结合视觉语言模型和OCR的混合架构，利用ColPali的patch级相似度分数作为空间相关性过滤器，实现文档区域级检索而非整页检索。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如ColPali）虽然能实现细粒度相似度计算，但只能返回整页结果，限制了在RAG中的实用性；而OCR系统虽然有坐标信息但缺乏语义相关性评估能力。

Method: 提出混合架构：统一两种范式，使用ColPali的patch级相似度分数作为OCR提取区域的空间相关性过滤器，形式化视觉transformer patch网格与OCR边界框的坐标映射，引入交集度量进行相关性传播。

Result: 建立了检索精度的理论界限，方法在推理时无需额外训练，发布了开源实现Snappy，实证评估正在进行中。

Conclusion: 提出的混合方法结合了视觉语言模型的语义理解能力和OCR的结构化文本提取优势，实现了文档区域级精确检索，有望提升RAG系统的实用性。
...
TL;DR: 提出PolarGuide-GSDR框架，将偏振先验嵌入3D高斯泼溅优化，实现高保真反射分离和全场景重建，无需环境贴图或材料假设，保持实时渲染能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限：基于NeRF的偏振感知方法训练慢、渲染效率低、依赖材料/视角假设；3DGS虽能实时渲染但难以准确重建反射，且添加延迟反射模块会引入环境贴图依赖。
...
<details>
  <summary>Details</summary>
Motivation: 反无人机跟踪研究虽然探索了RGB、TIR和RGB-T融合等多种模态，但缺乏统一的跨模态协作框架。现有方法主要关注独立任务的独立模型，忽视了跨模态信息共享的潜力，且当前解决方案在多模态数据融合方面效果不佳。

Method: 提出UAUTrack统一单目标跟踪框架，采用单流、单阶段、端到端架构。关键创新是引入文本先验提示策略，引导模型在不同场景下专注于无人机目标，有效整合多种模态信息。

Result: 在Anti-UAV和DUT Anti-UAV数据集上实现了最先进的性能，在Anti-UAV410数据集上保持了精度和速度的良好平衡，展示了在各种反无人机场景下的高精度和实际效率。

Conclusion: UAUTrack通过统一的跨模态协作框架解决了反无人机跟踪中的多模态融合挑战，其文本先验提示策略有效提升了模型性能，为反无人机跟踪领域提供了高效实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的超分辨率方法通常依赖大型骨干网络（如SDXL、DiT），导致训练和推理时计算和内存成本过高。需要开发轻量级扩散方法来解决这个问题。

Method: 1. 识别扩散骨干网络中的块内冗余，提出渐进式剪枝方法去除冗余块同时保留恢复能力；2. 提出相位交换适配器模块，利用输入的相位信息指导剪枝后的扩散模型以获得更好的恢复性能；3. 将渐进式剪枝和相位交换适配器集成到统一模型中。

Result: 大量实验表明，该方法在显著减少计算负载和内存消耗的同时，实现了具有竞争力的恢复质量。

Conclusion: PGP-DiffSR通过去除扩散模型中的冗余信息并结合相位信息指导，成功实现了高效轻量级的图像超分辨率，在质量和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D手部姿态估计方法在处理严重遮挡（如双手重叠）时存在困难，需要更好的局部特征与全局上下文关系建模。现有方法主要依赖ResNet等CNN架构，其归纳偏置可能不适合3D HPE任务，因为CNN建模全局上下文的能力有限。

Method: 提出Deformable Mamba（DF-Mamba）框架，基于状态空间建模（Mamba）进行视觉特征提取。核心创新包括：1）利用Mamba的选择性状态建模捕获全局上下文；2）提出可变形状态扫描，在聚合局部特征的同时选择性保留代表全局上下文的有用线索。

Result: 在五个多样化数据集上进行了广泛评估，包括单手和双手场景、手部单独和手物交互、RGB和深度估计。DF-Mamba在所有数据集上超越了最新的图像骨干网络（包括VMamba和Spatial-Mamba），实现了最先进的性能，同时推理速度与ResNet-50相当。

Conclusion: DF-Mamba通过结合状态空间建模和可变形扫描，有效解决了3D手部姿态估计中的遮挡问题，显著提升了结构化3D HPE的准确性，为视觉特征提取提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于槽注意力的对象中心表示学习方法通常不加区分地处理前景和背景区域，导致背景干扰和真实世界数据上实例发现性能不佳。需要一种能够显式分离前景和背景的方法来改进对象发现。

Method: 提出两阶段前景感知槽注意力框架：第一阶段通过双槽竞争机制进行粗粒度场景分解，区分前景和背景区域；第二阶段引入掩码槽注意力机制，第一个槽捕获背景，其余槽竞争表示单个前景对象。使用基于自监督图像特征构建的补丁亲和图的伪掩码指导来引导前景槽学习，解决过度分割问题。

Result: 在合成和真实世界数据集上的广泛实验表明，FASA始终优于最先进的方法，验证了显式前景建模和伪掩码指导对于鲁棒场景分解和对象一致表示的有效性。

Conclusion: FASA通过显式前景建模和伪掩码指导，有效解决了背景干扰问题，提高了无监督对象发现的性能，为鲁棒场景分解和对象一致表示提供了有效解决方案。
...
TL;DR: 提出RAMF框架，通过局部-全局上下文融合和语义交叉注意力解决多模态融合问题，引入对抗性推理增强对仇恨内容的理解，在两个真实数据集上超越SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 在线视频中的仇恨言论日益严重，现有方法难以有效融合多模态复杂语义关系，缺乏对微妙仇恨内容的理解能力。

Method: 提出RAMF框架：1) 局部-全局上下文融合(LGCF)捕捉局部显著线索和全局时间结构；2) 语义交叉注意力(SCA)实现细粒度多模态语义交互；3) 对抗性推理三阶段过程（客观描述、仇恨假设推理、非仇恨假设推理）提供互补语义视角。

Result: 在两个真实仇恨视频数据集上评估，相比SOTA方法，Macro-F1提升3%，仇恨类召回率提升7%，表现出强大的泛化性能。

Conclusion: RAMF框架通过创新的多模态融合和对抗性推理机制，有效解决了视频仇恨言论检测中的语义融合和微妙理解挑战，取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 异常分割对于自动驾驶安全至关重要，但异常数据的稀缺性和有限多样性严重限制了模型在开放世界环境中的泛化能力。现有合成数据方法缺乏上下文连贯性和物理真实性，导致合成数据与真实数据之间存在领域差距。

Method: 提出ClimaDrive框架，统一了结构引导的多天气生成和提示驱动的异常修复，能够创建视觉真实的训练数据。基于此框架构建了ClimaOoD基准，涵盖六种代表性驾驶场景和不同天气条件。
...
### [80] [ALDI-ray: Adapting the ALDI Framework for Security X-ray Object Detection](https://arxiv.org/abs/2512.02696)
*Omid Reza Heidari,Yang Wang,Xinxin Zuo*

Main category: cs.CV

TL;DR: ALDI++ 域适应框架在安全X射线图像检测中超越现有方法，结合自蒸馏、特征对齐和增强训练策略，在EDS数据集上取得最佳性能，ViTDet骨干网络表现最优。
...
Method: 应用ALDI++域适应框架，该框架集成了自蒸馏、特征对齐和增强训练策略，以有效缓解域偏移。使用Vision Transformer for Detection (ViTDet)作为骨干网络。

Result: 在EDS数据集上的广泛实验表明，ALDI++在多个适应场景中超越了最先进的域适应方法。使用ViTDet骨干的ALDI++实现了最高的平均精度(mAP)，类别分析显示在所有对象类别上都有一致的改进。

Conclusion: ALDI++为域自适应目标检测提供了高效解决方案，在安全X射线图像中建立了性能稳定性和跨域泛化的新基准，证明了基于Transformer的架构在跨域目标检测中的有效性。
...
TL;DR: 对抗训练意外增强了对抗样本的可迁移性，形成新的安全风险


<details>
  <summary>Details</summary>
Motivation: 尽管对抗训练是提升模型鲁棒性的主要方法，但其对对抗攻击可迁移性的影响尚未充分研究。本文旨在探究对抗训练是否会无意中增加对抗样本的可迁移性。

Method: 训练了包含CNN和ViT在内的36个多样化模型，并进行全面的可迁移性实验，研究对抗训练模型生成的扰动与标准模型的比较。

Result: 发现了一个明显悖论：对抗训练模型产生的扰动比标准模型更有效地迁移，这引入了新的生态系统风险。

Conclusion: 鲁棒性评估不仅应衡量模型对迁移攻击的抵抗能力，还应评估其产生可迁移对抗样本的倾向性。作者发布了所有模型、代码和实验脚本以促进可重复性和进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 传统卫星中心的地理定位方法在高分辨率或最新卫星图像不可用时鲁棒性受限，且未能充分利用跨视图（无人机、卫星、街景）和跨模态（语言和图像）的互补线索。

Method: 提出GeoBridge基础模型，采用新颖的语义锚机制，通过文本描述桥接多视图特征；构建了首个大规模跨模态多视图对齐数据集GeoLoc（包含5万+对图像和文本描述）。

Result: 实验证实GeoLoc预训练显著提高了GeoBridge的地理定位精度，同时促进了跨域泛化和跨模态知识迁移。

Conclusion: GeoBridge通过语义锚机制和GeoLoc数据集，实现了更鲁棒、灵活的地理定位，超越了传统的卫星中心范式，支持跨视图双向匹配和语言到图像检索。
...
TL;DR: VLM-Pruner：一种无需训练、平衡冗余性和空间稀疏性的视觉语言模型token剪枝算法，通过离心剪枝范式和空间稀疏性缓冲准则，在保持88.9%剪枝率的同时提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型(VLMs)的视觉token数量庞大导致计算成本高，阻碍在移动设备上的部署。现有剪枝方法要么只关注token重要性而忽略token间冗余，要么忽略token的空间关系，导致保留的token过于稀疏无法充分覆盖目标对象区域。

Method: 提出VLM-Pruner训练free token剪枝算法：1) 离心token剪枝范式，实现从近到远的选择同时优先保留细粒度对象细节；2) 空间稀疏性缓冲(BSS)准则，延迟选择空间距离较远的token；3) 并行贪婪策略高效进行token选择；4) 选择性融合被剪枝token的显著信息到保留token中。

Result: 在5个VLM模型上，VLM-Pruner在88.9%的剪枝率下持续优于强基线方法，同时实现了端到端推理加速。

Conclusion: VLM-Pruner通过平衡冗余性和空间稀疏性，有效解决了现有VLM token剪枝方法的局限性，在保持高性能的同时显著降低了计算成本，适合移动设备部署。
...
### [84] [Tissue-mask supported inter-subject whole-body image registration in the UK Biobank -- A method benchmarking study](https://arxiv.org/abs/2512.02702)
...
<details>
  <summary>Details</summary>
Motivation: UK Biobank收集了大量全身MR图像和非成像健康数据，需要准确的主体间图像配准来实现身体空间标准化，以及非成像数据与图像衍生参数（如组织体积或脂肪含量）的区域/体素相关性分析。

Method: 提出性别分层的全身MR图像配准方法，使用VIBESegmentator生成的皮下脂肪组织和肌肉掩膜来增强基于强度的图割配准。

Result: 在4000名受试者子集上评估，相比仅基于强度的方法，平均Dice分数提高6个百分点；相比uniGradICON提高8-9个百分点；相比MIRTK提高12-13个百分点。年龄相关性图谱噪声更少，解剖对齐更好。
...
TL;DR: 本文提出MRD框架，通过多分辨率语义融合和开放词汇目标检测来解决高分辨率图像理解中语义相似性偏差和物体碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法将高分辨率图像分割成小图块，使用预训练的RAG模型计算每个图块与查询的语义相似性，选择最相关的图块来定位目标物体并抑制无关信息。但这种方法会将完整物体分割到多个图块中，破坏语义相似性计算，导致语义相似性偏差。

Method: 提出MRD框架：1) 多分辨率语义融合方法，整合不同分辨率下获得的语义相似性图，产生更准确的语义信息并保持目标物体的完整性；2) 引入开放词汇目标检测模型，使用滑动窗口方法在全局尺度上直接定位目标物体区域。

Result: 在不同MLLMs上进行的高分辨率图像理解基准测试实验证明了该方法的有效性。

Conclusion: MRD框架通过多分辨率处理和开放词汇检测，有效解决了高分辨率图像理解中的语义相似性偏差和物体碎片化问题，提高了MLLMs处理高分辨率图像的能力。
...
TL;DR: GeoViS：一种地理空间奖励视觉搜索框架，将遥感视觉定位重新定义为渐进式搜索推理过程，通过树状视觉线索序列主动探索全局图像，实现小目标检测和复杂地理空间关系理解。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉定位方面取得显著进展，但将其能力迁移到遥感图像仍面临挑战：目标在千米级场景中极其微小，查询通常涉及复杂的地理空间关系（相对位置、空间层次、跨距离对象的上下文依赖）。
...
TL;DR: EGGS提出了一种可交换的高斯泼溅方法，通过整合2D和3D高斯表示来平衡渲染的外观质量和几何精度，解决了现有方法在视角一致性和纹理细节之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)虽然能实现实时渲染和高保真外观，但存在多视角不一致问题，限制了几何精度；而2D高斯泼溅(2DGS)虽然能保证多视角一致性，但会牺牲纹理细节。需要一种能平衡外观和几何质量的方法。

Method: 提出可交换高斯泼溅(EGGS)，包含三个关键技术：1) 混合高斯光栅化实现统一渲染；2) 自适应类型交换机制，动态调整2D和3D高斯之间的转换；3) 频率解耦优化策略，有效利用不同类型高斯的优势。

Result: 实验表明EGGS在渲染质量、几何精度和效率方面均优于现有方法，为高质量新视角合成提供了实用解决方案。

Conclusion: EGGS通过整合2D和3D高斯的优势，成功平衡了渲染的外观质量和几何精度，解决了现有方法的局限性，为新视角合成任务提供了高效且高质量的解决方案。
...
TL;DR: VideoScience-Bench：首个评估视频模型科学推理能力的基准，涵盖物理化学200个场景，要求模型理解科学概念生成正确现象


<details>
  <summary>Details</summary>
Motivation: 现有视频基准主要基于物理常识，无法评估视频模型的科学推理能力。需要开发能评估本科级别科学理解的基准，以推动视频生成模型在真实世界科学规律理解方面的发展

Method: 创建包含200个提示的VideoScience-Bench基准，涵盖14个主题和103个物理化学概念。使用专家标注评估7个最先进的视频模型，从五个维度评估：提示一致性、现象一致性、正确动态性、不变性和时空连续性。采用VLM-as-a-Judge方法评估视频生成

Result: VLM-as-a-Judge评估方法与人类评估有强相关性。基准成功评估了视频模型作为生成器和推理器的双重能力，发现现有模型在科学理解方面仍有局限

Conclusion: VideoScience-Bench是首个评估视频模型科学推理能力的基准，为视频生成模型的科学理解能力评估提供了重要工具，推动了零样本推理视频模型的发展
...
TL;DR: CAEVL：一种无需无人机训练图像的视觉定位方法，通过卫星图像增强模拟无人机视角，在GNSS拒止环境下实现高效定位


<details>
  <summary>Details</summary>
Motivation: 解决GNSS拒止环境下无人机视觉定位的挑战。现有方法需要大量配对的无人机-卫星图像数据集进行训练，这些数据获取成本高且往往不可用，限制了应用范围。

Method: 采用无需无人机图像的训练范式，直接从卫星参考图像学习。通过专门的增强策略模拟卫星视图与真实世界无人机视图之间的视觉域偏移。提出CAEVL高效模型，并在新发布的ViLD真实无人机图像数据集上进行验证。

Result: 该方法在无需配对数据训练的情况下，取得了与使用配对数据训练方法相竞争的性能，证明了其有效性和强大的泛化能力。
...
TL;DR: AttMetNet：一种基于注意力增强深度学习框架，利用Sentinel-2卫星影像进行甲烷羽流检测，通过融合NDMI指数和注意力机制U-Net，显著提升检测精度并降低误报率。


<details>
  <summary>Details</summary>
Motivation: 甲烷是强效温室气体，准确检测甲烷排放对应对气候变化至关重要。传统检测方法依赖波段差异或比值，但误报率高且需专家验证；现有深度学习方法缺乏针对甲烷特征的优先级机制。

Method: 提出AttMetNet框架：1）融合归一化甲烷差异指数（NDMI）与注意力增强U-Net，选择性放大甲烷吸收特征并抑制背景噪声；2）采用焦点损失函数处理类别不平衡问题；3）在真实甲烷羽流数据集上训练。

Result: 实验表明AttMetNet优于现有方法：误报率更低、查准率-查全率平衡更好、IoU更高，在真实卫星影像中实现更稳健的甲烷羽流检测。

Conclusion: AttMetNet首次将NDMI与注意力机制U-Net结合，为卫星影像甲烷检测提供了专门化架构，显著提升检测性能，有助于更准确监测甲烷排放以应对气候变化。
...
<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术中电刀或激光产生的烟雾会遮挡手术视野，现有去烟方法未能考虑不同烟雾类型（扩散烟雾和环境烟雾）在时空特征上的差异，导致去烟效果不佳。

Method: 提出STANet网络，包含烟雾掩码分割子网络（基于注意力加权掩码聚合进行烟雾掩码和类型预测）和无烟视频重建子网络（基于两种烟雾掩码指导去烟）。为解决两种烟雾纠缠问题，在掩码分割网络中嵌入粗到细解缠模块，通过烟雾类型感知的交叉注意力机制分离纠缠区域。同时构建了首个带烟雾类型标注的大规模合成视频去烟数据集。

Result: 实验表明，该方法在质量评估上优于现有最优方法，并在多个下游手术任务中展现出优越的泛化能力。

Conclusion: 通过区分烟雾类型并设计相应的解缠机制，STANet能够更有效地去除腹腔镜视频中的烟雾，提升手术视觉引导质量，为手术辅助系统提供更好的支持。
...
TL;DR: 提出端到端双耳空间音频生成任务，直接从无声视频生成双耳音频，解决现有方法两阶段流程导致的误差累积和时空不一致问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频到音频生成主要关注单声道输出，缺乏空间沉浸感。现有的双耳方法采用两阶段流程（先生成单声道音频再空间化），导致误差累积和时空不一致。

Method: 提出ViSAudio框架，采用条件流匹配和双分支音频生成架构，两个专用分支建模音频潜在流，结合条件时空模块平衡通道一致性和空间特性，确保音频与输入视频的精确时空对齐。

Result: ViSAudio在客观指标和主观评估上均优于现有最先进方法，能生成高质量双耳音频，有效适应视角变化、声源运动和不同声学环境。

Conclusion: 该研究成功实现了端到端双耳空间音频生成，通过ViSAudio框架和BiAudio数据集推动了该领域发展，为视频音频生成提供了更好的空间沉浸体验。
...
### [93] [TrackNetV5: Residual-Driven Spatio-Temporal Refinement and Motion Direction Decoupling for Fast Object Tracking](https://arxiv.org/abs/2512.02789)
...
<details>
  <summary>Details</summary>
Motivation: TrackNet系列在快速移动小物体跟踪方面建立了强基线，但存在显著限制：V1-V3依赖纯视觉线索难以处理遮挡；TrackNetV4虽然引入了运动输入，但其绝对差分方法丢弃了运动极性，导致方向模糊性。需要克服这些瓶颈以实现更鲁棒的跟踪。
...
### [94] [Video4Spatial: Towards Visuospatial Intelligence with Context-Guided Video Generation](https://arxiv.org/abs/2512.03040)
*Zeqi Xiao,Yiwei Zhao,Lingxiao Li,Yushi Lan,Yu Ning,Rahul Garg,Roshni Cooper,Mohammad H. Taghavi,Xingang Pan*

Main category: cs.CV

TL;DR: 视频生成模型仅通过视觉数据就能展现空间智能，Video4Spatial框架证明视频扩散模型仅基于视频场景上下文就能执行复杂空间任务


<details>
  <summary>Details</summary>
Motivation: 研究视频生成模型是否仅通过视觉数据就能展现人类认知核心的视觉空间智能，探索纯视频输入下的空间理解能力

Method: 提出Video4Spatial框架，使用仅基于视频的场景上下文条件化的视频扩散模型，通过简单的设计选择和数据整理，在纯视频输入下执行场景导航和物体定位任务

Result: 模型能端到端规划导航和定位目标物体，遵循相机姿态指令同时保持空间一致性，并能泛化到长上下文和域外环境

Conclusion: 这些结果推动视频生成模型向通用视觉空间推理发展，证明仅通过视频数据就能实现复杂的空间理解能力
...
### [95] [UnicEdit-10M: A Dataset and Benchmark Breaking the Scale-Quality Barrier via Unified Verification for Reasoning-Enriched Edits](https://arxiv.org/abs/2512.02790)
*Keming Ye,Zhipeng Huang,Canmiao Fu,Qingyang Liu,Jiani Cai,Zheqi Lv,Chen Li,Jing Lyu,Zhou Zhao,Shengyu Zhang*

Main category: cs.CV

TL;DR: 本文提出了UnicEdit-10M数据集和UnicBench基准，通过轻量级数据管道解决多模态图像编辑模型训练数据稀缺和质量问题，并引入新指标评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着GPT-4o、Nano Banana、Seedream 4.0等强大多模态模型的出现，闭源与开源模型之间的性能差距正在扩大。主要原因是缺乏大规模高质量训练数据和能够诊断各种编辑行为弱点的综合基准。现有数据构建方法面临规模与质量的权衡：人工标注质量高但不可扩展，自动化流程则存在错误传播和噪声问题。

Method: 提出轻量级数据管道，用端到端模型替代多工具链，并加入统一的后验证阶段。为可扩展的质量控制，训练了7B参数的双任务专家模型Qwen-Verify，用于高效失败检测和指令重述。该管道产生了UnicEdit-10M数据集（1000万规模，涵盖基础和复杂编辑任务）。同时提出UnicBench通用基准，超越基础编辑，明确评估空间和知识驱动推理能力，并引入新指标如"非编辑一致性"和"推理准确性"。

Result: 构建了UnicEdit-10M数据集和UnicBench基准，通过对主流模型在UnicBench上的分析，揭示了它们的局限性，并为未来研究提供了明确方向。

Conclusion: 提出的数据管道和基准解决了多模态图像编辑领域的数据稀缺和质量控制问题，为开源模型的发展提供了重要支持，并通过细粒度诊断指标帮助识别模型弱点。
...
### [96] [PPTArena: A Benchmark for Agentic PowerPoint Editing](https://arxiv.org/abs/2512.03042)
*Michael Ofengenden,Yunze Man,Ziqi Pang,Yu-Xiong Wang*

Main category: cs.CV

TL;DR: PPTArena是一个专注于PowerPoint编辑的基准测试，包含100个演示文稿、2125张幻灯片和800多个目标编辑任务。PPTPilot是基于此基准的结构感知幻灯片编辑代理，通过规划-编辑-检查循环实现精确控制，在复杂编辑任务上优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有PowerPoint编辑评估主要关注图像PDF渲染或文本到幻灯片生成，缺乏对真实幻灯片进行自然语言指令下的可靠修改的评估。需要建立一个全面的基准来衡量PPT编辑能力，特别是跨文本、图表、表格、动画和母版样式等多种编辑类型。

Method: PPTArena基准包含100个真实演示文稿、2125张幻灯片和800多个目标编辑案例，每个案例包括原始幻灯片、目标结果和双VLM评估管道（分别评估指令遵循和视觉质量）。PPTPilot采用结构感知编辑代理，规划语义编辑序列，在高级程序化工具和确定性XML操作之间路由，通过迭代的规划-编辑-检查循环验证输出。

Result: PPTPilot在复合编辑、布局敏感编辑和跨幻灯片编辑任务上比强大的专有代理和前沿VLM系统高出10个百分点以上，在视觉保真度和整个演示文稿一致性方面有显著优势。然而，现有代理在长视野、文档规模任务上仍表现不佳。

Conclusion: PPTArena为PPT编辑能力评估提供了全面基准，PPTPilot展示了结构感知编辑方法的有效性，但可靠PPT编辑仍面临长视野、文档规模任务的挑战，需要进一步研究。
...
TL;DR: 提出HUD框架，利用视频与文本信息密度差异解决组合视频检索中的指代歧义和语义聚焦问题，在CVR和CIR任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有组合视频检索方法忽视了视频和文本模态间信息密度的差异，导致两个关键问题：1) 修改主体指代歧义；2) 有限细节语义聚焦，从而降低模型性能。

Method: 提出分层不确定性感知消歧网络(HUD)，包含三个核心组件：(a) 整体代词消歧，(b) 原子不确定性建模，(c) 整体到原子对齐。通过整体跨模态交互利用重叠语义，通过原子级跨模态交互实现细粒度语义对齐。

Result: HUD在三个基准数据集上为组合视频检索(CVR)和组合图像检索(CIR)任务均实现了最先进的性能。

Conclusion: HUD是首个利用视频与文本信息密度差异来增强多模态查询理解的框架，有效解决了指代歧义和语义聚焦问题，实现了精确的组合特征学习。
...
TL;DR: IC-World是一个基于视频的世界建模框架，能够从一组输入图像并行生成多个视频，每个视频代表相同底层世界在不同相机姿态下的视图，通过强化学习优化几何和运动一致性。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型在合成多样动态视觉环境方面受到关注，但现有方法缺乏对共享世界建模的系统探索，即从同一底层世界在不同相机姿态下生成多个一致视频。

Method: 提出IC-World框架，利用大型视频模型的内在上下文生成能力并行处理所有输入图像；通过强化学习（Group Relative Policy Optimization）微调，结合两个新奖励模型：场景级几何一致性和对象级运动一致性。

Result: 大量实验表明IC-World在几何一致性和运动一致性方面显著优于现有最先进方法，是首个系统探索视频世界模型中共享世界建模问题的工作。

Conclusion: IC-World成功解决了共享世界建模问题，通过激活大型视频模型的上下文生成能力并结合强化学习优化一致性，为多视角视频生成提供了有效框架。
...
TL;DR: 提出了PhyCustom框架，通过两种新的正则化损失来激活扩散模型进行物理概念定制，解决了现有方法在物理属性定制上的不足。
...
Method: 提出了PhyCustom微调框架，包含两种新颖的正则化损失：等距损失旨在激活扩散模型学习物理概念，解耦损失帮助消除独立概念的混合学习。
...
TL;DR: AAG方法通过单帧RGB+深度特征结合先验动作信息，在动作预测任务上达到与视频时序聚合方法相当的性能


<details>
  <summary>Details</summary>
Motivation: 人类观察单帧场景即可预测后续动作，但传统方法依赖视频时序信息。研究探索是否能用其他模态替代视频聚合来实现动作预测

Method: AAG方法结合单帧RGB特征和深度线索增强空间推理，并融入先验动作信息（来自VLM文本摘要或单帧动作识别器预测）提供长期上下文

Result: 在IKEA-ASM、Meccano、Assembly101三个教学活动数据集上，AAG与视频时序基线方法和SOTA方法表现相当

Conclusion: 多模态单帧动作预测方法AAG能够有效替代视频时序聚合，在动作预测任务上实现竞争性性能
...
TL;DR: 该研究首次系统性地评估了身份保持型AI生成内容（IP-AIGC）检测器对印度和南亚面孔的鲁棒性和公平性，发现现有检测器在该场景下存在泛化能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现代图像编辑器能生成身份保持的AI生成内容（IP-AIGC），但现有检测器在该领域的鲁棒性和公平性尚不明确，特别是对印度和南亚等代表性不足的人群。

Method: 从FairFD和HAV-DF数据集中构建印度面孔的训练集，使用Gemini和ChatGPT等商业生成器创建两个保留身份的IP-AIGC测试集（HIDF-img-ip-genai和HIDF-vid-ip-genai），评估AIDE和Effort两种先进检测器在预训练和微调模式下的性能。

Result: 微调能提升域内性能（如Effort AUC从0.739提升到0.944），但对印度人群的保留身份IP-AIGC测试集性能显著下降（如AIDE AUC从0.923降到0.563），表明检测器过度拟合训练生成器的特征。在非IP图像上，预训练模型性能保持良好。

Conclusion: IP-AIGC-印度检测是一个具有挑战性的实际场景，需要开发保持表征的适应方法和印度感知的基准数据集来弥补检测器的泛化差距。
...
<details>
  <summary>Details</summary>
Motivation: FAME 2026挑战赛旨在研究多语言环境下的人脸-语音关联任务，特别是英语-德语跨语言场景，需要有效处理两种模态中的相关语义信息
...
TL;DR: STSR 2025挑战赛在MICCAI 2025举办，针对数字牙科中的CBCT和IOS数据标注稀缺问题，设置了两个半监督学习任务：牙齿和根管分割、CBCT-IOS配准，提供了标注和未标注数据集，吸引了社区参与并取得了良好结果。


<details>
  <summary>Details</summary>
Motivation: 数字牙科中CBCT和IOS数据标注稀缺，限制了自动化解决方案（如根管分割和跨模态配准）的发展，需要建立半监督学习基准来推动该领域进步。

Method: 组织STSR 2025挑战赛，提供60个标注和640个未标注IOS样本，30个标注和250个未标注CBCT扫描；设置两个任务：半监督分割（牙齿和根管）和半监督刚性配准（CBCT-IOS）。
...
Conclusion: 挑战赛成功建立了数字牙科半监督学习的基准，展示了社区对解决标注稀缺问题的创新方法，所有数据和代码已公开以确保可重复性，为未来研究提供了基础。
...
TL;DR: 提出在线强化学习后训练框架，优化预训练视频生成器以实现精确相机控制，通过可验证几何奖励提供密集分段反馈
...
Method: 设计可验证几何奖励函数：估计生成视频和参考视频的3D相机轨迹，分割为短片段，计算分段相对位姿，通过对比生成-参考片段对分配对齐分数作为奖励信号

Result: 在线RL后训练在相机控制精度、几何一致性和视觉质量方面明显优于SFT基线方法

Conclusion: 提出的在线强化学习后训练框架通过密集分段几何奖励有效提升了相机控制视频生成的性能，展示了其在推进该领域发展方面的优越性
...
TL;DR: MindGPT-4ov是一个多模态大语言模型，提出了一种通用的后训练范式，在多个基准测试中达到SOTA性能，同时降低了成本。该工作包含数据生成、监督微调和强化学习三个关键创新，并进行了基础设施优化。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在数据质量、训练效率和部署成本方面存在挑战。需要一种通用的后训练范式来提升基础能力、泛化能力，同时降低训练和部署成本，实现从学术研究到工业部署的无缝过渡。

Method: 1. 基于信息密度的数据生成方案，结合双维度树状标签系统，实现高质量跨领域数据的自动生成。2. 协作式课程监督微调方法，平衡领域知识注入和通用能力保持。3. 混合强化学习范式，增强推理能力同时优化多样性探索、多模态感知保持和响应简洁性等多目标。4. 基础设施优化：5D并行训练、算子优化和推理量化。

Result: MindGPT-4ov在MMBench、MMStar、MathVision、MathVista等多个基准测试中超越了现有最先进模型。在垂直领域任务中表现出优越的用户体验，实现了从学术研究到工业部署的无缝过渡。基于Qwen3-VL的变体将开源模型权重、数据集和代码。

Conclusion: MindGPT-4ov提供了一个适用于广泛多模态大语言模型的通用后训练范式，通过数据、训练和部署三个方面的创新，实现了高性能、低成本的多模态大语言模型开发，为社区发展提供了重要支持。
...
TL;DR: 本文系统研究了不同LiDAR到图像投影方式如何影响基于视觉基础模型的度量地点识别性能，发现精心设计的投影可以作为端到端3D学习的有效替代方案。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索LiDAR点云到2D图像的不同投影方式如何影响地点识别性能，特别是在结合先进视觉基础模型时，寻找能够替代端到端3D学习的高效方案。

Method: 采用模块化检索管道，控制骨干网络、聚合方法和评估协议，隔离2D投影本身的影响。使用一致的几何和结构通道，在多个数据集和部署场景下测试不同投影特性。

Result: 确定了最能决定判别能力、环境变化鲁棒性和实时自主适用性的投影特性。实验验证了精心设计的投影可以作为LiDAR地点识别中端到端3D学习的有效替代方案。

Conclusion: 通过系统研究LiDAR到图像的投影方式，发现投影特性对地点识别性能有显著影响，精心设计的投影能够有效替代端到端3D学习，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面取得了显著成功，但部署受到高计算成本和需要大量推理步骤的限制。现有的少步蒸馏方法虽然试图跳过冗余步骤，但通常存在重新训练成本高和泛化能力下降的问题

Method: 采用"智能加速而非均匀加速"的视角，对早期语义阶段应用较小的加速，对后期冗余阶段应用较大的加速。具体实现为两个专家模型：Slow-LoRA处理慢速去噪阶段，Fast-LoRA处理快速去噪阶段。仅需为基模型配备轻量级LoRA适配器，无需大规模重新训练

Result: 方法实现了相对于基模型高达5倍的加速，同时在多样化基准测试中保持了可比较的视觉质量。LoRA专家仅需1个样本在单个V100 GPU上训练一小时，但生成的模型在未见过的提示上表现出强大的泛化能力
...
TL;DR: DiverseAR通过自适应logits分布缩放和基于能量的生成路径搜索，解决了基于比特的自回归生成模型中样本多样性不足的问题，在保持视觉质量的同时显著提升了多样性。


<details>
  <summary>Details</summary>
Motivation: 基于比特的自回归生成模型存在样本多样性不足的问题，主要原因是比特建模的二元分类特性限制了预测空间，以及logits分布过于尖锐导致采样崩溃。

Method: 提出DiverseAR方法：1）自适应logits分布缩放机制，动态调整采样过程中二元输出分布的尖锐度；2）基于能量的生成路径搜索算法，避免采样低置信度token以保持视觉质量。

Result: 大量实验表明，DiverseAR显著提升了基于比特的自回归图像生成的样本多样性。

Conclusion: DiverseAR为比特自回归模型提供了一种有效提升样本多样性的方法，在保持视觉质量的同时解决了多样性不足的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本引导视频编辑方法通常依赖辅助掩码或参考图像，这限制了方法的可扩展性和泛化能力。需要一种无需外部控制信号就能实现精确时空一致编辑的方法。

Method: 提出LoVoRA框架，包含独特的数据集构建流程（图像到视频转换、光流掩码传播、视频修复）和核心创新：可学习的对象感知定位机制，通过扩散掩码预测器提供密集时空监督。

Result: 广泛的实验和人工评估表明LoVoRA在视频对象移除和添加任务上具有有效性和高质量性能，能够实现端到端的视频编辑而无需推理时的外部控制信号。

Conclusion: LoVoRA通过创新的对象感知定位机制解决了视频编辑中的时空一致性问题，为无需掩码的视频对象编辑提供了有效的解决方案，具有更好的可扩展性和泛化能力。
...
TL;DR: Layout Anything：基于Transformer的室内布局估计框架，采用OneFormer通用分割架构，通过布局退化策略和可微分几何损失实现高效准确的几何结构预测。


<details>
  <summary>Details</summary>
Motivation: 现有室内布局估计方法通常需要复杂的后处理流程，且缺乏对几何约束的直接建模。本文旨在开发一个端到端框架，既能保持曼哈顿世界约束，又能实现高效推理，适用于增强现实和大规模3D场景重建应用。

Method: 1. 基于OneFormer的通用分割架构，利用任务条件查询和对比学习；2. 布局退化策略：通过拓扑感知变换增强训练数据，同时保持曼哈顿世界约束；3. 可微分几何损失：直接强制平面一致性和锐利边界预测；4. 端到端训练，无需复杂后处理。
...
Conclusion: Layout Anything框架成功结合了几何感知和计算效率，消除了复杂后处理需求，在保持曼哈顿世界约束的同时实现了高速推理，特别适用于增强现实和大规模3D场景重建任务。
...
<details>
  <summary>Details</summary>
Motivation: 夜间驾驶等低光环境下图像质量严重下降，影响车载摄像头安全。现有增强算法计算量过大，不适合车载实时应用，需要轻量级解决方案

Method: 提出动态移位卷积(DSConv)核，仅12个可学习参数；结合不同移位距离构建多尺度移位残差块(MSRB)扩大感受野；采用残差结构和多级梯度感知损失函数解决轻量网络不稳定问题

Result: 在LOLI-Street数据集上PSNR达到26.51dB，比现有方法提升4.6dB，仅使用180个参数；最小配置仅需36个参数；在四个基准数据集上验证了实时性能与增强质量的优越平衡

Conclusion: UltraFast-LieNET实现了车载低光图像增强的实时性能与高质量平衡，参数极少，计算效率高，适合资源受限的车载应用场景
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR和相机在BEV表示中的融合方法由于两种传感器几何精度差异，简单融合会导致性能下降。需要一种更智能的融合策略来缓解空间错位问题。

Method: 提出BEVDilation框架：1）将图像BEV特征作为隐式引导而非简单拼接；2）稀疏体素扩张模块利用图像先验增密前景体素；3）语义引导BEV扩张模块通过图像语义引导和长距离上下文捕获增强LiDAR特征扩散。

Result: 在nuScenes基准测试中，BEVDilation优于现有最先进方法，同时保持计算效率竞争力。更重要的是，该LiDAR中心策略对深度噪声具有更强的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工干预或未充分利用智能体自主缓解幻觉的能力，而人类决策过程通过内部反思减少不确定性，再通过外部多视角验证做出最终决定，这一认知范式启发了InEx的设计。

Method: InEx采用基于熵的不确定性估计指导内部反思推理，决策智能体首先生成响应，然后通过编辑智能体和自反思智能体的外部跨模态多智能体协作进行迭代验证和精炼。

Result: 在通用和幻觉基准测试中，InEx持续优于现有方法，取得了4%-27%的性能提升，并展现出强大的鲁棒性。

Conclusion: InEx通过模拟人类认知决策过程，有效缓解了多模态大语言模型中的幻觉问题，为构建更可靠的多模态AI系统提供了新思路。
...
TL;DR: U4D：一种用于4D LiDAR世界建模的不确定性感知框架，通过"难到易"的两阶段生成和时空融合块，在复杂区域实现几何保真和时序一致性


<details>
  <summary>Details</summary>
Motivation: 现有生成框架通常对所有空间区域进行统一处理，忽略了现实场景中不同区域的不确定性差异，导致复杂或模糊区域出现伪影，限制了真实性和时序稳定性

Method: 1) 使用预训练分割模型估计空间不确定性图，定位语义挑战区域；2) "难到易"两阶段生成：不确定性区域建模（重建高熵区域）和不确定性条件补全（合成剩余区域）；3) 引入混合时空块自适应融合时空表示

Result: U4D能够生成几何保真且时序一致的LiDAR序列，提升了4D世界建模在自主感知和仿真中的可靠性

Conclusion: U4D通过不确定性感知的生成方法和时空融合机制，有效解决了4D LiDAR世界建模中的几何保真和时序一致性问题，为自动驾驶和具身AI提供了更可靠的4D世界模型
...
TL;DR: GraphFusion3D：通过自适应跨模态Transformer和图推理模块，结合多模态融合与高级特征学习的3D目标检测框架
...
Method: 提出自适应跨模态Transformer（ACMT）将图像特征自适应集成到点表示中；引入图推理模块（GRM）通过多尺度图注意力建模邻域关系；采用级联解码器进行多阶段预测精炼

Result: 在SUN RGB-D上达到70.6% AP25和51.2% AP50，在ScanNetV2上达到75.1% AP25和60.8% AP50，显著优于现有方法

Conclusion: GraphFusion3D通过有效的多模态融合和图推理机制，显著提升了3D目标检测性能，特别是在捕捉局部几何结构和全局语义上下文方面表现出色
...
TL;DR: TEXTRIX：基于原生3D属性网格和扩散Transformer的纹理生成框架，避免多视图融合问题，同时支持高精度3D分割


<details>
  <summary>Details</summary>
Motivation: 现有3D纹理生成方法依赖多视图融合，常面临视图间不一致性和复杂表面覆盖不全的问题，限制了生成内容的保真度和完整性

Method: 构建潜在3D属性网格，采用配备稀疏注意力的扩散Transformer，直接在体积空间为3D模型着色，避免多视图融合限制；同一架构可扩展用于预测网格上的语义属性以实现3D分割

Result: 在两个任务上都实现了最先进的性能：生成无缝、高保真纹理，以及具有精确边界的准确3D部件分割

Conclusion: TEXTRIX通过原生3D表示克服了多视图融合的局限性，为高保真纹理合成和精确3D分割提供了统一框架，展现了优越性能
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集通常来自有限模拟器或使用传统Structure-from-Motion进行尺度标注，且描述性标注有限，这限制了基础模型从单目视频准确解释真实世界动态的能力。需要弥合这一差距，使智能体能够像人类一样感知和理解动态物理世界。

Method: 采用大规模视觉、几何和多模态模型解释度量尺度静态几何、真实世界动态运动、实例级掩码和整体描述性标注。通过集成基于窗口的Bundle Adjustment与全局优化，将长真实世界视频序列转换为综合4D多模态格式。

Result: 创建了包含10万+视频、80万+标注掩码和1000万+帧的大规模数据集DynamicVerse。在视频深度估计、相机姿态估计和相机内参估计三个基准任务上的实验评估表明，该4D建模方法在捕捉物理尺度测量方面优于现有方法，具有更高的全局准确性。

Conclusion: DynamicVerse提供了一个物理尺度、多模态的4D世界建模框架，能够更好地理解和表示动态真实世界，为智能体感知和交互提供了更准确的基础，推动了从单目视频理解真实世界动态的能力。
...
Method: 提出Driving Gaussian Grounded Transformer (DGGT)，将相机姿态作为模型输出而非输入，联合预测每帧3D高斯地图和相机参数，通过轻量级动态头解耦动态，使用生命周期头保持时间一致性，并采用基于扩散的渲染细化减少伪影。

Result: 在Waymo、nuScenes、Argoverse2等大规模驾驶基准上训练和评估，DGGT在单数据集训练和跨数据集零样本迁移方面均优于先前工作，且随着输入帧数增加具有良好的扩展性。

Conclusion: DGGT实现了单次前向传播的姿态自由算法，在性能和速度上达到最先进水平，为自动驾驶的动态场景重建提供了更灵活、可扩展的解决方案。
...
TL;DR: 提出一种通用方法，通过插入稳定性适配器和高效训练过程，使帧基网络在视频处理中实现时间一致性和抗干扰鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 帧基网络在视频序列处理中存在时间不一致性问题（如帧间闪烁），特别是在输入包含时变干扰时问题更加严重，需要一种通用方法来提升视频推理的稳定性和鲁棒性。

Method: 提出稳定性适配器（可插入任何架构）和资源高效训练流程（基础网络冻结），引入统一概念框架描述时间稳定性和干扰鲁棒性，基于提出的精度-稳定性-鲁棒性损失函数进行理论分析。

Result: 在去噪、图像增强、单目深度估计、语义分割等多个视觉任务上验证了方法的有效性，显著提升了时间稳定性和对多种图像干扰（压缩伪影、噪声、恶劣天气）的鲁棒性，同时保持或提升了预测质量。

Conclusion: 该方法为帧基模型在视频处理中提供了通用的稳定性和鲁棒性增强方案，通过理论分析和实验验证证明了其有效性，适用于多种计算机视觉任务。
...
Method: 提出AutoBrep Transformer模型，采用统一标记化方案将几何基元（曲面和曲线）编码为潜在几何标记，结构关系定义为特殊拓扑参考标记，序列顺序遵循B-Rep面邻接图的广度优先遍历

Result: AutoBrep在质量和水密性方面优于基线方法，能扩展到复杂实体并保持良好保真度和推理速度，支持通过统一标记化实现B-Rep自动补全

Conclusion: AutoBrep通过统一表示和下一标记预测实现了高质量的B-Rep生成，支持用户可控的CAD生成和自动补全功能
...
<details>
  <summary>Details</summary>
Motivation: MRI采集时间长限制了临床实用性。现有展开网络重建方法存在中间步骤参数自由学习导致的不稳定演化问题，而基于扩散模型的方法虽然理论稳定但计算成本高。

Method: 作者理论上证明展开网络是条件概率流ODE的离散实现，基于此提出FLAT方法：从ODE离散化推导展开参数，并将中间重建与理想ODE轨迹对齐以提高稳定性和收敛性。

Result: 在三个MRI数据集上的实验表明，FLAT能够实现高质量重建，相比基于扩散的生成模型减少最多3倍迭代次数，相比展开网络显著提高稳定性。

Conclusion: 通过建立展开网络与条件概率流ODE的理论联系，FLAT方法在MRI重建中实现了更好的稳定性和收敛效率，为加速MRI采集提供了有效解决方案。
...
TL;DR: MAViD是一个用于音频-视觉对话理解和生成的新型多模态框架，采用Conductor-Creator架构，结合自回归和扩散模型，能够生成生动、上下文连贯的长时对话交互。
...
Conclusion: MAViD通过创新的架构设计解决了多模态音频-视觉对话理解和生成的关键挑战，实现了高质量的长时对话交互生成。
...
Method: 扩展预训练的单镜头模型，集成两种新颖的RoPE变体：1）多镜头叙事RoPE，在镜头转换时应用显式相位偏移；2）时空位置感知RoPE，结合参考标记和接地信号。建立自动化数据标注流程来提取多镜头视频数据

Result: 框架支持多镜头视频生成，具有文本驱动的镜头间一致性、自定义主体运动控制和背景驱动的自定义场景。镜头数量和时长可灵活配置，实验证明了其优越性能和出色的可控性
...
TL;DR: OneThinker是一个统一的多模态推理模型，能够处理图像和视频理解，涵盖问答、描述、时空定位、跟踪和分割等10个基础视觉任务，通过EMA-GRPO方法解决多任务强化学习中的奖励异质性问题。
...
Method: 构建OneThinker-600k训练语料库，使用商业模型进行思维链标注得到OneThinker-SFT-340k用于监督微调冷启动，提出EMA-GRPO方法通过跟踪任务级奖励标准差移动平均来处理多任务强化学习中的奖励异质性问题。

Result: 在31个基准测试中表现出色，涵盖10个基础视觉理解任务，展现出任务间的有效知识迁移和初步的零样本泛化能力。

Conclusion: OneThinker朝着统一的多模态推理通用模型迈出了一步，所有代码、模型和数据均已开源。
...
<details>
  <summary>Details</summary>
Motivation: 多视角扩散模型在生成一致性视角方面表现出色，但其实现视角一致性的机制尚不明确。研究发现这些模型的注意力图在训练过程中获得了几何对应关系，但这种对应信号不完整，在大视角变化时准确性下降。
...
Conclusion: CAMEO通过几何对应监督注意力图，有效提升了多视角扩散模型的训练效率和生成质量，揭示了注意力机制在实现视角一致性中的关键作用。
...
Method: 将创意意图解构为四个可控视觉线索层：内容层（创建什么）、空间层（放置位置）、结构层（形状结构）和颜色层（调色板）。采用专门的数据生成管道实现上下文感知内容集成，统一控制模块处理所有视觉线索，微调空间分支实现精确局部编辑。

Result: 广泛实验验证了这种分层方法能有效解决用户意图差距，为创作者提供直接、直观的生成过程控制。

Conclusion: 分层组合范式成功弥合了扩散模型的语义能力与传统图形软件的精细控制之间的鸿沟，实现了更可控的生成式图像编辑。
...
TL;DR: RxBench是一个用于评估大语言模型在处方审核任务中的综合基准，包含多种题型和14种常见处方错误类型，测试了18个前沿LLM并发现性能分层，领先模型在某些任务上可匹配或超越药师表现。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在临床决策支持中的快速应用，特别是在处方审核领域，需要建立一个系统化、细粒度的评估基准来全面衡量LLM在此关键医疗任务中的能力和局限性。

Method: 开发了RxBench基准，涵盖常见处方审核类别，整合了14种来自权威药学参考资料的常见处方错误类型。基准包含1,150个单选题、230个多选题和879个简答题，所有题目均由经验丰富的临床药师审核。对18个前沿LLM进行了基准测试，并基于评估见解对中等模型进行了针对性微调。

Result: 发现LLM在任务性能上存在明显分层：Gemini-2.5-pro-preview-05-06、Grok-4-0709和DeepSeek-R1-0528始终处于第一梯队，在准确性和鲁棒性上优于其他模型。领先LLM在某些任务上可匹配或超越持证药师表现。通过针对性微调中等模型，获得了在简答题任务上可与领先通用LLM相媲美的专用模型。

Conclusion: RxBench建立了一个标准化、错误类型导向的评估框架，不仅揭示了前沿LLM在处方审核中的能力和局限，还为构建更可靠、更专业的临床工具提供了基础资源，推动了临床决策支持系统的发展。
...
TL;DR: 这篇论文是关于深度研究系统的综述，系统梳理了将大语言模型与外部工具结合完成复杂开放任务的研究范式，提出了三阶段路线图、四大核心组件、优化技术和评估标准。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型虽然强大，但许多开放任务需要批判性思维、多源信息和可验证输出，这超出了单次提示或标准检索增强生成的能力。深度研究旨在结合LLM的推理能力和外部工具，使其成为能够完成复杂开放任务的研究代理。

Method: 论文采用系统性综述方法，首先形式化三阶段路线图，区分深度研究与相关范式；然后介绍四大核心组件（查询规划、信息获取、记忆管理、答案生成）及其细分子分类；总结优化技术（提示工程、监督微调、代理强化学习）；最后整合评估标准和开放挑战。

Result: 提出了深度研究系统的全面框架，包括清晰的路线图、基础组件、实现技术、重要挑战和未来方向。为深度研究领域提供了系统化的理论指导和实践参考。

Conclusion: 深度研究是将大语言模型与外部工具结合以解决复杂开放任务的重要范式。该综述为领域发展提供了系统指导，并承诺随着领域快速演进持续更新，以反映最新进展。
...
TL;DR: 该论文提出了一种模型选择方法论（MSM），通过定性和定量两个维度来帮助用户选择最适合特定用例的LLM模型，并以医疗领域为例展示了定量评估的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着新的基础模型快速涌现，为特定用例、应用或领域选择最合适的模型变得越来越复杂。现有模型选择缺乏系统方法，需要综合考虑定性和定量两个维度。

Method: 通过分析当前排行榜和基准测试来探索定量维度，以医疗领域为案例研究，展示定量评估的演变、现状和实际意义，并提出模型选择方法论（MSM）。

Result: 论文展示了医疗领域LLM模型的定量评估演变和现状，提出了一个系统化的模型选择框架，帮助用户导航、优先排序和选择最适合特定用例的模型。

Conclusion: 模型选择需要综合考虑定性和定量两个维度，提出的MSM方法论为选择最适合特定用例的LLM提供了系统指导，特别是在快速发展的基础模型生态系统中。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型的推理方法通常依赖局部、即时步的指标（如置信度或熵），缺乏更可靠的全局视角，导致采样轨迹不一致和生成质量欠佳。

Method: 提出连贯上下文解码（CCD）框架，包含两个核心创新：1）轨迹校正机制，利用历史上下文增强序列连贯性，早期拒绝次优路径；2）自适应采样策略，根据一致性指标动态调整每个步骤的解码预算。

Result: 在Dream和LLaDA等多个基准测试中，方法在推理速度和性能上同时提升，实现高达3.48倍的加速和3.91%的性能改进。

Conclusion: CCD通过理论驱动的轨迹校正和自适应预算分配，有效解决了扩散语言模型推理中的连贯性和效率问题，为高质量快速生成提供了新思路。
...
TL;DR: 本文提出基于对称和辛微分方程的可逆LLM架构，通过时间可逆动力学减少内存消耗，无需存储中间激活，同时提供将现有LLM转换为可逆架构的微调方法。


<details>
  <summary>Details</summary>
Motivation: LLM训练成本高昂且耗时，通常需要对预训练模型进行微调。传统架构需要存储所有中间激活，导致内存消耗大，限制了批处理大小和吞吐量。

Method: 1. 引入基于对称和辛微分方程的可逆架构，利用时间可逆动力学在反向传播时检索隐藏状态；2. 提出将现有非可逆LLM转换为可逆架构的高效微调方法。

Result: 在多个数据集和基准测试中，多个LLM上显示出相当或改进的性能，显著减少内存消耗，允许在相同内存下处理更大批处理大小，提高吞吐量。

Conclusion: 可逆LLM架构为降低从头训练和微调LLM的内存和计算成本提供了可扩展且高效的路径，同时保持或提升模型性能。
...
TL;DR: 该论文探索内存高效微调方法应用于语音模型，在保持准确率的同时显著降低GPU内存使用并加速训练速度
...
Method: 将最初为语言处理设计的内存高效微调方法应用于通用预训练语音模型，全面分析不同MEFT方法的GPU内存使用和微调速度，以Whisper模型在KeSpeech数据集上识别六种普通话子方言作为案例研究

Result: GPU内存使用减少高达73.25%，训练速度加速2.1倍，同时保持与普通微调和PEFT方法相当的准确率

Conclusion: 内存高效微调方法在语音方言识别任务中能显著提高内存效率和训练速度，同时保持模型性能，为语音模型的轻量化微调提供了有效解决方案
...
TL;DR: 提出一种数据高效的BERT微调策略，通过TF-IDF样本选择减少75%训练数据，同时通过词汇表增强处理新兴仇恨言论术语，在保持性能的同时提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的仇恨言论持续演变，新兴俚语和混淆术语不断出现以规避检测系统，现有BERT模型面临词汇表限制和训练效率问题。

Method: 1) 使用TF-IDF样本选择机制保留最具信息量的75%训练样本；2) 增强BERT分词器，添加领域特定的俚语和词汇变体；3) 在仇恨言论分类任务上微调BERT模型。

Result: 在广泛使用的仇恨言论数据集上，该方法在显著减少训练数据量的情况下仍能达到竞争性性能，同时提高了计算效率。

Conclusion: 该方法为可扩展和自适应的仇恨内容审核提供了潜力，通过数据高效微调和词汇增强有效应对不断演变的仇恨言论挑战。
...
TL;DR: 论文提出RESP框架，通过自生成校准、解码梯度重要性估计和渐进再生，解决推理大语言模型剪枝时性能崩溃的问题，在20-40%稀疏度下保持接近稠密模型的精度。


<details>
  <summary>Details</summary>
Motivation: 推理大语言模型（如OpenAI o1、DeepSeek-R1、Qwen3）虽然推理能力强，但模型大、推理输出长，部署成本高且不适合资源受限环境。现有剪枝方法对推理大语言模型效果差，即使中等稀疏度（如20%）也会导致精度崩溃和推理连贯性破坏。

Method: 提出RESP自反思结构化剪枝框架：1）使用模型自生成推理轨迹作为校准数据，更准确反映推理分布；2）解码梯度重要性估计，对齐剪枝决策与推理动态；3）渐进再生机制，随稀疏度增加保持校准保真度。

Result: 在Qwen3-8B上实验，RESP显著优于现有结构化剪枝方法。在20-30%稀疏度下保持接近稠密模型的精度，在更高稀疏度下大幅缓解性能崩溃。40%稀疏度时，GSM8K达到81.3%准确率，MathQA达到59.6%，分别比最强基线提升66.87%和47%。

Conclusion: RESP框架通过自生成校准、解码梯度重要性估计和渐进再生，成功解决了推理大语言模型剪枝的挑战，在保持推理能力的同时显著降低了计算和内存成本，为资源受限环境部署推理大语言模型提供了可行方案。
...
TL;DR: MODOMA系统是一个用于无监督语言习得实验的计算多智能体实验室环境，通过成人和儿童智能体之间的交互实现语言习得，能够获取和表示功能与内容范畴。


<details>
  <summary>Details</summary>
Motivation: 为语言习得研究提供一个完全参数化的计算实验环境，使研究人员能够控制实验的所有方面，并明确表示和查询获得的语法知识。

Method: 使用统计和基于规则的程序，通过两个语言模型（成人和儿童智能体）之间的交互进行无监督语言习得，系统完全参数化，研究人员可控制所有实验参数。

Result: 儿童智能体能够基于成人智能体生成的不同数量示例数据，成功获取和表示功能与内容范畴，机器生成数据中发现了与人类数据相似的模式。

Conclusion: 实验成功证明了儿童智能体能够获取离散的语法范畴，验证了MODOMA方法在建模语言习得方面的有效性，为计算语言习得实验提供了新的可能性。
...
Conclusion: LiteReason引导RL训练达到性能-计算权衡曲线中更高效的部分，为优化语言模型推理能力提供了一种计算效率更高的方法。
...
TL;DR: DETAIL框架评估提示词特异性对LLM推理性能的影响，发现更具体的提示能提高准确性，尤其对小模型和程序性任务效果显著。


<details>
  <summary>Details</summary>
Motivation: 提示词设计对大型语言模型的推理性能至关重要，但提示词特异性（详细程度）的影响尚未得到充分研究。需要系统评估不同特异性水平的提示对模型性能的影响。

Method: 提出DETAIL框架：1) 使用GPT-4生成多级特异性提示；2) 通过困惑度量化提示特异性；3) 使用基于GPT的语义等价性评估回答正确性。在30个新颖推理任务上对GPT-4和O3-mini进行实验。

Result: 特异性提高准确性，特别是对较小模型和程序性任务效果更明显。实验结果强调了需要自适应提示策略，并为后续研究提供了工具和数据支持。

Conclusion: 提示词特异性显著影响LLM推理性能，需要根据不同模型和任务类型采用自适应提示策略。DETAIL框架为相关研究提供了评估工具和基准数据。
...
TL;DR: CAIRNS框架通过结构化ScholarGuide提示提升可读性和引用可靠性，结合一致性加权混合评估器，实现无需微调的气候适应问答系统，在专家评估中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 气候适应策略对农业可持续发展至关重要，但相关信息分散在结构化（政府API）和非结构化（科学文献）数据中，专家难以快速获取可信的初步答案。

Method: 提出CAIRNS框架：1) 使用结构化ScholarGuide提示增强可读性和引用可靠性；2) 采用一致性加权混合评估器，利用模型间一致性结合专家判断进行鲁棒评估。

Result: 在专家策划的问题-答案数据集上，CAIRNS在大多数指标上优于基线方法；消融研究在所有指标上确认了结果；LLM评估与人类判断具有相关性。

Conclusion: CAIRNS框架无需微调或强化学习，即可实现可读、可验证、领域基础的气候适应问答，为农业专家提供可信的初步答案。
...
TL;DR: 研究人员创建了HealthContradict数据集，评估语言模型在长且矛盾的生物医学上下文中的推理能力，发现微调后的生物医学模型既能利用正确上下文，又能抵抗错误上下文的影响。


<details>
  <summary>Details</summary>
Motivation: 评估语言模型如何使用上下文信息回答健康问题，以及当上下文存在矛盾时模型的表现如何。现有医学问答评估基准在区分模型上下文推理能力方面不足，需要更精细的评估工具。

Method: 创建HealthContradict数据集（920个专家验证实例，包含健康问题、科学证据支持的事实答案和两个矛盾立场的文档）。采用多种提示设置（正确、错误或矛盾上下文），测量对模型输出的影响。

Result: HealthContradict比现有医学问答评估基准更能区分语言模型的上下文推理能力。实验显示，微调后的生物医学语言模型的优势不仅在于预训练获得的参数知识，还在于它们利用正确上下文同时抵抗错误上下文的能力。

Conclusion: 该研究提供了评估语言模型在矛盾生物医学上下文中推理能力的新工具，揭示了微调生物医学模型在上下文利用和抵抗方面的双重优势，对医疗AI应用有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究对LLM作为问题求解器和解决方案验证器的交互作用研究有限，主要关注自我验证，很少研究验证器如何判断同家族或不同家族模型的输出。现代LLM经过大量后训练，但其对验证能力的影响尚不清楚。

Method: 对37个模型进行系统研究，涵盖多个家族、不同规模、基础模型与后训练变体。在9个基准测试上进行评估，覆盖逻辑推理、结构化谜题、符号计算、数学、常识、事实回忆和领域知识。比较自我验证、同家族验证和跨家族验证。引入并实证验证了"验证器增益"指标来预测基于验证器的拒绝采样带来的性能改进。

Result: 跨家族验证特别有效；后训练会降低自我改进但增强跨家族改进；数学和逻辑任务表现出最高的固有可验证性。验证器增益和假阳性率随模型规模和后训练而变化，不同数据集的可验证性存在差异。

Conclusion: 该研究为LLM验证能力提供了系统性分析，揭示了跨家族验证的优势、后训练对验证能力的复杂影响，以及不同任务类型的可验证性差异，为构建更有效的验证系统提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在医疗政策、政府福利等敏感领域的问答系统中存在事实一致性和上下文对齐困难，需要整合异构知识源同时确保准确性和安全性。

Method: 提出KARMA框架：1) 双编码器架构融合结构化与非结构化知识源；2) 门控记忆单元动态调节外部知识集成；3) 安全感知可控解码器，通过安全分类和引导生成技术减轻不安全输出。

Result: 在专有QA数据集上的广泛实验表明，KARMA在回答质量和安全性方面均优于强基线模型。
...
TL;DR: TaleFrame是一个结合LLM和HCI的故事生成系统，通过将故事结构分解为实体、事件、关系和故事大纲四个基本单元，使用JSON2Story方法将结构化数据转化为连贯故事，提供直观界面支持用户交互式创作。
...
Method: 1) 将故事结构分解为实体、事件、关系和故事大纲四个基本单元；2) 利用Tinystories数据集构建包含9,851个JSON条目的偏好数据集；3) 微调本地Llama模型实现JSON2Story转换；4) 提供支持拖放、附加、连接等交互的直观界面；5) 在七个维度评估生成故事并提供改进建议。

Result: 构建了TaleFrame系统，能够通过结构化框架生成故事，支持用户交互式控制故事细节和进展。定量评估和用户研究证明了系统的实用性，数据集已在HuggingFace公开。

Conclusion: TaleFrame通过结合LLM和HCI，提供结构化故事生成方法，解决了现有系统缺乏细粒度控制的问题，实现了更精确的用户意图到故事输出的转换。
...
<details>
  <summary>Details</summary>
Motivation: 传统语言模型面临幻觉问题的挑战，这一问题对自然语言处理领域构成了重大威胁。理解当前各种幻觉类型、其产生原因以及缓解方法变得至关重要。

Method: 本文采用综述研究方法，系统性地整理和分析语言模型中的幻觉现象。文档提供了简洁明了的总结，涵盖幻觉的分类、产生机制以及缓解策略。

Result: 该文档作为一站式资源，为理解幻觉现象及其缓解方法提供了全面的概览，帮助读者建立对语言模型幻觉问题的整体认识框架。

Conclusion: 幻觉问题是语言模型领域的重要挑战，本文通过系统综述为研究人员和从业者提供了理解幻觉现象及其缓解策略的实用指南，有助于推动更可靠的语言模型发展。
...
TL;DR: 论文评估了假新闻检测和病毒性预测两个任务，发现文本内容对假新闻检测效果很好，而病毒性预测更具挑战性且对标签构建敏感。轻量级数值特征在计算受限时仍可行，不同文本嵌入模型差异不大。


<details>
  <summary>Details</summary>
Motivation: 研究在线虚假信息处理中的两个实际任务：假新闻检测和病毒性预测，特别关注操作环境下的快速反应需求。旨在比较不同方法在现实约束下的表现。
...
Result: 文本内容对假新闻检测效果显著，数值特征在语言模型不可用或计算受限时仍可行。病毒性预测比假新闻检测更难，对标签构建高度敏感。降维分析显示非线性结构对病毒性预测更有信息量。RoBERTa和Mistral嵌入差异不大。

Conclusion: 假新闻检测可依赖文本内容，病毒性预测需要更谨慎的标签设计和时间感知特征。轻量级数值特征在资源受限时是可行替代方案。研究强调了评估设计和现实约束对虚假信息研究的重要性。
...
TL;DR: ADORE是一个自持的电商搜索相关性建模框架，通过结合规则感知相关性判别、错误类型感知数据合成和关键属性增强知识蒸馏，自动化解决数据稀缺问题并提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 电商搜索相关性建模面临两个主要挑战：传统术语匹配方法（如BM25）存在语义鸿沟，而神经模型则受限于领域特定困难样本的稀缺性。需要一种能够自动生成高质量训练数据并增强模型鲁棒性的解决方案。
...
Conclusion: ADORE通过自动化标注、对抗生成和蒸馏过程，克服了数据稀缺问题，同时增强了推理能力，为电商搜索相关性建模提供了一种自持的解决方案。
...
TL;DR: DeepSeek-V3.2是一个高效推理模型，通过稀疏注意力机制、强化学习框架和大规模智能体任务合成管道，在计算效率和推理能力上取得突破，其高性能版本在数学和信息学奥赛中超越GPT-5，达到Gemini-3.0-Pro水平。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型在计算效率和推理能力之间的平衡问题，同时提升智能体任务中的工具使用和泛化能力，需要开发一个既能保持高性能又具备计算效率的模型。

Method: 1. DeepSeek稀疏注意力(DSA)：降低长上下文场景的计算复杂度；2. 可扩展强化学习框架：通过强化学习协议和扩展后训练计算；3. 大规模智能体任务合成管道：系统生成训练数据，实现可扩展的智能体后训练。

Result: DeepSeek-V3.2在计算效率上表现优异，其高性能版本DeepSeek-V3.2-Speciale超越GPT-5，推理能力与Gemini-3.0-Pro相当，在2025年国际数学奥赛和信息学奥赛中均获得金牌表现。

Conclusion: DeepSeek-V3.2成功实现了计算效率与推理性能的平衡，通过技术创新在多个基准测试中达到或超越当前最先进模型，为高效智能体系统的发展提供了重要参考。
...
TL;DR: CAPO是一种基于优势信号的课程学习机制，通过先使用正优势样本进行模仿学习建立基础，再引入负信号培养判别能力，提升大语言模型在推理任务中的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在训练早期不加区分地混合正负优势信号，可能导致指导模糊和收益有限。需要更有效的信号利用策略来提升大语言模型的推理能力。
...
Conclusion: CAPO通过课程式优势信号利用策略，解决了现有方法信号混合不当的问题，为强化学习优化大语言模型提供了更有效和通用的框架。
...
TL;DR: 本教程系统梳理了从级联ASR/NLU到端到端、检索和视觉增强系统的语音对话智能体演进路径，提供了实用方法和系统级路线图


<details>
  <summary>Details</summary>
Motivation: 随着语音对话智能体向语音原生大语言模型（LLMs）发展，需要系统梳理从传统级联架构到现代端到端系统的演进路径，为研究者和从业者提供清晰的路线图

Method: 通过框架化文本LLMs到音频的适配、跨模态对齐和联合语音文本训练；回顾数据集、评估指标和鲁棒性；比较级联vs端到端、后ASR修正、流式处理等设计选择

Result: 提供了从工业助手到当前开放域和任务导向智能体的连接，突出了可复现的基线，并概述了隐私、安全和评估方面的开放问题

Conclusion: 本教程为参会者提供了实用的方法和清晰的系统级路线图，帮助理解语音对话智能体的演进路径和未来发展方向
...
TL;DR: 研究发现LLM在生成多文档摘要时存在显著的首因效应，更倾向于语义对齐第一个看到的文档，这对依赖LLM生成概述的应用和AI代理系统构成风险。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM被广泛用于多文档摘要（如Google AI Overviews），但尚不清楚它们是否对所有输入文档给予同等权重。本研究旨在探究LLM在多文档摘要任务中是否存在位置偏见。

Method: 构建40个关于堕胎议题的支持-中立-反对立场文章三元组，每个三元组以6种不同顺序排列，使用Gemini 2.5 Flash生成中立概述。通过ROUGE-L（词汇重叠）、BERTScore（语义相似度）和SummaC（事实一致性）评估摘要与源文章的关系，并使用单因素方差分析检验位置效应。

Result: BERTScore在所有立场上都显示出显著的首因效应，摘要与第一个看到的文章在语义上更一致。成对比较显示位置1与位置2、3有显著差异，而位置2和3之间无差异，证实了LLM对第一个文档的选择性偏好。

Conclusion: LLM在多文档摘要中存在系统性位置偏见，更倾向于第一个输入文档，这对依赖LLM生成概述的应用和AI代理系统构成风险，因为LLM步骤可能不成比例地影响下游决策。
...
TL;DR: 本文实证调查了7种模型融合算法（Linear、Karcher Mean、SLERP、NuSLERP、TIES、DELLA、Nearswap）在13个开源大语言模型上用于减轻社会偏见的有效性，发现存在偏见减少与下游性能之间的权衡，其中SLERP在中等插值权重下表现最均衡。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）会继承甚至放大预训练语料库中的社会偏见，威胁公平性和社会信任。虽然已有研究探索通过编辑LLM参数来减轻偏见，但缺乏对不同模型融合算法的实证比较。

Method: 对7种模型融合算法（Linear、Karcher Mean、SLERP、NuSLERP、TIES、DELLA、Nearswap）在13个开源权重模型（GPT、LLaMA、Qwen系列）上进行实证调查，使用三个偏见数据集（BBQ、BOLD、HONEST）进行综合评估，并在SuperGLUE基准测试中测量这些技术对下游任务性能的影响。

Result: 发现偏见减少与下游性能之间存在权衡：实现更大偏见减轻的方法会降低准确性，特别是在需要阅读理解、常识和因果推理的任务上。在融合算法中，Linear、SLERP和Nearswap能持续减少偏见同时保持整体性能，其中中等插值权重的SLERP是最平衡的选择。

Conclusion: 模型融合算法在减轻偏见方面具有潜力，但过度的去偏见化或不适当的融合方法可能导致重要语言能力的退化。SLERP在中等插值权重下是平衡偏见减轻与性能保持的最佳选择。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM安全防护主要针对高资源语言，导致使用低资源语言的大量人口被忽视，需要开发能服务全球人口的语言无关安全系统。

Method: 提出CREST模型，采用基于聚类的跨语言迁移方法，仅用13种高资源语言训练，通过参数高效设计（0.5B参数）实现向100种语言的泛化，包括未见的高资源和低资源语言。

Result: 在六个安全基准测试中，CREST超越同规模现有防护模型，并与参数大5倍以上（2.5B+）的模型竞争，证明其高效性和有效性。

Conclusion: 语言特定防护存在局限，需要开发通用、语言无关的安全系统以有效服务全球人口，CREST展示了通过跨语言迁移实现这一目标的可行性。
...
TL;DR: 研究通过心理物理学范式评估LLMs是否表现出类似人类的最优贝叶斯多模态整合能力，发现准确率高的模型不一定具备稳健的贝叶斯策略，揭示了能力与策略的分离。


<details>
  <summary>Details</summary>
Motivation: 人类在感知任务中能直觉地使用近似最优的贝叶斯策略处理噪声信号，但LLMs的隐式计算策略尚不明确。研究旨在探索LLMs是否能在没有明确训练或指令的情况下表现出类似的最优多模态整合行为。

Method: 采用心理物理学范式，构建BayesBench基准测试（包含长度、位置、距离、持续时间四个文本和图像估计任务），评估9个LLMs和人类判断。通过控制噪声、上下文和指令提示的变化，测量多模态线索整合的性能、行为和效率，并引入贝叶斯一致性评分来检测贝叶斯一致的行为变化。

Result: 结果显示，虽然能力强的模型常以贝叶斯一致的方式适应，但准确率不能保证稳健性。GPT-5 Mini在文本任务中准确率完美，但无法高效整合视觉线索。这揭示了能力与策略的关键分离，表明以准确率为中心的基准测试可能过度关注性能而忽略了脆弱的确定性处理。

Conclusion: LLMs在处理不确定性方面表现出一定的原则性能力，准确率与贝叶斯倾向相关。研究发布的BayesBench基准和一致性指标可作为评估工具，为未来多模态架构设计提供参考，强调需要超越准确率来评估模型的稳健策略。
...
TL;DR: SurveyEval是一个用于评估LLM自动生成调查报告的综合基准，从整体质量、大纲连贯性和参考文献准确性三个维度进行评估，旨在解决复杂调查生成系统的评估挑战。


<details>
  <summary>Details</summary>
Motivation: 随着LLM自动调查系统的发展，这些系统能够整合检索、组织和内容合成到端到端生成流程中，但如何评估这种复杂系统仍然是一个重大挑战。现有研究主要关注开发新的生成流程，而缺乏系统性的评估方法。

Method: 提出了SurveyEval基准，从三个维度评估自动生成的调查报告：整体质量、大纲连贯性和参考文献准确性。该基准覆盖7个学科领域，并扩展了LLM-as-a-Judge框架，通过加入人类参考来增强评估与人类判断的一致性。

Result: 评估结果显示，通用的长文本或论文写作系统倾向于生成质量较低的调查报告，而专门的调查生成系统能够提供显著更高质量的结果。SurveyEval被证明是一个可扩展的测试平台。

Conclusion: SurveyEval作为一个全面的评估基准，能够帮助理解和改进自动调查系统在不同学科和评估标准下的表现，为这一领域的发展提供了重要的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前许多新提出的参数高效微调方法难以复现、部署和相互比较，这阻碍了PEFT领域的研究进展和实际应用。

Method: 基于流行的LLaMA-Factory开发下游框架PEFT-Factory，采用模块化设计，原生支持19种PEFT方法、27个分类和文本生成数据集，并提供标准和PEFT专用评估指标。

Result: PEFT-Factory提供了一个即用型、可控且稳定的环境，显著提升了PEFT方法的复现性和基准测试能力。

Conclusion: PEFT-Factory通过统一的框架解决了PEFT方法复现和比较的挑战，为研究社区提供了实用的工具支持。
...
TL;DR: UniFact是一个统一评估框架，用于直接比较事实验证(FV)和幻觉检测(HD)两种范式，发现两者互补，混合方法效果最佳。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生幻觉（看似流畅但事实错误的输出），这影响了实际应用的可信度。目前存在模型中心的幻觉检测(HD)和文本中心的事实验证(FV)两种研究范式，但它们各自独立发展，形成了研究分裂，阻碍了共同进步。

Method: 提出了UniFact统一评估框架，通过动态生成模型输出和相应的事实性标签，实现FV和HD在实例级别的直接比较。在多个LLM家族和检测方法上进行了大规模实验。

Result: 三个关键发现：1) 没有哪种范式普遍优越；2) HD和FV捕捉了事实错误的互补方面；3) 整合两种方法的混合方法始终达到最先进的性能。提供了首次深入分析解释FV和HD为何分化。

Conclusion: 需要一个新的、整合的研究议程来统一LLM中的幻觉检测和事实验证。实验结果表明两种范式应该结合而非孤立发展。
...
TL;DR: 提出一种三层次数据合成方法，用于对话式广义指代表达理解(GREC)，通过平衡真实性和可控性生成可扩展的监督数据，显著提升模型性能
...
Result: 在合成数据上微调的模型相比先前方法，在标准评估指标上获得一致且显著的改进

Conclusion: 提出的数据合成方法有效解决了对话式GREC中的数据稀缺和分布偏移问题，为模型提供了可扩展的监督信号
...
TL;DR: TriLex是一个三阶段检索增强框架，通过语料库提取、跨语言映射和RAG驱动的词汇精炼，为低资源非洲语言系统扩展情感词典，显著提升了情感分析性能。


<details>
  <summary>Details</summary>
Motivation: 低资源非洲语言在情感分析中代表性不足，限制了多语言NLP系统的词汇覆盖和性能，需要开发系统化的方法来扩展这些语言的情感词典。

Method: 提出TriLex框架，包含三个阶段：1) 语料库基础提取；2) 跨语言映射；3) 检索增强生成(RAG)驱动的词汇精炼。使用扩展后的词典评估两个非洲预训练语言模型(AfroXLMR和AfriBERTa)的性能。

Result: AfroXLMR表现最佳，在isiXhosa和isiZulu上F1分数超过80%，并展现出强大的跨语言稳定性。AfriBERTa虽然未在目标语言上预训练，仍能达到约64%的可靠F1分数。两种模型均优于传统机器学习基线，集成分析进一步提升了精度和鲁棒性。
...
TL;DR: 提出stable rank作为无需人工标注的LLM内在质量信号，基于模型表示的有效维度衡量回答质量，并开发SR-GRPO强化学习方法
...
Method: 提出stable rank概念，通过计算总方差与主导方向方差的比值来衡量隐藏状态的有效维度，捕捉信息在表示维度上的分布。基于此开发Stable Rank Group Relative Policy Optimization (SR-GRPO)强化学习方法
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在中医领域的应用日益增多，但现有评估方法局限于知识问答或证型识别，缺乏对治疗决策能力的评估。中医"辨证论治"具有个体化、整体性和多样性特点，需要更全面的临床能力评估基准。

Method: 提出基于临床病例的综合基准TCM-BEST4SDT，包含中医基础知识、医学伦理、内容安全和辨证论治四个任务。采用选择题评估、评判模型评估和奖励模型评估三种机制，其中奖励模型专门用于量化处方-证候一致性。数据标注遵循严格流程，由中医专家主导。

Result: 在15个主流大语言模型（包括通用和中医领域模型）上验证了TCM-BEST4SDT的有效性。基准已公开可用，促进智能中医研究发展。

Conclusion: TCM-BEST4SDT填补了中医大语言模型临床能力评估的空白，特别是对治疗决策能力的评估。该基准为智能中医发展提供了重要的评估工具和方向指引。
...
<details>
  <summary>Details</summary>
Motivation: 教育全球化与在线学习的快速增长使得教育内容本地化成为关键挑战。讲座材料本质上是多模态的，结合了音频和视觉幻灯片，需要能够处理多种输入模态的系统。为了提供可访问且完整的学习体验，翻译必须保留所有模态：用于阅读的文本、用于视觉理解的幻灯片以及用于听觉学习的语音。

Method: 提出BOOM系统，采用端到端方法联合翻译讲座音频和幻灯片，生成同步的多模态输出：翻译文本、保留视觉元素的本地化幻灯片以及合成语音。系统能够处理多模态输入并产生多模态输出。

Result: 实验表明，具有幻灯片感知的转录文本对下游任务（如摘要和问答）具有级联效益。系统已发布Slide Translation代码并集成到Lecture Translator中。

Conclusion: BOOM系统能够为学生提供母语访问讲座的完整体验，同时保留原始内容的完整性。多模态联合翻译方法在提升教育内容可访问性方面具有重要价值。
...
TL;DR: promptolution：一个统一的模块化开源框架，用于提示优化，整合多种离散提示优化器，独立于底层LLM实现
...
Method: 引入promptolution框架，这是一个统一且模块化的开源系统，在一个可扩展的系统中提供提示优化所需的所有组件，整合多种当代离散提示优化器，同时保持对底层LLM实现的不可知性

Result: 创建了一个为从业者和研究人员设计的单一可扩展系统，解决了现有实现分散、孤立的问题

Conclusion: promptolution框架通过提供统一、模块化且可扩展的开源系统，促进了提示优化的实际应用和研究发展
...
TL;DR: 本文提出一个四维评估框架来评估系统提示在跨语言环境中的效果，通过大规模实验发现某些提示组件与稳健的多语言行为相关，并开发了多语言提示优化框架，可提升所有指标5-10%。


<details>
  <summary>Details</summary>
Motivation: 虽然先前工作主要关注英语环境，但实际部署需要单一提示能在多种语言中可靠工作。系统提示为大型语言模型提供了轻量但强大的推理时调节机制，需要研究如何通过系统提示引导模型实现准确和稳健的跨语言行为。

Method: 提出统一的四维评估框架来评估多语言环境中的系统提示，在五种语言、三个LLM和三个基准上进行大规模实验。开发了多语言设置下的提示优化框架，并分析了超过1000万个推理单元。

Result: 发现某些提示组件（如CoT、情感和场景）与稳健的多语言行为相关。提示优化框架可自动发现能提升所有指标5-10%的提示。性能更好的系统提示能诱导更结构化、一致性的推理模式，同时减少不必要的语言切换。

Conclusion: 系统提示优化是实现准确和稳健多语言LLM行为的可扩展路径，为跨语言部署提供了有效方法。
...
TL;DR: 该研究针对孟加拉语仇恨言论检测任务，复现了官方基线方法并测试了多种Transformer模型，发现语言特定的预训练模型BanglaBERT表现最佳，强调了预训练语言模型对低资源语言的重要性。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为拥有2.3亿使用者的语言，在计算资源方面代表性不足，特别是在社交媒体自动审核需求日益增长的背景下。低资源语言面临数据集不足、拼写异质性和语言多样性等挑战，需要有效的仇恨言论检测解决方案。

Method: 研究复现了官方基线方法（多数类、随机、支持向量机），并增加了逻辑回归、随机森林和决策树作为基线。同时使用了基于Transformer的模型：DistilBERT、BanglaBERT、m-BERT和XLM-RoBERTa进行仇恨言论分类，在BLP 2025共享任务的子任务1A和1B上进行评估。

Result: 所有基于Transformer的模型（除DistilBERT外）在子任务上都优于基线方法。在Transformer模型中，BanglaBERT在两个子任务上都取得了最佳性能。尽管规模较小，BanglaBERT的表现超过了m-BERT和XLM-RoBERTa，表明语言特定的预训练非常重要。

Conclusion: 研究结果突显了预训练语言模型对低资源孟加拉语的重要性和潜力。语言特定的预训练模型在仇恨言论检测任务中表现出色，为低资源语言的NLP应用提供了有价值的见解。
...
TL;DR: ThinkMerge是一种无需训练的解码策略，通过并行推理轨迹的logit平均来提升开放端推理任务（如代码生成、深度研究）的性能，无需依赖完整输出的多数投票。


<details>
  <summary>Details</summary>
Motivation: 多数投票在封闭式问答中有效，但不适用于开放端推理任务（如代码生成、深度研究），因为完整解决方案的"多数"概念不明确。需要一种能利用并行推理优势但适用于开放端任务的方法。

Method: ThinkMerge是一种即插即用的解码策略，运行K个并行推理轨迹，在同步点平均它们的下一个token的logits，生成单个连贯输出。与vLLM/SGLang无缝集成，兼容Top-p/Top-k等标准解码技术。

Result: 在AIME和GPQA上匹配或超越多数投票；在LiveCodeBench（hard）上，DeepCoder-14B-Preview的pass@1提升+8.28%，Qwen3-8B提升+7.58%；在深度研究任务（GAIA、BrowseComp-en/zh、XbenchDeepSearch）上提升WebSailor-7B/32B性能。

Conclusion: 并行测试时扩展可以有益于开放端推理，无需依赖完整输出的投票。ThinkMerge为开放端推理任务提供了一种有效的训练免费解码策略。
...
TL;DR: SchED是一种无需训练、模型无关的早期退出算法，通过聚合全跨度logit边界并在满足平滑进度相关置信度阈值时停止解码，显著加速扩散大语言模型的推理速度。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)相比自回归模型有优势，但其迭代采样过程缓慢，严重限制了实际应用。需要一种高效的方法来加速dLLM的解码过程，同时保持生成质量。

Method: SchED是一种训练免费、模型无关的早期退出算法。它聚合全跨度logit边界，并在满足平滑进度相关置信度阈值时停止解码。算法不依赖模型特定训练，可应用于不同dLLM家族。

Result: 在指令调优模型上实现3.8-4.0倍加速，同时保持99.8-100%基线性能；在基础模型上实现一致加速增益，性能保持99.1-100%，在激进设置下可达2.34倍加速。在长文本生成任务中明显优于现有置信度早期退出方法。

Conclusion: SchED通过将真实的置信度稳定转化为计算节省，显著提高了dLLM解码效率。指令调优加速了预测熵的衰减，使SchED在保持高质量输出的同时实现大幅加速，为dLLM的实际部署提供了实用解决方案。
...
TL;DR: AutoNeural：专为NPU设计的视觉-语言模型架构，通过MobileNetV5风格视觉编码器和SSM-Transformer混合语言模型，实现整数量化稳定性和线性时间推理，在边缘设备上实现14倍延迟降低和3倍解码速度提升。


<details>
  <summary>Details</summary>
Motivation: 当前面向GPU优化的视觉-语言模型在NPU上表现不佳，主要由于ViT的量化脆弱性和自回归注意力机制的I/O瓶颈，无法充分利用NPU的高算术吞吐量，需要专门为NPU约束重新设计模型架构。

Method: 1) 用MobileNetV5风格的深度可分离卷积视觉编码器替代标准ViT，确保激活分布有界以实现稳定的INT4/8/16量化；2) 语言模型结合状态空间模型原理和Transformer层，使用门控卷积实现线性时间复杂度的混合设计，消除生成过程中的KV缓存内存I/O开销。

Result: 视觉编码器量化误差降低7倍，端到端延迟降低14倍，解码速度提升3倍，上下文窗口长度提升4倍。在Qualcomm SA8295P SoC上的真实汽车案例验证了驾驶舱应用的实时性能。

Conclusion: 专门为NPU约束重新思考模型拓扑结构是实现鲁棒多模态边缘智能的前提条件，AutoNeural展示了通过硬件感知的架构设计可以显著提升边缘AI效率。
...
TL;DR: 提出一个将英语句子转换为CNF形式的框架，结合传统NLP技术、自定义语法和微调语言模型来减少LLM在逻辑翻译中的幻觉问题


<details>
  <summary>Details</summary>
Motivation: LLM在自然语言到形式逻辑的自动翻译中存在幻觉问题，影响自动化推理、软件规范验证等应用的可靠性

Method: 结合传统NLP技术（自定义语法）、符号计算库和微调的语言模型，将英语句子先转换为逻辑表达式，再转换为CNF形式

Result: 微调后的模型能够有意识地纠正原始模型的同类幻觉错误，提供可靠的CNF生成

Conclusion: 提出的框架通过结合多种技术有效减少了LLM在逻辑翻译中的幻觉问题，为自动化推理提供了更可靠的CNF生成
...
TL;DR: MoCoP是一个无数据集、闭环的道德一致性评估框架，能够持续评估和解释LLM的道德稳定性，揭示道德一致性与毒性维度呈强负相关，与响应延迟几乎无关。


<details>
  <summary>Details</summary>
Motivation: 现有对齐框架依赖静态数据集和事后评估，难以捕捉道德推理在不同上下文或时间尺度上的演变。需要一种能够持续评估LLM道德一致性的动态方法。

Method: 提出Moral Consistency Pipeline (MoCoP)，包含三个支持层：(1)词汇完整性分析，(2)语义风险估计，(3)基于推理的判断建模。这是一个自维持架构，能够自主生成、评估和精炼道德场景，无需外部监督。

Result: 在GPT-4-Turbo和DeepSeek上的实证结果显示：道德维度与毒性维度呈强负相关(rET = -0.81, p<0.001)，与响应延迟几乎无关(rEL≈0)。表明道德一致性和语言安全性是模型行为的稳定可解释特征。

Conclusion: MoCoP通过将道德评估重构为动态、模型无关的道德内省形式，为可扩展的持续审计提供了可复现的基础，推动了自主AI系统中计算道德学的研究。
...
TL;DR: 该论文提出了LLM验证器收敛定理，为LLM辅助的形式验证提供了首个具有可证明终止性和收敛性保证的正式框架，并通过超过9万次实验验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 当前使用形式验证工具与大型语言模型（LLM）的方法虽然扩展了软件验证能力，但缺乏可靠的理论基础，导致验证过程不稳定（可能收敛、循环或发散），需要建立具有可证明保证的正式框架。

Method: 将LLM与验证器的交互建模为离散时间马尔可夫链，状态转移由关键参数错误减少概率（δ）决定。通过LLM-Verifier收敛定理证明对于任何δ>0，过程几乎必然达到验证状态，且期望迭代次数有界E[n]≤4/δ。

Result: 超过9万次实验验证了理论预测：所有运行都成功达到验证，收敛因子紧密聚集在Cf≈1.0附近，理论界限与实际行为一致。基于此将工作流划分为三个操作区域：边际、实用和高性能。

Conclusion: 该研究为LLM辅助验证提供了清晰的理论架构基础，消除了启发式调优的需求，为工程师提供了可预测的资源规划和性能预算框架，特别适用于安全关键软件环境的部署。
...
<details>
  <summary>Details</summary>
Motivation: 流程图是常见的流程沟通工具，但通常以静态图像形式分享，难以编辑或重用。现有工具缺乏结构化、版本可控的文本表示

Method: 使用详细系统提示和视觉语言模型将流程图图像转换为Mermaid.js代码，支持文本编辑、拖放节点插入和自然语言命令的AI助手

Result: 开发了一个轻量级Web系统，能够生成结构化、版本可控的文本表示，并与渲染图表保持同步。引入了评估结构准确性、流程正确性、语法有效性和完整性的指标

Conclusion: 与现有图像转图表工具不同，该方法产生了结构化、版本可控的文本表示，支持混合主动式编辑，为流程图重用和协作提供了新方法
...
TL;DR: 提出一个框架，用于将复杂的世界模型（用transducer表示）分解为可并行处理的子模块，提高计算效率并增强可解释性。


<details>
  <summary>Details</summary>
Motivation: 现实世界模型通常计算需求高，但真实场景往往包含模块化交互的子组件。为了在AI安全所需的结构透明度和实际推理所需的计算效率之间架起桥梁，需要开发可分解的建模框架。

Method: 开发了一个框架，用于分解由transducer（一种推广POMDP的模型类）表示的复杂世界模型。该方法将transducer分解为在独立输入-输出子空间上运行的子transducer，支持并行化和可解释的替代方案。

Result: 该方法能够将复杂的世界模型分解为可并行处理的子模块，为分布式推理提供支持，同时保持模型的可解释性。

Conclusion: 这些结果为连接AI安全所需的结构透明度和实际推理所需的计算效率奠定了基础，为模块化、可解释的世界建模提供了理论框架。
...
TL;DR: STRIDE框架通过系统评估任务特性，智能推荐三种AI部署模式：直接LLM调用、引导式AI助手或完全自主的智能体，避免不必要的智能体部署，降低成本风险。


<details>
  <summary>Details</summary>
Motivation: 随着AI从无状态大语言模型向自主智能体快速转变，需要解决核心问题：何时真正需要智能体AI？不加区分地部署智能体会导致成本、复杂性和风险增加，需要系统方法来确定合适的部署模式。

Method: 提出STRIDE框架，集成结构化任务分解、动态性归因和自我反思需求分析，生成智能体适用性评分，在三种模式间提供原则性推荐：直接LLM调用、引导式AI助手、完全自主智能体AI。

Result: 在30个真实世界任务（SRE、合规、企业自动化）评估中，STRIDE达到92%的模式选择准确率，减少45%不必要的智能体部署，降低37%资源成本。六个月专家验证确认其实际效用。

Conclusion: 将智能体采用重构为需求驱动的设计决策，确保自主性仅在收益证明成本合理时应用。STRIDE框架能有效区分需要简单LLM调用、引导助手或完全自主智能体的任务。
...
TL;DR: 该论文扩展了TheAgentCompany框架，创建了财富管理评估基准，发现LLM代理在端到端工作流可靠性方面存在限制，而非数学推理能力不足。


<details>
  <summary>Details</summary>
Motivation: 尽管现代工作依赖各种数字协作工具，但日常流程仍受人为错误和延迟困扰。该研究旨在探索通用LLM代理是否能准确且经济地完成财富管理任务，并创建有意义的评估体系来衡量代理在此领域的适用性。

Method: 扩展TheAgentCompany框架，创建金融专注环境；引入合成领域数据，丰富同事模拟，原型化自动任务生成管道；构建包含12个任务对的财富管理助理基准，涵盖检索、分析和综合/通信任务，具有明确的验收标准和确定性评分器；创建新的金融特定数据集，并为每个任务引入高自主性与低自主性变体。

Result: 研究发现：1) 代理的限制更多在于端到端工作流可靠性，而非数学推理能力；2) 自主性水平对代理性能有显著影响；3) 不正确的模型评估阻碍了基准测试的有效性。

Conclusion: LLM代理在财富管理任务中的表现受到工作流可靠性的限制，自主性水平是关键影响因素，需要改进评估方法以准确衡量代理在此领域的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM驱动的交易代理在真实金融市场中部署越来越多，但在高风险、不可逆的金融环境中，它们在对抗或故障条件下的可靠性和鲁棒性尚未得到充分检验。

Method: TradeTrap针对自主交易代理的四个核心组件（市场情报、策略制定、投资组合和账本处理、交易执行）进行系统级扰动测试，在真实美股市场数据的闭环历史回测环境中进行公平可重复的比较。

Result: 实验表明，单个组件的小扰动可以通过代理决策循环传播，导致极端集中、失控敞口和大幅投资组合回撤，证明当前自主交易代理在系统层面可被系统性误导。

Conclusion: 当前自主交易代理在系统层面存在脆弱性，需要更严格的鲁棒性评估框架来确保其在真实金融环境中的可靠性。
...
Method: 通过识别和分析当代AI模型的七大核心缺陷：缺乏内在自我监控、元认知意识缺失、固定非适应性学习机制、无法重构目标、缺乏表征维护、不足的具身反馈、内在能动性缺失。结合人工系统与生物认知的比较分析，整合AI研究、认知科学和神经科学的见解。

Result: 当前架构（包括深度学习和基于transformer的系统）存在结构性限制，仅靠规模扩展无法解决这些问题。这些缺陷导致AI缺乏真正的自主适应能力。

Conclusion: 需要向认知基础AI（认知自主性）进行范式转变，这种AI能够实现自我导向适应、动态表征管理和有意图的目标导向行为，同时配备改革性监督机制，确保自主系统保持可解释性、可治理性并与人类价值观保持一致。
...
TL;DR: DialogGuard是一个多智能体框架，用于评估LLM生成内容在五个高风险维度上的心理社会安全性：隐私侵犯、歧视行为、心理操纵、心理伤害和侮辱行为。


<details>
  <summary>Details</summary>
Motivation: LLM现在被广泛应用于心理健康、危机干预等情感敏感服务，但其在这些场景下的心理社会安全性仍然缺乏深入理解和有效评估方法。

Method: 提出DialogGuard框架，包含四种LLM作为评判者的流程：单智能体评分、双智能体修正、多智能体辩论和随机多数投票，基于共享的三级评估标准。

Result: 多智能体机制比非LLM基线和单智能体评判更准确地检测心理社会风险；双智能体修正和多数投票在准确性、与人类评分一致性及鲁棒性之间达到最佳平衡。

Conclusion: DialogGuard作为开源软件发布，提供维度风险评分和可解释的自然语言理由，支持面向脆弱用户的网络应用提示设计、审计和监督。
...
<details>
  <summary>Details</summary>
Motivation: 模型恢复（MR）在关键任务自主系统中能实现安全、可解释的决策，但当前基于神经ODE的迭代求解方法在FPGA上效率低下，内存和能耗限制了其在边缘设备上的实时部署

Method: 提出MERINDA框架，用可并行化的神经架构替代神经ODE的迭代求解器，专门针对FPGA加速设计

Result: 相比移动GPU，MERINDA实现了近11倍的DRAM使用降低和2.2倍的运行速度提升；实验显示在固定精度下内存与能耗呈反比关系

Conclusion: MERINDA特别适合资源受限、需要实时运行的关键任务自主系统，为边缘设备上的模型恢复提供了高效的FPGA加速解决方案
...
TL;DR: 使用量子启发的边缘增强和稳定多组件损失函数，仅用599张训练图像实现乳腺细胞分割95.5% Dice分数，大幅减少医学标注需求


<details>
  <summary>Details</summary>
Motivation: 医学图像标注需要大量专家时间和专业知识，特别是乳腺上皮细胞数据集标注需要病理学家投入数百小时。这是临床感知AI发展的主要瓶颈。

Method: 1. 量子启发边缘增强：多尺度Gabor滤波器创建第四输入通道增强边界检测
2. 稳定多组件损失函数：集成自适应Dice损失与边界感知项和自动正样本加权
3. 复杂度加权采样策略：优先处理具有挑战性的乳腺上皮细胞区域
4. EfficientNet-B7/UNet++架构：4通道到3通道投影，利用预训练权重
5. 鲁棒验证：指数移动平均和统计异常值检测

Result: 在仅599张训练图像（仅4%像素代表乳腺组织，60%图像无乳腺区域）上达到：
- Dice分数：95.5% +/- 0.3%
- IoU：91.2% +/- 0.4%
- 量子增强贡献边界精度提升2.1%
- 加权采样增加小病变检测3.8%

Conclusion: 该方法通过有限标注实现突破性性能，显著减少医学专家创建数据集所需时间，解决了临床感知AI发展的基本瓶颈问题。
...
TL;DR: OmniGuard是首个全模态安全护栏系统，通过精心策划的大型数据集和结构化安全标签，为处理文本、图像、视频和音频的全模态大语言模型提供统一的安全保障框架。


<details>
  <summary>Details</summary>
Motivation: 全模态大语言模型（OLLMs）处理多种模态数据带来了新的安全挑战，现有安全护栏研究主要针对单模态设置，且通常将安全保障视为二元分类问题，这限制了在不同模态和任务间的鲁棒性。

Method: 提出OmniGuard框架，构建包含超过21万个多样化样本的全模态安全数据集，涵盖所有模态的单模态和跨模态样本，每个样本都带有结构化安全标签和通过定向蒸馏从专家模型获得的精心策划的安全评析。

Result: 在15个基准测试上的广泛实验表明，OmniGuard在各种多模态安全场景中实现了强大的有效性和泛化能力，为全模态安全保障提供了统一框架。

Conclusion: OmniGuard通过统一框架在全模态中执行策略和缓解风险，为构建更鲁棒和强大的全模态安全保障系统铺平了道路。
...
Method: 提出了ReMindView-Bench基准测试，系统性地变化视角空间模式和查询类型来探究空间认知的关键因素。使用显性分阶段分析（LLM-as-a-judge和自一致性提示）和隐性分析（线性探测和熵动态）来诊断推理过程。

Result: 评估15个当前VLM显示，在多视角空间推理中存在跨视角对齐和视角转换的一致失败。模型在帧内感知表现良好，但在跨视角信息整合时性能急剧下降。隐性分析显示任务相关信息逐渐丢失，正确与错误轨迹之间的不确定性分离。

Conclusion: ReMindView-Bench为VLM空间推理提供了认知基础的诊断，揭示了多视角空间心理模型在推理过程中如何形成、退化和失稳。基准测试和源代码已公开可用。
...
TL;DR: 该论文提出了一种基于LLM的生成式智能体MMO模拟系统，用于优化游戏数值系统和机制设计，相比传统方法更真实、可解释且成本更低。


<details>
  <summary>Details</summary>
Motivation: 传统MMO游戏优化方法依赖大规模在线实验或基于预定义统计模型的参数调优，这些方法成本高、耗时长，且可能破坏玩家体验。简化的离线模拟系统又因保真度有限，无法准确模拟真实玩家的推理和对干预的反应。
...
TL;DR: 研究探索使用监督学习结合人工错误注入来诱导语言模型自我修正能力，但发现该方法效果有限，无法显著提升性能，即使模型发现错误也常常重复原错误。


<details>
  <summary>Details</summary>
Motivation: 强化学习已成为激发大语言模型推理和自我修正能力的主要方法，但计算成本高昂，因此需要探索替代方案。受自动驾驶和机器人技术启发，研究是否可以通过监督学习结合人工错误注入来诱导语言模型的自我修正能力。

Method: 在推理链中插入人工错误，将其掩码，然后监督模型识别和修正这些错误。通过合成错误注入的监督学习方法来训练模型。

Result: 该方法即使在简单合成任务上也无法显著提升多个模型的性能。即使模型发现了自己的错误，也常常只是重复原始错误。研究发现，从合成错误到策略错误的分布偏移显著降低了微调模型的错误修正能力，即使合成错误覆盖了策略错误。

Conclusion: 研究结果解释了为什么策略强化学习方法在激发自我修正能力方面特别有效。监督学习结合人工错误注入的方法虽然直观，但效果有限，无法替代强化学习。
...
TL;DR: AI代理管道自动聚类预测市场并识别市场间依赖关系，通过交易策略验证发现的关系具有实际价值
...
Method: 开发AI代理管道：1）使用自然语言理解对合约文本和元数据进行聚类，形成连贯主题组；2）识别组内市场对之间的结果依赖关系，包括相同结果（正相关）和不同结果（负相关）关系

Result: 在Polymarket历史数据集上评估：AI识别的关系准确率达到60-70%；基于这些关系的简单交易策略在周度时间范围内获得约20%的平均回报

Conclusion: 代理AI和大型语言模型能够有效发现预测市场中的潜在语义结构，识别的关系具有实际交易价值，为市场分析提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: AI自演化长期以来被视为通向超智能的途径，但实践中无引导的自演化系统往往快速达到平台期甚至退化，存在概念漂移、多样性崩溃和错误演化等问题。需要一种在最小化人工监督依赖的同时，实现稳定可控自演化的方法。

Method: 提出R-Few框架，包含Challenger和Solver两个组件。Challenger通过少量人工标注示例指导合成问题生成，Solver在在线难度课程下联合训练人工和合成示例。采用上下文接地和混合训练实现轻量级人工监督。

Result: 在数学和通用推理基准测试中，R-Few实现了持续迭代改进。Qwen3-8B-Base在数学任务上比R-Zero提升3.0分，性能与使用20倍人工数据的General-Reasoner相当。消融研究证实了接地挑战者训练和课程求解器训练的互补贡献。

Conclusion: R-Few通过引导式自演化框架有效缓解了概念漂移问题，产生了更稳定可控的协同演化动态，为实现AI稳定自演化提供了一种有前景的方法。
...
TL;DR: COPE：基于链式思维推理的轻量级开源框架，利用临床笔记预测急性缺血性卒中90天功能结局，性能与GPT-4.1相当，优于传统方法。
...
Method: 开发了链式思维预后预测引擎（COPE），基于LLaMA-3-8B模型的两步推理框架：第一步生成临床推理，第二步输出mRS预测。在464名AIS患者数据上评估，与GPT-4.1、ClinicalBERT、结构化变量机器学习模型和单步LLM进行比较。

Result: COPE的MAE为1.01，±1准确率74.4%，精确准确率32.8%，性能与GPT-4.1相当，优于ClinicalBERT（MAE 1.24）、临床ML（1.28）和单步LLM（1.20）。在不同亚组中表现一致。

Conclusion: COPE作为一个轻量级、可解释、保护隐私的开源框架，为非结构化临床文本的预后预测提供了准确实用的解决方案。
...
TL;DR: Aetheria是一个基于多智能体辩论与协作的多模态可解释内容安全框架，通过五个核心代理的协作架构和基于RAG的知识检索，显著提升了内容安全检测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 数字内容的指数级增长给内容安全带来了重大挑战。现有的内容审核系统通常基于单一模型或固定流程，在识别隐含风险和提供可解释的判断过程方面存在局限性。

Method: 提出Aetheria框架，采用五个核心代理的协作架构，通过动态的相互说服辩论机制对多模态内容进行深度分析和裁决，并基于RAG进行知识检索。

Result: 在提出的基准测试AIR-Bench上的综合实验验证，Aetheria不仅能生成详细可追溯的审计报告，而且在整体内容安全准确性上显著优于基线方法，特别是在识别隐含风险方面表现出优势。
...
TL;DR: 提出一种融合视频、音频和文本的多模态共情预测方法，利用监督文档作为特权信息增强训练，在预测阶段无需特权信息。


<details>
  <summary>Details</summary>
Motivation: 现有共情预测方法主要关注单一模态（通常是文本），忽视了多模态处理能力，并且没有充分利用可能包含额外共情内容的特权信息。

Method: 1. 多模态共情预测：使用预训练网络提取视频、音频和文本特征，进行跨模态融合得到多模态特征表示，用于预测共情标签。2. 监督文档辅助训练：在训练阶段引入监督文档作为特权信息，使用LDA模型识别潜在主题分布来约束文本特征，这些文档关注咨询主题和咨询师的共情表现。

Result: 在多模态和对话共情数据集上的实验结果表明，该方法优于现有方法。

Conclusion: 提出的多模态共情预测方法有效整合了多模态信息和特权信息，显著提升了共情预测性能。
...
TL;DR: PaperDebugger：一款基于多代理的Overleaf插件式学术写作助手，将LLM推理直接嵌入编辑器环境，支持上下文感知的文档操作


<details>
  <summary>Details</summary>
Motivation: 现有LLM写作助手与编辑器分离，无法深度交互文档状态、结构和修订历史，难以在LaTeX编辑器（如Overleaf）中实现上下文感知的智能操作

Method: 通过Chrome扩展、Kubernetes原生编排层和MCP工具链构建插件式多代理系统，实现可靠的双向同步、细粒度版本控制、安全状态管理、多代理调度和外部工具集成

Result: 开发了完整集成的工作流演示，包括本地化编辑、结构化审阅、并行代理执行和差异更新，早期聚合分析显示积极的用户参与度

Conclusion: PaperDebugger验证了编辑器原生、代理式写作助手的实用性，为学术写作提供了深度集成的LLM驱动支持
...
TL;DR: IACT是一种计算模型，通过用户对话驱动的动态递归代理拓扑结构，解决静态硬编码工作流的限制，实现运行时错误纠正和模糊性解析。


<details>
  <summary>Details</summary>
Motivation: 传统静态硬编码的代理工作流需要预定义图或专门编程，存在局限性。IACT旨在创建通用自主系统，通过纯用户对话驱动，解决开放任务的可扩展性和错误传播问题。

Method: IACT采用动态递归代理拓扑结构，根据高层目标自主增长组织复杂性。用双向状态对话替代刚性调用，引入交互冗余，实现运行时错误纠正和模糊性解析。

Result: 在kragent.ai系统中实现生产部署，通过真实工作流的定性证据展示其有效性，而非详尽基准测试结果。

Conclusion: IACT通过对话驱动的动态代理拓扑和交互冗余机制，成功解决了传统静态工作流的限制，实现了可扩展的自主系统。
...
TL;DR: 提出TACDA方法，通过目标域重建和退化阶段一致性对齐解决跨域RUL预测中的领域差异问题


<details>
  <summary>Details</summary>
Motivation: 现有对抗领域自适应方法忽略了目标域特定信息和退化阶段的一致性特征，导致在真实工业场景中跨域RUL预测性能不佳

Method: 提出TACDA方法：1）在对抗适应过程中加入目标域重建策略，保留目标特定信息的同时学习领域不变特征；2）开发聚类配对策略实现相似退化阶段的一致性对齐

Result: 实验结果表明TACDA方法在两个不同评估指标上显著优于现有最先进方法

Conclusion: TACDA通过同时考虑目标域特定信息和退化阶段一致性，有效解决了跨域RUL预测中的领域差异问题，取得了优异性能
...
Method: 提出将策略条件化于与自动机转换直接对齐的简单布尔公式序列，并通过图神经网络编码这些公式以生成结构化任务表示。

Result: 在复杂的基于象棋的环境中进行的实验证明了该方法的优势。

Conclusion: 该方法能够有效学习执行任意LTL指令的多任务策略，特别是在多个高级事件同时发生且复杂交互的环境中表现出色。
...
TL;DR: 该论文揭示了大语言模型在递归推理问题上的深度泛化限制，并提出了一种循环定位-替换管道方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理递归推理问题时面临深度泛化挑战，即无法处理比训练时更深的嵌套层次结构，即使它们能处理更长的非嵌套序列。

Method: 开发了一种循环定位-替换管道，包含两个专门模型：定位器识别可解子表达式，替换器评估这些组件同时保持整体结构。

Result: 在布尔代数、递归算术和命题逻辑三个可控递归深度的领域中，该方法有效缓解了在分布外递归深度测试时的性能衰减。

Conclusion: 标准Transformer架构在深度泛化方面存在根本限制，而提出的循环定位-替换管道为解决递归推理问题提供了有效途径。
...
TL;DR: MIGR框架通过引入模态重要性机制，重新组织推理序列从情感主导模态开始，解决多模态情感理解中的推理漂移问题，显著提升推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态情感理解方法存在推理漂移问题：模型逐渐依赖自身生成的文本而非多模态证据，且解释过度受视觉启动推理路径影响，导致推理不可靠。

Method: 提出模态重要性机制识别情感主导模态，基于此重新组织推理序列从关键模态开始。采用两阶段框架：模态对齐监督微调和模态感知奖励优化，生成情感接地、因果相关且保持连贯的解释。

Result: 在DFEW基准测试中，MIGR显著提升推理可靠性，将正确预测伴随情感不一致解释的比例从18.10%降至7.37%，验证了从情感主导模态开始推理的益处。

Conclusion: 通过模态重要性引导的推理框架能有效解决多模态情感理解中的推理漂移问题，提升模型解释的可靠性和一致性，为多模态大语言模型的情感理解提供了更可靠的方法。
...
TL;DR: 提出基于多模态大语言模型的框架，从图像中提取结构化三元组构建知识图谱，用于追踪生成模型输出与训练数据之间的潜在关联，支持版权分析、数据集透明度和可解释AI。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型能力增强，透明度、问责制和版权侵权问题日益突出。需要理解特定训练数据如何影响模型输出，以解决版权分析、数据集透明度和AI可解释性等挑战。

Method: 利用多模态大语言模型从图像中提取结构化三元组，构建与领域特定本体对齐的知识图谱。通过比较生成图像和训练图像的知识图谱，追踪潜在影响来源。

Result: 通过本地训练模型的遗忘实验和大规模模型的风格特定实验验证了方法的有效性。框架支持版权分析、数据集透明度和可解释AI开发。

Conclusion: 该框架为理解生成模型输出与训练数据关系提供了结构化方法，有助于开发促进人类协作、创造力和激发好奇心的AI系统。
...
TL;DR: Menta是一个专门为社交媒体心理健康预测优化的轻量级小语言模型，通过多任务训练在抑郁、压力和自杀倾向等任务上表现优于现有模型，并能在移动设备上实时部署。


<details>
  <summary>Details</summary>
Motivation: 心理健康问题影响全球数亿人，但早期检测仍然有限。虽然大语言模型在心理健康应用中有潜力，但其规模和计算需求阻碍了实际部署。小语言模型提供了轻量级替代方案，但在基于社交媒体的心理健康预测方面尚未得到充分探索。

Method: 提出Menta模型，这是一个专门为社交媒体心理健康预测优化的轻量级小语言模型。采用基于LoRA的多任务微调框架，跨数据集训练策略，以及面向平衡准确率的损失函数，在六个分类任务上进行联合训练。

Result: Menta在抑郁、压力和自杀倾向等任务上比最佳非微调小语言模型平均提升15.2%。在抑郁和压力分类任务上比130亿参数的大语言模型准确率更高，同时模型大小约小3.25倍。能在iPhone 15 Pro Max上实时部署，仅需约3GB内存。
...
TL;DR: StockMem：一个事件-反思双层记忆框架，通过结构化新闻事件、横向整合与纵向追踪来提取市场预期差异信息，结合事件-价格动态分析形成因果经验知识库，用于股票价格预测。


<details>
  <summary>Details</summary>
Motivation: 股票价格预测面临市场波动性和对实时事件的敏感性挑战。虽然大语言模型为基于文本的预测提供了新途径，但在金融领域的应用受到噪声新闻数据和文本中缺乏明确答案的阻碍。通用记忆架构难以识别价格变动的关键驱动因素。

Method: 提出StockMem事件-反思双层记忆框架：1）将新闻结构化为事件；2）横向整合整合每日事件；3）纵向追踪捕捉事件演变以提取反映市场预期差异的增量信息，构建时间事件知识库；4）通过分析事件-价格动态形成反思知识库（因果经验）；5）预测时检索类似历史场景，结合当前事件、增量数据和过去经验进行推理。

Result: 实验表明StockMem优于现有记忆架构，通过追踪影响价格的信息链提供更优、可解释的推理，增强了金融预测中的决策透明度。

Conclusion: StockMem通过事件结构化和双层记忆机制有效解决了金融文本预测中的噪声和因果关系识别问题，为股票价格预测提供了更准确、可解释的解决方案。
...
Method: 使用LLaMA和Gemma等最先进的大语言模型，在合成和真实匿名分类账上进行基准测试，与传统JETs和机器学习基线进行比较。
...
<details>
  <summary>Details</summary>
Motivation: 现有AAI能力评分是静态函数，需要扩展到动态系统来形式化代理的自改进过程，统一理解语言自玩、自校正、合成数据引导等不同方法

Method: 将代理形式化为由计算资源参数化的流ν_r，由递归的生成器-验证器-更新器(GVU)算子控制，证明该算子在参数流形Θ上生成向量场，推导保证自改进稳定性的方差不等式

Result: 证明了自改进系数κ是能力函数沿该流的李导数，方差不等式是自改进稳定性的充分条件，展示了STaR、SPIN、Reflexion、GANs、AlphaZero等架构都是满足方差不等式的GVU算子的具体拓扑实现

Conclusion: 该框架为理解各种自改进方法提供了统一的理论基础，方差不等式为设计稳定的自改进系统提供了指导原则，不同架构通过过滤、对抗判别或形式系统基础等方式满足该条件
...
TL;DR: 提出基于因果概念的后解释AI框架，通过概念干预的充分概率生成局部和全局解释，在CelebA数据集上验证，强调解释需同时满足可理解性和忠实性。


<details>
  <summary>Details</summary>
Motivation: 针对非可解释模型，需要既能被人类理解又忠实于原模型的解释方法。当前解释方法往往难以同时满足可理解性和忠实性要求。

Method: 提出因果概念后解释框架，通过计算概念干预的充分概率生成局部和全局解释。使用概念词汇表，基于因果解释，在CelebA数据集上构建概念验证模型。

Result: 展示了在CelebA数据集上生成的示例解释，证明了框架的可理解性（通过清晰的概念词汇）和忠实性（通过强调框架假设和解释上下文对齐）。

Conclusion: 提出的因果概念后解释框架能够为不可解释模型提供既易懂又忠实的解释，关键在于解释生成与解释理解的上下文必须保持一致。
...
TL;DR: 提出无需人工提示的协作智能体框架，通过验证和精炼智能体自动提升论文到代码生成的质量，相比基线提升约15%准确率和13%完整性。
...
Method: 提出无需提示的协作智能体框架，包含两个智能体：验证智能体检查每个步骤输出是否满足系统提示要求，精炼智能体根据识别的问题修订输出。仅使用原始系统提示实现自动验证和改进。

Result: 在PaperBench Code-Dev和Paper2CodeBench数据集上的实验表明，该方法显著提高了复现代码的准确性和完整性，相比无智能体的基线分别提升约15%和13%。与Self-Refine的比较验证了该方法的鲁棒性和一致性。

Conclusion: 提出的无需提示协作智能体框架能够有效自动提升论文到代码生成的质量，解决了现有方法依赖人工提示和缺乏验证机制的问题，具有更好的适应性和可扩展性。
...
Method: 以大语言模型为推理核心，构建智能代理系统，自主选择工具、规划和执行行动。编排的工具包括：区域定位、基于"think with image"范式的区域分析规划、策略性模板选择、质量评估和反馈驱动的自适应优化。

Result: 实验结果表明，Radiologist Copilot在放射学报告生成方面显著超越了其他最先进的方法。

Conclusion: Radiologist Copilot能够促进准确、完整、高效的放射学报告生成，协助放射科医生并提高临床效率。源代码将在接受后发布。
...
TL;DR: 提出基于贝叶斯主义和证伪原则的AI驱动科学方法，通过无监督学习和人机协作减少认知偏差，优化矿产勘探数据采集决策


<details>
  <summary>Details</summary>
Motivation: 尽管投资增加，但过去20年新矿产发现减少，需要解决勘探中的认知偏差和假阳性问题，降低勘探成本

Method: 提出基于贝叶斯主义和证伪原则的新科学方法，将数据采集视为证伪人类假设的手段，使用可验证指标量化决策；需要无监督学习与人机协作生成竞争性地质假设，以及人在回路AI算法优化地质、地球物理、地球化学和钻探数据采集

Result: 提供了一个可在任何勘探活动中使用的实用协议模板，通过AI实现更理性的决策制定和不确定性降低
...
TL;DR: 该研究提出基于鞅性质的评估框架，用于检测LLM推理中的信念固化现象，发现迭代推理可能导致确认偏误而非提升求真能力。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM推理技术取得进展，但迭代推理可能强化信念固化和确认偏误，而非促进求真行为。需要系统评估LLM推理中的信念固化问题。

Method: 提出基于贝叶斯统计中鞅性质的无监督回归评分方法（Martingale Score），通过测量违反鞅性质的程度来评估信念固化。该方法在事件预测、价值负载问题和学术论文评审等开放领域进行测试。

Result: 研究发现信念固化现象在各类模型和设置中普遍存在，当前信念能正向预测未来信念更新。识别了更易出现信念固化的模型、推理技术和领域。验证了Martingale Score在有真实标签的领域中能预测准确性。

Conclusion: Martingale Score作为无监督指标，即使在没有真实标签的领域也能有效评估推理过程的求真能力，揭示了LLM推理中存在的系统性信念固化问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（偏好训练、提示工程、输入输出过滤）虽然能提升LLM鲁棒性，但在长上下文场景下仍易受攻击，越狱概率随上下文长度增加而增加，需要更强的安全保证

Method: 提出侵入式上下文工程，在LLM上下文中插入控制语句作为安全约束，该方法可推广到思维链过程以防止恶意推理，不依赖LLM训练，避免了长上下文场景下的数据短缺问题

Result: 该方法能部分解决长上下文场景下的LLM安全问题，提供了一种不依赖训练的安全增强方案

Conclusion: 侵入式上下文工程是一种有效的LLM安全增强技术，特别适用于长上下文场景，可防止越狱和恶意行为，且避免了训练数据短缺的问题
...
TL;DR: LLMs可用于在线冲突调解，通过判断对话公平性和情感动态，并生成共情、缓和的信息来引导参与者解决问题。评估显示API模型在调解推理和干预对齐方面优于开源模型。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs越来越多地中介在线交流，它们促进共情和建设性对话的潜力成为负责任AI研究的重要前沿。探索LLMs是否不仅能作为检测有害内容的审核者，还能作为理解和化解在线冲突的调解者。

Method: 将调解分解为两个子任务：判断（评估对话公平性和情感动态）和引导（生成共情、缓和的信息引导参与者解决问题）。构建大型Reddit数据集，提出多阶段评估管道，结合基于原则的评分、用户模拟和人工比较。

Result: 实验表明，API模型在调解推理和干预对齐方面优于开源模型。API模型在调解任务中表现出更强的能力。

Conclusion: 当前LLMs作为在线社交调解新兴代理既有前景也有局限性。研究强调了LLMs在促进建设性对话方面的潜力，同时也指出了当前模型的限制。
...
### [207] [CoatFusion: Controllable Material Coating in Images](https://arxiv.org/abs/2512.02143)
*Sagie Levy,Elad Aharoni,Matan Levy,Ariel Shamir,Dani Lischinski*

Main category: cs.GR

TL;DR: 提出Material Coating新任务：在物体表面模拟薄材料涂层，保留原有几何细节；构建大规模合成数据集DataCoat110K，开发CoatFusion架构，通过扩散模型结合2D反照率纹理和PBR参数控制，实现真实可控的涂层效果。


<details>
  <summary>Details</summary>
Motivation: 现有"材料转移"方法会替换物体固有材料并覆盖细节，而实际应用中需要在不破坏原有几何结构的前提下为物体添加薄材料涂层，这需要新的解决方案。

Method: 1) 构建DataCoat110K合成数据集（11万图像），包含多样化物理基础涂层；2) 提出CoatFusion架构，基于扩散模型，同时以2D反照率纹理和PBR参数（粗糙度、金属度、透射率、厚度）为条件控制涂层效果。

Result: CoatFusion能生成真实可控的涂层效果，在Material Coating任务上显著优于现有材料编辑和转移方法，用户研究也证实了其优越性。

Conclusion: Material Coating是一个新颖且有价值的图像编辑任务，CoatFusion通过结合纹理和参数化控制成功实现了这一任务，为材料涂层模拟提供了有效解决方案。
...
### [208] [SMP: Reusable Score-Matching Motion Priors for Physics-Based Character Control](https://arxiv.org/abs/2512.03028)
...
TL;DR: SMP利用预训练运动扩散模型和分数蒸馏采样创建可重用、任务无关的运动先验，无需为每个新控制器重新训练，可组合不同风格生成新风格。


<details>
  <summary>Details</summary>
Motivation: 对抗模仿学习需要为每个新控制器重新训练运动先验，限制了重用性且需要保留参考运动数据。需要可重用、任务无关的运动先验方法。

Method: 使用预训练运动扩散模型和分数蒸馏采样(SDS)创建Score-Matching Motion Priors(SMP)。SMP可独立于控制策略或任务进行预训练，训练后保持冻结状态，作为通用奖励函数训练下游任务策略。

Result: SMP能够将通用运动先验重新用于多种风格特定先验，组合不同风格合成原始数据集中不存在的新风格，生成的运动质量与最先进的对抗模仿学习方法相当。

Conclusion: SMP通过可重用和模块化的运动先验，在多样化控制任务中有效生成高质量自然运动，解决了对抗模仿学习中先验重用性的限制问题。
