...
TL;DR: SuperHead：一种从低分辨率动态3D头部重建高质量可动画化头像的新框架，通过3D生成模型的动态感知反演方案提升几何和纹理质量


<details>
  <summary>Details</summary>
Motivation: 现有低质量图像或视频源导致3D重建效果差，而传统超分辨率方法无法处理动态3D输入，需要一种能保持3D一致性、时间一致性和身份特征的高质量可动画化头像生成方法

Method: 利用预训练3D生成模型的先验知识，通过动态感知3D反演方案优化生成模型的潜在表示，生成超分辨率的3D高斯泼溅模型，然后绑定到参数化头部模型（如FLAME）进行动画控制，使用多视角、多表情的2D渲染和深度图进行联合监督

Result: SuperHead在动态运动下生成具有精细面部细节的头像，在视觉质量上显著优于基线方法

Conclusion: SuperHead框架成功解决了低质量源生成高质量可动画化3D头像的挑战，通过3D生成模型先验和动态感知反演实现了高质量的几何纹理重建和动画一致性
...
TL;DR: EgoAVU：一个用于自动生成第一人称视角音频-视频叙述、问答的数据引擎，用于训练多模态大语言模型理解第一人称视频中的视听信息。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型虽然能接受视觉和音频输入，但由于缺乏包含连贯多模态信息的文本标签，它们在理解第一人称视频中的联合模态信息方面仍有不足。

Method: 提出EgoAVU数据引擎，通过跨模态关联建模自动生成音频-视频叙述；使用基于token的视频过滤和模块化、基于图的筛选确保数据多样性和质量；构建了包含300万样本的训练数据集EgoAVU-Instruct和手动验证的评估集EgoAVU-Bench。

Result: EgoAVU-Bench揭示了现有MLLM严重偏向视觉信号，常忽略音频线索或无法将音频与视觉源对应；在EgoAVU-Instruct上微调的MLLM在EgoAVU-Bench上性能提升高达113%，在其他基准测试上也有最高28%的相对性能提升。

Conclusion: EgoAVU解决了第一人称视频中多模态理解的训练数据瓶颈，显著提升了MLLM的音频-视频联合理解能力，并将开源代码促进社区发展。
...
TL;DR: MGP-KAD：一种用于复杂场景单视图3D重建的多模态特征融合框架，结合RGB和几何先验，使用KAN混合解码器，在Pix3D数据集上达到SOTA性能。
...
Method: 提出MGP-KAD多模态特征融合框架：1) 集成RGB和几何先验；2) 通过对真实物体数据采样聚类生成类级几何特征，并在训练中动态调整；3) 基于Kolmogorov-Arnold Networks的混合解码器处理复杂多模态输入。
...
TL;DR: DwD提出使用Vision Foundation Model特征作为Sim2Real视频生成的统一桥梁，通过主成分投影和随机通道丢弃解决一致性-真实感困境，并引入空间对齐模块和因果时间聚合器提升控制精度和时序稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有Sim2Real方法面临一致性-真实感困境：低层信号（边缘、模糊图像）能精确控制但会保留合成伪影，高层先验（深度、语义、HDMaps）能实现照片级真实感但缺乏结构细节。需要一种能同时保证控制一致性和真实感的中间表示。

Method: 1) 使用DINOv3特征作为统一桥梁；2) 主成分投影去除导致"纹理烘焙"的高频成分；3) 随机通道尾部丢弃缓解维度减少的结构损失；4) 可学习空间对齐模块适配高分辨率特征；5) 因果时间聚合器使用因果卷积保留历史运动上下文。

Result: DwD框架在自动驾驶视频生成中实现了更好的控制一致性和真实感平衡，有效缓解了运动模糊，保证了时序稳定性，相比现有方法在Sim2Real转换质量上有显著提升。

Conclusion: DwD通过Vision Foundation Model特征作为Sim2Real的统一桥梁，成功解决了现有方法的一致性-真实感困境，为可控视频生成提供了新的解决方案，在自动驾驶视频生成中展现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于隐式SDF的单视图3D重建方法需要大量标注数据，限制了可扩展性。为解决这一问题，需要利用丰富的未标注图像资源
...
Conclusion: MetaSSP通过有效利用未标注数据，显著提升了单视图3D重建的质量，为隐式SDF方法的可扩展性提供了有效解决方案
...
TL;DR: M3是一个无需训练的多智能体框架，通过迭代推理时优化解决文本到图像生成中的复杂组合提示问题，使开源模型超越商业系统性能。
...
Result: 在开源模型上应用M3，在具有挑战性的OneIG-EN基准测试中取得了显著成果，Qwen-Image+M3超越了包括Imagen4(0.515)和Seedream 3.0(0.530)在内的商业旗舰系统，达到最先进的性能(0.532总体)。M3还显著改善了GenEval组合指标，在强化测试集上将空间推理性能有效翻倍。

Conclusion: 作为一个与任何预训练T2I模型兼容的即插即用模块，M3为无需昂贵重新训练的组合生成建立了新范式，证明智能多智能体推理可以将开源模型提升到超越专有替代品的水平。
...
TL;DR: 提出一个用于盆腔MRI的无监督异常检测基准框架，使用残差变分自编码器仅基于健康扫描训练，实现疾病和参数无关的实时异常检测。


<details>
  <summary>Details</summary>
Motivation: 女性盆腔疾病诊断常因解剖结构变异大而延迟，现有AI方法多为疾病特异性且缺乏实时兼容性，限制了临床应用。

Method: 使用残差变分自编码器，仅用健康矢状位T2加权扫描训练，通过重建误差热图检测病理区域，无需异常数据标签，并加入扩散生成合成数据增强鲁棒性。
...
TL;DR: 提出PhenoLIP框架，通过构建大规模表型知识图谱PhenoKG，将结构化医学表型知识融入视觉语言模型预训练，显著提升医学图像分析和跨模态检索性能。
...
Method: 1) 构建PhenoKG知识图谱（52万图像-文本对，关联3000+表型）；2) 提出PhenoLIP两阶段预训练框架：先学习知识增强的表型嵌入空间，再通过教师引导的知识蒸馏将结构化知识融入多模态预训练；3) 创建专家验证的PhenoBench基准（7800+图像-文本对，覆盖1000+表型）。

Result: PhenoLIP显著超越现有最佳基线：在表型分类准确率上比BiomedCLIP提升8.85%，在跨模态检索上比BIOMEDICA提升15.03%，证明整合表型中心先验知识对结构化医学图像理解的价值。

Conclusion: 通过显式整合结构化表型知识到医学视觉语言模型中，PhenoLIP框架实现了更结构化、可解释的医学图像理解，为医学多模态学习提供了新范式。
...
TL;DR: DeDPO：通过因果推断的去偏估计技术改进DPO，利用合成AI反馈增强有限的人类偏好数据，实现可扩展且成本效益高的人类-AI对齐方法。


<details>
  <summary>Details</summary>
Motivation: DPO作为扩散模型的主流对齐方法，依赖大规模高质量人类偏好标注，存在成本和可扩展性瓶颈。需要解决合成AI反馈中的系统性偏差和噪声问题。

Method: 提出Debiased DPO (DeDPO)，将因果推断中的去偏估计技术整合到DPO目标中，通过识别和校正合成标注器固有的系统偏差和噪声，实现从自训练和视觉语言模型等不完美反馈源的鲁棒学习。

Result: DeDPO对合成标注方法的变化具有鲁棒性，性能达到甚至偶尔超过完全使用人类标注数据训练的理论上限，证明了其有效性。

Conclusion: DeDPO为使用廉价合成监督实现人类-AI对齐提供了可扩展的解决方案，解决了DPO的成本和可扩展性瓶颈。
...
TL;DR: AnyThermal是一个通用的热成像骨干网络，能够从热图像中提取鲁棒的任务无关特征，适用于跨模态地点识别、热分割和单目深度估计等多种任务，无需针对特定任务进行训练。


<details>
  <summary>Details</summary>
Motivation: 现有的热成像骨干网络通常基于小规模数据进行特定任务训练，导致其适用性局限于特定环境和任务。需要一种能够适应多种环境和任务的通用热成像特征提取方法。

Method: 通过从视觉基础模型（如DINOv2）中蒸馏特征表示到热成像编码器中，使用来自多个环境的热数据。为了解决现有RGB-热数据集多样性不足的问题，开发了TartanRGBT平台和数据集，包含4种不同环境的同步RGB-热图像采集。

Result: AnyThermal在多种环境和下游任务中取得了最先进的结果，在现有数据集上的性能提升高达36%。TartanRGBT数据集提供了多样且平衡的数据支持。

Conclusion: AnyThermal展示了作为通用热成像骨干网络的有效性，能够适应室内、空中、越野和城市等多种环境，为热成像计算机视觉任务提供了强大的基础模型。
...
TL;DR: DroneKey++：无需先验信息的无人机3D姿态估计框架，结合关键点检测、分类和姿态估计，使用合成数据集6DroneSyn进行训练验证
...
Method: 提出DroneKey++框架：1) 关键点编码器同时进行关键点检测和分类；2) 姿态解码器使用基于射线的几何推理和类别嵌入估计3D姿态；3) 构建6DroneSyn大规模合成数据集，包含7种无人机模型和88种户外背景

Result: 旋转误差MAE 17.34度、MedAE 17.1度，平移误差MAE 0.135米、MedAE 0.242米，推理速度CPU 19.25 FPS、GPU 414.07 FPS，展现良好泛化能力和实时性

Conclusion: DroneKey++无需先验信息即可实现无人机3D姿态估计，结合合成数据集解决了现有方法泛化能力不足的问题，适用于实时安防监控应用
...
TL;DR: 提出一种可微分车辆模型框架，将动作序列转换为轨迹点，使动作型自动驾驶模型能在轨迹点基准上训练和评估


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统主要分为轨迹点型和动作型，但大多数基准协议和训练流程都是轨迹点型的，这使得动作型策略难以训练和比较，阻碍了其发展
...
Conclusion: 该框架成功弥合了轨迹点-动作之间的差距，首次使动作型架构能够在轨迹点基准上进行训练和评估，无需修改底层评估协议
...
TL;DR: 该论文提出一种对齐稀疏自编码器方法，通过能量一致性假设分析视觉语言模型的共享嵌入空间几何结构，发现稀疏双模态原子承载跨模态对齐信号，单模态原子解释模态差距。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在图像和文本对齐方面取得了显著成功，但其共享嵌入空间的几何结构仍然缺乏深入理解。研究者希望探索这种几何结构，特别是理解跨模态对齐的本质。

Method: 提出等能量假设：真正共享的概念在不同模态中应具有相同的平均能量。基于此假设开发对齐稀疏自编码器，在训练中鼓励能量一致性同时保持重构能力。该方法在已知真实数据的受控环境中进行验证，然后应用于基础视觉语言模型。

Result: 研究发现：1）稀疏双模态原子承载所有跨模态对齐信号；2）单模态原子作为模态特定偏置，完全解释模态差距；3）移除单模态原子可以消除差距而不损害性能；4）将向量运算限制在双模态子空间内可实现分布内编辑和改进检索。

Conclusion: 正确的归纳偏置既能保持模型保真度，又能使潜在几何结构变得可解释和可操作。该框架揭示了视觉语言模型共享嵌入空间的清晰结构，并展示了实际应用价值。
...
TL;DR: ForeHOI：一种前馈模型，能够直接从单目手-物交互视频中在1分钟内重建3D物体几何，无需预处理，通过联合预测2D掩码修复和3D形状补全来处理严重遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 日常手-物交互的单目视频为具身智能提供了宝贵资源，但现有方法在重建物体时面临严重遮挡和复杂耦合运动的挑战，需要解决这些限制以实现高效准确的3D物体重建。

Method: 提出前馈模型ForeHOI，通过联合预测2D掩码修复和3D形状补全来处理严重遮挡问题。2D和3D形状补全之间的信息交换提升了整体重建质量。同时创建了首个大规模高保真合成手-物交互数据集用于训练。

Result: ForeHOI在物体重建方面达到最先进性能，显著优于先前方法，并实现了约100倍的加速。推理时间在1分钟内，无需任何预处理步骤。

Conclusion: ForeHOI通过联合2D掩码修复和3D形状补全的前馈框架，有效解决了单目手持物体视频中的严重遮挡问题，实现了快速准确的3D物体重建，为手-物交互理解提供了实用解决方案。
...
TL;DR: 提出ASMa方法，通过非对称时空掩码策略学习更全面的骨架动作表示，结合知识蒸馏实现轻量化部署


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要关注高运动帧和高度数关节的掩码，导致特征表示存在偏差且不完整，难以泛化到不同运动模式

Method: 提出非对称时空掩码(ASMa)：1) 选择性掩码高度数关节和低运动帧；2) 掩码低度数关节和高运动帧。引入可学习的特征对齐模块，并使用知识蒸馏压缩模型

Result: 在NTU RGB+D 60/120和PKU-MMD数据集上，相比现有SSL方法平均提升2.7-4.4%，在噪声数据集迁移学习上提升达5.9%。蒸馏模型参数减少91.4%，推理速度提升3倍

Conclusion: ASMa方法通过平衡的掩码策略学习更全面的骨架动作表示，结合知识蒸馏实现了高效轻量化，适合资源受限场景部署
...
<details>
  <summary>Details</summary>
Motivation: Kabuki综合征和Wiedemann-Steiner综合征是两种临床表现重叠的罕见发育障碍，许多患者因遗传检测和专家资源的限制而无法确诊。皮肤纹理异常作为遗传综合征的标志性特征，在分子检测时代尚未得到充分利用。

Method: 开发基于视觉Transformer的深度学习模型，利用指纹图像区分Kabuki综合征、Wiedemann-Steiner综合征患者和健康对照，评估三个二元分类任务（对照vs.KS、对照vs.WSS、KS vs.WSS），并应用注意力可视化增强模型可解释性。

Result: 模型在三个分类任务中分别获得AUC分数0.80（对照vs.KS）、0.73（对照vs.WSS）和0.85（KS vs.WSS），对应F1分数分别为0.71、0.72和0.83。注意力可视化识别出对模型预测最关键的指纹区域。

Conclusion: 研究证实存在综合征特异性指纹特征，表明基于指纹的AI工具可作为非侵入性、可解释且易于获取的未来诊断辅助手段，用于早期诊断未确诊的遗传综合征。
...
TL;DR: 提出了MMEarth-Bench多模态环境任务基准，包含5个任务12种模态的全球数据，并提出了测试时多模态重建训练方法(TTT-MMR)来提升模型的地理泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有地理空间基准数据集模态单一、全球代表性不足，难以评估多模态预训练模型在全球尺度的性能，需要建立更全面的多模态环境任务基准。

Method: 1) 构建MMEarth-Bench数据集：包含5个多模态环境任务、12种模态、全球分布数据，包含分布内和分布外测试划分；2) 提出TTT-MMR方法：测试时利用所有可用模态进行多模态重建训练，提升模型适应新任务和地理域的能力。

Result: 基准测试显示多模态预训练在有限数据设置下能提升模型鲁棒性，但地理泛化能力仍然较差。TTT-MMR方法在随机和地理测试划分上均能提升模型性能，地理批处理在正则化和专业化之间取得良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 手术导航中的多模态图像配准面临两大挑战：多模态图像间的差异，以及术中组织位移和切除导致的图像变形。这些因素使得术前和术中多模态图像的有效配准变得困难。

Method: 1. 基于模态独立邻域描述符提取各模态特征，将多模态图像映射到特征空间；2. 设计多级金字塔融合优化机制，通过密集相关性分析和权重平衡耦合凸优化，在不同尺度上实现位移场的全局优化和局部细节补充。

Result: 在Learn2Reg 2025的ReMIND2Reg任务中，验证阶段和测试阶段均获得第一名。在Resect数据集上达到平均TRE 1.798mm，证明了方法在术前-术中图像配准中的广泛适用性。

Conclusion: MCPO方法有效解决了多模态图像配准的挑战，在手术导航应用中表现出色，代码已开源。
...
TL;DR: 提出一种将Vision Transformer线性层替换为卷积操作的方法，使其能在CNN优化的BPU硬件上高效部署，无需重新训练


<details>
  <summary>Details</summary>
Motivation: 随着深度学习发展，BPU等专用硬件为CNN提供INT8加速，但Vision Transformer（如DeiT）在计算机视觉任务中表现优异。然而，Transformer的线性层处理三维数据，而BPU针对四维卷积优化，导致ViT无法充分利用BPU的加速优势

Method: 通过精心设计的卷积算子替换Vision Transformer中的线性层和层归一化操作，使DeiT能够完全利用BPU的加速能力，同时允许原始权重参数被重构模型继承，无需重新训练或微调

Result: 量化后的DeiT-Base模型在ImageNet上达到80.4%准确率（原版81.8%），同时获得高达3.8倍的推理加速。在花卉分类数据集上微调的DeiT模型也表现优异，DeiT-Base仅下降0.5%准确率

Conclusion: 这是首个成功部署Vision Transformer并完全利用BPU加速的工作，通过将线性层转换为卷积操作，解决了硬件架构不匹配问题，实现了高效的ViT部署
...
TL;DR: 提出一种基于自适应重置的长期持续测试时域适应方法，通过监测标签翻转变化来动态调整模型权重重置间隔，以维持长期性能


<details>
  <summary>Details</summary>
Motivation: 现有持续测试时域适应方法主要关注短期适应优化，但缺乏对模型在长期非平稳环境中持续适应能力的关注。需要解决模型能否在长期变化环境中保持良好性能的问题。

Method: 提出自适应平衡重置方法，通过观察标签翻转轨迹模式与长期性能的关联，基于标签翻转变化动态确定权重重置间隔，在适当时间点重新初始化模型权重以保持长期适应能力。

Result: 在多个CTTA基准测试中验证了方法的有效性，相比现有方法取得了更优的性能表现，证明自适应重置策略能够有效维持模型在长期变化环境中的性能。

Conclusion: 通过自适应权重重置策略可以显著提升模型在长期持续测试时域适应中的性能，标签翻转变化是确定重置时机的有效指标，该方法为长期CTTA问题提供了简单而有效的解决方案。
...
TL;DR: 提出CER框架，通过级联早期拒绝机制实现高效OOD检测，包含SES和SHE两个模块，在CIFAR-100上FPR95从33.58%降至22.84%，计算开销减少32%


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法对低级统计噪声进行全尺度推理，导致计算资源浪费和语义幻觉问题，即深度网络将物理异常强行解释为高置信度语义特征

Method: 提出级联早期拒绝(CER)框架，包含：1)结构能量筛(SES)：在网络入口使用拉普拉斯算子建立非参数屏障，高效拦截物理信号异常；2)语义感知超球面能量(SHE)检测器：在中间层解耦特征大小和方向，识别细粒度语义偏差

Result: 在CIFAR-100基准测试中，平均FPR95从33.58%大幅降至22.84%，AUROC提升至93.97%，计算开销减少32%。在模拟传感器故障的真实场景中，性能远超SOTA方法

Conclusion: CER作为通用插件可无缝集成到各种SOTA模型中提供性能提升，解决了现有OOD检测方法的计算不匹配和语义幻觉问题
...
<details>
  <summary>Details</summary>
Motivation: SAM3通过可提示概念分割革新了开放词汇分割，但其依赖预定义概念，当目标域出现数据漂移或概念漂移时，视觉证据与提示之间的对齐关系会被破坏。

Method: 构建数据集特定的概念库：1) 通过类级视觉原型锚定目标域证据；2) 挖掘代表性支持点以抑制数据漂移下的异常值；3) 融合候选概念以纠正概念漂移。

Result: ConceptBank能有效使SAM3适应分布漂移，包括具有挑战性的自然场景和遥感场景，为OVS的鲁棒性和效率建立了新基准。

Conclusion: ConceptBank提供了一个参数免费的校准框架，能够动态恢复视觉证据与概念提示之间的对齐，提升开放词汇分割模型在分布漂移下的鲁棒性。
...
TL;DR: SPDA-SAM：一种自提示深度感知的SAM模型，通过语义-空间自提示模块和粗到细RGB-D融合模块，在无需人工提示的情况下实现更好的实例分割性能。
...
Method: 提出SPDA-SAM模型，包含两个核心模块：1）语义-空间自提示模块（SSSPM），从SAM的图像编码器和掩码解码器中提取语义和空间提示；2）粗到细RGB-D融合模块（C2FFM），将RGB图像特征与估计的深度图特征进行融合，深度图提供粗粒度结构指导，局部深度变化编码细粒度特征。

Result: 在12个不同数据集上的实验结果表明，SPDA-SAM超越了当前最先进的对比方法，证明了自提示机制和粗到细RGB-D融合操作的有效性。

Conclusion: SPDA-SAM通过自提示机制解决了SAM对人工提示的依赖问题，并通过深度信息补偿了RGB图像的空间信息损失，实现了更优的实例分割性能，为SAM模型的自提示和深度感知应用提供了新思路。
...
TL;DR: U-4DGS：一种用于单目视频中动态人体渲染的新框架，通过概率变形网络和双重光栅化管道处理遮挡问题，利用不确定性映射自适应调节梯度，实现高保真渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法在遮挡情况下渲染动态人体时存在严重问题：基于生成模型的方法会导致时间闪烁，而基于刚性几何启发式的方法无法捕捉多样外观。需要一种能鲁棒处理遮挡的渲染框架。

Method: 将任务重新表述为异方差观测噪声下的最大后验估计问题。提出U-4DGS框架，包含概率变形网络和双重光栅化管道，生成像素对齐的不确定性映射作为自适应梯度调节器。同时引入置信感知正则化，防止缺乏可靠视觉线索区域的几何漂移。

Result: 在ZJU-MoCap和OcMotion数据集上的大量实验表明，U-4DGS在渲染保真度和鲁棒性方面达到了最先进的水平。
...
TL;DR: FlowConsist：通过强制轨迹一致性解决快速流模型的两个核心问题，在ImageNet 256×256上仅用1步采样达到FID 1.52的新SOTA
...
Conclusion: 通过强制轨迹一致性，FlowConsist有效解决了快速流模型的轨迹漂移和误差累积问题，实现了高效的单步高质量图像生成
...
TL;DR: Di3PO提出一种新的偏好调优方法，通过隔离特定改进区域构建正负图像对，解决现有方法效率低、差异不显著的问题，在文本渲染任务上优于SFT和DPO基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型偏好调优方法依赖计算昂贵的生成步骤来创建正负图像对，这些方法存在以下问题：1) 生成的训练对缺乏有意义的差异；2) 采样和过滤成本高；3) 不相关像素区域方差大，从而降低训练效率。

Method: Di3PO提出一种新颖的方法来构建正负图像对，该方法能够隔离偏好调优过程中需要改进的特定区域，同时保持图像周围环境的稳定性。

Result: 该方法在扩散模型文本渲染这一具有挑战性的任务上展示了有效性，相比SFT和DPO基线方法取得了改进。

Conclusion: Di3PO通过针对性地隔离改进区域并保持上下文稳定，提供了一种更高效的偏好调优方法，解决了现有方法在训练效率和效果上的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨模态行人检测方法在RGB、热红外(TIR)和近红外(NIR)三模态场景中面临挑战：实际应用中可能无法同时获取所有模态，导致现有方法性能显著下降。需要解决任意模态组合下的鲁棒检测问题。

Method: 提出自适应不确定性感知网络(AUNet)，包含统一模态验证精炼(UMVR)模块和模态感知交互(MAI)模块。UMVR通过不确定性感知路由器验证模态可用性并进行语义精炼，MAI根据UMVR输出自适应激活/停用内部交互机制，实现可用模态的有效互补信息融合。

Result: 构建了包含8,281个像素对齐图像三元组的TRNT数据集，为算法研究提供全面基础。提出的AUNet方法能够在不确定输入组合下准确判别模态可用性并充分利用可用信息。

Conclusion: 通过AUNet框架有效解决了三模态行人检测中的模态缺失问题，实现了在任意模态组合下的鲁棒检测性能，为全天候监控系统提供了更可靠的解决方案。
...
TL;DR: 本文提出观察者中心显著目标检测(OC-SOD)，将显著目标检测从客观预测任务转变为考虑观察者特定因素的主观问题，构建首个OC-SOD数据集OC-SODBench，并设计基于"感知-反思-调整"过程的智能体基线OC-SODAgent。


<details>
  <summary>Details</summary>
Motivation: 现有显著目标检测方法将其视为客观预测任务，每个图像只有单一真值分割图，忽略了人类感知的主观性和多样性。这导致问题定义不完整且本质上不适定，无法捕捉不同观察者因先验知识差异而产生的不同显著感知。

Method: 1) 提出观察者中心显著目标检测(OC-SOD)框架，同时考虑视觉线索和观察者特定因素；2) 利用多模态大语言模型构建高效数据标注流程，创建首个OC-SOD数据集OC-SODBench，包含33k图像和152k文本提示-对象对；3) 设计OC-SODAgent智能体基线，通过类似人类的"感知-反思-调整"过程执行OC-SOD任务。

Result: 在提出的OC-SODBench数据集上进行广泛实验，验证了所提方法的有效性。通过观察者中心视角，弥合了人类感知与计算建模之间的差距，提供了对"显著"的更现实和灵活的理解。

Conclusion: OC-SOD将显著目标检测重新定义为考虑观察者特定因素的主观问题，更好地捕捉了人类感知的模糊性和多样性。构建的数据集和智能体基线为个性化、上下文感知的显著预测提供了新方向，使显著目标检测更加符合人类实际感知过程。
...
<details>
  <summary>Details</summary>
Motivation: GUI代理在自动化数字工作流程中潜力巨大，而GUI定位是端到端任务执行的基础能力。现有方法通常微调已有空间感知能力的模型，本研究希望从基础模型开始掌握完整技术流程

Method: 1. 精炼数据工程：统一开源数据集格式，采用增强、过滤和难度分级策略；2. 改进训练策略：持续微调视觉编码器提升感知精度，保持训练推理分辨率一致；3. 强化学习：利用可验证奖励机制，在感知密集型GUI定位任务中提升精度

Result: POINTS-GUI-G-8B在多个基准测试中取得SOTA：ScreenSpot-Pro 59.9分，OSWorld-G 66.0分，ScreenSpot-v2 95.7分，UI-Vision 49.9分

Conclusion: 从基础模型开始构建GUI定位能力是可行的，强化学习在感知密集型任务中能显著提升精度，GUI定位为强化学习提供了天然优势（奖励易验证且准确）
...
TL;DR: EUGen是一种新型高效稠密层，通过随机特征和输入范数依赖来近似标准全连接层，将推理复杂度从二次降为线性，同时保持表达能力。
...
Method: 提出EUGen层，利用随机特征近似标准全连接层，并引入输入范数依赖。该方法统一了现有高效全连接层扩展，通过层间知识转移技术实现无需反向传播的高效适配。

Result: 在Transformer和MLP中集成EUGen层，在图像分类、语言模型预训练和3D场景重建等任务中，推理速度提升达27%，内存效率提升达30%。

Conclusion: EUGen层通过线性时间复杂度和参数减少，在保持表达能力的同时显著提升效率，为大规模神经网络在现实场景中的可扩展部署提供了潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D语义占据预测方法依赖3D体素或高斯分布，难以高效捕捉驾驶环境中细粒度几何细节，需要更灵活的基础表示和多传感器融合方法

Method: 提出TFusionOcc框架：1）使用对象中心表示和可变形超二次曲面等几何灵活基元；2）采用多阶段多传感器融合；3）利用学生t分布和T混合模型处理不确定性

Result: 在nuScenes基准上达到最先进性能，在nuScenes-C数据集上验证了不同相机和激光雷达损坏场景下的鲁棒性

Conclusion: TFusionOcc通过几何灵活基元和多传感器融合有效提升了3D语义占据预测的精度和鲁棒性，为自动驾驶感知提供了更精细的环境理解
...
TL;DR: 提出了一种受昆虫视觉启发的生物启发视觉模型，通过完全自监督对比学习生成稀疏、判别性编码，在花朵识别和自然图像基准测试中表现优异，支持人工神经网络和脉冲神经网络两种实现。


<details>
  <summary>Details</summary>
Motivation: 当前许多视觉模型过于关注任务性能而忽视了生物真实的处理通路。昆虫视觉支持复杂行为但缺乏能够捕捉其处理原则的计算模型，需要开发既能体现生物真实性又能支持跨任务泛化的视觉模型。

Method: 开发了受昆虫视觉系统启发的生物启发视觉模型，将密集视觉输入转换为稀疏判别性编码。采用完全自监督对比目标进行训练，无需标注数据，支持跨任务重用。实现了人工神经网络和脉冲神经网络两种版本。

Result: 模型在花朵识别任务和自然图像基准测试中产生可靠的稀疏编码，能区分视觉相似的输入。在模拟定位设置中，优于简单的图像下采样基线，展示了神经形态视觉处理通路的功能优势。

Conclusion: 该研究推进了昆虫计算建模，提供了一个通用的生物启发视觉模型，能够在多样化任务中实现稀疏计算，为神经形态视觉处理提供了功能性验证。
...
<details>
  <summary>Details</summary>
Motivation: 基于LiDAR的3D目标检测器在远距离物体检测上表现不佳，因为点云在远距离变得稀疏，缺乏可靠的几何线索。现有方法通过RGB图像生成的深度补全虚拟点来增强数据，但直接使用所有虚拟点会增加计算成本并带来融合挑战。

Method: 提出Point Virtual Transformer (PointViT)框架，联合处理原始LiDAR点和选择性采样的虚拟点。探索多种融合策略（从早期点级融合到BEV门控融合），分析精度与效率的权衡。融合点云体素化后通过稀疏卷积编码为BEV表示，初始化高置信度物体查询，通过Transformer上下文聚合模块进行优化。

Result: 在KITTI基准测试中，Car类别的3D AP达到91.16%，BEV AP达到95.94%，2D检测AP达到99.36%，表现出优异的检测性能。

Conclusion: PointViT通过选择性融合虚拟点有效解决了远距离物体检测问题，在保持计算效率的同时显著提升了检测精度，为LiDAR与图像数据的有效融合提供了新思路。
...
TL;DR: 论文提出SemGeo-AttentionNet，一种双流架构，通过非对称跨模态融合结合几何处理和语义识别，首次实现考虑3D网格拓扑的时序扫描路径生成，显著提升3D视觉注意力预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D显著性方法依赖手工几何特征或缺乏语义感知的学习方法，无法解释人类为何注视语义重要但几何不显著的区域。需要一种能同时建模几何处理和语义识别的认知动机架构。

Method: 提出双流架构SemGeo-AttentionNet：1) 几何流使用点云transformer处理几何特征；2) 语义流通过几何条件多视图渲染获取扩散模型语义先验；3) 非对称跨模态融合让几何特征查询语义内容；4) 通过强化学习扩展到时序扫描路径生成，首次考虑3D网格拓扑和抑制返回动态。

Result: 在SAL3D、NUS3D和3DVA数据集上评估，相比现有方法有显著改进，验证了认知动机架构能有效建模人类在三维表面上的视觉注意力。

Conclusion: 通过明确建模几何-语义二分法的双流架构，结合扩散语义先验和点云transformer，能更好地解释和预测人类在3D物体上的视觉注意力，为3D显著性预测提供了新范式。
...
TL;DR: TP-GRPO改进GRPO框架，通过引入步级增量奖励和识别转折点来解决扩散模型强化学习中奖励稀疏和长期效应建模问题
...
Method: 提出TP-GRPO框架，包含两个关键创新：1）用步级增量奖励替代基于结果的奖励，提供密集的、步感知的学习信号，更好隔离每个去噪动作的"纯"效应；2）识别转折点（改变局部奖励趋势并使后续奖励演化与整体轨迹趋势一致的步骤），为这些动作分配聚合的长期奖励以捕捉其延迟影响。转折点仅通过增量奖励的符号变化检测，使TP-GRPO高效且无超参数

Result: 大量实验表明TP-GRPO能更有效地利用奖励信号，并持续改进生成质量

Conclusion: TP-GRPO通过解决奖励稀疏性和显式建模去噪轨迹中的长期效应，提高了GRPO在文本到图像生成中的效果，且方法高效无需超参数调整
...
TL;DR: 提出POPL-KF，一种基于卡尔曼滤波的VIO系统，使用仅姿态几何表示来处理点和线特征，减少线性化误差并实现即时视觉测量更新，在挑战性场景中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统VIO系统依赖点特征，在挑战性场景中性能下降。基于MSCKF的VIO系统存在特征3D坐标线性化误差和测量更新延迟问题，影响定位精度。
...
Conclusion: POPL-KF通过消除特征坐标的线性化误差和实现即时测量更新，显著提高了VIO在挑战性场景中的性能，为滤波基VIO系统提供了有效的解决方案。
...
TL;DR: 提出了一种无需先验知识的室外到室内指令驱动导航任务及相应框架，通过视觉中心的方法和首个开源数据集解决实际部署中的无缝过渡问题。


<details>
  <summary>Details</summary>
Motivation: 现有导航方法局限于室内或室外环境，依赖精确坐标系统，无法实现从室外到室内的精细入口导航，限制了实际应用中的实用性。
...
TL;DR: ChatUMM：首个支持多轮对话的统一多模态模型，通过交错多轮训练策略和系统化对话数据合成，在视觉理解和指令编辑任务上达到SOTA，同时保持文本到图像生成的竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型（UMMs）局限于单轮交互范式，只能处理独立请求而非连续对话。需要开发能够进行多轮对话、保持上下文一致性的统一多模态助手。

Method: 1. 交错多轮训练策略：将序列化的文本-图像流建模为连续对话流；2. 系统化对话数据合成管道：将标准单轮数据集转化为流畅对话，包括构建基础状态对话、通过"干扰"轮次和依赖历史的查询重写强制长程依赖解析、合成自然交错的多模态响应。

Result: 在开源统一模型中，ChatUMM在视觉理解和指令引导编辑基准测试中达到最先进性能，同时在文本到图像生成中保持竞争力的保真度。在复杂多轮场景中表现出卓越的鲁棒性，确保流畅、上下文感知的对话。

Conclusion: ChatUMM成功将统一多模态模型从单轮求解器扩展到多轮对话助手，通过创新的训练策略和数据合成方法，实现了在保持生成质量的同时增强对话能力的重要突破。
...
TL;DR: UnionST提出了一种新的文本合成数据引擎，通过增强语料、字体和布局多样性来缩小合成与真实数据的差距，并开发了自进化学习框架进行有效标注。


<details>
  <summary>Details</summary>
Motivation: 大规模且类别平衡的文本数据对场景文本识别（STR）模型训练至关重要，但真实数据收集困难。现有合成数据存在领域差距，在复杂场景中表现不佳，主要受限于语料、字体和布局多样性不足。

Method: 1. 分析主流基于渲染的合成数据集局限性；2. 提出UnionST数据引擎，合成覆盖挑战性样本并更好对齐真实场景复杂性的文本；3. 构建UnionST-S大规模合成数据集；4. 开发自进化学习（SEL）框架进行有效真实数据标注。

Result: 在UnionST-S上训练的模型相比现有合成数据集有显著提升，在某些场景下甚至超越真实数据性能。使用SEL框架时，仅需9%真实数据标签即可达到竞争性性能。

Conclusion: UnionST通过增强合成数据的多样性和复杂性有效缩小了合成与真实数据的差距，配合自进化学习框架能够显著减少对标注真实数据的依赖，为场景文本识别提供了高效的数据解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI生成方法（特别是扩散模型）合成的伪造人脸质量越来越高，现有基于空间和频率特征的检测方法对高质量完全合成伪造效果有限。面部属性受复杂物理定律和多参数控制，难以完美复制。

Method: 1. 基于Retinex理论的快速准确人脸纹理估计方法，实现精确的镜面反射分离；2. 设计Specular-Reflection-Inconsistency-Network (SRI-Net)，采用两阶段交叉注意力机制捕捉镜面反射与对应人脸纹理、直接光照之间的关系，并将镜面反射相关特征与图像特征融合进行伪造检测。

Result: 实验结果表明，该方法在传统深度伪造数据集和生成式深度伪造数据集上均取得优异性能，特别是在包含扩散模型生成的伪造人脸的数据集上表现突出。

Conclusion: 通过利用镜面反射这一难以复制的物理属性及其与相关因素的关系，提出的SRI-Net能够有效检测高质量AI生成的伪造人脸，为深度伪造检测提供了新的有效途径。
...
TL;DR: 提出LAB-Det方法，通过语言作为领域不变桥梁，无需训练即可实现单样本领域泛化，在数据稀缺的检测任务中超越微调方法


<details>
  <summary>Details</summary>
Motivation: 基础目标检测器在通用领域表现良好，但在专业领域（如水下图像、工业缺陷）性能下降。传统跨域少样本方法需要微调稀缺目标数据，成本高且易过拟合。研究能否让冻结的检测器仅凭每类一个样本无需训练就能适应新领域

Method: 提出LAB-Det方法，利用语言作为领域不变桥梁。不调整视觉特征，而是将每个样本投影为描述性文本，用这些语言描述来引导冻结的检测器。语言条件化替代了基于梯度的适应，实现数据稀缺领域的鲁棒泛化

Result: 在UODD（水下）和NEU-DET（工业缺陷）两个数据稀缺检测基准上评估，LAB-Det在无需更新任何参数的情况下，比最先进的微调基线提升高达5.4 mAP
...
TL;DR: 提出Efficient-LVSM，一种用于新视角合成的双流架构，通过解耦的协同精炼机制解决了LVSM中全自注意力设计的计算效率低下问题，在保持性能的同时显著提升了训练和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有基于transformer的新视角合成方法LVSM采用全自注意力设计存在两个主要问题：1) 输入视图数量的二次复杂度计算开销大；2) 异构token之间的刚性参数共享不够灵活。需要更高效的架构设计。

Method: 提出Efficient-LVSM双流架构，采用解耦的协同精炼机制：对输入视图使用视图内自注意力，对目标视图使用自注意力后交叉注意力的设计，消除不必要的计算。支持KV缓存实现增量推理。

Result: 在RealEstate10K数据集上，使用2个输入视图时达到29.86 dB PSNR，比LVSM提升0.2 dB，训练收敛速度快2倍，推理速度快4.4倍。在多个基准测试中达到SOTA，对未见视图数量有强零样本泛化能力。

Conclusion: Efficient-LVSM通过解耦的协同精炼机制有效解决了LVSM的计算效率问题，在保持高性能的同时显著提升了训练和推理速度，并具备良好的泛化能力和增量推理特性。
...
TL;DR: 提出实例无关域自适应目标检测新问题，针对目标域只有背景数据的情况，设计RSCN网络通过背景特征原型对齐和特征关系一致性实现域适应


<details>
  <summary>Details</summary>
Motivation: 现有域自适应目标检测方法假设目标域包含足够前景实例，但实际应用中（如野生动物监测、病变检测）收集含目标的数据成本高，而背景数据丰富。这种实例缺失的域适应场景具有重要实际意义和技术挑战。

Method: 提出关系与结构一致性网络（RSCN），采用基于背景特征原型的对齐策略，同时鼓励源域前景特征与背景特征之间关系的一致性，实现无目标实例情况下的鲁棒域适应。

Result: 在模拟自动驾驶检测、野生动物检测和肺结节检测三个专门基准上，RSCN在实例无关场景下显著优于现有DAOD方法。

Conclusion: 首次提出实例无关域自适应目标检测问题，通过RSCN网络有效解决了目标域只有背景数据的实际挑战，为相关应用提供了新解决方案，并发布了三个基准数据集促进研究。
...
TL;DR: 该论文提出了一个改进的无监督单目3D占据预测基准，通过重新定义占据概率表示、对齐评估协议，并引入遮挡感知极化机制来提升遮挡区域的3D结构推断能力。


<details>
  <summary>Details</summary>
Motivation: 现有无监督方法在训练和评估协议上存在不一致性，且2D真值无法有效解决遮挡区域固有的几何模糊问题，这限制了单目图像3D结构推断在自动驾驶中的应用。

Method: 1) 重新解释体渲染过程中的变量，识别最物理一致的占据概率表示；2) 改进评估协议，将新识别表示与体素级3D占据真值对齐；3) 引入遮挡感知极化机制，利用多视角视觉线索增强遮挡区域占据与自由空间的区分。

Result: 实验表明，该方法不仅显著优于现有无监督方法，还能达到与有监督方法相当的性能水平。

Conclusion: 通过重新制定基准、改进评估协议和引入遮挡感知机制，有效解决了无监督单目3D占据预测中的训练-评估不一致性和遮挡区域模糊问题，为自动驾驶视觉感知提供了更可靠的3D结构推断方案。
...
Conclusion: DreamHome-Pano通过创新的Prompt-LLM和冲突无控制架构，有效解决了室内生成中的条件冲突问题，实现了高质量、可控的全景室内合成，为个性化室内设计提供了专业级工具。
...
TL;DR: 使用深度估计模型Depth Anything V2，通过大规模机载LiDAR数据训练，可从卫星RGB影像直接估算森林冠层高度，精度优于现有全球产品。


<details>
  <summary>Details</summary>
Motivation: 大规模高分辨率森林冠层高度制图对理解碳循环和水循环至关重要。现有星载LiDAR（如ICESat-2、GEDI）数据稀疏且有不确定性，而机载/UAV LiDAR数据精度高但覆盖有限。需要一种能从广泛可得的卫星RGB影像估算冠层高度的新方法。

Method: 使用约16,000 km²的公开机载LiDAR点云数据生成的冠层高度模型（CHMs），结合3米分辨率的PlanetScope和机载RGB影像，训练最先进的单目深度估计模型Depth Anything V2。训练后的模型称为Depth2CHM，可直接从PlanetScope RGB影像估算空间连续的冠层高度。

Result: 在中国（约1 km²）和美国（约116 km²）的独立验证显示：Depth2CHM能准确估算冠层高度，两个站点的偏差分别为0.59 m和0.41 m，均方根误差（RMSE）分别为2.54 m和5.75 m。与现有全球米级分辨率CHM产品相比，平均绝对误差降低约1.5 m，RMSE降低约2 m。

Conclusion: 使用大规模机载LiDAR数据训练的单目深度估计网络，为从卫星RGB影像进行高分辨率、空间连续的森林冠层高度估算提供了一条有前景且可扩展的途径。
...
<details>
  <summary>Details</summary>
Motivation: 将栅格平面图转换为工程级矢量图形具有挑战性，因为需要处理复杂的拓扑结构和严格的几何约束。现有方法（基于像素的启发式方法或基于查询的transformer）存在脆弱性或产生碎片化房间的问题。

Method: 1. 将平面图矢量化重新定义为图像条件序列建模任务，采用"像素到序列"范式直接输出结构化JSON序列；2. 构建大规模数据集Floorplan-2M和高保真子集Floorplan-HQ-300K；3. 采用渐进训练策略：监督微调（SFT）用于结构基础和质量退火，然后使用组相对策略优化（GRPO）进行严格几何对齐。

Result: 在FPBench-2K基准测试中，FloorplanVLM表现出卓越的结构有效性，达到92.52%的外墙IoU，并在非曼哈顿架构上展现出强大的泛化能力。

Conclusion: FloorplanVLM通过统一的图像条件序列建模框架，成功解决了复杂平面图矢量化问题，实现了高精度几何约束满足和强大的泛化能力，为工程级矢量图形转换提供了有效解决方案。
...
TL;DR: DriveWorld-VLA是一个统一世界建模与规划的端到端自动驾驶框架，通过潜在空间集成视觉-语言-动作模型与世界模型，实现可控的特征级场景演化想象，减少对密集标注监督的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法未能有效统一未来场景演化与动作规划，主要因为潜在状态共享不足，限制了视觉想象对动作决策的影响。

Method: 提出DriveWorld-VLA框架，在潜在空间统一世界建模与规划，将世界模型的潜在状态作为VLA规划器的核心决策状态，支持可控的动作条件特征级想象，避免昂贵的像素级推演。

Result: 在NAVSIMv1上达到91.3 PDMS，NAVSIMv2上达到86.8 EPDMS，nuScenes上达到0.16的3秒平均碰撞率，均达到最先进性能。

Conclusion: DriveWorld-VLA通过潜在空间统一世界建模与规划，使VLA规划器能从整体场景演化建模中直接受益，有效提升自动驾驶决策性能。
...
TL;DR: 提出MicroBi-ConvLSTM，一种超轻量级卷积循环架构，仅需11.4K参数，比现有方法减少2.9-11.9倍参数，在资源受限的可穿戴设备上保持竞争力性能。


<details>
  <summary>Details</summary>
Motivation: 资源受限可穿戴设备上的人体活动识别需要平衡准确性和严格内存/计算预算的模型。现有轻量级架构如TinierHAR和TinyHAR虽然准确率高，但考虑到操作系统开销后仍超出微控制器的SRAM限制。

Method: 提出MicroBi-ConvLSTM架构：采用两阶段卷积特征提取（4倍时间池化）和单层双向LSTM层，实现超轻量化设计（平均11.4K参数），保持线性O(N)复杂度。

Result: 在八个HAR基准测试中表现良好：UCI-HAR上93.41%宏F1，SKODA装配手势94.46%，Daphnet步态冻结检测88.98%。INT8后训练量化仅导致0.21%平均F1下降，部署占用仅23.0KB。

Conclusion: MicroBi-ConvLSTM在超轻量级范围内保持竞争力性能，显著减少参数数量，适合内存受限的边缘设备部署，为资源受限可穿戴设备上的HAR提供了实用解决方案。
...
TL;DR: 提出AdaptOVCD，一种基于双维多级信息融合的无训练开放词汇变化检测架构，能在零样本方式下检测任意类别变化
...
Method: 基于双维多级信息融合：垂直方向融合数据、特征和决策三个层次的信息，水平方向加入针对性自适应设计，实现异构预训练模型的深度协同

Result: 在九个场景的综合评估中，AdaptOVCD以零样本方式检测任意类别变化，显著优于现有无训练方法；在跨数据集评估中达到全监督性能上限的84.89%，展现优越泛化能力

Conclusion: AdaptOVCD通过双维多级信息融合架构有效解决了开放词汇变化检测问题，在无训练条件下实现了高性能和强泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 现有AIGC检测器评估协议忽视了反取证攻击，无法确保在实际应用中的全面鲁棒性。作者发现基于Vision-Language Models（如CLIP）的检测器存在对抗性漏洞，因为下游检测器继承了这些公开可访问模型的特征空间

Method: 提出ForgeryEraser框架，设计多模态引导损失（而非传统的基于logit的优化），在VLM特征空间中驱动伪造图像嵌入向文本衍生的真实锚点靠近以消除伪造痕迹，同时使其远离伪造锚点

Result: 在全局合成和局部编辑基准测试中，ForgeryEraser显著降低了先进AIGC检测器的性能。此外，它还能使可解释的取证模型为伪造图像生成与真实图像一致的解释

Conclusion: 该研究揭示了基于VLM的AIGC检测器的对抗性漏洞，提出的ForgeryEraser框架展示了无需访问目标检测器即可执行有效反取证攻击的能力，强调了在实际应用中评估检测器鲁棒性的重要性
...
Result: NECromancer在高压缩下实现高保真重建，有效解耦运动与骨架结构，支持跨物种运动迁移、组合、去噪、生成和文本-运动检索。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在测试时扩展（动态调整推理计算预算）方面存在脆弱性：非结构化的思维链将感知和推理纠缠在一起，导致长而混乱的上下文，小的感知错误可能级联成完全错误的答案。此外，需要昂贵的手工奖励强化学习才能获得良好性能。

Method: SPARC采用模块化框架，明确分离视觉感知和推理。受大脑序列感觉-认知处理启发，实现两阶段流水线：1）执行显式视觉搜索定位问题相关区域；2）基于这些区域进行推理生成最终答案。支持非对称计算分配、选择性优化和压缩上下文（低分辨率全局搜索+高分辨率区域处理）。

Result: 在挑战性视觉推理基准测试中，SPARC超越整体式基线和强视觉接地方法。例如，在V* VQA基准上，将Qwen3VL-4B的准确率提升6.7个百分点；在OOD任务上超越"thinking with images"方法4.6分，同时仅需200倍更低的token预算。

Conclusion: SPARC通过明确分离感知和推理，提供了更稳健、高效的视觉语言模型框架，支持灵活的计算分配和优化策略，在保持高性能的同时显著降低计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于原型的模型在ImageNet规模上泛化能力差，需要计算昂贵的骨干网络微调，且存在"原型漂移"问题——原型缺乏训练分布的真实基础，在小扰动下激活会变化。

Method: 使用潜在向量量化，将原型约束在潜在空间的离散学习码本中，确保原型忠实表示训练数据，无需更新骨干网络，作为高效可解释的头部模块工作。

Result: 在ImageNet和细粒度基准测试（CUB-200、Cars-196）上评估，ProtoQuant实现了有竞争力的分类准确性，能泛化到ImageNet，可解释性指标与其他基于原型部件的方法相当。

Conclusion: ProtoQuant通过潜在向量量化解决了原型漂移问题，实现了原型稳定性和可解释性，能够扩展到大规模数据集，无需骨干网络微调，为可解释AI提供了有效解决方案。
...
TL;DR: DAVE是一种基于ViT梯度分解的归因方法，通过分离局部等变稳定分量和架构诱导伪影，为Vision Transformers生成稳定高分辨率的归因图。
...
Method: DAVE基于输入梯度的结构化分解，利用ViTs的架构特性，分离出输入-输出映射的局部等变稳定分量，将其与架构诱导伪影和其他不稳定性来源隔离开来。

Result: 该方法能够为Vision Transformers生成稳定且高分辨率的归因图，解决了现有方法中存在的结构化伪影问题。

Conclusion: DAVE通过数学基础的梯度分解方法，有效提升了ViT模型归因的可解释性和稳定性，为理解Transformer架构的决策过程提供了更好的工具。
...
### [58] [An Integer Linear Programming Approach to Geometrically Consistent Partial-Partial Shape Matching](https://arxiv.org/abs/2602.06590)
...
<details>
  <summary>Details</summary>
Motivation: 部分-部分3D形状匹配是现实世界中最常见的场景（如3D扫描），但现有研究较少，因为需要同时计算准确对应关系和确定未知重叠区域，具有独特挑战性。
...
Conclusion: 该方法成功解决了部分-部分3D形状匹配的核心挑战，为现实世界应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统矿物勘探成本高、现场数据有限，需要开发基于卫星影像的大规模找矿潜力制图方法。本研究旨在探索如何利用生成式表示学习从多光谱卫星影像中识别含金区域。

Method: 使用在FalconSpace-S2 v1.0数据集上预训练的Isometric自编码器基础模型，从Sentinel-2多光谱影像中提取信息密集的光谱-空间表示，然后将这些表示作为轻量级XGBoost分类器的输入。与原始光谱输入基线进行比较。

Result: 在63个已知含金和非含金位置的Sentinel-2影像数据集上，提出的方法将斑块级准确率从0.51提升到0.68，影像级准确率从0.55提升到0.73，表明生成式嵌入能够捕捉可迁移的矿物学模式。

Conclusion: 基础模型的表示学习能够显著提升卫星影像矿物勘探的准确性，使矿物勘探更加高效、可扩展和全球适用，特别是在标记数据有限的情况下。
...
TL;DR: CauCLIP：基于因果关系的视觉语言框架，利用CLIP学习领域不变表示，用于无需目标领域数据的外科阶段识别，显著优于现有方法
...
Method: 提出CauCLIP框架：1）基于频率的数据增强策略扰动领域特定属性同时保留语义结构；2）因果抑制损失减少非因果偏差并强化因果手术特征；3）统一训练框架使模型专注于手术工作流的稳定因果因素

Result: 在SurgVisDom硬适应基准测试中，该方法显著优于所有竞争方法，展示了因果引导的视觉语言模型在领域泛化手术视频理解中的有效性

Conclusion: CauCLIP通过因果推理和视觉语言建模的结合，为无需目标领域数据的外科阶段识别提供了有效的解决方案，推动了领域泛化手术视频理解的发展
...
<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在自然图像生成和多模态推理方面表现出色，但其在计算机使用规划任务（如空间推理和程序理解）中的能力尚未得到充分探索，而这些任务与日常生活密切相关。

Method: 设计了PlanViz基准，包含三个需要规划步骤的日常子任务：路线规划、工作流程图和网页UI展示。通过人工标注的问题和参考图像确保数据质量，并提出任务自适应评分PlanScore来评估生成图像的正确性、视觉质量和效率。

Result: 通过实验揭示了统一多模态模型在计算机使用任务中图像生成和编辑的关键局限性，为未来研究指明了方向。

Conclusion: PlanViz基准填补了统一多模态模型在计算机使用规划任务评估方面的空白，提出的PlanScore为全面准确评估提供了有效工具，有助于推动相关领域的研究发展。
...
TL;DR: CytoCrowd是一个新的细胞学分析公共基准数据集，包含446张高分辨率图像，每张图像既有4位病理学家的原始冲突标注，又有资深专家建立的高质量金标准，可用于标准计算机视觉任务和标注聚合算法评估。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像分析数据集存在关键缺陷：大多数数据集要么只提供单一干净的金标准（掩盖了真实世界的专家分歧），要么提供多个标注但没有单独的金标准进行客观评估。需要填补这一空白。

Method: 创建CytoCrowd数据集，包含446张高分辨率细胞学图像，每张图像有两个关键组成部分：1）来自4位独立病理学家的原始冲突标注；2）由资深专家建立的单独高质量金标准。这种双重结构使数据集具有多功能性。

Result: CytoCrowd数据集为细胞学分析提供了一个全面的基准，既可作为标准计算机视觉任务（如目标检测和分类）的基准，又可作为评估标注聚合算法的真实测试平台。实验展示了CytoCrowd带来的挑战并证明了其价值。

Conclusion: CytoCrowd填补了医学图像分析数据集的关键空白，通过提供既有专家冲突标注又有单独金标准的双重结构，成为开发下一代医学图像分析模型的宝贵资源。
...
### [63] [Rethinking Multi-Condition DiTs: Eliminating Redundant Attention via Position-Alignment and Keyword-Scoping](https://arxiv.org/abs/2602.06850)
*Chao Zhou,Tianyi Wei,Yiling Chen,Wenbo Zhou,Nenghai Yu*

Main category: cs.CV

TL;DR: 提出PKA框架，通过位置对齐和关键词范围注意力机制，高效解决多条件控制中的计算冗余问题，实现10倍推理加速和5倍显存节省。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型缺乏细粒度控制能力，多条件控制集成到DiTs中受限于传统的"连接-注意力"策略，存在二次计算和内存开销，且跨模态交互存在空间和语义冗余。

Method: 提出PKA框架：1) 位置对齐注意力(PAA)通过局部补丁对齐线性化空间控制；2) 关键词范围注意力(KSA)通过语义感知掩码修剪无关的主题驱动交互；3) 条件敏感性感知采样(CSAS)重新加权训练目标，加速收敛并增强条件保真度。
...
### [64] [Can We Build a Monolithic Model for Fake Image Detection? SICA: Semantic-Induced Constrained Adaptation for Unified-Yet-Discriminative Artifact Feature Space Reconstruction](https://arxiv.org/abs/2602.06676)
*Bo Du,Xiaochen Ma,Xuekang Zhu,Zhe Yang,Chaogun Niu,Jian Liu,Ji-Zhe Zhou*

Main category: cs.CV

TL;DR: SICA提出首个单模型假图检测框架，通过语义引导的约束适应重建统一但可区分的伪造特征空间，在OpenMMSec数据集上超越15个SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有单模型假图检测(FID)方法理论上更有前景但实际性能不佳，研究发现这是由于跨子域伪造痕迹的"异质性现象"导致特征空间崩溃。需要解决"统一但可区分"的特征空间重建这一核心挑战。

Method: 提出语义引导约束适应(SICA)范式，假设高层语义可作为特征空间重建的结构先验，通过语义诱导实现统一但可区分的伪造特征空间重建。

Result: 在OpenMMSec数据集上，SICA超越15个SOTA方法，以近乎正交的方式重建了目标统一但可区分的伪造特征空间，验证了语义作为结构先验的假设。

Conclusion: SICA是首个实用的单模型FID范式，通过语义引导成功解决了跨子域伪造痕迹异质性导致的特征空间崩溃问题，实现了统一但可区分的伪造检测。
...
### [65] [NanoFLUX: Distillation-Driven Compression of Large Text-to-Image Generation Models for Mobile Devices](https://arxiv.org/abs/2602.06879)
*Ruchika Chavhan,Malcolm Chadwick,Alberto Gil Couto Pimentel Ramos,Luca Morreale,Mehdi Noroozi,Abhinav Mehrotra*

Main category: cs.CV

TL;DR: NanoFLUX是一个2.4B参数的文本到图像流匹配模型，通过渐进式压缩从17B FLUX.1-Schnell蒸馏而来，能在移动设备上2.5秒生成512x512高质量图像。


<details>
  <summary>Details</summary>
Motivation: 解决大型文本到图像扩散模型与设备端解决方案之间日益扩大的差距，实现高质量的设备端文本到图像生成。
...
Conclusion: 通过创新的压缩和蒸馏技术，成功缩小了最先进模型与设备端解决方案之间的差距，为移动设备上的高质量文本到图像生成提供了实用方案。
...
### [66] [Clinical-Prior Guided Multi-Modal Learning with Latent Attention Pooling for Gait-Based Scoliosis Screening](https://arxiv.org/abs/2602.06743)
...
<details>
  <summary>Details</summary>
Motivation: 青少年特发性脊柱侧弯（AIS）早期检测很重要，但传统筛查方法主观、难以扩展且依赖专家经验。现有视频步态分析方法存在数据泄漏问题（同一受试者视频重复使用）或模型过于简化缺乏临床可解释性。
...
Conclusion: 这项工作为青少年特发性脊柱侧弯的早期检测提供了稳健、可解释且临床基础扎实的解决方案，通过视频步态分析实现了可扩展、非侵入性的评估，解决了现有方法的数据泄漏和可解释性问题。
...
### [67] [PANC: Prior-Aware Normalized Cut for Object Segmentation](https://arxiv.org/abs/2602.06912)
...
<details>
  <summary>Details</summary>
Motivation: 完全无监督分割方法通常只寻找最显著的对象，导致分割结果对初始化、种子顺序和阈值启发式敏感，缺乏稳定性和可复现性。

Method: 基于TokenCut方法，在token-token亲和力图中加入少量与锚节点耦合的先验，通过操纵图拓扑结构，使光谱特征空间偏向与标注一致的分割。
...
TL;DR: 提出一种将复杂情绪状态描述为情绪类别概率分布的新方法，通过VAD空间映射自动重新标注现有数据集，使情绪识别能表达多种情绪的混合状态。


<details>
  <summary>Details</summary>
Motivation: 传统面部情绪识别将情绪视为单一标签分类问题（六种基本情绪之一），这过度简化了实际情绪状态的多面性和复杂性。真实情绪通常是多种情绪以不同强度组合的结果，而现有研究数据集通常只标注单一情绪类别，限制了分布学习方法的有效性。

Method: 提出一种自动重新标注现有数据集的方法：1）利用已有研究将基本和复合情绪映射到VAD（效价-唤醒-支配）空间的概率分布；2）给定带有VAD值标注的人脸图像，估计其属于每个情绪分布的可能性；3）将情绪状态描述为情绪混合的概率分布，丰富描述并考虑感知的模糊性。

Result: 通过初步实验展示了该方法的优势，并提供了新的研究方向。相关数据标注已在GitHub上公开。

Conclusion: 该方法能够更准确地描述复杂的情绪状态，将情绪识别从单一标签分类扩展到概率分布表示，考虑了情绪的多面性和感知模糊性，为情绪识别研究开辟了新方向。
...
TL;DR: 开发基于计算机视觉的自动评估甘薯象甲损害方法，替代传统人工评分，提高评估效率和准确性


<details>
  <summary>Details</summary>
Motivation: 甘薯象甲是甘薯生产中最具破坏性的害虫之一，特别是在撒哈拉以南非洲地区。传统的损害评估方法主要依赖人工评分，劳动强度大、主观性强且结果不一致，严重阻碍了培育抗性甘薯品种的育种计划。

Method: 采用计算机视觉方法，在田间和实验室两种环境下进行评估。田间数据用于训练分类模型预测根部损害严重程度；实验室数据采用YOLO12目标检测模型，结合两阶段管道（根部分割和切片策略）来检测微小的象甲取食孔。

Result: 田间分类模型测试准确率达到71.43%；实验室目标检测模型在识别微小象甲取食孔方面达到77.7%的平均精度。计算机视觉技术提供了高效、客观且可扩展的评估工具。

Conclusion: 计算机视觉技术能够提供与当代育种工作流程无缝对接的高效、客观、可扩展评估工具，显著提高甘薯育种计划中的表型分析效率，对减轻象甲对粮食安全的不利影响具有重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 在计算机视觉中，理解不同视角下图像局部区域之间的几何关系对于特征匹配、三维重建等任务至关重要。现有方法通常需要复杂的计算或近似，缺乏精确的闭式表达式。

Method: 基于校准相机模型和几何关系，推导了从相对相机位姿、图像坐标和局部表面法向到仿射变换的数学闭式表达式。

Result: 得到了局部图像块仿射变换的精确闭式解，该变换完全由相对相机位姿、图像坐标和局部表面法向决定。

Conclusion: 该闭式表达式为理解多视角几何关系提供了理论基础，可用于改进特征匹配、三维重建等计算机视觉任务的精度和效率。
...
TL;DR: RAIGen是一个无监督框架，用于在扩散模型中自动发现罕见或少数属性，这些属性在训练数据中代表性不足但仍编码在模型表示中。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么关注预定义的公平类别（如性别、种族），要么识别主导输出的多数属性，但都忽略了发现数据分布中代表性不足的罕见属性（社会、文化或风格特征）。

Method: 使用Matryoshka稀疏自编码器和结合神经元激活频率与语义独特性的新型少数度量，识别可解释的神经元，其最高激活图像揭示代表性不足的属性。

Result: RAIGen能够在Stable Diffusion中发现超越固定公平类别的属性，可扩展到SDXL等更大模型，支持跨架构的系统性审计，并能在生成过程中有针对性地放大罕见属性。

Conclusion: RAIGen是第一个用于扩散模型中无监督罕见属性发现的框架，填补了现有方法在发现数据分布中代表性不足但编码在模型中的罕见属性方面的空白。
...
Method: 提出基于分析性高斯误差量化的简化框架，核心贡献是从3DGS渲染方程推导出新的误差准则，能精确测量每个高斯对渲染图像的贡献。引入高效算法，支持单次前向传播完成误差计算，框架支持训练中剪枝和训练后简化两种应用场景。

Result: 实验结果表明，该方法在两种应用场景下均优于现有最先进的剪枝方法，实现了模型紧凑性与高渲染质量之间的优越权衡。

Conclusion: GaussianPOP提供了一个准确且灵活的简化框架，通过分析性误差量化实现了比现有方法更优的3D高斯泼溅模型简化效果。
...
Method: 提出AdaRoute方法，采用混合专家架构，每个专家是可训练参数矩阵。通过动态参数路由机制，每个AdaRoute模块根据当前输入动态生成定制化的权重矩阵，实现输入相关的低秩适应。

Result: 在语义分割、目标检测、实例分割和全景分割等多种视觉任务上的广泛实验证明了AdaRoute的优越性能。

Conclusion: AdaRoute通过动态参数路由和共享专家中心，实现了输入相关的低秩适应和跨层特征交互，在参数高效微调方面取得了显著改进。
...
TL;DR: 提出RFDM（残差流扩散模型），一种因果、高效的视频编辑方法，能够处理可变长度视频并逐帧编辑，计算效率接近图像模型


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要固定长度输入和大量计算资源，而自回归视频生成虽然高效但很少用于视频编辑。需要一种既能处理可变长度视频又计算高效的编辑方法

Method: 从2D图像扩散模型出发，通过条件化当前时间步编辑于前一预测结果来适应视频编辑。提出残差流扩散模型（RFDM），利用视频时间冗余性，让模型预测目标输出与前一预测之间的残差，专注于连续帧之间的变化

Result: RFDM在全局/局部风格迁移和物体移除任务上超越了基于图像的方法，与完全时空3D视频模型竞争，同时计算效率接近图像模型，且输入视频长度独立
...
TL;DR: 论文发现多模态扩散变换器（MMDiTs）在文本到图像生成中存在"提示遗忘"现象，并提出无需训练的方法"提示重注入"来缓解此问题，显著提升了指令跟随能力和生成质量。


<details>
  <summary>Details</summary>
Motivation: 研究发现当前的多模态扩散变换器（如SD3、SD3.5、FLUX.1）在文本到图像生成过程中存在一个关键问题：随着网络深度的增加，文本分支中的提示语义信息会逐渐被遗忘，这影响了模型的指令跟随能力。

Method: 提出了一种无需训练的"提示重注入"方法：将早期层的提示表示重新注入到后期层中，以缓解提示遗忘现象。该方法通过在不同层之间传递和强化文本语义信息来改善模型性能。

Result: 在GenEval、DPG和T2I-CompBench++等基准测试中，该方法在指令跟随能力上取得了显著提升，同时在偏好、美学和整体文本-图像生成质量等指标上都有改善。

Conclusion: 提示遗忘是多模态扩散变换器中的一个重要问题，通过简单的提示重注入方法可以有效缓解这一问题，显著提升文本到图像生成模型的性能和指令跟随能力。
...
TL;DR: 该研究通过构建合成基准数据集和度量指标，探究了视觉大语言模型在处理细粒度视觉信息时的冗余问题，发现任务复杂度与视觉压缩之间存在关联，高复杂度视觉数据对改善VLLM的视觉表示分布至关重要。


<details>
  <summary>Details</summary>
Motivation: 视觉大语言模型的视觉能力一直落后于语言能力，特别是在需要细粒度视觉信息或空间推理的任务上表现不佳。现有研究将问题归因于视觉冗余，但缺乏对具体信息处理机制和丢弃类型的深入理解。

Method: 1. 构建专门设计的合成基准数据集来探测各种视觉特征；2. 开发测量视觉冗余的度量指标；3. 在多个复杂视觉任务上微调VLLMs，研究冗余和压缩如何随训练数据复杂度变化。

Result: 发现任务复杂度与视觉压缩之间存在关联，表明拥有足够比例的高复杂度视觉数据对于改变VLLMs的视觉表示分布方式至关重要，从而提升其在复杂视觉任务上的性能。

Conclusion: 该研究为理解VLLMs的视觉信息处理机制提供了新见解，特别是揭示了视觉冗余与任务复杂度之间的关系，为训练下一代VLLMs提供了有价值的指导。
...
Method: 提出一个医学数据集错误标签检测框架，在最大的公开胶囊内镜视频数据集上验证，通过三位经验丰富的胃肠病专家重新标注潜在错误标签样本。

Result: 框架成功检测出错误标签数据，清洗数据集后相比现有基线方法，异常检测性能得到提升。

Conclusion: 提出的错误标签检测框架能有效识别医学数据集中的标注错误，通过数据清洗可提升模型性能，对医学影像分析有重要价值。
...
TL;DR: CineScene是一个用于电影级视频生成的框架，通过解耦场景上下文，使用隐式3D感知场景表示，实现相机轨迹控制的场景一致视频合成。


<details>
  <summary>Details</summary>
Motivation: 传统电影制作需要物理布景，成本高昂。为了解决这个问题，研究者提出了解耦场景上下文的电影视频生成任务：给定静态环境的多张图像，合成包含动态主体的高质量视频，同时保持场景一致性并遵循用户指定的相机轨迹。

Method: CineScene采用隐式3D感知场景表示，通过VGGT将场景图像编码为视觉表示，然后通过额外的上下文拼接将空间先验注入预训练的文本到视频生成模型。训练时采用随机打乱输入场景图像的策略增强鲁棒性，并使用Unreal Engine 5构建场景解耦数据集。

Result: CineScene在场景一致的电影视频生成方面实现了最先进的性能，能够处理大幅度的相机运动，并在多样化环境中展现出良好的泛化能力。

Conclusion: CineScene通过创新的上下文条件机制和隐式3D特征注入，成功解决了电影视频生成中的场景一致性和相机控制问题，为低成本电影制作提供了有效解决方案。
...
TL;DR: MedMO是一个专门为医学领域构建的多模态大语言模型，通过多阶段训练在医学视觉问答、报告生成和疾病定位等任务上显著优于现有开源模型。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学领域的应用受到领域覆盖不足、模态对齐不充分和推理能力有限的限制，需要专门针对医学领域构建的模型来解决这些问题。

Method: 采用三阶段训练方案：1）跨模态预训练对齐视觉编码器和医学语言骨干；2）多任务指令微调涵盖图像描述、视觉问答、报告生成、检索和疾病定位；3）强化学习结合事实性检查和边界框GIoU奖励来增强空间定位和逐步推理能力。

Result: 在视觉问答基准上平均准确率比基线提升13.7%，接近SOTA模型Fleming-VL（差距仅1.9%）；文本问答比基线提升6.9%，比Fleming-VL提升14.5%；在医学报告生成中语义和临床准确性均有显著提升；疾病定位能力IoU比基线提升40.4%，比Fleming-VL提升37.0%。

Conclusion: MedMO展示了专门针对医学领域训练的多模态大语言模型在跨模态任务上的优越性能，特别是在空间定位和临床推理方面表现突出，为医学AI应用提供了强有力的基础模型。
...
TL;DR: 论文提出基于名人名言重构的广告口号生成新范式，通过模块化框架将口号生成分解为可解释的子任务，相比现有LLM方法在多样性、新颖性等方面有提升


<details>
  <summary>Details</summary>
Motivation: 广告疲劳降低了重复口号的效果，现有LLM方法生成的口号风格冗余、缺乏品牌个性且过于机器化，需要新颖、有创意、有洞察力的口号生成方法

Method: 提出模块化框架，将口号生成分解为四个可解释子任务：名言匹配、结构分解、词汇替换和混合生成，通过重新情境化名人名言来生成口号

Result: 通过自动和人工评估，在多样性、新颖性、情感影响和人类偏好方面相比三种最先进的LLM基线方法有边际改进

Conclusion: 名人名言是创意口号生成的强大资源，提出的模块化框架能够生成平衡新颖性与熟悉性的有效口号，在多个评估维度上优于现有方法
...
TL;DR: 提出RMCD解码方法，通过基于相关性的多上下文对比解码，有效聚合多个相关上下文信息并抑制无关上下文的负面影响，无需额外训练即可提升大视觉语言模型的知识密集型视觉问答性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大视觉语言模型能力显著，但仍缺乏对特定实体的详细知识。检索增强生成是常用解决方案，但现有解码方法未能充分利用多个相关上下文，也无法有效抑制无关上下文的负面影响。

Method: 提出相关性感知的多上下文对比解码方法，通过基于每个上下文与问题相关性的加权组合来生成最终预测，有效聚合有用信息并抵消无关上下文的负面影响。

Result: 在多个大视觉语言模型和三个知识密集型视觉问答基准测试中，RMCD始终优于其他解码方法，且无需额外训练，对检索结果具有鲁棒性。

Conclusion: RMCD是一种简单有效的解码方法，能够显著提升检索增强生成在大视觉语言模型中的性能，通过加权聚合多个上下文信息并抑制无关内容的影响。
...
TL;DR: CAST：受戏剧理论启发的角色与场景记忆架构，通过构建3D场景（时间/地点/主题）并组织成角色档案来表示情景记忆，结合图语义记忆形成双记忆系统，显著提升对话问答性能


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆系统主要关注语义记忆，将经验表示为键值对、向量或图等结构，难以表示和检索连贯的事件（情景记忆）。人类的情景记忆能够回忆具体事件的人物、时间、地点等要素，而现有系统在这方面存在不足。

Method: 提出CAST（Character-and-Scene based memory）架构：1）构建3D场景（时间/地点/主题）表示情景记忆；2）将场景组织成角色档案，总结角色的相关事件；3）结合图基础的语义记忆，形成双记忆设计。

Result: 在多个数据集上，CAST相比基线平均提升8.11% F1分数和10.21%的LLM-as-a-Judge评分，特别是在开放性和时间敏感的对话问题上表现更优。
...
TL;DR: 该论文是一篇关于智能体记忆系统的综述，提出了一个统一的三维框架（记忆基底、认知机制、记忆主体），分析了不同智能体拓扑中的记忆实现，并讨论了评估基准和未来挑战。


<details>
  <summary>Details</summary>
Motivation: 人工智能研究正从追求基准分数转向关注实际问题定义和真实世界评估。在"下半场"中，核心挑战是智能体在长期、动态、用户依赖环境中的实际效用，需要处理上下文爆炸并持续管理大量信息。记忆系统成为填补效用差距的关键解决方案。

Method: 提出了一个统一的三维框架来分析基础智能体记忆：1) 记忆基底（内部和外部），2) 认知机制（情景、语义、感知、工作、程序记忆），3) 记忆主体（智能体中心vs用户中心）。分析了不同智能体拓扑中的记忆实现，并讨论了记忆操作的学习策略。

Result: 建立了一个全面的智能体记忆分类和分析框架，涵盖了记忆系统的多个维度。该框架能够统一理解当前数百篇记忆相关论文的研究成果，为智能体记忆系统的设计和评估提供理论基础。

Conclusion: 记忆系统是解决智能体在复杂环境中实际效用问题的关键。论文提出的三维框架为理解和设计智能体记忆系统提供了统一视角，同时指出了评估基准、实际效用验证等开放挑战和未来研究方向。
...
TL;DR: PersonaPlex：一种结合角色条件与语音克隆的双工对话语音模型，通过混合系统提示和合成数据集训练，在多角色客服场景中超越现有方法。
...
Method: 提出PersonaPlex模型，采用混合系统提示（角色条件+文本提示+语音克隆），利用开源LLM和TTS模型生成的大规模合成对话数据集进行训练，并扩展Full-Duplex-Bench基准以评估多角色客服场景。

Result: PersonaPlex在角色条件行为、语音条件语音和自然对话响应方面表现优异，在角色遵循度、说话人相似度、延迟和自然度方面超越最先进的双工语音模型和基于LLM的混合语音系统。

Conclusion: PersonaPlex成功解决了现有双工语音模型的局限性，通过角色和语音条件实现了更灵活、个性化的语音交互，为结构化角色驱动应用提供了有效解决方案。
...
TL;DR: 提出一个基于文献感知的新颖性评估框架，通过学习同行评审报告来模拟人类对研究新颖性的判断，并将其与现有研究进行结构化比较
...
Method: 使用近8万条来自顶级AI会议的新颖性标注评审报告，微调大语言模型来捕捉与评审者一致的新颖性评估行为。系统提取论文的结构化表示（思想、方法、主张），检索语义相关论文，构建相似性图进行细粒度概念级比较

Result: 模型能够产生校准的新颖性评分和类似人类的解释性评估，相对于现有方法减少了高估并提高了评估一致性

Conclusion: 该文献感知的新颖性评估框架通过结构化比较现有研究，能够提供更客观、一致的新颖性评估，减少主观偏差
...
TL;DR: 提出新的注释极化度量与统计检验方法，用于分析不同社会人口学群体间的标注差异，特别适用于主观性任务如仇恨言论检测。


<details>
  <summary>Details</summary>
Motivation: 现有标注一致性度量不适用于群体间分析，对群体规模不平衡敏感，且局限于单标注设置，无法满足毒性检测等主观任务的需求。

Method: 引入可量化度量指标，配合统计显著性检验，能够将极化归因于不同注释者群体，支持不平衡社会人口学和意识形态子群的直接比较，并适用于多标签设置。

Result: 应用于三个仇恨言论数据集和一个毒性检测数据集发现：1) 种族是仇恨言论任务中持续强烈的极化因素；2) 宗教注释者内部基本一致，但与其他群体存在分歧；3) 教育程度较低的注释者更主观，而教育程度高的注释者之间一致性更高。

Conclusion: 新度量方法能够有效揭示不同子群的标注模式，提供了获取稳健结果所需的最小注释者数量估计，并开源了Python实现库。
...
TL;DR: COVER提出一种新的并行扩散解码验证方法，通过KV缓存覆盖实现单前向传播内的留一验证和稳定草稿生成，减少不必要的修订并加速推理。


<details>
  <summary>Details</summary>
Motivation: 现有并行扩散解码方法中，可撤销解码的验证方案常导致"翻转振荡"问题：令牌被重新掩码后又恢复原状，这会削弱并行草稿生成的上下文条件，并消耗修订预算而进展甚微。

Method: COVER采用KV缓存覆盖技术：1) 构建两个注意力视图：选定种子令牌被掩码用于验证，同时将其缓存的键值状态注入所有其他查询以保留上下文信息；2) 使用封闭形式对角线校正防止种子位置的自泄漏；3) 基于稳定性感知评分优先选择种子，平衡不确定性、下游影响和缓存漂移；4) 自适应调整每步验证的种子数量。
...
TL;DR: 量化后训练会改变大语言模型的社会偏见模式，产生"量化诱导的掩蔽偏见翻转"现象，即21%的响应在偏见和无偏见状态之间翻转，而聚合指标无法捕捉这些变化。


<details>
  <summary>Details</summary>
Motivation: 研究量化对大型语言模型社会偏见的影响，发现现有聚合指标无法捕捉量化带来的偏见模式根本性变化，需要更细致的评估方法。

Method: 使用PostTrainingBiasBench基准测试，评估50个量化模型在13个封闭式和开放式偏见数据集上的表现，分析量化强度（4位vs8位）、模型不确定性等因素对偏见变化的影响。

Result: 发现量化诱导的掩蔽偏见翻转现象，21%的响应在偏见状态间翻转；高不确定性响应翻转概率是低不确定性的3-11倍；4位量化模型的行为变化是8位的4-6倍；偏见变化对不同人口群体产生不对称影响，某些群体偏见恶化18.6%，其他改善14.1%。

Conclusion: 压缩从根本上改变了偏见模式，需要关键的后量化评估和干预措施以确保实际应用的可靠性，大模型并未表现出一致的鲁棒性优势。
...
TL;DR: BenchMarker工具包使用LLM法官检测多选题基准中的三种常见缺陷：污染、捷径和写作错误，揭示了这些缺陷如何影响评估准确性，并发现现有修复方法可能引入新问题。


<details>
  <summary>Details</summary>
Motivation: 当前NLP领域的多选题问答基准缺乏严格的质量控制，存在各种缺陷，这些缺陷会降低评估的可靠性。需要一种系统化的方法来检测和修复这些基准中的问题。

Method: 开发BenchMarker工具包，采用教育研究启发的方法，使用LLM法官来检测三种常见缺陷：1) 污染（题目完全出现在网上）；2) 捷径（选项中的线索使猜测成为可能）；3) 写作错误（基于19条教育规则的语法结构问题）。通过人工标注验证工具的有效性，然后对12个基准进行审计。

Result: 1) 验证显示BenchMarker与人工标注一致；2) 污染题目倾向于提高准确率，而写作错误倾向于降低准确率并改变排名；3) 现有的基准修复方法虽然解决了目标问题（如使用LLM编写的干扰项降低准确率），但无意中引入了新缺陷（如不合理的干扰项、多个正确答案）。

Conclusion: 多选题中的缺陷会降低NLP评估的质量，但教育研究提供了改进的路径。BenchMarker工具包的发布旨在连接两个领域，改进多选题问答基准的设计。
...
TL;DR: 研究发现，通过仅提供单方面论据，可以简单直观地引导大语言模型在争议性问题中表达特定立场，这种观点引导在不同模型、论据数量和主题中都存在。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理争议性问题时，虽然可以提供平衡回答，但通常会采取单一对齐观点或拒绝回答。本研究旨在探索是否可以通过仅提供单方面论据来简单直观地引导LLM表达特定观点。

Method: 构建小型数据集，系统研究三个维度：(i) LLM回应中诱导的立场，(ii) 争议性问题的表述方式，(iii) 论据的呈现方式。测试不同模型、论据数量和主题下的观点引导效果。

Result: 研究发现观点引导在三个维度上都有效，且在不同模型、论据数量和主题中普遍存在。切换到其他论据会一致性地减少观点引导效果。

Conclusion: 仅提供单方面论据可以有效地引导大语言模型在争议性问题中表达特定立场，这表明LLM的观点表达容易受到输入论据的影响。
...
TL;DR: 该论文提出使用递归量化分析(RQA)作为分析推理模型测试时计算的非文本方法，通过分析隐藏嵌入的动态轨迹来量化推理链中的重复和停滞模式。


<details>
  <summary>Details</summary>
Motivation: 当前通过生成文本分析大型推理模型的推理行为变得越来越不切实际且不可靠。响应长度常被用作推理努力的粗略代理，但这种方法无法捕捉思维链(CoT)或生成token的动态和有效性。

Method: 将token生成视为动态系统，在每个生成步骤提取隐藏嵌入，并对得到的轨迹应用递归量化分析(RQA)。RQA指标如确定性和层流性量化了模型潜在表示中的重复和停滞模式。

Result: 通过分析DeepSeek-R1-Distill的3,600个生成轨迹，RQA不仅捕捉到了响应长度无法反映的信号，还将任务复杂度的预测提高了8%。

Conclusion: RQA为研究推理模型中测试时扩展的潜在token生成动态提供了一个原则性工具，能够更好地分析模型的推理链行为。
...
TL;DR: MPIB是一个医疗提示注入基准测试套件，用于评估LLM和RAG系统在临床环境中的安全性，通过临床伤害事件率(CHER)和攻击成功率(ASR)来衡量风险。
...
Method: 创建MPIB基准测试套件，包含9,697个经过多阶段质量控制和临床安全检查的实例，评估直接提示注入和间接RAG介导的注入，使用CHER和ASR指标。

Result: 评估发现ASR和CHER可能显著分歧，鲁棒性关键取决于对抗性指令出现在用户查询还是检索上下文中，攻击成功率与临床伤害风险不一定相关。

Conclusion: MPIB为临床提示注入研究提供了可重复的系统评估框架，强调需要区分指令遵从性和下游患者风险，以更好地评估临床安全性。
...
TL;DR: VowelPrompt：基于语言学原理的语音情感识别框架，通过提取元音级别的韵律特征并转换为自然语言描述，结合LLM进行联合推理，显著提升情感识别性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的情感识别主要依赖文本转录，忽略了语音中精细的韵律信息（如音高、能量、时长等），这限制了模型的性能和可解释性。需要一种能同时处理语言内容和韵律特征的方法。

Method: 提出VowelPrompt框架：1）基于语言学原理提取时间对齐的元音段落的音高、能量和时长特征；2）将这些特征转换为自然语言描述；3）采用两阶段适应方法：监督微调（SFT）后接基于可验证奖励的强化学习（RLVR），通过组相对策略优化（GRPO）实现，以增强推理能力、确保结构化输出并提升跨领域泛化能力。

Result: 在多个基准数据集上的广泛评估表明，VowelPrompt在零样本、微调、跨领域和跨语言条件下均优于最先进的情感识别方法，同时能够生成基于上下文语义和精细韵律结构的可解释解释。

Conclusion: VowelPrompt通过将精细的元音级别韵律特征与LLM的语义推理能力相结合，实现了更准确、更可解释的语音情感识别，为多模态情感分析提供了新的方向。
...
TL;DR: RoPE-LIME：一种用于解释闭源LLM输出的方法，通过使用开源代理模型和改进的扰动策略，减少API调用并提高归因质量


<details>
  <summary>Details</summary>
Motivation: 解释闭源LLM输出具有挑战性，因为API访问阻止了基于梯度的归因方法，而基于扰动的传统方法成本高且噪声大，特别是在依赖重新生成文本时

Method: 1. 将推理与解释解耦：使用较小的开源代理模型为固定闭源模型输出计算基于概率目标的token级归因；2. 引入RoPE嵌入空间中的松弛词移动距离作为局部性核函数；3. 提出Sparse-K采样，一种高效的扰动策略以提高有限预算下的交互覆盖

Result: 在HotpotQA（句子特征）和手工标注的MMLU子集（词特征）上的实验表明，RoPE-LIME比留一采样产生更具信息量的归因，优于gSMILE，同时显著减少闭源模型API调用

Conclusion: RoPE-LIME为闭源LLM提供了一种高效、准确的解释方法，通过解耦推理与解释、改进相似性度量和扰动策略，在减少计算成本的同时提高了归因质量
...
<details>
  <summary>Details</summary>
Motivation: 当前推理模型在生成研究级数学问题的解决方案方面取得进展，但验证仍然是瓶颈，消耗专家时间。假设有意义的解决方案应包含足够的方法级信息，当应用于相关问题时，应比错误解决方案产生更好的下游性能。

Method: 提出Consequence-Based Utility评估器，无需专家标注，通过测试每个候选解决方案作为上下文示例解决相关可验证问题的价值来评分。在原始研究级数学问题集上评估，每个问题配有一个专家解决方案和九个LLM生成的解决方案。

Result: Consequence-Based Utility在排名质量上一致优于奖励模型、生成奖励模型和LLM评判者。对于GPT-OSS-120B，将Acc@1从67.2提高到76.3，AUC从71.4提高到79.6；对于GPT-OSS-20B，AUC从69.0提高到79.2。相比LLM评判者，展现出更大的求解器-评估器差距，即使在底层求解器经常失败的实例上也能保持更强的正确-错误区分。

Conclusion: Consequence-Based Utility是一种有效的无专家标注评估方法，能够准确评估数学推理解决方案的质量，通过测试解决方案作为上下文示例解决相关问题的能力来验证其价值。
...
TL;DR: 本文提出针对口语代码转换的解析方法，包括新的评估指标FLEX-UD和解耦代理解析框架DECAP，显著提升解析性能


<details>
  <summary>Details</summary>
Motivation: 口语代码转换（CSW）对句法解析提出独特挑战，包括不流利、重复、省略和话语驱动结构等，这些现象违反标准通用依存（UD）假设，导致现有解析器和大型语言模型在口语数据上表现不佳。现有评估指标过于刚性，将合理的语言变体误判为错误。

Method: 1. 提出基于语言学的口语CSW现象分类法；2. 创建专家标注的黄金基准SpokeBench；3. 提出模糊感知评估指标FLEX-UD；4. 设计解耦代理解析框架DECAP，将口语现象处理与核心句法分析分离。

Result: DECAP框架在不重新训练的情况下产生更鲁棒和可解释的解析结果，相比现有解析技术提升高达52.6%。FLEX-UD评估揭示了标准指标掩盖的定性改进。

Conclusion: 口语代码转换解析需要专门的系统方法，包括语言学驱动的基准、灵活的评估指标和解耦的解析框架。DECAP和FLEX-UD为口语CSW解析提供了更准确和实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 因果推理对决策至关重要，但非专家难以掌握。大语言模型在该领域有潜力，但其精确因果估计能力有限，且后训练对这些能力的影响研究不足

Method: 引入CauGym数据集，包含7个核心因果任务用于训练和5个多样化测试集。系统评估5种后训练方法：SFT、DPO、KTO、PPO和GRPO

Result: 适当后训练使较小LLM在因果推理上表现优异，常超越更大模型。14B参数模型在CaLM基准上达到93.5%准确率，相比OpenAI o3的55.4%。后训练LLM在分布偏移和噪声数据等现实条件下表现出强泛化性和鲁棒性

Conclusion: 首次系统证明针对性后训练可产生可靠且鲁棒的基于LLM的因果推理器。数据和GRPO模型已开源
...
TL;DR: SHINE是一种可扩展的超网络，能够将多样化的有意义上下文映射为高质量LoRA适配器，通过单次前向传播将上下文知识转化为参数知识，无需微调即可更新LLM参数。


<details>
  <summary>Details</summary>
Motivation: 解决现有超网络在表达能力和可扩展性方面的限制，实现更高效地将上下文知识转化为LLM参数知识，减少传统基于监督微调（SFT）的LLM适应方法的时间、计算和内存成本。

Method: 采用可扩展的超网络设计，重用冻结LLM的参数进行上下文超网络构建，引入架构创新，通过预训练和指令微调流程训练超网络，使其能够从多样化上下文中单次前向传播生成高质量LoRA适配器。

Result: 在各种任务上取得优异结果，相比基于SFT的LLM适应方法显著节省时间、计算和内存成本，展示了良好的扩展潜力，能够在不直接访问上下文的情况下立即启用复杂的问答任务。

Conclusion: SHINE通过创新的超网络设计成功实现了将上下文知识高效转化为参数知识，为LLM适应提供了一种更高效、可扩展的解决方案，具有重要的实际应用价值。
...
TL;DR: 比较了两种文本分类范式：基于提示的LLM（零/少样本）与微调的编码器模型，发现微调模型在性能相当的情况下成本低1-2个数量级，延迟也更低。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在文本分类任务中常被盲目采用，仅关注预测性能而忽视生产环境中的操作约束（延迟、成本）。需要系统比较不同范式的实际部署效果。

Method: 在四个基准数据集（IMDB、SST-2、AG News、DBPedia）上比较零/少样本提示的LLM与微调的BERT类编码器模型，评估预测质量（宏F1）、推理延迟和货币成本，使用帕累托前沿和多目标决策分析。

Result: 微调编码器模型达到竞争性甚至更优的分类性能，同时成本低1-2个数量级，延迟也更低。LLM在标准文本分类任务中可能不是最优选择。

Conclusion: 不应盲目使用LLM进行标准文本分类，微调编码器是结构化NLP流程中更稳健高效的组件，LLM更适合作为混合架构中的补充元素。
...
TL;DR: ReBeCA框架通过因果分析揭示自反思的行为层级，发现仅有少数语义行为真正影响自反思效果，且更多行为不一定更好。
...
Result: 发现：1)语义行为层级影响自反思结果；2)仅少数语义行为具有泛化性；3)看似积极的语义行为组合反而可能损害自反思效果。ICP验证显示稀疏因果父节点带来49.6%结构似然提升。

Conclusion: ReBeCA提供了一种严谨方法，能够从自反思动态中分离真正的因果机制与虚假关联，为理解自反思提供了因果解释框架。
...
TL;DR: FMBench：一个评估大语言模型Markdown格式输出能力的基准，并提出结合监督微调和强化学习的轻量级对齐方法，以平衡语义意图和格式约束。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在用户界面和系统集成工作流中需要同时满足语义意图和格式约束。Markdown格式虽然普遍应用，但容易出现难以检测的细微错误（如损坏的列表、格式错误的表格、不一致的标题和无效的代码块），这些错误会严重影响下游可用性。

Method: 提出FMBench基准，用于评估模型在多样化指令遵循场景下的Markdown格式输出能力。同时提出轻量级对齐流程：首先在指令-响应对上进行监督微调（SFT），然后通过强化学习微调优化平衡语义保真度和结构正确性的复合目标。

Result: 在两个模型家族（OpenPangu和Qwen）上的实验表明，SFT能持续改善语义对齐，而强化学习在具有挑战性的Markdown指令上提供了额外的鲁棒性增益。结果揭示了语义目标和结构目标之间的固有权衡，强调了精心设计奖励对于可靠格式生成的重要性。

Conclusion: FMBench为评估和改进大语言模型的Markdown格式输出能力提供了有效的基准和方法。结合SFT和强化学习的对齐流程能有效平衡语义意图和格式约束，为部署到用户界面和系统集成工作流中的语言模型提供了实用解决方案。
...
TL;DR: SureLock通过锁定已稳定位置来减少掩码扩散语言模型的计算开销，将每次迭代计算成本从O(N²d)降至O(MNd)，在LLaDA-8B上减少30-50%FLOPs


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在生成序列时，即使许多未掩码位置已经稳定，仍会为每个位置重新计算注意力机制和前馈网络，造成大量计算浪费
...
Conclusion: SureLock通过动态锁定稳定位置显著减少掩码扩散语言模型的计算开销，理论分析表明仅监控锁定步骤的局部KL散度足以约束最终令牌概率的偏差
...
Method: 提出HOMER框架，基于GTVH幽默理论，采用多角色LLM协作：1）冲突脚本提取器：识别关键脚本对立；2）检索增强的层次化想象器：识别幽默目标并通过联想树扩展创意空间；3）字幕生成器：基于获得的知识生成幽默字幕。

Result: 在两个New Yorker Cartoon基准数据集上的实验表明，HOMER在多模态幽默字幕生成任务上优于最先进的基线方法和强大的LLM推理策略。

Conclusion: HOMER通过幽默理论驱动的多角色LLM协作框架，有效解决了多模态幽默生成的挑战，在创造性和可解释性方面表现优异。
...
TL;DR: 通过构建情感词语义网络，验证Plutchik情感轮模型的结构有效性，发现整体相似但局部存在差异


<details>
  <summary>Details</summary>
Motivation: Plutchik情感轮模型在NLP情感分析中被广泛使用，但其结构有效性尚未得到充分验证，需要实证检验该模型是否准确反映了人类情感的组织结构

Method: 收集情感词对的相似性和关联性数据，构建语义网络，通过社区检测分析网络结构，并与Plutchik情感轮模型进行比较

Result: 构建的网络结构与情感轮模型在整体上相似，但在局部存在差异，表明情感轮模型基本有效但需要局部修正

Conclusion: Plutchik情感轮模型在整体结构上具有有效性，但局部结构需要调整以更准确地反映人类情感的组织方式
...
TL;DR: 该论文提出了一种基于强化学习的历史感知越狱框架，通过分析并重新加权历史交互中的漏洞信号来指导攻击决策，显著提高了越狱成功率和查询效率。


<details>
  <summary>Details</summary>
Motivation: 现有越狱技术大多未能有效利用早期交互轮次中揭示的漏洞，导致攻击效率低下且不稳定。由于越狱涉及序列化交互，强化学习为此提供了自然框架，因此需要开发能够利用历史漏洞信号的强化学习方法。

Method: 提出历史感知的强化学习越狱框架，包含两个关键组件：1）基础框架利用历史信息改进越狱成功率；2）引入基于注意力的重新加权机制，突出交互历史中的关键漏洞，实现更高效的探索和更少的查询。

Result: 在AdvBench和HarmBench上的大量实验表明，该方法实现了最先进的越狱性能，同时显著提高了查询效率。仅利用历史信息就能提高越狱成功率，而注意力机制进一步增强了攻击效率。

Conclusion: 历史漏洞信号在强化学习驱动的越狱策略中至关重要，该方法为推进LLM安全防护的对抗研究提供了原则性途径，强调了利用历史信息进行高效越狱攻击的重要性。
...
TL;DR: CORE评估发现LLMs在语义推理中存在严重缺陷：虽然能识别相关关系（86.5-100%准确率），但在判断无关关系时表现极差（0-41.35%），且置信度虚高，存在系统性语义崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估很少测试其区分有意义语义关系和真正无关关系的能力，而这是语义推理和安全性的关键方面。

Method: 构建CORE数据集，包含225K多选题覆盖74个学科，以及一个203题的开放基准，涵盖24种语义关系类型，确保无关关系对等的代表性。评估29个SOTA LLMs，并与1000+参与者的人类基线对比。

Result: 人类基线准确率92.6%（无关关系95.1%），而LLMs整体准确率48.25-70.9%，在无关关系上表现严重恶化（0-41.35%），置信度却相似（92-94%）。无关关系上的校准误差增加2-4倍，语义崩溃率平均37.6%。在225K数据集上准确率降至约2%。

Conclusion: 无关关系推理是LLM评估和安全的关键前沿，当前模型存在系统性缺陷，倾向于生成虚假关系，需要专门评估和改进。
...
TL;DR: ClinMPO是一个强化学习框架，通过专业精神病学实践对齐LLM内部推理，使轻量级模型在复杂精神病学诊断任务上超越医学生表现


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在精神病学应用中存在幻觉和浅层推理问题，特别是轻量级模型在隐私保护和临床部署中至关重要，但现有训练范式过于注重语言流畅性而非结构化临床逻辑，与专业诊断认知存在根本性不匹配

Method: 引入ClinMPO强化学习框架，使用基于4,474篇精神病学期刊文章构建的专业数据集训练专门奖励模型，依据循证医学原则结构化数据，将LLM内部推理与专业精神病学实践对齐

Result: 在专门设计用于隔离推理能力而非机械记忆的测试集上，ClinMPO调优的Qwen3-8B模型达到31.4%的诊断准确率，超越了300名医学生30.8%的基准表现，特别是在大型模型也经常失败的复杂病例上
...
TL;DR: RelayGen：无需训练的推理模型动态切换框架，通过分析生成不确定性识别难度变化，将低难度片段委派给小模型处理，在保持大模型准确性的同时显著降低推理延迟
...
Method: 提出RelayGen框架：1）通过token概率边际分析生成不确定性；2）识别模型特定的切换线索，检测推理轨迹中难度变化；3）动态将低难度片段委派给小模型继续生成，高难度推理仍由大模型处理；4）无需额外训练或学习路由组件

Result: 在多个推理基准测试中，RelayGen显著降低推理延迟，同时保持大模型大部分准确性。结合推测解码技术，实现最高2.2倍端到端加速，准确度下降小于2%

Conclusion: RelayGen证明了利用推理轨迹中难度变化的粗粒度段级控制是有效的，无需复杂监督路由即可实现高效推理，为大型推理模型的部署提供了实用的效率优化方案
...
TL;DR: DiSPO是一种用于掩码扩散语言模型的信用分配方法，通过优化中间填充决策来改进生成质量，无需额外的多步扩散展开。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型通过多步去噪迭代填充掩码标记，但仅基于最终完成的终端奖励学习会导致对中间决策的信用分配粗糙，影响生成质量。

Method: DiSPO在选定的中间掩码状态进行分支：从缓存的对数概率中重新采样当前掩码位置的填充，对生成的完成进行评分，并仅更新新填充的标记，无需额外的多步扩散展开。该方法形式化了分支完成的固定状态目标，并推导出可与终端反馈策略优化结合的策略梯度估计器。

Result: 在LLaDA-8B-Instruct模型上，DiSPO在匹配的展开计算和优化器步骤下，在数学和规划基准测试中持续优于终端反馈的diffu-GRPO基线。

Conclusion: DiSPO作为一种插件式信用分配层，通过直接优化中间填充决策，有效改进了掩码扩散语言模型的生成质量，为扩散模型的强化学习提供了更精细的信用分配机制。
...
TL;DR: UNO框架利用用户日志优化LLM系统，通过将日志转化为结构化规则和偏好对，进行聚类分析并量化认知差距，自适应过滤噪声反馈，显著提升效果和效率。


<details>
  <summary>Details</summary>
Motivation: 传统LLM依赖大规模高质量数据和参数扩展，但面临数据稀缺和计算成本回报递减的问题。用户交互日志作为真实人类反馈和程序性知识的丰富来源，但因其非结构化和噪声特性难以有效利用。

Method: UNO框架：1) 将用户日志蒸馏为半结构化规则和偏好对；2) 使用查询-反馈驱动聚类管理数据异质性；3) 量化模型先验知识与日志数据间的认知差距；4) 自适应过滤噪声反馈，构建主体验和反思体验模块。

Result: 实验表明UNO在效果和效率上达到最先进水平，显著优于检索增强生成(RAG)和基于记忆的基线方法。

Conclusion: UNO为从用户日志中学习提供了统一框架，有效解决了日志噪声和离策略优化问题，为LLM系统的持续改进提供了实用解决方案。
...
TL;DR: 本文挑战了Transformer中传统的窄-宽-窄MLP设计，提出使用更深的沙漏形FFN（宽-窄-宽结构）作为替代，在固定参数预算下通过减少FFN参数来扩大隐藏维度，实现了更好的性能表现。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer架构中，每个层都采用注意力模块后接窄-宽-窄MLP的FFN设计，大部分参数分配给MLP且扩展比在2-4之间。最近研究表明残差宽-窄-宽（沙漏形）MLP具有更优的函数逼近能力，因此作者重新审视了Transformer中长期存在的MLP形状惯例，质疑窄-宽-窄设计的必要性。

Method: 开发了一种Transformer变体，用更深的沙漏形FFN替代传统的FFN。该沙漏形FFN由多个通过残差连接堆叠的沙漏形子MLP组成。作者假设更轻量但更深的沙漏形FFN可以作为传统FFN的有竞争力替代方案，并且通过使用更轻量的沙漏形FFN节省的参数可以更有效地利用，例如在固定预算下扩大模型的隐藏维度。

Result: 实证验证表明：在400M参数规模内，沙漏形FFN优于传统FFN；在1B参数规模上，两者性能相当。在匹配预算下，减少FFN参数并增加注意力参数的沙漏形FFN变体相比传统配置显示出持续改进。

Conclusion: 这些发现为近期研究提供了新视角，促使重新思考窄-宽-窄MLP惯例以及注意力与FFN之间的平衡，有助于构建更高效和表达能力更强的现代语言模型。
...
TL;DR: DREAM是一个基于多轮辩论的LLM代理相关性评估框架，通过对立立场和迭代批判提高标注准确性，仅需3.5%人工参与达到95.2%准确率。基于此构建的BRIDGE基准发现了29,824个缺失相关块，缓解评估偏差。


<details>
  <summary>Details</summary>
Motivation: 现有IR评估面临挑战，因为基准数据集包含未标注的相关块。虽然LLM和LLM-人类混合策略减少了人工成本，但仍存在LLM过度自信和AI到人类升级无效的问题。

Method: 提出DREAM框架：基于对立初始立场和迭代相互批判的多轮辩论式相关性评估方法。通过基于共识的辩论，对确定案例产生更准确标注，对不确定案例实现更可靠的AI到人类升级。

Result: 达到95.2%的标注准确率，仅需3.5%人工参与。构建BRIDGE基准，发现29,824个缺失相关块，缓解评估偏差，实现更公平的检索器比较。重新评估IR系统并扩展到RAG评估。

Conclusion: DREAM框架有效解决了IR评估中的标注完整性问题，未解决的评估漏洞不仅扭曲检索器排名，还会导致检索-生成错位。该框架和数据集已开源。
...
TL;DR: 首个公开的英-希机器翻译质量评估基准MTQE.en-he发布，包含959个英语片段及其希伯来语机器翻译，由三位专家标注质量分数。实验显示集成ChatGPT、TransQuest和CometKiwi三种模型效果最佳，比单一最佳模型提升6.4个百分点。参数高效微调方法稳定有效，提升2-3个百分点。


<details>
  <summary>Details</summary>
Motivation: 为希伯来语这一资源稀缺语言对建立首个公开的机器翻译质量评估基准，填补该领域空白，促进未来研究发展。

Method: 构建MTQE.en-he数据集（来自WMT24++的959个英语片段及其希伯来语机器翻译，三位专家标注直接评估分数）。基准测试三种方法：ChatGPT提示、TransQuest、CometKiwi，并进行模型集成。进行微调实验，比较全模型更新与参数高效方法（LoRA、BitFit、FTHead）。

Result: 模型集成（ChatGPT+TransQuest+CometKiwi）比单一最佳模型CometKiwi提升6.4个Pearson百分点和5.6个Spearman百分点。参数高效微调方法训练稳定，提升2-3个百分点，而全模型更新容易过拟合和分布崩溃。

Conclusion: MTQE.en-he基准数据集和实验结果将为英-希这一资源稀缺语言对的机器翻译质量评估研究提供重要基础，参数高效微调是有效的改进方法。
...
TL;DR: Baichuan-M3是一个医疗增强的大型语言模型，将被动问答转变为主动的临床级决策支持，通过专门训练流程模拟医生工作流程，在多个医疗基准测试中超越GPT-5.2


<details>
  <summary>Details</summary>
Motivation: 解决现有系统在开放式医疗咨询中的局限性，从被动问答转向主动的临床决策支持，提升医疗AI的实用性和可靠性

Method: 采用专门训练流程模拟医生系统工作流程，包括主动信息获取解决歧义、长程推理整合分散证据、自适应幻觉抑制确保事实可靠性

Result: 在HealthBench、新引入的HealthBench-Hallu和ScanBench上取得最先进结果，在临床咨询、建议和安全性方面显著超越GPT-5.2

Conclusion: Baichuan-M3成功实现了从被动问答到主动临床决策支持的范式转变，模型已在HuggingFace公开可用，为医疗AI应用提供了可靠工具
...
TL;DR: 提出Inference-Time Rethinking框架，通过将推理分解为潜在思维向量和语言化解码器，实现迭代式自我修正，在小型模型上超越大参数模型


<details>
  <summary>Details</summary>
Motivation: 标准思维链推理在单次前向传播中生成解决方案，一旦出现早期错误就无法修正，缺乏自我纠错机制

Method: 将推理分解为连续潜在思维向量（推理内容）和条件解码器（如何推理），采用Gibbs式交替过程：生成候选推理轨迹，优化潜在向量以更好解释该轨迹

Result: 在GSM8K上训练的0.2B参数模型，经过30次重新思考迭代后，超越了参数多10-15倍的基线模型，包括3B参数模型

Conclusion: 有效的数学推理可以通过复杂的推理时计算实现，而不仅仅依赖于大规模参数数量，展示了推理过程优化的重要性
...
TL;DR: 论文提出利用大推理模型中的"提示回响"现象作为计算分配机制，通过理论分析和两种方法（ED-SFT和EP）来提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么注入任务无关的标记，要么使用启发式方法，忽略了模型内部链中自发重复提示的现象。本文旨在分析和利用这种"提示回响"现象作为前置计算分配机制。

Method: 1. 理论分析：将回响移除视为基于拒绝的条件化，定义"回响似然差距"作为可计算的代理指标；2. 提出两种方法：Echo-Distilled SFT（通过监督微调建立"回响-推理"模式）和Echoic Prompting（无需训练，在推理过程中重新锚定模型）。

Result: 在GSM8K、MathQA、Hendrycks-MATH、AIME24和MATH-500等数据集上，在相同的解码设置和计算预算下，相比基线方法获得了持续的性能提升。

Conclusion: 提示回响现象可以作为有效的计算分配机制，通过理论分析和相应方法能够提升大推理模型的性能，且这种提升与注意力重新聚焦机制相关。
...
TL;DR: 提出一种针对LLM隐藏毒性模式的目标子空间干预策略，在保持生成流畅性的同时有效降低毒性


<details>
  <summary>Details</summary>
Motivation: LLM即使接收无害提示也可能生成有毒内容，现有方法面临安全性与流畅性的权衡，且毒性检测困难

Method: 通过识别和抑制底层模型表示中的隐藏毒性模式，采用目标子空间干预策略

Result: 在RealToxicityPrompts上优于现有基线，将最先进解毒系统的毒性降低8-20%，同时保持流畅性

Conclusion: 该方法能有效降低毒性而不损害生成性能，在多个LLM上一致优于现有基线
...
TL;DR: FairJudge是一个自适应、去偏、一致的LLM-as-a-Judge系统，通过将评判行为建模为可学习的正则化策略，解决现有系统在适应性、偏见和一致性方面的局限性。
...
Conclusion: FairJudge通过将评判行为建模为可学习的正则化策略，成功解决了现有LLM-as-a-Judge系统的适应性、偏见和一致性问题，为未来研究提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 口语内容（如在线视频和播客）通常涉及多个主题，需要自动主题分割来辅助用户导航和下游应用。现有方法未能充分利用音频特征，存在改进空间。
...
Conclusion: 学习到的声学特征对于鲁棒的主题分割具有重要价值，多模态方法能有效提升主题分割性能。
...
TL;DR: PACT框架通过分层安全策略和结构化推理路径，实现LLM的动态安全控制，缓解安全性与实用性之间的权衡问题。
...
Method: 提出PACT框架，采用分层策略架构：不可覆盖的全局安全策略设定关键风险的不可变边界，用户定义策略可引入领域特定风险类别并指定标签到行为的映射。框架将安全决策分解为结构化的分类→行动路径，通过风险感知推理实现透明决策。

Result: 实验表明PACT在全局策略评估下达到接近最先进的安全性能，在用户特定策略评估下获得最佳可控性，有效缓解了安全性与实用性之间的权衡。

Conclusion: PACT通过动态安全控制和分层策略架构，为可控安全对齐提供了可复现的研究框架，将发布模型套件、训练数据和评估协议。
...
<details>
  <summary>Details</summary>
Motivation: 指令微调虽然提升了LLMs的指令遵循能力和帮助性，但常常导致生成多样性下降（模式崩溃），在开放场景中产生重复输出。研究发现LLM的不同层承担不同功能角色，因此假设模式崩溃可定位到特定层，通过恢复这些层到预训练权重可恢复多样性。

Method: 1. 设计代理任务CRC（约束随机字符），具有明确的有效性集和自然多样性目标；2. 通过CRC实验确定层恢复的最佳范围；3. 提出选择性层恢复(SLR)方法，无需训练，将微调后模型中的选定层恢复到预训练权重，形成混合模型；4. 在三个任务（创意写作、开放问答、多步推理）和三个模型家族（Llama、Qwen、Gemma）上验证。

Result: CRC实验揭示了多样性-有效性权衡关系，并找到了能增加多样性且质量损失最小的层配置。SLR方法在多个任务和模型上一致且显著地提升了输出多样性，同时保持了高质量输出。

Conclusion: 模式崩溃可定位到LLM的特定层，通过选择性层恢复方法可有效解决指令微调导致的多样性下降问题，在保持模型架构和参数数量不变、不增加推理成本的情况下，实现多样性与质量的平衡。
...
TL;DR: 法国政府开发的compar:IA平台收集法语为主的AI偏好数据，已获60万提示和25万投票，89%为法语，旨在解决非英语LLM性能和文化对齐问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在非英语语言中表现不佳，主要因为训练数据和人类偏好数据以英语为主。RLHF和DPO等方法需要人类偏好数据，但英语以外的语言缺乏公开的大规模数据集。

Method: 开发compar:IA开源数字公共服务平台，采用盲对比界面收集用户对语言模型的偏好数据。平台降低参与门槛，使用隐私保护的自动过滤机制，收集自由形式的提示和用户判断。

Result: 截至2026年2月7日，收集了超过60万个自由形式提示和25万个偏好投票，其中约89%为法语数据。发布了三个互补数据集（对话、投票、反应），并提供了法语模型排行榜和用户交互模式分析。

Conclusion: compar:IA不仅服务于法语环境，正在发展成为国际数字公共产品，为多语言模型训练、评估和人机交互研究提供可重用基础设施，填补非英语人类偏好数据的空白。
...
TL;DR: 研究探索如何通过提示工程控制LLM生成文本的情感，发现Few-Shot提示最有效，为情感自适应AI提供实用方案


<details>
  <summary>Details</summary>
Motivation: 大型语言模型为情感自适应AI提供了新机会，但精确控制生成文本的情感仍然具有挑战性。现有方法如微调成本高且需要大量数据，需要寻找更资源友好且易于访问的替代方案。

Method: 使用Ekman的六种基本情感（如喜悦、厌恶），研究多种提示工程技术，包括零样本提示和思维链提示（使用GPT-3.5-turbo），并与微调方法进行比较。

Result: 提示工程能有效引导AI生成文本中的情感，为微调提供了实用且成本效益高的替代方案，特别适用于数据受限的环境。其中，使用人工编写示例的Few-Shot提示在所有技术中最为有效。

Conclusion: 提示工程是控制LLM生成文本情感的有效方法，Few-Shot提示尤其有效，这为开发情感自适应AI系统提供了有价值的见解和实用途径。
...
TL;DR: TaS将信息搜索任务重构为表格完成任务，通过结构化表格管理搜索状态，显著提升长视野搜索的聚焦性和鲁棒性


<details>
  <summary>Details</summary>
Motivation: 当前信息搜索代理在长视野探索中难以保持聚焦和连贯性，因为在一个纯文本上下文中跟踪搜索状态（包括规划过程和大量搜索结果）本质上是脆弱的

Method: 提出Table-as-Search框架，将查询映射到外部数据库中的结构化表格模式，行代表搜索候选，列表示约束或所需信息，已填充单元格记录历史，空单元格作为显式搜索计划

Result: TaS在三种基准测试中显著优于多个最先进的基线方法，包括多智能体框架和商业系统，验证了其在长视野信息搜索中的优越鲁棒性、效率、可扩展性和灵活性

Conclusion: TaS通过结构化表格表示统一了深度搜索、广度搜索和深度广度搜索任务，为信息搜索代理提供了一种鲁棒、可扩展的解决方案
...
TL;DR: 本文提出R-Align方法，通过监督推理一致性来提升生成式奖励模型在RLHF中的表现，减少虚假正确性，改善下游策略性能。


<details>
  <summary>Details</summary>
Motivation: 虽然生成式奖励模型（GenRMs）通过生成推理过程来预测偏好，但在训练和评估中仍只关注结果标签，忽略了推理质量。研究发现推理保真度（模型偏好决策与参考决策推理之间的一致性）对下游RLHF结果有重要预测价值。

Method: 提出Rationale-Centric Alignment (R-Align)方法：1) 重新定义虚假正确性(S-Corr)指标，衡量标签正确但推理与黄金判断不一致的决策比例；2) 在训练中加入黄金判断，并显式监督推理对齐；3) 通过增强推理一致性来提升模型性能。

Result: 实证评估显示：1) 即使是竞争性GenRMs也存在显著S-Corr；2) 高S-Corr与优化过程中的策略退化相关；3) R-Align在奖励模型基准上减少S-Corr，并在STEM、编程、指令遵循和通用任务上带来一致的性能提升。

Conclusion: 推理保真度是预测RLHF结果的关键因素，R-Align通过关注推理一致性而非仅标签准确性，有效提升了生成式奖励模型在下游对齐任务中的表现。
...
Method: 采用数据驱动方法自动构建细粒度推理错误分类法（rubrics），利用这些分类法增强LLM驱动的错误检测，并将其转化为强化学习奖励函数用于模型训练。

Result: 基于错误分类法的分类方法在编程、数学、化学工程等技术领域表现出优于基线方法的错误识别能力；使用这些奖励函数训练的模型在困难领域任务准确率比通用LLM-as-judge训练模型提高+45%，仅需20%黄金标签即可接近使用可验证奖励训练模型的性能。
...
Method: 开发可解释的视觉词义消歧框架，利用CLIP将歧义语言和候选图像投影到共享多模态空间。通过包含WordNet同义词的语义和照片提示双通道集成丰富文本嵌入，同时通过鲁棒的测试时增强优化图像嵌入，最后使用余弦相似度确定最佳匹配图像。

Result: 在SemEval-2023 VWSD数据集上，增强嵌入将MRR从0.7227提升到0.7590，命中率从0.5810提升到0.6220。消融研究表明双通道提示提供强大低延迟性能，而激进的图像增强仅带来边际收益。

Conclusion: 精确的CLIP对齐提示对于视觉词义消歧最为有效，嘈杂的外部信号会稀释语义特异性。双通道提示策略在性能和效率之间取得了良好平衡。
...
TL;DR: 该研究发现概念表征通过几何关系而非概念本身实现共享，数字表征在不同任务中保持稳定的关系结构，任务特定表征位于不同子空间但可通过线性映射相互转换。
...
Method: 使用数字概念作为测试平台，语言模型作为高维计算基底，分析数字表征在不同任务中的几何关系结构，研究任务特定表征的子空间分布及其线性映射关系。

Result: 数字表征在不同任务中保持稳定的关系结构；任务特定表征嵌入不同的子空间，低级特征如大小和奇偶性编码在可分离的线性方向上；这些子空间主要通过线性映射相互转换，表明表征共享关系结构。

Conclusion: 语言模型通过将任务特定变换应用于概念表征的共享底层关系结构，平衡了数字表征的共享结构与功能灵活性，这为理解概念表征的机制提供了新视角。
...
TL;DR: SEMA是一种用于多轮越狱攻击的简单有效框架，通过预填充自调优和意图漂移感知的强化学习训练攻击者，在多个数据集和受害者模型上实现了最先进的攻击成功率。
...
Method: SEMA包含两个阶段：1）预填充自调优，通过微调非拒绝、结构良好的多轮对抗提示来稳定学习；2）意图漂移感知的强化学习，训练攻击者生成有效的多轮对抗提示，同时保持有害目标不变。

Result: 在多个数据集、受害者模型和越狱评估中，SEMA实现了最先进的攻击成功率（ASR），在AdvBench上平均达到80.1%的ASR@1，比现有最佳方法高出33.9%。
...
TL;DR: 该论文研究了多目标对齐中LLM的跨目标干扰问题，提出了基于协方差分析的理论框架和CTWA方法来缓解干扰，并提供了全局收敛性分析。


<details>
  <summary>Details</summary>
Motivation: 多目标对齐训练中，提升某些目标性能时会导致其他目标性能下降，这种跨目标干扰现象是LLM对齐中的持续性问题，需要系统研究和解决方案。

Method: 1) 形式化跨目标干扰现象；2) 推导局部协方差定律，分析目标改进条件；3) 扩展到现代对齐中的裁剪代理目标；4) 提出CTWA方法维持正协方差；5) 在Polyak-Łojasiewicz条件下进行全局收敛分析。

Result: 1) 跨目标干扰普遍存在且具有强模型依赖性；2) 协方差定律有效解释了干扰机制；3) CTWA方法能有效缓解跨目标干扰；4) 建立了非凸标量化优化的全局收敛条件。

Conclusion: 跨目标干扰是多目标对齐中的核心问题，基于协方差分析的理论框架提供了深入理解，CTWA方法为缓解干扰提供了有效工具，全局收敛分析为优化过程提供了理论保证。
...
TL;DR: Halluverse-M³是一个多语言、多任务、多幻觉类型的数据集，用于系统分析大语言模型的幻觉问题，涵盖英语、阿拉伯语、印地语和土耳其语四种语言，支持问答和对话摘要两种生成任务，区分实体级、关系级和句子级幻觉。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多语言和生成式场景中的幻觉问题仍然是一个持续挑战，特别是在保持事实一致性方面。虽然现有模型在英语基准测试中表现良好，但它们在跨语言、跨任务和不同幻觉类型上的行为尚未得到充分理解。

Method: 构建Halluverse-M³数据集，涵盖四种语言（英语、阿拉伯语、印地语、土耳其语）和两种生成任务（问答、对话摘要），明确区分实体级、关系级和句子级幻觉。通过受控编辑过程构建幻觉输出，并由人工标注者验证，确保原始内容与幻觉生成之间的清晰对应关系。

Result: 评估结果显示：问答任务比对话摘要更容易；句子级幻觉即使对最强模型也具有挑战性；英语表现最佳，低资源语言性能下降，印地语的检测准确率最低。

Conclusion: Halluverse-M³为研究多语言、多任务环境中的幻觉问题提供了一个现实且具有挑战性的基准，数据集已公开发布以支持未来的幻觉检测和缓解研究。
...
TL;DR: 土耳其语等形态丰富语言中，首个系统性研究子词分词方法的综合框架，通过控制词汇量、训练语料规模和分词器类型，建立形态感知评估工具包，为MRLs提供可操作的分词器构建指南。


<details>
  <summary>Details</summary>
Motivation: 土耳其语等形态丰富语言中，高产的粘着现象对词汇效率和形态保真度构成挑战。先前研究存在局限性：词汇变化缺乏系统控制、内在诊断有限、下游任务评估范围窄。

Method: 提出"子词宣言"框架，联合变化词汇量和分词器训练语料规模，在匹配参数预算下比较多种分词器家族（WordPiece、形态级别、字符基线），在语义、句法和形态敏感任务上进行评估，并引入形态感知诊断工具包。

Result: 建立了词汇-语料-成功三元组的系统性调查，创建了统一形态感知评估框架，识别了字符级和形态级分词的优势场景，并开源了评估代码、分词器流水线和模型。

Conclusion: 该研究为形态丰富语言构建有效分词器提供了可操作指导，建立了可复现的研究基础，是首个此类综合性工作，对MRLs分词研究具有重要意义。
...
TL;DR: DAWN是一种无需训练、依赖感知的解码方法，用于加速扩散大语言模型推理，通过提取token依赖关系并选择更可靠的解掩码位置，在保持生成质量的同时实现1.80-8.06倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型推理方案受限于质量-速度权衡，采用保守的并行策略，未能充分利用效率潜力。核心挑战在于并行解码假设每个位置可以独立填充，但token之间存在语义耦合，忽略这些依赖关系会导致输出质量下降。

Method: 提出DAWN方法：1) 提取token依赖关系构建依赖图；2) 基于两个关键观察：依赖确定位置的token更可靠，同时解掩码强耦合的不确定位置会引发错误；3) 利用依赖图在每次迭代中选择更可靠的解掩码位置，实现高并行度。

Result: 在多个模型和数据集上的实验表明，DAWN相比基线方法实现了1.80-8.06倍的推理加速，同时保持了生成质量。该方法无需额外训练，可直接应用于现有模型。

Conclusion: DAWN通过建模token间依赖关系，解决了扩散大语言模型并行解码中的质量-速度权衡问题，实现了高效且高质量的文本生成，为扩散模型推理提供了新的解决方案。
...
TL;DR: InftyThink+ 是一个强化学习框架，通过优化迭代推理轨迹来解决长链思维推理的效率和效果问题，显著提升推理精度并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 当前大规模推理模型通过扩展推理时链式思维获得强大性能，但存在二次成本、上下文长度限制和"中间迷失"效应导致推理退化的问题。现有的迭代推理方法依赖监督学习或固定启发式规则，无法优化何时总结、保留什么以及如何恢复推理。

Method: 提出 InftyThink+ 端到端强化学习框架，基于模型控制的迭代边界和显式总结来优化整个迭代推理轨迹。采用两阶段训练方案：监督冷启动后接轨迹级强化学习，使模型学习战略性的总结和继续决策。

Result: 在 DeepSeek-R1-Distill-Qwen-1.5B 上的实验显示，InftyThink+ 在 AIME24 上准确率提升 21%，明显优于传统长链思维强化学习方法，在分布外基准测试上泛化能力更强。同时显著降低推理延迟并加速强化学习训练。

Conclusion: InftyThink+ 通过强化学习优化迭代推理，在提升推理性能的同时提高了效率，为解决长链推理问题提供了有效方案。
...
TL;DR: Jackpot框架通过最优预算拒绝采样减少rollout模型与策略分布不匹配，实现RL for LLMs的高效解耦训练


<details>
  <summary>Details</summary>
Motivation: LLM的强化学习成本高昂，主要因为rollout生成昂贵。解耦rollout生成与策略优化可提高效率，但会引入严重的分布不匹配问题，导致学习不稳定。

Method: 提出Jackpot框架，采用最优预算拒绝采样直接减少rollout模型与演化策略之间的差异。包含原则性OBRS程序、联合更新策略和rollout模型的统一训练目标，以及基于top-k概率估计和批次级偏差校正的高效系统实现。

Result: 理论分析表明OBRS在可控接受预算下持续将rollout分布移向目标分布。实验显示相比重要性采样基线显著提高训练稳定性，在Qwen3-8B-Base上训练300步（批次大小64）时达到与on-policy RL相当的性能。

Conclusion: OBRS-based alignment使RL for LLMs的rollout生成与策略优化解耦更接近实用有效，为解决分布不匹配问题提供了可行方案。
...
TL;DR: 该论文首次对LLM推理失败进行全面调查，提出了新的分类框架，将推理分为具身与非具身类型，并将推理失败分为基础架构缺陷、应用特定限制和鲁棒性问题三类，为理解LLM系统性弱点提供了结构化视角。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM展现出卓越的推理能力，但在看似简单的场景中仍存在显著的推理失败。目前缺乏对这些失败的系统性理解，研究较为分散，需要统一的框架来分类、分析和缓解这些推理缺陷。

Method: 提出了新颖的分类框架：将推理分为具身推理和非具身推理（后者进一步分为非正式/直觉推理和正式/逻辑推理）。同时将推理失败分为三类：基础架构固有缺陷、应用特定限制、鲁棒性问题。对每种失败提供定义、分析现有研究、探索根本原因并提出缓解策略。

Result: 创建了首个全面的LLM推理失败调查，提供了结构化视角来理解LLM系统性弱点。发布了GitHub仓库收集相关研究工作，为该领域提供了便捷的入口点。

Conclusion: 该调查统一了分散的研究工作，为理解LLM推理失败提供了系统框架，有助于指导未来研究，构建更强、更可靠、更鲁棒的推理能力。
...
TL;DR: 提出基于LTLfMT的MDP非马尔可夫奖励逻辑规范框架，解决大状态空间下复杂任务的表达问题，结合奖励机器和HER方法处理奖励稀疏性


<details>
  <summary>Details</summary>
Motivation: 传统马尔可夫决策过程在处理非结构化异构数据域的复杂任务时表达能力有限，需要手动编码谓词，缺乏统一可重用的规范框架

Method: 1) 使用LTLfMT（线性时序逻辑模理论）增强表达能力；2) 识别理论可处理的LTLfMT片段；3) 结合奖励机器和事后经验回放(HER)方法处理奖励稀疏性

Result: 在连续控制环境中使用非线性算术理论评估，实验结果表明定制的HER实现对于解决复杂目标任务至关重要，能够自然地规范复杂任务

Conclusion: 提出的LTLfMT框架为无限状态空间下的复杂任务奖励规范提供了统一、可重用且表达力强的解决方案，通过理论分析和实践方法解决了相关计算挑战
...
TL;DR: 该研究检验LLM是否作为理性效用最大化者，在医疗诊断等高风险决策中具有一致的信念和稳定的偏好，通过可证伪条件评估其报告概率与真实信念的对应关系。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地部署在需要根据世界不确定性和不同结果效用做出最优决策的高风险领域，但其决策逻辑难以解释。需要研究LLM是否具有理性效用最大化者的特征，即具有一致的信念和稳定的偏好。

Method: 考虑模型在诊断挑战问题中的行为，通过可证伪条件评估报告概率与真实信念的对应关系，将方法应用于多个医疗诊断领域，并跨多个LLM进行评估。

Result: 研究结果为LLM推理与理想贝叶斯效用最大化的关系提供了见解，包括报告概率与真实信念的对应性，以及在不同医疗诊断领域和多个LLM中的评估结果。

Conclusion: 研究讨论了结果对LLM在高风险决策中应用的启示，并提出了未来发展方向，强调了理解LLM决策逻辑的重要性。
...
TL;DR: GrAlgoBench是一个用于评估大型推理模型的图算法基准测试，通过9个任务揭示了当前模型在长上下文推理中的两大弱点：随着节点数增加准确率急剧下降，以及过度思考导致的无效自我验证。


<details>
  <summary>Details</summary>
Motivation: 现有数学、代码和常识推理基准存在局限性：缺乏长上下文评估、挑战性不足、答案难以程序化验证。需要一个新的基准来全面评估大型推理模型的推理能力。

Method: 提出GrAlgoBench基准，使用图算法问题评估推理能力。图算法问题需要长上下文推理、可精细控制难度、支持标准化程序化评估。包含9个任务进行系统实验。

Result: 实验发现两大主要弱点：1）随着上下文长度增加（图节点超过120个），准确率急剧下降至50%以下，主要由执行错误、弱记忆和冗余推理导致；2）存在过度思考现象，大量无效的自我验证增加了推理痕迹但未提高正确性。

Conclusion: GrAlgoBench通过揭示当前大型推理模型的局限性，确立了图算法问题作为严谨、多维且实际相关的测试平台，有助于推进推理模型的研究。代码已开源。
...
TL;DR: Trifuse是一个基于注意力的GUI grounding框架，通过整合注意力机制、OCR文本线索和图标级语义，无需任务特定微调即可实现精确的界面元素定位。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding方法主要依赖大规模数据集微调MLLMs来预测坐标，这种方法数据密集且对未见界面的泛化能力差。基于注意力的替代方案虽然无需微调，但缺乏明确的空间锚点导致可靠性低。

Method: Trifuse框架显式整合互补的空间锚点，通过注意力机制、OCR提取的文本线索和图标级语义描述，采用Consensus-SinglePeak融合策略，在保持跨模态一致性的同时保留尖锐的定位峰值。

Result: 在四个grounding基准测试中，Trifuse无需任务特定微调即取得强劲性能，显著减少对昂贵标注数据的依赖。消融研究表明，整合OCR和语义线索能持续提升不同骨干网络的性能。

Conclusion: Trifuse通过整合互补的空间锚点，为GUI grounding提供了一个有效的通用框架，在减少数据依赖的同时提高了定位的可靠性和泛化能力。
...
TL;DR: DEPO通过在线难度估计器动态筛选训练数据，优先处理高学习潜力样本，将推理对齐的rollout成本降低2倍而不影响性能。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法在问题过于简单或复杂时会出现梯度信号衰减，而DAPO等变体虽然解决了梯度消失问题，但无法缓解低效用样本上大量rollout带来的计算开销。
...
TL;DR: QA-Token是一种质量感知的分词方法，通过考虑数据可靠性来优化词汇表构建，在基因组学和金融领域显著提升下游任务性能


<details>
  <summary>Details</summary>
Motivation: 当前的分词方法处理序列数据时不考虑信号质量，限制了它们在现实世界嘈杂语料库上的有效性。需要一种能够将数据可靠性直接纳入词汇表构建的方法。
...
Conclusion: QA-Token解锁了现实世界嘈杂语料库（包括petabases级别的基因组序列和terabytes级别的金融时间序列）用于基础模型训练，且推理时无额外开销。
...
TL;DR: 论文提出自回归生成存在内在稳定性极限，导致大语言模型在长程任务中性能急剧下降，这源于过程级不稳定性而非单纯任务复杂度，需要离散分段和图状执行结构来解决。


<details>
  <summary>Details</summary>
Motivation: 传统解释将LLMs在长程任务中的性能下降归因于任务复杂度（如组合搜索爆炸或长期信用分配），但作者认为这些解释不完整。即使在无分支、无语义模糊的线性任务中，自回归执行也存在内在稳定性极限，需要从结构治理角度重新理解长程推理问题。

Method: 提出理论分析框架，推导定理A证明单路径自回归推理中的决策优势随执行长度呈指数衰减，为可维持的推理链设定了基本界限。通过合成环境和真实TextWorld任务的实证研究验证理论预测。

Result: 实证研究显示性能悬崖现象与理论预测一致。研究发现短程评估协议可能掩盖结构不稳定性，表明未来推理系统需要从单纯扩展转向结构化治理。

Conclusion: 长程推理失败的根本约束源于自回归生成的过程级不稳定性，而非单纯搜索或任务复杂度。稳定长程推理需要离散分段，自然诱导出有向无环图等图状执行结构。这为理解长程推理失败提供了动力学视角，并揭示了纯自回归架构在维持长期连贯性方面的新限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体系统过度依赖大规模模型，而边缘规模模型（4B参数级别）的潜力尚未被充分探索。本文旨在系统研究如何训练高性能的4B参数智能体模型。

Method: 提出AgentCPM-Explore模型和整体训练框架：1）参数空间模型融合解决SFT中的灾难性遗忘；2）奖励信号去噪应对RL中的噪声敏感性；3）上下文信息精炼处理长上下文场景中的推理退化问题。

Result: AgentCPM-Explore在4B类模型中达到SOTA性能，在四个基准测试中匹配或超越8B类SOTA模型，在五个基准测试中甚至超越Claude-4.5-Sonnet或DeepSeek-v3.2等更大规模模型，在GAIA文本任务中达到97.09%准确率（pass@64）。

Conclusion: 边缘规模模型的瓶颈并非其固有能力上限，而是推理稳定性问题。通过建立的训练框架，AgentCPM-Explore有效释放了边缘规模模型先前被低估的显著潜力。
...
TL;DR: JADE是一个双层评估框架，结合了静态评估标准的严谨性和动态评估的灵活性，用于评估开放式专业任务中的智能体AI。


<details>
  <summary>Details</summary>
Motivation: 当前评估智能体AI在开放式专业任务时面临两难：静态评估标准虽然严谨可重复，但无法适应多样化的有效响应策略；而基于LLM的评估方法虽然能适应个体响应，但存在不稳定性和偏见问题。

Method: JADE采用双层评估框架：第一层将专家知识编码为预定义的评估技能，提供稳定的评估标准；第二层执行报告特定的、基于声明的评估，灵活评估多样化的推理策略，并通过证据依赖性门控来使基于被反驳声明的结论无效。

Result: 在BizBench上的实验表明，JADE提高了评估稳定性，并揭示了整体性LLM评估器遗漏的关键智能体失败模式。该框架与专家编写的评估标准高度一致，并能有效迁移到医疗领域基准测试。

Conclusion: JADE通过结合静态评估标准的严谨性和动态评估的灵活性，有效解决了开放式专业任务评估中的两难问题，在不同专业领域都表现出良好的验证效果。
...
TL;DR: 提出进度约束机制，将行为树与强化学习结合，通过可行性估计器约束动作集，解决控制器相互抵消问题，提升性能、样本效率和约束满足度。


<details>
  <summary>Details</summary>
Motivation: 行为树(BTs)提供结构化反应式决策框架，强化学习(RL)能学习接近最优控制器但存在稀疏奖励、安全探索和长期信用分配问题。两者结合有潜力互补：BT设计编码领域知识简化RL训练，RL自动学习BT内控制器。但简单结合可能导致控制器相互抵消，破坏已实现子目标，降低整体性能。

Method: 提出进度约束机制，基于行为树收敛理论结果，使用可行性估计器约束允许的动作集。在2D概念验证和高保真仓库环境中进行实证评估。

Result: 与先前BT-RL集成方法相比，该方法在性能、样本效率和约束满足度方面均有改进。

Conclusion: 进度约束机制有效解决了BT与RL集成中的控制器冲突问题，实现了两者的优势互补，在复杂环境中表现出更好的性能。
...
TL;DR: HyPER：一种用于专家混合模型多路径解码的训练免费在线控制策略，通过动态扩展-缩减控制优化探索-利用权衡，在固定计算预算下提升推理准确率并减少token使用。


<details>
  <summary>Details</summary>
Motivation: 现有方法在探索-利用权衡上处理方式僵化：树结构搜索通过脆弱的扩展规则硬编码探索，干扰后训练推理；并行推理过度探索冗余假设路径且依赖弱答案选择。最优平衡是阶段依赖的，正确与错误推理路径往往在后期才分叉。

Method: 将测试时扩展重新表述为假设池上的动态扩展-缩减控制问题。HyPER包含：1）在线控制器，随假设池演化从探索转向利用；2）token级精炼机制，无需完整路径重采样即可实现生成时高效利用；3）长度和置信度感知的聚合策略，实现可靠答案时利用。

Result: 在四个专家混合语言模型和多样化推理基准上的实验表明，HyPER始终实现更优的准确率-计算权衡，准确率提升8-10%，token使用减少25-40%。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在各种逻辑推理基准测试中表现出色，但尚不清楚它们真正掌握了哪些核心逻辑技能。需要设计一个统一的基准来隔离和评估形式推理中的基本技能。

Method: 引入LogicSkills基准，专注于一阶逻辑双变量片段（不含恒等关系）中的三项核心技能：形式符号化（将前提翻译为一阶逻辑）、反模型构建（构建使所有前提为真而结论为假的有限结构）、有效性评估（判断结论是否从前提推出）。测试项目以自然英语和Carroll风格的非词语言呈现，并使用SMT求解器Z3验证正确性和非平凡性。

Result: 领先的大语言模型在有效性评估上表现良好，但在形式符号化和反模型构建上的表现显著较差，表明模型主要依赖表面模式而非真正的符号或基于规则的推理。

Conclusion: 大语言模型在逻辑推理方面存在局限性，特别是在需要符号操作和反模型构建的深层推理任务上表现不足，这揭示了当前模型在形式推理能力上的真实水平。
...
TL;DR: AgentCPM-Report是一个轻量级本地深度研究报告生成系统，采用8B参数模型，通过动态修订大纲的WARP策略和交替的证据起草与推理深化过程，在多项基准测试中超越闭源系统。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究报告生成方法依赖计划-写作范式，其性能严重依赖初始大纲质量，而构建全面大纲需要强大推理能力，导致系统几乎完全依赖闭源或在线大模型，带来部署障碍以及用户数据安全和隐私问题。

Method: 提出AgentCPM-Report系统，包含模拟人类写作过程的框架和8B参数深度研究智能体。框架采用Writing As Reasoning Policy (WARP)，使模型能在报告生成过程中动态修订大纲。智能体在Evidence-Based Drafting和Reasoning-Driven Deepening之间交替，支持信息获取、知识精炼和迭代大纲演进。为增强小模型能力，引入多阶段智能体训练策略，包括冷启动、原子技能强化学习和整体管道强化学习。

Result: 在DeepResearch Bench、DeepConsult和DeepResearch Gym上的实验表明，AgentCPM-Report在Insight指标上取得显著提升，性能超越领先的闭源系统。

Conclusion: AgentCPM-Report提供了一个轻量级、高性能的本地解决方案，通过动态大纲修订和交替的起草-深化过程，有效解决了深度研究报告生成中的信息获取和洞察分析挑战，同时避免了依赖闭源模型带来的部署和安全隐私问题。
...
TL;DR: 提出SeeUPO算法，解决现有RL算法在LLM智能体多轮交互中缺乏收敛保证的问题，通过序列级顺序更新策略优化实现无critic的收敛保证。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习算法在LLM智能体场景中缺乏经过验证的收敛保证，特别是在多轮交互设置中，这会导致训练不稳定和无法收敛到最优策略。

Method: 提出SeeUPO（序列级顺序更新策略优化），将多轮交互建模为顺序执行的多臂赌博机问题，通过反向执行顺序的逐轮顺序策略更新，确保单调改进并通过逆向归纳收敛到全局最优解。

Result: 在AppWorld和BFCL v4基准测试中，SeeUPO相比现有骨干算法取得显著改进：在Qwen3-14B上相对增益43.3%-54.6%，在Qwen2.5-14B上相对增益24.1%-41.9%，并展现出优越的训练稳定性。

Conclusion: SeeUPO解决了现有RL算法在多轮交互中无法同时实现无critic和收敛保证的问题，为LLM智能体训练提供了具有理论保证的稳定收敛方法。
...
TL;DR: 本文提出一个表示感知和频率感知的评估框架，发现VLMs存在三种失效模式：预测答案稳定但内部表示漂移严重、模型规模增大不提升鲁棒性、扰动对不同任务有不同影响。


<details>
  <summary>Details</summary>
Motivation: 现有VLM鲁棒性评估仅关注输出层面的不变性，假设稳定预测反映稳定的多模态处理，但这一假设可能不足。需要更深入分析内部表示的变化。

Method: 提出表示感知和频率感知的评估框架，测量内部嵌入漂移、频谱敏感性和结构平滑性（视觉token的空间一致性），结合标准标签指标。在SEEDBench、MMMU和POPE数据集上测试现代VLMs。

Result: 发现三种失效模式：1) 预测答案稳定但内部表示漂移严重（如文本叠加扰动时漂移接近图像间变异性）；2) 鲁棒性不随规模提升，大模型准确率更高但敏感性相同或更大；3) 扰动对不同任务影响不同：破坏粗/细视觉线索组合会损害推理，但在幻觉基准上可能减少误报。

Conclusion: 仅依赖输出不变性评估VLM鲁棒性不足，需要更全面的内部表示分析。模型规模增大不保证鲁棒性提升，扰动对不同任务有复杂影响。
...
TL;DR: ARK模型通过自回归方式将知识图谱生成视为三元组序列生成任务，无需显式规则监督即可学习语义约束，在IntelliGraphs基准上达到89.2%-100%语义有效性，同时能生成未见过的图结构。
...
Method: 提出ARK（自回归知识图谱生成）模型家族，将图视为(head, relation, tail)三元组序列进行自回归生成。模型直接从数据中学习隐式语义约束（类型一致性、时间有效性、关系模式），无需显式规则监督。还提出SAIL作为ARK的变分扩展，通过学习的潜在表示实现可控生成。

Result: 在IntelliGraphs基准上，模型在多样化数据集上达到89.2%到100.0%的语义有效性，同时能生成训练中未见的新图。分析表明模型容量（隐藏维度≥64）比架构深度更重要，循环架构在达到与基于Transformer的替代方案相当有效性的同时提供显著计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 多任务强化学习中，需要学习一个能泛化到任意（可能未见）任务的通用策略。现有方法在处理复杂LTL规范时存在局限性，需要更有效的任务表示方法。

Method: 利用新一代语义LTL到自动机转换技术，构建语义标记的自动机，从中提取结构化任务嵌入来条件化策略，支持完整LTL规范。

Result: 实验结果表明，该方法在多个领域达到最先进性能，能够扩展到现有方法无法处理的复杂规范。

Conclusion: 基于语义LTL到自动机转换的任务嵌入技术能有效支持多任务强化学习，特别是在处理复杂时序逻辑规范方面具有优势。
...
TL;DR: SSM代码模型在预训练中优于Transformer捕捉代码语法语义，但在微调时会遗忘某些语法语义关系，尤其是短程依赖。作者提出SSM-Interpret诊断框架和架构改进提升性能。


<details>
  <summary>Details</summary>
Motivation: 虽然状态空间模型(SSMs)在代码理解任务上表现出色，但其内部机制仍是黑盒。需要系统分析SSM代码模型学习的内容，并与Transformer模型进行首次比较分析。

Method: 1. 对SSM和Transformer代码模型进行系统比较分析；2. 提出SSM-Interpret频率域框架，诊断微调过程中的频谱变化；3. 基于分析结果提出架构改进方案。

Result: SSMs在预训练中优于Transformer捕捉代码语法和语义，但在微调时会遗忘某些语法语义关系，特别是当任务强调短程依赖时。SSM-Interpret框架揭示了微调过程中频谱向短程依赖的偏移。
...
TL;DR: 研究比较了两种主动概念学习策略：理性主动学习（最大化期望信息增益）和人类式积极测试策略（查询当前最佳假设预测为正的实例），发现后者在简单概念上表现更好，这可能是因为它避免了支持失配问题。


<details>
  <summary>Details</summary>
Motivation: 研究人类主动概念学习中的权衡：查询的信息性（减少不确定性）与学习器的稳定性（生成和评分假设的能力）。探讨在神经符号贝叶斯学习器中，当假设由大语言模型生成时，这种权衡如何体现。

Method: 使用神经符号贝叶斯学习器，假设是大型语言模型生成的可执行程序，通过贝叶斯更新重新加权。比较两种查询选择策略：1）理性主动学习（Rational Active Learner），最大化近似期望信息增益；2）人类式积极测试策略（Positive Test Strategy），查询当前最佳假设预测为正的实例。在经典数字游戏的概念学习任务中进行实验。

Result: 期望信息增益策略在需要证伪的复杂概念（如复合规则或例外规则）上有效，但在简单概念上表现不佳。这是因为高度诊断性的边界查询导致后验分布向生成器产生无效或过于特定程序的区域移动，造成粒子近似中的支持失配陷阱。积极测试策略虽然信息次优，但通过选择"安全"查询保持提议有效性，在简单规则上收敛更快。

Conclusion: "确认偏误"可能不是认知错误，而是在人类思维特有的稀疏、开放假设空间中，为保持可处理推理而做出的理性适应。积极测试策略通过避免支持失配问题，在简单概念学习中表现更好。
...
Method: ScaleEnv框架通过程序化测试确保环境可靠性，通过工具依赖图扩展和可执行动作验证来保证任务完整性和可解性，使智能体能够在完全从零构建的交互环境中进行探索学习。

Result: 在未见的多轮工具使用基准（如τ²-Bench和VitaBench）上表现出显著的性能提升，展示了强大的泛化能力；同时研究了领域数量增加与模型泛化性能之间的关系，提供了扩展环境多样性对鲁棒智能体学习至关重要的实证证据。

Conclusion: ScaleEnv通过从零构建交互式环境和可验证任务，解决了现有环境合成方法的多样性和可扩展性限制，为智能体学习提供了可靠的基础，并证明了扩展环境多样性对提升智能体泛化能力的重要性。
...
TL;DR: POP是一种高效的在线结构化剪枝框架，通过分区引导实现上下文条件动态剪枝，在自回归生成过程中最小化计算开销


<details>
  <summary>Details</summary>
Motivation: 现有结构化剪枝方法在推理时采用固定的剪枝决策，忽视了自回归token生成过程中出现的稀疏性模式。大型基础模型通过扩展获得强大性能，但需要更智能的剪枝策略来适应动态推理过程。

Method: POP将模型通道划分为保留区、候选区和剪枝区：预填充阶段定义粗粒度剪枝分区，解码阶段在候选区内生成细粒度掩码，避免全通道重新评估。该方法轻量级、即插即用，无需预处理、离线校准、重新训练或学习预测器。

Result: 在多种大型基础模型（包括大语言模型、专家混合模型和视觉语言模型）上的广泛评估表明，POP比现有剪枝方法始终提供更高的准确性，同时产生更小的计算开销并最小化推理延迟。

Conclusion: POP是一种有效的在线结构化剪枝框架，能够实现上下文条件动态剪枝，在自回归生成过程中显著减少计算开销，为大型基础模型的高效推理提供了实用解决方案。
...
TL;DR: 提出基于博弈论的框架，通过纳什均衡分析预测和引导大语言模型群体的行为，避免开放文本空间中的均衡计算难题，将代理行为建模为人类子群体的混合策略。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型群体行为难以预测和引导，特别是在开放文本空间中均衡计算不可行。需要一种既能预测群体行为又能提供可操作指导的方法，以引导模型群体朝向社会期望的结果。

Method: 将每个代理的行为建模为人类子群体的混合策略，代理主动战略性地选择与哪些群体对齐。采用标准凹效用假设推导封闭形式的纳什均衡特征，作为现有对齐流程（如RLHF）之上的主动对齐层。

Result: 在社交媒体场景中，大语言模型群体（特别是基于推理的模型）可能表现出政治排斥现象，即某些子群体被所有LLM代理忽视。该方法能够避免这种病理现象，展示了在多智能体LLM动态调节中的应用潜力。

Conclusion: 该博弈论框架能够预测和引导大语言模型群体行为，通过纳什均衡分析提供可操作的系统级指导，避免政治排斥等病理现象，有望在不同领域调节多智能体LLM动态。
...
TL;DR: 提出自适应差分隐私联邦学习框架，通过客户端表示压缩、自适应梯度裁剪和约束感知聚合，解决异构数据和隐私约束下的训练不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 实际联邦学习中存在设备异构性、非独立同分布数据，导致梯度更新不稳定和偏差。当加入差分隐私时，固定的梯度裁剪和高斯噪声注入会进一步放大梯度扰动，造成训练震荡和性能下降。

Method: 1) 客户端：引入轻量级本地压缩模块，正则化中间表示并约束梯度变异性，减轻本地优化时的噪声放大；2) 服务器端：基于历史更新统计动态调整裁剪阈值的自适应梯度裁剪策略；3) 约束感知聚合机制，抑制不可靠或噪声主导的客户端更新。
...
TL;DR: 该研究比较了静态分类任务中的归因解释与智能体系统中的轨迹诊断，发现归因方法在静态场景有效但在智能体轨迹诊断中不可靠，而基于轨迹的评估能有效定位行为故障。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型发展，智能体AI系统通过多步决策序列运行，但现有的可解释AI方法主要针对静态预测设计，不清楚这些方法如何适用于行为随时间演化的智能体场景。

Method: 通过实证比较静态分类任务中的归因解释方法（如特征重要性排序）与智能体基准测试（TAU-bench Airline和AssistantBench）中的轨迹诊断方法，区分两种解释范式的应用效果。

Result: 归因方法在静态设置中特征排序稳定（Spearman ρ=0.86），但无法可靠诊断智能体轨迹中的执行级故障；基于轨迹的评估能一致定位行为故障，发现状态跟踪不一致在失败运行中高2.7倍，降低成功概率49%。

Conclusion: 研究结果表明需要从静态解释转向轨迹级可解释性，以有效评估和诊断自主AI系统的行为，特别是在智能体系统中。
...
TL;DR: AIRS-Bench是一个AI科研基准测试套件，包含20个从顶级机器学习论文中提取的任务，用于评估LLM智能体在整个科研生命周期中的能力，包括想法生成、实验分析和迭代优化。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在推动科学研究方面具有巨大潜力，但目前缺乏一个全面的基准测试来评估智能体在整个科研生命周期中的能力，包括从想法生成到实验分析和迭代优化的全过程。

Method: 从最先进的机器学习论文中提取20个任务，涵盖语言建模、数学、生物信息学和时间序列预测等多个领域。任务格式灵活，便于集成新任务和比较不同智能体框架。使用前沿模型配合顺序和并行框架建立基线。

Result: 智能体在4个任务中超越了人类最佳水平，但在16个任务中未能达到人类水平。即使智能体超越了人类基准，也未能达到底层任务的理论性能上限，表明基准远未饱和，有巨大改进空间。

Conclusion: AIRS-Bench是一个尚未饱和的基准测试，为自主科学研究提供了重要的评估工具和发展方向。开源任务定义和评估代码将促进该领域的进一步发展。
...
TL;DR: AI代理在任务执行前、中、后预测成功率时表现出过度自信，成功率仅22%的代理预测77%成功率。反直觉的是，信息更少的执行前评估比标准执行后评估有更好区分度。对抗性提示将评估重构为bug发现可实现最佳校准。


<details>
  <summary>Details</summary>
Motivation: 研究AI代理是否能准确预测自身任务成功率，探索代理不确定性，检验代理在不同执行阶段（执行前、执行中、执行后）的成功概率估计准确性。

Method: 通过提示AI代理在任务执行前、执行中和执行后提供成功概率估计，比较不同阶段的预测准确性。使用对抗性提示方法，将评估重构为bug发现任务来改善校准。

Result: 发现代理过度自信现象：实际成功率仅22%的代理预测77%成功率。反直觉发现：信息更少的执行前评估比标准执行后评估有更好区分度（差异不总是显著）。对抗性提示的bug发现方法实现了最佳校准。

Conclusion: AI代理在预测自身任务成功率时存在系统性过度自信，但通过对抗性提示将评估重构为bug发现可以改善校准。执行前评估虽然信息较少，但可能比执行后评估有更好的区分能力。
...
### [163] [Real time, cross platform visualizations with zero dependencies for the N-body package REBOUND](https://arxiv.org/abs/2602.06735)
*Hanno Rein*

Main category: cs.GR

TL;DR: 论文提出了一种基于浏览器的新方法，用于N-body模拟软件REBOUND的实时可视化，无需外部库，通过WebAssembly重用OpenGL代码，支持跨平台3D交互可视化。


<details>
  <summary>Details</summary>
Motivation: 现有可视化工具通常依赖外部图形/UI库，这会引入新的依赖关系、增加开发者和用户的学习门槛，并带来长期支持的不确定性。需要一种更简洁、跨平台的实时可视化解决方案。

Method: 采用浏览器作为GPU加速渲染平台，利用WebAssembly重用现有的OpenGL可视化代码，通过HTTP通信和内置Web服务器实现本地和远程实时可视化，无需任何外部库。

Result: 实现了支持所有主流操作系统的3D交互式可视化，提供了多种操作模式：浏览器实时可视化、完全在浏览器中运行的模拟、Jupyter笔记本中的可视化，以及传统的OpenGL独立可视化。

Conclusion: 该方法成功解决了跨平台实时可视化的依赖问题，降低了开发和使用门槛，虽然针对REBOUND实现，但其概念和思想可广泛应用于其他需要科学和非科学实时可视化的领域。
