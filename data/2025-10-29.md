...
Motivation: 现有3D场景生成方法局限于单对象生成、需要领域特定训练或缺乏360度全视角支持，需要一种更通用、可扩展的解决方案。

Method: 将场景生成重新表述为多瓦片去噪问题，独立生成重叠的3D区域并通过加权平均无缝融合，利用对象级先验知识实现语义控制。

Result: 方法支持多样化的场景布局、高效生成和灵活编辑，无需场景级数据集或重新训练，继承了对象级先验的泛化能力。

Conclusion: 建立了一个简单而强大的基础，用于通用语言驱动的3D场景构建，展示了模块化方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统RTI方法（如PTM和HSH）使用固定基函数，难以准确捕捉复杂反射场，而NeuralRTI虽然质量更好但计算成本高，无法在有限硬件上实现全分辨率交互渲染。

Method: 采用知识蒸馏技术，从大型教师网络训练小型学生网络，在保持渲染质量的同时显著降低计算成本。

Result: DisK-NeuralRTI在保持与NeuralRTI相当渲染质量的同时，大幅减少了计算开销，使其能够在有限硬件上实现全分辨率交互渲染。

Conclusion: 知识蒸馏是降低NeuralRTI计算成本的有效方法，使高质量RTI渲染在资源受限环境中变得可行。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像的真实感不断增强，验证视觉真实性面临挑战，需要开发能够检测、定位并解释图像伪影的系统。

Method: 使用轻量级卷积分类器（Faster-Than-Lies）结合视觉语言模型（Qwen2-VL-7B），通过自编码器重建误差图生成伪影定位热图，并将70种视觉伪影分类为8个语义组。

Result: 在扩展的CiFAKE数据集上达到96.5%的准确率，在8核CPU上推理时间为175ms，能够生成可解释的文本描述每个检测到的异常。

Conclusion: 该工作证明了结合视觉和语言推理在低分辨率图像中进行可解释真实性检测的可行性，并指出了在取证、工业检测和社交媒体审核等领域的跨领域应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够通过感知视觉重复和结构关系来轻松计数各种物体，而不依赖类别身份。但现有计数模型在物体形状复杂、内部对称或重叠时容易出错，需要开发能够复制人类计数能力的方法。

Method: 基于CounTR架构，用自监督基础模型DINOv2替换视觉编码器以获得更丰富和空间一致的特征表示。加入位置嵌入融合来保持几何关系，通过轻量卷积解码器将特征解码为密度图。
...
<details>
  <summary>Details</summary>
Motivation: 河流中漂浮的人造垃圾对环境、生物多样性和人类活动造成严重影响，需要有效的监测方法。

Method: 使用固定原位摄像头采集数据，应用深度学习模型进行垃圾检测和量化，结合相机内外参数构建几何模型估算物体尺寸，并测试不同环境条件和学习配置。

Result: 验证了深度学习方法在复杂环境条件下的可行性，强调了数据集构建协议的重要性，特别是负样本整合和时间泄漏考虑。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像超分辨率在分布外条件下（如罕见地貌特征和多样化传感器）经常失败，产生视觉合理但物理不准确的结果，需要开发对分布外条件鲁棒的物理感知方法。

Method: 采用双条件架构：门控ControlNet保持低分辨率输入的几何保真度，文本提示提供语义指导；引入多面损失函数确保光谱和辐射一致性；使用随机前向传播量化预测不确定性。

Result: 在盲评估中，地球物理专家认为模型输出接近地面真实图像的保真度，显著优于最先进基线；定量指标显示FID减少近40%。
...
<details>
  <summary>Details</summary>
Motivation: CLIP模型的图像级预训练目标与像素级视觉理解存在不对齐问题，导致分割性能不理想。现有方法继承前层的全局对齐偏差，需要解决这一问题。

Method: 提出三种互补技术：语义-空间重加权、选择性头增强和异常token替换，无需额外训练即可恢复视觉区分能力。

Result: 在8个常用语义分割基准测试中达到最先进性能，证明其有效性和实际部署实用性。

Conclusion: LHT-CLIP通过系统利用CLIP的多层次视觉区分能力，成功解决了CLIP在语义分割中的对齐问题，为实际应用提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 教学视频中的场景级字幕需要理解视觉线索和时间结构，但现有字幕往往缺乏连贯性和质量，可能造成混淆并削弱视频的教育意图。

Method: 使用自适应帧采样和多模态窗口捕获场景内关键转换，采用多模态思维链过程生成多个动作-对象对，通过动态步长窗口选择算法平衡时间上下文和冗余，最终生成结合视觉语义和时间推理的单个教学字幕。

Result: 在N-gram指标（BLEU、METEOR）和语义相似度指标（BERTScore、CLIPScore）上相比强基线模型获得一致提升，定性分析显示生成的字幕在时间连贯性和信息量方面更优。
...
Method: 采用前馈图像到虚拟形象生成流程获取初始3D表示和噪声渲染，然后使用基于输入图像的单步扩散模型进行多视角一致的渲染细化，并通过合成多视图数据预训练和高质量真实图像微调的训练策略。

Result: 该方法在质量和数量上都优于当前最先进的人像新视角合成方法，同时保持了时间效率。

Conclusion: TurboPortrait3D成功地将图像扩散模型用于增强图像到虚拟形象方法的质量，实现了高质量、多视角一致且低延迟的人像3D生成。
...
Method: 设计了语言提示平面先验(LP3)管道，使用预训练视觉语言分割模型，并通过交叉视图融合和几何先验检查优化区域提议。在3D高斯优化中增加了平面先验监督和几何先验监督项。

Result: 在标准室内基准测试中，PlanarGS重建出准确详细的3D表面，大幅优于现有最先进方法。

Conclusion: PlanarGS通过引入平面和几何先验，有效提升了3DGS在室内场景中的重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在选择输入频率和架构时面临挑战，通常需要启发式方法和繁重的超参数优化，且存在参数冗余问题。

Method: 采用神经元剪枝机制避免冗余，通过目标权重衰减将信息转移到剩余神经元，然后进行结构化剪枝；同时通过输入频率密集化在信号欠拟合的频谱区域添加频率。

Result: 在图像和SDF上的实验表明，AIRe能够减小模型大小，同时保持甚至提升重建质量。

Conclusion: AIRe通过自适应训练方案实现了网络大小与重建质量之间的更好权衡，代码和预训练模型将公开发布。
...
Method: 借鉴计算机图形学中的USD标准，引入分层结构化场景表示，采用微调方法确保控制信号的解耦。
...
Method: 提出有效的数据收集和生成框架、策略遵循训练流程、定制化损失函数，以及多样化的问答生成和训练策略来增强学习效果。
...
Method: 采用两阶段训练方法：首先进行推理风格的监督微调，然后使用基于可验证奖励的强化学习，结合高保真视觉编码生成系统化的推理过程。

Result: 在分布外评估中达到竞争性的多标签分类性能，同时提升可解释性；在专家阅读研究中，完整推理轨迹提高了置信度，支持错误审计，并减少了报告完成时间。
...
Conclusion: 联合预测方法相比独立预测能更好地实现成本-质量可控的任意尺度超分辨率
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常孤立评估AI助手能力，缺乏真实的流式场景，或仅支持短期任务，无法满足现实世界中多模态输入、实时响应和长期记忆保持的需求。

Method: 构建包含工作学习、生活方式、社交活动和外出文化四个领域的同步多模态数据集，所有数据在统一时间线上对齐，包含高质量视觉叙述和语音转录，通过人工精炼。定义了12个子任务和3个核心能力评估。

Result: 数据集包含3,291个人工验证的问答项目，涵盖多种问题格式，在严格流式设置下评估。提出了实时准确性和记忆持久时间两个关键指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在糖尿病视网膜病变检测中面临分布变化（如设备差异、人口统计学差异、成像条件）导致的鲁棒性挑战，需要提升模型在未见数据上的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 主动脉血管树自动分析具有巨大临床潜力，但缺乏共享高质量数据阻碍了发展，因此创建公开数据集以推动该领域进步。

Method: 通过SEG.A挑战赛引入大型多机构公开数据集，对隐藏测试集进行基准测试，后续可选表面网格化任务用于计算模拟。

Result: 深度学习方法是主流，3D U-Net架构主导顶级提交；集成排名最高算法显著优于单个模型；性能与算法设计（特别是定制后处理步骤）和训练数据特征密切相关。

Conclusion: 该倡议不仅建立了新的性能基准，还提供了持久资源以推动未来创新，开发稳健且可临床转化的工具。
...
<details>
  <summary>Details</summary>
Motivation: 火星科学缺乏标准化的基准测试和评估框架，这限制了火星基础模型的发展。

Method: 提供标准化的即用数据集，并使用在自然图像、地球卫星数据和最先进的视觉语言模型上预训练的模型进行基线评估。

Result: 所有分析结果表明，火星特定的基础模型可能比通用领域模型具有优势。

Conclusion: Mars-Bench旨在为火星科学中机器学习的开发和比较建立标准化基础，并鼓励进一步探索领域自适应预训练。
...
Method: 采用交替优化微调管道，结合对抗性后缀优化和LLM微调，并集成双重规避策略：通过辅助LLM困惑度评分确保人类可读性，以及引入禁止词惩罚来抑制黑名单词生成。

Result: 实验证明该方法在红队测试中表现优异，生成的人类可读对抗性提示能有效抵抗过滤器，并具有出色的零样本迁移能力，能适应未见过的提示和商业API。

Conclusion: APT框架成功暴露了文本到图像模型的关键安全漏洞，即使在商业API中也能有效工作，为模型安全评估提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度卷积神经网络训练中的梯度消失问题，使得能够训练数百层的深度网络。

Method: 使用跳跃连接（skip connections）构建残差网络，允许梯度通过捷径连接直接传播，绕过中间层。

Result: 在CIFAR-10数据集上，ResNet-18达到89.9%准确率，相比传统深度CNN的84.1%提升了5.8%，且收敛速度更快、训练更稳定。
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏调优方法存在两个问题：1) 两阶段方法基于梯度信息定位任务相关权重，忽略了微调过程中的参数调整；2) 稀疏掩码梯度更新导致高内存使用。

Method: 提出SNELLA单阶段方法：1) 通过两个低秩可学习矩阵合并的稀疏矩阵选择性更新权重矩阵；2) 引入非线性核函数扩展低秩分解，防止权重更新间的相互依赖；3) 自适应双层稀疏分配机制，基于重要性分数在层间和层内进行端到端竞争。
...
Conclusion: SNELLA通过单阶段设计和创新的稀疏分配机制，有效解决了现有PEFT方法在性能和内存效率方面的限制，为参数高效微调提供了新的解决方案。
...
Method: COLA首先将对抗图像嵌入投影到类别文本特征张成的子空间，过滤非语义失真；然后将图像和文本建模为多个增强视图上的离散分布，通过最优传输细化对齐，并将子空间投影无缝集成到成本计算中。

Result: 在14个零样本分类基准上的广泛评估显示，COLA在PGD对抗攻击下在ImageNet及其变体上平均提升6.7%，同时在干净样本上保持高准确率。

Conclusion: COLA是一种无需训练且兼容现有微调模型的有效对抗防御方法，通过跨模态对齐显著提升CLIP模型在对抗攻击下的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前使用T2I模型生成合成训练数据时，通过少量真实样本微调模型可能造成过拟合和多样性降低的问题，特别是在细粒度分类任务中。

Method: 首先从少量真实样本中提取类无关属性（场景背景、物体姿态等），在微调T2I模型时显式条件化这些属性，然后在生成合成数据时边缘化这些属性。

Result: 在多个T2I模型、骨干网络和数据集上的实验表明，BOB方法在低样本细粒度分类任务中达到最先进性能，在Aircraft数据集上比DataDream提升7.4%，在18/24个实验设置中优于先前方法。

Conclusion: BOB方法通过显式条件化和边缘化类无关属性，有效缓解了T2I模型微调时的过拟合问题，保持了生成多样性，显著提升了合成数据在细粒度分类任务中的有效性。
...
Method: 研究交叉注意力和自注意力机制的特性，应用自注意力反转实现文本移除，重分布交叉注意力减少文本幻觉。引入潜在优化框架中的新型损失函数：交叉注意力内容损失提高文本渲染准确性，自注意力样式损失促进样式定制。

Result: OmniText在多个任务和指标上实现了最先进的性能，与其他文本修复方法相比表现优异，并且与专门化方法相媲美。还提出了OmniText-Bench基准数据集用于评估多样化文本图像操作任务。

Conclusion: OmniText是第一个能够执行多样化文本图像操作任务的通才方法，通过创新的注意力机制操作和损失函数设计，有效解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 预训练视觉模型在广泛应用中对表示可解释性提出了新要求，但尚不清楚预训练表示是否能同时实现高可解释性和高分类性能。

Method: 提出固有可解释性评分(IIS)，通过评估解释过程中的信息损失来量化表示的可解释性，并利用可解释性最大化进行微调。

Result: 发现可解释性和分类性能呈正相关，通过可解释性最大化微调可进一步提升分类性能，基于解释的预测精度下降更小。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法主要针对同构模型设计，在异构场景下效果下降，特别是涉及中间特征时。架构多样性导致语义差异，阻碍了中间表示的有效利用

Method: 应用傅里叶变换捕捉全局特征信息，通过特征转换模块（FTM）生成紧凑的频域表示，使用可学习的特征对齐模块（FAM）进行多级匹配对齐，结合中间特征的均方误差和logits的KL散度作为联合训练目标

Result: 在CIFAR-100和ImageNet-1K数据集上分别比最新方法提升了5.59%和0.83%
...
<details>
  <summary>Details</summary>
Motivation: 解决现有狗运动数据集缺乏多视角、真实3D数据以及规模和多样性不足的问题。

Method: 提出三阶段实例特定优化流程：粗对齐、密集对应监督和时间正则化，将SMAL模型拟合到运动序列中。

Result: 建立了四个运动恢复基准设置，支持单目/多视角、RGB/RGB-D输入的系统评估。

Conclusion: DogMo数据集和方法为狗运动恢复研究提供了基础，开辟了计算机视觉、图形学和动物行为建模交叉领域的新方向。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但迭代采样成本高昂。现有训练免费加速方法忽略去噪趋势且缺乏误差控制，导致轨迹偏差和结果不一致

Method: 1) 引入一致性趋势预测器，利用扩散轨迹的平滑连续性，将历史去噪模式投影到稳定未来方向；2) 提出模型特定误差容限搜索机制，通过识别从语义规划到质量精炼的过渡点来推导修正阈值

Result: ETC相比FLUX实现了2.65倍加速，一致性退化极小（-0.074 SSIM分数）

Conclusion: ETC框架通过结合趋势一致性预测和误差容限控制，有效解决了扩散模型加速中的轨迹偏差问题，实现了高质量加速
...
Method: 利用大型语言模型从输入提示合成显式布局，将这些布局注入图像生成过程，并使用对象中心视觉语言模型对多个候选结果进行重排序，迭代选择最符合提示的结果。

Result: 通过结合显式布局基础和基于自优化的推理时扩展，该框架相比最近的文本到图像模型实现了更强的场景与提示对齐。
...
Method: 提出了VC4VG框架，从T2V角度分析字幕内容，将视频重建所需要素分解为多个维度，并建立了系统化的字幕设计方法。还构建了VC4VG-Bench基准，包含细粒度、多维度、必要性分级的评估指标。

Result: 广泛的T2V微调实验表明，改进的字幕质量与视频生成性能之间存在强相关性，验证了该方法的有效性。

Conclusion: VC4VG为文本到视频生成提供了有效的字幕优化框架，通过系统化的字幕设计和专用评估基准，显著提升了视频生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在自动驾驶场景理解中面临的多样化任务干扰问题，提升在感知、预测、规划和腐败检测等安全关键任务上的性能。

Method: 1) 混合提示路由器分类问题并分发到任务特定专家提示；2) 任务特定提示嵌入坐标系、空间推理规则、角色扮演等；3) 视觉组装模块组合多视角图像；4) 按任务配置模型推理参数。

Result: 在Qwen2.5-VL-72B上实现，Phase-1（干净数据）平均准确率70.87%，Phase-2（腐败数据）72.85%。
...
<details>
  <summary>Details</summary>
Motivation: SAM2作为SAM的继任者，在视频分割中表现出强大的泛化能力，但其对抗鲁棒性尚未被探索。现有对SAM的攻击是否能直接迁移到SAM2存在疑问，且两者架构差异带来了方向性提示引导和跨帧语义纠缠两大挑战。

Method: 设计了目标扫描策略将每帧划分为k个区域并随机分配提示，以减少优化过程中的提示依赖性；提出双语义偏差框架，通过扭曲当前帧语义和破坏跨帧语义一致性来优化UAP。

Result: 在六个数据集的两个分割任务上的广泛实验表明，该方法对SAM2有效，且显著优于现有最先进攻击方法。

Conclusion: UAP-SAM2成功解决了SAM2的对抗鲁棒性问题，在跨提示迁移性和攻击效果方面都表现出色，为视频分割模型的鲁棒性研究提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统CNN模型在泛化性、鲁棒性和处理不确定性方面存在局限，影响分割性能，需要解决这些问题以提高医学图像分割的准确性。

Method: 提出CLFSeg框架，集成模糊卷积模块结合卷积层和模糊逻辑，使用二元交叉熵与dice损失处理类别不平衡问题。

Result: 在四个公开数据集上表现优异，超越现有SOTA性能，在解剖结构中关注相关感兴趣区域。

Conclusion: CLFSeg在确保计算效率的同时提高性能，是现实世界医学诊断场景的潜在解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自回归视觉生成虽然展现出潜力，但由于逐token生成导致推理速度缓慢，需要数千步才能生成单个样本，严重限制了实际应用。

Method: 提出基于耦合的信息论方法MC-SJD，通过最大化连续迭代中采样相同草稿token的概率来显著加速标准SJD，同时保持其无损特性，仅需对现有算法进行单行修改。

Result: 在图像生成中实现约4.2倍加速，在视频生成中实现约13.3倍加速，相比标准自回归解码，且输出质量无任何下降。

Conclusion: MC-SJD通过简单而有效的方法解决了自回归视觉生成的推理速度瓶颈，为实际应用提供了可行的加速方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决主流扩散模型依赖推理时干预（如负引导或基于能量的优化）导致的分布偏移和身份与非身份属性纠缠问题，这些方法会降低视觉保真度和数据效用。

Method: 设计条件扩散模型，采用身份掩码学习方案。包括身份解耦潜在重组器（使用身份变分自编码器建模身份特征）和身份引导潜在协调器（通过软门控融合表示）。训练时使用基于重组的重建损失强制解耦，推理时从学习的身份空间采样随机身份向量实现匿名化。

Result: 实验表明ID²Face在视觉质量、身份抑制和效用保持方面优于现有方法。

Conclusion: ID²Face通过训练中心的方法有效解决了身份匿名化中的分布偏移和属性纠缠问题，实现了更好的匿名化效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉标记修剪方法主要基于注意力分数选择最显著的标记，导致所选标记语义不完整，存在冗余计算问题。

Method: 提出SCOPE评分，结合标记的显著性和覆盖度增益，迭代选择评分最高的标记，使用集合覆盖度量化标记关系。

Result: 在多个视觉语言理解基准测试中，使用LLaVA-1.5和LLaVA-Next模型，该方法始终优于先前方法。

Conclusion: SCOPE方法能有效减少计算开销，同时更好地保持语义完整性，在多模态大语言模型中具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统微扫视研究方法成本高、可扩展性差且时间分辨率有限，而事件感知提供了高速、低延迟的替代方案，能有效捕捉精细的时空变化。

Method: 使用Blender渲染高保真眼动场景，模拟0.5-2.0度角位移的微扫视，分为7类，通过v2e转换为事件流，并使用Spiking-VGG系列网络和光学流增强的Spiking-VGG16Flow进行分类。

Result: 模型平均准确率约90%，能够独立于事件数量或持续时间，成功按角位移分类微扫视。

Conclusion: 证明了脉冲神经网络在精细运动识别方面的潜力，为基于事件的视觉研究建立了基准数据集和方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将图像恢复作为预处理步骤，但恢复网络和检测网络之间的功能不匹配会导致不稳定性和集成困难，这一问题尚未得到充分研究。

Method: 通过Lipschitz连续性分析恢复网络和检测网络的功能差异，提出LROD框架，将图像恢复直接集成到检测器特征学习中，实现Lipschitz连续性的协调。

Result: 在雾霾和低光照基准测试上的广泛实验表明，LR-YOLO持续提高了检测稳定性、优化平滑度和整体准确率。

Conclusion: LROD框架有效解决了恢复-检测级联系统中的功能不匹配问题，为恶劣条件下的鲁棒目标检测提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的阴影去除方法会混合不相关区域的照明线索，导致结构扭曲和颜色不一致。需要一种能选择性整合相关上下文并保持颜色一致性的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法主要关注内在数据属性（如保真度和多样性），而忽略了任务特定需求。不同任务和网络架构对训练数据的要求差异很大，因此需要为下游任务量身定制数据生成过程。

Method: 引入权重分配网络评估每个合成样本的任务特定效用，采用双层优化策略：模型级优化调整生成模型以适应下游任务，实例级优化在每轮生成中调整生成策略（如提示嵌入和初始噪声）。

Result: 在8个不同复杂度和粒度的基准数据集上的实验表明，UtilGen始终实现优越性能，相比之前SOTA平均准确率提升3.87%。数据分析显示UtilGen产生更具影响力和任务相关性的合成数据。

Conclusion: UtilGen验证了从视觉特征为中心到任务效用为中心的数据增强范式转变的有效性，能够生成更符合下游任务需求的高效用训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 解决合成图像来源归因在数据稀缺条件下的挑战，特别是在少样本或零样本分类场景中，需要开发有效的归因方法。

Method: 训练免费的基于图像重合成的单样本归因方法：生成图像描述提示词，用所有候选源模型重合成图像，在特征空间中选择最接近原始图像的重合成结果进行归因。

Result: 提出的重合成方法在只有少量训练样本可用时，优于现有的少样本方法和其他基线方法。

Conclusion: 该方法在数据稀缺条件下表现出色，新构建的数据集为开发和评估未来的少样本和零样本方法提供了有价值的基准。
...
Method: 基于两阶段任务设计ViPER自引导框架，通过图像级和实例级重构与两阶段强化学习策略相结合，建立闭环训练范式，利用内部合成数据增强感知能力。

Result: 在Qwen2.5-VL模型上应用ViPER得到Qwen-Viper系列，在7个综合基准测试中平均提升1.7%，在细粒度感知任务上提升达6.0%，在各种视觉语言场景中表现优越且保持泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 遥感应用依赖深度学习进行场景分类，但面临标注数据稀缺和跨域标注成本高的问题。尽管视觉语言模型如CLIP显示出潜力，但由于显著的领域差距和任务特定语义适应的需求，直接应用于遥感仍不理想。

Method: 评估了多种代表性提示学习方法：上下文优化、条件上下文优化、多模态提示学习、以及带自调节约束的提示学习。这些方法反映了互补的设计理念，从静态上下文优化到条件提示增强泛化能力，再到多模态提示联合视觉语言适应，以及语义正则化提示实现稳定学习。

Result: 通过在多个基准遥感数据集上的广泛实验，包括跨数据集泛化测试，证明提示学习在少样本场景中持续优于两个标准基线：使用手工提示的零样本CLIP和在冻结CLIP特征上训练的线性探针。带自调节约束的提示学习实现了最稳健的跨域性能。
...
Conclusion: 该即插即用框架通过改进师生协作机制，有效解决了半监督医学图像分割中的知识传递可靠性问题。
...
TL;DR: W2-Net通过双查询机制将指代表达式计数问题解耦为"计数什么"和"在哪里看"，引入专门的where-to-see查询来提取属性特定区域特征，并提出了子类可分离匹配策略来增强子类间区分度。


<details>
  <summary>Details</summary>
Motivation: 现有指代表达式计数方法存在根本挑战：标注点通常放在类别代表性位置（如头部），迫使模型关注类别级特征而忽略其他视觉区域（如腿部）的属性信息。

Method: 提出W2-Net框架，通过双查询机制解耦问题：what-to-count查询定位对象，where-to-see查询专门从属性特定视觉区域提取特征。还引入了子类可分离匹配策略，在标签分配时加入排斥力增强子类间可分离性。

Result: 在REC-8K数据集上显著优于现有最优方法，计数误差分别降低22.5%（验证集）和18.0%（测试集），定位F1分别提高7%和8%。

Conclusion: W2-Net通过解耦"计数什么"和"在哪里看"，有效解决了指代表达式计数中属性信息被忽视的问题，在计数精度和定位准确性方面都取得了显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习框架（如nnU-Net）在脑卒中病灶分割中性能优异，但由于依赖复杂和设计单一，难以在临床环境中部署使用。
...
Conclusion: 高性能的研究流水线可以转化为便携、临床可用的工具，实现研究到临床的转化。
...
Method: 在编码器阶段通过亮度分支生成多尺度空间权重矩阵，动态加权注入亮度到特征图；在瓶颈层设计全局-局部特征融合机制，通过窗口自注意力计算平衡全局上下文和局部细节；在解码器阶段提出亮度增强模块，建立亮度分布与纹理特征的映射关系。

Result: 在MSP、PIF和GAND数据集上的大量实验验证，提出的MLSN在主观和客观评估中优于最先进方法，MS-SSIM和SD指标分别比其他方法的平均值高出8.57%、60.64%、10.26%、63.53%、22.21%和54.31%。

Conclusion: MLSN网络有效解决了偏振图像融合中的亮度适应性问题，提出的MSP数据集填补了高质量偏振图像融合数据集的空白，方法在多个数据集上表现出优越性能。
...
TL;DR: 研究探讨了如何利用放射学报告改进医学图像分类，发现在预训练阶段当标签与文本强相关时有益，但在弱相关时可能有害；在微调阶段使用报告能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像通常配有包含专家注释的放射学报告，但依赖这些报告需要放射科医生的手动工作。研究旨在探索何时以及如何在训练中利用这些报告来改进仅基于图像的分类。

Method: 系统研究了放射学报告在预训练和微调阶段的使用，涵盖诊断和预后任务，并在不同训练集规模下进行评估。

Result: 发现：(1) 当标签与文本强相关时，预训练阶段利用报告有益；但当标签与文本弱相关时，显式图像-文本对齐可能有害；(2) 微调阶段使用报告能带来显著改进，在某些情况下比预训练方法影响更大。

Conclusion: 研究为如何利用特权文本数据训练医学图像分类器提供了实用指导，同时指出了当前研究的空白。
...
<details>
  <summary>Details</summary>
Motivation: 卒中后MRI显示的继发性结构变化（如萎缩和脑室扩大）是恢复和预后的重要影像生物标志物，但现有监督分割方法难以有效捕捉这些异常

Method: 使用基于流的生成模型REFLECT进行无监督异常检测，在ATLAS数据集上使用双专家中心切片标注，通过Free-Response ROC分析评估异常图性能，比较在卒中患者无病灶切片和健康对照数据上训练的模型

Result: 在ATLAS测试对象上，使用健康对照数据训练的模型在病灶分割（Dice = 0.37 vs 0.27）和非病灶异常敏感性（FROC = 0.62 vs 0.43）方面均优于卒中患者数据训练的模型

Conclusion: 在完全健康的解剖结构上训练可以更好地建模正常变异性，从而实现对结构异常更广泛和可靠的检测
...
<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪中目标数量未知和时变的问题，特别是在保持目标身份一致性和处理非线性动态方面的挑战，以及如何有效应对弱检测器和遮挡情况。

Method: 采用混合跟踪方法结合随机和确定性策略，利用粒子群优化(PSO)引导粒子向目标分布模式移动，整合目标间的社会交互来增强跟踪，并建立基于空间一致性、外观、检测置信度、轨迹惩罚和社会评分的综合状态观测模型。

Result: 在标准基准测试和实际场景中，GenTrack相比最先进的跟踪器表现出优越性能，特别是在减少身份切换和轨迹丢失方面，尤其是在遮挡情况下。

Conclusion: GenTrack提供了一个有效且灵活的多目标跟踪解决方案，通过集成PSO和社会交互机制显著提升了跟踪性能，并提供了开源实现便于进一步研究和应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决未知和时变目标数量下的标识一致性问题，处理非线性动态和非高斯噪声，在目标交互和长时间遮挡时保持弱轨迹的稳定性。

Method: 使用随机粒子滤波处理非线性动态，结合PSO引导粒子；确定性关联通过成本矩阵确保标识一致性；提出平滑更新方案和速度回归机制。

Result: 实验结果表明，与最先进的跟踪器相比具有优越性能，适用于预录制视频和实时摄像头流。

Conclusion: 该方法通过随机和确定性机制的联合使用，在多目标跟踪中实现了更好的标识一致性和跟踪性能。
...
TL;DR: 该研究提出了一种基于卫星影像和机器学习的无传感器方法，用于监测黎巴嫩Qaraaoun水库的蓄水量，通过水面积分割和支持向量回归模型实现高精度体积估算。


<details>
  <summary>Details</summary>
Motivation: 由于传感器频繁故障和维护能力有限，需要开发不依赖地面测量的可靠方法来持续监测水库蓄水量。

Method: 使用Sentinel-2和Landsat卫星影像，结合新提出的水分割指数提取水面面积，然后训练支持向量回归模型，利用水面面积估算水库体积。

Result: 水面积分割准确率超过95%，SVR模型误差低于水库总容量的1.5%，决定系数超过0.98。

Conclusion: 该方法具有鲁棒性和成本效益，为水库蓄水量监测提供了实用的无传感器解决方案，可推广到其他水体监测。
...
Method: 开发了一个全面的评估框架，专门针对语义分割的可解释AI方法，采用像素级评估策略和精心设计的指标，使用基于类激活映射的XAI方案进行模拟验证。

Result: 模拟结果表明，所提出的方法在效率、鲁棒性和可靠性方面表现优异，能够提供细粒度的可解释性洞察。

Conclusion: 这项工作有助于推进透明、可信和可问责的语义分割模型的发展，为语义分割领域的可解释AI评估提供了系统性的解决方案。
...
TL;DR: 提出了一种基于功能分解的深度条件图像压缩框架（DCIC-sgp），通过自生成先验来分离图像的结构骨架和纹理细节，显著减少了低码率下的几何变形伪影。


<details>
  <summary>Details</summary>
Motivation: 当前学习型图像压缩方法难以有效建模自然图像中复杂的相关性结构，特别是全局结构与局部纹理的纠缠问题，导致低码率下出现严重几何变形。

Method: 使用自生成先验来封装图像的结构骨架，并深度调节整个压缩流程，特别是分析变换，使其专注于编码剩余的高熵细节，实现信息流的分层解耦。

Result: 在Kodak、CLIC和Tecnick数据集上，相比VVC测试模型VTM-12.1，分别实现了14.4%、15.7%和15.1%的BD-rate降低，显著缓解了低码率下的几何变形伪影。

Conclusion: 通过功能分解和深度条件调节，提出的DCIC-sgp框架有效解决了图像压缩中结构-纹理纠缠问题，在保持高率失真性能的同时显著改善了视觉质量。
...
TL;DR: 视频扩散模型(VDMs)通过时空数据预训练获得结构和动态的强归纳偏置，在视觉任务中比语言模型(LMs)表现出更高的数据效率和适应性。


<details>
  <summary>Details</summary>
Motivation: 语言模型在语言领域通过大规模预训练实现了快速适应新问题的能力，但这种成功尚未有效转化到视觉领域，视觉模型仍在组合理解、样本效率和通用问题解决方面存在困难。
...
TL;DR: 使用深度学习技术检测茶叶病害，评估SSD MobileNet V2和Faster R-CNN ResNet50 V1两种目标检测模型，并使用Mask R-CNN进行实例分割来计算叶片受损面积。


<details>
  <summary>Details</summary>
Motivation: 开发能够自动识别三种茶叶病害（红锈病、茶角盲蝽和红蜘蛛螨）并量化叶片受损区域的智能检测系统，以帮助茶农及时防治病害。

Method: 采用SSD MobileNet V2和Faster R-CNN ResNet50 V1进行目标检测，使用Mask R-CNN进行实例分割，并开发自定义方法来计算叶片受损区域面积。

Result: Faster R-CNN ResNet50 V1表现更好，mAP为25%，而SSD MobileNet V2的mAP为20.9%。同时成功实现了对叶片受损区域的量化分析。

Conclusion: Faster R-CNN ResNet50 V1在茶叶病害检测中表现优于SSD MobileNet V2，结合Mask R-CNN的实例分割技术可以有效量化病害对叶片的损害程度。
...
TL;DR: Kineo是一个完全自动化的、无需标定的多视角运动捕捉系统，使用未同步、未标定的消费级RGB相机，通过2D关键点同时标定相机和重建3D关键点，在精度和效率上大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的无标定运动捕捉方法需要精确的相机标定，限制了非专家用户的使用和野外场景的采集，且现有无标定方法存在计算成本高和重建精度低的问题。

Method: 利用现成的2D关键点检测器，通过置信度驱动的时空关键点采样策略和基于图的全局优化，同时标定相机（包括布朗-康拉迪畸变系数）并重建3D关键点和密集场景点云，使用成对重投影一致性评分量化3D重建可靠性。

Result: 在EgoHumans和Human3.6M数据集上的评估显示，相比现有方法，相机平移误差降低83-85%，相机角度误差降低86-92%，世界平均关节误差降低83-91%。处理效率高，特定配置下36分钟可处理1小时20分钟的素材。

Conclusion: Kineo提供了一个高效、准确的无标定运动捕捉解决方案，显著提升了无标定方法的性能，并开源了完整代码以促进实际应用。
...
Method: 提出解耦均值流方法，通过条件化扩散变换器的最终块到后续时间步，将预训练流模型直接重用作流映射，结合增强训练技术。

Result: 在ImageNet 256x256和512x256上，1步FID分别达到2.16和2.12，4步FID达到1.51和1.68，相比流模型推理速度提升100倍以上。
...
TL;DR: Latent Sketchpad是一个为多模态大语言模型添加内部视觉草稿纸的框架，通过将视觉生成集成到自回归推理过程中，使模型能够交替进行文本推理和视觉潜在表示生成，从而增强复杂场景下的视觉规划和想象能力。
...
Method: 该方法包含两个核心组件：上下文感知视觉头用于自回归生成视觉表示，预训练的草图解码器将这些表示渲染为人类可解释的图像。该框架将视觉生成直接集成到模型的原生自回归推理过程中。

Result: 在MazePlanning数据集上的实验表明，Latent Sketchpad在推理性能上达到甚至超越了其骨干模型。该框架在不同前沿MLLMs（包括Gemma3和Qwen2.5-VL）上都表现出良好的泛化能力。
...
Method: 设计新颖的自动代码生成管道创建工具，结合现有工具精选集，经过严格人工验证得到158个高质量工具，涵盖7个常见应用。

Result: MCP工具普遍提高任务成功率（如OpenAI o3从8.3%提升到20.4%，Claude 4 Sonnet从40.1%提升到43.3%），但即使最强模型工具调用率也仅36.3%。

Conclusion: OSWorld-MCP通过明确测量MCP工具使用技能，加深了对多模态代理的理解，为复杂工具辅助环境中的性能评估设定了新标准。
...
Method: 利用散射概率密度分布在投影域具有旋转对称性的物理先验，使用高斯RBF建模点散射函数并嵌入KAN网络层，结合散射光子分布的物理特性和KAN的复杂函数映射能力。

Result: 在合成和真实扫描实验中验证了方法的有效性，能够有效校正重建图像中的散射伪影，在定量指标上优于现有方法。

Conclusion: 该方法通过结合物理先验知识和深度学习，成功提升了CBCT散射伪影校正的性能。
...
TL;DR: 提出了一种用于人脸伪造检测的双分支卷积神经网络，结合空间和频域特征，通过注意力机制融合异构特征，使用统一损失函数提升检测性能。
...
Method: 双分支CNN架构：RGB分支捕获语义信息，频域分支关注高频伪影；引入通道注意力模块自适应融合异构特征；设计FSC统一损失函数结合焦点损失、监督对比损失和频域中心边界损失。

Result: 在DiFF基准测试中表现优异，涵盖文本到图像、图像到图像、人脸交换和人脸编辑四种伪造方法，性能超越平均人类准确率。

Conclusion: 该方法在检测人脸伪造方面具有有效性，为保护AI生态系统免受视觉伪造攻击做出了潜在贡献。
...
TL;DR: PathoGaze1.0是一个全面的病理学家行为数据集，记录了癌症诊断过程中完整的视觉搜索和决策过程，包括眼动追踪、鼠标交互和诊断决策数据。


<details>
  <summary>Details</summary>
Motivation: 病理学家解读千兆像素全玻片图像的诊断准确率约为70%，且增加第二位病理学家并不能显著提高决策一致性。该领域缺乏足够的行为数据来解释诊断错误和不一致性。

Method: 使用PTAH应用基础测试平台收集19位病理学家解读397张WSI的数据，包括眼动追踪、鼠标交互、刺激追踪、视口导航和诊断决策数据，强调生态效度。

Result: 共记录了18.69小时的数据，包括171,909次注视、263,320次扫视和1,867,362次鼠标交互事件。
...
TL;DR: 提出了Group Relative Attention Guidance方法，通过重新加权注意力机制中的delta值来实现对图像编辑程度的连续精细控制。
...
Method: 分析DiT模型中的MM-Attention机制，发现Query和Key令牌共享仅与层相关的偏置向量。将该偏置解释为模型的固有编辑行为，而每个令牌与其对应偏置之间的delta编码了内容特定的编辑信号。通过重新加权不同令牌的delta值来调节模型对输入图像相对于编辑指令的关注度。

Result: GRAG方法只需四行代码即可集成到现有图像编辑框架中，持续提升编辑质量。与常用的Classifier-Free Guidance相比，GRAG实现了更平滑和精确的编辑程度控制。

Conclusion: GRAG是一种简单而有效的方法，无需任何调优即可实现对编辑强度的连续精细控制，显著提升了图像编辑的质量和可控性。
...
TL;DR: 提出SAGE方法，一种零样本视频过渡技术，通过结构引导和生成合成实现平滑、语义一致的视频过渡


<details>
  <summary>Details</summary>
Motivation: 传统视频过渡方法在处理具有大时间间隔或显著语义差异的多样化片段时存在困难，需要内容感知和视觉连贯的过渡解决方案

Method: 结合结构引导（通过线条图和运动流提供）与生成合成，无需微调即可实现平滑过渡

Result: 在定量指标和用户研究中优于现有方法（FILM、TVG、DiffMorpher、VACE、GI）

Conclusion: SAGE方法能够有效处理多样化片段之间的过渡，在保持结构完整性和感知连续性方面表现优异
...
<details>
  <summary>Details</summary>
Motivation: 解决基础设施感知中现有摄像头检测模型性能不佳的问题，包括多视角设置、异构摄像头配置、视觉质量下降和不同道路布局等挑战。

Method: 提出基于Transformer的BEV感知框架，包含图增强融合模块，利用摄像头与BEV单元间的几何关系和潜在视觉线索，将多视角图像特征集成到BEV空间。

Result: 在M2I合成数据集和RoScenes真实数据集上的实验表明，MIC-BEV在3D目标检测中达到最先进性能，在极端天气和传感器退化等挑战条件下保持鲁棒性。

Conclusion: MIC-BEV展示了在真实世界部署的潜力，为智能交通系统提供可靠的基础设施感知解决方案。
...
TL;DR: 研究发现预训练的视觉变换器（ViTs）能够自然涌现出物体绑定能力，即识别哪些图像块属于同一物体的能力，这种能力在自监督模型中尤其明显，并主动引导注意力机制。


<details>
  <summary>Details</summary>
Motivation: 探索视觉变换器是否能够自然涌现出物体绑定能力，即识别图像中哪些部分属于同一物体的能力，这对于理解ViTs的内部表示机制具有重要意义。

Method: 使用相似性探针从ViT各层的补丁嵌入中解码IsSameObject属性，比较不同预训练目标（自监督vs监督学习）下的表现，并通过消融实验验证该信号的功能。

Result: 自监督ViTs（DINO、MAE、CLIP）能够可靠地涌现物体绑定能力，准确率超过90%，而ImageNet监督模型表现明显较弱；IsSameObject在物体特征之上的低维子空间中被编码，并主动引导注意力。

Conclusion: ViTs确实能够自然涌现物体绑定能力，这种能力不是架构的简单产物，而是通过特定预训练目标获得的，挑战了ViTs缺乏物体绑定的传统观点。
...
TL;DR: ProMoE是一个针对扩散变换器的MoE框架，通过两步路由器和显式路由指导来解决视觉MoE中的专家专业化问题，在ImageNet基准测试中超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有尝试将MoE应用于扩散变换器(DiTs)的效果有限，作者认为这是由于语言和视觉token的根本差异：语言token语义密集且变化明显，而视觉token存在空间冗余和功能异质性，阻碍了视觉MoE中的专家专业化。
...
Conclusion: 显式语义指导对视觉MoE至关重要，ProMoE通过两步路由机制有效促进了专家专业化，在扩散变换器中实现了更好的性能。
...
Method: URSA将视频生成任务制定为离散时空标记的迭代全局优化，核心设计包括线性化度量路径和分辨率相关时间步长偏移机制，并采用异步时间微调策略统一多种任务。

Result: 在具有挑战性的视频和图像生成基准测试中，URSA始终优于现有离散方法，并实现了与最先进连续扩散方法相当的性能。

Conclusion: URSA证明了离散生成建模在视频生成任务中的潜力，通过精心设计的机制成功解决了离散方法面临的关键挑战，为可扩展视频生成提供了新的解决方案。
...
TL;DR: 提出Generative View Stitching (GVS)方法，通过并行采样整个视频序列来解决自回归视频扩散模型在相机引导生成中的碰撞问题，实现与预定义相机轨迹的完全一致。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型虽然能生成稳定且与历史一致的长序列，但无法利用未来信息来指导当前生成，在预定义相机轨迹的场景中容易发生碰撞并导致生成崩溃。

Method: GVS采样算法将机器人规划中的扩散缝合技术扩展到视频生成，兼容任何使用Diffusion Forcing训练的现成视频模型，并引入Omni Guidance技术通过同时考虑过去和未来信息来增强时间一致性。

Result: GVS实现了稳定、无碰撞、帧间一致且能闭合循环的相机引导视频生成，适用于各种预定义相机路径，包括奥斯卡·路透斯瓦德的不可能阶梯。

Conclusion: GVS方法成功解决了自回归视频扩散模型在相机引导生成中的局限性，实现了与预定义相机轨迹完全一致的视频生成，同时保持了高质量的时间一致性。
...
TL;DR: 系统评估了增强记忆方法在长上下文对话任务中的效果，发现记忆增强方法能减少90%以上的token使用，同时保持竞争力准确率。记忆架构复杂度应与模型能力相匹配。
...
Method: 使用LoCoMo基准测试系统评估了完整上下文提示、通过检索增强生成的语义记忆和代理记忆、通过上下文学习的片段记忆、以及通过提示优化的程序记忆。

Result: 记忆增强方法减少了超过90%的token使用，同时保持竞争力准确率。小型基础模型从RAG中获益最多，而强大的指令调优推理模型从片段学习和更复杂的代理语义记忆中获益。

Conclusion: 记忆架构复杂度应该与模型能力相匹配，片段记忆可以帮助LLM识别自身知识的局限性。
...
TL;DR: BitSkip是一个混合架构框架，研究发现简单的8位量化模型（BitSkip-V1）在性能上优于更复杂的4位和Hadamard增强版本，甚至能与全精度基线竞争。


<details>
  <summary>Details</summary>
Motivation: 探索极端量化和动态路由等复杂技术在大型语言模型中的组合效应，这些方法的交互作用目前尚未得到充分理解。

Method: 引入BitSkip混合架构框架，系统研究不同量化方法和Hadamard变换的组合效果，包括8位量化、4位量化和Hadamard增强版本。

Result: BitSkip-V1（8位量化无Hadamard变换）在困惑度上达到1.13，优于全精度基线的1.19；Hadamard变换即使使用8位精度也会导致性能灾难性下降37,000%；BitSkip-V1在第18层提供最佳早期退出特性，实现32.5%速度提升，仅损失4%质量。

Conclusion: 简单的8位量化模型比复杂方法更有效，Hadamard变换会引发训练不稳定性，BitSkip-V1在速度和质量之间提供了最佳平衡。
...
TL;DR: 评估大语言模型处理文化相关语言的能力，特别是理解和实用化使用体现当地知识和文化细微差别的比喻表达。结果显示模型在阿拉伯语谚语、埃及阿拉伯语习语和英语谚语上的表现存在明显差异，表明比喻语言可作为文化推理的有效诊断工具。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs处理文化基础语言的能力，特别是理解和实用化使用体现当地知识和文化细微差别的比喻表达，以比喻语言作为文化细微差别和当地知识的代理。

Method: 设计评估任务包括上下文理解、实用化使用和内涵解释，在阿拉伯语和英语中评估22个开源和闭源LLMs，使用埃及阿拉伯语习语、多方言阿拉伯语谚语和英语谚语。

Result: 结果显示一致的层次结构：阿拉伯语谚语平均准确率比英语谚语低4.29%，埃及习语比阿拉伯语谚语低10.28%。实用化使用任务准确率比理解任务下降14.07%，但提供上下文习语句子可提高准确率10.66%。模型在内涵含义理解上最多达到85.58%与人类标注者的一致性。

Conclusion: 比喻语言可作为文化推理的有效诊断工具：虽然LLMs通常能解释比喻意义，但在适当使用方面面临挑战。发布了Kinayat数据集支持未来研究。
...
TL;DR: 本文分析了美国手语(ASL)在STEM对话中的动态变化，发现对话中的手势比孤立手势短24.6%-44.6%，并展示了对话特有的协调现象。
...
Method: 收集ASL STEM对话的动作捕捉数据集，使用连续运动学特征区分对话特有的协调与个体努力减少，分析STEM术语重复提及时的时空变化。

Result: 对话手势平均比孤立手势短24.6%-44.6%，在独白语境中不存在的显著减少。评估手语嵌入模型识别STEM手势和近似参与者随时间协调程度的能力。

Conclusion: 研究将语言分析与计算建模相结合，理解语用学如何塑造手语发音及其在手语技术中的表示。
...
TL;DR: CRADLE BENCH是一个用于多维度危机检测的基准测试，涵盖7种临床标准定义的危机类型，首次引入时间标签，包含600个临床医生标注的评估样本和420个开发样本，以及约4K个通过多语言模型多数投票自动标注的训练样本。


<details>
  <summary>Details</summary>
Motivation: 检测心理健康危机情况（如自杀意念、强奸、家庭暴力、儿童虐待和性骚扰）是语言模型面临的关键但未被充分探索的挑战。当这些情况在用户与模型交互中出现时，模型必须可靠地标记它们，因为未能这样做可能带来严重后果。

Method: 引入CRADLE BENCH基准测试，覆盖7种临床标准定义的危机类型，首次纳入时间标签。使用多语言模型多数投票集成自动标注约4K个训练样本，提供600个临床医生标注的评估样本和420个开发样本。在基于共识和一致集成协议定义的子集上微调了六个危机检测模型。

Result: 多模型多数投票集成在标注方面显著优于单模型标注。提供了在不同协议标准下训练的互补模型。

Conclusion: CRADLE BENCH为心理健康危机检测提供了一个全面且临床标准对齐的基准测试，通过多模型集成标注和不同协议标准的模型训练，为这一关键但未被充分探索的领域提供了重要工具。
...
TL;DR: 论文研究了LLM代理在时间感知方面的局限性，提出了TicToc-v1测试集来评估LLM在时间敏感场景下的工具调用决策能力，发现当前模型在时间感知方面表现不佳，需要专门的训练对齐。


<details>
  <summary>Details</summary>
Motivation: LLM代理在多轮对话环境中存在时间盲视问题，无法感知消息间的时间流逝，导致工具调用决策不当（过度依赖或忽略历史上下文），这在动态环境中成为关键限制。

Method: 引入TicToc-v1测试集，包含34个时间敏感场景的多轮对话轨迹；通过为对话消息添加时间戳来提供时间上下文；收集人类偏好数据创建prefer-noTool和prefer-Tool子集；评估LLM工具调用决策与人类偏好的对齐程度。

Result: 无时间信息时，大多数模型表现仅略优于随机猜测，最高对齐率约60%；添加时间戳后略有改善，特别是对较大模型，但改善有限，峰值约65%；基于提示的对齐方法效果有限。

Conclusion: 当前LLM在多轮工具使用中与人类时间感知存在显著差距，需要进行特定的训练后对齐来提升时间感知能力。
...
TL;DR: 提出Combo-Eval评估方法，结合多种现有方法的优势，用于评估LLM生成的表格结果自然语言表示，在保持评估保真度的同时显著减少25-61%的LLM调用次数。


<details>
  <summary>Details</summary>
Motivation: 在多轮对话代理等现代工业系统中，Text-to-SQL技术连接自然语言问题和数据库查询。将表格数据库结果转换为自然语言表示(NLR)可实现基于聊天的交互。目前NLR生成通常由大语言模型处理，但表格结果在自然语言呈现中的信息丢失或错误问题尚未得到充分探索。

Method: 提出Combo-Eval评估方法，结合多种现有评估方法的优势。同时创建了首个专门用于NLR基准测试的数据集NLR-BIRD。通过人工评估验证方法有效性。

Result: Combo-Eval在保持评估保真度的同时，显著减少了25-61%的LLM调用次数。人工评估表明该方法与人类判断具有优越的一致性，适用于有和无地面真值参考的多种场景。

Conclusion: Combo-Eval方法为LLM生成的表格结果自然语言表示提供了有效的评估方案，在减少计算成本的同时保持了评估质量，填补了该领域评估方法的空白。
...
TL;DR: OraPlan-SQL是一个用于Archer NL2SQL评估挑战赛的双语系统，采用代理框架，包含规划代理和SQL代理，通过反馈引导的元提示策略和计划多样化来提高性能。


<details>
  <summary>Details</summary>
Motivation: 解决复杂推理（如算术、常识和假设推理）在双语NL2SQL任务中的挑战，同时克服多子代理方法的编排开销问题。

Method: 使用两阶段代理框架：规划代理生成逐步自然语言计划，SQL代理将计划转换为可执行SQL。引入反馈引导的元提示策略来精炼单个规划代理，并采用计划多样化（多数投票）提高可靠性。

Result: 在Archer NL2SQL评估挑战赛2025中排名第一，执行准确率超过第二名6%以上（英文55.0%，中文56.7%），SQL有效性超过99%。

Conclusion: 通过反馈引导的元提示策略和计划多样化，OraPlan-SQL在双语NL2SQL任务中实现了最先进的性能，同时保持了系统的简洁性。
...
TL;DR: 提出一种轻量级框架，将冻结的大型语言模型适配于纵向临床数据分析，无需微调即可生成准确预测


<details>
  <summary>Details</summary>
Motivation: 利用大型语言模型分析纵向临床数据，特别是神经心理学评估，为早期阿尔茨海默病监测提供解决方案

Method: 在语言模型空间中整合患者历史和上下文信息，采用冻结模型架构避免模型微调

Result: 即使在训练数据极少的情况下，也能实现准确可靠的性能表现
...
TL;DR: 提出了AfriMTEB基准和AfriE5模型，旨在解决非洲语言在文本嵌入评估中的代表性不足问题，并在多语言任务上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 非洲语言在现有的多语言文本嵌入基准（如MMTEB）中代表性严重不足，现有任务大多基于翻译基准改编，缺乏针对非洲语言特点的专门评估。

Method: 1) 构建AfriMTEB基准，覆盖59种非洲语言、14个任务和38个数据集，包括6个新数据集；2) 开发AfriE5模型，通过跨语言对比蒸馏方法适配非洲语言。

Result: AfriE5模型在评估中表现出色，超越了Gemini-Embeddings和mE5等强基线模型，达到了最先进的性能水平。

Conclusion: AfriMTEB基准填补了非洲语言在文本嵌入评估中的空白，AfriE5模型为非洲语言处理提供了有效的嵌入解决方案，对提升多语言NLP系统的公平性具有重要意义。
...
TL;DR: 提出了一个用于评估LLM偏见的新增广框架，发现即使是最先进的LLM也容易受到输入扰动的影响，表现出更高的刻板印象倾向，特别是在研究较少的社群中。


<details>
  <summary>Details</summary>
Motivation: 现有偏见对齐方法存在脆弱性，需要开发更稳健的评估框架来测试LLM对刻板印象的敏感性。

Method: 设计了一个包含三个即插即用步骤的增广框架，应用于偏见基准数据集(BBQ)，通过输入扰动测试LLM的响应。

Result: 发现包括最先进的开源和闭源LLM都对输入扰动敏感，更容易表现出刻板印象行为，特别是在研究较少的社群中偏见更明显。

Conclusion: 需要将公平性和安全性研究扩展到更多样化的社群，当前偏见对齐方法存在局限性，LLM对输入扰动的敏感性揭示了其内在偏见问题。
...
TL;DR: 提出基于指令跟随LLM的自动化声明匹配方法，使用两阶段流程：先用LLM生成提示，再用LLM进行二元分类的声明匹配。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在声明匹配任务中的潜力，特别是通过自动生成提示来提升性能，并研究不同规模LLM在此过程中的表现差异。

Method: 采用两阶段流水线方法：第一步用LLM生成提示，第二步用LLM进行二元分类的声明匹配任务。支持使用不同LLM分别处理两个步骤。

Result: LLM生成的提示优于人工生成的SOTA方法；较小LLM在提示生成过程中表现与较大LLM相当，可节省计算资源；使用不同LLM分别处理两个步骤有效。

Conclusion: 该方法展示了LLM在声明匹配任务中的有效性，特别是自动提示生成的优势，为理解LLM对声明匹配任务的理解提供了新视角。
...
TL;DR: 提出了一种无需训练、自动生成和优化提示词的级联系统，用于评估电商产品质量，显著提升了精度和召回率，同时大幅减少了专家工作量。
...
Conclusion: 该自动提示级联系统在电商产品质量评估中有效，显著提升了性能并大幅降低了人工成本，具有良好的泛化能力。
...
TL;DR: 提出了一种基于连接组信息的LLM框架，将动态fMRI连接性编码为时间序列，通过稳健归一化处理后映射到预训练LLM中进行临床预测，在早期阿尔茨海默症检测中实现了高灵敏度预测。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够利用动态fMRI连接性数据进行早期阿尔茨海默症检测的方法，旨在实现比临床公认标准更低的错误率，为及时干预提供可能。

Method: 使用连接组信息LLM框架，将动态fMRI连接性编码为时间序列，应用稳健归一化技术，然后将这些数据映射到冻结的预训练LLM中进行临床预测。

Result: 在早期阿尔茨海默症检测中实现了高灵敏度预测，错误率显著低于临床公认标准。

Conclusion: 该方法在早期阿尔茨海默症检测方面表现出色，错误率远低于临床标准，具有重要的临床干预意义。
...
TL;DR: 该论文发现多语言大语言模型在遗忘学习时会出现语言混淆问题，导致基于参考的评估指标失效，并提出N-Mix分数来量化这一问题，并建议使用基于语义的评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明仅使用英语数据进行多语言知识遗忘是不够的，但分析过于性能导向。论文从评估角度出发，发现在多语言LLM使用并行多语言数据集进行微调后遗忘时，会出现语言混淆现象。

Method: 提出三步方法：1) 引入基于N-gram的语言混合(N-Mix)分数来量化语言混淆；2) 证明当N-Mix分数高时，基于参考的指标会产生假阴性；3) 建议需要能直接评估生成句子内容的新型遗忘评估方法。

Result: 发现语言混淆在多语言LLM中普遍存在且一致，基于参考的评估指标在语言混淆严重时会失效，产生错误的评估结果。
...
TL;DR: 提出M-Eval方法，通过异质性分析检查RAG系统在医疗问答中的事实错误，提高回答准确性达23.31%


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统在医疗问答中仍存在生成错误信息（如幻觉）和无法正确使用外部知识的问题

Method: 基于循证医学的异质性分析方法，从外部知识库提取额外医学文献，检查证据是否支持回答中的不同观点，并评估RAG系统提供证据的可靠性

Result: 在多种LLM上准确率提升最高达23.31%，能有效检测当前基于RAG的医疗系统中的错误

Conclusion: 该方法使LLM应用更加可靠，减少诊断错误，有助于提高医疗问答系统的可靠性
...
TL;DR: PICOs-RAG方法通过将用户查询扩展和规范化，使用PICO格式提取关键信息，显著提升了基于证据医学的检索增强生成系统的性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于证据医学的研究依赖人工查询文献数据库，缺乏客观性和效率。现有RAG方法在处理临床复杂查询时表现不佳，特别是当查询信息不完整或语言不精确时。

Method: 提出PICOs-RAG方法，将用户查询扩展和规范化为专业格式，使用PICO（患者、干预、比较、结果）格式提取关键信息用于检索。

Result: 该方法显著提升了检索效率和相关性，相比基线方法性能提升达8.8%。

Conclusion: PICOs-RAG将大语言模型提升为基于证据医学中有用且可靠的医疗助手。
...
Method: 结合多种循证医学方法模拟元分析，包括可靠性分析、异质性分析和外推分析，通过多原则筛选最佳医学证据供LLM诊断使用。

Result: 实验结果显示准确率提升高达11.4%，成功从PubMed数据集中提取更高质量和更可靠的证据。

Conclusion: 该方法能减少错误知识注入响应，帮助用户获得更有效的回复，提升循证医学应用的效果。
...
TL;DR: 提出TEXT2DB任务，将信息抽取结果与目标数据库集成，通过OPAL框架实现基于用户指令的数据库更新
...
Method: 提出OPAL框架，包含Observer、Planner、Analyzer三个组件，通过生成代码计划和调用IE模型来适配不同数据库模式

Result: OPAL能够成功适配不同的数据库模式，生成不同的代码计划并调用所需的信息抽取模型

Conclusion: 该任务能够有效解决信息抽取与数据库集成的挑战，但处理大型复杂数据库和抽取幻觉等问题仍需进一步研究
...
Conclusion: 提出的细粒度语义置信度奖励方法能够更精确地指导LLMs拒绝回答超出知识范围的问题，有效缓解幻觉问题。
...
TL;DR: 提出Speculative Knowledge Distillation (SpecKD)框架，通过动态令牌级门控机制，仅在教师模型接受的令牌上应用蒸馏损失，避免学习教师的不确定预测，提升学生模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法对所有令牌均匀应用蒸馏损失，迫使学生学习教师的不确定或高熵预测，这会引入噪声并损害学生性能，特别是当教师模型远大于学生模型时。

Method: 引入基于推测解码'提议-验证'范式的动态令牌级门控机制：学生生成令牌提议，与教师分布进行验证，仅对'接受'令牌应用蒸馏损失，'拒绝'令牌被屏蔽。

Result: 在多样化文本生成任务上的广泛实验表明，SpecKD持续显著优于强知识蒸馏基线，带来更稳定的训练和更强大的学生模型，达到最先进结果。

Conclusion: SpecKD是一种即插即用框架，通过选择性蒸馏机制有效提升知识蒸馏效果，特别是在教师模型远大于学生模型的情况下表现优异。
...
TL;DR: 提出了一种训练大型多模态模型形成语言约定的方法，通过模拟参考游戏让模型学会高效沟通，在人类交互中能减少41%消息长度并提高15%成功率。


<details>
  <summary>Details</summary>
Motivation: 人类通过共享对话上下文形成临时语言约定来实现高效沟通，希望让AI模型也能具备这种能力，从而与人类进行更有效的交互。

Method: 使用模拟参考游戏在模型之间进行训练，无需额外人工数据。在涉及照片和七巧板图像的重复参考游戏中训练模型形成约定。

Result: 模型与人类交互时消息长度减少41%，成功率提高15%。人类听众与形成约定的模型交互时响应更快。仅基于成功或成本训练都不足以引发约定形成。

Conclusion: 通过同时优化成功率和沟通成本，可以训练模型形成语言约定，实现与人类的高效沟通。
...
TL;DR: Pie是一个可编程的LLM服务系统，通过将传统生成循环分解为细粒度服务处理器，让用户程序控制生成过程，支持自定义KV缓存策略和生成逻辑。


<details>
  <summary>Details</summary>
Motivation: 现有LLM服务系统采用单一令牌生成循环，无法适应多样化推理策略和智能体工作流的需求。

Method: 将生成循环分解为服务处理器API，通过用户提供的inferlets程序控制生成过程，使用WebAssembly执行inferlets。

Result: 在标准任务上性能接近最优（3-12%延迟开销），在智能体工作流上延迟和吞吐量显著提升（1.3x-3.4x）。

Conclusion: Pie系统在保持高性能的同时提供了灵活的可编程性，能够有效支持复杂的LLM应用场景。
...
TL;DR: 提出了HalloMTBench多语言幻觉基准，用于诊断大型语言模型在机器翻译中的幻觉问题，涵盖11种语言方向，包含5,435个高质量实例。
...
Method: 构建了诊断框架，将幻觉分为指令脱离和源文本脱离两类，通过4个前沿LLM生成候选翻译，使用LLM评判团和专家验证来筛选高质量实例。

Result: 评估了17个LLM，发现了独特的"幻觉触发器"，包括模型规模、源文本长度敏感性、语言偏见和强化学习放大的语言混合问题。
...
TL;DR: 本文研究了孟加拉语方言的语音和形态特征，探索了为地区方言构建自动语音识别系统的可行性，并发布了相关数据集。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语具有丰富的方言多样性，但对其计算处理的研究有限。本研究旨在记录和分析这些方言的语音形态特征，推动包容性数字工具的发展。

Method: 通过记录和分析孟加拉语主要方言的语音和形态特征，探索构建针对地区方言的自动语音识别系统。

Result: 建立了孟加拉语方言数据集并公开发布，验证了为地区方言构建计算模型的可行性。

Conclusion: 这项研究有助于保护方言多样性，推进孟加拉语社区包容性数字工具的发展，为虚拟助手等语言技术应用提供了基础。
...
TL;DR: Squrve是一个统一、模块化且广泛的Text-to-SQL框架，通过标准化调用接口和多角色协作机制，将研究进展与实际应用相结合。


<details>
  <summary>Details</summary>
Motivation: 尽管Text-to-SQL技术在学术上取得了显著成果，但在真实系统中部署仍面临挑战，主要由于集成工具有限。

Method: 首先建立通用执行范式来标准化调用接口，然后基于七个抽象有效原子角色组件提出多角色协作机制。

Result: 在广泛采用的基准测试中，协作工作流持续优于原始个体方法。

Conclusion: 为处理复杂真实世界查询开辟了新的有效途径。
...
TL;DR: 使用强化学习训练的14B参数模型在法律文档搜索基准上表现优于前沿模型，准确率从78%提升至85%。研究表明多轮交互能进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的LLM代理方法在解决复杂任务时表现良好，但本研究旨在探索强化学习如何通过从经验中学习来显著提升模型能力。

Method: 在法律文档搜索基准上使用强化学习训练14B参数模型，并探索训练和测试时的轮次限制对性能的影响。

Result: RL训练的14B模型准确率达到85%，优于前沿模型的78%。多轮交互设置下性能进一步提升。

Conclusion: 强化学习能显著提升LLM代理的能力，多轮交互策略有助于获得更好的结果。
...
TL;DR: 本文提出了两种模式感知的文本过滤方法：模式感知行级去重（PLD）和模式感知尾随标点过滤（PTF），通过考虑文本序列分布来保留结构重要内容，在英语和韩语的小型语言模型训练中显著提升了性能。
...
Result: 在英语和韩语的1B参数小型语言模型训练中，这些方法在多项选择题基准测试中持续提升性能，并在SQuAD v1和KorQuAD v1的生成式问答准确性上显著提高。

Conclusion: 模式感知的过滤方法能够有效保留结构重要内容，显著提升语言模型的性能表现。
...
TL;DR: Ko-MuSR是首个全面评估韩语长篇叙事中多步骤软推理能力的基准，旨在最小化数据污染，包含完全韩语的叙事、推理链和多项选择题，并由人工验证逻辑一致性和可回答性。


<details>
  <summary>Details</summary>
Motivation: 为推进韩语自然语言处理，需要系统评估长上下文推理和提示策略，而现有基准在韩语多步骤软推理方面存在不足。

Method: 遵循MuSR框架构建，包含韩语叙事、推理链和多项选择题，通过人工标注验证逻辑一致性和可回答性，评估四种大型语言模型（两种多语言和两种韩语专用）。

Result: 多语言模型在韩语推理任务中表现优于韩语专用模型，表明推理能力具有跨语言泛化性。精心设计的提示策略（结合少量示例、推理轨迹和任务特定提示）显著提升准确率，接近人类水平。
...
TL;DR: MuSaG是首个德语多模态讽刺检测数据集，包含来自德国电视节目的33分钟人工标注数据，支持文本、音频和视频模态的评估。
...
Method: 创建MuSaG数据集，包含33分钟德国电视节目的人工选择和标注数据，每个实例提供对齐的文本、音频和视频模态，分别由人类标注。评估了9个开源和商业模型，涵盖文本、音频、视觉和多模态架构。

Result: 人类在对话环境中主要依赖音频线索，而模型在文本上表现最佳。这突显了当前多模态模型的差距，表明需要开发更适合现实场景的模型。

Conclusion: MuSaG数据集支持多模态讽刺检测和人类-模型对齐的未来研究，公开发布以促进相关领域发展。
...
TL;DR: 该论文研究了外部知识整合对自然语言生成的影响，通过创建KITGI基准测试，比较了完整外部知识和过滤知识条件下的句子生成效果，发现相关知识对保持生成内容的连贯性和概念覆盖率至关重要。


<details>
  <summary>Details</summary>
Motivation: 探索外部知识整合在自然语言生成中的作用，特别是常识生成任务中相关知识的重要性，旨在设计可解释的知识增强型NLG系统。

Method: 采用三阶段方法：(1)识别并移除关键知识，(2)重新生成句子，(3)人工评估输出的常识合理性和概念覆盖率，使用T5-Large模型在CommonGen数据集扩展的KITGI基准上进行实验。

Result: 完整知识条件下生成的句子在正确性方面达到91%，而过滤知识后性能急剧下降至6%，表明相关知识对保持生成质量至关重要。

Conclusion: 相关工作对维持NLG的连贯性和概念覆盖率具有关键作用，需要设计可解释的知识增强型NLG系统，并开发超越表面指标的评价框架来捕捉底层推理过程。
...
TL;DR: 提出了一种基于潜在空间语义对齐的LLM跨尺度知识迁移方法，通过激活值而非参数直接复用实现更有效的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在不同规模模型间细粒度知识迁移的挑战，由于神经不兼容性（架构和参数差异），现有直接复用层参数的方法受限。

Method: 以激活值作为层间知识迁移媒介，利用潜在空间语义对齐，实现跨尺度LLM的知识迁移。

Result: 在四个基准测试中表现优于现有方法，能更好地在不同规模模型间对齐模型行为。

Conclusion: 潜在空间语义对齐是跨尺度知识迁移的关键前提，该方法为LLM知识迁移提供了新思路。
...
TL;DR: 提出了一个基于知识和确定性两个维度的幻觉分类框架，通过模型特定数据集构建和激活操纵验证方法，揭示了不同类型幻觉需要针对性缓解策略的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要从外部特征对LLM幻觉进行分类，忽略了基于内部机制的针对性缓解策略需求。幻觉可能因知识缺失或确定性错误而产生，需要不同的处理方法。

Method: 提出基于知识和确定性的二维分类框架，通过模型特定数据集构建区分幻觉类型，使用激活操纵方法验证知识分类，并引入新的评估指标分析缓解方法效果。

Result: 验证了两种幻觉类型存在显著差异，发现模型间存在共享参数知识但仍产生不同幻觉的现象，识别出模型具有正确知识但确定性地产生幻觉的严重情况。

Conclusion: 幻觉分析必须同时考虑知识和确定性维度，需要针对幻觉底层因素的定向缓解方法，现有缓解方法在关键案例上表现不佳。
...
TL;DR: 研究发现LLM的解释忠实度受推理和训练选择影响，包括少样本示例的数量和质量、提示设计以及指令调优阶段，这些因素在医疗等敏感领域对提升模型可解释性和可信度至关重要。


<details>
  <summary>Details</summary>
Motivation: LLM生成的解释往往不能真实反映其预测依据，在医疗等敏感领域，这种不忠实会削弱临床医生的信任并导致不安全的决策支持。

Method: 评估三个LLM在BBQ和MedQA数据集上的表现，操纵少样本示例的数量和类型、提示策略以及训练过程。

Result: 少样本示例的数量和质量显著影响模型忠实度；忠实度对提示设计敏感；指令调优阶段提高了MedQA上的忠实度测量。

Conclusion: 这些发现为在敏感领域增强LLM可解释性和可信度提供了策略见解。
...
TL;DR: 提出了一种结合文本和语音信息的多模态方法来解决阿拉伯语方言的变音符号恢复任务，通过两种融合策略（早期融合和交叉注意力）实现，在训练中随机禁用语音输入以提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语方言句子中的变音符号恢复任务，利用多模态方法结合文本和语音信息来提高恢复准确性。

Method: 使用CATT预训练模型的编码器处理文本模态，OpenAI Whisper基础模型的编码器处理语音模态。采用两种融合策略：早期融合（将语音token与文本token合并）和交叉注意力融合。训练时随机禁用语音输入以增强模型鲁棒性。

Result: 在开发集上获得0.25的词错误率和0.9的字符错误率，在测试集上获得0.55的词错误率和0.13的字符错误率。

Conclusion: 提出的多模态方法在阿拉伯语方言变音符号恢复任务中表现良好，通过结合文本和语音信息以及鲁棒性训练策略，实现了较低的词错误率和字符错误率。
...
TL;DR: 比较五种大语言模型生成挪威语儿童对话的能力，发现大多数模型生成的语言比目标年龄段更复杂，GPT-4和NorBloom-7b表现相对较好。


<details>
  <summary>Details</summary>
Motivation: 由于大语言模型主要基于成人对话数据训练，在生成真实、符合儿童特点的对话方面面临挑战，特别是在低资源语言环境中。

Method: 通过盲评实验，让11位教育专业人士评估真实儿童访谈数据和LLM生成的文本样本，评估真实性和发展适宜性。

Result: 评估者表现出强的评分者间信度(ICC=0.75)，对5岁儿童年龄预测准确率高于9岁儿童。大多数模型生成的语言被认为比目标年龄段更复杂。

Conclusion: 研究揭示了为儿童开发LLM系统时面临的关键数据相关挑战，特别是在低资源语言环境中，缺乏全面的适龄词汇资源。
...
TL;DR: 本文提出了一种基于损失景观曲率的权重分解方法，能够识别和分离transformer模型中的记忆化表示，并通过权重编辑有效抑制非目标记忆化数据的复现。


<details>
  <summary>Details</summary>
Motivation: 基于理论和实证研究发现，记忆化训练点的损失景观曲率比非记忆化点更尖锐，这启发了通过曲率排序权重分量来区分记忆化内容，而无需显式标签。

Method: 使用基于损失景观曲率的权重分解方法，对语言模型和视觉transformer的权重进行编辑，抑制高曲率分量以移除记忆化内容。

Result: 权重编辑方法比最近的遗忘方法(BalancedSubnet)更有效地抑制非目标记忆化数据的复现，同时保持较低的困惑度。事实检索和算术任务性能受到显著负面影响，而开放式事实检索和一般逻辑推理能力得以保留。

Conclusion: 这项工作增强了对神经网络中记忆化的理解，提供了移除记忆化的实用方法，并证明了数学和事实检索等任务依赖于权重空间中特殊化、窄用途的结构。
...
TL;DR: 评估大型语言模型将自然语言指令转换为分层强化学习中出现的内部符号表示的能力，发现其性能对分区粒度和任务复杂度高度敏感


<details>
  <summary>Details</summary>
Motivation: 研究LLMs能否将人类自然语言指令转换为分层强化学习中出现的内部符号表示，这对发展学习智能体的规划和泛化能力至关重要
...
TL;DR: 提出MERGE方法，通过替换开放类词汇自动生成保持推理逻辑的NLI问题变体，评估模型在最小修改问题上的泛化能力，发现模型性能下降4-20%。


<details>
  <summary>Details</summary>
Motivation: 手动创建NLI泛化基准成本高，自动生成高质量变体困难，需要评估模型在保持推理逻辑的最小修改问题上的泛化能力。

Method: 通过替换开放类词汇自动生成NLI问题变体，同时保持底层推理逻辑不变，创建MERGE泛化测试基准。

Result: NLI模型在变体问题上的性能下降4-20%，表明即使在最小修改的问题上泛化能力也较差。分析了替换词类别、词概率和合理性对模型性能的影响。

Conclusion: 当前NLI模型在保持推理逻辑的最小修改问题上泛化能力不足，MERGE方法为评估模型泛化能力提供了有效工具。
...
TL;DR: 提出LATR方法解决强化学习中轨迹多样性不足问题，通过前瞻树分支策略提升策略学习效率和最终性能


<details>
  <summary>Details</summary>
Motivation: 当前强化学习管道中组rollout阶段采样的轨迹多样性有限，同质化轨迹会削弱策略更新的回报信号，阻碍有效策略学习

Method: LATR方法包含三个阶段：在高不确定性生成步骤进行分支、对每个新分支执行前瞻模拟、在模拟过程中剪除相似度过高的分支

Result: 相比随机采样，LATR平均加速策略学习131%，在GRPO和DAPO算法上提升最终pass@1性能4.2%

Conclusion: LATR通过显式促进轨迹级多样性，有效解决了强化学习中轨迹同质化问题，显著提升了策略学习效率和性能
...
Result: 在多个任务和模型上的实验表明，Critique-RL带来显著性能提升。例如，Qwen2.5-7B在领域内任务上获得9.02%提升，领域外任务上获得5.70%提升。

Conclusion: Critique-RL证明了无需强监督即可有效训练评论语言模型的可行性，为提升LLM在复杂推理任务中的表现提供了有前景的解决方案。
...
TL;DR: 该论文提出了一种评估大语言模型在阿拉伯语方言和文化内容上表现的方法，发现模型在方言内容上表现不佳，阿拉伯语模型在选择题表现好但在开放性问题表现差，思维链能提升判断正确性但n-gram指标结果不一。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在日常问答中应用日益广泛，但在文化基础和方言内容上的表现存在语言间的不均衡，需要评估模型在阿拉伯语方言上的表现差距。

Method: 将现代标准阿拉伯语选择题翻译成英语和多种阿拉伯语方言，转换为开放性问题，在零样本和微调设置下评估多种LLM，生成思维链理由来微调模型进行逐步推理。

Result: 模型在阿拉伯语方言上表现不佳，显示文化基础和方言知识的持续差距；阿拉伯语模型在选择题表现良好但在开放性问题表现差；思维链能提升判断正确性但n-gram指标结果不一。

Conclusion: 开发的数据集将公开发布以支持文化和语言包容性评估的进一步研究，揭示了模型在方言和文化内容评估中的局限性。
...
Method: 采用约束验证评估(CoV-Eval)方法：先定义可验证目标，然后系统生成相应的查询、文本材料和约束，确保任务既真实又可客观评估。支持自定义输入/输出长度(最高64K/8K tokens)和七种不同任务。

Result: 对23个LLMs的评估显示，即使最先进的模型在长文本生成方面也面临显著挑战，特别是随着现实复杂性和输出长度的增加。

Conclusion: LongWeave基准通过CoV-Eval方法有效评估了LLMs在长文本生成中的能力，揭示了现有模型在应对现实复杂约束时的局限性。
...
TL;DR: 该论文探索了在句子嵌入空间中学习文本简化转换的方法，通过小型前馈神经网络在嵌入空间中进行高复杂度到低复杂度文本的转换，取得了与大型模型相当的效果。


<details>
  <summary>Details</summary>
Motivation: 研究句子嵌入解码重构原始文本的效果，探索在嵌入空间中进行文本简化的可行性，旨在开发小型但强大的文本生成模型。

Method: 使用小型前馈神经网络学习高复杂度与低复杂度文本句子嵌入之间的转换，并与Seq2Seq和基于LLM的方法进行比较。

Result: 在未见过的简化数据集(MedEASI)和训练数据外的语言(西班牙语、德语)上验证了转换的有效性，展示了良好的泛化能力。

Conclusion: 在句子嵌入空间中学习转换是未来研究的有前景方向，有潜力开发小型但强大的文本简化和自然语言生成模型。
...
TL;DR: COMPEFFDIST是一个用于情感分析的综合高效蒸馏框架，通过自动指令构建和数据过滤，使3B学生模型在大多数任务上达到20倍大教师模型的性能，且仅需10%数据即可达到相同性能水平。
...
Result: 在多个模型系列(Llama-3、Qwen-3、Gemma-3)上应用该方法，使3B学生模型在大多数任务上匹配20倍大教师模型的性能，且在数据效率上大幅超越基线方法，仅需10%数据即可达到相同性能水平。
...
TL;DR: SynthWorlds框架通过构建结构相同但知识内容不同的真实映射世界和合成映射世界，来区分语言模型的推理能力和事实记忆能力，解决了现有评估方法无法清晰分离这两者的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型评估方法无法清晰区分推理能力和事实记忆能力，因为模型可以利用其参数化世界知识来回答问题，这使得基准测试表现往往反映的是事实回忆而非真正的推理能力。

Method: 构建两个结构相同但知识内容不同的平行语料库：真实映射世界（模型可利用参数化知识）和合成映射世界（此类知识无意义）。在此基础上设计镜像任务（多跳问答和页面导航），保持两个世界中的推理难度相同。

Result: 实验发现在仅参数化设置和知识增强设置中，模型从记忆的参数化世界知识中获得的知识优势差距持续存在。知识获取和整合机制可以减少但无法消除这一差距。
...
TL;DR: LuxIT是一个用于卢森堡语指令调优的新型单语数据集，旨在解决低资源语言环境下训练数据不足的问题。通过使用DeepSeek-R1-0528从原生卢森堡语文本合成数据，并采用LLM-as-a-judge方法进行质量保证。


<details>
  <summary>Details</summary>
Motivation: 指令调优的大型语言模型在低资源语言环境中效果有限，主要原因是缺乏高质量的训练数据。卢森堡语作为低资源语言，需要专门的数据集来提升模型性能。

Method: 从原生卢森堡语文本语料库合成数据集，使用DeepSeek-R1-0528（因其在卢森堡语方面表现优异），生成后采用LLM-as-a-judge方法进行质量保证，并在多个小规模LLM上进行微调。

Result: 在卢森堡语能力测试中，微调后的模型表现结果不一，不同模型的性能差异显著。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于伊斯兰指导，但存在错误引用文本、误用法学或产生文化不一致回答的风险，需要评估其可靠性和准确性。
...
Conclusion: 尽管GPT-4o表现相对较好，但所有模型在可靠生成准确伊斯兰内容和引用方面仍有不足，需要以穆斯林视角为中心的社区驱动基准，这是迈向更可靠AI的重要一步。
...
TL;DR: 这是一篇关于在数据稀缺场景下微调大语言模型的实用调查论文，系统回顾了参数高效微调、领域和跨语言适应、模型专业化策略以及偏好对齐方法。


<details>
  <summary>Details</summary>
Motivation: 解决在低资源语言、专业领域和受限部署环境中，使用有限数据微调大语言模型的实际挑战。

Method: 系统性地调查了参数高效微调技术、领域和跨语言适应方法、模型专业化策略以及偏好对齐方法。

Result: 提供了基于任务约束选择合适技术的经验权衡、选择标准和最佳实践，包括模型缩放、数据缩放和缓解灾难性遗忘。
...
TL;DR: 本文提出了一种新的对抗性改写任务，通过生成语义等效但能降低分割性能的文本查询来评估多模态大语言模型在推理分割任务中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注图像输入的扰动，而语义等效的文本改写在实际应用中很常见但研究不足，需要评估模型对多样化用户表达的鲁棒性。

Method: 提出了SPARTA方法，这是一种在黑盒设置下运行的句子级优化方法，利用文本自编码器的低维语义潜在空间，并通过强化学习进行指导。

Result: SPARTA在两个数据集（ReasonSeg和LLMSeg-40k）上取得了显著更高的成功率，比现有方法高出最多2倍。

Conclusion: 先进的推理分割模型即使在严格的语义和语法约束下，仍然容易受到对抗性改写的攻击，揭示了这些模型在鲁棒性方面的重要漏洞。
...
TL;DR: 本文提出了一个针对多语言或非英语场景的基准测试分类法，并推荐了欧洲语言基准测试开发的最佳实践和质量标准。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力的不断提升，现有的基准测试主要关注英语，而在非英语语言中的评估仍是一个未被充分探索的领域。

Method: 首先概述了LLM基准测试的最新发展，然后提出了一个专门针对多语言或非英语使用场景的基准测试分类法。

Result: 提出了一个协调欧洲语言基准测试开发的最佳实践和质量标准框架。

Conclusion: 建议提高评估方法的语言和文化敏感性，以促进更协调的多语言基准测试开发。
...
TL;DR: PerFine是一个无需训练的统一框架，通过基于配置文件的迭代反馈来增强个性化文本生成，在多个数据集上显著提升个性化质量。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法在个性化文本生成中经常产生风格、语调或主题漂移的问题，需要更有效的个性化对齐机制。

Method: 采用训练免费的批判-精炼框架：LLM生成器基于检索配置文件生成草稿，批判LLM提供结构化反馈，生成器进行修订，并通过淘汰策略保留更好的版本。

Result: 在Yelp、Goodreads和Amazon数据集上，PerFine相比PGraphRAG将GEval指标提升了7-13%，在3-5次精炼迭代中持续改进，且可随批判模型规模扩展。

Conclusion: 基于配置文件的后期反馈为个性化LLM生成提供了强大范式，该方法无需训练且模型无关。
...
Conclusion: RAG和推理增强在缓解LLM幻觉方面具有重要协同作用，系统整合这两种方法能更好地平衡创造性和可靠性。
...
TL;DR: 提出了Talk2Ref数据集和RPT任务，用于从科学讲座中自动预测相关论文引用，通过双编码器架构显著提升了引用预测性能。
...
Method: 构建了包含6,279个讲座和43,429篇引用论文的Talk2Ref数据集，评估了最先进的文本嵌入模型的零样本检索性能，并提出了基于Talk2Ref训练的双编码器架构，探索了处理长文本和领域适应的策略。

Result: 在Talk2Ref上进行微调显著提高了引用预测性能，证明了任务的挑战性以及从口语科学内容中学习语义表示的有效性。
...
TL;DR: 提出了一种基于词联想网络的LLM隐式偏见评估方法，通过模拟语义启动效应来检测LLM中的社会偏见，并与人类偏见进行比较。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的广泛应用，其内在的社会偏见问题日益突出。由于这些偏见往往是隐性的而非显性的，需要开发能够评估LLM隐式知识表示的方法。

Method: 基于词联想网络的提示方法，模拟LLM生成的词联想网络中的语义启动效应，从量化和定性角度评估偏见。

Result: 在性别、宗教、种族、性取向和政治党派等社会偏见维度上，发现了LLM与人类偏见之间的趋同和分歧。

Conclusion: 该方法为评估和比较多个LLM与人类偏见提供了系统化、可扩展和可推广的框架，有助于推进透明和负责任的语言技术发展。
...
TL;DR: 提出使用自然语言批评来增强大语言模型的置信度校准，包括Self-Critique和CritiCal两种方法，其中CritiCal通过批评校准训练显著提升置信度校准效果。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以准确评估大语言模型的置信度，而精确的黄金置信度标签难以获取，需要多轮生成。自然语言批评能有效解决置信度校准问题。

Method: 提出两种方法：Self-Critique让LLM自我批评优化置信度；CritiCal是新颖的批评校准训练方法，利用自然语言批评改进置信度校准，超越直接数值优化。

Result: CritiCal显著优于Self-Critique和其他基线方法，在复杂推理任务中甚至超越其教师模型GPT-4o，在分布外设置中也表现出稳健的泛化能力。

Conclusion: 自然语言批评能有效提升LLM的置信度校准，CritiCal方法在复杂任务中表现出色，增强了LLM的可靠性。
...
TL;DR: 本文提出了一种用于词性标注的局部消歧方法，旨在实现零静默率（正确标签永不丢失），并分析了在INTEX系统中实现该方法时需要考虑的路径交互和组合效应。


<details>
  <summary>Details</summary>
Motivation: 解决词性标注中的歧义问题，确保在自然语言处理系统中正确标签不会被丢弃，实现零静默率的目标。

Method: 基于Silberztein的INTEX系统，使用局部消歧语法和转换器路径，强调需要验证路径间的交互作用。

Result: 发现单独考虑转换器路径不足以验证局部消歧语法，必须考虑它们的交互作用；组合多个转换器时结果无法孤立预测。

Conclusion: 为实现零静默率，局部语法需要仔细测试，需要详细规范语法在文本应用时的行为，因为语法直觉可能因未预见的构造或歧义而不准确。
...
<details>
  <summary>Details</summary>
Motivation: 韩语历史存在口语与书面语差异，以及从汉字到韩文的关键转变，但由于缺乏可访问的历史语料库，这一语言演变在NLP领域尚未得到充分探索。

Method: 构建开放韩语历史语料库，收集19个来源的数据，时间跨度从7世纪到2025年，涵盖韩式汉文（Idu）和汉字-韩文混合书写等系统。

Result: 量化分析发现：(1) Idu使用在1860年代达到顶峰后急剧下降；(2) 从汉字到韩文的转变始于1890年左右，是快速转型；(3) 朝鲜词汇差异导致现代分词器产生高达51倍的未登录词率。

Conclusion: 该语料库为韩语历史的定量历时分析提供了基础资源，也可作为大语言模型的预训练语料，提升其对现代韩文中的汉字词和古书写系统的理解。
...
<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在多语言训练后，在领域外和低资源场景下表现不佳，特别是在缺乏标注数据的情况下

Method: 提出BEARD框架，结合BEST-RQ目标和冻结教师编码器的知识蒸馏，使用未标注数据适配Whisper编码器，确保与预训练解码器的互补性

Result: 在ATCO2航空交通管制语料库上，使用5000小时未标注语音和2小时标注语音，相比微调模型获得12%的相对性能提升
...
<details>
  <summary>Details</summary>
Motivation: 为了评估AI代理在科学研究中的可靠性，需要测试其能否忠实地复制整个研究论文的核心贡献。

Method: 将论文分解为需要复制的任务，包括实验设置、推导、数据分析和代码库，每个任务都与原论文作者共同开发，针对关键科学结果进行客观评估。

Result: 当前最先进的语言模型在ReplicationBench上表现不佳，最佳模型得分低于20%，发现了丰富的失败模式。

Conclusion: ReplicationBench建立了首个论文规模、专家验证的天体物理学研究任务基准，揭示了AI代理在科学研究中的局限性，并为衡量其在科学研究中的可靠性提供了可扩展框架。
...
TL;DR: 提出了ReForm方法，通过集成语义一致性评估和迭代修正来解决自动形式化中的语义保真问题，在四个基准测试中平均提升17.2个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法将自动形式化视为简单翻译任务，缺乏自我反思和迭代修正机制，导致生成的正式语句无法保持原始问题的语义意图。

Method: 提出ReForm反思性自动形式化方法，集成语义一致性评估，使模型能够迭代生成正式语句、评估语义保真度并通过渐进修正自我纠正错误。使用前瞻有界序列优化(PBSO)训练模型，在不同序列位置使用不同奖励来确保准确的自动形式化和语义验证。

Result: 在四个自动形式化基准测试中，ReForm相比最强基线平均提升17.2个百分点。还引入了ConsistencyCheck基准，包含859个专家标注项目，验证LLM作为评判者的可靠性。

Conclusion: 自动形式化具有内在难度，即使人类专家也会在高达38.5%的情况下产生语义错误。ReForm方法通过反思机制显著提升了自动形式化的性能。
...
TL;DR: 提出dLLM-Var方法，让基于扩散的大语言模型能够原生支持可变生成长度，解决了传统dLLMs必须预先设定生成长度的问题，实现了30.1倍的速度提升。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的大语言模型存在固定生成长度的问题，必须在解码前确定生成长度作为超参数，这导致了效率和灵活性的问题。

Method: 训练模型准确预测[EOS]标记，使dLLM能够以块扩散方式原生推断，同时保持全局双向注意力和高并行性。

Result: 在标准基准测试中，相比传统dLLM推理范式实现了30.1倍加速，相比自回归模型如Qwen和Llama实现了2.4倍加速，同时达到更高准确度。

Conclusion: 该方法实现了更高准确度和更快推理，将dLLMs从单纯的学术新颖性提升到支持实际应用的水平。
...
<details>
  <summary>Details</summary>
Motivation: 现有静态稀疏方法（如滑动窗口或全局token）由于静态性无法适应注意力内容依赖的变化，而动态方法依赖预定义模板或启发式机制，限制了通用性并可能剪枝掉重要的上下文token。

Method: DHSA自适应地将序列分割为变长块，通过长度归一化聚合计算块表示，然后将块级相似度上采样到token级相似度来计算重要性分数，动态确定需要保留的token级交互。

Result: 在Gemma2上的实验表明，DHSA在保持密集注意力准确性的同时，将预填充延迟降低20-60%，峰值内存使用降低35%。相比块稀疏注意力等基线方法，DHSA实现了6-18%的相对准确率提升。

Conclusion: DHSA为长上下文设备端LLM提供了高效且适应性强的解决方案，在保持准确性的同时显著降低了计算和内存成本。
...
TL;DR: 本文系统研究了三种基于前缀的方法在零样本跨语言迁移中的表现，发现这些方法在Llama和Mistral等解码器模型上优于LoRA基线，特别是在低资源多语言场景中。


<details>
  <summary>Details</summary>
Motivation: 随着Llama和Mistral等大型语言模型的发布，零样本跨语言迁移变得更加可行，但将这些仅解码器模型适应新任务仍具挑战性。参数高效微调技术如LoRA被广泛使用，但基于前缀的技术在仅解码器模型中的零样本迁移研究较少。

Method: 使用三种基于前缀的方法（软提示调优、前缀调优和Llama Adapter）进行从英语到35+种高资源和低资源语言的零样本跨语言迁移研究，分析了跨语言家族和文字的影响，以及从1B到24B模型规模的扩展效果。

Result: 在Llama 3.1 8B上，前缀方法比LoRA基线在Belebele基准上高出6%；在Mistral v0.3 7B上也观察到类似改进；尽管前缀调优仅使用1.23M学习参数，但在多样化基准上实现了一致的改进。
...
TL;DR: 本文提出相对缩放定律，研究不同测试分布之间的性能差距如何随模型规模变化，发现缩放并非万能均衡器，不同领域的性能差异呈现多样化轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统的缩放定律基于聚合测试集，平均了异构子群体的性能差异。作者希望揭示缩放过程中不同测试分布间的相对性能变化，以更好地理解模型性能的异质性。

Method: 训练了255个解码器Transformer模型，在匹配计算预算（10^18-10^20 FLOPs）下使用标准预训练数据集，分析不同测试分布间的相对性能差距演变。

Result: 发现多样化轨迹：学术领域趋于均衡；区域英语方言性能取决于人口规模；AI风险行为分裂，能力和影响相关风险在预训练中增加，而对抗性风险不变。

Conclusion: 缩放虽然提升整体性能，但不是普遍均衡器。作者发布所有模型检查点，鼓励研究者同时测量相对和传统缩放定律，以更好地优先处理鲁棒性挑战。
...
<details>
  <summary>Details</summary>
Motivation: 对话代理无法识别用户的修复发起，导致对话中断或用户脱离。人类对话中，修复特别是他人发起修复对于维持相互理解至关重要。

Method: 整合语言学和韵律特征，基于对话分析，结合预训练的文本和音频嵌入，构建多模态模型来检测修复发起。

Result: 韵律线索补充了语言特征，显著改善了预训练文本和音频嵌入的结果，揭示了不同特征如何相互作用。

Conclusion: 多模态方法能有效检测对话修复发起，未来方向包括整合视觉线索，探索多语言和跨语境语料库以评估鲁棒性和泛化能力。
...
TL;DR: OpenRM是一个工具增强的长文本奖励模型，通过调用外部工具收集相关证据来系统评估开放式回答，显著提升了在知识密集型任务上的表现。
...
Method: 使用Group Relative Policy Optimization (GRPO)在超过27K个合成的成对示例上训练OpenRM，训练目标同时监督中间工具使用和最终结果准确性，激励奖励模型学习基于证据的判断策略。

Result: 在三个新收集的数据集和两个广泛使用的基准测试中，OpenRM显著优于现有的奖励建模方法。将其集成到推理时回答选择和训练时数据选择中，在下游LLM对齐任务中带来了一致的性能提升。
...
TL;DR: 通过眼动追踪研究，量化了阅读障碍者在阅读过程中的额外时间成本，发现他们对词汇长度、频率和可预测性更敏感，其中可预测性的影响最大。


<details>
  <summary>Details</summary>
Motivation: 探究在什么条件下阅读障碍者会产生额外的阅读成本，以及这些成本的大小，为干预措施和计算模型提供指导。

Method: 使用大规模自然阅读数据集，结合眼动追踪和词汇特征（词汇长度、频率、可预测性），建模这些特征对阅读障碍者阅读时间的影响。

Result: 所有三个词汇特征都会影响典型读者和阅读障碍者的阅读时间，但阅读障碍者对每个特征都表现出更强的敏感性，尤其是可预测性。通过反事实操作这些特征，阅读障碍与控制组之间的差距缩小了约三分之一。
...
<details>
  <summary>Details</summary>
Motivation: 随着RAG技术普及，信息检索的角色从为人类用户检索转向为AI系统提供上下文知识，相关性难以预先定义或标注。

Method: R3框架通过检索结果与环境交互产生对比信号，自动指导检索器的自我改进，无需依赖标注或合成数据进行监督微调。

Result: 在多样化任务上的实验表明，R3比原始检索器提升5.2%的RAG性能，超越最先进检索器4.9%，达到与基于后训练或指令调优LLM的检索和RAG系统相当的结果。

Conclusion: R3是一个高效实用的框架，仅需4个GPU并在一天内完成训练，为RAG系统提供了无需标注数据的动态相关性优化方案。
...
TL;DR: 提出了一个基于强化学习的框架，训练大型语言模型作为诊断代理，能够在多轮诊断过程中管理诊断流程、自适应选择检查项目并做出最终诊断。


<details>
  <summary>Details</summary>
Motivation: 传统的指令调优模型在静态病例摘要上训练，无法获得动态诊断策略。本文旨在通过交互式探索和基于结果的反馈，让模型学习真实的诊断策略。

Method: 开发了DiagGym诊断世界模型作为虚拟临床环境，使用电子健康记录训练；通过端到端多轮强化学习训练DiagAgent；构建了DiagBench诊断基准数据集。

Result: DiagAgent在多种诊断设置中显著优于10个最先进的LLM和两个提示工程代理。在单轮设置中诊断准确率提高9.34%，检查推荐命中率提高44.03%；在端到端设置中诊断准确率提高15.12%，检查推荐F1分数提高23.09%。
...
Method: 采用两阶段的MQM重新标注范式，标注者审查和编辑已有的MQM标注（可能来自自己、其他人类标注者或自动系统）。

Result: 重新标注行为符合预期目标，能产生更高质量的标注，主要原因是发现了首次标注时遗漏的错误。

Conclusion: MQM重新标注是提升机器翻译评估质量的有效方法，能够更准确地捕捉翻译质量改进。
...
TL;DR: 提出了InteractComp基准来评估搜索代理在查询模糊时主动交互解决的能力，发现现有模型存在系统性过度自信问题，强制交互能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有搜索代理假设用户查询是完整明确的，但现实中用户往往从模糊查询开始，需要交互澄清。现有基准无法评估这种交互能力。

Method: 采用目标-干扰方法构建210个专家策划的问题，涵盖9个领域，创建只能通过交互解决的真正模糊性。评估了17个模型，包括强制交互实验。

Result: 最佳模型准确率仅13.73%（完整上下文下为71.50%），显示系统性过度自信而非推理缺陷。强制交互能显著提升性能。纵向分析显示交互能力15个月内停滞，而搜索性能提升7倍。

Conclusion: 搜索代理存在交互能力盲点，InteractComp为评估和训练搜索代理的交互能力提供了宝贵资源。
...
TL;DR: 该研究评估了基于提示的角色扮演(PBRP)对大型语言模型医学推理能力的影响，发现角色提示主要影响语言风格而非推理能力，未能真正模拟临床角色的认知差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究基于提示的角色扮演(PBRP)方法是否真的能引导LLMs产生不同临床角色的认知过程，还是仅仅改变语言风格。

Method: 提出RP-Neuron-Activated评估框架(RPNA)，在三个医学问答数据集上使用神经元消融和表示分析技术来评估推理路径的变化。

Result: 结果显示角色提示不会显著增强LLMs的医学推理能力，主要影响表层语言特征，没有证据表明不同临床角色之间存在不同的推理路径或认知差异。
...
TL;DR: SPICE是一个强化学习框架，通过让单一模型扮演挑战者和推理者两个角色，在大型语料库环境中进行自我对抗训练，实现持续的自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有的无基础自我对弈方法改进效果有限，需要环境交互来实现持续适应。SPICE旨在通过语料库基础提供丰富、几乎无穷尽的外部信号，实现持续自我改进。

Method: 单一模型扮演两个角色：挑战者从大型语料库中挖掘文档生成多样化推理任务，推理者解决这些任务。通过对抗动态，挑战者在推理者能力边界创建自动课程。

Result: 在多个模型系列上，SPICE在数学推理基准上获得+8.9%的提升，在一般推理基准上获得+9.8%的提升，显著优于现有方法。

Conclusion: 文档基础是SPICE持续生成日益挑战性目标并实现这些目标的关键要素，能够实现持续的自我改进。
...
TL;DR: 提出基于最近发展区理论的AgentFrontier引擎，通过自动合成高质量多学科数据来训练LLM代理，在复杂推理任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 在LLM能力边界上训练代理是解锁高级推理的关键，但缺乏针对这一边界的高质量训练数据。

Method: 基于最近发展区理论构建AgentFrontier引擎，自动合成位于LLM能力边界的高质量多学科数据，支持预训练和后训练。

Result: 训练的AgentFrontier-30B-A3B模型在Humanity's Last Exam等基准测试中达到SOTA，甚至超越部分领先专有代理。

Conclusion: 最近发展区指导的数据合成方法为构建更强大LLM代理提供了可扩展且有效的路径。
...
Method: 将信息搜索建模为树状结构推理问题，在受限上下文中嵌入更多目标实体。利用维基百科表格提出三种任务合成变体：基础型、联合型和反向联合型，系统提升搜索效率和效果。通过保留同时准确且高效的训练轨迹来优化模型。

Result: 在五个信息搜索基准测试（BrowserComp、GAIA、xbench-DeepSearch、WideSearch和Seal-0）上的广泛实验表明，该方法在基础设置和综合设置下均能持续提升效果和效率。

Conclusion: WebLeaper框架通过系统化的任务构建和轨迹优化，有效解决了LLM智能体在信息搜索中的效率问题，在多个基准测试上实现了效果和效率的双重提升。
...
TL;DR: ParallelMuse是一个两阶段范式，通过功能指定部分展开和压缩推理聚合来提高深度信息搜索代理的问题解决能力，在减少30%探索令牌消耗的同时实现高达62%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统并行思维在信息搜索代理中面临两个关键挑战：从头开始重复展开的低效率，以及由于上下文容量限制而难以在答案生成过程中整合长视野推理轨迹。

Method: 提出ParallelMuse两阶段方法：第一阶段功能指定部分展开将生成序列划分为功能区域，执行不确定性引导的路径重用和分支；第二阶段压缩推理聚合利用推理冗余无损压缩与答案推导相关的信息，并合成连贯的最终答案。

Result: 在多个开源代理和基准测试上的实验表明，该方法实现了高达62%的性能提升，同时减少了10-30%的探索令牌消耗。

Conclusion: ParallelMuse通过高效的探索和推理整合，显著提升了深度信息搜索代理的问题解决能力，为解决传统并行思维的局限性提供了有效方案。
...
TL;DR: AgentFold是一种基于LLM的网页代理新范式，通过主动上下文管理解决长时任务中的上下文饱和问题，采用多尺度折叠操作来动态管理历史轨迹，在BrowseComp基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有ReAct代理在长时信息搜索任务中面临上下文饱和问题，而固定摘要方法可能导致关键细节丢失，需要一种更智能的上下文管理方法。

Method: 受人类回顾性认知过程启发，AgentFold将上下文视为动态认知工作空间，通过'折叠'操作在多尺度上管理历史轨迹：细粒度压缩保留关键细节，深度整合抽象多步子任务。

Result: 仅通过监督微调，AgentFold-30B-A3B在BrowseComp上达到36.2%，在BrowseComp-ZH上达到47.3%，超越了更大规模的开源模型和领先的专有代理。

Conclusion: AgentFold通过主动上下文管理有效解决了长时任务中的上下文饱和问题，证明了智能上下文管理对提升代理性能的重要性。
...
TL;DR: Tongyi DeepResearch是一个专为深度信息检索任务设计的智能代理大语言模型，通过端到端训练框架实现自主深度研究能力，在多个基准测试中达到最先进水平。
...
Method: 采用端到端训练框架，结合智能代理中期训练和后期训练，设计高度可扩展的自动数据合成流程，构建定制化环境实现稳定交互。

Result: 模型拥有305亿总参数，每token仅激活33亿参数，在多个深度研究基准测试中取得最先进性能。
...
TL;DR: 提出了agent data protocol (ADP)作为异构格式agent数据集与统一训练管道之间的中间表示语言，解决了agent训练数据碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 大规模监督微调AI agent的公共研究成果较少，主要因为agent训练数据收集面临独特挑战，数据碎片化在不同格式、工具和接口中。

Method: 设计ADP轻量级表示语言作为中间语言，能够表达多种任务类型（API/工具使用、浏览、编码、软件工程等），同时保持简单易解析。将13个现有agent训练数据集统一转换为ADP格式。

Result: 通过SFT实验，相比基础模型平均性能提升约20%，在标准编码、浏览、工具使用和研究基准上达到SOTA或接近SOTA性能，无需领域特定调优。

Conclusion: ADP有助于降低标准化、可扩展和可复现agent训练的门槛，所有代码和数据已公开。
...
TL;DR: ComboBench是一个评估大语言模型将语义动作转换为VR设备操作序列能力的基准测试，涵盖4款流行VR游戏的262个场景。测试发现顶尖模型在任务分解方面表现良好，但在程序推理和空间理解上仍落后于人类。
...
Method: 创建ComboBench基准测试，评估7个LLM在4款VR游戏（Half-Life: Alyx, Into the Radius, Moss: Book II, Vivecraft）的262个场景中将语义动作转换为设备操作序列的能力，并与人工标注和人类表现进行对比。

Result: Gemini-1.5-Pro等顶尖模型展现出强大的任务分解能力，但在程序推理和空间理解方面仍不及人类。不同游戏间性能差异显著，表明对交互复杂度的敏感性。少量示例能显著提升性能。

Conclusion: 大语言模型在VR设备操作转换方面具有潜力，但需要在程序推理和空间理解方面进一步改进。few-shot学习是提升模型VR操作能力的有效方法。
...
TL;DR: 提出了MetricX-25和GemSpanEval两个模型，分别用于WMT25翻译评估任务的质量评分预测和错误跨度检测子任务，基于Gemma 3模型进行微调。


<details>
  <summary>Details</summary>
Motivation: 参与WMT25翻译评估共享任务，改进翻译质量评估方法，特别是质量评分预测和错误跨度检测两个子任务。

Method: 使用Gemma 3多语言模型进行微调：MetricX-25采用编码器架构加回归头预测MQM和ESA质量分数；GemSpanEval采用解码器架构，将错误跨度检测作为生成任务，输出错误跨度及其严重程度和类别。

Result: MetricX-25显著优于前代模型；GemSpanEval在错误跨度检测上与xCOMET基线模型竞争，并能明确识别错误跨度上下文。

Conclusion: 基于Gemma 3的微调方法在翻译评估任务中表现优异，MetricX-25在质量评分预测上取得进步，GemSpanEval展示了生成式错误跨度检测的可行性。
...
Result: 在两个公开数据集上，模型在7K数据集上的平均精度、召回率、F1分数、AUC和测试准确率分别为0.9884±0.0151、0.9900±0.0151、0.9900±0.0145、0.9999±0.00006和0.9905±0.0025；在100K数据集上分别为0.9904±0.0091、0.9900±0.0071、0.9900±0.0071、0.9997±0.00016和0.9902±0.0006。
...
TL;DR: Game-TARS是一个通用游戏智能体，使用统一、可扩展的键盘鼠标动作空间进行训练，通过大规模跨域预训练在多个游戏环境中表现出色。
...
Result: 在开放世界Minecraft任务中成功率是之前最佳模型的2倍，在未见过的网页3D游戏中接近人类水平，在FPS基准测试中优于GPT-5、Gemini-2.5-Pro和Claude-4-Sonnet。
...
TL;DR: 本文探讨AI在科学问题解决中的作用，特别是对学科创造力的影响。通过区分创造性方法和创造性产品，引入学科创造力的概念，并通过数学案例说明AI可能取代而非扩展学科创造力。


<details>
  <summary>Details</summary>
Motivation: 研究AI对科学创造力的影响，特别是AI如何改变学科专家运用专业知识解决领域内重要问题的创造性过程。

Method: 基于创造力哲学理论，区分创造性方法和产品，引入学科创造力概念，并通过两个数学案例进行实证分析。

Result: 研究发现计算可以扩展学科创造力，但某些AI方法会取代学科创造力，这可能改变甚至削弱科学追求的价值。

Conclusion: AI在科学问题解决中具有双重作用：既能扩展学科创造力，也可能通过取代创造性过程而影响科学追求的根本价值。
...
Method: 将ME-POMDPs推广为AB-POMDPs，证明ME-POMDPs可以简化为仅变化转移和奖励函数或仅变化观测和奖励函数的模型，并设计了精确和近似（基于点的）算法。
...
TL;DR: 提出了一种基于测试时调优的框架，直接从串联质谱和分子式进行端到端的从头分子结构生成，无需数据库匹配或中间步骤，在两个基准测试上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前方法依赖数据库匹配或多步骤流程，难以识别数据库中未包含的未知化合物，需要一种能够直接从质谱数据生成分子结构的端到端方法。

Method: 利用测试时调优增强预训练transformer模型的学习能力，直接从串联质谱和分子式生成分子结构，绕过手动注释和中间步骤。

Result: 在NPLIB1和MassSpecGym基准测试上分别比DiffMS方法提升100%和20%，测试时调优相比传统微调在MassSpecGym上获得62%的相对性能提升。

Conclusion: 该框架能够动态适应新质谱数据，即使预测与真实值有偏差，生成的分子候选结构仍然准确，为人工解释和可靠识别提供有价值指导。
...
TL;DR: 论文研究生成式AI在象棋谜题领域的创造力，开发了一个能生成具有美学吸引力、新颖性、反直觉和独特解法的象棋谜题AI系统，并由三位世界知名专家评估其创造力。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，需要探究其是否能够产生真正具有创造性和新颖性的输出，特别是在象棋谜题这一需要高度创造力的领域。

Method: 开发专门用于生成象棋谜题的AI系统，该系统能够产生具有美学吸引力、新颖性、反直觉和独特解法的谜题，并邀请三位国际象棋专家对生成的谜题进行评估。

Result: 三位世界知名象棋专家（国际象棋排局大师Amatzia Avni、特级大师Jonathan Levitt和Matthew Sadler）对AI生成的谜题进行了评估，选择了他们最喜欢的谜题并解释了其吸引力所在。

Conclusion: 通过专家评估验证了AI系统在象棋谜题生成方面具有一定的创造力，能够产生具有美学价值和挑战性的新颖谜题。
...
TL;DR: 病理学基础模型在癌症诊断、预后和多模态检索方面表现不佳，存在诊断准确性低、鲁棒性差、几何不稳定、计算需求大和安全漏洞等问题，需要从根本上重新思考该范式。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在计算机视觉和语言处理领域取得了革命性突破，但在计算病理学中的快速应用并未带来预期的突破，反而显示出根本性弱点。

Method: 通过系统评估分析病理学基础模型的缺陷，识别出七个相互关联的原因：生物复杂性、无效的自监督、过度泛化、过度架构复杂性、缺乏领域特定创新、数据不足以及与组织切片尺寸相关的根本设计缺陷。

Result: 研究发现当前病理学基础模型在诊断准确性、鲁棒性、几何稳定性、计算效率和安全性方面存在严重问题，与组织形态学的本质存在概念性不匹配。

Conclusion: 病理学基础模型在概念上与组织形态学的复杂性不匹配，需要从根本上重新思考建模范式，而不是简单套用通用基础模型方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在需要多步推理和动态重规划的长时程任务中面临的挑战，包括顺序提示方法容易产生上下文漂移、目标信息丢失和重复失败循环，以及分层提示方法削弱跨级连续性或产生大量运行时开销的问题。

Method: ReCAP框架结合三个关键机制：(1) 计划先行分解：模型生成完整子任务列表，执行第一个项目并细化剩余部分；(2) 结构化重注入父计划：在递归返回时保持一致的多级上下文；(3) 内存高效执行：限制活动提示，使成本随任务深度线性扩展。

Result: 实验表明ReCAP在各种长时程推理基准测试中显著提高了子目标对齐和成功率，在严格pass@1协议下，同步Robotouille提高了32%，异步Robotouille提高了29%。
...
TL;DR: 本文研究了去中心化多智能体路径规划中的目标分配问题，通过比较贪心启发式、最优分配和基于大语言模型的智能体方法，发现LLM智能体在良好提示下能实现接近最优的性能。


<details>
  <summary>Details</summary>
Motivation: 解决去中心化条件下多智能体在共享环境中的协调挑战，特别是目标分配问题，探索LLM在去中心化决策中的潜力。

Method: 智能体基于环境结构化表示独立生成目标偏好排名，交换排名后通过固定冲突解决规则进行分配，不进行协商或迭代协调。系统比较了贪心启发式、最优分配和LLM智能体方法。
...
TL;DR: IBM开发了通用计算机代理CUGA，采用分层规划-执行架构，在学术基准上表现优异，并在企业业务流程外包人才招聘领域进行了试点，展示了在企业环境中应用的潜力。
...
Result: CUGA在AppWorld和WebArena上达到最先进性能。在企业试点中，接近专业代理的准确性，同时显示出减少开发时间和成本的潜力。

Conclusion: 研究提供了通用代理在企业规模运行的早期证据，并总结了技术和组织经验。为将研究级架构发展为稳健的企业就绪系统提出了要求和下一步计划。
...
TL;DR: 本文提出了一种基于强化学习的框架，用于生成具有创造性、美学价值和反直觉性的国际象棋谜题，通过新颖的奖励机制显著提升了反直觉谜题的生成效果。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在多个领域快速发展，但在生成真正具有创造性、美学价值和反直觉性的输出方面仍面临挑战，特别是在国际象棋谜题领域。

Method: 首先对生成式AI架构进行基准测试，然后引入基于国际象棋引擎搜索统计的强化学习框架，设计了增强谜题独特性、反直觉性、多样性和真实性的奖励机制。

Result: 强化学习方法将反直觉谜题生成率从0.22%（监督学习）提升至2.5%，超过了现有数据集（2.1%）和最佳Lichess训练模型（0.4%）。生成的谜题在人类专家评估中被认为比传统书籍谜题更具创造性、趣味性和反直觉性。

Conclusion: 该方法成功生成了高质量的AI生成谜题，得到了三位世界知名专家的认可，证明了强化学习在创造性任务中的有效性。
...
TL;DR: 该研究比较了四种预测模型（线性、物理建模、LSTM、混合建模）和三种控制策略（MPC、RL、LLM控制）在数字孪生系统中的应用，发现HAM模型在精度、泛化性和计算效率方面表现最均衡，MPC控制器稳健可靠，RL适应性强，LLM控制器支持灵活的人机交互。
...
Method: 以微型温室为测试平台，开发四种预测模型（线性、PBM、LSTM、HAM）和三种控制策略（MPC、RL、LLM控制），在插值和外推场景下进行比较评估。

Result: HAM模型在精度、泛化性和计算效率方面表现最均衡；LSTM精度高但资源消耗大；MPC控制器稳健可预测；RL适应性强；LLM控制器结合预测工具可实现灵活的人机交互。

Conclusion: HAM模型和MPC控制器在数字孪生系统中提供了最佳平衡性能，而RL和LLM控制器分别在适应性和人机交互方面具有独特优势。
...
<details>
  <summary>Details</summary>
Motivation: 智能代理AI系统在自动化中展现出强大能力，但其在Web、软件和物理环境中的自主执行能力带来了新的安全风险，这些风险既不同于传统AI安全也不同于常规软件安全。

Method: 构建了智能代理AI特有的威胁分类法，回顾了最近的基准测试和评估方法，并从技术和治理角度讨论了防御策略。

Result: 综合了当前研究，识别了智能代理AI系统的独特安全威胁，并提出了相应的评估框架和防御方法。

Conclusion: 强调了开发安全设计的智能代理系统的重要性，并指出了该领域面临的开放挑战。
...
TL;DR: 提出基于摊销变分推理的可扩展训练算法，通过多样性强化学习改进LVLMs的思维链推理，在七个推理基准上提升性能


<details>
  <summary>Details</summary>
Motivation: 现有训练算法（SFT、PPO、GRPO）在未见推理任务上泛化能力差，且严重依赖有偏的奖励模型

Method: 将LVLMs推理重新表述为后验推断，使用摊销变分推理和多样性强化学习，引入稀疏奖励函数鼓励多样化高似然潜在CoT

Result: 在七个推理基准上提升了最先进LVLMs的有效性、泛化性和可解释性

Conclusion: 该方法通过变分推理框架有效解决了LVLMs推理训练中的泛化和奖励模型依赖问题
...
TL;DR: 提出了基于judo演算的直觉主义去中心化因果发现框架，使用j-稳定因果推断和j-do-演算在层拓扑中形式化上下文依赖的因果效应。


<details>
  <summary>Details</summary>
Motivation: 现实应用中因果效应依赖于具体情境（如年龄、国家、剂量等），需要形式化这种上下文依赖性。

Method: 结合层理论和Lawvere-Tierney模态算子j，开发了judo演算算法框架，并与基于分数、约束和梯度的标准因果发现方法相结合。

Result: 实验显示该框架在计算效率上优于经典因果发现方法，在合成和真实数据集（生物、经济）上表现良好。

Conclusion: judo演算为上下文依赖的因果发现提供了形式化框架，实现了去中心化计算和性能提升。
...
Result: 在LLM对齐的数字孪生模拟中，sign estimator显著减少了偏好扭曲，将估计误差降低了近35%，与真实群体偏好的不一致性从12%降至8%。

Conclusion: sign estimator方法简单、可证明一致且高效，优于显式建模用户异质性的面板数据启发式方法，同时保持了现有LLM对齐管道的实现简单性。
...
<details>
  <summary>Details</summary>
Motivation: 预测破坏性事件前个体移动模式变化具有挑战性，原因包括：缺乏衡量个体异质性社会基础设施韧性的指标；个体移动模式与空间背景的复杂交互关系未被充分捕捉；个体级移动数据稀疏且不适合传统预测方法。

Method: 开发了一个条件深度学习模型，整合个体的社会基础设施韧性(SIR)和局部空间背景，利用大规模稀疏的个体级数据来捕捉个体移动模式与空间背景的复杂关系。

Result: 实验表明，整合个体SIR和空间背景能够增强模型预测事件后个体移动模式的能力。条件模型能够捕捉到具有相似事件前移动模式但SIR不同的个体在移动模式上的差异变化。
...
TL;DR: 利用大型语言模型(LLMs)发现新的启发式算法来解决单机总延迟调度问题，提出了EDD Challenger和MDD Challenger两种算法，在大型实例上表现优于传统启发式方法。


<details>
  <summary>Details</summary>
Motivation: 传统启发式算法在单机总延迟调度问题上表现有限，而精确方法在大规模实例中计算不可行。研究探索LLMs在发现高性能启发式算法方面的潜力。

Method: 开发了两种LLM发现的启发式算法(EDDC和MDDC)，基于经典的EDD和MDD规则。使用混合整数规划(MIP)公式进行严格评估，包括最优性差距和求解时间。

Result: 对于超过100个作业的实例，EDDC改进了经典EDD规则和其他广泛使用的算法。MDDC始终优于传统启发式方法，在大型复杂实例上与精确方法竞争。

Conclusion: 人-LLM协作可以产生可扩展的高性能启发式算法，用于NP难约束组合优化问题，即使在有限资源下也能有效配置。
...
TL;DR: OneCast是一个跨域时间序列预测框架，通过将时间序列分解为季节性和趋势组件，分别使用轻量级投影模块和基于离散扩散的标记化方法进行建模，在多个领域实验中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的跨域时间序列预测方法在处理异构数据时，难以有效应对领域特定的趋势变化和不一致的周期性模式，主要问题在于将时间序列视为未分化的序列而没有显式解耦其内在结构组件。

Method: 提出OneCast框架：1）将时间序列分解为季节性和趋势组件；2）季节性组件通过轻量级投影模块使用可解释基函数重建周期性模式；3）趋势组件通过语义感知标记器编码为离散标记，并通过掩码离散扩散机制进行推断；4）两个分支的输出结合生成最终预测。

Result: 在八个领域的广泛实验表明，OneCast在大多数情况下优于最先进的基线方法。

Conclusion: 通过显式解耦时间序列的结构组件并采用模块化建模方法，OneCast能够有效捕捉季节性模式同时跟踪领域特定趋势，实现了跨域时间序列预测的显著改进。
...
TL;DR: LLMLogAnalyzer是一个基于聚类的日志分析聊天机器人，结合大语言模型和机器学习算法，简化日志分析流程，在四个不同领域日志上表现优于现有LLM聊天机器人39%-68%。
...
Method: 使用真实世界数据集，比较了IDM、OVM、OVRV、CACC等经典模型和随机森林回归器，通过最小化RMSE来校准模型参数。

Result: 随机森林模型表现最佳，在中等、长和超长车距情况下的RMSE分别为0.0046、0.0016和0.0025；经典模型中CACC表现最好，长车距时RMSE为2.67。

Conclusion: 机器学习模型在预测电动汽车跟驰行为方面优于传统物理模型，对模拟EV行为和混合自动驾驶交通动态分析具有重要价值。
...
TL;DR: 开发了HistoLens系统，这是一个透明的AI助手，让病理学家能够用自然语言提问并获得带有视觉证明的分析结果，提高诊断效率和可信度。
...
Method: 创建HistoLens系统，将自然语言问题转换为精确查询，提供结构化报告和热图可视化证据，并训练AI忽略背景噪音只关注组织区域。

Result: 实现了病理学家保持主导地位的工作流程，使用可信赖的AI助手验证见解，做出更快、更自信的诊断。

Conclusion: HistoLens通过透明推理和视觉证明，使AI成为病理学家的可靠协作伙伴，增强了医生对AI的信任和诊断效率。
...
TL;DR: OpsAgent是一个轻量级、自演进的多代理系统，用于云系统事件管理，通过免训练数据处理和多代理协作框架实现自动化诊断，并在OPENRCA基准测试中表现出最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 手动事件管理劳动密集且容易出错，现有自动化方法在跨系统泛化、可解释性和部署成本方面存在局限，阻碍了实际应用。

Method: 使用免训练数据处理器将异构可观测数据转换为结构化文本描述，采用多代理协作框架进行透明可审计的诊断推理，并引入双自演进机制支持持续能力增长。

Result: 在OPENRCA基准测试中实现了最先进的性能，证明OpsAgent具有泛化性、可解释性、成本效益和自演进特性。

Conclusion: OpsAgent是一个可实际部署且可持续的解决方案，适用于真实云系统的长期运维。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多跳问答数据集稀缺且主要设计用于评估，不适合监督微调或强化学习训练。人工构建这类数据成本高昂且难以扩展。

Method: 使用NLI关系分类和多样性扩展构建证据簇，通过反向问题构建创建模糊线索，采用多模型共识过滤和结构化约束分解进行质量评估。

Result: 开发了一个可扩展的流程，生成复杂、检索困难但可验证的问题，适用于SFT/RL训练和挑战性评估。

Conclusion: 该框架显著减少了人工标注工作量，同时保持了强评估基准的难度特征，为训练高能力检索推理智能体提供了解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM在数字-物理空间间泛化能力差，VLA缺乏高级具身推理能力，ELLM局限于数字空间。需要统一模型实现跨空间、跨具身的无缝操作。

Method: 两阶段训练：阶段I通过数字语料注入具身知识，保持语言能力；阶段II训练策略模块，通过意图桥接接口从MLLM提取高级语义指导控制，不微调MLLM主干。

Result: 在数字和物理基准测试中，单个BLM₁实例优于四类模型家族（MLLM、ELLM、VLA、GMLM），数字任务提升约6%，物理任务提升约3%。
...
Method: 1. HFTDN：通过历史轨迹相似性检索相关未来轨迹，生成跨数据集规划指导；2. GFTM：学习多数据集的历史-未来相关性，将历史轨迹转换为通用规划先验；3. S2D：训练时选择性抑制规划先验实现鲁棒学习，推理时充分利用先验最大化性能。

Result: UniPlanner实现了跨数据集的统一学习，能够从多个数据集中学习鲁棒的规划知识，同时防止捷径学习，确保规划知识的安全迁移。

Conclusion: 该框架突破了单数据集训练的限制，通过多数据集集成显著提升了自动驾驶规划系统的鲁棒性和性能，为实际应用中的复杂场景规划提供了有效解决方案。
...
TL;DR: 提出了一种基于记忆驱动的GUI代理MGA，采用"先观察后决策"原则，通过独立建模每个步骤的环境状态来提升GUI交互的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理存在两个主要问题：对历史轨迹的依赖导致错误传播放大，以及"决策优先、观察滞后"机制忽略了关键界面线索。

Method: MGA将每个步骤建模为独立的环境状态，包含当前截图、任务无关的空间信息和动态更新的结构化记忆三元组，采用"先观察后决策"的交互原则。

Result: 在OSworld基准测试、真实桌面应用和跨任务迁移实验中，MGA相比最先进基线方法在鲁棒性、泛化性和效率方面取得显著提升。

Conclusion: MGA通过重新构建GUI交互范式，有效解决了现有方法中的错误传播和局部探索偏差问题，为GUI代理发展提供了新思路。
...
TL;DR: MCP-Flow是一个自动化网络代理驱动的管道，用于大规模服务器发现、数据合成和模型训练，以提升LLM在MCP生态系统中的工具使用能力。
...
Method: 通过自动化管道从1166个服务器和11536个工具中收集筛选数据，生成68733个高质量指令-函数调用对和6439条轨迹。
...
TL;DR: 提出了几种替代随机策略的抽象内策略，用于解决MCTS中抽象节点内多个动作具有相同UCB值时的选择问题，多数环境和参数设置下表现优于随机策略。


<details>
  <summary>Details</summary>
Motivation: 解决MCTS中抽象技术的一个弱点：当同一抽象节点内多个动作具有相同UCB值时，现有方法（如pruned OGA）使用随机策略进行选择，这可能导致性能下降。

Method: 提出并实证评估了多种抽象内策略，用于在抽象节点内多个动作具有相同UCB值时进行选择，替代原有的随机策略。

Result: 实验结果表明，提出的几种替代策略在多数环境和参数设置下表现优于随机策略。

Conclusion: 通过设计更智能的抽象内策略，可以有效改进MCTS的抽象技术，提升算法性能。
...
TL;DR: 提出了一种基于LLM内部行为自我指示推理路径可信度的方法，通过计算输入问题与输出推理路径的相关性矩阵秩来评估推理正确性，无需额外训练或复杂提示。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易产生错误和幻觉，现有验证方法依赖外部资源导致计算开销大且适用范围有限，需要探索基于模型内部行为的自我验证方法。

Method: 利用LLM内部行为，通过计算输入问题与输出推理路径的相关性矩阵秩作为推理正确性指标，设计简单的自指示方法对候选推理路径进行重加权。

Result: 在多个不同规模和家族的LLM上实验，该方法能超过75%准确率区分正确与错误推理路径，在三个推理基准上提升准确率超过8%。

Conclusion: LLM内部行为确实蕴含推理路径可信度信息，提出的自指示方法简单有效，显著优于其他投票和验证方法，且计算开销极小。
...
TL;DR: 提出了一种用于判断预测的多智能体框架，通过不同智能体对声明真实性进行辩论，生成定量双极论证框架(QBAFs)来支持声明验证，实验证明多智能体组合能提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 将判断预测视为声明验证任务，需要评估未来事件的可能性。传统方法缺乏多视角辩论和可解释的证据组合机制。

Method: 开发了三种基于LLM的智能体：ArgLLM、RbAM和RAG-ArgLLM，分别采用不同方式生成QBAFs。通过2-3个智能体的辩论框架进行声明验证。

Result: 在标准判断预测数据集上的实验表明，多智能体组合（特别是三个智能体）能显著提高预测准确性，同时提供可解释的证据组合。

Conclusion: 多智能体辩论框架能有效提升判断预测的准确性和可解释性，为声明验证提供了新的解决方案。
...
TL;DR: 本文探讨了生成式大语言模型在传播学研究内容分析中的应用，提出了应对七大挑战的最佳实践指南。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式大语言模型在内容分析中表现出色，但在传播学研究中的方法论整合仍不充分，需要系统指导来确保研究质量。

Method: 综合新兴研究，提出包含代码本开发、提示工程、模型选择、参数调优、迭代优化、可靠性验证和性能提升的七步最佳实践框架。

Result: 建立了一个使gLLM辅助内容分析更易用且符合学科质量标准的方法论体系。

Conclusion: 本文为传播学研究者提供了实用的gLLM辅助内容分析指南，促进了该技术在研究中的规范应用。
...
TL;DR: VDSAgents是一个基于可预测性-可计算性-稳定性(PCS)原则的多智能体系统，用于改进LLM驱动的数据科学工作流，在多个数据集上优于现有的端到端系统。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的数据科学系统仅依赖LLM内部推理，缺乏科学和理论原则指导，限制了在噪声和复杂现实数据集上的可信度和鲁棒性。
...
TL;DR: 提出了一个名为"脑相似空间"的统一几何空间框架，通过将AI模型的内在空间注意力拓扑映射到人类功能性脑网络上，实现跨模态、跨任务的AI模型比较。
...
Method: 通过分析151个基于Transformer的模型，将模型的内在空间注意力拓扑映射到标准人类功能性脑网络上，构建统一的脑相似空间。

Result: 发现模型在脑相似空间中呈现连续的弧状几何结构，脑相似度逐渐增加；不同模型分布模式各异，受预训练范式和位置编码方案影响。

Conclusion: 脑相似度为第一个统一框架，用于跨领域定位、量化和比较智能，揭示了连接机器与大脑的深层组织原则。
...
<details>
  <summary>Details</summary>
Motivation: 当前医疗AI系统服务于平均患者，在罕见变异、多病共存和代表性不足人群等边缘情况下表现不佳，这种平均患者谬误侵蚀了公平性和信任。

Method: 构建多智能体生态系统，按器官系统、患者群体和分析模式聚类，共享模型库和证据合成工具，通过协调层评估可靠性、不确定性和数据密度，生成决策支持包。

Result: 验证从群体平均转向个体可靠性，测量低密度区域误差、小样本校准和风险-覆盖权衡。

Conclusion: 该方法将医疗AI与医学首要原则对齐：提供透明、公平、以个体为中心的护理，通过缓存策略、共识检查和适应性试验框架解决计算需求、自动化偏差和监管适应性等挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有透明度工具（如模型卡、数据卡）缺乏规范性约束层，无法确保AI代理在部署时遵守操作、监管和伦理要求。

Method: 通过定义包含允许/拒绝规则、义务、证据要求和与NIST AI RMF、ISO/IEC 42001、欧盟AI法案等保证框架映射的规范性层。

Result: 每个Policy Card可自动验证、版本控制，并与运行时执行或持续审计管道链接，为多代理生态系统中的分布式保证奠定基础。

Conclusion: Policy Cards为集成高层治理与工程实践提供了实用机制，实现了可扩展的负责任自主性。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM推理存在计算效率低和推理策略僵化的问题，无法同时满足Web服务对高质量推理和低延迟高吞吐量的双重需求。

Method: 将查询推理分解为两个协同阶段：关键点生成（通过检索增强的少样本提示生成逻辑结构化的关键点）和内容并行扩展（基于依赖图并发扩展内容确保逻辑一致性），并引入流水线调度机制实现跨查询并行。

Result: 在多样化基准测试中，Orion相比基线实现了最高4.33倍的token生成速度和3.42倍的答案延迟降低，同时推理质量提升了18.75%。

Conclusion: Orion通过显式建模点间依赖关系，成功平衡了LLM推理的效率与质量，为实时Web应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基于视觉语言模型的AI代理在移动平台操作中展现出类人能力，但其潜在的安全风险（如系统破坏和隐私泄露）引发了严重担忧，而现有研究对此关注不足

Method: 构建MobileRisk-Live动态沙盒环境，提出OS-Sentinel混合检测框架，结合形式验证器检测系统级违规和基于VLM的上下文判断器评估情境风险

Result: 实验显示OS-Sentinel在多个指标上比现有方法提升10%-30%性能

Conclusion: 该研究为移动代理安全研究奠定了基础，促进了更安全可靠的自主移动代理发展
...
TL;DR: 本研究比较了多个大型语言模型在逻辑和抽象推理能力上的表现，并与人类基准进行对比，揭示了LLM在演绎推理方面的不足。
...
Method: 使用八道自定义设计的推理问题，比较GPT、Claude、DeepSeek、Gemini、Grok、Llama、Mistral、Perplexity和Sabi'a等LLM的逻辑和抽象推理技能，并将结果与人类在相同任务上的表现进行基准测试。

Result: LLM结果与人类表现存在显著差异，表明LLM在演绎推理方面存在困难。

Conclusion: 大型语言模型在逻辑和抽象推理能力上仍有明显局限，特别是在演绎推理方面需要进一步改进。
...
TL;DR: Law in Silico是一个基于LLM的法律社会模拟框架，通过AI代理模拟法律决策和制度机制，能够复现现实犯罪趋势并提供法律系统优化见解。
...
Method: 引入Law in Silico框架，基于LLM代理模拟法律场景，包括个体决策和立法、裁决、执法等制度机制。

Result: 模拟犯罪率与真实数据比较显示，LLM代理能够复现宏观犯罪趋势，微观模拟表明功能完善、透明且适应性的法律系统能更好保护弱势个体权利。
...
TL;DR: 提出了两种架构模式：DOM转换模式用于简化网页DOM结构，超媒体功能识别模式用于动态发现和集成未知Web服务，共同为软件代理构建可操作的世界模型提供框架。


<details>
  <summary>Details</summary>
Motivation: 解决软件代理从结构化数据构建世界模型的两个关键挑战：原始HTML的冗长性使基础模型难以直接处理，以及硬编码API集成的静态性阻碍了代理适应不断发展的服务。

Method: 引入两种互补的架构模式：DOM转换模式将冗长的原始DOM提炼为紧凑的任务相关表示；超媒体功能识别模式通过解析标准化语义描述来动态发现和集成未知Web服务。

Result: 这些模式为工程化代理提供了稳健框架，使其能够高效构建和维护准确的世界模型。

Conclusion: 该模式语言支持软件代理在Web及其扩展资源上实现可扩展、自适应和互操作的自动化。
...
TL;DR: 该论文提出了一种改进SNN训练的方法，通过分析替代梯度斜率设置和引入引导策略来提升强化学习性能，在真实无人机控制任务中显著优于现有方法。
...
Method: 系统分析替代梯度斜率设置，提出使用特权引导策略来引导学习过程，并结合自适应斜率调度方法。

Result: 在强化学习设置中，较浅斜率或调度斜率使训练和最终部署性能提升2.1倍。在真实无人机位置控制任务中达到平均回报400分，显著优于行为克隆和TD3BC方法（最多-200分）。

Conclusion: 该工作推进了对SNN中替代梯度学习的理论理解，并为神经形态控制器在实际机器人系统中的实用训练方法提供了进展。
...
TL;DR: 提出一种成本效益高的两阶段流程，通过跨任务示例和基于图的标签传播方法，减少对LLM数据标注的依赖，为上下文学习构建高质量演示数据。
...
Conclusion: 该流程结合了跨任务监督的灵活性和无LLM传播的可扩展性，为上下文学习构建了有效的伪标注数据集。
...
TL;DR: 本文提出了一种以数据为中心的设计范式，将医疗数据生态系统作为生成式AI系统的基础，通过语义向量搜索和上下文查询等技术，支持医疗AI模型训练和临床应用部署。
...
TL;DR: FunReason-MT是一个用于多轮工具调用的数据合成框架，通过环境-API图交互、高级工具查询合成和引导迭代链来解决现有方法在真实世界环境中的局限性，显著提升了函数调用能力。


<details>
  <summary>Details</summary>
Motivation: 现有数据合成方法（如随机环境采样或多智能体角色扮演）不足以在真实世界环境中生成高质量的多轮工具调用训练数据，存在目标模型训练、工具架构隔离和多轮逻辑依赖三大挑战。

Method: FunReason-MT采用三种核心技术：1）环境-API图交互收集多样化高质量轨迹；2）高级工具查询合成简化复杂查询构建；3）引导迭代链生成复杂思维链。

Result: 在伯克利函数调用排行榜（BFCLv3）上，基于FunReason-MT生成的4B参数模型在同类规模模型中达到最先进性能，甚至超越大多数闭源模型。在BFCLv4上的进一步性能提升证实了其可靠性。
...
Method: 通过文献综述方法，筛选约40篇相关论文，分析基础模型（特别是大语言模型和视觉语言模型）在作物定点病害管理中的应用，包括自适应学习、强化学习和数字孪生框架。

Result: 主要发现：(a) 基础模型在2023-24年获得广泛关注；(b) 视觉语言模型发展快于大语言模型，发表量增长5-10倍；(c) 强化学习和自适应学习在智能喷洒中仍处于早期阶段；(d) 数字孪生结合强化学习可虚拟模拟定点喷洒；(e) 解决仿真到现实的差距对实际部署至关重要；(f) 人机协作有限，特别是在人机协同方法中；(g) 多模态基础模型与实时反馈将推动下一代定点病害管理。

Conclusion: 基础模型在作物定点病害管理中展现出巨大潜力，特别是视觉语言模型和多模态方法的发展。未来需要解决仿真到现实的差距，加强人机协作，并开发具有实时反馈能力的多模态基础模型系统。
...
TL;DR: 提出了OrchDAG合成数据生成管道，将工具执行建模为有向无环图，用于多轮工具交互的基准测试，并提出了基于图的奖励来增强RLVR训练。
...
Method: 引入OrchDAG合成数据生成管道，将工具执行建模为可控复杂度的有向无环图，并提出基于图的奖励机制结合GRPO风格算法进行训练。

Result: 实验表明该数据集提供了一个具有挑战性但可解决的基准，所提出的奖励机制在结合GRPO算法时表现有效。

Conclusion: 在多轮工具使用中，利用拓扑结构和数据复杂性具有重要意义，提出的方法为此类复杂交互提供了有效的解决方案。
...
TL;DR: 提出了一个通过融合工具知识图谱和文档知识图谱来增强示例工件生成的框架，利用工具间的依赖关系提升规划生成效果。


<details>
  <summary>Details</summary>
Motivation: 现有的工具增强规划方法往往忽视工具间的依赖关系和领域知识，限制了规划的质量和实用性。

Method: 构建工具知识图谱和文档知识图谱，采用深度稀疏集成策略将工具结构依赖与程序知识对齐，生成示例计划。

Result: 实验表明该统一框架能有效建模工具交互并改善计划生成，验证了工具图谱与领域知识图谱融合的价值。

Conclusion: 将工具图谱与领域知识图谱链接对于工具增强推理和规划具有显著益处，能够提升规划质量和实用性。
