...
### [1] [SplatFont3D: Structure-Aware Text-to-3D Artistic Font Generation with Part-Level Style Control](https://arxiv.org/abs/2512.00413)
*Ji Gan,Lingxu Chen,Jiaxu Leng,Xinbo Gao*

Main category: cs.CV

TL;DR: SplatFont3D：基于3D高斯泼溅的结构感知文本到3D艺术字体生成框架，实现精确部件级风格控制


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注2D艺术字体，而个性化3D艺术字体生成（3D-AFG）尚未充分探索。3D字体不仅可用于游戏和动画等沉浸式3D环境，还能通过渲染新颖视角增强2D字体生成。3D字体具有精确语义和强结构约束，需要细粒度的部件级风格控制。

Method: 提出SplatFont3D框架：1）Glyph2Cloud模块逐步增强2D字形（或部件）的形状和风格，生成对应3D点云用于高斯初始化；2）通过分数蒸馏采样与预训练2D扩散模型交互优化3D高斯；3）动态部件分配策略利用3D高斯的几何先验划分部件，缓解优化过程中的漂移纠缠。

Result: SplatFont3D在风格-文本一致性、视觉质量和渲染效率方面优于现有3D模型。相比NeRF，提供更明确有效的部件级风格控制，渲染效率更高。

Conclusion: SplatFont3D成功解决了3D艺术字体生成的挑战，实现了从多样化风格文本提示创建3D艺术字体，并具备精确的部件级风格控制能力，为3D字体设计提供了有效解决方案。
...
### [2] [MOTION: ML-Assisted On-Device Low-Latency Motion Recognition](https://arxiv.org/abs/2512.00008)
*Veeramani Pugazhenthi,Wei-Hsiang Chu,Junwei Lu,Jadyn N. Miyahira,Soheil Salehi*

Main category: cs.CV

TL;DR: 使用AutoML和轻量级机器学习算法在可穿戴设备上实现高效的手势识别，神经网络在精度、延迟和内存使用方面表现最佳


<details>
  <summary>Details</summary>
Motivation: 随着小型设备在低延迟手势识别中的应用日益普及，特别是在医疗监测领域，需要快速高效的运动跟踪解决方案，同时避免误报

Method: 使用三轴加速度计传感器构建高效运动模型，探索AutoML管道提取数据段最重要特征，训练多种轻量级机器学习算法，在WeBe Band多传感器可穿戴设备上实现

Result: 神经网络在精度、延迟和内存使用方面提供了最佳平衡，能够在WeBe Band上实现可靠的实时手势识别

Conclusion: 该方法展示了在可穿戴设备上实现实时手势识别的可行性，为需要安全快速响应的实时医疗监测解决方案提供了巨大潜力
...
### [3] [Gaussian Swaying: Surface-Based Framework for Aerodynamic Simulation with 3D Gaussians](https://arxiv.org/abs/2512.01306)
...
<details>
  <summary>Details</summary>
Motivation: 自然场景中的空气动力学效应（如树枝摇曳、旗帜飘动）对视觉和图形学的真实感至关重要，现有方法如基于网格的方法需要昂贵网格化，基于粒子的方法依赖离散位置数据，存在效率或精度问题

Method: 使用3D高斯连续建模表面，提出高斯补丁统一模拟与渲染，支持动力学力计算同时提供法线进行轻量级着色

Result: 在合成和真实数据集上的综合实验表明，Gaussian Swaying在多项指标上达到最先进的性能和效率

Conclusion: Gaussian Swaying为真实空气动力学场景模拟提供了可扩展的方法，在效率和精细度方面优于现有方法
...
### [4] [Closing the Gap: Data-Centric Fine-Tuning of Vision Language Models for the Standardized Exam Questions](https://arxiv.org/abs/2512.00042)
*Egemen Sert,Şeyda Ertekin*

Main category: cs.CV

TL;DR: 通过高质量数据监督微调，多模态推理模型在标准化考试问题上达到接近顶级专有模型的性能


<details>
  <summary>Details</summary>
Motivation: 多模态推理是现代AI研究的关键，标准化考试问题提供了严格的测试平台。虽然近期研究主要集中在算法改进（如强化学习），但视觉语言推理的数据中心基础仍较少被探索。作者希望证明高质量数据的监督微调可以媲美专有方法。

Method: 收集了1.614亿token的多模态数据集，包含教科书问题-解决方案对、课程对齐图表和上下文材料。使用优化的推理语法（QMSA）对Qwen-2.5VL-32B模型进行监督微调。发布了新的基准测试YKSUniform，包含1,854个多模态考试问题，覆盖309个课程主题。

Result: 微调后的模型在新发布的YKSUniform基准上达到78.6%的准确率，仅比Gemini 2.0 Flash低1.0%。这表明数据组合和表示语法在多模态推理中起决定性作用。

Conclusion: 这项工作建立了一个数据中心框架，用于推进开放权重的视觉语言模型。研究表明，精心策划和基于课程的多模态数据可以将监督微调提升到接近最先进的性能水平。
...
### [5] [PEFT-DML: Parameter-Efficient Fine-Tuning Deep Metric Learning for Robust Multi-Modal 3D Object Detection in Autonomous Driving](https://arxiv.org/abs/2512.00060)
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态3D检测模型假设传感器固定可用，但在实际自动驾驶场景中，传感器可能丢失或出现未见组合，需要鲁棒性更强的解决方案。

Method: 通过LoRA和适配器层将多种模态（LiDAR、雷达、相机、IMU、GNSS）映射到共享潜在空间，实现参数高效学习，增强对传感器丢失和未见组合的鲁棒性。

Result: 在nuScenes基准测试中表现出优越的准确性，同时显著提升训练效率，增强对快速运动、天气变化和域偏移的鲁棒性。

Conclusion: PEFT-DML为自动驾驶中的多模态3D检测提供了高效且鲁棒的解决方案，特别适用于传感器不可靠或动态变化的实际场景。
...
### [6] [DL-CapsNet: A Deep and Light Capsule Network](https://arxiv.org/abs/2512.00061)
...
<details>
  <summary>Details</summary>
Motivation: 胶囊网络（CapsNet）是比卷积神经网络（CNN）更有前景的分类器，在检测具有重叠类别和仿射变换的图像方面更准确。但现有CapsNet需要改进以处理复杂数据集

Method: 提出深度胶囊网络变体（DL-CapsNet），包含多个胶囊层，并设计胶囊汇总层来减少参数数量，降低复杂度

Result: DL-CapsNet在保持高精度的同时，使用较少的参数，实现了更快的训练和推理速度，能够处理类别数量多的复杂数据集

Conclusion: DL-CapsNet通过深度架构和参数优化，为胶囊网络在实际复杂应用中的部署提供了高效解决方案
...
TL;DR: 提出Satellite-to-Street框架，通过改进的双输入U-Net架构和类别感知加权损失函数，实现灾后卫星图像的细粒度像素级损伤评估，优于传统分割和变化检测模型。


<details>
  <summary>Details</summary>
Motivation: 传统人工卫星图像解释速度慢、主观性强且难以扩展，而现有的深度学习分割模型（如U-Net）和变化检测模型在处理细微结构变化和严重类别不平衡时表现不佳，特别是在高度受损区域的检测上存在问题。

Method: 提出Satellite-to-Street框架，采用改进的双输入U-Net架构，增强特征融合以捕捉局部结构变化和更广泛的上下文线索，并集成类别感知加权损失函数来处理真实灾害数据集中未受损像素占主导的问题。

Result: 在公开可用的灾害数据集上的实验表明，相比传统分割和基线变化检测模型，该框架在结构损伤的定位和分类方面有显著改进。

Conclusion: 该模型生成的损伤地图提供了一种快速、一致的评估机制，旨在支持而非替代专家决策，从而实现更高效、数据驱动的灾害管理。
...
TL;DR: ProvRain：一种用于夜间雨天车辆预检测的轻量级MobileNet-U-Net去噪管道，通过课程学习提高恶劣天气条件下的检测性能


<details>
  <summary>Details</summary>
Motivation: 夜间车辆预检测主要依赖车灯特征，但在雨雪等恶劣天气条件下，噪声会严重影响检测准确性。需要一种既能处理噪声又能保持检测精度的解决方案。

Method: 提出ProvRain管道，采用轻量级MobileNet-U-Net架构，通过课程学习概念训练以泛化到恶劣天气条件。使用合成数据和PVDN数据集混合训练，并与基于Faster RCNN的基准模型对比。

Result: 在雨天夜间帧中，车辆检测准确率提升8.94%，召回率提升10.25%。MobileNet-U-Net架构相比其他Transformer方法，PSNR提升10-15%，SSIM提升5-6%，感知误差(LPIPS)降低高达67%。

Conclusion: ProvRain管道通过集成去噪架构有效提高了恶劣天气条件下的车辆预检测性能，证明了课程学习和轻量级架构在夜间视觉任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 随着扩散模型的快速发展，零样本图像生成技术能够在无需修改模型权重的情况下，仅用一张肖像或艺术品就能高保真地复制面部身份或艺术风格。虽然这些技术显著增强了创作可能性，但也带来了知识产权侵权的重大风险，包括未经授权的身份克隆和风格模仿。需要一种防御方案来保护个人图像免遭滥用。

Method: 1. 研究当前零样本方法如何通过图像编码器提取嵌入向量，并通过交叉注意力层输入扩散模型的UNet。2. 构建可逆加密系统，根据不同的密钥将原始嵌入映射到不同的加密表示。3. 授权用户可通过解密模块和正确密钥恢复真实嵌入，实现正常生成任务。4. 为保护目的，设计多目标对抗扰动方法，主动将原始嵌入向指定的加密模式偏移，使受保护图像嵌入防御层，确保未授权用户只能产生扭曲或加密的输出。

Result: 广泛评估表明，该方法在阻止未经授权的零样本图像合成方面超越了现有最先进的防御方法，同时支持灵活且安全的验证用户访问控制。

Conclusion: Adapter Shield是首个通用且集成认证的解决方案，能有效防御零样本生成场景中的图像滥用，通过可逆加密和对抗扰动机制实现了授权与未授权用户之间的明确区分，在保护知识产权的同时支持合法的创作使用。
...
<details>
  <summary>Details</summary>
Motivation: 明场显微镜中的准确单细胞检测对生物学研究至关重要，但数据稀缺和标注瓶颈限制了深度学习方法的发展。需要寻找减少对大量手动标注依赖的解决方案。

Method: 训练基于U-Net的扩散模型生成合成明场显微镜图像，创建不同合成/真实图像比例的数据集，使用YOLOv8、YOLOv9和RT-DETR进行目标检测实验，并通过人类专家调查评估生成图像的真实性。

Result: 使用合成数据训练可以显著提高检测准确率（成本极低），人类专家无法区分合成图像与真实图像（准确率50%），扩散模型生成的图像具有高度真实性。
...
TL;DR: MARWIN机器人采用深度视觉立体里程计(DVSO)替代现有导航系统，以提升在加速器隧道中的自主导航能力，解决传统方法在未知几何和障碍物环境中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前MARWIN机器人在欧洲XFEL加速器隧道中的导航系统结合了激光雷达边缘检测、轮式/激光雷达里程计和QR码参考，虽然在预定区域表现稳健，但缺乏应对未知几何结构和障碍物的灵活性。

Method: 提出采用深度视觉立体里程计(DVSO)作为替代方案，该方法基于立体视觉、光流和自监督学习，无需标记数据即可联合估计深度和自我运动，并可后续与绝对参考(如地标)或其他传感器融合以确保全局一致性。
...
TL;DR: 诊断式提示能显著提升多模态大语言模型评估亚马逊搜索结果页面视觉复杂度的可靠性，但绝对性能仍有限，模型与人类在评估标准上存在差异。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索诊断式提示是否能提高多模态大语言模型评估亚马逊搜索结果页面视觉复杂度的可靠性，以解决当前MLLM在视觉评估任务中与人类判断不一致的问题。

Method: 使用200个亚马逊搜索结果页面和人类专家标注，比较诊断式提示与基于格式塔原理的标准提示方法，通过决策树分析模型决策模式，并进行失败案例分析。

Result: 诊断式提示显著提升预测人类复杂度判断的性能，F1分数从0.031提升到0.297（相对提升858%），但绝对性能仍有限（Cohen's κ = 0.071）。模型更关注视觉设计元素（徽章杂乱度占38.6%重要性），而人类更强调内容相似性。

Conclusion: 诊断式提示是实现人类对齐的MLLM评估的有前景的初步方法，但在产品相似性和颜色强度评估方面仍存在挑战，需要更大规模的真实数据集和提示方法改进才能实现可靠的实际部署。
...
TL;DR: FastTextDiff是一种基于扩散模型的标签高效分割方法，通过整合医学文本标注来增强语义表示，使用ModernBERT处理临床笔记，在MIMIC数据集上训练，相比传统扩散模型提高了分割准确性和训练效率。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要密集的像素级标注，这些标注昂贵、耗时且需要专业知识。现有的扩散模型虽然有效，但分割性能受限于标注需求。因此需要开发标签高效的分割方法。

Method: 提出FastTextDiff模型，整合医学文本标注来增强语义表示。使用ModernBERT处理长临床笔记，紧密连接文本标注与医学图像的语义内容。通过交叉模态注意力机制连接视觉和文本特征，利用FlashAttention 2、交替注意力机制和2万亿token语料库。

Result: FastTextDiff在分割准确性和训练效率上优于传统扩散模型。ModernBERT被验证为扩散分割流程中比Clinical BioBERT更快、更可扩展的替代方案。

Conclusion: 多模态技术（结合医学文本和图像）在医学图像分析中具有前景。ModernBERT与扩散模型的结合为标签高效的医学图像分割提供了有效解决方案。
...
TL;DR: 提出一种用于超低功耗物联网平台的多模态设备上学习技术，通过稀疏更新方案在MCU上实现单目深度估计模型的在线微调，显著提升新环境下的深度估计精度。


<details>
  <summary>Details</summary>
Motivation: 物联网平台上的单目深度估计模型参数有限，当传感器数据与训练数据集存在显著域偏移时，精度会严重下降。需要解决新环境下的域适应问题。

Method: 1) 集成MCU、单目相机和8x8像素深度传感器的物联网设备；2) 正常运行时使用107k参数的μPyD-Net模型进行推理；3) 在新环境中激活深度传感器收集伪标签数据；4) 提出内存驱动的稀疏更新方案，将微调内存降至1.2MB；5) 完全在MCU上执行设备上学习。

Result: 1) 微调内存减少2.2倍至1.2MB；2) 在KITTI和NYUv2数据集上精度仅下降2%和1.5%；3) 现场测试中，仅用3k个自标注样本，在17.8分钟内将均方根误差从4.9m降至0.6m；4) 系统总功耗约300mW。

Conclusion: 首次在物联网节点上实现了单目深度估计的设备上学习，通过多模态稀疏更新方案有效解决了域偏移问题，显著提升了新环境下的深度估计精度，同时保持了低功耗特性。
...
TL;DR: 该研究探索AI驱动的课堂录音分析，通过多模态教学活动和话语识别为教师提供可操作的反馈。使用164小时视频和68节课转录的密集标注数据集，设计了并行、模态特定的处理流程，微调模型在视频和文本分析上均优于提示方法。


<details>
  <summary>Details</summary>
Motivation: 当前课堂互动观察依赖人工标注，资源密集且难以扩展，需要开发自动化的AI分析方法来提供可扩展的教师反馈系统。

Method: 使用密集标注数据集（164小时视频和68节课转录），设计并行模态特定流程：视频分析评估零样本多模态LLM、微调视觉语言模型和自监督视频变换器（24个活动标签）；转录分析微调基于变换器的分类器（19个话语标签）。采用逐标签阈值、上下文窗口和不平衡感知损失函数处理类别不平衡和多标签复杂性。

Result: 微调模型在视频分析上达到0.577的宏观F1分数，在转录分析上达到0.460的宏观F1分数，均优于提示方法，证明了自动化课堂分析的可行性。

Conclusion: 研究证明了AI驱动课堂分析的可行性，为可扩展的教师反馈系统奠定了基础，微调方法在视频和文本分析上都优于提示方法。
...
TL;DR: SemImage将文本文档表示为二维语义图像，使用HSV色彩空间编码不同语言特征（色调编码主题、饱和度编码情感、明度编码强度），通过CNN处理实现文档分类，在保持竞争力的准确率同时提供更好的可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统文本处理方法（如BERT）虽然有效但缺乏可解释性，难以直观理解模型决策过程。作者希望创建一种既能保持分类性能又能提供可视化解释的文档表示方法。

Method: 将文档转换为二维语义图像：每行对应一个句子，单词作为像素，使用HSV色彩空间编码语言特征（色调编码主题、饱和度编码情感、明度编码强度）。通过多任务学习框架强制解耦表示，并在句子间插入动态计算的边界行以突出语义转换。

Result: 在多标签数据集（包含主题和情感标注）和单标签基准测试中，SemImage能够达到或超过BERT和分层注意力网络等强基线的准确率，同时提供更好的可解释性。消融研究证实了多通道HSV表示和动态边界行的重要性。

Conclusion: SemImage提供了一种新颖的文档表示方法，将文本转换为可解释的语义图像，使语言特征对人和机器都可见。该方法在保持竞争力的分类性能同时，通过可视化模式揭示了主题转换和情感变化，为文本分析提供了新的视角。
...
TL;DR: TeleViT模型通过融合局部火灾驱动因子、全球粗粒度场和遥相关指数，使用非对称标记化策略和Transformer架构，显著提升了长达4个月的野火预测性能。


<details>
  <summary>Details</summary>
Motivation: 野火的长期预测（数周至数月）对燃料处理和资源分配至关重要，但现有方法主要依赖局部天气条件，难以捕捉地球系统的全球互连性和遥相关模式。

Method: 提出TeleViT模型：采用非对称标记化策略生成异质标记（局部火灾驱动因子、全球粗粒度场、遥相关指数），通过Transformer编码器联合处理，解码器保持空间结构将局部标记映射到预测区域。

Result: 在SeasFire数据集（2001-2021，8天分辨率）上，TeleViT在所有预测时间（包括长达4个月）均优于U-Net++、ViT和气候学基准。在零延迟时AUPRC达0.630，4个月延迟时仍保持0.601-0.603，显著超过基准模型。

Conclusion: 显式编码大尺度地球系统上下文的架构可以扩展野火在次季节到季节时间尺度上的可预测性，局部标记是主要预测依据，全球场和指数提供粗粒度上下文信息。
...
TL;DR: 本文提出了一种用于挤出式和喷射式3D混凝土打印中丝状物几何质量控制的自动化方法，该方法独立于数据采集传感器，可用于新鲜或固化状态的材料，支持在线和打印后质量控制。


<details>
  <summary>Details</summary>
Motivation: 在建筑、工程和施工行业中，3D混凝土打印技术不断发展以满足可持续建筑需求。丝状物作为定义打印结构的基本单元，其几何质量控制至关重要，但目前缺乏独立于传感器的自动化质量控制方法。

Method: 开发了一种独立于传感器的自动化质量控制工作流程，适用于挤出式（Contour Crafting）和喷射式3D打印方法。该方法可使用相机、结构光系统或地面激光扫描仪等多种传感器采集数据，并适用于新鲜或固化状态的材料。

Result: 提出了一种通用的质量控制方法，能够实现3D混凝土打印中丝状物几何参数的自动化检测，支持在线实时监控和打印后评估，提高了质量控制的效率和准确性。

Conclusion: 该研究为3D混凝土打印提供了一种灵活、自动化的质量控制解决方案，能够适应不同的打印技术和材料状态，有助于提升建筑3D打印的质量和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 探索不同图像识别方法在基于日常活动记录图像进行心理健康状况识别中的表现差异，特别是针对抑郁和精神分裂症的识别。

Method: 将手腕活动信号转换为30×48像素图像，使用Psykose和Depresjon数据集，采用三折受试者分层交叉验证，比较VGG16、ViT-B/16和CoAtNet-Tiny三种模型的性能。

Result: CoAtNet-Tiny表现最佳，获得最高平均准确率和最稳定的跨折性能，在抑郁和精神分裂症等少数类上表现出最强的精确率、召回率和F1分数；VGG16改进稳定但准确率较低；ViT-B/16在某些运行中表现强劲但稳定性差。

Conclusion: CoAtNet-Tiny在活动记录图像上表现最一致，某些混合设计可能特别适合基于活动记录图像的心理健康研究。
...
TL;DR: 该论文提出TinyViT系统，仅使用可见光图像就能对光伏板表面故障进行分类和严重程度评估，无需依赖昂贵的多模态成像设备。


<details>
  <summary>Details</summary>
Motivation: 太阳能光伏资产长期运行需要准确检测和优先处理大面积分布式模块的表面故障。虽然多模态成像策略很流行，但它们在常规农场级部署中引入了物流和经济障碍。需要一种仅使用可见光图像就能实现稳健表面异常分类和严重程度估计的方法。

Method: 提出TinyViT紧凑管道，集成基于Transformer的分割、光谱-空间特征工程和集成回归。系统接收消费级彩色相机拍摄的光伏板图像，对七种细微表面故障进行分类，并生成可操作的严重程度等级用于维护优先级排序。

Result: 在真实公共数据集上的实验验证了分类和回归子模块，实现了与专门方法相竞争的准确性和可解释性。通过消除对电致发光或红外传感器的依赖，该方法为资源有限的安装提供了经济实惠、可扩展的维护方案。

Conclusion: 该方法将太阳能健康监测推向普遍现场可访问性，仅使用可见光图像就能实现稳健的表面异常分类和严重程度估计，为资源有限的安装提供了经济实惠、可扩展的维护解决方案。
...
TL;DR: 提出混合合成数据生成框架，结合仿真渲染、领域随机化和真实背景合成，实现无需人工标注的工业零件视觉检测，在严重类别不平衡下达到90%以上准确率。


<details>
  <summary>Details</summary>
Motivation: 工业质量检测中，深度学习需要大量高质量标注数据，但缺陷样本稀少导致类别不平衡，数据获取成本高，阻碍了机器学习在真实生产环境中的广泛应用。

Method: 提出混合合成数据生成框架：1) 仿真渲染生成合成零件图像；2) 领域随机化变化几何、光照和表面属性；3) 将合成零件合成到真实图像背景上。使用YOLOv8n进行目标检测，MobileNetV3-small进行质量分类，完全在合成数据上训练。

Result: 在300个真实工业零件上评估：检测mAP@0.5达到0.995，分类准确率96%，平衡准确率90.1%。在严重类别不平衡下达到90-91%平衡准确率，而少样本真实数据基线方法仅达到50%准确率。

Conclusion: 提出的SDG方法实现了无需人工标注、可扩展且鲁棒的工业质量检测，为真实制造应用提供了可行的解决方案，显著优于传统少样本学习方法。
...
TL;DR: 该研究提出了一种结合ResNet50 OOD过滤与YOLO架构的乳腺癌检测系统，通过拒绝非乳腺X光图像来提高模型在分布外数据上的可靠性，同时保持高检测精度。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在乳腺癌检测中面临分布外（OOD）输入（如CT、MRI、其他X光设备）的可靠性问题，导致检测不可靠和误诊，需要提高系统在临床环境中的鲁棒性。

Method: 采用综合方法：1）使用ResNet50基于余弦相似度建立域内图库，严格拒绝非乳腺X光图像；2）集成YOLO架构（YOLOv8/v11/v12）进行准确检测；3）通过Grad-CAM增强可解释性。

Result: OOD检测组件达到99.77%总体准确率，在OOD测试集上100%准确；检测性能mAP@0.5为0.947；ResNet50经12种CNN架构搜索被选为最优骨干网络。

Conclusion: 该研究为在数据异质性临床环境中部署可靠的AI乳腺癌检测系统提供了基础框架，OOD过滤显著提高了系统可靠性，防止了在分布外输入上的误报，同时保持了高检测精度。
...
TL;DR: LGCOAMix是一种基于超像素的网格混合数据增强方法，通过超像素注意力机制和跨图像超像素对比学习局部特征，解决了传统CutMix方法忽略局部上下文信息、损失物体部分信息以及标签不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有CutMix方法主要关注全局语义，过度减少对类别判别性局部上下文的注意力，导致性能提升瓶颈；同时，矩形或正方形切割会损失物体部分信息；现有方法解决标签不一致问题需要双重前向传播或依赖外部预训练网络，效率低下。

Method: 提出LGCOAMix，一种高效的上下文感知和物体部分感知的超像素网格混合方法。首次使用超像素注意力策略进行标签混合，从判别性超像素区域和跨图像超像素对比中学习局部特征。

Result: 在多个基准数据集上的实验表明，LGCOAMix在分类任务上优于最先进的CutMix数据增强方法，在CUB200-2011上实现了弱监督物体定位。该方法对CNN和Transformer网络都有效。

Conclusion: LGCOAMix通过超像素注意力机制解决了传统CutMix方法的局限性，在保持效率的同时显著提升了性能，为数据增强提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 激光切割中准确的材料识别至关重要，误识别会导致切割质量差、机器损坏或有害气体释放。现有散斑传感方法要么计算成本高，要么只能识别有限材料子集。

Method: 提出专门针对散斑模式设计的轻量级卷积神经网络，最小化参数同时保持高判别能力。使用完整的SensiCut数据集（59类材料），网络仅包含341k可训练参数。

Result: 在59类材料上达到95.05%测试准确率，宏平均和加权F1分数均为0.951。推理速度达295图像/秒，可在树莓派和Jetson设备上部署。当材料重新分组为9类和5类实用家族时，召回率超过98%并接近100%。

Conclusion: 紧凑的领域特定CNN在基于散斑的材料分类中优于大型骨干网络，推进了材料感知、可边缘部署的激光切割系统的可行性。
...
TL;DR: ICVision是首个通过AI视觉和自然语言推理模拟专家级EEG ICA成分分类的系统，直接解读ICA仪表板可视化，超越传统分类器性能。


<details>
  <summary>Details</summary>
Motivation: 传统EEG ICA成分分类器（如ICLabel）依赖手工特征，而ICVision旨在模拟神经学家通过视觉解读EEG成分的方式，实现AI在神经生理学中的视觉认知。

Method: 使用多模态大语言模型（GPT-4 Vision）直接解读ICA仪表板可视化（地形图、时间序列、功率谱、ERP图），将成分分类为六类（脑、眼、心、肌肉、通道噪声、其他噪声），并提供置信度和人类可理解的解释。

Result: 在124个EEG数据集的3,168个ICA成分上评估，ICVision与专家共识的一致性达到k=0.677，超越MNE ICLabel，97%的输出被专家评为可解释和可操作，在模糊情况下保留临床相关脑信号。

Conclusion: ICVision标志着科学AI的范式转变，模型不仅能分类，还能看、推理和沟通，为全球可扩展、可解释和可复现的EEG工作流程开辟道路，展示了AI在脑科学中专家级视觉决策能力。
...
TL;DR: Mammo-FM是首个专门针对乳腺X线摄影的领域特定基础模型，在140,677名患者的大规模数据集上预训练，能够在一个统一框架内完成癌症诊断、病理定位、结构化报告生成和癌症风险预测等核心临床任务。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球女性主要死因之一，现有通用基础模型在乳腺影像领域表现不足，缺乏专门针对该临床领域的统一模型，且临床应用中需要更好的可解释性和可审计性。

Method: 构建了迄今为止最大最全面的乳腺X线摄影数据集（140,677名患者，821,326张影像），开发了专门针对乳腺影像的领域特定基础模型Mammo-FM，采用图像-文本对齐架构，支持多任务学习。

Result: Mammo-FM在诊断、预后和报告生成任务中，在分布内和分布外数据集上均表现优异，尽管使用原生分辨率乳腺X线影像且参数量仅为最先进通用基础模型的三分之一，但在多个公共和私有基准测试中均超越现有模型。

Conclusion: 领域特定基础模型在临床应用中具有显著优势，围绕特定临床领域全谱任务设计的模型比通用模型更高效，强调了领域对齐评估的重要性，为乳腺影像临床实践提供了统一的强大工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在生成长序列3D人体反应动作时面临挑战，特别是在保持时间一致性和处理复杂动作（如舞蹈、武术）方面存在局限，需要更高效的框架来生成真实且多样的长序列反应动作。

Method: 结合运动VAE进行高效动作编码，使用基于Mamba的状态空间模型解码时间一致的反应动作。该设计支持从简单短序列到复杂长序列（如舞蹈、武术）的生成。

Result: 在NTU120-AS、Lindy Hop和InterX三个数据集上评估，在真实感、多样性和长序列生成方面与InterFormer、ReMoS、Ready-to-React等方法相比具有竞争力，同时推理速度有显著提升。

Conclusion: ReactionMamba是一个有效的3D人体反应动作生成框架，能够处理各种复杂度的动作序列，在保持高质量生成的同时大幅提升计算效率，为长序列动作生成提供了新解决方案。
...
TL;DR: DenseScan是一个新颖的3D场景理解数据集，通过自动化流程生成多层次描述，结合几何细节与语义丰富性，提升3D环境中的对象级理解和问答性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景理解数据集通常只提供几何和实例级信息，缺乏支持细致视觉-语言任务所需的丰富语义标注。高质量数据对于推动3D理解社区发展至关重要。

Method: 采用自动化流程，利用多视角2D图像和多模态大语言模型生成详细的多层次描述。包括场景元素的密集标注和基于场景的问题生成，整合对象属性、空间关系和场景上下文。

Result: 实验结果表明，与传统标注流程相比，该方法显著提升了3D环境中的对象级理解和问答性能。数据集和标注流程已公开发布。

Conclusion: DenseScan通过结合几何细节与语义丰富性，拓宽了下游任务范围，旨在催化3D场景理解的新途径，使研究人员能够用更丰富、更具上下文感知的标注处理现实世界环境的复杂性。
...
TL;DR: RHC是一种基于稀疏RGB视频的人物特定方法，通过单次网络前向传播实现自由视角渲染和重光照，无需传统的OLAT采集，使用物理启发的特征和3D高斯泼溅技术。


<details>
  <summary>Details</summary>
Motivation: 传统基于OLAT（一次一光）的人物重光照方法需要昂贵的采集过程和基础光照生成，限制了实际应用。需要一种能够从稀疏视角视频中直接实现高质量重光照的高效方法。

Method: 1) 提出新的采集策略和数据集：在多视角光照舞台中交替使用随机环境光照和均匀光照帧；2) 从粗糙人体网格代理推导物理启发特征（几何、反照率、着色、虚拟相机视角）；3) 基于Transformer的RelightNet通过交叉注意力机制处理这些特征和新光照条件，回归出与纹理对齐的3D高斯泼溅表示的重光照外观。

Result: 实验表明，该方法在视觉保真度和光照再现方面优于现有最先进方法，能够高效地在单次前向传播中计算渲染方程。

Conclusion: RHC方法通过创新的采集策略、物理启发特征表示和Transformer架构，实现了从稀疏视频输入的高质量人物重光照，为动态人物渲染提供了高效实用的解决方案。
...
TL;DR: UniDiff：一种参数高效框架，通过单ImageNet预训练扩散模型适应多模态遥感数据，仅需5%参数调整，解决稀疏标注问题


<details>
  <summary>Details</summary>
Motivation: 稀疏标注严重限制了多模态遥感的发展，即使像MSFMamba这样的先进监督方法也受限于标注数据可用性。ImageNet预训练模型虽有丰富视觉表示，但难以适应HSI和SAR等异构模态且无需大量标注数据

Method: 提出UniDiff框架：1) 使用FiLM-based timestep-modality conditioning进行时间步-模态条件化；2) 仅调整约5%参数的参数高效适应；3) 伪RGB锚定技术以保留预训练表示并防止灾难性遗忘

Result: 在两个已建立的多模态基准数据集上验证，无监督适应预训练扩散模型有效缓解了标注约束，实现了多模态遥感数据的有效融合

Conclusion: UniDiff通过参数高效的无监督适应方法，使单ImageNet预训练扩散模型能够有效提取多模态遥感特征，解决了稀疏标注下的多模态遥感挑战
...
<details>
  <summary>Details</summary>
Motivation: 传统cine MRI仅提供2D切片图像，限制了全面理解心脏形态和生理机制，特别是在健康和病理条件下

Method: 提出HeartFormer点云补全网络，包含SA-DSTNet（生成初始粗点云）和SA-GFRTNet（渐进细化），将传统单类点云补全扩展到多类

Result: 在HeartCompv1和UK Biobank数据集上的跨域实验表明，HeartFormer实现了鲁棒、准确且可泛化的性能，一致超越SOTA方法
...
TL;DR: USB是一个统一的双向病理-健康脑图像生成与编辑框架，通过配对扩散机制建模病变与脑解剖结构的联合分布，实现病理和健康图像的生成与编辑。
...
Result: 在六个公共脑MRI数据集（包括健康对照、中风和阿尔茨海默病患者）上的实验表明，USB能够产生多样且真实的结果，建立了首个脑图像生成与编辑的统一基准。

Conclusion: USB作为首个统一的双向病理-健康脑图像生成与编辑框架，为可扩展数据集创建和稳健神经影像分析开辟了机会，代码已开源。
...
TL;DR: 提出HIMOSA轻量级遥感图像超分辨率框架，通过内容感知稀疏注意力机制和分层窗口扩展，在保持重建性能的同时实现快速推理。


<details>
  <summary>Details</summary>
Motivation: 遥感应用（如灾害检测和响应）需要实时效率和模型轻量化，现有遥感图像超分辨率方法往往需要在模型性能和计算效率之间进行权衡。

Method: 提出HIMOSA框架：1）利用遥感图像固有冗余性，引入内容感知稀疏注意力机制；2）通过分层窗口扩展有效利用遥感图像中的多尺度重复模式；3）通过调整注意力稀疏度降低计算复杂度。
...
TL;DR: AI系统在低剂量CT扫描中直接进行肺结节检测和恶性诊断，在25,709次扫描上训练，性能优于放射科医生和现有AI模型，AUC达0.98，对早期癌症检测尤其出色。


<details>
  <summary>Details</summary>
Motivation: 当前肺癌筛查依赖于结节大小和生长变化，这会导致诊断延迟。需要开发能够在结节层面直接进行检测和恶性诊断的AI系统，以克服数据集规模和可解释性限制。

Method: 设计了浅层深度学习和基于特征的专业模型集成系统，在25,709次CT扫描（包含69,449个标注结节）上进行训练和评估。

Result: 系统内部AUC达0.98，独立队列AUC达0.945，每扫描0.5个假阳性下灵敏度达99.3%。在所有结节大小和分期上均优于放射科医生，在1期癌症和所有生长指标（包括体积倍增时间）上表现突出，对不确定和缓慢生长结节的诊断提前达一年。

Conclusion: 该AI系统重新定义了肺癌筛查，直接进行结节层面的检测和恶性诊断，解决了AI采用的关键障碍，有望显著改善早期肺癌检测。
...
TL;DR: 提出模块化AR代理系统，集成多模态大语言模型与视觉模型，实现空间关系推理和语言条件空间检索，支持从简单物体识别到多物体关系推理的复杂查询。


<details>
  <summary>Details</summary>
Motivation: 传统AR系统依赖固定类别检测器或标记物，无法处理复杂开放词汇的自然语言查询，需要增强对物理环境的交互式场景理解能力。

Method: 采用模块化AR代理架构，集成MLLMs与坐标感知感知工具，构建动态AR场景图编码九种关系类型，支持任务自适应感兴趣区域高亮和上下文空间检索。

Result: 系统能够处理从简单物体识别到多物体关系推理的复杂查询，返回米级精度的3D锚点，支持人类在环优化，并引入GroundedAR-Bench评估框架。

Conclusion: 模块化AR代理作为中介，为MLLMs增强现实世界空间智能，支持即插即用视觉语言模型，实现交互式场景理解，推动AR系统向更自然的人机交互发展。
...
TL;DR: TGSFormer：一个可扩展的时序高斯泼溅框架，用于具身3D语义场景补全，通过持久高斯内存和置信感知融合实现高效长时场景建模


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的方法存在冗余和可扩展性问题：随机初始化导致冗余，深度引导方法仍受限于局部性，随着场景规模增大会产生延迟和内存开销

Method: 提出TGSFormer框架：1) 维护持久高斯内存进行时序预测；2) 双时序编码器通过置信感知交叉注意力处理当前和历史高斯特征；3) 置信感知体素融合模块将重叠基元合并为体素对齐表示，调节密度并保持紧凑性

Result: 在局部和具身SSC基准测试中达到最先进结果，以显著更少的基元实现更优的准确性和可扩展性，同时保持长期场景完整性

Conclusion: TGSFormer解决了现有高斯方法在冗余和可扩展性方面的限制，通过时序融合和紧凑表示实现了高效、准确的具身3D语义场景补全
...
TL;DR: 提出基于最优传输的细粒度数据集蒸馏方法，通过全局和实例级对齐提升蒸馏效果，在ImageNet-1K上比SOTA方法提升至少4%准确率
...
Result: 在多种架构和大规模数据集上的实验表明，该方法始终优于现有最先进方法，在IPC=10设置下，每种架构在ImageNet-1K上至少获得4%的准确率提升

Conclusion: 通过最优传输框架实现细粒度的全局和实例级对齐，能够有效保留复杂高维分布的几何特性，显著提升数据集蒸馏的性能
...
TL;DR: ART-ASyn是一个用于胸部X光片的无监督异常检测框架，通过生成解剖结构一致且纹理逼真的肺部异常合成图像，提供像素级监督训练，并支持零样本异常分割。


<details>
  <summary>Details</summary>
Motivation: 现有合成异常方法生成的异常与真实病理模式视觉差异大，且忽略解剖结构一致性，需要更逼真、解剖学合理的异常合成方法来提升无监督异常检测性能。

Method: 提出ART-ASyn框架：1) 使用渐进二值阈值分割方法(PBTSeg)进行肺部分割；2) 基于纹理增强生成解剖结构一致的肺部异常；3) 为每个正常样本生成合成异常及其精确像素级掩码，提供显式分割监督。

Result: 框架能够生成逼真且解剖结构一致的肺部异常，不仅支持传统的一类分类任务，还能在未见数据集上进行零样本异常分割，展示了良好的泛化能力。

Conclusion: ART-ASyn通过生成解剖结构一致且逼真的合成异常，为无监督异常检测提供了有效的像素级监督，并在零样本异常分割任务中表现出良好的泛化性能。
...
TL;DR: 提出一种基于直线特征和惯性测量的视觉里程计新方法，通过分析图像空间中直线运动形成的直纹曲面来估计相机运动，避免传统点特征匹配的高计算成本和精度问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉里程计依赖图像序列中的特征点提取和光流计算，这种点对点对应关系计算成本高且精度不稳定，影响里程计估计质量。虽然已有研究尝试使用直线特征或融合其他传感器（事件相机、IMU）来改善性能，但仍严重依赖对应关系。

Method: 提出一种新颖算法，利用相机观察直线运动时在图像空间-时间中形成的直纹曲面。通过分析这些曲面的形状来获取里程计信息，仅需要基于点-线关联的微分计算更新。结合IMU的惯性测量约束曲面，大幅降低解空间的维度。

Result: 该方法能够从直纹曲面重建3D场景并计算视觉里程计，特别适合事件相机擅长边缘检测的特性，通过IMU约束提高了估计的稳定性和准确性。

Conclusion: 通过分析直线运动形成的直纹曲面并结合IMU约束，提出了一种计算效率更高、更稳定的视觉里程计方法，避免了传统点特征匹配的局限性，特别适用于事件相机等边缘检测优势明显的传感器。
...
<details>
  <summary>Details</summary>
Motivation: 现有合成视频数据集主要关注视觉模态，少数包含音频的数据集局限于面部深度伪造，无法应对日益增长的多模态AI生成内容，阻碍了可信检测系统的发展。

Method: 创建MVAD数据集，包含三种真实视频-音频伪造模式，使用多种最先进的生成模型确保高质量，涵盖现实和动漫视觉风格、四种内容类别和四种多模态数据类型。

Result: MVAD是首个专门用于检测AI生成多模态视频-音频内容的综合数据集，具有真实多模态性、高感知质量和全面多样性三个关键特征。

Conclusion: MVAD填补了多模态AI生成内容检测的关键空白，将促进可信检测系统的发展，数据集将在GitHub上公开提供。
...
TL;DR: AMDET提出了一种无需先验知识的模型级后门检测框架，通过分析文本编码器中的特征同化现象来检测Vision-Language预训练模型中的后门攻击。


<details>
  <summary>Details</summary>
Motivation: 现有的后门检测方法通常依赖于训练数据集、后门触发器、攻击目标或下游分类器的先验知识，这在现实应用中不切实际。需要一种无需这些先验知识的检测方法。

Method: AMDET基于后门文本编码器的特征同化特性：后门样本中所有token的表示具有高度相似性。通过梯度反演token嵌入来恢复能够激活后门行为的隐式特征，并通过分析损失景观过滤自然后门特征。

Result: 在3,600个后门和良性微调模型上的实验表明，AMDET检测后门的F1分数达到89.90%，在RTX 4090 GPU上约5分钟完成一次完整检测，并对自适应攻击具有强鲁棒性。

Conclusion: AMDET是一种有效的无需先验知识的模型级后门检测框架，能够检测Vision-Language预训练模型中的后门攻击，包括识别OpenAI官方CLIP模型中的自然后门特征。
...
TL;DR: 首次提出基于毫米波雷达的人体运动预测方法mmPred，通过双域历史运动表示和全局骨骼关系Transformer，解决了雷达信号噪声和时域不一致问题，在mmBody和mm-Fi数据集上分别超越现有方法8.6%和22%。


<details>
  <summary>Details</summary>
Motivation: 现有基于RGB-D相机的人体运动预测方法对光照条件敏感且存在隐私问题，限制了在消防、医疗等实际应用。毫米波雷达具有鲁棒性和隐私保护特性，但其信号存在镜面反射和多径效应，导致噪声和时域不一致的测量结果。

Method: 提出mmPred框架，包含：1) 双域历史运动表示：时域姿态细化分支学习细粒度细节，频域主导运动分支捕捉全局运动趋势并抑制帧级不一致；2) 全局骨骼关系Transformer作为扩散主干，建模全局关节间协作，使受损关节能动态聚合其他关节信息。

Result: 在mmBody和mm-Fi数据集上实现了最先进性能，分别超越现有方法8.6%和22%。

Conclusion: 首次将毫米波雷达引入人体运动预测，提出的mmPred框架有效解决了雷达信号特有的噪声和时域不一致问题，为隐私保护的运动预测提供了新解决方案。
...
TL;DR: SMamDiff：基于空间Mamba的单阶段扩散模型，通过残差DCT运动编码和骨架绘制空间Mamba模块，实现时空一致的人体运动预测，在保持运动学合理性的同时平衡准确性与多样性。


<details>
  <summary>Details</summary>
Motivation: 现有人体运动预测方法存在局限性：确定性方法忽略不确定性，概率性方法牺牲运动学合理性，扩散模型虽改善准确性与多样性权衡但依赖多阶段流程，不适合边缘部署。需要确保单阶段扩散模型中的时空一致性。

Method: 提出SMamDiff模型：1）残差DCT运动编码：减去最后观察姿态后进行时间DCT，减少DC分量主导，突出高频信息，让模型学习关节如何运动而非位置；2）骨架绘制空间Mamba模块：按顺序逐关节处理，使后续关节条件依赖于先前关节，诱导长距离跨关节依赖。

Result: 在Human3.6M和HumanEva数据集上，该模型在单阶段概率性HMP方法中达到最先进结果，同时比多阶段扩散基线具有更低的延迟和内存消耗。

Conclusion: 通过残差DCT编码和空间Mamba模块确保时空一致性，SMamDiff在单阶段扩散模型中实现了准确性与多样性的更好平衡，适合边缘部署的实际应用。
...
TL;DR: MM-DETR：基于Mamba的轻量级多模态遥感目标检测框架，通过双粒度融合编码器、区域感知2D选择性扫描完成分支和轻量级频率感知模态适配器，在保持高性能的同时大幅减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有多模态目标检测方法面临性能与轻量化设计的平衡问题：基于注意力或可变形卷积的融合方法复杂；共享主干网络无法充分建模模态特异性；双流架构参数翻倍，限制实际部署。
...
Conclusion: MM-DETR通过创新的融合编码器、模态补全分支和轻量级适配器设计，成功解决了多模态目标检测中性能与轻量化的平衡问题，为实际部署提供了高效解决方案。
...
TL;DR: 较小视觉分割模型自然形成类似人类的粗粒度身体表征，而较大模型则倾向于过度详细的细粒度编码，表明有限计算资源下可涌现出类似人类的物理推理表征。


<details>
  <summary>Details</summary>
Motivation: 人类物理推理依赖于内部的"身体"表征——捕捉物体范围的粗粒度体积近似，支持对运动和物理的直观预测。虽然心理物理学证据表明人类使用这种粗粒度表征，但其内部结构仍不清楚。本研究探索视觉分割模型是否发展出类似表征。

Method: 将50名人类参与者的心理物理学实验改编为语义分割任务，测试了7个不同大小的分割网络家族。比较了不同规模模型形成的表征与人类表征的相似性。

Result: 较小模型自然形成类似人类的粗粒度身体表征，而较大模型倾向于形成过度详细的细粒度编码。这表明粗粒度表征可以在有限计算资源下自然涌现。

Conclusion: 机器表征为理解大脑物理推理结构提供了可扩展的路径，粗粒度表征在有限计算资源下自然形成，而过度复杂的模型可能偏离人类认知模式。
...
TL;DR: 提出THCRL方法解决多视图聚类中的不可信融合问题，通过DSHF模块进行可信融合和AKCL模块增强表示学习，在深度多视图聚类任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 多视图聚类面临不可信融合的挑战，主要源于两个问题：1) 现有方法忽略单视图中的固有噪声；2) 传统对比学习方法只关注同一实例的不同视图，而忽略同一簇内最近邻的结构信息，导致多视图融合方向错误。

Method: 提出可信层次对比表示学习(THCRL)，包含两个关键模块：1) 深度对称层次融合(DSHF)模块，利用UNet架构结合多种去噪机制实现多视图数据的可信融合；2) 平均K最近邻对比学习(AKCL)模块，将融合表示与视图特定表示对齐，增强同一簇内样本的表示相似性而非仅关注跨视图的同一实例。

Result: 大量实验表明，THCRL在深度多视图聚类任务中实现了最先进的性能。

Conclusion: THCRL通过解决多视图聚类中的不可信融合问题，提供了一种有效的深度多视图聚类方法，在性能上超越了现有方法。
...
TL;DR: POLARIS通过将引导尺度ω作为步进变量，推导出最小化反演误差的数学公式，显著改善扩散模型反演质量，只需一行代码即可实现。


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的反演-去噪范式在图像编辑和修复任务中表现出色，但存在重建退化问题。研究发现关键因素是近似噪声误差——用t-1步的预测来近似t步的噪声，导致反演过程中严重的误差累积。

Method: 提出POLARIS方法，将反演问题从误差补偿问题重新表述为误差溯源问题。不通过优化嵌入或潜在代码来抵消累积漂移，而是将引导尺度ω作为步进变量，推导出数学上严谨的公式来最小化每一步的反演误差。

Result: POLARIS仅需一行代码即可显著改善反演潜在质量，性能开销可忽略不计。能大幅减轻噪声近似误差，持续提升下游任务的准确性。

Conclusion: POLARIS通过数学推导最小化反演误差，为扩散模型的反演-去噪范式提供了更稳健和自适应的解决方案，显著改善了图像编辑和修复任务的质量。
...
TL;DR: 提出了PorePatch数据集和DMCE框架来解决面部弱纹理区域匹配问题，深度学习描述符在匹配任务上表现优异但在3D重建中优势有限。


<details>
  <summary>Details</summary>
Motivation: 面部皮肤区域的弱纹理特性给局部描述符匹配带来挑战，而缺乏毛孔尺度图像块数据集限制了深度学习描述符在面部领域的发展。

Method: 提出PorePatch数据集和Data-Model Co-Evolution (DMCE)框架，从高分辨率面部图像生成渐进精炼的高质量数据集，并在该数据集上训练现有SOTA模型。

Result: SOTA模型在匹配任务上达到1.91%的FPR95，显著优于PSIFT的22.41%，但在3D重建任务中与传统描述符相比优势不明显。

Conclusion: 深度学习描述符在面部弱纹理区域处理上仍有局限性，该领域还需要更多研究工作。
...
TL;DR: 提出EZ-SP：一种完全GPU加速的可学习超点分割算法，比现有方法快13倍，结合轻量级分类器，在保持SOTA精度的同时实现72倍推理加速和120倍参数减少。


<details>
  <summary>Details</summary>
Motivation: 基于超点的3D语义分割管道通常受限于CPU绑定的分区步骤，这成为性能瓶颈。需要一种完全GPU加速的、可学习的超点生成方法，以提高效率并支持实时推理。

Method: 提出可学习的全GPU分区算法，无需手工特征，使用可微代理损失训练（<60k参数，训练<20分钟）。结合轻量级超点分类器，整个管道占用<2MB显存。

Result: 超点生成比现有方法快13倍，完整管道推理快72倍，参数减少120倍。在室内扫描（S3DIS）、自动驾驶（KITTI-360）和航空LiDAR（DALES）三个领域达到与点基SOTA模型相当的精度。

Conclusion: EZ-SP提供了一种高效、紧凑的超点分割解决方案，支持实时推理和大规模场景处理，在保持精度的同时显著提升效率，适用于资源受限的边缘设备。
...
TL;DR: WiseEdit是一个知识密集型图像编辑基准测试，用于全面评估认知和创意导向的图像编辑能力，包含1220个测试案例，揭示了当前最先进模型在知识推理和创意合成方面的局限性。
...
Method: 基于人类认知创作过程，将图像编辑分解为三个级联步骤：意识(Awareness)、解释(Interpretation)和想象(Imagination)，每个步骤对应特定挑战任务。同时包含三种基本知识类型：陈述性知识、程序性知识和元认知知识。

Result: 构建了包含1220个测试案例的WiseEdit基准测试，客观揭示了当前最先进图像编辑模型在基于知识的认知推理和创意合成能力方面的局限性。

Conclusion: WiseEdit提供了一个全面的知识密集型基准测试，能够更准确地评估图像编辑模型的认知和创意能力，为未来模型发展提供了重要的评估工具。
...
TL;DR: STAMP提出了一种新的多模态大语言模型范式，通过"全掩码预测"同时解决对话能力、分割性能和推理速度的三难问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在集成分割任务时面临三难困境：需要同时保持对话能力、实现高分割性能、确保快速推理。现有方法要么引入冲突的像素级目标损害对话能力，要么使用自回归方法在分割性能和推理速度之间做出妥协。

Method: 提出全掩码预测范式，将自回归对话生成与非自回归掩码预测解耦。STAMP模型在生成文本响应后，将分割掩码预测视为图像块的并行"填空"任务，在单次前向传播中预测整个掩码。

Result: STAMP在多个分割基准测试中显著优于最先进的方法，在保持对话能力的同时实现了高分割性能和快速推理，无需任何妥协。

Conclusion: STAMP通过创新的全掩码预测范式成功解决了多模态大语言模型中的分割三难问题，为同时实现高质量对话、优秀分割性能和快速推理提供了有效解决方案。
...
TL;DR: DiSCo是一个语义视频压缩框架，在极低码率下通过传输文本描述、降质视频和可选草图/姿态等紧凑模态，利用条件视频扩散模型重建高质量视频，相比传统编解码器在感知质量上提升2-10倍。


<details>
  <summary>Details</summary>
Motivation: 传统视频编解码器在极低码率下会崩溃并产生严重伪影，这是因为像素精度与人类感知之间存在根本性不匹配。需要一种新的压缩方法，专注于传输最有意义的信息而非像素保真度。

Method: 将源视频分解为三个紧凑模态：文本描述（语义）、时空降质视频（外观）和可选草图/姿态（运动）。提出时间前向填充、令牌交错和模态特定编解码器来改善多模态生成和模态紧凑性。使用条件视频扩散模型从这些紧凑表示重建高质量、时间一致的视频。

Result: 在低码率下，该方法在感知指标上优于基线语义编解码器和传统编解码器2-10倍，能够在极低比特率下生成高质量、时间连贯的视频。

Conclusion: DiSCo通过将视频分解为语义、外观和运动线索的紧凑表示，并利用生成先验进行细节合成，为极低码率视频压缩提供了一种有效解决方案，超越了传统基于像素保真度的方法。
...
TL;DR: 提出了一种结合物理指导的3D形状生成方法，通过交替更新过程将物理属性融入生成过程，提升工业设计形状的真实性


<details>
  <summary>Details</summary>
Motivation: 现有3D形状生成模型虽然能合成视觉上逼真的形状，但对于经过工程设计过程的形状（如汽车），其真实性与物理属性（如空气动力学效率）紧密相关。现有方法缺乏物理知识，无法利用物理属性增强形状生成的真实性

Method: 提出统一的基于物理的3D形状生成流程：1) 引入具有明确物理指导的流匹配模型，采用交替更新过程；2) 迭代执行基于速度的更新和基于物理的细化，逐步调整潜在代码以对齐期望的3D形状和物理属性；3) 在速度更新步骤中加入物理感知正则化项；4) 构建形状-物理变分自编码器(SP-VAE)，将形状和物理信息联合编码到统一的潜在空间

Result: 在三个基准测试上的实验表明，这种协同方法超越了单纯的视觉逼真度，显著提高了形状的真实性

Conclusion: 通过将物理指导融入生成过程，提出的方法能够生成不仅视觉逼真，而且物理有效的3D形状，特别适用于工业设计应用
...
TL;DR: 利用车载CCTV监控，结合目标检测、跟踪、重识别、OCR和时间戳等技术，为撒哈拉以南非洲的公共交通恢复乘客起讫点流量数据，在良好条件下准确率高，但在实际拥挤、光照变化等压力下性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 撒哈拉以南非洲的公共交通经常在过度拥挤条件下运营，现有自动化系统无法可靠捕获乘客流量数据。利用已部署用于安全的车载CCTV监控，为这一地区开发可行的乘客流量监测方案。

Method: 提出一个基线管道，结合YOLOv12目标检测、BotSORT跟踪、OSNet重识别嵌入、基于OCR的时间戳和基于遥测的站点分类，恢复公交车的起讫点流量数据。

Result: 在内罗毕和基加利公交车的标注CCTV片段上，系统在低密度、光照良好条件下达到高计数准确率（召回率≈95%，精确率≈91%，F1≈93%），生成的OD矩阵与人工计数高度匹配。但在实际压力下（如过度拥挤、彩色转单色、姿态变化、非标准门使用），性能急剧下降（例如高峰时段登车计数低估约40%，单色片段召回率下降约17个百分点）。

Conclusion: 系统揭示了部署特定的故障模式，并强调了为撒哈拉以南非洲公共交通开发更鲁棒、面向部署的重识别方法的必要性。现有方法在理想条件下有效，但在实际运营压力下需要改进。
...
TL;DR: NewtonRewards：首个基于可验证奖励的物理基础后训练框架，通过提取光流和外观特征作为速度和质量的代理，强制执行牛顿运动学和质量守恒约束，显著提升视频生成的物理合理性。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型虽然能生成视觉上逼真的片段，但经常违反基本物理定律（如物体漂浮、加速度漂移、碰撞不一致），揭示了视觉真实性与物理真实性之间的持久差距。

Method: 提出NewtonRewards框架：1）使用冻结的实用模型从生成视频中提取可测量代理（光流作为速度代理，高级外观特征作为质量代理）；2）通过两个互补的奖励强制执行牛顿结构：牛顿运动学约束（强制恒定加速度动力学）和质量守恒奖励（防止退化解）。

Result: 在NewtonBench-60K基准测试的五个牛顿运动基元（自由落体、水平/抛物线投掷、斜坡下滑/上滑）上，NewtonRewards在视觉和物理指标上持续优于现有后训练方法，提高了物理合理性、运动平滑性和时间一致性，并在高度、速度和摩擦力的分布外偏移下保持强大性能。
...
TL;DR: 使用Google Nano Banana生成的合成胸部X光图像训练肺炎分类器，在真实世界数据上验证显示可行，但存在多样性和对齐挑战。


<details>
  <summary>Details</summary>
Motivation: 探索使用AI生成的合成医学图像数据来训练医疗AI模型的可能性，解决真实医疗数据获取困难、隐私保护等问题。

Method: 使用Google最新AI图像生成模型Nano Banana生成合成胸部X光图像，训练肺炎分类器，并在两个真实世界数据集（RSNA肺炎检测数据集和胸部X光数据集）上进行外部验证。

Result: 在RSNA数据集上AUROC为0.923，AUPR为0.900；在胸部X光数据集上AUROC为0.824，AUPR为0.913。结果显示仅使用合成数据训练的模型在真实数据上表现良好。

Conclusion: 合成数据在医疗AI开发中具有潜力，但存在提示设计多样性控制、后处理对齐等挑战，临床转化需要充分验证、监管批准和伦理监督。
...
TL;DR: FR-TTS：一种针对文本生成的新型测试时扩展方法，通过填充方案估计中间序列的未来轨迹，解决图像生成中测试时扩展难以应用于文本生成的问题。


<details>
  <summary>Details</summary>
Motivation: 测试时扩展在图像生成中效果显著，但难以应用于文本生成，因为中间token序列的奖励与最终生成结果的奖励相关性低，导致中间表示无法有效指导剪枝方向。

Method: 提出填充式奖励（FR），通过寻找合理的填充方案来补全中间序列，估计其未来轨迹；基于此开发FR-TTS策略，结合多样性奖励和动态权重调度来全面评估中间样本。

Result: 实验证明FR-TTS在多个基准测试和不同奖励模型上均优于现有方法，中间样本与最终样本的奖励相关性显著提高，token置信度等内在信号也验证了FR的有效性。

Conclusion: FR-TTS成功将测试时扩展技术应用于文本生成，通过填充式奖励解决了中间序列评估难题，为提升文本生成质量提供了有效的新方法。
...
TL;DR: RecruitView数据集包含2011个自然视频面试片段，27,000对比较标注，涵盖12个维度。提出CRMF几何深度学习框架，在多流形上建模行为表征，参数减少40-50%，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 自动人格和软技能评估面临数据集有限和现有方法无法捕捉人类特质几何结构的挑战。需要更好的数据集和方法来建模多层次的行为表征。

Method: 提出CRMF（跨模态回归与流形融合）框架，在双曲、球面和欧几里得流形上建模行为表征。使用几何特定专家网络捕捉层次结构、方向性模式和连续变化，通过自适应路由机制动态加权专家贡献，采用切空间融合减少参数。

Result: CRMF显著优于基线方法，Spearman相关系数提升达11.4%，一致性指数提升6.0%。相比大型多模态模型，可训练参数减少40-50%。RecruitView数据集已公开。

Conclusion: RecruitView数据集和CRMF框架为多模态行为分析提供了有效解决方案，通过几何深度学习建模人类特质的固有结构，在减少参数的同时实现了性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有面部情感分析方法虽然基于动作单元(AU)，但很少从数据中推断AU与表情之间心理学上合理的因果关系。需要建立可解释的因果推理框架来理解面部行为背后的潜在依赖关系。

Method: 提出CausalAffect框架：1）两级极性感知因果层次结构（AU-AU和AU-Expression），整合群体级规律与样本自适应结构；2）特征级反事实干预机制，增强真实因果效应同时抑制虚假相关性；3）无需联合标注数据集或手工因果先验。

Result: 在六个基准测试上的实验表明：1）恢复的因果结构与既定心理学理论一致；2）揭示了新的抑制性和先前未表征的依赖关系；3）在AU检测和表情识别任务上均达到最先进水平。

Conclusion: CausalAffect建立了因果发现与可解释面部行为之间的原则性连接，为面部情感分析提供了首个因果图发现框架，在保持心理学合理性的同时提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 尽管GPT-Image-1和Qwen-Image等先进模型在文本到图像一致性和世界知识方面取得显著进展，但在生成逼真图像方面仍存在不足，常产生带有明显AI伪影的"虚假"图像，如"过度光滑的皮肤"和"油腻的面部光泽"，未能实现"与现实无法区分"的生成目标。

Method: RealGen框架包含三个核心组件：1) LLM组件用于提示优化；2) 扩散模型用于逼真图像生成；3) 受对抗生成启发的"检测器奖励"机制，通过语义级和特征级合成图像检测器量化伪影并评估真实感。使用GRPO算法优化整个生成流程，并提出了RealBench自动评估基准，采用检测器评分和竞技场评分实现无需人工参与的逼真度评估。

Result: 实验表明，RealGen在真实感、细节和美学方面显著优于通用模型（如GPT-Image-1和Qwen-Image）以及专门的逼真模型（如FLUX-Krea）。

Conclusion: RealGen通过创新的检测器奖励机制和优化方法，成功提升了文本到图像生成的逼真度，实现了更接近"与现实无法区分"的生成目标，并提供了有效的自动评估方法。
...
TL;DR: 提出结构化上下文学习(SCL)方法，通过结构化序列划分(SPoS)为GEBD任务提供结构化上下文，实现更好的速度-精度权衡
...
Method: 1. 引入结构化序列划分(SPoS)将输入帧序列分区，提供结构化上下文；2. 计算组间相似性捕捉帧间差异；3. 使用轻量级全卷积网络基于分组相似性图确定事件边界；4. 采用高斯核预处理地面真值边界以解决标注模糊性。

Result: 在Kinetics-GEBD、TAPOS和镜头转换检测数据集上进行了广泛评估，证明了该方法优于现有最先进方法。SPoS的计算复杂度与视频长度呈线性关系。

Conclusion: 提出的结构化上下文学习方法通过SPoS提供了灵活且高效的事件边界检测框架，实现了更好的速度-精度权衡，并在多个数据集上表现出优越性能。
...
TL;DR: TACS框架通过任务对齐的上下文选择，让ViTs学习选择真正能提升任务性能的配对示例，而不是仅选择表面相似的示例


<details>
  <summary>Details</summary>
Motivation: 人类通过比较图像与相关示例来解决视觉不确定性，但ViTs缺乏识别哪些示例能真正改善预测的能力。现有方法通常基于相似性检索，但这不一定能提升任务性能。

Method: 提出任务对齐上下文选择(TACS)框架，联合训练选择器网络和任务模型，采用梯度监督和强化学习的混合优化方案，使检索成为学习目标的一部分。

Result: 在18个数据集（涵盖细粒度识别、医学图像分类和分割）上，TACS始终优于基于相似性的检索方法，特别是在具有挑战性或数据有限的场景中。

Conclusion: 通过将选择与任务奖励对齐，TACS使判别模型能够发现哪些上下文示例真正有帮助，为视觉模型的有效上下文选择提供了新方法。
...
TL;DR: CC-FMO是一个零样本、相机条件化的单图像到3D场景生成管道，通过混合实例生成器和相机条件化尺度求解算法，生成高质量、相机对齐的合成场景。


<details>
  <summary>Details</summary>
Motivation: 从单图像生成高质量3D场景对AR/VR和具身AI应用至关重要。现有方法存在泛化能力差、依赖小数据集训练、场景级一致性不足等问题，特别是基于大规模3D基础模型的方法在场景生成中仍面临物体姿态估计不准确和空间不一致的挑战。

Method: CC-FMO采用零样本、相机条件化的管道，包含：1) 混合实例生成器，结合语义感知的向量集表示和细节丰富的结构化潜在表示；2) 相机条件化尺度求解算法，使基础姿态估计模型能应用于场景生成任务，确保场景级一致性。

Result: 大量实验表明，CC-FMO能一致地生成高保真度、相机对齐的合成场景，在所有最先进方法中表现最优。

Conclusion: CC-FMO通过创新的混合表示和相机条件化尺度求解，解决了单图像到3D场景生成中的场景一致性和实例保真度问题，为AR/VR和具身AI应用提供了有效的解决方案。
...
TL;DR: 提出基于智能手机的结构光系统，使用2D-DTW算法匹配地面投影网格，实现低成本移动机器人的地形感知


<details>
  <summary>Details</summary>
Motivation: 低成本移动机器人在不平坦地形上运行时，小的颠簸或倾斜难以视觉感知但显著影响运动稳定性，需要一种简单有效的地形感知方案

Method: 使用智能手机结构光系统投影网格到地面，提出拓扑约束的二维动态时间规整(2D-DTW)算法进行网格匹配，在资源有限平台上实现准确三角测量

Result: 开发了可在资源有限平台上运行的系统，2D-DTW算法不仅能用于地形感知，还可作为图像处理中结构化网格匹配的通用工具

Conclusion: 提出的智能手机结构光系统和2D-DTW算法为低成本移动机器人提供了有效的地形感知解决方案，同时扩展了网格匹配的应用范围
...
TL;DR: 利用预训练图像生成模型作为机器人视觉规划器，通过LoRA微调生成连贯的机器人操作视频，无需大规模视频数据


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型需要大量领域特定数据且泛化能力有限，而预训练语言-图像模型展现出强组合性和时间连贯性，暗示其具有视频生成潜力，可作为机器人视觉规划器
...
TL;DR: 提出Cross-Temporal 3D Gaussian Splatting框架，利用稀疏图像和历史场景先验，高效重建和更新不同时间段的3D场景，支持非连续捕获和时间变化分析。


<details>
  <summary>Details</summary>
Motivation: 在计算机视觉中，保持3D场景表示的时间一致性是一个重大挑战。从稀疏视角观测更新3D场景对城市规划、灾害评估、历史遗址保护等实际应用至关重要，因为这些场景通常无法获得密集扫描或扫描不切实际。

Method: 提出三阶段框架：1) 跨时间相机对齐，估计和对齐不同时间戳的相机姿态；2) 基于干扰的置信度初始化，识别时间戳间未变化区域以指导更新；3) 渐进式跨时间优化，迭代整合历史先验信息到3D场景中提升重建质量。

Result: 实验结果显示，在重建质量和数据效率方面相比基线方法有显著改进。该方法支持非连续捕获，不仅能用新稀疏视图更新现有场景，还能借助当前捕获从有限数据中恢复过去场景。

Conclusion: 该方法为场景版本控制、跨时间数字孪生和长期空间文档记录提供了有前景的解决方案，展示了仅用稀疏图像实现时间变化分析的潜力，并能按需重建为详细3D表示。
...
TL;DR: 提出SAIDO框架，通过场景感知专家模块和重要性引导动态优化机制，解决AI生成图像检测中的泛化问题，在持续学习和开放世界场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: AI生成图像技术的滥用引发安全担忧，但现有检测方法面临泛化挑战：难以适应新兴生成方法和真实世界场景中的内容类型变化。
...
Conclusion: SAIDO框架通过场景感知和重要性引导优化，有效解决了AI生成图像检测的泛化问题，在持续学习和开放世界场景中表现出优越性能。
...
TL;DR: 提出混合方法结合3D生成模型、语义感知变形和3D高斯溅射，用于多人多物体动态场景的几何重建，在HOI-M3数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界的人造环境高度动态，包含多人及其与周围物体的复杂交互。虽然3D几何建模对AR/VR、游戏和具身AI等应用至关重要，但由于多样的运动模式和频繁遮挡等挑战，这一领域仍未被充分探索。单目设置下，仅基于高斯溅射渲染损失优化场景时，在严重遮挡下保持结构一致性变得困难。

Method: 提出混合方法：1) 使用3D生成模型生成场景元素的高保真网格；2) 语义感知变形（刚性物体的刚性变换和基于LBS的人体变形），将变形后的高保真网格映射到动态场景中；3) 基于高斯溅射优化各个元素以进一步细化其在场景中的对齐。

Result: 在HOI-M3数据集（唯一包含多人多物体交互动态场景的数据集）上评估，该方法在表面重建方面优于现有最先进方法。

Conclusion: 混合方法能有效处理多人多物体动态场景的挑战，即使在严重遮挡下也能保持物体结构，并能产生多视角和时间一致的几何。
...
TL;DR: NeuroVolve是一个生成框架，通过优化预训练视觉语言模型嵌入空间中的神经目标函数，实现大脑引导的图像合成，能够生成满足复杂多脑区约束的连贯场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注孤立脑区的类别选择性（如FFA中的面孔），但无法揭示脑区在复杂自然视觉中的交互机制。需要开发能够探索脑区间相互作用和分布式表征的新方法。

Method: 提出NeuroVolve生成框架，在预训练视觉语言模型的嵌入空间中优化神经目标函数。通过可编程的神经目标（激活或抑制单个或多个脑区）引导图像生成，并追踪优化步骤以揭示语义轨迹。

Result: 验证了已知脑区选择性，生成了满足复杂多脑区约束的连贯场景。能够生成低层和语义特征特定的刺激，揭示脑区间的协同和拮抗调谐关系，并捕获个体特异性偏好。
...
TL;DR: DAAAM是一个用于大规模实时4D场景理解的新型时空记忆框架，通过优化前端加速语义描述推理，构建层次化4D场景图，在时空问答和任务落地任务中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成丰富开放词汇描述与实时3D落地之间存在权衡，计算机视觉和机器人应用需要既能捕获几何结构又能包含语义细节的时空记忆框架。

Method: 提出DAAAM框架：1）引入基于优化的前端，从局部描述模型（如DAM）推断详细语义描述，通过批处理将推理速度提升一个数量级；2）利用语义理解构建层次化4D场景图，作为全局时空一致的内存表示；3）4D场景图可与工具调用代理良好接口进行推理和推理。

Result: 在NaVQA基准测试的时空问答任务中表现优异，在SG3D基准测试的序列任务落地中展示泛化能力。在扩展的OC-NaVQA基准测试中：问题准确率提升53.6%，位置误差降低21.9%，时间误差降低21.6%，SG3D任务落地准确率提升27.8%。

Conclusion: DAAAM能够构建具有详细几何落地描述的4D场景图，同时保持实时性能，为大规模实时4D场景理解提供了有效的时空记忆框架解决方案。
...
TL;DR: 本研究提出了Yoga-16数据集，系统评估了三种深度学习架构在三种输入模态下的瑜伽姿势分类性能，发现基于骨架的表示优于原始图像输入，VGG16+MediaPipe Pose骨架输入达到最高96.09%准确率。


<details>
  <summary>Details</summary>
Motivation: 瑜伽作为全球流行的锻炼方式，错误姿势可能导致伤害。自动瑜伽姿势分类可以减少对专家指导的依赖，但现有研究多集中于原始图像或单一姿势提取模型，缺乏系统性基准测试。

Method: 创建Yoga-16数据集解决现有数据集局限性；系统评估VGG16、ResNet50和Xception三种架构；使用三种输入模态：原始图像、MediaPipe Pose骨架图像、YOLOv8 Pose骨架图像；使用Grad-CAM进行可解释性分析；进行交叉验证分析。

Result: 骨架表示优于原始图像输入；VGG16+MediaPipe Pose骨架输入达到最高96.09%准确率；可解释性分析揭示了模型决策依据；交叉验证分析验证了结果稳定性。

Conclusion: 基于骨架的表示在瑜伽姿势分类中表现优异，VGG16+MediaPipe Pose组合效果最佳。研究提供了系统性基准测试和可解释性分析，为自动瑜伽姿势分类系统开发提供了重要参考。
...
TL;DR: 提出SatireDecoder框架，通过多智能体视觉级联解耦和不确定性引导的思维链推理，提升视觉讽刺图像的理解能力，减少幻觉并提高解释准确性。
...
Method: 提出SatireDecoder训练免费框架：1）多智能体系统执行视觉级联解耦，将图像分解为细粒度的局部和全局语义表示；2）引入不确定性分析引导的思维链推理策略，将复杂的讽刺理解过程分解为具有最小不确定性的顺序子任务。

Result: 实验结果表明，SatireDecoder在理解视觉讽刺方面优于现有基线方法，显著提高了解释准确性，同时减少了幻觉现象。

Conclusion: SatireDecoder为视觉语言推理在微妙、高级语义任务中提供了有前景的方向，能够更好地理解视觉讽刺的复杂语义层次。
...
TL;DR: MedCT-VLM：一种参数高效的医学CT视觉语言模型，通过LoRA微调将大规模CT基础模型适配到下游临床任务，在零样本分类中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言预训练的基础模型在多个图像领域表现出强大的零样本能力，但在医学体积成像（如CT）中的应用仍然有限。需要一种参数高效的方法来将大规模CT基础模型适配到下游临床任务，特别是在标注数据稀缺的情况下。

Method: 提出MedCT-VLM框架，采用参数高效的LoRA（低秩适应）方法，在CT-CLIP模型的视觉和文本编码器的注意力层中插入低秩分解矩阵，仅训练1.67M参数（占总参数的0.38%），而不是直接微调440M参数。
...
TL;DR: 该研究评估了多种深度学习模型（YOLOv9、U-Net、Swin Transformer、DeepLabV3、Mask R-CNN）用于自动检测树木横截面髓心的性能，Swin Transformer表现最佳（准确率0.94），并通过数据增强和NMS等技术解决了边界精度和重叠检测等问题。


<details>
  <summary>Details</summary>
Motivation: 树木横截面髓心检测对林业和木材质量分析至关重要，但目前仍依赖人工操作，容易出错且效率低下。需要开发自动化的深度学习方法来解决这一实际问题。

Method: 使用582张标注图像的数据集，通过动态数据增强提高泛化能力。评估了五种深度学习模型：YOLOv9（边界框检测）、U-Net、Swin Transformer、DeepLabV3和Mask R-CNN（实例分割）。使用俄勒冈州立大学树轮实验室的11张橡树图像测试泛化能力，并对最差模型使用额外64张图像进行训练以改进性能。

Result: Swin Transformer表现最佳，准确率达0.94，在精细分割方面表现出色。YOLOv9在边界框检测中表现良好但边界精度不足。U-Net对结构化模式有效，DeepLabV3能捕获多尺度特征但边界略有不准。Mask R-CNN初始IoU仅0.45，应用NMS后提升至0.80。额外训练最差模型未能显著改善其在橡树数据集上的泛化能力。

Conclusion: 深度学习在树木横截面髓心检测中具有巨大潜力，模型选择应基于数据集特征和应用需求。Swin Transformer在精度方面表现最优，而不同模型各有优缺点。数据增强、超参数调优和NMS等技术能有效解决边界不一致和重叠检测等问题。
...
TL;DR: 提出結合DCGAN數據增強、ResNet-50分類器和XAI解釋技術的皮膚病CAD系統，解決數據不平衡和模型黑盒問題，達到92.50%準確率。


<details>
  <summary>Details</summary>
Motivation: 皮膚病多類別診斷面臨主觀性、數據不平衡（如HAM10000數據集）和深度學習模型黑盒問題，需要可信賴且高準確的計算機輔助診斷系統。

Method: 使用DCGAN進行每類數據增強解決類別不平衡，微調ResNet-50分類器在增強數據集上訓練，並整合LIME和SHAP XAI技術提供預測可解釋性。

Result: 系統達到92.50%總體準確率和98.82%宏觀AUC，優於多個基準架構，並能確認預測基於臨床相關特徵（如不規則形態）。

Conclusion: 成功驗證了結合高性能與臨床可解釋性的可驗證框架，未來研究應優先提升關鍵類別（如黑色素瘤）的鑑別能力。
...
TL;DR: 使用YOLOv5算法对甲状腺结节进行实例分割，评估不同模型变体在有无多普勒图像的数据集上的表现，发现包含多普勒图像能显著提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 甲状腺癌发病率上升，需要开发计算机辅助检测方法。甲状腺结节的准确分割是开发AI辅助临床决策支持系统的关键第一步。

Method: 使用YOLOv5算法（Nano、Small、Medium、Large、XLarge变体）在超声图像上进行甲状腺结节实例分割，评估有无多普勒图像的两个数据集版本。

Result: YOLOv5-Large算法在包含多普勒图像的数据集上表现最佳，dice分数达91%，mAP为0.87。包含多普勒图像显著提升了所有模型变体的性能，YOLOv5-Small模型在排除多普勒图像时dice分数为79%，包含后性能提升。

Conclusion: YOLOv5实例分割为甲状腺结节检测提供了有效的实时方法，多普勒图像虽然通常被医生排除，但能显著改善分割性能，在自动化诊断系统中具有潜在临床应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 跨域面部表情识别面临训练数据和部署数据之间的严重域偏移问题，现有方法难以有效处理这种域间差异

Method: 使用ResNet-50作为主干网络，结合批级图注意力网络建模样本间关系，将小批量数据构建为稀疏环图，通过注意力机制聚合跨样本信息。采用梯度反转层进行对抗学习，结合CORAL和MMD进行统计对齐

Result: 在标准无监督域适应协议下，平均跨域准确率达到74.39%。在RAF-DB到FER2013的迁移任务中达到98.0%准确率，比最佳基线提升约36个百分点

Conclusion: GAT-ADA框架通过图注意力网络建模样本间关系和对抗域对齐，有效解决了跨域面部表情识别中的域偏移问题，显著提升了识别性能
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉Mamba的效率受限于输入token数量，而传统的token剪枝或合并方法会导致信息丢失。特别是当这些方法统一应用于所有图像的细粒度token表示时，问题更加严重，因为并非所有图像都需要细粒度处理。

Method: 提出Coarse-to-Fine Vision Mamba (CF-ViM)框架：1) 首先将输入图像划分为大块进行粗粒度推理，显著减少token长度和计算量；2) 当模型预测置信度低时，选择特定区域以更精细的分辨率重新处理，以恢复关键视觉细节；3) 采用动态分辨率分配策略，根据图像复杂度自适应分配计算资源。

Result: 在ImageNet上的实验表明，CF-ViM在准确性和效率方面均优于基线视觉Mamba和最先进的token缩减技术。

Conclusion: CF-ViM通过自适应粗到细处理策略，能够根据图像复杂度动态分配计算资源，在保持视觉信息完整性的同时实现高效推理，为视觉Mamba的效率优化提供了新思路。
...
TL;DR: 论文提出创建更真实的多数字书写者基准数据集，发现孤立数字分类器在多数字识别中表现不佳，需要新方法提升真实场景的数字识别性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中数字通常以多位数形式出现（如邮政编码、支票金额、预约时间），但现有研究主要关注孤立数字分类。需要创建更真实的基准数据集来推动解决实际数字识别问题。

Method: 利用NIST数字图像中书写者信息创建多数字书写者基准数据集，设计特定任务的性能指标，超越传统错误计算，更贴近实际应用影响。

Result: 孤立数字分类器在多数字识别任务中表现不佳，表明需要额外进展来解决实际数字识别问题。新基准数据集为开发利用任务特定知识的方法创造了机会。

Conclusion: 要解决实际数字识别问题，需要超越孤立数字分类的进步。多数字书写者基准数据集提供了更现实的评估框架，能够推动开发利用任务特定知识的方法，显著提升实际应用性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的4D表示方法（如Dynamic NeRF和4DGS）已能重建动态4D场景，但文本驱动的4D场景编辑仍面临挑战，主要问题在于确保编辑过程中的多视角和时间一致性。现有方法依赖2D扩散模型独立编辑帧，常导致运动失真、几何漂移和不完整编辑。

Method: 提出Dynamic-eDiTor框架，结合多模态扩散变换器（MM-DiT）和4D高斯泼溅（4DGS）。采用两种关键机制：1）时空子网格注意力（STGA）用于局部一致的跨视角和时间融合；2）上下文令牌传播（CTP）通过令牌继承和光流引导的令牌替换实现全局传播。无需额外训练，可直接优化预训练的源4DGS。

Result: 在DyNeRF多视角视频数据集上的大量实验表明，该方法在编辑保真度、多视角一致性和时间一致性方面优于现有方法。

Conclusion: Dynamic-eDiTor实现了无缝、全局一致的多视角视频编辑，解决了文本驱动4D场景编辑中的关键挑战，为动态4D场景编辑提供了有效的无训练解决方案。
...
TL;DR: FoundationGait是首个可扩展的自监督预训练步态理解框架，拥有近1.3亿参数，在12个公共数据集上预训练，在多种步态任务上表现优异。
...
Result: FoundationGait在多种步态数据集、条件、任务（如人体识别、脊柱侧弯筛查、抑郁预测、属性估计）和输入模态上表现稳健。在Gait3D数据集上达到48.0%的零样本Rank-1准确率，在OU-MVLP数据集上达到64.5%，创下步态识别新里程碑。

Conclusion: FoundationGait成功解决了步态模型的可扩展性和泛化性问题，建立了首个统一的步态基础模型，为步态理解领域带来了突破性进展。
...
TL;DR: AFD是一种持续视频-语言理解方法，通过将视频映射到缓慢变化的"可供性"token作为共享的时间对齐基底，结合轻量级查询路由调度器实现稳定性和可塑性的明确分离，在多个持续学习协议上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法在视频-语言理解中存在三个主要问题：1) 模糊了应该保持稳定与应该适应的部分；2) 依赖静态路由/容量；3) 需要重放过去的视频数据。作者希望在实际内存和隐私约束下，明确指定稳定性所在和可塑性应聚焦的位置。

Method: 提出Affordance-First Decomposition (AFD)：1) 将视频映射到缓慢变化的可供性token，形成共享的时间对齐基底；2) 使用轻量级、查询路由、冲突感知的调度器集中适应并仅在需要时增加容量；3) 通过弱对齐和教师一致性稳定基底；4) 训练时仅使用问题重放。

Result: 在多个持续学习协议上取得SOTA：域增量VideoQA平均准确率51.6%，遗忘率-1.8%；ViLCo R@1@0.5达到29.6%(MQ)和20.7%(NLQ)，VQ任务18.4% stAP@0.25；时间增量iVQA准确率39.5%，遗忘率-1.6%。

Conclusion: AFD提供了一种明确、可解释的方法，将稳定的交互中心基底与有针对性的适应分离，在持续视频-语言理解中实现了更好的稳定性和可塑性平衡。
...
TL;DR: 论文提出通用医学短语定位(GMPG)任务，解决传统REC范式在医学报告中处理多区域、非诊断性文本和非可定位短语的局限性，并开发了MedGrounder模型。


<details>
  <summary>Details</summary>
Motivation: 现有医学短语定位(MPG)系统大多遵循指称表达理解(REC)范式，每个短语只返回一个边界框。但真实医学报告常包含多区域发现、非诊断文本以及否定句或正常解剖描述等不可定位短语，需要更通用的任务定义。

Method: 提出通用医学短语定位(GMPG)任务，将每个句子映射到零个、一个或多个带分数的区域。开发MedGrounder模型，采用两阶段训练：先在报告句子-解剖框对齐数据集上预训练，再在报告句子-人工标注框数据集上微调。

Result: 在PadChest-GR和MS-CXR数据集上的实验表明，MedGrounder实现了强大的零样本迁移能力，在多区域和不可定位短语上优于REC风格和基础报告生成基线，同时使用更少的人工标注框。

Conclusion: MedGrounder是首个GMPG模型，能更好地处理真实医学报告的复杂性，并且可以与现有报告生成器组合使用，无需重新训练生成器即可生成基础报告。
...
TL;DR: CAR-net是一种用于旋转运动模糊图像去模糊的级联细化网络，专门处理半盲场景（仅有模糊角度的噪声信息），通过渐进细化过程逐步抑制伪影并恢复细节。
...
Method: 提出CAR-net架构：1）从频域反演获得初始去模糊估计；2）通过一系列细化阶段，每个阶段对当前去模糊图像预测并应用残差校正；3）可选角度检测模块可端到端训练以处理参数不确定性。

Result: 通过合成和真实图像实验验证了CAR-net的有效性，代码、模型和数据集链接已在GitHub上公开。

Conclusion: CAR-net通过级联细化架构有效解决了旋转运动模糊图像的去模糊问题，特别是在半盲场景下，能够渐进地抑制伪影并恢复细节，同时可选的端到端角度检测模块增强了系统的鲁棒性。
...
TL;DR: 提出约束引导框架，结合几何推理与神经语义，通过AlphaGo式搜索确保可行性，使用微调视觉语言模型评估语义对齐，在七巧板组装任务中优于扩散和自回归基线。


<details>
  <summary>Details</summary>
Motivation: 研究抽象视觉组合问题，其中身份主要由少量几何基元（如部件、对称性、拓扑）的空间配置和关系决定。这些结构对纹理和照片级细节具有不变性。在几何约束和模糊目标规范（如文本）下从固定组件组合此类结构具有挑战性，因为组合放置选择多、数据有限、离散可行性要求（无重叠、允许方向）导致解空间稀疏，不适合纯统计像素空间生成器。

Method: 提出约束引导框架，结合显式几何推理与神经语义。采用AlphaGo式搜索强制执行可行性，同时使用微调视觉语言模型将语义对齐作为奖励信号评分。算法使用策略网络作为蒙特卡洛树搜索的启发式方法，并通过搜索生成的计划微调网络。受生成对抗网络启发，使用生成实例进行对抗性奖励细化，使生成逐渐接近实际数据。

Result: 在七巧板组装任务中，该方法比扩散和自回归基线产生更高的有效性和语义保真度，特别是在约束收紧时表现更优。

Conclusion: 约束引导框架成功解决了抽象视觉组合问题，通过结合几何推理和神经语义的方法，在保持可行性的同时提高了语义对齐质量，为组合约束下的生成任务提供了有效解决方案。
...
TL;DR: 提出了一种通过幻觉分类器和动态样本重加权DPO算法来减少大型视觉语言模型幻觉的方法，显著降低了多个基准上的幻觉率。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现出色，但幻觉问题仍然是关键挑战。现有标注方法会在训练样本中引入额外幻觉，可能增强模型的幻觉模式。

Method: 1) 分析发现on-policy数据显著优于off-policy数据；2) 提出幻觉分类器提供二元标注，确保后续对齐的样本干净；3) 设计动态样本重加权的迭代DPO算法，充分利用on-policy数据。

Result: 在三个基准测试中与8个SOTA基线比较：LLaVA-1.5-7B在MMHalBench上的幻觉率降低50.8%；在Object HalBench上的平均幻觉率降低79.5%；LLaVA-1.5-13B甚至超越了GPT-4V的性能。

Conclusion: 该方法通过幻觉分类器和动态DPO算法有效缓解了LVLM的幻觉问题，充分挖掘了开源模型的潜力，实现了显著的性能提升。
...
TL;DR: 深度学习模型（CNN、Transformer等）通过分析多模态医学影像数据，结合放射基因组学，实现了癌症早期检测和个性化肿瘤学的新范式


<details>
  <summary>Details</summary>
Motivation: 癌症早期检测是现代医疗的关键挑战，延迟诊断会显著降低生存率。传统放射学评估存在局限性，需要更精准的检测方法。

Method: 采用深度学习计算机视觉模型（CNN、Transformer、混合注意力架构），从MRI、CT、PET、乳腺摄影、组织病理学、超声等多模态成像数据中自动提取复杂空间、形态和时间模式。结合放射基因组学，将定量影像特征与基因组学、转录组学和表观遗传生物标志物融合。

Result: 深度学习模型超越了传统放射学评估，能够识别人眼无法察觉的细微组织异常和肿瘤微环境变化。放射基因组融合能够预测肿瘤基因型、免疫反应、分子亚型和治疗耐药性，无需侵入性活检。

Conclusion: 深度学习与放射基因组学的结合为个性化肿瘤学引入了新范式，通过多模态影像分析实现了癌症早期检测和精准医疗的突破性进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有交互式图像分割方法主要针对自然图像设计，难以泛化到遥感领域，因为遥感图像存在尺度变化、不规则边界和复杂背景等问题，且标注数据有限、计算开销大。

Method: 提出RS-ISRefiner框架：1）采用适配器调优策略，保留视觉基础模型的通用表示，同时高效学习遥感特有的空间和边界特征；2）结合卷积局部建模和Transformer全局推理的混合注意力机制，增强对尺度多样性和场景复杂性的鲁棒性；3）改进的概率图调制方案，有效整合历史用户交互，实现更稳定的迭代优化和更高边界保真度。
...
Conclusion: 该框架有效解决了遥感图像交互分割的挑战，证实了其有效性和泛化能力，非常适合实际遥感场景中的高质量实例分割应用。
...
TL;DR: StreamGaze是首个评估MLLMs在流式视频中利用人类注视信号进行时序推理和意图预测的基准，通过注视引导的过去、现在和前瞻任务全面评估模型能力，发现现有模型与人类表现存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频基准仅评估时序推理，缺乏对MLLMs是否能够解释和利用人类注视信号的评估。为填补这一空白，需要创建能够评估模型如何有效利用注视进行时序和前瞻推理的基准。

Method: 开发注视-视频QA生成流水线，通过注视点提取、区域特定视觉提示和扫描路径构建，将自我中心视频与原始注视轨迹对齐，生成时空接地的QA对，反映人类感知动态。

Result: 在所有StreamGaze任务中，最先进的MLLMs与人类表现之间存在显著性能差距，揭示了在基于注视的时序推理、意图建模和前瞻预测方面的基本局限性。

Conclusion: StreamGaze基准揭示了当前MLLMs在注视引导的流式视频理解方面的不足，为未来模型开发提供了方向，所有数据和代码将公开以支持持续研究。
...
<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统虽然能有效提取环境特征，但依赖辅助感知任务需要昂贵的人工标注。开发无需感知标注的规划范式变得至关重要。

Method: 提出TrajDiff框架，仅需原始传感器输入和未来轨迹，构建高斯BEV热图目标来捕捉驾驶模式。设计轨迹导向的BEV编码器提取TrajBEV特征，无需感知监督。引入轨迹导向的BEV扩散Transformer（TB-DiT），利用自车状态信息和预测的TrajBEV特征直接生成多样化轨迹。

Result: 在NAVSIM基准测试中达到87.5 PDMS，在所有无标注方法中达到最先进性能。通过数据扩展进一步提升至88.5 PDMS，与先进的基于感知的方法相当。

Conclusion: TrajDiff成功建立了完全无需感知标注的生成式端到端自动驾驶方法，不仅实现了架构创新，还能探索无标注设置下的数据扩展优势，为自动驾驶系统开发提供了更经济高效的解决方案。
...
TL;DR: Multi-GRPO提出了一种多组优势估计框架，通过时间分组和奖励分组机制，解决了现有GRPO方法在信用分配和奖励混合方面的局限性，在单奖励和多目标对齐任务中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的文本到图像模型对齐方法存在两个关键问题：1）共享信用分配问题——轨迹级优势均匀应用于所有时间步，无法准确估计早期去噪步骤的潜力；2）奖励混合问题——预定义权重组合多目标奖励导致梯度不稳定和冲突更新。

Method: 提出Multi-GRPO框架，包含两种正交分组机制：1）基于蒙特卡洛树搜索的时间分组，在早期去噪步骤分支形成时间组，通过后代叶子准确估计早期步骤优势；2）奖励分组，独立计算每个奖励函数的优势后再聚合，解耦冲突信号。还构建了OCR-Color-10数据集用于多目标对齐评估。

Result: 在单奖励PickScore-25k和多目标OCR-Color-10基准测试中，Multi-GRPO实现了优越的稳定性和对齐性能，有效平衡了冲突目标。

Conclusion: Multi-GRPO通过创新的分组机制解决了GRPO方法的局限性，为文本到图像模型的多目标对齐提供了更稳定有效的解决方案。
...
TL;DR: 提出MGTPCN图像压缩网络，结合先验引导卷积(PGConv)和多尺度门控变换器(MGT)，在性能和复杂度间取得更好平衡
...
Method: 1) 提出PGConv增强局部特征提取：使用非对称卷积强化骨架元素，差分卷积提取高频信息，采用重参数化降低计算复杂度；2) 提出MGT增强非局部特征提取：使用不同膨胀率的膨胀窗口多头自注意力块和不同核大小的深度卷积层提取多尺度特征，引入门机制增强非线性；3) 将两者结合构建MGTPCN网络

Result: 实验结果表明MGTPCN在性能和复杂度权衡方面超越了现有最先进算法
...
TL;DR: ProSeg提出了一种概率建模方法，用于多标注者医学图像分割，同时实现多样化和个性化分割输出。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割存在数据不确定性，包括图像边界模糊和不同专家标注的差异。现有方法要么只能生成缺乏专家特异性的多样化分割，要么只能复制单个标注者的个性化输出，无法同时满足两种需求。

Method: 引入两个潜在变量分别建模专家标注偏好和图像边界模糊性，通过变分推断获得条件概率分布，通过从这些分布中采样生成分割输出。

Result: 在鼻咽癌数据集（NPC）和肺结节数据集（LIDC-IDRI）上的实验表明，ProSeg达到了新的最先进性能，提供既多样化又专家个性化的分割结果。

Conclusion: ProSeg通过概率建模同时实现了多标注者医学图像分割的多样化和个性化，为解决医学图像分割中的数据不确定性提供了有效方法。
...
TL;DR: FigEdit是一个大规模科学图表编辑基准，包含30,000+样本，涵盖10种图表类型和复杂编辑指令，揭示现有生成模型在结构化图表编辑上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型（如扩散和自回归方法）在自然图像编辑上表现出色，但应用于科学图表时存在根本性缺陷：图表不是简单的像素排列，而是受图形语法约束的结构化数据可视化表示。图表编辑本质上是结构化转换问题，而非像素操作任务。

Method: 引入FigEdit基准，基于真实数据构建，包含30,000+样本，覆盖10种图表类型和丰富编辑指令。基准组织为五个渐进挑战性任务：单编辑、多编辑、对话式编辑、视觉引导编辑和风格迁移。

Result: 评估现有最先进模型显示它们在科学图表编辑上表现不佳，无法处理所需的结构化转换。传统评估指标（如SSIM、PSNR）在捕捉图表编辑语义正确性方面存在局限性。

Conclusion: FigEdit基准揭示了像素级操作的深刻局限性，为开发和评估未来结构感知模型提供了坚实基础。通过发布该基准，旨在推动结构感知图表编辑的系统性进展，为公平比较提供共同基础，并鼓励研究同时理解科学图表视觉和语义层的模型。
...
TL;DR: 该论文提出了一种端到端的可微分逆图形框架，能够从视频中恢复不可见的物理力场，如通过观察树叶落地来估计风场。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉长期以来关注从视频中建模运动，但导致物体变形和运动的不可见物理相互作用（如力场）在很大程度上尚未被探索。本文旨在从视觉观察中恢复这些不可见的力。

Method: 提出端到端的可微分逆图形框架，联合建模物体几何、物理属性和相互作用，通过反向传播从物体运动中恢复力表示。

Result: 在合成和真实场景中验证了方法有效性，能够从视频中推断出合理的力场。展示了在基于物理的视频生成和编辑方面的应用潜力。
...
TL;DR: TESP-Attack是一种针对交通标志分类的隐蔽对抗补丁方法，通过边缘对齐掩码和U-Net生成器制作与环境无缝融合的对抗补丁，在保持高攻击成功率的同时实现视觉隐蔽。


<details>
  <summary>Details</summary>
Motivation: 当前针对交通标志的物理对抗攻击缺乏隐蔽性，大多数方法在标志中心区域添加扰动，形成视觉上明显的模式，容易被人类观察者检测到，限制了实际应用。智能驾驶系统容易受到此类攻击，导致错误分类并可能引发连锁故障。

Method: 基于人类视觉注意力主要关注交通标志中心区域的观察，使用实例分割生成符合标志形状特征的边缘对齐掩码。采用U-Net生成器制作对抗补丁，通过颜色和纹理约束以及频域分析进行优化，实现与背景环境的无缝融合。

Result: 该方法在不同架构的交通标志分类模型上表现出色，在有限查询预算下攻击成功率超过90%。同时展现出强大的跨模型可迁移性，并在不同角度和距离下保持稳定的实际性能。

Conclusion: TESP-Attack提出了一种有效的隐蔽对抗攻击方法，解决了当前物理攻击缺乏隐蔽性的问题，为智能驾驶系统的安全评估提供了新工具，同时也强调了V2X网络中对抗攻击传播的风险。
...
<details>
  <summary>Details</summary>
Motivation: 现有RGB-only方法在动态物体和极端光照等现实条件下表现不佳，因为传统相机存在固有局限性。需要更鲁棒的3D几何估计方法来应对这些挑战。

Method: 基于MonST3R骨干网络，引入两个关键创新：1）Retinex启发的图像增强模块和轻量级事件适配器，带有SNR感知融合机制，根据局部可靠性自适应融合RGB和事件特征；2）新颖的事件驱动光度一致性损失，在全局优化中增强时空一致性。

Result: EAG3R在单目深度估计、相机姿态跟踪和动态重建任务中显著优于最先进的RGB-only基线方法，无需在夜间数据上重新训练。

Conclusion: 通过结合异步事件流增强点图重建，EAG3R能够在具有挑战性的动态低光场景中实现鲁棒的3D几何估计，为自动驾驶、SLAM和3D场景重建等应用提供了更可靠的解决方案。
...
TL;DR: 构建了日本首个大规模视觉语言数据集DEJIMA，包含388万图像-文本对，远超现有资源，通过可扩展的流水线确保数据质量和日本文化代表性。


<details>
  <summary>Details</summary>
Motivation: 日本视觉语言建模领域缺乏高质量、大规模的数据资源，现有数据集要么规模小，要么通过翻译或人工标注，缺乏日本文化特色和自然语言表达。

Method: 开发了可扩展、可复现的流水线：大规模网络收集→严格过滤/去重→基于目标检测的证据提取→在接地约束下使用大语言模型进行精炼。构建了两个数据集：DEJIMA-Cap（图像描述）和DEJIMA-VQA（视觉问答）。

Result: 构建了各含388万图像-文本对的两个数据集，远超现有日本V&L数据集。人工评估显示DEJIMA在"日本性"和语言自然性上显著优于翻译或人工标注数据集，事实准确性达到人类标注水平。图像特征分布分析证实其广泛覆盖日本特色视觉领域。

Conclusion: 文化接地的大规模资源对提升模型性能至关重要。在DEJIMA上训练的模型在多个日本多模态基准测试中表现一致提升。所有数据源和模块均允许商业使用，公开发布数据集和元数据以促进研究和工业应用。
...
TL;DR: PolarGS：利用偏振光学先验解决3D高斯泼溅中反射和无纹理区域的光度歧义问题，提升表面重建精度


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅（3DGS）的表面重建方法在反射和无纹理等光度歧义区域性能下降，因为不可靠的光度线索破坏了光度一致性并阻碍准确几何估计。偏振光能揭示表面方向信息，可作为光度线索的光学补充来解决这些歧义。

Method: 提出PolarGS，包含两个互补模块：1）偏振引导的光度校正策略，通过线性偏振度（DoLP）识别反射区域，并使用颜色细化图优化反射高斯；2）偏振增强的高斯致密化机制，将角度和线性偏振度（A/DoLP）集成到基于PatchMatch的深度补全过程中，实现新高斯的后向投影和融合。

Result: PolarGS是框架无关的，相比最先进方法实现了更优越的几何精度。

Conclusion: 利用偏振作为光学先验能有效解决3D高斯泼溅中的光度歧义问题，显著提升反射和无纹理区域的表面重建质量。
...
TL;DR: CircleFlow：一种通过流引导边缘定位实现高保真PSF估计的框架，使用圆形网格目标编码各向异性和空间变化的PSF，通过隐式神经表示和可微分优化实现精确的模糊表征。
...
Method: 1. 使用圆形网格目标进行结构化捕获，编码局部各向异性和空间变化的PSF，利用目标的二值亮度先验解耦图像和核估计；2. 通过光流引导的亚像素对齐重建潜在锐利图像；3. 将PSF建模为能量约束的隐式神经表示；4. 在去马赛克感知的可微分框架中联合优化两个组件。

Result: 在模拟和真实世界数据上的大量实验表明，CircleFlow实现了最先进的准确性和可靠性，验证了其在实际PSF校准中的有效性。

Conclusion: CircleFlow通过流引导边缘定位实现了高保真PSF估计，通过精确的边缘定位确保了物理一致性和鲁棒性，为实际PSF校准提供了有效的解决方案。
...
TL;DR: SpecTemp：基于强化学习的推测性时序推理框架，通过双模型设计解耦时序感知与推理，显著提升长视频理解效率


<details>
  <summary>Details</summary>
Motivation: 现有"thinking-with-frames"范式虽然提升了视频多模态大语言模型的推理能力，但由于多模态上下文不断增长且冗余，存在显著的效率瓶颈。需要解决长视频理解中的效率问题。

Method: 提出SpecTemp框架：采用轻量级草稿MLLM快速探索密集采样时序区域并提出显著帧，强大目标MLLM专注于时序推理并验证草稿提议，迭代精炼注意力直至收敛。构建SpecTemp-80K数据集支持训练，包含粗粒度证据跨度和细粒度帧级证据的双层标注。

Result: 在多个视频理解基准测试中，SpecTemp不仅保持了有竞争力的准确性，而且相比现有thinking-with-frames方法显著加速了推理过程。

Conclusion: SpecTemp通过双模型协作设计有效解决了长视频理解中的效率瓶颈，模仿人脑协作通路，在效率和准确性之间取得良好平衡，为长视频理解提供了高效解决方案。
...
TL;DR: IRPO提出了一种基于GRPO的低层视觉后训练范式，通过数据筛选和奖励建模，在图像修复任务中实现了SOTA性能，显著提升了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法依赖像素级硬拟合，存在过平滑和泛化能力差的问题。后训练范式在高层生成任务中表现出色，但在低层视觉领域尚未充分探索。

Method: 1) 数据筛选原则：从预训练阶段选择表现不佳的样本进行后训练；2) 奖励建模系统：包含通用奖励（结构保真）、专家奖励（基于Qwen-VL的感知对齐）和修复奖励（任务特定质量）。

Result: 在6个域内和5个域外低层视觉基准测试中，IRPO均取得SOTA结果：域内任务比AdaIR基线提升0.83dB，域外设置提升3.43dB。

Conclusion: IRPO成功将后训练范式应用于低层视觉，通过创新的数据筛选和奖励建模策略，显著提升了图像修复的性能和泛化能力。
...
TL;DR: PanFlow：一种利用全景球面特性解耦相机旋转与光流条件的新方法，通过球面噪声扭曲策略提升边界运动一致性，显著提升大动态运动生成质量


<details>
  <summary>Details</summary>
Motivation: 现有全景视频生成方法缺乏显式运动控制，难以生成具有大而复杂运动的场景，需要更精确控制大动态运动的方法

Method: 利用全景球面特性将高动态相机旋转从输入光流条件中解耦，引入球面噪声扭曲策略促进全景边界运动的循环一致性，构建大规模运动丰富的全景视频数据集进行训练

Result: PanFlow在运动保真度、视觉质量和时间一致性方面显著优于现有方法，并在运动转移和视频编辑等应用中展示有效性

Conclusion: PanFlow通过球面解耦和噪声扭曲策略实现了对全景视频中大动态运动的精确控制，为虚拟现实和沉浸式媒体应用提供了有效解决方案
...
TL;DR: AFRAgent是一个基于instruct-BLIP的多模态架构，通过自适应特征重归一化技术增强图像嵌入，在GUI自动化任务中实现了最先进性能，且模型尺寸仅为最近竞争对手的四分之一。


<details>
  <summary>Details</summary>
Motivation: 移动UI自动化需求日益增长，现有视觉语言模型在GUI自动化中存在两个主要问题：1) 由于视觉编码器特征中空间信息有限，难以准确识别控件和确定操作；2) 性能最佳的模型通常规模庞大，需要大量训练且推理延迟高。

Method: 提出AFRAgent，一个基于instruct-BLIP的多模态架构。核心创新是自适应特征重归一化技术（基于token级别的仿射变换），有效增强低分辨率图像嵌入并融合高分辨率细节，从而改善LLM流水线中的图像表示。

Result: 在Meta-GUI和AITW基准测试中评估AFRAgent，建立了智能手机自动化的新最先进基线。模型尺寸小于最近竞争对手的四分之一，同时实现了优越性能。

Conclusion: AFRAgent通过自适应特征重归一化技术有效解决了现有VLM在GUI自动化中的空间信息不足问题，实现了高性能且轻量化的移动UI自动化解决方案。
...
TL;DR: Smol-GS是一种用于3D高斯泼溅的紧凑表示学习方法，通过递归体素层次结构和泼溅特征编码实现高效3D场景压缩，在保持高质量渲染的同时大幅减小存储需求。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）需要大量存储空间，限制了其在实际应用中的部署。需要一种既能保持渲染质量又能大幅压缩场景表示的方法。

Method: 采用递归体素层次结构捕获泼溅坐标，同时使用泼溅特征存储颜色、透明度、变换和材质属性等抽象信息，实现空间和语义信息的集成编码。

Result: 在标准基准测试中实现了最先进的压缩效果，同时保持高质量渲染。压缩比达到数量级提升，且离散表示有潜力支持导航、规划等下游任务。

Conclusion: Smol-GS提供了一种高效压缩3DGS表示的方法，不仅保持视觉保真度，还为3D场景理解的下游应用奠定了基础。
...
TL;DR: TAP-CT：一种用于CT影像的3D基础模型，通过自监督预训练生成任务无关的冻结特征表示，无需大量微调即可在多种下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有医学领域基础模型通常需要大量微调或依赖资源密集的解码器训练，且许多编码器的预训练目标偏向特定任务，需要一种强健、任务无关、只需特征提取的基础模型。

Method: 将Vision Transformers和DINOv2适配到体数据，通过修改patch嵌入、位置编码和体数据增强，在105K CT体数据上进行大规模3D自监督预训练。

Result: 大规模3D预训练产生稳定、鲁棒的冻结表示，在多种下游任务中表现出强大的泛化能力。

Conclusion: TAP-CT为医学影像研究提供了强大的低资源基线，将发布所有预训练模型、实验配置和下游基准代码以促进透明性和可复现性。
...
TL;DR: DeepPriorCBCT：三阶段深度学习框架，仅用常规剂量1/6实现诊断级CBCT重建，显著降低术中辐射风险
...
Method: 提出DeepPriorCBCT三阶段深度学习框架，使用12个中心4102名患者8675次CBCT扫描进行开发验证，并进行前瞻性交叉试验（NCT07035977）评估临床适用性

Result: 11名医师确认重建图像与原始扫描无法区分；诊断性能和图像质量与标准重建算法相当；前瞻性试验中5名放射科医师报告图像质量和病灶评估无显著差异；25名介入医师对模型图像和全采样图像无偏好；辐射暴露降至常规方法的1/6

Conclusion: DeepPriorCBCT能在稀疏采样条件下实现高质量CBCT重建，显著降低术中辐射风险，具有临床实用价值
...
TL;DR: 提出一种新颖的前馈式3D高斯泼溅压缩框架，通过大规模上下文结构和精细空间-通道自回归熵模型，实现20倍压缩比，在通用编解码器中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）作为一种革命性的3D表示方法，其庞大的数据规模阻碍了广泛应用。现有前馈压缩方法由于变换编码网络感受野有限和熵模型上下文容量不足，难以建模长程空间依赖关系。

Method: 提出基于Morton序列化的大规模上下文结构（包含数千个高斯）；设计精细的空间-通道自回归熵模型以充分利用扩展上下文；开发基于注意力的变换编码模型，通过聚合广泛邻域高斯特征来提取信息丰富的潜在先验。

Result: 实现了20倍的3DGS压缩比，在通用编解码器中取得了最先进的性能。

Conclusion: 该方法通过有效建模长程相关性，实现了高度紧凑且可泛化的3D表示，解决了3DGS数据规模过大的问题。
...
TL;DR: 提出量子启发的几何框架，将神经算子表示为布洛赫超球面上的归一化奇异值谱，建立谱与功能的等价关系，并基于此提出量子度量驱动的功能冗余图和一次性结构化剪枝方法。


<details>
  <summary>Details</summary>
Motivation: 多模态智能在资源受限的异构硬件上快速增长，面临多模态特征异构性、动态场景中的实时需求以及硬件特定算子冗余等关键瓶颈问题。

Method: 引入量子启发的几何框架，将每个算子表示为其在布洛赫超球面上的归一化奇异值谱；证明谱与功能的等价定理；提出量子度量驱动的功能冗余图(QM-FRG)和一次性结构化剪枝方法。

Result: 受控仿真验证了所提度量方法优于幅度和随机基线；大规模多模态transformer和异构硬件（华为昇腾、寒武纪MLU、昆仑芯）的广泛实验验证将在扩展期刊版本中呈现。

Conclusion: 该工作为跨模态和跨架构算子可替代性建立了首个严格理论基础，为解决资源受限异构硬件上的多模态智能瓶颈提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在精准农业等专业领域存在性能瓶颈，主要原因是"推理驱动幻觉"——语言先验覆盖视觉感知，以及"模态鸿沟"——视觉嵌入无法有效激活模型参数中的专家知识

Method: 提出三阶段推理框架：1) Look阶段生成客观视觉描述和候选集；2) Recite阶段使用轻量级1.7B路由器将视觉线索转化为针对性查询，触发候选特定参数知识；3) Answer阶段并行对齐描述和知识，选择最一致的标签

Result: 在AgroBench上取得SOTA结果，杂草识别准确率比Qwen-VL提高23.6%，超越GPT-4o且无需外部搜索开销

Conclusion: 该模块化设计通过将被动感知转化为主动可控的知识检索，有效缓解幻觉问题，为专业领域视觉语言模型应用提供参数高效解决方案
...
TL;DR: HanDyVQA是一个细粒度的手-物交互视频问答基准，包含6种问题类型共11.1K个多选题对，用于评估模型对手-物交互动态的时空推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有手-物交互基准要么关注操作要么关注结果，缺乏细粒度时空推理来捕捉交互动态。需要更全面的基准来评估模型对HOI动态的理解。

Method: 构建HanDyVQA基准，包含6种互补问题类型：动作、过程、物体、位置、状态变化和物体部件。收集11.1K个多选题对，识别操作风格、手/物体运动和部件级状态变化。还包括10.3K个分割掩码用于物体和部件问题。

Result: 最佳模型Gemini-2.5-Pro仅达到73%平均准确率，远低于人类表现（97%）。分析显示模型在空间关系、运动和部件级几何理解方面仍面临挑战。将显式HOI线索整合到视觉特征中能提升性能。
...
TL;DR: 提出首个免训练的多语言遥感图像描述方法，基于检索增强提示，在10种语言上达到与全监督英文系统竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感图像描述方法依赖大规模标注数据集且主要针对英语，限制了全球适用性。需要开发无需训练、支持多语言的方法。

Method: 基于检索增强提示：1) 使用领域适应的SigLIP2编码器检索相关描述和少样本示例；2) 探索两种变体：仅文本提示的多语言LLM和联合处理图像与提示的VLM；3) 引入基于PageRank的图重排序策略提高检索内容一致性。

Result: 在4个基准数据集、10种语言上，方法性能与全监督英文系统相当；PageRank重排序带来高达35%的性能提升；直接生成目标语言描述优于翻译策略；VLM生成视觉接地但词汇多样，LLM在BLEU和CIDEr分数上更强。

Conclusion: 提出了首个系统性的免训练多语言遥感图像描述评估框架，推动了更具包容性和可扩展性的多模态地球观测系统发展。
...
TL;DR: STC是一个用于流式视频大语言模型的即插即用分层框架，通过缓存相似帧特征和剪枝冗余视觉token，显著降低计算延迟同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 流式视频大语言模型在实时部署中面临计算成本高的问题，主要瓶颈在于ViT编码阶段对时间相似帧的冗余处理，以及LLM预填充阶段过长的token序列导致延迟和内存开销。
...
Conclusion: STC框架有效解决了流式视频大语言模型的计算效率问题，通过分层token压缩实现了显著的延迟降低，同时保持了模型精度，为实时视频理解应用提供了实用解决方案。
...
TL;DR: SwiftVLA提出了一种高效的4D视觉语言动作模型架构，通过4D视觉几何变换器和融合令牌增强紧凑模型的时空理解能力，在保持高效设计的同时实现与更大模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言动作模型参数量大，实用性受限。轻量级VLM虽然参数少，但牺牲了时空推理能力。现有方法即使加入3D输入，仍依赖大型VLM融合2D/3D信息且缺乏时间维度理解。

Method: 1) 使用预训练的4D视觉几何变换器（带时间缓存）从2D图像提取4D特征；2) 引入融合令牌，通过未来预测目标训练生成统一表示；3) 采用掩码重构策略，训练VLA重构被掩码的4D输入，使VLM学习有效4D表示，推理时可丢弃4D分支。

Result: 在真实和模拟环境中，SwiftVLA超越轻量级基线，性能媲美参数量7倍大的VLA模型。在边缘设备上实现相当性能的同时，速度快18倍，内存占用减少12倍。

Conclusion: SwiftVLA通过创新的4D增强架构，在保持紧凑模型效率的同时显著提升了时空推理能力，为边缘设备上的高效VLA应用提供了可行方案。
...
TL;DR: CAE是一种无需训练的层次化语义对齐图像聚类方法，通过结合caption级描述和noun级概念构建对齐的语义空间，显著提升聚类性能。
...
Method: 提出CAE方法：1) 从WordNet选择相关名词和caption数据集选择描述构建语义空间；2) 通过最优传输将图像特征与选定的名词和caption对齐；3) 结合增强的语义和图像特征进行聚类。

Result: 在8个数据集上的实验表明方法有效，在ImageNet-1K数据集上准确率提升4.2%，调整兰德指数提升2.9%，超越现有无需训练方法。

Conclusion: CAE通过层次化语义对齐有效解决了名词歧义问题，在无需训练的情况下显著提升了图像聚类性能。
...
TL;DR: TalkingPose是一个基于扩散模型的框架，专门用于生成长时间、时序一致的上半身人体动画，通过反馈驱动机制实现无限时长的动画生成，无需额外计算成本或二次训练。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的角色驱动动画方法虽然能从单张RGB图像和驱动姿态生成高质量动作，但受限于计算和内存限制，通常只能处理短视频片段，难以生成时序一致的长时内容。

Method: TalkingPose利用驱动帧精确捕捉面部和手部表情动作，通过稳定扩散骨干网络将其无缝转移到目标角色。引入基于图像扩散模型的反馈驱动机制来确保连续运动和增强时序一致性，该机制无需额外计算成本或二次训练。

Result: 提出了一个专门用于人体上半身动画的大规模数据集作为新基准。TalkingPose能够生成无限时长的时序一致动画，解决了长时内容生成的挑战。

Conclusion: TalkingPose通过创新的反馈驱动机制，成功解决了基于扩散模型的长时人体动画生成中的时序一致性问题，为无限时长动画生成提供了有效解决方案，并建立了新的基准数据集。
...
TL;DR: 提出双流角度感知生成网络，联合估计相机倾斜角度并重建直立全景图像，通过CNN和ViT双分支融合解决非直立全景图像校正问题


<details>
  <summary>Details</summary>
Motivation: 全景相机在机器人视觉中很重要，但机器人姿态不稳定会导致非直立全景图像，影响下游任务。传统IMU方法存在漂移和外部干扰问题，视觉方法提供有前景的替代方案

Method: 使用双流角度感知生成网络，包含CNN分支（从等距柱状投影提取局部几何结构）和ViT分支（从立方体贴图投影捕获全局上下文线索），通过双投影自适应融合模块整合，并加入高频增强块、循环填充和通道注意力机制

Result: 在SUN360和M3D数据集上的实验表明，该方法在倾斜角度估计和直立全景生成方面优于现有方法，消融研究验证了各模块的贡献和两个任务之间的协同作用

Conclusion: 提出的双流网络有效解决了非直立全景图像校正问题，通过多投影融合和增强机制实现了更好的性能，为机器人视觉中的全景图像处理提供了有效解决方案
...
TL;DR: 该研究开发了一个深度学习流水线，使用从3D扫描获得的2D微CT切片，对12种有孔虫物种进行自动分类，最终集成模型达到95.64%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 为有孔虫分类研究建立AI辅助的微古生物学识别新基准，弥合深度学习与应用地球科学之间的差距，提供可重复的框架。

Method: 使用97个微CT扫描标本（27个物种）构建数据集，选择12个代表性物种，采用标本级数据分割防止数据泄漏，评估7种最先进的2D CNN架构，最终构建ConvNeXt-Large和EfficientNetV2-Small的集成模型。

Result: 集成模型测试准确率达95.64%，top-3准确率99.6%，所有物种的ROC曲线下面积(AUC)为0.998，并开发了支持实时切片分类和3D切片匹配的交互式高级仪表板。

Conclusion: 该研究为有孔虫分类建立了新的AI辅助识别基准，提供了完全可重复的框架，推动了深度学习在应用地球科学中的应用。
...
TL;DR: LAHNet提出了一种用于点云配准的局部注意力哈希网络，通过局部注意力机制和分组Transformer捕获合理的远程上下文，利用局部敏感哈希均匀划分点云，并采用交叉窗口策略扩展特征感受野，在室内外基准测试中取得显著配准结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的点云描述符主要关注局部信息感知，但合理的更广泛感受野对于增强特征区分性至关重要。需要引入局部注意力机制来提升点云描述符的性能。

Method: 提出LAHNet网络：1）设计Group Transformer，采用线性邻域搜索策略（局部敏感哈希）将点云均匀划分为非重叠窗口，捕获合理的远程上下文；2）采用高效的交叉窗口策略进一步扩展特征感受野；3）基于有效窗口策略提出Interaction Transformer，通过计算重叠矩阵来匹配点云对之间的重叠区域。

Result: 在真实世界的室内和室外基准测试中，LAHNet能够学习到鲁棒且具有区分性的特征，取得了显著的配准结果。

Conclusion: LAHNet通过引入局部注意力机制和合理的窗口划分策略，有效扩展了特征感受野，提升了点云描述符的区分性和鲁棒性，在点云配准任务中表现出色。
...
TL;DR: SceneProp将场景图定位重新表述为马尔可夫随机场中的MAP推理问题，通过全局推理联合优化所有约束，解决了现有方法在查询图增大时性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理包含多个对象和关系的复杂组合查询时存在困难。标准短语定位方法擅长定位单个对象，但缺乏解析复杂关系描述的结构归纳偏置。现有的场景图定位方法也存在悖论：随着查询图增大，性能反而下降，未能利用本应使定位更容易的信息。

Method: 将场景图定位重新表述为马尔可夫随机场中的最大后验概率推理问题，通过可微分的置信传播算法进行全局推理，在端到端框架中找到图像区域到图节点的最优分配，联合满足所有约束。

Result: 在四个基准测试中，SceneProp显著优于先前工作。关键的是，其准确性随着查询图的大小和复杂性而持续提高，首次证明更多关系上下文能够且应该带来更好的定位效果。

Conclusion: 通过将场景图定位重新表述为MAP推理问题，SceneProp解决了现有方法的局限性，展示了关系上下文对提升定位性能的重要性，为复杂视觉查询的定位提供了有效解决方案。
...
TL;DR: 提出了一种基于3D高斯泼溅的粗到细二进制编码方案，用于高效语义分割，显著减少内存占用并提升细粒度分割能力。
...
Result: 在多个基准测试中实现了最先进的分割性能，同时显著减少了内存消耗并加速了推理过程。

Conclusion: 提出的方法有效解决了3D-GS分割中的内存开销和细粒度分割挑战，为高效的3D语义分割提供了有前景的解决方案。
...
TL;DR: 提出DEMR框架，通过去偏证据学习改进视频时刻检索，解决传统方法在模态不平衡和不确定性估计方面的缺陷，显著提升检索准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频时刻检索中，传统预训练模型难以处理细粒度信息和确定性推理，在复杂或模糊时刻对齐方面存在困难。现有证据回归方法存在模态不平衡问题和启发式不确定性正则器的结构差异，导致不确定性估计偏差，将高不确定性错误关联到准确样本而非挑战性样本。

Method: 提出DEMR框架：1) 引入反射翻转融合(RFF)块进行跨模态对齐；2) 添加查询重构任务增强文本敏感性；3) 设计Geom-regularizer优化不确定性预测，实现与困难时刻的自适应对齐。

Result: 在标准数据集和去偏数据集ActivityNet-CD、Charades-CD上的广泛测试表明，该方法在有效性、鲁棒性和可解释性方面均有显著提升，为时刻检索的时域语义鲁棒性提供了有前景的解决方案。

Conclusion: DEMR框架通过去偏证据学习有效解决了视频时刻检索中的模态不平衡和不确定性估计偏差问题，提高了对复杂视频场景的适应性，在多个数据集上展现出优越性能。
...
TL;DR: 提出4DHOISolver优化框架，通过稀疏人工标注的接触点约束4D人-物交互重建，并构建大规模Open4DHOI数据集，展示其在机器人模仿学习中的应用价值。


<details>
  <summary>Details</summary>
Motivation: 从单目网络视频中准确、可扩展地提取4D交互数据是一个未解决的挑战，而这类视频包含了丰富多样的人类活动、物体和环境数据，对通用机器人学习至关重要。

Method: 提出4DHOISolver优化框架，利用稀疏的人工标注接触点来约束病态的4D人-物交互重建问题，同时保持高时空一致性和物理合理性。

Result: 构建了Open4DHOI大规模4D人-物交互数据集，包含144种物体类型和103种动作；展示重建数据可用于RL智能体模仿学习；现有3D基础模型在预测精确接触对应关系方面仍有不足。

Conclusion: 4DHOISolver框架能有效重建4D人-物交互数据，人工标注接触点的策略在当前阶段仍有必要，同时为社区提出了自动预测精确接触对应关系的开放挑战。
...
TL;DR: MM-ACT是一个统一的视觉-语言-动作模型，通过共享token空间集成文本、图像和动作，采用重新掩码并行解码策略生成多模态内容，在机器人任务中表现出色。
...
Method: 提出MM-ACT统一VLA模型，在共享token空间中集成文本、图像和动作；采用重新掩码并行解码策略生成文本和图像，一步并行解码策略生成动作；引入上下文共享多模态学习训练范式。

Result: 在LIBERO仿真中成功率96.3%，真实Franka机器人三个任务中72.0%，RoboTwin2.0八个双手任务中52.38%，跨模态学习带来额外9.25%提升。
...
### [125] [Provenance-Driven Reliable Semantic Medical Image Vector Reconstruction via Lightweight Blockchain-Verified Latent Fingerprints](https://arxiv.org/abs/2512.00999)
*Mohsin Rasheed,Abdullah Al-Mamun*

Main category: cs.CV

TL;DR: 提出语义感知医学图像重建框架，结合混合U-Net架构和区块链溯源层，在恢复图像时保留临床相关结构并确保可验证记录。


<details>
  <summary>Details</summary>
Motivation: 真实医学图像常受损坏、噪声和篡改影响，传统重建方法注重像素级恢复但可能损害解剖保真度，影响临床结果可靠性。

Method: 1) 语义感知重建框架：集成高级潜在嵌入与混合U-Net架构，保留临床相关结构；2) 轻量级区块链溯源层：采用无标度图设计，记录重建事件可验证且低开销。

Result: 多数据集和损坏类型评估显示，相比现有方法，在结构一致性、恢复准确性和溯源完整性方面均有改进。

Conclusion: 通过结合语义引导重建和安全可追溯性，该方案推进了医学影像的可靠AI，增强了医疗环境中的诊断信心和监管合规性。
...
### [126] [PhotoFramer: Multi-modal Image Composition Instruction](https://arxiv.org/abs/2512.00993)
...
<details>
  <summary>Details</summary>
Motivation: 许多普通用户在拍照时难以获得良好构图，需要实用的构图指导工具来帮助改善照片质量。

Method: 构建大规模数据集，将构图指导分为移位、放大和视角变换三个子任务；通过两阶段流程合成训练数据，然后微调多模态模型同时生成文本指导和示例图像。

Result: 文本指令能有效引导图像构图，结合示例图像比仅使用示例的基线方法有持续改进，为日常用户提供了实用的构图助手。

Conclusion: PhotoFramer通过多模态方法提供可操作的构图指导，使专业摄影知识更易于普通用户使用，是向实用构图助手迈出的重要一步。
...
TL;DR: S2AM3D：结合2D分割先验与3D一致性监督的点云部件分割方法，通过点一致性编码器和尺度感知提示解码器解决现有方法的泛化性和一致性挑战，并构建大规模高质量数据集。


<details>
  <summary>Details</summary>
Motivation: 现有部件级点云分割面临两大挑战：1）原生3D模型因数据稀缺缺乏泛化能力；2）引入2D预训练知识常导致不同视角分割结果不一致。需要一种能结合2D分割先验同时保持3D一致性的方法。

Method: 提出S2AM3D框架：1）点一致性部件编码器通过原生3D对比学习聚合多视角2D特征，生成全局一致的点特征；2）尺度感知提示解码器通过连续尺度信号实时调整分割粒度；3）构建包含超过10万个样本的大规模高质量部件级点云数据集。

Result: S2AM3D在多个评估设置中取得领先性能，在处理复杂结构和尺寸变化显著的部件时表现出卓越的鲁棒性和可控性。

Conclusion: S2AM3D成功解决了部件级点云分割中的泛化性和一致性问题，通过结合2D分割先验与3D一致性监督，实现了高性能、鲁棒且可控的分割效果。
...
TL;DR: 研究发现在ViT-B/16模型中，MLP层存在过度参数化问题，通过两种简单的参数减少策略（权重共享和隐藏层减半）可以在减少参数的同时保持甚至略微提升性能。


<details>
  <summary>Details</summary>
Motivation: 虽然缩放定律和经验结果表明增大视觉Transformer的规模通常能提升性能，但模型准确率和训练行为并不总是随规模单调增长。本研究旨在探索ViT-B/16在ImageNet-1K上是否存在过度参数化问题，以及是否可以通过更高效的参数分配来改善性能。

Method: 针对ViT-B/16模型，提出了两种MLP块参数减少策略：1) GroupedMLP：在相邻transformer块之间共享MLP权重；2) ShallowMLP：将MLP隐藏层维度减半。两种方法分别减少了32.7%的基线参数。

Result: GroupedMLP达到81.47% top-1准确率（保持基线计算成本），ShallowMLP达到81.25% top-1准确率（推理吞吐量提升38%）。两者都优于86.6M参数的基线模型（81.05%），并显著改善了训练稳定性，将峰值到最终准确率下降从0.47%降低到0.03%-0.06%。

Conclusion: ViT-B/16在ImageNet-1K上存在过度参数化问题，MLP容量可以减少而不损害性能，甚至能略微提升。参数共享和减少宽度等架构约束可以作为有用的归纳偏置，参数分配方式在视觉Transformer设计中至关重要。
...
### [129] [CycliST: A Video Language Model Benchmark for Reasoning on Cyclical State Transitions](https://arxiv.org/abs/2512.01095)
*Simon Kohaut,Daniel Ochs,Shun Zhang,Benedict Flade,Julian Eggert,Kristian Kersting,Devendra Singh Dhami*

Main category: cs.CV

TL;DR: CycliST是一个用于评估视频语言模型在周期性状态转换上文本推理能力的新基准数据集，通过合成视频序列测试模型对循环模式的时空认知，发现当前模型在检测和利用周期性模式方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频语言模型在理解现实世界中的周期性过程方面存在局限，需要专门的基准来评估模型对循环状态转换的时空推理能力，以推动模型在理解周期性模式方面的进步。

Method: 创建CycliST基准数据集，生成包含周期性物体运动和视觉属性变化的合成视频序列；采用分层评估系统，通过增加循环物体数量、场景杂乱度和光照条件等变量逐步提高难度；对当前最先进的开源和专有视频语言模型进行广泛实验。

Result: 实验显示当前视频语言模型在泛化到循环动态（如线性和轨道运动）以及时间依赖的视觉属性变化（如颜色和尺度）方面存在局限；模型无法可靠检测和利用循环模式，缺乏时间理解概念，不能从场景中提取定量信息；没有单一模型在所有任务上表现一致优秀，模型大小或架构与性能无强相关性。

Conclusion: CycliST基准揭示了当前视频语言模型在理解周期性模式方面的显著技术差距，为开发超越现有技术水平、能够理解周期性模式的视觉推理模型提供了有针对性的挑战和全面的评估框架。
...
### [130] [LISA-3D: Lifting Language-Image Segmentation to 3D via Multi-View Consistency](https://arxiv.org/abs/2512.01008)
*Zhongbin Guo,Jiahe Liu,Wenyu Gao,Yushan Li,Chengzhi Li,Ping Jian*

Main category: cs.CV

TL;DR: LISA-3D是一个两阶段框架，通过将语言图像分割提升到3D，使用几何感知的LoRA层改进LISA模型，并重用冻结的SAM-3D重建器，实现文本驱动的3D重建。
...
Result: 在ScanRefer和Nr3D数据集上，LISA-3D将语言到3D的准确率比单视角基线提高了最多15.6个百分点，仅需适应1160万个参数。系统模块化、数据高效，支持零样本部署到未见类别。

Conclusion: LISA-3D提供了一个实用的语言引导3D内容创建方案，通过结合语言图像分割和3D重建，实现了高效、准确的文本驱动3D重建，无需额外的3D-文本监督。
...
### [131] [SocialFusion: Addressing Social Degradation in Pre-trained Vision-Language Models](https://arxiv.org/abs/2512.01148)
*Hamza Tahboub,Weiyan Shi,Gang Hua,Huaizu Jiang*

Main category: cs.CV

TL;DR: 本文发现现有视觉语言模型在社交感知任务上存在负迁移问题，提出SocialFusion框架通过冻结视觉编码器与语言模型的最小连接实现正迁移，在五个社交任务上达到与任务专用SOTA相当的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管预训练的视觉语言模型展现出强大的通用能力，但在同时学习多个社交感知任务时却表现出负迁移现象。研究发现这是由于"社交退化"问题导致的——通用的视觉语言预训练过程损害了视觉编码器表示细微社交信息的能力。

Method: 提出SocialFusion统一框架，通过线性表示探测和梯度冲突分析两个视角研究社交退化现象，然后构建一个冻结视觉编码器与语言模型之间的最小连接，避免负迁移并实现正迁移。

Result: SocialFusion在五个社交任务上展现出正迁移效应，利用任务间的协同作用提升整体性能，在各种基准测试中达到与任务专用SOTA模型相当的性能。

Conclusion: 当前VLM预训练策略可能对获取通用社交能力有害，需要更注重社交感知的训练范式。SocialFusion通过最小化连接解决了社交退化问题，为构建社交智能AI提供了有效方案。
...
TL;DR: Lotus-2是一个两阶段确定性框架，通过利用预训练扩散模型的世界先验，使用少量训练数据（仅59K样本）在单目深度估计和表面法线预测任务中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 从单张图像恢复像素级几何属性是病态问题。现有方法存在局限性：判别式回归模型受限于数据规模和质量，而扩散模型虽然具有强大的世界先验，但其随机生成公式不适合确定性几何推理。需要一种能充分利用预训练生成先验的确定性框架。

Method: 提出两阶段确定性框架：1）核心预测器使用单步确定性公式，包含干净数据目标和轻量级局部连续性模块，生成全局一致结构；2）细节锐化器在核心预测器定义的流形内执行约束多步整流流细化，通过无噪声确定性流匹配增强细粒度几何。

Result: 仅使用59K训练样本（不到现有大规模数据集的1%），在单目深度估计中取得新的SOTA结果，在表面法线预测中达到高度竞争力。证明了扩散模型可作为确定性世界先验，实现超越传统判别式和生成范式的高质量几何推理。

Conclusion: 扩散模型可以作为确定性世界先验，实现高质量几何推理。Lotus-2框架通过两阶段确定性方法，充分利用预训练生成先验，在少量数据下实现稳定、准确和细粒度的几何密集预测，超越了传统方法的局限性。
...
### [133] [DPAC: Distribution-Preserving Adversarial Control for Diffusion Sampling](https://arxiv.org/abs/2512.01153)
*Han-Jin Lee,Han-Ju Lee,Jin-Seong Kim,Seok-Hwan Choi*

Main category: cs.CV

TL;DR: 提出DPAC方法，通过将对抗梯度投影到生成分数几何定义的切空间，在保持分类效果的同时最小化路径KL散度，提高样本质量。


<details>
  <summary>Details</summary>
Motivation: 现有对抗引导的扩散采样虽然能达到目标类别，但样本质量会随着对抗控制轨迹与名义轨迹偏差的累积而下降。需要一种方法在保持分类效果的同时最小化分布漂移。

Method: 基于随机最优控制视角，将退化形式化为路径KL散度，推导出最优控制的一阶条件：在获得相同分类增益的方向中，切向分量最小化路径KL，法向分量增加分布漂移。提出DPAC方法，将对抗梯度投影到由生成分数几何定义的切空间。

Result: 理论证明：在离散求解器中，切向投影消除了Wasserstein距离中的O(Δt)主导误差项，实现O(Δt²)质量差距；对分数或度量近似保持二阶鲁棒性。ImageNet-100实验验证DPAC在相同攻击成功率下获得更低的FID和估计路径KL。

Conclusion: DPAC通过将对抗梯度投影到生成分数的切空间，在保持分类效果的同时最小化分布漂移，实现了对抗引导扩散采样中分类准确性和样本质量的最佳平衡。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理时序理解任务时，可能依赖静态特征偏差（如背景或物体特征）而非动态视觉变化，这会导致下游任务的系统性预测错误。识别这些偏差对实际部署至关重要。

Method: TRoVe从带标注的验证数据集中提取候选静态特征，通过两个维度评分：(1)特征对分类错误的影响，(2)模型预测时对该特征的依赖程度。

Result: 在包含101个训练好的时序视觉语言模型和真实标注偏差的评估框架中，TRoVe比最接近的基线方法提升了28.6%的准确率。应用于7个现成模型和2个时序理解任务，成功发现了先前未知的静态特征偏差。

Conclusion: TRoVe能有效识别视觉语言模型中的错误诱导静态特征偏差，了解这些偏差有助于提升模型在测试时的性能，为模型部署前的偏差检测提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统标注方法在农业等需要快速决策的应用中过于耗时费力，需要数据收集后的大量人工标注，阻碍了YOLO等目标检测模型在实际应用中的部署。

Method: 在边缘设备上部署YOLO模型实现实时标注，使用三种主流YOLO架构（YOLOv5、YOLOv8、YOLOv12）进行对比分析，评估单类别vs多类别标注、预训练vs从头训练等配置。

Result: 预训练和单类别配置在模型收敛、性能和鲁棒性方面具有显著优势，验证了实时标注框架的可行性和有效性，大幅减少数据集准备时间同时保持高质量标注。

Conclusion: 实时标注框架能够显著提高农业等实际应用中数据集准备的效率，为YOLO模型的快速部署提供了有效的解决方案。
...
### [136] [M4-BLIP: Advancing Multi-Modal Media Manipulation Detection through Face-Enhanced Local Analysis](https://arxiv.org/abs/2512.01214)
*Hang Wu,Ke Sun,Jiayi Ji,Xiaoshuai Sun,Rongrong Ji*

Main category: cs.CV

TL;DR: 提出M4-BLIP框架，利用BLIP-2提取局部特征，结合面部先验知识，通过对齐融合模块整合局部与全局特征，提高多模态媒体篡改检测精度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态媒体篡改检测方法忽视局部信息，而篡改常发生在特定区域（尤其是面部），这威胁信息传播的可靠性和完整性。

Method: 基于BLIP-2模型提取局部特征，引入面部先验知识，设计对齐融合模块整合局部与全局特征，并与大语言模型结合提升结果可解释性。

Result: 大量定量和可视化实验表明，M4-BLIP框架在检测效果上优于现有最先进方法。

Conclusion: M4-BLIP框架通过关注局部特征（特别是面部区域）并结合大语言模型，有效提高了多模态媒体篡改检测的准确性和可解释性。
...
### [137] [Accelerating Inference of Masked Image Generators via Reinforcement Learning](https://arxiv.org/abs/2512.01094)
...
<details>
  <summary>Details</summary>
Motivation: 掩码生成模型（MGM）虽然能生成高质量图像，但需要大量采样步骤导致推理速度慢。传统蒸馏方法将加速问题视为分布匹配问题，但本文认为可以将其视为强化学习问题。

Method: 提出Speed-RL范式，将加速问题构建为强化学习问题。结合质量奖励和速度奖励，使用强化学习对基础模型进行微调，优化目标是生成高质量图像的同时减少步骤。
...
TL;DR: S²-MLLM：通过隐式空间推理增强多模态大语言模型在3D视觉定位中的能力，避免低效的点云重建，实现高效准确的空间理解。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视角相关的点云重建来为MLLMs提供3D结构指导，导致效率低下且空间推理能力有限。需要一种更高效的方法来增强MLLMs对3D空间结构的理解能力。

Method: 提出S²-MLLM框架，采用隐式空间推理策略：1）利用前馈3D重建的结构感知能力获取3D结构理解；2）设计结构增强模块，使用视图内和视图间注意力机制捕获依赖关系；3）集成多级位置编码关联视觉表示与空间位置和视角信息。

Result: 在ScanRefer、Nr3D和Sr3D数据集上取得显著性能提升，统一了优越性能、泛化能力和效率，超越了现有方法。

Conclusion: S²-MLLM通过隐式空间推理有效解决了MLLMs在3D视觉定位中的空间理解难题，避免了低效的点云重建过程，实现了高效准确的3D场景理解。
...
<details>
  <summary>Details</summary>
Motivation: 传统几何处理方法需要手动选择算子、进行离散化和特征分解，这些步骤在非结构化数据和高维空间中变得复杂且受限。需要一种能够直接从数据中学习谱基的统一框架。

Method: 基于最优逼近理论，训练网络通过最小化重构误差来分解隐式逼近算子，学习谱基。方法同时恢复隐式度量的采样密度和底层算子的特征值，无需对数据流形做假设。

Result: 在3D表面点云和高维图像流形上，该方法能够产生有意义的谱基，类似于拉普拉斯算子的特征函数，而无需显式构造算子。

Conclusion: 该学习框架为传统几何处理流程提供了原则性的数据驱动替代方案，特别适用于非结构化数据和高维空间，为几何处理开辟了新可能性。
...
TL;DR: SlotSPE：基于槽注意力的多模态癌症生存预测框架，通过压缩高维输入为稀疏的结构化事件表示，提升预测性能与可解释性


<details>
  <summary>Details</summary>
Motivation: 当前多模态癌症生存预测方法难以高效建模模态内和模态间交互，且难以从复杂输入中捕获稀疏但关键的预后事件（如空间组织学模式或通路共激活），这些事件虽少但决定患者预后

Method: 提出SlotSPE框架，受因子编码原理启发，使用槽注意力将每个患者的多模态输入压缩为紧凑、模态特定的互异槽集合，将这些槽表示作为预后事件的编码，实现高效建模复杂交互并融入生物学先验

Result: 在10个癌症基准测试中，SlotSPE在8个队列中优于现有方法，整体提升2.9%，在基因组数据缺失时保持鲁棒性，并通过结构化事件分解显著提升可解释性

Conclusion: SlotSPE通过槽注意力有效建模稀疏预后事件，在多模态癌症生存预测中实现性能提升、鲁棒性和可解释性改进，为理解癌症预后机制提供新途径
...
<details>
  <summary>Details</summary>
Motivation: 当前人脸伪造检测方法通常采用任务特定的独立模型，导致计算冗余且忽略了相关任务之间的潜在关联，需要更高效统一的解决方案。

Method: 1) 共享Swin Transformer编码器提取统一的4D时空表示；2) 跨任务交互模块通过可学习查询和注意力机制动态捕获任务间依赖关系；3) 轻量解码头将精炼表示转换为各任务预测。

Result: OmniFD在多个基准测试中优于任务特定模型，通过多任务学习捕获跨任务通用表示，特别是细粒度知识迁移提升性能（如视频分类准确率提升4.63%），同时减少63%参数和50%训练时间。

Conclusion: OmniFD为实际应用中的人脸伪造检测提供了一个实用且可推广的统一解决方案，通过统一框架实现高效、可扩展的全面检测能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注离散微表情分类，而连续强度随时间演变的研究较少。由于缺乏逐帧强度标注，完全监督回归方法不切实际，需要开发仅使用弱时间标签的连续强度估计方法。

Method: 使用简单的三角先验将稀疏时间地标（起始点、顶点、结束点）转换为密集伪强度轨迹，结合轻量级时序回归模型（ResNet18编码器+双向GRU）直接从图像序列预测逐帧强度。采用统一的预处理和时序对齐流程。

Result: 在SAMM数据集上达到Spearman相关系数0.9014和Kendall相关系数0.7999，优于逐帧基线。在CASME II数据集上分别达到0.9116和0.8168。消融研究证实时序建模和结构化伪标签对捕捉微表情的上升-顶点-下降动态至关重要。

Conclusion: 这是首个仅使用稀疏时间标注的连续微表情强度估计统一方法，无需逐帧标注工作，在不同数据集上表现一致，有效捕捉微表情的时序动态。
...
TL;DR: MedSegLatDiff：基于潜在扩散模型的高效医学图像分割框架，通过VAE压缩和潜在空间扩散生成多样分割假设，在保持小结构的同时提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割方法只生成单一分割掩码，无法捕捉不确定性；现有生成模型虽能生成多个可能分割但计算成本高。需要一种既能生成多样分割假设又高效的方法。

Method: 结合变分自编码器（VAE）和潜在扩散模型：VAE将输入压缩到低维潜在空间以减少噪声和加速训练，扩散过程在压缩表示中直接操作；用加权交叉熵替换传统MSE损失以更好保留小结构。

Result: 在ISIC-2018（皮肤病变）、CVC-Clinic（息肉）和LIDC-IDRI（肺结节）数据集上达到SOTA或高度竞争力的Dice和IoU分数，同时能生成多样分割假设和置信度图。

Conclusion: MedSegLatDiff相比确定性基线提供更好的可解释性和可靠性，特别适合临床部署，既能高效生成多样分割假设，又能保留微小结构。
...
TL;DR: 提出了首个针对飞机油箱内异物的数据集FOD-S2R，包含真实和合成图像，用于评估合成数据在封闭环境中提升异物检测性能的效果。
...
Method: 创建FOD-S2R数据集，包含3,114张真实图像（在受控油箱复制品中拍摄）和3,137张合成图像（使用Unreal Engine生成），涵盖不同视场、距离、光照、颜色和物体尺寸。对多种先进目标检测模型进行基准测试。

Result: 实验表明，引入合成数据能提高检测精度并增强模型对真实世界条件的泛化能力，缩小了Sim2Real差距。

Conclusion: 合成数据能有效提升模型性能，为开发航空维护自动化异物检测系统提供了有价值的基础。
...
### [145] [Rethinking Intracranial Aneurysm Vessel Segmentation: A Perspective from Computational Fluid Dynamics Applications](https://arxiv.org/abs/2512.01319)
...
Method: 构建了IAVS多中心数据集，包含图像-掩码对和血流动力学分析结果；建立了两个评估基准（动脉瘤全局定位和精细分割）；开发了简单有效的两阶段框架作为基准方法；建立了标准化的CFD适用性评估系统。

Result: 创建了首个包含641个3D MRA图像和587个标注的全面数据集，建立了标准化的CFD适用性评估系统，提供了可直接使用的两阶段基准框架。

Conclusion: IAVS数据集填补了现有数据集的空白，通过提供血流动力学分析和CFD适用性评估，促进了临床相关技术的发展，为颅内动脉瘤分割研究提供了重要资源。
...
TL;DR: VSRD++：一种用于单目3D目标检测的弱监督框架，无需3D标注，通过神经场体渲染和2D监督实现


<details>
  <summary>Details</summary>
Motivation: 现有单目3D目标检测方法严重依赖大量3D标注，这些标注通常来自LiDAR点云且标注过程耗时费力。需要一种减少标注依赖的方法

Method: 采用两阶段流水线：1）多视图3D自动标注：使用符号距离场表示物体表面，通过实例感知体素轮廓渲染生成实例掩码，将SDF分解为立方体SDF和残差距离场；2）单目3D检测器训练：将优化的3D边界框作为伪标签训练检测器

Result: 在KITTI-360数据集上的实验表明，VSRD++在静态和动态场景中都显著优于现有的弱监督单目3D目标检测方法

Conclusion: VSRD++成功消除了对3D标注的依赖，通过神经场体渲染和弱2D监督实现了有效的单目3D目标检测，为减少标注成本提供了可行方案
...
### [147] [Rice-VL: Evaluating Vision-Language Models for Cultural Understanding Across ASEAN Countries](https://arxiv.org/abs/2512.01419)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在西方中心偏见，在东南亚等文化多元地区的表现受限，需要专门的基准来评估模型的文化理解能力。

Method: 构建RICE-VL基准，包含11个东盟国家的28,000+人工标注VQA样本（包括判断题、填空题和开放式问题）和1,000个视觉定位样本，涵盖14个子类别。提出SEA-LAVE评估指标，衡量文本准确性、文化对齐和国家识别能力。

Result: 评估6个开源和闭源VLM显示，模型在低资源国家和抽象文化领域存在显著性能差距。视觉定位任务测试了模型在复杂场景中定位文化重要元素的能力，揭示了空间和上下文准确性的不足。

Conclusion: RICE-VL暴露了VLM在文化理解方面的局限性，强调了需要开发更具包容性的模型来更好地服务全球多样化人群。
...
TL;DR: TabletopGen：无需训练的自动框架，从参考图像生成交互式3D桌面场景，通过实例分割、3D重建和空间对齐实现高保真物理模拟


<details>
  <summary>Details</summary>
Motivation: 现有文本或图像驱动的3D场景生成方法主要关注大尺度场景，难以捕捉桌面场景的高密度布局和复杂空间关系，而高质量的3D桌面场景对具身AI和机器人操作策略学习至关重要

Method: 接受参考图像输入（可由文本到图像模型生成），进行实例分割和补全，将每个实例重建为3D模型并进行规范坐标对齐，然后通过姿态和尺度估计，组装成无碰撞的模拟就绪场景。核心创新是两阶段空间对齐方法：可微分旋转优化器用于精确旋转恢复，顶视图空间对齐机制用于鲁棒的平移和尺度估计

Result: 在视觉保真度、布局准确性和物理合理性方面达到最先进水平，显著超越现有方法，能够生成具有丰富风格和空间多样性的逼真桌面场景

Conclusion: TabletopGen是一个无需训练、完全自动的框架，能够生成多样化的实例级交互式3D桌面场景，解决了现有方法在处理桌面场景高密度布局和复杂空间关系方面的挑战，为具身AI和机器人操作提供了高质量的模拟环境
...
TL;DR: 提出两种简单的实例级极小极大重构方法，用于优化部分AUC（PAUC），消除了近似误差，具有线性计算复杂度和收敛保证。


<details>
  <summary>Details</summary>
Motivation: PAUC是处理类别不平衡和决策约束的重要评估指标，但现有优化方法存在不可控的近似误差或可扩展性有限的问题，需要更精确高效的解决方案。

Method: 提出两种实例级极小极大重构：一种具有渐近消失的近似误差，另一种无偏但需要更多变量。通过建立等价的实例级问题降低时间复杂度，通过阈值学习简化样本选择，应用不同的平滑技术。

Result: 算法具有线性迭代计算复杂度，收敛率为O(ε^{-1/3})，提供了紧致的泛化边界，实验验证了方法的有效性。

Conclusion: 成功消除了PAUC优化的近似误差，提出了高效可扩展的优化方法，为实际应用提供了理论保证和实用工具。
...
TL;DR: 本文提出了一种基于模糊规则的潜在空间多路径扩散模型（DFS），用于解决传统扩散模型在处理特征差异大的图像集合时的局限性，通过多路径学习不同特征类别、规则链推理协调路径，以及模糊隶属度压缩机制降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在生成高分辨率逼真图像方面表现出色，但在处理特征差异显著的图像集合时仍存在困难。传统方法尝试通过多路径学习不同图像区域，但存在路径协调效率低和计算成本高的问题。

Method: 提出扩散模糊系统（DFS）：1）使用多个扩散路径，每个路径专门学习特定类别的图像特征；2）采用基于规则链的推理动态引导扩散过程，实现多路径高效协调；3）引入基于模糊隶属度的潜在空间压缩机制降低计算成本。

Result: 在LSUN Bedroom、LSUN Church和MS COCO三个公开数据集上的实验表明：DFS比现有单路径和多路径扩散模型训练更稳定、收敛更快；在图像质量、文本-图像对齐度方面超越基线模型；生成图像与目标参考的对比精度也有所提升。
...
TL;DR: 该研究提出了一个大规模、多中心的脑影像深度无监督异常检测基准测试，评估了多种算法在不同扫描仪、病变类型和人口统计学因素下的表现，发现重建方法（特别是扩散模型）在病变分割上表现最佳，但所有算法都存在系统性偏差。
...
Method: 使用大规模多中心数据集：训练集包含2,976个T1和2,972个T2加权健康扫描，验证集92个扫描，测试集2,221个T1w和1,262个T2w扫描。系统评估不同算法在扫描仪、病变类型/大小、人口统计学因素（年龄、性别）下的表现。

Result: 所有算法的Dice分割性能在0.03-0.65之间变化。重建方法（特别是扩散模型）在病变分割上表现最强，而基于特征的方法在分布偏移下更稳健。但大多数算法存在系统性偏差：小病变和低对比度病变更易漏检，假阳性随年龄和性别变化。增加健康训练数据仅带来有限改善。

Conclusion: 当前无监督异常检测框架主要受算法限制而非数据可用性。基准测试为未来研究提供了透明基础，并确定了临床转化的优先事项：图像原生预训练、原则性偏差测量、公平感知建模和稳健域适应。
...
### [152] [MasHeNe: A Benchmark for Head and Neck CT Mass Segmentation using Window-Enhanced Mamba with Frequency-Domain Integration](https://arxiv.org/abs/2512.01563)
...
TL;DR: 提出了MasHeNe数据集和WEMF模型用于头颈部肿块分割，填补了现有数据集只关注恶性肿瘤的空白，在挑战性任务上取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有公开数据集主要关注恶性病变，忽视了头颈部其他占位性病变。为了填补这一空白，需要创建包含肿瘤和囊肿的全面数据集，并开发有效的分割方法。

Method: 1) 创建MasHeNe数据集：包含3,779张增强CT切片，具有像素级标注；2) 提出WEMF模型：采用三窗增强丰富输入外观，在多频注意力机制下融合跳跃连接信息，基于U形Mamba架构。

Result: WEMF在MasHeNe数据集上取得最佳性能：Dice 70.45%，IoU 66.89%，NSD 72.33%，HD95 5.12 mm。模型表现出稳定且强大的结果，但误差模式表明该任务仍具挑战性。

Conclusion: MasHeNe为超越仅恶性肿瘤的头颈部肿块分割提供了基准，WEMF模型在该挑战性任务上表现优异。观察到的误差模式表明该领域仍需进一步研究。
...
### [153] [PSR: Scaling Multi-Subject Personalized Image Generation with Pairwise Subject-Consistency Rewards](https://arxiv.org/abs/2512.01236)
*Shulei Wang,Longhui Wei,Xin He,Jianbo Ouyang,Hui Lu,Zhou Zhao,Qi Tian*

Main category: cs.CV

TL;DR: 提出PSR方法解决多主体个性化图像生成问题，通过数据生成管道和强化学习奖励机制提升主体一致性和文本可控性


<details>
  <summary>Details</summary>
Motivation: 现有单主体个性化生成模型效果显著，但扩展到多主体时性能下降，特别是在保持主体一致性和遵循文本提示方面。这主要由于缺乏高质量多主体数据集和精细的后训练策略。

Method: 1) 提出可扩展的多主体数据生成管道，利用强大的单主体生成模型构建多样高质量的多主体训练数据；2) 设计成对主体一致性奖励和通用奖励，通过强化学习阶段提升主体一致性和文本可控性；3) 引入包含7个子集的新基准，从三个维度全面评估多主体个性化生成性能。

Result: 大量实验证明该方法在推进多主体个性化图像生成方面的有效性。模型在多主体场景下能更好地保持主体一致性和文本可控性。

Conclusion: PSR方法通过创新的数据生成管道和强化学习奖励机制，有效解决了多主体个性化图像生成中的主体一致性和文本可控性问题，为多主体个性化生成提供了新解决方案。
...
TL;DR: RoleMotion是一个大规模人体动作数据集，专注于角色扮演和功能性动作，包含25个场景、110个角色、500多种行为，以及1万多个高质量身体和手部动作序列，带有精细文本标注。


<details>
  <summary>Details</summary>
Motivation: 现有文本动作数据集存在以下问题：1）数据分散，由不同子集拼凑而成；2）动作数据缺乏功能性，难以协同覆盖社交活动；3）动作质量不一致；4）文本标注缺乏细粒度细节。需要构建一个专注于场景和角色的高质量数据集。

Method: 精心设计和收集RoleMotion数据集，包含25个经典场景、110个功能性角色、500多种行为、10296个高质量身体和手部动作序列，标注了27831个细粒度文本描述。构建了比现有方法更强的评估器，并验证其可靠性。

Result: 实验结果表明，RoleMotion数据集在文本驱动的全身动作生成方面表现出高质量和功能性。评估了各种文本到动作方法在该数据集上的性能，并探索了身体和手部动作生成的相互作用。

Conclusion: RoleMotion是一个高质量、功能性强的大规模人体动作数据集，专注于场景和角色，为文本驱动的全身动作生成提供了更好的基准和评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 表格识别（TR）作为文档解析的核心组件，传统上依赖监督学习，但获取大规模标注数据成本高昂。虽然专有模型不断突破性能边界，但受限于资源和隐私法规，开源模型性能仍远落后。需要一种无需标注数据的方法来缩小这一差距。

Method: 提出TRivia自监督微调方法，基于Group Relative Policy Optimization，自动识别最能促进学习的无标签样本。通过注意力引导模块为每个表格图像生成多样化问题，以问答形式的奖励机制提供反馈，形成闭环优化过程，让模型自主学习表格识别、结构化和推理。

Result: 推出了TRivia-3B开源模型，在三个流行基准测试中超越了现有系统（如Gemini 2.5 Pro、MinerU2.5），成为紧凑且最先进的表格识别模型。

Conclusion: TRivia方法成功实现了无需标注数据的表格识别学习，显著提升了开源模型的性能，为受隐私法规限制的实际应用提供了可行解决方案，缩小了与专有模型的差距。
...
<details>
  <summary>Details</summary>
Motivation: 传统粘度计具有侵入性，需要在受控实验室环境中使用，与实际工艺条件差异大。需要一种非接触、适应实际工艺条件的粘度测量方法。

Method: 利用计算机视觉技术，通过固定背景图案在混合过程中自由表面变形引起的光折射扭曲来推断粘度。采用多图案策略增强视觉线索，并加入不确定性量化确保传感器可靠性。

Result: 在不同光照条件下，系统在log m² s⁻¹单位上的平均绝对误差为0.113，粘度等级预测准确率最高达81%。多图案策略提高了鲁棒性，不确定性量化提供了置信度估计。

Conclusion: 这种非接触式粘度计为现有粘度测量方法提供了实用的自动化替代方案，适用于过程监控和自主实验室操作。
...
TL;DR: 论文提出了Envision基准，用于评估多图像序列生成中的因果事件进展能力，发现当前模型在时空一致性方面存在局限
...
Method: 提出Envision基准，包含1000个四阶段提示，涵盖6个科学和人文领域，基于世界知识和时空因果关系构建；引入Envision-Score综合评估指标，整合多维度一致性、物理性和美学

Result: 评估15个模型发现：专用T2I模型美学渲染能力强但缺乏内在世界知识；统一多模态模型在因果叙事连贯性上表现更好，但仍落后于闭源模型，且在时空一致性方面存在挑战
...
### [158] [nnMobileNet++: Towards Efficient Hybrid Networks for Retinal Image Analysis](https://arxiv.org/abs/2512.01273)
*Xin Li,Wenhui Zhu,Xuanzhao Dong,Hao Wang,Yujian Xiong,Oana Dumitrascu,Yalin Wang*

Main category: cs.CV

TL;DR: nnMobileNet++：一种结合卷积和Transformer的混合架构，用于视网膜图像分析，在保持低计算成本的同时实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 传统卷积网络在视网膜图像分析中难以捕捉长距离依赖关系和不规则病变特征，而血管特征对临床诊断至关重要。需要一种轻量级但能有效建模全局上下文的架构。

Method: 提出nnMobileNet++混合架构，包含三个关键组件：1) 动态蛇形卷积用于边界感知特征提取；2) 在第二个下采样阶段后引入阶段特定的Transformer块进行全局上下文建模；3) 视网膜图像预训练以提高泛化能力。

Result: 在多个公共视网膜数据集上的分类实验和消融研究表明，nnMobileNet++实现了最先进或极具竞争力的准确率，同时保持低计算成本。

Conclusion: nnMobileNet++作为一种轻量级但有效的视网膜图像分析框架具有重要潜力，成功结合了卷积的局部特征提取能力和Transformer的全局建模优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有声纳图像AI模型过度依赖海床特征，导致泛化能力差，需要解决背景偏差问题并提高模型可解释性

Method: 提出两个模块：1) TCU模块扩展三元组损失减少海床背景偏差；2) UESF框架提供可视化解释，并适配LIME解释器生成更准确的归因分析

Result: 在真实和合成声纳数据集上的实验验证了方法在去学习效果、模型鲁棒性和可解释性方面的显著改进
...
TL;DR: EGG-Fusion：一种实时3D重建系统，通过鲁棒的稀疏到稠密相机跟踪和几何感知的高斯面元映射，结合信息滤波器融合方法，在考虑传感器噪声的情况下实现高精度表面重建，在保持24FPS实时处理的同时，精度比现有方法提升20%以上。


<details>
  <summary>Details</summary>
Motivation: 当前基于可微分渲染的SLAM系统面临实时计算和传感器噪声敏感性的双重挑战，导致场景重建的几何保真度下降和实用性受限。需要一种既能实时处理又能抵抗噪声的高精度重建方法。

Method: 提出EGG-Fusion系统，包含鲁棒的稀疏到稠密相机跟踪模块和几何感知的高斯面元映射模块。采用基于信息滤波器的融合方法，显式考虑传感器噪声，实现高精度表面重建。可微分高斯面元映射有效建模多视角一致表面，同时实现高效参数优化。

Result: 在Replica和ScanNet++等标准基准数据集上，表面重建误差达到0.6cm，相比最先进的基于高斯分布的方法精度提升超过20%。系统保持24FPS的实时处理能力，成为最准确的基于可微分渲染的实时重建系统之一。

Conclusion: EGG-Fusion成功解决了实时3D重建中计算效率与噪声敏感性的平衡问题，通过创新的高斯面元映射和信息滤波器融合方法，在保持实时性的同时显著提升了重建精度，为可微分渲染SLAM系统提供了实用化解决方案。
...
TL;DR: TBT-Former通过增强的Transformer骨干、跨尺度特征金字塔和边界分布回归头，解决了TAL中模糊边界定位和多尺度信息融合问题，在THUMOS14和EPIC-Kitchens 100上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有单阶段无锚模型（如ActionFormer）在时间动作定位中存在两个主要问题：1）对模糊时间边界的精确定位困难；2）多尺度上下文信息融合效果有限。需要解决这些限制来提升TAL性能。

Method: 提出TBT-Former架构，包含三个核心创新：1）增强容量的缩放Transformer骨干（更多注意力头和扩展的MLP维度）；2）跨尺度特征金字塔网络（FPN），通过自上而下路径和横向连接融合高层语义和低层时间细节；3）新颖的边界分布回归头，将边界回归重新定义为概率分布学习问题，显式表示边界不确定性。

Result: 在竞争激烈的THUMOS14和EPIC-Kitchens 100数据集上建立了新的性能水平，同时在大规模ActivityNet-1.3上保持竞争力。

Conclusion: TBT-Former通过增强的Transformer骨干、改进的多尺度特征融合和概率边界回归，有效解决了时间动作定位中的模糊边界和上下文融合问题，超越了现有Transformer基线的性能。
...
TL;DR: DCText：一种无需训练的可视化文本生成方法，采用分治策略解决多文本生成问题，在保持图像质量的同时实现最佳文本准确率和最低生成延迟


<details>
  <summary>Details</summary>
Motivation: 尽管现有的文本到图像模型在文本渲染方面取得了高保真度，但在处理长文本或多个文本时仍存在困难，主要原因是全局注意力被稀释

Method: 采用分治策略，首先通过提取和分割目标文本分解提示，然后将每个文本分配到指定区域。引入两种注意力掩码（文本聚焦和上下文扩展）在去噪过程中顺序应用，同时使用局部噪声初始化进一步提高文本准确性和区域对齐

Result: 在单句和多句基准测试中，DCText在保持图像质量的同时实现了最佳的文本准确率，并且具有最低的生成延迟

Conclusion: DCText通过分治策略有效解决了多文本生成问题，在文本准确性和计算效率方面都表现出色，为文本到图像生成提供了有效的解决方案
...
### [163] [GrndCtrl: Grounding World Models via Self-Supervised Reward Alignment](https://arxiv.org/abs/2512.01952)
...
TL;DR: 提出RLWG框架，通过几何和感知奖励对预训练世界模型进行自监督后训练对齐，提升空间一致性和导航稳定性


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型虽然视觉保真度高，但缺乏几何基础，限制了在需要空间一致性和长时程稳定性的导航任务中的应用

Method: 提出RLWG自监督后训练框架，使用姿态循环一致性、深度重投影和时间一致性等多重奖励，基于GRPO的GrndCtrl方法进行奖励对齐

Result: RLWG框架产生保持稳定轨迹、一致几何和可靠rollout的世界模型，在室外环境中优于监督微调

Conclusion: 类似于大语言模型的后训练对齐，RLWG通过可验证奖励桥接生成预训练和基础行为，实现优越的空间一致性和导航稳定性
...
### [164] [Visual Sync: Multi-Camera Synchronization via Cross-View Object Motion](https://arxiv.org/abs/2512.02017)
*Shaowei Liu,David Yifan Yao,Saurabh Gupta,Shenlong Wang*

Main category: cs.CV

TL;DR: VisualSync是一个基于多视角动态的优化框架，能够以毫秒级精度对齐未标定、未同步的视频，通过利用运动3D点的对极约束来实现跨摄像头同步。


<details>
  <summary>Details</summary>
Motivation: 随着消费级摄像头的普及，人们可以轻松记录各种重要时刻，但跨摄像头视频流的同步仍然具有挑战性。现有方法通常需要受控环境、特定目标、手动校正或昂贵硬件。

Method: VisualSync基于多视角动态优化框架，利用运动3D点在两个摄像头中可见时遵循对极约束的原理。它使用现成的3D重建、特征匹配和密集跟踪来提取轨迹、相对姿态和跨视角对应关系，然后通过最小化对极误差来估计每个摄像头的时间偏移。

Result: 在四个多样化、具有挑战性的数据集上的实验表明，VisualSync优于基线方法，实现了中位数同步误差低于50毫秒的精度。

Conclusion: VisualSync提供了一种有效的方法来解决跨摄像头视频同步问题，无需受控环境、特定目标、手动校正或昂贵硬件，实现了高精度的自动同步。
...
TL;DR: 研究探讨了脑影像数据质量对预训练的影响，发现不同质量水平的数据在下游任务中表现差异显著，揭示了数据质量在医学影像预训练中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 大规模脑影像数据集为领域基础模型预训练提供了机会，但这些数据存在高度异质性（从高质量扫描到严重失真或不完整的脑体积）。核心问题是：噪声或低质量扫描是否能对预训练做出有意义的贡献，还是会阻碍模型学习？

Method: 系统探索数据质量水平在预训练中的作用及其对下游任务的影响。具体方法：在不同质量水平的数据集上进行预训练，然后在外部队列上进行脑年龄预测的微调。

Result: 结果显示不同质量水平之间存在显著的性能差异，揭示了机会和局限性。高质量数据通常带来更好的下游任务性能，但某些情况下低质量数据也可能提供有用的学习信号。

Conclusion: 计算机视觉实践与临床神经影像标准之间存在差距，强调需要领域感知的数据管理来确保可信且可泛化的领域特定基础模型。数据质量是医学影像预训练的关键考虑因素。
...
### [166] [IVCR-200K: A Large-Scale Multi-turn Dialogue Benchmark for Interactive Video Corpus Retrieval](https://arxiv.org/abs/2512.01312)
*Ning Han,Yawen Zeng,Shaohua Long,Chengqing Li,Sijie Yang,Dun Tan,Jianfeng Dong,Jingjing Chen*

Main category: cs.CV

TL;DR: 提出交互式视频语料检索(IVCR)任务，构建IVCR-200K双语多轮对话数据集，基于多模态大语言模型开发交互框架
...
Method: 引入IVCR任务，构建IVCR-200K双语多轮对话数据集，提出基于多模态大语言模型的综合框架支持多种交互模式

Result: 实验证明数据集和框架的有效性，支持视频检索和时刻检索的交互式对话

Conclusion: IVCR任务更贴近现实需求，提出的数据集和框架为交互式视频检索研究提供了基础
...
### [167] [TokenPure: Watermark Removal through Tokenized Appearance and Structural Guidance](https://arxiv.org/abs/2512.01314)
*Pei Yang,Yepeng Liu,Kelly Peng,Yuan Gao,Yiren Song*

Main category: cs.CV

TL;DR: TokenPure是一个基于扩散变换器的水印去除框架，通过令牌化条件重建解决水印去除与内容一致性的平衡问题，在去除水印的同时保持图像的结构完整性和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 在数字经济时代，数字水印对于AI生成内容和其他虚拟资产的所有权证明至关重要。设计能够抵抗各种攻击和处理操作的鲁棒水印变得尤为重要，因此需要开发有效的水印去除方法来评估和改进水印系统的安全性。

Method: TokenPure采用基于扩散变换器的框架，将水印去除任务重新定义为条件生成问题。该方法将水印图像分解为两个互补的令牌集：用于纹理的视觉令牌和用于几何的结构令牌。这些令牌共同调节扩散过程，使框架能够合成无水印图像，同时保持细粒度一致性和结构完整性。

Result: 综合实验表明，TokenPure在水印去除和重建保真度方面达到了最先进的性能，在感知质量和一致性方面显著优于现有基线方法。

Conclusion: TokenPure通过令牌化条件重建方法有效解决了水印去除与内容一致性之间的权衡问题，为数字水印系统的安全性评估提供了强大的工具，同时保持了图像的结构完整性和视觉质量。
...
### [168] [Optimizing Stroke Risk Prediction: A Machine Learning Pipeline Combining ROS-Balanced Ensembles and XAI](https://arxiv.org/abs/2512.01333)
*A S M Ahsanul Sarkar Akib,Raduana Khawla,Abdul Hasib*

Main category: cs.CV

TL;DR: 该研究开发了一个结合集成学习和可解释AI的机器学习框架，用于卒中风险预测，在卒中预测数据集上达到99.09%的准确率，并识别出年龄、高血压和血糖水平三个关键临床变量。


<details>
  <summary>Details</summary>
Motivation: 卒中是全球主要的死亡和永久性损伤原因，早期风险评估对于及时干预和有效预防策略至关重要。当前需要既准确又可解释的卒中风险预测方法。

Method: 采用集成建模和可解释AI技术，包括特征工程、数据预处理（使用随机过采样解决类别不平衡）、10种机器学习模型的5折交叉验证评估，最终构建优化的集成模型（随机森林+ExtraTrees+XGBoost）。

Result: 优化的集成模型在卒中预测数据集上表现出色，达到99.09%的准确率。通过LIME可解释性分析识别出三个关键临床变量：年龄、高血压和血糖水平。

Conclusion: 结合集成学习和可解释AI可以提供高准确性和可解释性的卒中风险评估，通过早期预测实现数据驱动的预防和个性化临床决策，有望改变卒中预测和心血管风险管理。
...
### [169] [AlignVid: Training-Free Attention Scaling for Semantic Fidelity in Text-Guided Image-to-Video Generation](https://arxiv.org/abs/2512.01334)
*Yexin Liu,Wen-Jie Shu,Zile Huang,Haoze Zheng,Yueze Wang,Manyuan Zhang,Ser-Nam Lim,Harry Yang*

Main category: cs.CV

TL;DR: 提出AlignVid框架，通过注意力缩放调制和引导调度改进文本引导图像到视频生成的语义遵从性，解决现有方法在图像显著变换时的语义忽视问题


<details>
  <summary>Details</summary>
Motivation: 现有文本引导图像到视频生成方法在处理需要图像显著变换（如对象添加、删除或修改）的提示时，存在语义忽视问题，无法充分遵从细粒度提示语义

Method: 提出AlignVid框架：1) 注意力缩放调制(ASM)，通过轻量级的Q或K缩放直接重新加权注意力；2) 引导调度(GS)，在transformer块和去噪步骤中选择性应用ASM以减少视觉质量退化

Result: 实验表明AlignVid能显著提升语义保真度，同时引入OmitI2V评估数据集（367个人工标注样本，涵盖添加、删除和修改场景）来评估TI2V生成中的语义忽视问题

Conclusion: AlignVid通过最小干预有效解决了文本引导图像到视频生成中的语义忽视问题，在提升语义遵从性的同时限制了美学质量退化，为TI2V生成提供了有效的训练免费解决方案
...
### [170] [EvalTalker: Learning to Evaluate Real-Portrait-Driven Multi-Subject Talking Humans](https://arxiv.org/abs/2512.01340)
...
TL;DR: 该论文构建了首个大规模多说话人生成质量评估数据集THQA-MT，并提出EvalTalker框架来评估多说话人生成质量，解决了当前多说话人生成技术质量下降的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多说话人（Multi-Talker）生成技术存在明显的质量下降问题，导致用户体验不佳。现有研究缺乏专门的多说话人生成质量评估数据集和方法，限制了高质量多说话人生成技术的发展。

Method: 1. 构建THQA-MT数据集：包含5,492个多说话人生成视频，来自15个代表性多说话人生成方法，基于400个在线收集的真实肖像；2. 通过主观实验分析不同多说话人生成方法的感知差异，识别12种常见失真类型；3. 提出EvalTalker评估框架，具备感知全局质量、人类特征和身份一致性的能力，并集成Qwen-Sync感知多模态同步性。

Result: 实验结果表明，EvalTalker与主观评分具有优越的相关性，为未来高质量多说话人生成和评估研究提供了坚实基础。

Conclusion: 该研究填补了多说话人生成质量评估的空白，提出的THQA-MT数据集和EvalTalker框架为提升多说话人生成质量提供了有效的评估工具，将推动更高质量的多说话人生成技术发展。
...
### [171] [InternVideo-Next: Towards General Video Foundation Models without Video-Text Supervision](https://arxiv.org/abs/2512.01342)
*Chenting Wang,Yuhan Zhu,Yicheng Xu,Jiange Yang,Ziang Yan,Yali Wang,Yi Wang,Limin Wang*

Main category: cs.CV

TL;DR: 论文提出InternVideo-Next，一种两阶段视频预训练框架，通过Encoder-Predictor-Decoder架构和条件扩散解码器解决传统掩码视频建模中像素重建与语义抽象的矛盾，在公开无标签视频上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视频-文本预训练依赖噪声合成字幕，缺乏隐式世界知识（如物体运动、3D几何、物理线索）；而掩码视频建模直接利用时空结构但性能落后于文本监督方法。这种差距源于架构问题：像素级重建收敛困难且与语义冲突，潜在预测易导致捷径学习。

Method: 提出Encoder-Predictor-Decoder框架，其中预测器作为潜在世界模型。采用两阶段预训练：第一阶段使用条件扩散解码器注入图像级语义先验，平衡像素保真度与高层语义抽象；第二阶段在构建的潜在空间中预测冻结的第一阶段目标，避免捷径学习。

Result: 在公开无标签视频上训练的InternVideo-Next在多个基准测试中达到最先进性能，为通用视频表示学习提供了可扩展路径。

Conclusion: 通过解耦编码器-解码器设计为Encoder-Predictor-Decoder框架，并采用两阶段训练策略，成功解决了传统掩码视频建模中像素重建与语义抽象的冲突，实现了语义一致且细节保留的潜在空间构建。
...
### [172] [Handwritten Text Recognition for Low Resource Languages](https://arxiv.org/abs/2512.01348)
*Sayantan Dey,Alireza Alaei,Partha Pratim Roy*

Main category: cs.CV

TL;DR: 提出BharatOCR，一种用于印地语和乌尔都语的无分割段落级手写文本识别系统，采用ViT-Transformer Decoder-LM架构，在多个数据集上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 低资源语言（如印地语、乌尔都语）的段落级手写文本识别仍然具有挑战性，这些语言通常缺乏全面的语言资源，需要开发鲁棒的光学字符识别系统。

Method: 提出ViT-Transformer Decoder-LM架构：Vision Transformer提取视觉特征，Transformer解码器生成文本序列，预训练语言模型优化输出。采用DeiT进行掩码图像建模，RoBERTa进行掩码语言建模，通过隐式行分割逐行处理段落图像。

Result: 在NUST-UHWR、PUCIT-OUHL和Parimal-Urdu数据集上分别达到96.24%、92.05%和94.80%的字符识别率，在印地语数据集上达到80.64%的字符识别率，超越了多种最先进的乌尔都语文本识别方法。

Conclusion: BharatOCR在低资源语言的段落级手写文本识别方面表现出色，为印地语和乌尔都语等语言提供了有效的OCR解决方案，并在多个数据集上建立了新的基准。
...
### [173] [OpenBox: Annotate Any Bounding Boxes in 3D](https://arxiv.org/abs/2512.01352)
*In-Jae Lee,Mungyeom Kim,Kwonyoung Ryu,Pierre Musacchio,Jaesik Park*

Main category: cs.CV

TL;DR: OpenBox：一种利用2D视觉基础模型的两阶段自动标注流水线，用于无监督和开放词汇的3D物体检测，无需自训练即可生成高质量的3D边界框标注。


<details>
  <summary>Details</summary>
Motivation: 现有3D物体检测方法存在三个主要问题：1）统一标注3D边界框，忽略物体的物理状态；2）需要多次自训练迭代进行标注细化；3）导致标注质量不佳且计算开销大。在自动驾驶领域，降低标注成本和识别未见物体对安全性和可扩展性至关重要。

Method: OpenBox采用两阶段流水线：第一阶段通过跨模态实例对齐，将2D视觉基础模型处理的图像中的实例级线索与对应的3D点云关联；第二阶段根据刚性和运动状态对实例进行分类，然后使用类别特定的尺寸统计生成自适应边界框。

Result: 在Waymo Open Dataset、Lyft Level 5 Perception dataset和nuScenes数据集上的实验表明，OpenBox在准确性和效率上均优于基线方法，能够生成高质量的3D边界框标注。

Conclusion: OpenBox通过利用2D视觉基础模型和两阶段标注流程，成功解决了现有3D物体检测方法的问题，实现了无需自训练的高质量自动标注，为无监督和开放词汇的3D物体检测提供了有效解决方案。
...
### [174] [BlinkBud: Detecting Hazards from Behind via Sampled Monocular 3D Detection on a Single Earbud](https://arxiv.org/abs/2512.01366)
*Yunzhe Li,Jiajun Yan,Yuzhou Wei,Kechen Liu,Yize Zhao,Chong Zhang,Hongzi Zhu,Li Lu,Shan Chang,Minyi Guo*

Main category: cs.CV

TL;DR: BlinkBud：利用单耳塞和配对手机在线检测用户后方接近的危险物体，通过3D目标跟踪算法和强化学习优化图像采样策略，实现低功耗高精度检测。


<details>
  <summary>Details</summary>
Motivation: 行人/骑行者难以察觉后方快速接近的车辆，这对道路安全构成重大威胁。需要一种低功耗、高精度的解决方案来检测后方危险物体。

Method: 1. 使用单耳塞和配对手机系统；2. 开发新颖的3D目标跟踪算法，结合卡尔曼滤波轨迹估计和基于强化学习的最优图像采样策略；3. 利用估计的俯仰角和偏航角校正目标深度估计，并将相机坐标系对齐到用户身体坐标系，消除头部运动影响。

Result: 原型系统在真实环境中测试：耳塞平均功耗29.8mW，智能手机702.6mW；危险检测平均假阳性率4.90%，假阴性率1.47%，表现出轻量级和高精度特性。

Conclusion: BlinkBud系统能够以超低功耗准确检测用户后方接近的危险物体，有效提升行人/骑行者的道路安全，解决了传统方法在功耗和精度方面的平衡问题。
...
### [175] [SRAM: Shape-Realism Alignment Metric for No Reference 3D Shape Evaluation](https://arxiv.org/abs/2512.01373)
*Sheng Liu,Tianyu Luan,Phani Nuney,Xuelu Feng,Junsong Yuan*

Main category: cs.CV

TL;DR: 提出Shape-Realism Alignment Metric，利用大语言模型作为桥梁评估3D形状的真实感，无需ground truth参考，并在新数据集RealismGrading上验证了其与人类感知的良好相关性。


<details>
  <summary>Details</summary>
Motivation: 传统3D形状评估方法依赖ground truth测量网格保真度，但在实际应用中，形状的真实感并不需要ground truth参考。现有方法无法有效评估形状的真实感，特别是在没有参考形状的情况下。

Method: 1) 采用网格编码方法将3D形状转换为语言标记空间；2) 设计专门的真实感解码器，使语言模型输出与人类真实感感知对齐；3) 引入新数据集RealismGrading，包含16种不同算法生成的形状和人类标注的真实感分数。

Result: 通过k折交叉验证验证了指标的性能和泛化能力。实验结果表明，该指标与人类感知相关性良好，优于现有方法，并具有良好的泛化性。

Conclusion: 提出的Shape-Realism Alignment Metric能够有效评估3D形状的真实感，无需ground truth参考，为3D生成和重建提供了更实用的评估工具。
...
### [176] [Textured Geometry Evaluation: Perceptual 3D Textured Shape Metric via 3D Latent-Geometry Network](https://arxiv.org/abs/2512.01380)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D模型保真度评估方法存在不足：传统几何指标（如Chamfer Distance）与人类感知不一致；基于渲染的方法存在结构覆盖不全、视角选择敏感问题；多数方法基于合成失真训练，与真实失真存在领域差距

Method: 提出Textured Geometry Evaluation (TGE)方法，直接基于带纹理的3D网格，联合利用几何和颜色信息计算输入纹理网格相对于参考彩色形状的保真度，并构建了人工标注的真实世界失真数据集进行训练和评估

Result: 实验表明，TGE在真实世界失真数据集上优于基于渲染的方法和仅使用几何信息的方法

Conclusion: TGE方法通过直接处理纹理3D网格并利用真实世界失真数据，实现了更符合人类感知的3D模型保真度评估
...
### [177] [Reversible Inversion for Training-Free Exemplar-guided Image Editing](https://arxiv.org/abs/2512.01382)
*Yuke Li,Lianli Gao,Ji Zhang,Pengpeng Zeng,Lichuan Xiang,Hongkai Wen,Heng Tao Shen,Jingkuan Song*

Main category: cs.CV

TL;DR: ReInversion是一种免训练的图像编辑方法，通过两阶段去噪和掩码引导的选择性去噪，实现高效且高质量的示例引导图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有示例引导图像编辑方法需要大规模预训练，计算成本高；而标准反演技术虽然免训练，但质量差且效率低，需要改进。

Method: 提出ReInversion方法：1) 两阶段去噪过程：先基于源图像去噪，再基于参考图像去噪；2) 掩码引导的选择性去噪策略，限制编辑到目标区域，保持背景结构一致性。

Result: 定性和定量比较表明，ReInversion方法在示例引导图像编辑任务上达到最先进性能，同时计算开销最低。

Conclusion: ReInversion是一种高效且有效的免训练示例引导图像编辑方法，通过创新的两阶段去噪和选择性编辑策略，解决了现有方法的计算成本和质量问题。
...
### [178] [PointNet4D: A Lightweight 4D Point Cloud Video Backbone for Online and Offline Perception in Robotic Applications](https://arxiv.org/abs/2512.01383)
*Yunze Liu,Zifan Wang,Peiran Wu,Jiayang Ao*

Main category: cs.CV

TL;DR: PointNet4D：一种轻量级4D骨干网络，结合Mamba状态空间模型和Transformer，用于实时处理动态4D环境（3D空间随时间演化），支持在线和离线场景，在多种任务和数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 机器人系统和交互应用需要实时处理流式点云视频（4D数据），但现有4D骨干网络依赖计算密集的时空卷积和Transformer，不适合实时应用。需要轻量级、高效的4D处理架构。

Method: 提出PointNet4D，核心是混合Mamba-Transformer时序融合块，结合Mamba的高效状态空间建模和Transformer的双向建模能力。引入4DMAP帧级掩码自回归预训练策略来捕捉跨帧运动线索。

Result: 在7个数据集的9个任务上广泛评估，显示在多个领域的一致改进。构建了两个机器人应用系统：4D Diffusion Policy和4D Imitation Learning，在RoboTwin和HandoverSim基准上取得显著提升。

Conclusion: PointNet4D是一种高效、轻量级的4D骨干网络，适用于实时机器人应用，通过混合Mamba-Transformer架构和创新的预训练策略，在多种任务上表现优异，展示了实际机器人应用的潜力。
...
### [179] [FRAMER: Frequency-Aligned Self-Distillation with Adaptive Modulation Leveraging Diffusion Priors for Real-World Image Super-Resolution](https://arxiv.org/abs/2512.01390)
*Seungho Choi,Jeahun Sung,Jihyong Oh*

Main category: cs.CV

TL;DR: FRAMER是一种即插即用的训练方案，通过频域分解和对比学习，解决扩散模型在真实图像超分辨率中的低频偏置问题，提升高频细节重建。
...
Method: 提出FRAMER训练方案：1) 使用最终层特征图指导所有中间层；2) 通过FFT掩码将特征分解为低频/高频带；3) 低频使用IntraCL稳定全局结构；4) 高频使用InterCL锐化实例细节；5) 引入FAW和FAM自适应调制器重新加权和门控蒸馏。

Result: 在U-Net和DiT骨干网络（如Stable Diffusion 2、3）上，FRAMER一致提升了PSNR/SSIM和感知指标（LPIPS、NIQE、MANIQA、MUSIQ）。消融实验验证了最终层教师和随机层负样本的有效性。

Conclusion: FRAMER通过频域感知的对比蒸馏，有效解决了扩散模型在真实图像超分辨率中的低频偏置问题，无需改变骨干网络或推理过程即可显著提升重建质量。
...
### [180] [MDiff4STR: Mask Diffusion Model for Scene Text Recognition](https://arxiv.org/abs/2512.01422)
*Yongkun Du,Miaomiao Zhao,Songlin Fan,Zhineng Chen,Caiyan Jia,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: MDiff4STR将掩码扩散模型首次引入场景文本识别任务，通过两种改进策略解决了训练推理噪声差异和过度自信预测问题，在保持高效推理的同时超越了自回归模型的准确率。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在视觉语言任务中展现出效率与准确性的良好平衡，但直接应用于场景文本识别任务时，准确率落后于自回归模型。需要解决训练推理噪声差异和过度自信预测两大挑战，以提升掩码扩散模型在文本识别中的性能。

Method: 提出MDiff4STR模型，包含两种关键改进策略：1) 开发六种噪声策略以缩小训练与推理之间的噪声差异；2) 提出令牌替换噪声机制，提供非掩码噪声类型，促使模型重新考虑和修正过度自信的错误预测。

Result: 在标准及挑战性STR基准测试中，包括不规则、艺术、遮挡和中文文本等多种场景，MDiff4STR始终优于流行的STR模型，在准确率上超越最先进的自回归模型，同时仅需三个去噪步骤即可实现快速推理。

Conclusion: MDiff4STR成功将掩码扩散模型应用于场景文本识别，通过针对性的改进策略解决了该任务中的特有挑战，实现了效率与准确性的双重优势，为STR任务提供了新的有效解决方案。
...
### [181] [\textit{ViRectify}: A Challenging Benchmark for Video Reasoning Correction with Multimodal Large Language Models](https://arxiv.org/abs/2512.01424)
*Xusen Hei,Jiali Chen,Jinyu Yang,Mengchen Zhao,Yi Cai*

Main category: cs.CV

TL;DR: ViRectify是一个评估多模态大语言模型视频推理错误纠正能力的基准，包含3万多个实例，涵盖动态感知、科学推理和具身决策领域，挑战模型进行细粒度错误识别和基于视频证据的纠正。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视频推理场景中经常出错，但现有基准缺乏对其错误识别和纠正能力的系统评估。需要建立一个全面的基准来揭示模型弱点并提升性能。

Method: 通过AI辅助标注流程和人工验证构建超过3万个实例的数据集。提出轨迹证据驱动的纠正框架，包括步骤级错误轨迹和基于视觉证据的奖励建模，使模型能专注于错误传播和关键时间戳。

Result: 评估16个先进MLLM显示ViRectify是一个具有挑战性的测试平台，GPT-5仅达到31.94%的纠正准确率。提出的框架使Qwen2.5-VL-7B模型在ViRectify上持续优于72B变体，展示了方法的有效性。

Conclusion: ViRectify为全面评估MLLM在视频推理中的能力提供了新方向，揭示了模型在错误纠正方面的系统性不对称性，数据集也可用于反思学习，是宝贵的数据资源。
...
### [182] [ResDiT: Evoking the Intrinsic Resolution Scalability in Diffusion Transformers](https://arxiv.org/abs/2512.01426)
*Yiyang Ma,Feng Zhou,Xuedan Yin,Pu Cao,Yonghao Dang,Jianqin Yin*

Main category: cs.CV

TL;DR: 提出ResDiT方法，无需训练即可扩展扩散变换器到高分辨率图像合成，解决布局崩溃和纹理退化问题


<details>
  <summary>Details</summary>
Motivation: 预训练扩散变换器用于高分辨率图像合成时会出现空间布局崩溃和纹理保真度下降的问题，现有方法需要复杂的多阶段流程，作者希望探索更高效的解决方案

Method: 1) 识别位置嵌入是控制空间布局的核心因素，提出位置嵌入缩放技术纠正分辨率变化时的位置编码；2) 基于基础分辨率局部注意力开发局部增强机制；3) 设计补丁级融合模块聚合全局和局部线索；4) 使用高斯加权拼接策略消除网格伪影

Result: ResDiT能够一致地生成高保真度高分辨率图像，无缝集成到下游任务中，包括空间可控生成

Conclusion: 通过分析扩散变换器的内在生成机制，提出了无需训练的ResDiT方法，有效解决了高分辨率图像合成中的布局崩溃和纹理退化问题，为扩散变换器的高效分辨率扩展提供了新思路
...
### [183] [Language-Guided Open-World Anomaly Segmentation](https://arxiv.org/abs/2512.01427)
*Klara Reichard,Nikolas Brasch,Nassir Navab,Federico Tombari*

Main category: cs.CV

TL;DR: Clipomaly：首个基于CLIP的自动驾驶开放世界和异常分割方法，无需异常训练数据，能动态扩展词汇表并给未知物体分配可解释的名称


<details>
  <summary>Details</summary>
Motivation: 现有开放世界和异常分割方法无法给未知区域分配有语义意义的标签，难以区分和学习未知类别的表示。开放词汇分割方法虽然能泛化到新类别，但需要固定推理词汇表，无法直接应用于类别无约束的异常分割。

Method: 基于CLIP的零样本方法，无需异常特定训练数据，利用CLIP共享的图像-文本嵌入空间，既能分割未知物体，又能为它们分配人类可解释的名称。与开放词汇方法不同，该模型能在推理时动态扩展词汇表而无需重新训练。

Result: 在已建立的异常分割基准测试中达到最先进的性能，同时提供实际部署所需的可解释性和灵活性。

Conclusion: Clipomaly是首个基于CLIP的自动驾驶开放世界和异常分割方法，成功解决了现有方法的局限性，实现了对超出常见类别定义（如Cityscapes）的异常的鲁棒检测和命名。
...
### [184] [FastAnimate: Towards Learnable Template Construction and Pose Deformation for Fast 3D Human Avatar Animation](https://arxiv.org/abs/2512.01444)
*Jian Shu,Nanjie Yao,Gangjian Zhang,Junlong Ren,Yu Feng,Hao Wang*

Main category: cs.CV

TL;DR: 提出统一学习框架解决3D人体化身动画中的模板构建和目标姿态变形问题，通过U-Net解耦纹理姿态信息实现快速模板生成，并采用数据驱动细化技术提升结构完整性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D人体化身动画分为规范模板构建和目标姿态变形两个阶段，但模板构建需要大量骨骼绑定且容易产生伪影，而目标姿态变形因线性混合蒙皮导致结构扭曲，严重影响动画真实感。

Method: 提出统一学习框架分两阶段处理：第一阶段使用U-Net架构在前馈过程中解耦纹理和姿态信息，实现快速人体模板生成；第二阶段提出数据驱动细化技术增强结构完整性。

Result: 大量实验表明，该模型在不同姿态下表现一致，在效率和质量之间达到最佳平衡，超越了现有最先进方法。

Conclusion: 提出的统一学习框架有效解决了3D人体化身动画中的模板构建和目标姿态变形问题，实现了高效且高质量的动画生成。
...
### [185] [CourtMotion: Learning Event-Driven Motion Representations from Skeletal Data for Basketball](https://arxiv.org/abs/2512.01478)
*Omer Sela,Michael Chertok,Lior Wolf*

Main category: cs.CV

TL;DR: CourtMotion是一个用于分析和预测职业篮球比赛中时空事件的框架，通过骨骼追踪数据和图神经网络捕捉球员运动模式，结合Transformer建模球员交互，显著提升了轨迹预测和篮球分析任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于球员位置的方法无法捕捉身体朝向、防守姿态、投篮准备动作等关键指标，而这些对于理解篮球比赛的语义意义至关重要。需要一种能够同时理解物理运动模式及其战术目的的方法。

Method: 采用两阶段方法：第一阶段使用图神经网络处理骨骼追踪数据捕捉细微运动模式；第二阶段使用具有专门注意力机制的Transformer架构建模球员交互。引入事件投影头将球员动作与传球、投篮、抢断等篮球事件明确关联。

Result: 在NBA追踪数据上的实验显示：相比最先进的基于位置的模型，轨迹预测误差减少35%；在关键篮球分析任务上持续性能提升，包括掩护检测、投篮者识别、助攻预测、投篮位置分类和投篮类型识别等下游任务。

Conclusion: CourtMotion框架通过结合骨骼追踪数据和语义理解，显著提升了篮球事件预测和分析能力，预训练模型可作为多个下游任务的强大基础，为篮球分析提供了更全面的时空建模方法。
...
### [186] [ChronosObserver: Taming 4D World with Hyperspace Diffusion Sampling](https://arxiv.org/abs/2512.01481)
...
Method: 提出ChronosObserver方法，包括：1）世界状态超空间：表示4D世界场景的时空约束；2）超空间引导采样：利用超空间同步多个视角的扩散采样轨迹。该方法无需训练或微调扩散模型。

Result: 实验结果表明，该方法能够实现高保真、3D一致的时间同步多视角视频生成，无需对扩散模型进行训练或微调。

Conclusion: ChronosObserver提供了一种无需训练的方法，成功解决了4D世界场景中多视角视频生成的时空同步问题，为驯服4D世界提供了关键能力。
...
### [187] [A variational method for curve extraction with curvature-dependent energies](https://arxiv.org/abs/2512.01494)
...
<details>
  <summary>Details</summary>
Motivation: 需要一种能够自动从图像中提取曲线和1D结构的方法，特别是在给定端点列表的情况下，并且希望方法能够处理曲率相关的能量

Method: 基于变分方法和Smirnov向量场分解定理，采用双层最小化方法，通过离散化能量函数来提取曲线；进一步扩展到曲率相关能量，使用经典提升方法将曲线映射到位置-方向空间，并配备适当的子黎曼或Finsler度量

Result: 开发了一种主要无监督的曲线提取方法，能够处理端点列表之间的曲线提取，并能扩展到曲率相关的能量优化

Conclusion: 该方法提供了一种有效的曲线提取框架，结合了变分方法、向量场分解和几何提升技术，适用于图像分析和计算机视觉中的曲线检测任务
...
### [188] [ELVIS: Enhance Low-Light for Video Instance Segmentation in the Dark](https://arxiv.org/abs/2512.01495)
*Joanne Lin,Ruirui Lin,Yini Li,David Bull,Nantheera Anantrasirichai*

Main category: cs.CV

TL;DR: ELVIS框架通过合成低光视频管道、退化特征解耦和增强解码器，显著提升了低光视频实例分割性能


<details>
  <summary>Details</summary>
Motivation: 低光视频实例分割面临三大挑战：1) 缺乏大规模标注数据集；2) 现有合成管道难以建模时间退化；3) 现有VIS方法对低光退化不鲁棒，即使在低光数据上微调也表现不佳

Method: ELVIS框架包含三个核心组件：1) 无监督合成低光视频管道，建模空间和时间退化；2) 无需校准的退化特征合成网络(VDP-Net)；3) 增强解码器头，从内容特征中解耦退化特征

Result: 在合成的低光YouTube-VIS 2019数据集上，ELVIS将性能提升了高达+3.7AP

Conclusion: ELVIS通过有效的域适应方法，成功将最先进的VIS模型迁移到低光场景，解决了低光视频实例分割的关键挑战
...
### [189] [Semantic-aware Random Convolution and Source Matching for Domain Generalization in Medical Image Segmentation](https://arxiv.org/abs/2512.01510)
*Franz Thaler,Martin Urschler,Mateusz Kozinski,Matthias AF Gsell,Gernot Plank,Darko Stern*

Main category: cs.CV

TL;DR: SRCSM方法通过语义感知随机卷积增强源域多样性，并在测试时通过强度映射适应目标域，实现了医学图像分割的单源域泛化，在多个跨模态和跨中心任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像分割中的单源域泛化问题，即训练时仅使用单一域（如CT）数据，但需要模型能够直接泛化到不同域（如MR），而无需目标域数据或模型微调。

Method: 提出SRCSM方法：1）训练时使用语义感知随机卷积，根据标注标签对源图像不同区域进行差异化增强；2）测试时通过强度映射将目标域图像映射到源域风格，使目标域数据与训练域相似。

Result: 在腹部、全心和前列腺分割的多种跨模态和跨中心泛化设置中，SRCSM在绝大多数实验中超越了之前的域泛化技术，甚至在某些设置中达到了与域内基线相当的分割性能。

Conclusion: SRCSM是医学图像分割域泛化的新SOTA方法，能够有效缩小域间差距，在更具挑战性的跨扫描仪硬件和心脏时相设置中也展现出良好潜力。
...
### [190] [QuantumCanvas: A Multimodal Benchmark for Visual Learning of Atomic Interactions](https://arxiv.org/abs/2512.01519)
...
<details>
  <summary>Details</summary>
Motivation: 当前分子和材料机器学习模型大多缺乏物理可迁移性，它们拟合整个分子或晶体的相关性，而不是学习原子对之间的量子相互作用。然而，化学键、电荷重新分布、轨道杂化和电子耦合都源于这些定义多体系统中局域量子场的双体相互作用。

Method: 引入QuantumCanvas数据集，涵盖2,850个元素-元素对，每个对标注18个电子、热力学和几何性质，并配以十通道图像表示（包括l-和m-分辨的轨道密度、角场变换、共占据图和电荷密度投影）。这些物理基础的图像编码了空间、角和静电对称性，无需显式坐标。
...
Conclusion: 通过将轨道物理与基于视觉的表征学习相结合，QuantumCanvas为通过耦合视觉和数值模态学习可迁移的量子相互作用提供了原则性和可解释的基础。该数据集和模型实现已开源。
...
TL;DR: FlashVGGT提出了一种高效的3D重建方法，通过描述符注意力机制替代VGGT的全自注意力，显著降低了计算复杂度，能在保持精度的同时处理超长图像序列。
...
Method: 提出描述符注意力机制：将每帧的空间信息压缩为紧凑的描述符token集合，然后通过图像token与描述符之间的交叉注意力替代全局自注意力，并采用分块递归机制重用缓存描述符以实现在线长序列推理。

Result: FlashVGGT在重建精度上与VGGT相当，但推理时间仅为VGGT的9.3%（1000张图像），并能高效扩展到超过3000张图像的长序列。

Conclusion: FlashVGGT通过描述符注意力机制有效解决了3D重建中全自注意力的计算瓶颈，实现了高效且可扩展的长序列处理，为实时应用提供了可能。
...
### [192] [Toward Content-based Indexing and Retrieval of Head and Neck CT with Abscess Segmentation](https://arxiv.org/abs/2512.01589)
...
TL;DR: AbscessHeNe数据集包含4,926张增强CT切片，用于头颈部脓肿语义分割，最佳模型Dice系数0.39，显示任务挑战性大。


<details>
  <summary>Details</summary>
Motivation: 头颈部脓肿是可能危及生命的急性感染，需要准确影像检测和分割来支持诊断、治疗规划和手术干预。

Method: 创建AbscessHeNe数据集，包含临床确认的头颈部脓肿增强CT切片，评估CNN、Transformer和Mamba等先进分割架构建立性能基准。

Result: 最佳模型达到Dice相似系数0.39、交并比0.27和归一化表面距离0.67，表明任务具有挑战性，需要进一步研究。

Conclusion: AbscessHeNe数据集为头颈部脓肿分割提供了基准，支持未来基于内容的索引和检索系统开发，数据集将公开可用。
...
### [193] [Depth Matching Method Based on ShapeDTW for Oil-Based Mud Imager](https://arxiv.org/abs/2512.01611)
...
<details>
  <summary>Details</summary>
Motivation: 油基泥浆微电阻率成像仪采用交错设计的上下极板，即使在速度校正后，极板图像之间仍存在深度错位问题，需要更精确的匹配方法

Method: 使用Shape Dynamic Time Warping算法，结合一维方向梯度直方图（HOG1D）和原始信号作为形状描述符，构建形态敏感的距离矩阵，更好地保持序列间的结构相似性

Result: 现场测试表明，该方法能够精确对齐具有复杂纹理、深度偏移或局部缩放的图像，并为特征扩展提供了灵活框架

Conclusion: 基于ShapeDTW的深度匹配方法有效解决了油基泥浆微电阻率成像仪图像深度错位问题，具有高精度和可扩展性
...
### [194] [SPARK: Sim-ready Part-level Articulated Reconstruction with VLM Knowledge](https://arxiv.org/abs/2512.01629)
*Yumeng He,Ying Jiang,Jiayin Lu,Yin Yang,Chenfanfu Jiang*

Main category: cs.CV

TL;DR: SPARK：从单张RGB图像重建物理一致、具有运动学部件层次的铰接式3D物体的框架


<details>
  <summary>Details</summary>
Motivation: 铰接式3D物体对于具身AI、机器人和交互式场景理解至关重要，但目前创建仿真就绪的资产仍然劳动密集，需要专家对部件层次和运动结构进行建模。

Method: 首先利用视觉语言模型提取粗略的URDF参数并生成部件级参考图像；然后将部件图像指导和推断的结构图集成到生成扩散变换器中，合成一致的部件和完整形状；最后通过可微分前向运动学和可微分渲染优化关节类型、轴和原点。

Result: 广泛实验表明，SPARK能够跨不同类别生成高质量的仿真就绪铰接式资产，支持机器人操作和交互建模等下游应用。

Conclusion: SPARK框架能够从单张RGB图像重建物理一致的运动学部件级铰接式物体，解决了创建仿真就绪资产的劳动密集问题。
...
### [195] [Generative Editing in the Joint Vision-Language Space for Zero-Shot Composed Image Retrieval](https://arxiv.org/abs/2512.01636)
...
<details>
  <summary>Details</summary>
Motivation: 监督式组合图像检索方法需要昂贵的三元组标注，这促使了零样本解决方案的发展。现有基于文本中心或扩散的方法难以有效弥合视觉-语言模态差距，这是零样本CIR的核心挑战。

Method: 提出Fusion-Diff框架：1）在联合视觉-语言空间中引入多模态融合特征编辑策略，显著缩小模态差距；2）包含轻量级Control-Adapter，仅需在20万合成样本上进行微调即可实现高性能。

Result: 在标准CIR基准测试（CIRR、FashionIQ和CIRCO）上的广泛实验表明，Fusion-Diff显著优于先前的零样本方法。通过可视化融合的多模态表示增强了模型的可解释性。

Conclusion: Fusion-Diff通过创新的多模态融合特征编辑和高效的数据利用策略，有效解决了零样本组合图像检索中的模态对齐问题，在保持高数据效率的同时实现了最先进的性能。
...
### [196] [ViT$^3$: Unlocking Test-Time Training in Vision](https://arxiv.org/abs/2512.01643)
*Dongchen Han,Yining Li,Tianyu Li,Zixuan Cao,Ziming Wang,Jun Song,Yu Cheng,Bo Zheng,Gao Huang*

Main category: cs.CV

TL;DR: 本文系统研究了视觉序列建模中的测试时训练设计，提出了Vision Test-Time Training模型，在线性复杂度下实现了与优化视觉Transformer相当的性能。


<details>
  <summary>Details</summary>
Motivation: 测试时训练为高效序列建模提供了新方向，将注意力操作重构为在线学习问题，实现线性计算复杂度。然而，视觉TTT设计缺乏系统指导和实践原则，需要填补这一关键空白。

Method: 通过系统实证研究视觉TTT设计，从实验分析中提炼出6个实用见解，建立了有效的视觉TTT设计原则。基于这些发现构建了Vision Test-Time Training模型，实现线性复杂度和可并行计算。

Result: ViT³在图像分类、图像生成、目标检测和语义分割等视觉任务中，一致匹配或优于先进的线性复杂度模型，有效缩小了与高度优化视觉Transformer的差距。

Conclusion: 本研究为视觉TTT模型提供了系统指导和实用原则，提出的ViT³基准模型将促进未来视觉TTT模型的研究发展。
...
### [197] [DB-KAUNet: An Adaptive Dual Branch Kolmogorov-Arnold UNet for Retinal Vessel Segmentation](https://arxiv.org/abs/2512.01657)
*Hongyu Xu,Panpan Meng,Meng Wang,Dayu Hu,Liming Liang,Xiaoqi Sheng*

Main category: cs.CV

TL;DR: 提出DB-KAUNet用于视网膜血管分割，结合CNN和Transformer双分支编码器，引入KANConv和KAT块，通过CCI和SFE-GAF模块优化特征处理，在多个数据集上取得领先性能。
...
Method: 提出自适应双分支Kolmogorov-Arnold UNet (DB-KAUNet)，包含异构双分支编码器(HDBE)，并行CNN和Transformer路径，集成KANConv和KAT块，采用CCI模块进行通道特征交互，以及SFE-GAF模块进行空间特征增强和自适应融合。

Result: 在DRIVE、STARE和CHASE_DB1数据集上的实验验证了DB-KAUNet取得了领先的分割性能，并展现出卓越的鲁棒性。
...
### [198] [Bridging the Scale Gap: Balanced Tiny and General Object Detection in Remote Sensing Imagery](https://arxiv.org/abs/2512.01665)
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像中密集微小目标与大型目标共存时，现有方法难以实现跨尺度的均衡检测性能。虽然大型基础模型在通用视觉任务上取得突破，但由于遥感图像中极端的尺度变化和密度分布，其在微小目标检测中的应用尚未探索。

Method: 提出ScaleBridge-Det框架，包含两个核心模块：1）路由增强混合注意力（REM）模块，通过自适应路由动态选择和融合尺度特定的专家特征，生成互补且具判别性的多尺度表示；2）密度引导动态查询（DGQ）模块，预测目标密度并自适应调整查询位置和数量，实现不同尺度目标的高效资源分配。

Result: 在AI-TOD-V2和DTOD基准数据集上达到最先进性能，同时在VisDrone跨域数据集上表现出优越的跨域鲁棒性，能够同时优化密集微小目标和一般目标的检测性能而无需权衡。

Conclusion: ScaleBridge-Det是首个针对遥感图像微小目标检测的大型检测框架，通过创新的尺度自适应专家路由和密度引导查询分配机制，成功解决了跨尺度目标检测的挑战，在多个数据集上实现了均衡且优越的检测性能。
...
### [199] [GRASP: Guided Residual Adapters with Sample-wise Partitioning](https://arxiv.org/abs/2512.01675)
*Felix Nützel,Mischa Dombrowski,Bernhard Kainz*

Main category: cs.CV

TL;DR: GRASP：通过样本划分和残差适配器解决扩散模型在长尾数据（如医学影像）中的模式崩溃问题，提升罕见类别的生成质量和多样性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在长尾场景（如医学影像）中表现不佳，罕见病理数据稀少导致模式崩溃——尾部类别输出质量差、多样性低，这削弱了合成数据增强对罕见病症的价值。

Method: 提出GRASP：引导残差适配器与样本划分。使用外部先验静态划分样本为最小化组内梯度冲突的簇，然后在预训练模型的transformer前馈层注入簇特定的残差适配器进行微调，绕过学习门控以实现稳定性和效率。

Result: 在长尾MIMIC-CXR-LT数据集上，GRASP在FID和多样性指标上表现优异，尤其对罕见类别，优于普通微调和混合专家变体等基线方法。在NIH-CXR-LT的下游分类任务中，尾部标签性能显著提升。在ImageNet-LT上的泛化验证了广泛适用性。

Conclusion: GRASP是一种轻量级、可扩展的方法，能有效解决扩散模型在长尾数据中的梯度冲突问题，提升罕见类别的生成质量，并易于集成到现有扩散管道中。
...
### [200] [Open-world Hand-Object Interaction Video Generation Based on Structure and Contact-aware Representation](https://arxiv.org/abs/2512.01677)
...
TL;DR: 提出SCAR方法，通过结构感知和接触感知的表示来生成逼真的手-物体交互视频，解决了2D与3D表示之间的权衡问题


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成手-物体交互视频时面临2D与3D表示的困境：2D表示可扩展但交互保真度低，3D表示保真度高但需要标注且可扩展性差。需要一种既能保证可扩展性又能保持交互保真度的表示方法

Method: 提出结构感知和接触感知的表示(SCAR)，捕捉手-物体接触、遮挡和整体结构上下文，无需3D标注。采用联合生成范式，通过共享-专业化策略同时生成交互导向的表示和视频

Result: 在两个真实世界数据集上优于现有方法，生成物理真实且时间一致的手-物体交互视频。在具有挑战性的开放世界场景中表现出强大的泛化能力

Conclusion: SCAR表示解决了2D与3D表示的权衡问题，通过可扩展的交互导向监督信号实现了逼真的手-物体交互视频生成，在开放世界场景中具有良好的泛化能力
...
### [201] [Cross-Domain Validation of a Resection-Trained Self-Supervised Model on Multicentre Mesothelioma Biopsies](https://arxiv.org/abs/2512.01681)
*Farzaneh Seyedshahi,Francesca Damiola,Sylvie Lantuejoul,Ke Yuan,John Le Quesne*

Main category: cs.CV

TL;DR: 基于自监督编码器的AI模型，利用切除组织训练后应用于活检样本，可预测间皮瘤患者生存期和亚型分类


<details>
  <summary>Details</summary>
Motivation: 间皮瘤的准确亚型分类和预后预测对指导治疗至关重要。现有计算病理模型大多基于大块切除组织训练，难以应用于临床常见的小活检样本

Method: 使用切除组织训练自监督编码器，然后将该编码器应用于活检材料，捕获有意义的形态学模式，用于预测患者生存和肿瘤亚型分类

Result: 模型能够从活检样本中捕获有意义的形态学模式，成功预测患者生存期并分类肿瘤亚型

Conclusion: 该方法展示了AI驱动工具在支持间皮瘤诊断和治疗规划方面的潜力，解决了临床实践中活检样本应用的限制
...
### [202] [DreamingComics: A Story Visualization Pipeline via Subject and Layout Customized Generation using Video Models](https://arxiv.org/abs/2512.01686)
*Patrick Kwon,Chen Chen*

Main category: cs.CV

TL;DR: DreamingComics是一个布局感知的故事可视化框架，通过改进的扩散变换器模型和区域感知位置编码，解决了现有方法在艺术一致性和角色定位方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前故事可视化方法仅依赖文本定位角色，难以保持艺术一致性，需要更有效的布局控制和角色一致性方法。

Method: 基于预训练视频扩散变换器模型，提出RegionalRoPE区域感知位置编码方案，使用掩码条件损失约束视觉特征，并集成LLM布局生成器从自然语言脚本推断漫画风格布局。

Result: 相比先前方法，角色一致性提升29.2%，风格相似度提升36.2%，同时展现出高空间准确性。

Conclusion: DreamingComics框架通过布局感知控制和一致性增强技术，显著提升了故事可视化的艺术质量和角色定位准确性。
...
### [203] [SSR: Semantic and Spatial Rectification for CLIP-based Weakly Supervised Segmentation](https://arxiv.org/abs/2512.01701)
*Xiuli Bi,Die Xiao,Junchao Fan,Bin Xiao*

Main category: cs.CV

TL;DR: 本文提出语义与空间校正(SSR)方法，通过跨模态原型对齐(CMPA)和超像素引导校正(SGC)解决CLIP在弱监督语义分割中的过激活问题，在PASCAL VOC和MS COCO数据集上取得最佳性能。
...
Method: 提出语义与空间校正(SSR)方法，包含两个核心组件：1) 跨模态原型对齐(CMPA)：通过对比学习机制强制跨模态特征对齐，减少类间重叠，增强语义相关性；2) 超像素引导校正(SGC)：利用超像素空间先验在亲和力传播中精确过滤非目标区域干扰。

Result: 在PASCAL VOC数据集上达到79.5% mIoU，在MS COCO数据集上达到50.6% mIoU，优于所有单阶段方法和更复杂的多阶段方法。

Conclusion: SSR方法有效解决了CLIP在弱监督语义分割中的过激活问题，通过语义和空间层面的双重校正显著提升了分割性能，为CLIP在视觉任务中的应用提供了新思路。
...
### [204] [FreqEdit: Preserving High-Frequency Features for Robust Multi-Turn Image Editing](https://arxiv.org/abs/2512.01755)
*Yucheng Liao,Jiajun Liang,Kaiqian Cui,Baoquan Zhao,Haoran Xie,Wei Liu,Qing Li,Xudong Mao*

Main category: cs.CV

TL;DR: FreqEdit是一个无需训练的框架，通过高频特征注入、自适应注入策略和路径补偿机制，解决多轮图像编辑中的高频信息丢失问题，实现稳定编辑。


<details>
  <summary>Details</summary>
Motivation: 当前基于指令的图像编辑模型在单次编辑中表现优秀，但在多轮编辑时会出现严重的质量退化问题。研究发现高频信息的渐进式丢失是导致质量退化的主要原因。

Method: FreqEdit包含三个协同组件：1）从参考速度场注入高频特征以保留细节；2）自适应注入策略，空间调制注入强度以实现精确的区域控制；3）路径补偿机制，定期重新校准编辑轨迹以防止过度约束。

Result: 实验表明，FreqEdit在10+次连续编辑迭代中表现稳定，在身份保持和指令遵循方面优于7个最先进的基线方法。

Conclusion: FreqEdit通过解决高频信息丢失问题，为多轮图像编辑提供了稳定有效的解决方案，显著提升了多轮编辑的质量和稳定性。
...
### [205] [HiconAgent: History Context-aware Policy Optimization for GUI Agents](https://arxiv.org/abs/2512.01763)
*Xurui Zhou,Gongwei Chen,Yuquan Xie,Zaijing Li,Kaiwen Zhou,Shuai Wang,Shuo Yang,Zhuotao Tian,Rui Shao*

Main category: cs.CV

TL;DR: HiconAgent：一种通过历史上下文感知策略优化（HCPO）训练的高效GUI代理，使用动态上下文采样和锚定引导的历史压缩来优化历史信息利用，在保持高性能的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: GUI代理需要有效利用历史上下文来执行顺序导航任务，但直接使用完整历史会导致计算开销过大和无关信息干扰，需要一种高效且有效的历史信息利用方法。

Method: 提出历史上下文感知策略优化（HCPO），包含两个组件：1）动态上下文采样（DCS）在采样时提供可变长度的历史上下文；2）锚定引导的历史压缩（AHC）在策略更新阶段采用双分支策略，压缩分支移除历史观察但保留历史动作作为信息流锚点，通过历史增强对齐损失耦合两个分支。

Result: 在主流GUI导航基准测试中表现优异：HiconAgent-3B在GUI-Odyssey上比GUI-R1-7B提升8.46%的grounding准确率和11.32%的步骤成功率，在AndroidControl和AITW上取得可比结果，同时实现最高2.47倍计算加速和60%的FLOPs减少。

Conclusion: HiconAgent通过HCPO框架实现了高效且有效的历史上下文利用，在保持高性能的同时显著降低了计算开销，为GUI代理的历史信息管理提供了有效解决方案。
...
### [206] [VideoScoop: A Non-Traditional Domain-Independent Framework For Video Analysis](https://arxiv.org/abs/2512.01769)
*Hafsa Billah*
...
<details>
  <summary>Details</summary>
Motivation: 当前视频情境分析要么依赖人工（易出错且劳动密集），要么使用特定算法（缺乏通用性），需要开发通用框架来解决这些限制

Method: 使用先进视频内容提取技术，采用扩展关系模型(R++)和图形模型两种表示方式，支持连续查询处理和图形算法检测，通过参数化模板实现领域独立性

Result: 在辅助生活、市政监控和一般监控三个领域进行了广泛实验，评估了准确性、效率和鲁棒性

Conclusion: 提出的通用VSA框架克服了现有限制，结合关系模型和图形模型，支持跨领域情境分析，具有实际应用价值
...
<details>
  <summary>Details</summary>
Motivation: 传统医学图像配准方法在处理对比度差异、空间变形和多模态变化时存在困难，需要更有效的特征提取方法来改善配准性能。

Method: 集成可学习边缘核与学习式刚性/非刚性配准技术，从预定义边缘检测核开始，加入随机噪声扰动，在训练中学习提取任务特定的最优边缘特征。

Result: 在医学大学数据集（三种设置：带/不带颅骨移除的刚性配准、非刚性配准）和两个公开数据集上，方法均优于现有技术。

Conclusion: 提出的自适应边缘检测方法能有效提升多模态医学图像配准性能，改善解剖结构分析，具有临床应用潜力。
...
TL;DR: SAM2扩展用于视频语义分割，通过两种方法：1) 提取对象掩码并与分割网络结合；2) 提取特征向量进行分类，最终结合分类和掩码。实验表明SAM2能提升VSS性能，主要得益于其精确的对象边界预测。


<details>
  <summary>Details</summary>
Motivation: 虽然SAM2在图像和视频对象分割中表现出色，但将其扩展到密集视频语义分割(VSS)面临挑战，需要空间准确性、时间一致性以及处理多对象复杂边界和尺度变化的能力。

Method: 提出两种方法：1) 使用SAM2从图像中提取唯一对象掩码，同时使用分割网络生成和细化初始预测；2) 使用预测掩码提取特征向量，输入简单网络进行分类，最后结合分类和掩码生成最终分割。

Result: 实验表明，利用SAM2能够提升VSS的整体性能，主要得益于其精确的对象边界预测能力。

Conclusion: SAM2可以成功扩展到视频语义分割任务，其精确的对象边界预测能力对提升VSS性能有重要贡献。
...
TL;DR: 评估学习压缩算法在卫星地球观测数据压缩中的表现，与传统JPEG 2000对比，在光学图像上学习压缩表现更好，但在热红外数据上传统方法仍有优势
...
Method: 比较传统压缩（JPEG 2000）与学习压缩方法（离散混合高斯似然）在三个EO分割任务上：火灾、云和建筑物检测

Result: 学习压缩在大规模多通道光学图像上显著优于JPEG 2000（重建质量和分割精度），但在小型单通道热红外数据集上传统编码器仍有竞争力；联合端到端优化并未比单独优化带来性能提升

Conclusion: 学习压缩在EO数据压缩中具有潜力，特别是在大规模光学图像上，但需要根据数据类型和规模选择合适的压缩方法
...
TL;DR: SAM3-UNet是Segment Anything Model 3的简化变体，通过添加适配器和轻量级U-Net解码器，以低成本适应下游分割任务，在多个任务上表现优于SAM2-UNet和其他SOTA方法，训练时GPU内存需求低于6GB。


<details>
  <summary>Details</summary>
Motivation: 为了以低成本将强大的SAM3模型适配到下游分割任务中，需要开发一个参数高效的微调方案，减少计算资源需求的同时保持高性能。

Method: 提出SAM3-UNet架构，包含三个组件：1) SAM3图像编码器；2) 简单的参数高效微调适配器；3) 轻量级U-Net风格解码器。通过这种设计实现低成本的下游任务适应。

Result: 在镜像检测和显著性目标检测等多个任务上的初步实验表明，SAM3-UNet优于之前的SAM2-UNet和其他最先进方法，在批量大小为12的训练中GPU内存需求低于6GB。

Conclusion: SAM3-UNet成功地将SAM3适配到下游分割任务中，以较低的计算成本实现了优异的性能，代码已开源供社区使用。
...
TL;DR: 提出一种基于真实人体动作学习潜在空间的新评估指标，用于评估生成视频中复杂人体动作的视觉和时间正确性，相比现有方法提升68%以上。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型缺乏评估复杂人体动作视觉和时间正确性的鲁棒指标。现有的纯视觉编码器和多模态大语言模型存在外观偏见、缺乏时间理解能力，难以识别生成视频中复杂的运动动态和解剖学不合理性。

Method: 方法首先通过融合外观无关的人体骨骼几何特征和基于外观的特征，捕捉真实世界运动的细微差别、约束和时间平滑性。然后通过测量生成视频底层表示与学习到的真实世界动作分布之间的距离，量化其动作质量。

Result: 在新设计的多层面基准测试中，该指标相比现有最先进方法实现了超过68%的显著改进；在现有外部基准测试中表现具有竞争力；与人类感知有更强的相关性。

Conclusion: 该研究揭示了当前视频生成模型的关键局限性，为视频生成领域的高级研究建立了新标准，提供了一种更准确评估人体动作真实性的方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的空间推理能力不足，主要依赖基于文本描述的调优方法，存在"视觉文盲"问题——仅通过文本符号学习空间概念，缺乏与视觉表现的连接。

Method: 提出MILO隐式空间世界建模范式，集成视觉生成器提供几何感知反馈；设计RePE相对位置编码方案捕捉相对相机姿态变换；构建GeoGen大规模几何感知生成数据集（2,241个视频，67,827个观察-动作-结果三元组）。

Result: 实验表明该方法在多个基线和基准测试中显著提升了空间推理能力，提供了对3D空间更全面的理解。

Conclusion: MILO通过模拟人类空间想象，将符号推理隐式地建立在感知经验基础上，为解决多模态大语言模型的空间推理瓶颈提供了有效方案。
...
TL;DR: 论文提出视觉因果发现任务，构建VCG-32K数据集，开发CauSight模型，通过因果感知推理在视觉场景中发现实体级因果关系，性能显著超越GPT-4.1。


<details>
  <summary>Details</summary>
Motivation: 人类具备因果思维能力，能够理解现象背后的原因。为了让现代AI系统复制这种能力，需要让模型不仅能感知视觉实体的存在，还能推断它们之间的因果关系，因此提出了视觉因果发现任务。

Method: 1) 构建VCG-32K数据集，包含32,000多张带有实体级因果图标注的图像；2) 开发CauSight视觉语言模型，采用三部分训练策略：从VCG-32K进行训练数据整理、使用因果思维树(ToCT)合成推理轨迹、通过设计的因果奖励进行强化学习来优化推理策略。

Result: CauSight在视觉因果发现任务上显著优于GPT-4.1，实现了超过三倍的性能提升（21%的绝对增益）。

Conclusion: 该研究成功开发了视觉因果发现任务和相应解决方案，CauSight模型展示了强大的因果推理能力，为AI系统理解视觉场景中的因果关系提供了有效途径。代码、模型和数据集均已开源。
...
TL;DR: OpenREAD是一个端到端强化微调的视觉语言模型自动驾驶框架，通过大规模思维链标注和LLM作为评判器来量化推理质量，在推理和规划任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前两阶段微调方法存在局限性：监督微调(SFT)的推理泛化能力有限，而强化微调(RFT)主要应用于下游任务，因为场景理解是开放性问题难以量化奖励。需要一种端到端的RFT框架来提升从高层推理到低层轨迹规划的完整自动驾驶性能。

Method: 1) 在开源驾驶知识数据集上构建大规模思维链(CoT)标注；2) 使用强大的Qwen3大语言模型作为RFT中的评判器，为开放性问题量化推理质量；3) 实施端到端的联合强化微调，覆盖从高层推理到低层轨迹规划的完整谱系。

Result: 广泛的实验证实，联合端到端RFT在上游和下游任务中都带来了显著改进，使OpenREAD在推理和规划基准测试中达到了最先进的性能。

Conclusion: OpenREAD通过端到端强化微调和LLM驱动的奖励建模，有效解决了自动驾驶中开放性问题推理质量量化难题，为知识驱动的自动驾驶范式提供了更强大的框架。
...
TL;DR: 论文提出了PhyDetEx方法，通过构建物理不可行检测数据集(PID)和微调视觉语言模型，来评估文本到视频生成模型对物理定律的遵循程度。


<details>
  <summary>Details</summary>
Motivation: 虽然文本到视频生成模型在视频质量和长度方面取得了进展，但它们是否能够理解物理并生成物理上合理的视频仍然是个问题。现有的视觉语言模型难以识别生成视频中的物理不可能内容。

Method: 构建了PID数据集（包含500个手动标注的测试视频和2,588个配对视频的训练集），通过重写真实视频的标题来诱导T2V模型生成物理不可行的内容。然后采用轻量级微调方法，使视觉语言模型能够检测物理不可行事件并生成违反物理原理的文本解释。

Result: 开发了PhyDetEx物理合理性检测器和解释器，对一系列最先进的T2V模型进行了基准测试。结果显示，虽然最近的T2V模型在生成物理合理内容方面取得了显著进展，但理解和遵循物理定律仍然是一个挑战，特别是对于开源模型。

Conclusion: 文本到视频生成模型在物理合理性方面仍有改进空间，PhyDetEx提供了一个有效的评估框架和工具，有助于推动更符合物理定律的视频生成模型的发展。
...
TL;DR: 提出一种将点云配准视为条件生成任务的新方法，通过学习连续的点级速度场将噪声点传输到配准场景，直接生成配准后的点云。


<details>
  <summary>Details</summary>
Motivation: 传统点云配准方法通常通过对应点匹配估计点云对之间的变换，然后优化成对变换实现多视图配准，这种方法在低重叠情况下效果不佳。本文希望直接生成配准点云，避免复杂的对应点匹配和优化过程。

Method: 将配准建模为条件生成问题：学习一个连续的点级速度场，将噪声点传输到配准场景；使用轻量级局部特征提取器和测试时刚性约束；直接生成配准点云而非估计变换。

Result: 在成对和多视图配准基准测试中达到最先进水平，特别是在低重叠情况下表现优异；能够跨尺度和传感器模态泛化；支持重定位、多机器人SLAM和多会话地图融合等下游任务。

Conclusion: 将点云配准重新定义为条件生成任务是一种有效方法，通过学习速度场直接生成配准点云，避免了传统对应点匹配的复杂性，在多种场景下表现出优越性能并支持多种下游应用。
...
TL;DR: 提出一个可重构的多智能体系统作为体育视频理解的基础框架，通过专门的认知工具代理实现从微观动作到宏观策略的层次化分析，解决现有端到端模型泛化性差、开发成本高和可解释性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 智能体育视频分析需要理解从微观动作到宏观策略的完整时间上下文，但现有端到端模型存在三个主要问题：缺乏泛化性、新任务开发成本高、可解释性差。需要一种能够处理时间层次结构、灵活适应不同任务且易于解释的框架。

Method: 提出可重构的多智能体系统框架，每个代理作为专门的"认知工具"负责特定分析方面。通过迭代调用和灵活组合这些代理，系统可以构建自适应管道，既能处理短期分析推理（如回合问答），也能处理长期生成性总结（如比赛总结）。

Result: 在羽毛球分析的两个代表性任务中展示了该框架的适应性，证明其能够桥接细粒度事件检测和全局语义组织，实现了灵活、可扩展和可解释的跨任务体育视频智能分析。

Conclusion: 这项工作提出了向灵活、可扩展和可解释系统的范式转变，为鲁棒的跨任务体育视频智能提供了基础框架，通过多智能体系统实现了从微观到宏观的时间层次理解。
...
<details>
  <summary>Details</summary>
Motivation: 现有细胞追踪方法主要针对单一恒定信号的视频，无法检测细胞死亡等关键事件，且不适用于随时间波动的瞬态荧光信号（如细胞节律）。需要开发能处理多通道瞬态信号并识别关键细胞事件的追踪方法。

Method: 采用轻量级深度学习框架，直接在细胞检测嵌入上进行匹配，无需量化追踪特异性特征。整合Transformer网络、使用所有检测框的多阶段匹配，以及用卡尔曼滤波器插值缺失的轨迹片段。

Result: 该统一框架在不同条件下表现出色，能有效追踪细胞并捕捉细胞分裂和死亡事件。已成功应用于分析化疗药物在单细胞水平的疗效，展示了其在癌症细胞动力学定量研究中的潜力。

Conclusion: TransientTrack能够处理具有瞬态信号的多通道显微镜视频，识别关键细胞事件，为癌症细胞动力学和治疗响应的定量研究提供了有力工具，可促进对治疗抵抗机制的详细表征。
...
TL;DR: KM-ViPE是一个实时开放词汇SLAM框架，使用未校准的单目相机在动态环境中运行，无需深度传感器和离线校准，通过结合DINO视觉特征与几何约束实现同时定位和语义建图。


<details>
  <summary>Details</summary>
Motivation: 现有SLAM系统通常需要深度传感器、离线校准，或者缺乏对动态场景的鲁棒性。KM-ViPE旨在解决这些问题，使系统能够在未校准的单目相机上实时运行，并处理动态环境，适用于自主机器人和AR/VR应用。

Method: KM-ViPE将DINO视觉特征与几何约束紧密耦合，通过基于高级特征的自适应鲁棒核处理移动物体和可移动静态物体。系统融合几何特征和深度视觉特征，并与语言嵌入对齐，实现同时在线定位和开放词汇语义建图。

Result: KM-ViPE在性能上与最先进方法竞争，同时具备在线操作、未校准单目输入和动态场景鲁棒处理的独特组合优势。系统受益于互联网规模训练，适用于自主机器人和AR/VR应用。

Conclusion: KM-ViPE通过结合在线操作、未校准单目输入和动态场景鲁棒处理，推进了具身AI的实际空间智能能力，为自主机器人和AR/VR应用提供了良好的解决方案。
...
TL;DR: StyleYourSmile：一种无需多风格配对数据的单次跨域人脸重定向方法，通过双编码器框架和扩散模型实现身份、表情和风格属性的解耦控制


<details>
  <summary>Details</summary>
Motivation: 现有跨域人脸重定向方法存在局限性：要么无法跨域泛化，要么需要测试时优化，要么需要精心策划的多风格配对数据集来获取域不变的身份表示。需要一种更高效、无需多风格配对数据的解决方案。

Method: 提出高效数据增强策略和双编码器框架，分别提取域不变身份线索和捕获域特定风格变化。利用这些解耦的控制信号，通过扩散模型实现跨域面部表情重定向。

Result: 在广泛视觉域上实现了优越的身份保持和重定向保真度，相比现有方法表现更佳。

Conclusion: StyleYourSmile成功解决了跨域人脸重定向中的关键挑战，无需多风格配对数据即可实现高质量的身份保持和表情迁移，为跨域人脸编辑提供了有效解决方案。
...
TL;DR: SARL：一种空间感知的自监督学习框架，通过三个地图级目标增强BYOL架构，在融合视觉-触觉数据上实现更好的机器人感知性能


<details>
  <summary>Details</summary>
Motivation: 机器人操作需要编码局部几何的表示。视觉提供全局上下文但缺乏纹理、硬度等直接测量，而触觉提供这些线索。现有的自监督学习框架将特征图压缩为全局向量，丢弃了空间结构，与操作任务需求不匹配。

Method: 提出SARL框架，在BYOL架构基础上增加三个地图级目标：显著性对齐(SAL)、补丁原型分布对齐(PPDA)和区域亲和匹配(RAM)，保持注意力焦点、部件组成和几何关系在不同视图间一致。

Result: 在6个下游任务中持续优于9个自监督学习基线。在几何敏感的边姿态回归任务上，MAE达到0.3955，比次优方法(0.5682 MAE)相对提升30%，接近监督学习上限。

Conclusion: 对于融合视觉-触觉数据，最有效的信号是结构化空间等变性，其中特征随对象几何可预测地变化，这能实现更强大的机器人感知能力。
...
TL;DR: Med-VCD是一种稀疏视觉对比解码方法，通过动态选择视觉信息标记来减少医疗大型视觉语言模型的幻觉输出，无需二次解码的时间开销，在多个医疗数据集上显著提升事实准确性和减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 医疗大型视觉语言模型在医疗应用中容易产生看似合理但实际错误的幻觉输出。现有方法大多依赖二次解码或回滚过程，显著降低推理速度，且通常是领域特定的，可能引入模态间或生成内容与真实内容之间的不对齐。

Method: Med-VCD是一种稀疏视觉对比解码方法，包含新颖的标记稀疏化策略，能够动态选择视觉信息标记，在保留关键视觉上下文的同时减少冗余，平衡效率与可靠性。

Result: 在涵盖眼科、放射学和病理学的8个医疗数据集上评估，包括视觉问答、报告生成和专门的幻觉基准测试，Med-VCD将事实准确性平均提高了13%，幻觉准确性提高了6%。

Conclusion: Med-VCD有效缓解了医疗大型视觉语言模型的幻觉问题，无需二次解码的时间开销，在多个医疗任务中显著提升了模型的可靠性和准确性。
...
TL;DR: AdvTraj：首个针对检测跟踪式MOT的在线物理ID操控攻击，通过对抗性轨迹将攻击者ID转移至目标物体，无需攻击目标检测模块


<details>
  <summary>Details</summary>
Motivation: 多目标跟踪算法在安全关键应用中至关重要，但现有攻击研究不足。先前攻击要么针对单个物体跟踪器，要么通过攻击目标检测模块来操控ID，存在模型特定、非鲁棒、仅影响离线数据等局限

Method: 提出AdvTraj攻击方法：攻击者使用对抗性轨迹将其ID转移至目标物体以混淆跟踪系统。在CARLA模拟器中实现，分析对抗轨迹模式并提出两种可由人类在日常生活场景中执行的通用对抗机动

Result: 白盒攻击SORT时，在各种场景下ID分配欺骗成功率100%；对SOTA MOT算法具有高攻击可迁移性（攻击成功率高达93%）。揭示了SOTA MOT系统在物体关联阶段的未充分探索的弱点

Conclusion: AdvTraj是首个针对检测跟踪式MOT的在线物理ID操控攻击，无需攻击目标检测模块。该工作揭示了SOTA MOT系统在物体关联阶段的弱点，为增强此类系统的鲁棒性提供了见解
...
TL;DR: Script是一种无需重新训练的即插即用剪枝方法，通过图结构剪枝模块去除视觉冗余token，结合查询条件语义剪枝模块保留查询相关视觉信息，在多种MLLMs上实现高效推理并保持性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中视觉token的快速增长导致内存消耗和推理延迟过高，特别是在处理高分辨率图像和视频时。现有的token剪枝方法往往忽略用户查询相关性或受注意力机制限制，降低了适应性和有效性。

Method: 提出Script方法，包含两个模块：1) 图结构剪枝模块去除视觉冗余token；2) 查询条件语义剪枝模块保留查询相关视觉信息。该方法无需重新训练，可跨多种MLLMs通用。

Result: 在14个图像和视频理解基准测试中，Script相比现有剪枝方法在模型效率和预测准确性方面表现更优。在LLaVA-NeXT-7B上实现高达6.8倍的预填充加速和10倍的FLOP减少，同时保留96.88%的原始性能。

Conclusion: Script是一种有效的即插即用token剪枝方法，能够显著提升多模态大语言模型的推理效率，同时保持与用户查询相关的语义信息，在各种视觉理解任务中表现优异。
...
TL;DR: SpriteHand是一个自回归视频生成框架，能够实时合成各种手-物体交互视频，支持多种物体类型和运动模式，在单GPU上实现约18FPS的实时生成。


<details>
  <summary>Details</summary>
Motivation: 传统基于物理引擎的方法依赖明确定义的刚性物体模型和预设手势，无法有效处理与非刚性或铰接实体（如可变形织物、弹性材料、铰链结构、毛绒表面甚至生物）的动态交互。

Method: SpriteHand采用自回归视频生成框架，输入静态物体图像和手部交互视频流，通过因果推理架构进行自回归生成，并利用混合后训练方法增强视觉真实性和时间一致性。

Result: 1.3B参数模型在单张NVIDIA RTX 5090 GPU上实现约18FPS、640x368分辨率的实时流式生成，延迟约150毫秒，支持超过1分钟的连续输出，在视觉质量、物理合理性和交互保真度方面优于生成式和引擎基线方法。

Conclusion: SpriteHand能够实时合成多样化的手-物体交互视频，克服了传统物理引擎方法的局限性，为处理复杂手-物体交互提供了有效的生成式解决方案。
...
TL;DR: 提出SegCaptioning新任务：用简单提示（如边界框）生成多样化的（描述，掩码）对，通过场景图引导扩散模型实现相关掩码-描述预测
...
Method: 提出场景图引导扩散模型：1) 提示中心场景图适配器将用户提示映射到场景图；2) 场景图引导双模态Transformer通过扩散过程预测相关描述-掩码对；3) 多实体对比学习损失显式对齐视觉和文本实体

Result: 在两个数据集上的实验表明，SGDiff在SegCaptioning任务中取得优越性能，用最小提示输入在描述和分割任务上都获得有希望的结果

Conclusion: 提出的SegCaptioning任务和SGDiff方法能够从简单提示生成多样化且语义对齐的描述-掩码对，为用户提供灵活的结果选择
...
TL;DR: Artemis是一个感知策略学习框架，使用结构化基于提议的推理（标签-边界框对）代替纯语言中间推理，在视觉感知任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有基于纯语言中间推理的视觉感知策略框架在感知任务上性能下降，核心问题在于推理形式不匹配：语言推理在语义空间进行，而视觉感知需要在空间和物体中心空间进行推理

Method: 提出Artemis框架，采用结构化基于提议的推理，每个中间步骤表示为（标签，边界框）对，捕获可验证的视觉状态，支持中间状态显式跟踪和提议质量直接监督

Result: 基于Qwen2.5-VL-3B构建的Artemis在grounding和检测任务上表现强劲，在计数和几何感知任务上展现显著泛化能力，在通用MLLM基准测试中也取得竞争性性能

Conclusion: 将推理与空间表示对齐能增强感知策略学习，基于空间接地的推理为可扩展和通用的感知策略提供了原则性路径
...
<details>
  <summary>Details</summary>
Motivation: 物理AI需要能够感知和预测真实世界动态的模型，但当前多模态大语言模型和视频生成模型在这些能力上的表现尚不清楚，需要系统评估。

Method: 提出了PAI-Bench基准测试，包含2808个真实世界案例，使用任务对齐的指标来评估物理合理性和领域特定推理能力，涵盖视频生成、条件视频生成和视频理解三个任务。

Result: 视频生成模型虽然视觉保真度高，但难以保持物理一致的动态；多模态大语言模型在预测和因果解释方面表现有限。当前系统在满足物理AI的感知和预测需求方面仍处于早期阶段。

Conclusion: PAI-Bench为评估物理AI建立了现实基础，并指出了未来系统需要解决的关键差距，强调了物理一致性在AI系统中的重要性。
...
TL;DR: 本文提出音频-视觉可供性定位(AV-AG)新任务，通过动作声音分割物体交互区域，并构建首个AV-AG数据集和AVAGFormer模型。
...
Conclusion: 音频为可供性定位提供有效线索，AVAGFormer模型能有效融合音频视觉信息，在零样本泛化方面表现良好。代码和数据集已开源。
...
<details>
  <summary>Details</summary>
Motivation: 多视角相机系统能够提供复杂真实场景的丰富观测，理解多视角设置中的动态物体已成为各种应用的核心。然而，现有方法在多视角点跟踪方面存在局限性，需要更完整可靠的轨迹估计方法。

Method: MV-TAP利用相机几何和跨视角注意力机制，聚合跨视角的时空信息。该方法构建了大规模合成训练数据集和真实世界评估集，专门针对多视角跟踪任务进行优化。
...
TL;DR: AirSim360是一个用于空中视角360度全景数据生成的仿真平台，通过无人机实现大范围场景采样，支持几何、语义和实体级理解，包含交互式行人感知系统和自动轨迹生成，收集了6万+全景样本。


<details>
  <summary>Details</summary>
Motivation: 360度全方位理解领域缺乏大规模多样化数据，现有仿真器无法系统建模4D真实世界，需要专门针对空中全景视角的数据生成平台。

Method: 提出AirSim360仿真平台，包含三个关键方面：1）渲染对齐的数据和标注范式，支持像素级几何、语义和实体级理解；2）交互式行人感知系统，建模人类行为；3）自动轨迹生成范式，支持导航任务。

Result: 收集了超过60,000个全景样本，通过多个任务的广泛实验证明了仿真器的有效性，是首个系统建模4D真实世界在全方位设置下的工作。

Conclusion: AirSim360填补了360度全方位理解领域的数据空白，提供了首个系统建模4D真实世界的仿真平台，平台工具包、插件和数据集将公开可用。
...
TL;DR: 提出改进的MeanFlow（iMF）方法，解决原始MF框架在训练目标和引导机制上的问题，在ImageNet 256×256上实现1.72 FID（单步评估），显著超越同类方法


<details>
  <summary>Details</summary>
Motivation: 原始MeanFlow框架存在两个关键问题：1）训练目标不仅依赖于真实场还依赖于网络自身，导致训练不稳定；2）分类器无关引导在训练时固定尺度，牺牲了测试时的灵活性

Method: 1）将训练目标重新表述为瞬时速度v的损失，通过预测平均速度u的网络重新参数化；2）将引导机制公式化为显式条件变量，使用上下文条件处理不同条件，保持测试时灵活性

Result: iMF在ImageNet 256×256上达到1.72 FID（单步评估），显著优于同类方法，缩小了与多步方法的差距，且无需蒸馏

Conclusion: 改进的MeanFlow方法解决了原始框架的关键问题，为快速前向生成建模作为独立范式提供了重要进展
...
TL;DR: TUNA提出了一种原生统一多模态模型，通过级联VAE编码器和表示编码器构建统一的连续视觉表示空间，在理解和生成任务上都实现了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型通常使用解耦的表示方法，导致表示格式不匹配问题。TUNA旨在通过构建统一的连续视觉表示空间来解决这一问题，实现更好的多模态理解和生成性能。

Method: TUNA采用级联结构：VAE编码器+表示编码器，构建统一的连续视觉表示空间。该统一表示支持端到端的图像和视频处理，同时适用于理解和生成任务。使用更强的预训练表示编码器提升性能，并通过联合训练理解和生成数据实现任务间的相互促进。

Result: 在图像和视频理解、图像和视频生成、图像编辑等多个基准测试中取得最先进结果。相比解耦表示方法，TUNA在理解和生成任务上都有更好表现。更强的预训练表示编码器在所有多模态任务上都能带来性能提升。

Conclusion: TUNA的统一表示设计是有效且可扩展的，解决了表示格式不匹配问题。联合训练理解和生成任务可以实现相互促进而非干扰，为统一多模态模型的发展提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法在复杂物体运动下存在局限：I2V方法缺乏全场景上下文，V2V方法只能提供视角变化或基本平移，难以控制细粒度物体运动

Method: 采用轨迹条件化的V2V框架，通过3D点轨迹建立源视频和目标运动之间的稀疏对应关系，利用3D轨迹的深度信息解决遮挡和深度顺序问题，采用合成和真实数据的两阶段训练

Result: 支持多种运动编辑：相机/物体联合操控、运动转移、非刚性变形，在保持时空一致性的同时实现精确运动编辑

Conclusion: 提出的3D轨迹条件化框架为视频编辑解锁了新的创作潜力，能够处理复杂运动场景下的精确编辑需求
...
TL;DR: 视频生成器在模拟重力时存在系统性偏差，物体下落加速度明显偏慢。研究者通过无单位双物体协议排除尺度混淆因素，发现模型违反了伽利略等效原理。通过少量数据微调可显著改善物理模拟能力。


<details>
  <summary>Details</summary>
Motivation: 评估视频生成器作为世界模型的潜力，需要测试其对基本物理定律（如重力）的编码和理解能力。当前视频生成器在模拟重力时存在明显偏差，需要系统性地分析这些物理错误是否源于尺度混淆等问题。

Method: 1) 首先分析物理错误是否源于帧率假设等尺度混淆因素；2) 引入无单位双物体协议，测试与重力加速度g、焦距和尺度无关的时间平方比与高度比关系；3) 使用轻量级低秩适配器，仅用100个单球下落视频片段进行微调。

Result: 1) 视频生成器生成的物体下落加速度明显偏慢（geff=1.81 m/s²）；2) 相对测试揭示了伽利略等效原理的违反；3) 微调后加速度提升至6.43 m/s²（达到地球重力的65%）；4) 适配器能零样本泛化到双球下落和斜面场景。

Conclusion: 视频生成器在重力模拟方面存在系统性物理偏差，但通过针对性的少量数据微调可以显著改善特定物理定律的编码能力，为构建更准确的世界模型提供了可行路径。
...
TL;DR: 提出混合真实与虚拟数据生成管道，解决自驱动实验室中气泡检测训练数据稀缺问题，特别是负样本不足，实现99.6%准确率


<details>
  <summary>Details</summary>
Motivation: 自驱动实验室需要高精度模型，但训练所需的大规模标注数据（特别是负样本）在常规实践中难以获取，阻碍了生物科学中自动化工作流程的发展

Method: 构建混合管道融合真实和虚拟数据生成：真实轨道采用人机协同方案，结合自动采集和选择性人工验证；虚拟轨道使用参考条件提示引导的图像生成增强真实数据，并进行筛选验证，最终生成类别平衡数据集

Result: 在保留的真实测试集上，仅使用自动采集的真实图像训练的模型达到99.6%准确率；混合真实和生成数据训练维持99.4%准确率，同时显著减少了数据收集和审核工作量

Conclusion: 该方法为自驱动实验室工作流程提供了可扩展且经济高效的视觉反馈数据供应策略，为解决罕见事件检测和更广泛视觉任务中的数据稀缺问题提供了实用解决方案
...
TL;DR: 研究比较人类专家与开源大语言模型在归纳式编码中的表现，发现人类擅长复杂句子但易错简单句子，而LLMs则相反，且人类标注虽偏离标准但更受专家青睐


<details>
  <summary>Details</summary>
Motivation: 传统定性数据分析依赖预定义标签的演绎方法，本研究探索使用大语言模型自动化归纳式编码过程，让标签从数据中自然涌现，以评估LLMs在定性分析中的潜力

Method: 比较六个开源大语言模型与人类专家在归纳式编码任务中的表现，专家对编码句子的难度进行评分，系统分析人类和LLM生成标签与测试集黄金标准的偏差

Result: 发现有趣的二分现象：人类编码者在复杂句子标注上表现良好但在简单句子上易出错，而LLMs则相反；人类标注虽偏离黄金标准但更受其他专家好评，部分LLMs更接近真实标签却获得较低评价

Conclusion: 人类和LLMs在定性数据分析中各有优势和局限性，人类标注更符合人类偏好但可能偏离客观标准，LLMs在某些方面更准确但缺乏人类认可，暗示了人机协作的潜力
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM越来越多地部署在协作环境中，但人们对它们作为黑盒智能体时如何协调知之甚少。需要理解LLM在无明确角色提示下的自发协调行为。

Method: 在归纳编码任务中模拟7500次多智能体、多轮讨论，生成超过125000条话语。引入过程级指标：代码稳定性、语义自一致性、词汇置信度、情感和收敛度量。分析输出嵌入的几何演化，观察内在维度变化。

Result: LLM群体在词汇和语义上收敛，发展出不对称影响模式，尽管没有明确的角色提示，仍表现出类似协商的行为。输出嵌入的内在维度随轮次下降，表明语义压缩。

Conclusion: 黑盒交互分析能够揭示涌现的协调策略，为基于内部探针的可解释性方法提供了可扩展的补充。展示了LLM在协作任务中自发形成协调模式的能力。
...
TL;DR: 论文提出Tree Matching Networks (TMN)，基于依赖句法树结构进行自然语言推理，相比BERT等序列模型在参数效率、训练时间和内存占用方面有显著优势，但当前聚合方法限制了可扩展性。


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的NLI模型（如BERT）虽然准确率高，但需要数亿参数，计算成本高。这些模型从零开始学习单词间关系，而显式利用依赖句法树等语言学结构可能利用先验知识，提高学习效率。

Method: 将Graph Matching Networks (GMN)适配到依赖句法树上，创建Tree Matching Networks (TMN)。在SNLI蕴含任务和SemEval相似性任务上比较TMN与BERT模型。提出多头注意力聚合方法来解决当前聚合方法的可扩展性限制。

Result: TMN在SNLI任务上比BERT模型取得显著更好的结果，同时内存占用大幅减少，训练时间更短。两种模型在SemEval任务上都表现不佳。显式结构表示在可比规模下显著优于序列模型，但当前聚合方法限制了可扩展性。

Conclusion: 利用依赖句法树等显式语言学结构可以显著提高NLI任务的效率和性能，但需要改进聚合方法以实现更好的可扩展性。提出的多头注意力聚合是解决这一限制的有前景的方向。
...
TL;DR: LLM智能体框架用于基于语料库的语法分析，通过自然语言任务解释、代码生成和数据驱动推理，在UD语料库上测试多语言语法任务，评估了13个词序特征和170多种语言。


<details>
  <summary>Details</summary>
Motivation: 经验语法研究越来越数据驱动，但系统分析标注语料库仍需要大量方法和技术努力。探索如何利用智能大语言模型简化这一过程，通过推理标注语料库产生可解释、数据基础的语法问题答案。

Method: 引入基于语料库的语法分析智能体框架，集成自然语言任务解释、代码生成和数据驱动推理概念。作为概念验证，应用于Universal Dependencies语料库，测试受世界语言结构图谱启发的多语言语法任务。

Result: 评估涵盖13个词序特征和超过170种语言，从三个互补维度评估系统性能：主导顺序准确性、顺序覆盖完整性和分布保真度。结果展示了将LLM推理与结构化语言数据结合的可行性。

Conclusion: 该研究为实现可解释、可扩展的基于语料库的语法查询自动化迈出了第一步，证明了LLM推理与结构化语言数据结合的可能性。
...
TL;DR: 该论文提出了一种无需数据增强的印度语言语法纠错方法，使用指令调优的大型语言模型和保守解码策略，在马来语和印地语上取得了优异成绩。


<details>
  <summary>Details</summary>
Motivation: 印度语言的语法纠错面临监督数据有限、文字多样、形态丰富等挑战，需要开发高效且可复现的解决方案。

Method: 使用12B参数的GEMMA 3模型进行指令调优，采用4位精度和参数高效微调。通过基于错误分类器分类体系的确定性提示设计，结合轻量级规范化器和约束感知的保守解码策略。

Result: 在官方未调优的GLEU评估中，马来语得分92.41（排名第六），印地语得分81.44（排名第三），表明该方法在计算效率和性能方面具有优势。

Conclusion: 基于分类器提示设计、适配器指令调优和确定性解码的方法为印度语言语法纠错提供了一种可复现且计算高效的替代方案，值得进一步研究更强的形态句法约束和以人为中心的评估。
...
TL;DR: OmniFusion是一个端到端的多模态翻译系统，通过融合多模态基础模型和翻译专用LLM，实现语音、图像和文本的联合翻译，相比级联流水线降低了延迟并提升了质量。


<details>
  <summary>Details</summary>
Motivation: 当前开源文本翻译LLM虽然语言覆盖和质量有所提升，但在语音翻译中只能用于级联流水线（先语音识别再翻译），这会增加延迟（对同声传译尤其关键），且无法利用多模态上下文（如图像）进行消歧。而多模态基础模型虽具备跨模态感知推理能力，但缺乏多语言覆盖和专门的翻译性能。

Method: 提出端到端融合方法，将预训练多模态基础模型（MMFM）与翻译专用LLM结合。采用新颖的融合策略，将MMFM多个隐藏层的状态连接到翻译LLM，实现联合端到端训练。具体模型OmniFusion基于Omni 2.5-7B作为MMFM，SeedX PPO-7B作为翻译LLM。

Result: OmniFusion能够执行语音到文本、语音加图像到文本、文本加图像到文本的翻译。实验表明，模型能有效利用音频和视觉输入，在同声传译中相比级联流水线降低1秒延迟，并提升整体翻译质量。

Conclusion: 通过融合多模态基础模型和翻译专用LLM的端到端方法，成功构建了有效的多模态翻译系统，解决了级联流水线的延迟问题，并能利用多模态上下文提升翻译性能。
...
TL;DR: LLMs在印度语言中无法直接识别情绪状态，需要通过翻译到英语或中文才能有效聚类情绪表达


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在精神病学中有应用前景，但主要是英语中心化的。不同语言有独特的痛苦表达方式，需要评估LLMs在印度语言中理解情绪状态的能力

Method: 收集11种印度语言的247个独特短语，代表四种情绪状态。测试七种实验条件：直接嵌入原生和罗马化脚本（使用多语言和印度特定模型），以及将短语翻译成英语和中文后的嵌入。使用基于调整兰德指数、归一化互信息、同质性和完整性的综合评分衡量性能

Result: 直接嵌入印度语言无法聚类情绪状态（综合评分=0.002）。所有基于翻译的方法都有显著改善。使用Gemini翻译的英语（0.60）和人工翻译的英语（0.61）嵌入Gemini模型表现良好。人工翻译的英语再翻译成中文并用中文模型嵌入表现最佳（0.67）。专门的印度模型表现不佳

Conclusion: 当前模型无法直接从印度语言中有意义地表示情绪状态，这对在印度的精神病学应用构成了根本障碍。虽然高质量翻译可以弥补这一差距，但依赖专有模型或复杂翻译流程不可持续。模型必须首先能够理解多样化的本地语言，才能在全球心理健康中有效应用
...
TL;DR: EduEval是一个针对中文K-12教育的分层基准测试，包含24种任务类型和11,000多个问题，评估LLMs在六个认知维度的表现，发现开源模型在复杂教育推理上优于专有系统。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在教育应用中具有巨大潜力，但未经审查的部署可能危及教育标准，因此需要严格的评估框架来确保LLMs在教育领域的可靠性和有效性。

Method: 提出EduEval基准测试，包含三个关键贡献：1) EduAbility认知框架，整合布鲁姆分类法和韦伯知识深度理论；2) 真实性，整合真实考试题、课堂对话、学生作文和专家设计的提示；3) 规模，包含24种任务类型和11,000多个问题。评估了14个领先LLMs在零样本和少样本设置下的表现。

Result: 模型在事实性任务上表现良好，但在课堂对话分类方面存在困难，创造性内容生成结果不一致。有趣的是，多个开源模型在复杂教育推理上优于专有系统。少样本提示在不同认知维度上效果各异，表明不同教育目标需要定制化方法。

Conclusion: EduEval为开发专门针对多样化中文教育任务优化的LLMs提供了有针对性的基准测试指标，强调了教育应用需要根据具体认知维度和任务类型采用不同的评估和优化策略。
...
TL;DR: 本研究对47个基于上下文的问答模型在8个数据集上进行了基准测试，发现ahotrod/electra_large_discriminator_squad2_512模型表现最佳，平均准确率43%，并分析了模型性能与上下文长度、答案长度和复杂度的关系。


<details>
  <summary>Details</summary>
Motivation: CBQA模型通过考虑上下文信息提供更准确相关的答案，在用户支持、信息检索和教育平台中有广泛应用。本研究旨在评估不同CBQA模型在多个数据集上的性能，找出无需额外微调的最佳模型，以简化实际应用中的部署流程。

Method: 对来自Hugging Face的47个CBQA模型在8个不同数据集上进行基准测试，分析模型性能与上下文长度、答案长度和复杂度的关系，并使用遗传算法通过集成多个模型的响应来提高整体准确率。

Result: 最佳模型ahotrod/electra_large_discriminator_squad2_512在所有数据集上平均准确率为43%，在特定数据集上表现优异：bioasq10b-factoid (65.92%)、biomedical_cpgQA (96.45%)、QuAC (11.13%)和Question Answer Dataset (41.6%)。Bert-large-uncased-whole-word-masking-finetuned-squad在IELTS数据集上达到82%准确率。模型计算时间取决于上下文长度和模型大小，性能通常随答案长度增加而下降。

Conclusion: 在SQuAD v2或SQuAD v1数据集上训练的模型表现最佳，ahotrod/electra_large_discriminator_squad2_512是整体最优模型。模型性能受上下文复杂度、答案长度和模型大小影响，遗传算法集成方法可进一步提高准确率。这些发现有助于在实际应用中快速选择合适模型而无需额外微调。
...
TL;DR: 该论文提出了一种基于SQL的方法来处理演化半结构化表格的时间推理问题，发现模式设计质量对问答精度的影响大于模型容量，并提出了三个设计原则。


<details>
  <summary>Details</summary>
Motivation: 当前QA系统在处理演化半结构化表格的时间推理方面存在挑战，需要更有效的方法来处理Wikipedia信息框等动态变化的结构化数据。

Method: 采用SQL-based方法，包括三个步骤：(1) 从Wikipedia信息框生成3NF模式，(2) 生成SQL查询，(3) 执行查询。提出了三个模式设计原则：保留上下文的规范化、减少歧义的语义命名、一致的时间锚定。

Result: 最佳配置（Gemini 2.5 Flash模式 + Gemini-2.0-Flash查询）达到80.39 EM，相比基线（68.89 EM）提升了16.8%。证明了模式设计质量对问答精度的影响大于模型容量。

Conclusion: 模式设计质量比模型容量对QA精度的影响更大，提出了三个有效的模式设计原则，为处理演化半结构化表格的时间推理提供了实用方法。
...
TL;DR: 论文提出A-CC评估框架，用于评估多轮工具调用LLM在面临误导性断言时的鲁棒性，发现模型对用户源断言和函数源断言都高度脆弱。


<details>
  <summary>Details</summary>
Motivation: 多轮工具调用LLM在安全关键行业部署面临挑战，现有基准测试缺乏对多轮对话级鲁棒性的评估，特别是模型在面对真实世界误导信息时的表现。

Method: 提出Assertion-Conditioned Compliance (A-CC)评估范式，包含两个评估维度：用户源断言(USAs)评估模型对错误用户信念的顺从性，函数源断言(FSAs)评估模型对矛盾系统策略的遵从性。

Result: 实验结果显示模型对USA顺从性和FSA策略冲突都高度脆弱，证实A-CC是部署智能体中潜在的关键漏洞。

Conclusion: A-CC框架揭示了多轮工具调用LLM在鲁棒性方面的严重缺陷，为安全关键应用中的模型评估提供了重要工具，强调了需要改进模型对误导信息的抵抗力。
...
TL;DR: IndicParam是一个包含13,000多道选择题的人工策划基准，涵盖11种低资源和极低资源的印度语言，评估了19个LLM，发现即使是表现最好的GPT-5平均准确率也只有45.0%，揭示了这些语言在LLM评估中的严重不足。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在高资源多语言任务上表现出色，但低资源和极低资源的印度语言仍然严重缺乏评估。现有基准对这些语言覆盖不足，需要专门的基准来评估LLM在这些语言上的能力。

Method: 创建了IndicParam基准，包含13,000多道人工策划的多选题，涵盖11种印度语言（包括低资源和极低资源语言）以及梵英混合语集。评估了19个专有和开源LLM，并对问题进行分类（知识导向vs纯语言），同时评估LLM处理不同问题格式的能力。

Result: 评估结果显示，即使是表现最好的GPT-5平均准确率也只有45.0%，DeepSeek-3.2为43.1%，Claude-4.5为42.7%。这表明当前LLM在低资源印度语言上的表现仍然有限。基准还提供了关于跨语言迁移局限性的见解。

Conclusion: IndicParam为低资源印度语言建立了一个具有挑战性的基准，揭示了LLM在这些语言上的局限性，并为未来的研究和模型改进提供了重要参考。数据集和评估脚本已公开可用。
...
TL;DR: 提出CourseTimeQA数据集和CrossFusion-RAG模型，在单GPU预算下实现教育视频时间戳问答，相比BLIP-2提升检索性能同时保持低延迟


<details>
  <summary>Details</summary>
Motivation: 解决教育视频时间戳问答任务，在单GPU内存/延迟预算约束下，需要高效检索相关时间戳片段并生成基于视频内容的答案

Method: 提出CrossFusion-RAG：轻量级跨模态检索器，结合冻结编码器、视觉投影、浅层查询无关跨注意力（带时序一致性正则化）和小型交叉注意力重排序器

Result: 在CourseTimeQA数据集上，CrossFusion-RAG相比BLIP-2提升nDCG@10 0.10和MRR 0.08，在单A100上实现约1.55秒中位端到端延迟

Conclusion: 提出的轻量级跨模态检索框架在有限计算资源下有效提升了教育视频时间戳问答性能，为实际部署提供了可行方案
...
TL;DR: 研究发现LLM中模拟的人格特质（大五人格）会影响相关性标注中的启动效应，某些人格特征（如高开放性和低神经质）能降低启动敏感性，提出人格提示作为缓解阈值启动的方法。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM被用作可扩展的相关性标注工具，但研究表明它们容易受到启动效应的影响（先前相关性判断影响后续判断）。虽然心理学理论将人格特质与此类偏见联系起来，但尚不清楚LLM中模拟的人格是否表现出类似效应。

Method: 研究LLM中大五人格特征如何影响相关性标注中的启动效应，使用多个LLM在TREC 2021和2022深度学习赛道数据集上进行实验。

Result: 结果显示某些人格特征（如高开放性和低神经质）能持续降低启动敏感性。此外，最有效缓解启动的人格可能因模型和任务类型而异。
...
TL;DR: 开发了一个基于大型语言模型的AI辅助英语写作错误分析系统，能够识别、分类和纠正写作错误，为EFL教学提供自动化详细反馈。


<details>
  <summary>Details</summary>
Motivation: 传统基于量规的评估方法无法提供详细的错误分析反馈，需要开发能够自动化识别和分类英语写作错误的系统，以提升英语作为外语（EFL）教学的效果。

Method: 使用Claude 3.5 Sonnet和DeepSeek R1等大型语言模型，基于Corder、Richards和James的语言学理论构建详细错误分类法，通过Python编码的API调用实现系统，涵盖拼写、语法和标点等词句层面错误。

Result: 系统成功识别了多种错误类型，但在上下文理解方面存在局限，遇到未编码错误时会生成新的错误类别。使用"English as she is spoke"文本测试显示系统能够处理复杂的多层分析。

Conclusion: AI在EFL教学中具有自动化详细错误分析和反馈的潜力，但需要进一步开发以提高上下文准确性，并将分类法扩展到文体和语篇层面的错误。
...
TL;DR: CryptoBench是首个专家策划的动态基准测试，用于评估LLM代理在加密货币领域的实际能力，涵盖检索和预测任务，揭示了检索-预测不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 加密货币领域对LLM代理评估提出了独特挑战：极端时间敏感性、高度对抗性信息环境、需要整合多样化专业数据源。现有通用基准无法充分评估这些专业能力。

Method: 构建动态基准，每月50个专家设计的问题，采用四象限分类系统：简单检索、复杂检索、简单预测、复杂预测。评估10个LLM模型在代理框架下的表现。

Result: 发现检索-预测不平衡现象：许多领先模型在数据检索方面表现良好，但在需要预测分析的任务中表现明显不足，表明模型缺乏深度分析合成信息的能力。

Conclusion: CryptoBench为LLM代理在加密货币领域的评估提供了有价值的基准，揭示了当前模型在预测分析能力上的不足，为未来模型改进提供了方向。
...
TL;DR: SCALE框架通过选择性分配计算资源，针对数学推理中不同难度的子问题采用不同处理模式，相比均匀分配方法显著提升性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前测试时计算扩展方法对所有推理子问题采用均匀资源分配，导致困难子问题资源不足而简单操作浪费资源，造成性能瓶颈和资源利用效率低下。

Method: 提出SCALE框架，包含四个阶段：问题分解为顺序推理子问题、评估每个子问题难度、根据难度分配处理模式（简单用System 1，复杂用System 2）、顺序执行并传播上下文。

Result: SCALE显著优于均匀扩展基线，在AIME25上准确率提升13.75个百分点（从57.50%到71.25%），同时计算成本降低33%-53%。

Conclusion: SCALE通过选择性资源分配解决了当前测试时扩展方法的根本限制，实现了更好的性能提升和资源利用效率，是测试时计算扩展的重要进展。
...
TL;DR: CACARA提出了一种通过涌现对齐学习实现多模态多语言架构的方法，无需完整重新训练即可将新模态集成到现有模型中，并在仅使用英语数据微调新模态的情况下支持100多种语言。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型通常需要跨多种模态进行资源密集型的训练，扩展到新语言也需要类似的资源密集型策略。需要一种更高效的方法来集成新模态和支持多语言能力。

Method: 提出CACARA架构，采用涌现对齐学习范式，允许将新模态无缝集成到现有的双模态/多模态模型中，无需完整重新训练。仅使用英语对齐数据微调新模态，就能使模型支持100多种语言。

Result: 该方法在音频到文本检索任务中实现了高达14.24个百分点的R@1改进，超越了最先进的多模态模型，同时保持了与单语模型相当的训练成本。

Conclusion: 涌现对齐学习范式能够高效解锁多语言能力，仅通过英语数据微调新模态即可支持多种语言，显著降低了多模态多语言模型的训练成本，同时保持了先前学到的知识。
...
TL;DR: G-KV是一种KV缓存淘汰方法，使用全局评分机制结合局部和历史注意力分数来评估token重要性，并通过强化学习和蒸馏进行后训练优化，以提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在处理长序列时面临计算和内存挑战，现有的KV缓存压缩方法通常只关注提示压缩或基于局部注意力分数的token淘汰，忽略了token的长期重要性。

Method: 提出G-KV方法：1）使用全局评分机制，结合局部和历史注意力分数来准确评估token重要性；2）引入后训练技术，包括强化学习和蒸馏，优化模型在压缩KV缓存设置下的性能。

Result: 论文提供了代码实现（GitHub链接），表明该方法能有效提高推理效率，但具体性能数据未在摘要中说明。

Conclusion: G-KV通过全局评分机制和后训练优化，为KV缓存压缩提供了一种更准确的token重要性评估方法，有助于解决大语言模型在长序列处理中的效率问题。
...
<details>
  <summary>Details</summary>
Motivation: 针对土耳其语情感分析缺乏全面研究的现状，同时扩展英语情感分析技术，开发一个考虑多方面因素的综合情感分析框架。

Method: 1. 结合无监督、半监督和监督指标生成新颖特征集；2. 使用半监督领域特定方法创建极性词典；3. 对土耳其语进行精细形态分析确定语素极性；4. 为英语构建结合循环和递归神经网络的新架构；5. 构建利用情感、句法、语义和词汇特征的新词嵌入；6. 将上下文窗口重新定义为子句。

Result: 1. 在土耳其语和英语不同体裁数据集上，经典机器学习方法优于神经网络模型；2. 所有原始方法都取得了最先进和显著的结果；3. 创建了土耳其语首个半监督领域特定极性词典；4. 构建了新颖的神经网络架构和词嵌入。

Conclusion: 该论文是截至2020年7月土耳其语情感分析最详细全面的研究，对英语观点分类问题也有贡献，提出的方法可适应其他形态丰富或粘着语语言，以及自然语言处理其他任务。
...
TL;DR: 该论文提出了一个诊断框架，通过四个互补维度（前向-后向一致性、传递性覆盖、反事实敏感性、扰动鲁棒性）来区分语言模型中的真实数学推理与表面模式匹配，揭示了传统准确率指标无法发现的推理缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型的数学推理评估主要依赖答案准确率，这可能掩盖逻辑计算的根本性失败。需要更精细的诊断方法来区分真实的数学推理和表面的模式匹配。

Method: 提出了一个包含四个互补维度的诊断框架：1) 前向-后向一致性：检查模型在正向和反向推理中的一致性；2) 传递性覆盖：评估模型处理传递关系的能力；3) 反事实敏感性：测试模型对反事实条件的敏感性；4) 扰动鲁棒性：衡量模型对输入扰动的鲁棒性。在Qwen3-0.6B模型和MenatQA数据集上进行了案例研究。

Result: 在Qwen3-0.6B模型上的实验显示：虽然模型达到70%+的答案准确率，但前向-后向一致性仅15%，传递性覆盖仅32.2%，对扰动敏感。这表明模型主要依赖模式匹配而非真实逻辑计算。

Conclusion: 该诊断框架能够揭示传统准确率指标无法发现的推理缺陷，表明小模型主要依赖模式匹配而非真实数学推理。框架具有模型无关性和可推广性，为评估不同规模和架构模型的推理保真度提供了新方法。
...
TL;DR: 为斯洛伐克语开发首个概念词典，解决低资源语言缺乏机器可读语言数据的问题


<details>
  <summary>Details</summary>
Motivation: 斯洛伐克语作为资源有限的语言，缺乏足够大的机器可读语言数据源，导致许多需要自动化文本处理的任务效果较差或几乎无法解决

Method: 引入一种新型概念词典作为斯洛伐克语的第一个此类语言工具

Result: 创建了斯洛伐克语的首个概念词典，填补了该语言在机器可读语言数据方面的空白

Conclusion: 该概念词典为斯洛伐克语的NLP任务提供了必要的词典工具，有助于改善低资源语言的文本处理效果
...
TL;DR: Wikontic是一个多阶段管道，从开放域文本构建知识图谱，通过提取带限定符的三元组、强制类型/关系约束和实体规范化来创建紧凑、本体一致且连接良好的知识图谱。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的系统通常将知识图谱作为文本检索的辅助结构，未能充分利用其内在质量。需要一种方法直接从开放域文本构建高质量、本体一致的知识图谱，以增强LLM的结构化知识利用。

Method: 提出多阶段管道：1) 提取带限定符的候选三元组；2) 强制基于Wikidata的类型和关系约束；3) 规范化实体以减少重复。整个构建过程使用少于1000个输出token，效率高。

Result: 在MuSiQue上，正确答案实体出现在96%的生成三元组中；在HotpotQA上达到76.0 F1，在MuSiQue上达到59.8 F1，匹配或超越需要文本上下文的检索增强生成基线；在MINE-1基准上达到86%的信息保留率（SOTA）。

Conclusion: Wikontic提升了生成知识图谱的质量，为LLM中利用结构化知识提供了可扩展的解决方案，证明了仅使用三元组就能达到与需要文本上下文的系统相当的性能。
...
TL;DR: Prism是一个小型、组合式的元语言，用于指定工具使用软件代理的行为，基于固定核心上下文Core1，通过领域扩展实现自然语言决策规则到可检查、可执行策略的映射。


<details>
  <summary>Details</summary>
Motivation: 为工具使用软件代理提供一种结构化、可组合的规范语言，避免引入临时控制结构，实现自然语言决策规则到可执行策略的转换，同时支持分析、验证和安全约束。

Method: 基于固定核心上下文Core1（包含数字、字符串、用户提示、工具等基本类别和布尔值、谓词、对、列表的组合子），使用单一抽象运算符编写代理策略，条件语句表现为替代项选择而非命令式if-else块。领域通过定义自己的上下文微型语法扩展核心，引入新类别、谓词和外部工具。

Result: 通过恒温器控制、家庭安全、电子商务推荐和医疗监控等实例展示了自然语言决策规则如何映射为可检查、可执行的策略。从语言角度实现了可重用语法核心与领域特定词汇的清晰分离，从工程角度提供了紧凑的代理控制接口语言。

Conclusion: Prism提供了一种结构化方法，将自然语言决策规则转换为可检查、可执行的代理策略，支持组合性、可扩展性和形式化分析，在软件代理规范领域具有实用价值。
...
Method: 采用锦标赛式ELO排名和多智能体推理框架，让多个LLM智能体通过结构化竞赛工作流竞争、批评和协作，生成共识响应
...
TL;DR: 本文回顾了大型语言模型中奉承行为测量的方法论挑战，识别了五种核心操作化方式，并指出当前研究缺乏对人类感知的评估，为未来研究提供了建议。


<details>
  <summary>Details</summary>
Motivation: 随着文献中越来越多地声称大型语言模型存在奉承响应模式，需要系统性地回顾测量LLM奉承行为的方法论挑战，并识别当前研究的局限性。

Method: 通过文献回顾和分析，识别了五种测量LLM奉承行为的核心操作化方式，并分析了当前研究方法在区分奉承响应与AI对齐相关概念方面的困难。

Result: 研究发现：1）奉承行为本质上是人类中心的，但当前研究未评估人类感知；2）难以区分奉承响应与AI对齐中的相关概念；3）提出了未来研究的可操作建议。

Conclusion: 需要更严谨的方法来测量LLM奉承行为，特别是要纳入人类感知评估，并更好地区分奉承行为与其他AI对齐概念，以推动该领域的研究进展。
...
TL;DR: 提出一个可视化知识图谱框架，通过将模型生成内容与专有知识源链接，帮助用户识别和诊断大语言模型幻觉问题


<details>
  <summary>Details</summary>
Motivation: 企业环境中大语言模型常因上下文窗口限制、预训练数据与专有知识不一致导致幻觉，现有缓解策略成本高且缺乏确定性保证

Method: 构建交互式可视化知识图谱，将专有知识和模型生成内容组织成图结构，链接模型断言与真实来源，显示置信度，创建人机协作反馈循环

Result: 提供直观的可视化界面，使用户能够诊断不一致性、识别薄弱推理链并提供纠正反馈，增强模型可靠性

Conclusion: 该框架通过结构化的人机协作工作流程，为检测和缓解大语言模型幻觉提供了有效解决方案，能够持续提升响应质量
...
<details>
  <summary>Details</summary>
Motivation: 虽然生成式AI工具如ChatGPT在计算社会科学中应用日益广泛，但对其在复杂任务（如分类和标注包含细微语言的文本）中的性能仍缺乏充分理解，需要评估其在处理复杂语言时的表现

Method: 研究比较了GPT-4与人工标注者的性能，测试了ChatGPT 3.5、4和4o版本，设计了四种提示风格作为输入，评估了精确率、召回率和F1分数，并进行定量和定性分析

Result: 定量和定性评估显示，虽然提示中包含标签定义可能有助于提升性能，但总体而言GPT-4在处理细微语言分类时存在困难。定性分析揭示了四个具体发现，表明ChatGPT在涉及细微语言的分类任务中应谨慎使用

Conclusion: ChatGPT在处理细微语言分类任务时表现有限，建议在涉及复杂语言分类的应用中谨慎使用，并考虑结合人工标注
...
TL;DR: 提出一个语言无关的基于Transformer的POS标注框架，针对低资源语言，以孟加拉语和印地语为案例，仅需少量代码修改即可实现跨语言适配。


<details>
  <summary>Details</summary>
Motivation: 为低资源语言开发POS标注系统，解决传统方法需要大量语言特定工程和资源的问题，促进在资源匮乏语言上的NLP研究。

Method: 使用基于Transformer的语言无关框架，通过模块化设计，仅需三行框架特定代码即可从孟加拉语适配到印地语，利用底层Transformer架构的泛化能力。

Result: 在孟加拉语和印地语上分别达到96.85%和97%的词级准确率，F1分数表现强劲，尽管存在数据集不平衡和语言重叠问题。特定POS类别存在性能差异，凸显数据集标注挑战。

Conclusion: 该框架的模块化、开源设计显著降低了跨语言适配的模型设计和调优开销，使研究者能专注于语言预处理和数据集优化，这对推进低资源语言的NLP研究至关重要。
...
TL;DR: 提出Entropy Equilibrium Sampling (EES)，一种无需超参数调优的token采样策略，通过平衡归一化熵与概率质量来动态调整候选集，在各种任务和模型架构中表现稳定。
...
Result: EES在推理和生成任务中表现稳定，在不同温度设置下保持竞争力，同时维持多样性和连贯性，简化了部署流程。

Conclusion: EES是一种有效的超参数无关采样策略，能简化大语言模型部署，在各种任务和模型架构中表现稳定。
...
TL;DR: 在资源受限的孟加拉语NLP环境中，自动混合精度训练能显著提升计算效率，加速44.5%并减少17.6%内存使用，同时保持99.7%的模型性能。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语NLP开发面临计算资源有限、训练成本高的挑战，需要找到在硬件受限环境下仍能高效训练模型的方法。

Method: 使用自动混合精度训练技术，动态混合16位和32位浮点计算，在四个孟加拉语NLP任务（情感分析、命名实体识别、错误分类、问答）上评估四种Transformer模型。

Result: AMP训练加速44.5%，减少17.6%内存消耗，同时F1分数保持在完整精度基准的99.7%以内，性能损失极小。

Conclusion: 自动混合精度训练能有效降低计算门槛，有助于在硬件受限环境中普及最先进的NLP能力，促进孟加拉语NLP发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM水印方法在操纵token生成概率时面临检测能力与文本质量之间的权衡：强水印信号和随机性会降低下游任务性能

Method: 提出控制种子池的嵌入方案，支持多样化并行生成水印文本；基于此设计WaterSearch框架，联合优化分布保真度和水印信号特性，并配备句子级检测方法

Result: 在10个任务上评估，平均性能比SOTA基线提升51.01%（检测强度95%）；短文本和低熵输出场景分别提升47.78%和36.47%；在插入、同义词替换、改写等攻击下保持高检测率

Conclusion: WaterSearch通过搜索框架有效解决了水印检测与文本质量之间的权衡问题，在多种场景和攻击下表现出优越的鲁棒性和性能
...
TL;DR: EffiLoRA是一种高效的LoRA改进方法，通过共享A矩阵和动态选择B矩阵更新来减少参数冗余，在多种模态任务中优于原始LoRA。


<details>
  <summary>Details</summary>
Motivation: LoRA虽然被广泛采用，但在复杂数据集上仍存在显著的计算开销和参数干扰问题。现有方法通过解耦LoRA更新矩阵来利用矩阵不对称性，但训练成本仍然很高。

Method: EffiLoRA从层间和层内参数冗余的角度重新审视LoRA，采用统一的A矩阵跨所有Transformer层共享，并引入运行时选择性B矩阵更新机制，动态平衡系统资源预算和模型性能。

Result: EffiLoRA在多种模态任务中持续优于LoRA，包括常识推理、视觉指令调优和图像生成，展示了更高的效率和鲁棒性。

Conclusion: EffiLoRA为语言、多模态和扩散模型提供了一种轻量级且可泛化的高效微调方法，显著降低了参数冗余和计算开销。
...
TL;DR: Reward Auditor：一个假设检验框架，用于评估奖励模型在真实世界扰动场景下的适用性（Suitability），通过统计显著性检验和效应大小量化来推断奖励模型的系统性漏洞。


<details>
  <summary>Details</summary>
Motivation: 当前奖励模型评估方法仅关注特定场景下的偏好感知准确率，无法揭示奖励模型在真实世界场景中的关键漏洞。需要评估奖励模型在真实世界扰动下的条件可靠性（适用性），为构建更安全、鲁棒、可信的LLM对齐系统奠定基础。

Method: 提出Reward Auditor假设检验框架，通过审计奖励模型偏好感知置信度的分布退化来量化统计显著性和效应大小。该方法不回答"奖励模型在给定样本上的偏好感知准确率如何"，而是回答"能否推断奖励模型在特定真实世界场景中存在系统性漏洞"。

Result: Reward Auditor能够推断奖励模型在不同真实世界场景中漏洞的确定性和严重程度，为构建可验证安全、更鲁棒、更可信的下一代LLM对齐系统提供坚实基础。

Conclusion: Reward Auditor框架解决了评估奖励模型适用性的关键挑战，通过科学审计方法揭示奖励模型在真实世界扰动下的系统性漏洞，为LLM安全对齐提供了更可靠的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在生成过程中会出现上下文不一致的幻觉问题，即模型输出与用户提示不匹配。本研究旨在探索提示工程方法能否缓解零样本科学文本摘要中的这种幻觉问题。

Method: 使用8篇酵母生物技术论文摘要，对6个指令调优的大语言模型应用7种提示方法：基线提示、两种复杂度递增的指令(PE-1和PE-2)、两种上下文重复(CR-K1和CR-K2)、两种随机添加(RA-K1和RA-K2)。上下文重复涉及识别并重复摘要中的K个关键句子，随机添加则重复K个随机选择的句子(K=1或2)。生成336个摘要，使用ROUGE、BERTScore、METEOR、余弦相似度等6个指标评估词汇和语义对齐。

Result: 统计分析3744个数据点显示，上下文重复(CR)和随机添加(RA)方法能显著改善大语言模型生成摘要与原文的词汇对齐。使用偏差校正加速自举置信区间和Bonferroni-Holm校正的Wilcoxon符号秩检验验证了统计显著性。

Conclusion: 提示工程方法，特别是上下文重复和随机添加，对减少零样本科学摘要任务中的幻觉具有潜在影响，能够改善摘要与原文的词汇对齐。
...
TL;DR: DeformAr是一个专门用于分析和解释阿拉伯语与英语命名实体识别系统性能差异的调试与评估框架，通过跨组件分析和行为分析来诊断模型行为与数据因素。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在英语NLP任务中表现出色，但在阿拉伯语命名实体识别中效果有限，即使使用更大的预训练模型。这种性能差距源于分词、数据集质量和标注不一致等多种因素，现有研究往往孤立分析这些问题，未能捕捉它们对系统行为和性能的联合影响。

Method: 提出DeformAr框架，包含数据提取库和交互式仪表板，支持两种评估模式：1) 跨组件分析：将每种语言分为数据集和模型组件，系统诊断各子组件；2) 行为分析：结合可解释性技术、token级指标、交互式可视化和表示空间分析，实现组件感知的诊断过程。

Result: DeformAr是首个阿拉伯语特定的、基于组件的可解释性工具，能够检测模型行为并通过将其与底层表示模式和数据因素联系起来进行解释，为资源不足语言中的模型分析提供了关键资源。

Conclusion: 该框架能够系统分析阿拉伯语与英语NER系统的性能差异，通过组件化的诊断方法揭示模型行为与数据因素的关联，有助于推动资源不足语言中的模型分析进展。
...
TL;DR: 轻量级开源大语言模型（如Qwen3 8B和Llama3 8B）在金融情感分析任务上表现优异，即使只使用5%的训练数据也能达到竞争力性能，为资源有限的研究者和从业者提供了成本效益高的解决方案。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型在金融市场分析中依赖大量计算资源和专有数据集，成本高昂且难以获取，限制了研究者和从业者的使用。需要探索轻量级开源模型在资源受限环境下的可行性。

Method: 比较基准金融NLP模型FinBERT与三个开源轻量级LLM（DeepSeek-LLM 7B、Llama3 8B Instruct、Qwen3 8B）在五个公开数据集上的表现，包括不同规模、来源、格式和语言的金融文本数据，评估零样本、少样本和有限训练数据场景。

Result: 轻量级LLM（特别是Qwen3 8B和Llama3 8B）在大多数场景中表现最佳，即使只使用5%的训练数据也能保持竞争力。这些结果在零样本和少样本学习场景中同样成立。

Conclusion: 轻量级开源大语言模型是成本效益高的选择，即使在有限标注数据上训练，也能在异构文本数据上实现竞争力性能，为资源受限的金融分析应用提供了可行方案。
...
TL;DR: 论文提出TAMO框架，将表格作为独立模态与文本标记集成，通过超图神经网络编码表格结构，显著提升LLMs在表格推理任务上的性能


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在处理表格数据时存在不足，即使最先进的GPT模型在表格推理任务上表现不佳。主要原因是现有方法通常简单地将表格数据序列化输入，导致结构信息丢失

Method: 提出TAMO框架，将表格视为独立模态与文本标记集成。使用超图神经网络作为全局表格编码器，与主流LLM无缝集成，形成多模态框架

Result: 在HiTab、WikiTQ、WikiSQL、FeTaQA和StructQA等多个基准数据集上取得显著改进，平均相对增益达到42.65%

Conclusion: 将表格作为独立模态处理，通过超图神经网络编码结构信息，能够有效提升LLMs在表格推理任务上的泛化能力
...
TL;DR: 提出了Dr.Mi-Bench基准测试，用于评估科学领域深度研究代理的规划、检索和推理能力，包含200个跨10个科学领域的人类标注实例。
...
Method: 提出了Dr.Mi-Bench基准测试和Dr.Mi-Eval评估范式。基准基于学术文献构建，包含200个跨10个科学领域的人类标注实例。评估范式采用模块化集成方法，利用学术论文的丰富结构，通过端到端评估和孤立评估两种互补模式来评估规划、检索和推理能力。

Result: 实验结果显示碎片化的性能格局：代理表现出专业优势但存在关键弱点，特别是在多源检索（用于综述类任务）和跨不同科学领域的一致性表现方面。此外，提高高层规划能力是解锁基础LLM作为骨干推理潜力的关键因素。

Conclusion: Dr.Mi-Bench通过暴露可操作的失败模式，为开发更可靠的学术研究助手提供了诊断工具，能够指导深度研究代理的发展。
...
TL;DR: 比较Graph RAG与Advanced RAG两种检索策略在QA任务中的表现，并评估LLMs生成幻灯片和播客脚本等非传统学术输出的能力。GPT 4o mini配合Advanced RAG表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型评估主要关注事实问答或简短摘要等标准任务，本研究希望扩展评估范围：1) 比较两种检索策略在QA任务中的效果；2) 评估LLMs生成幻灯片和播客脚本等非传统学术输出的质量。

Method: 结合Meta的LLaMA 3 70B开源模型和OpenAI的GPT 4o mini API构建原型系统。使用人类评分（11个质量维度）和LLM法官进行交叉验证评估QA性能。通过文档检索测试幻灯片和播客脚本生成能力。

Result: GPT 4o mini配合Advanced RAG在QA任务中产生最准确的回答。Graph RAG改进有限且导致更多幻觉（部分源于其结构复杂性和手动设置）。在幻灯片和播客生成中，GPT 4o mini再次表现最佳，但LLaMA 3在叙事连贯性方面有潜力。

Conclusion: 人类评审者在检测布局和风格缺陷方面至关重要，突显了在评估新兴学术输出时需要结合人类和LLM评估。Advanced RAG在QA任务中优于Graph RAG，GPT 4o mini在两种任务中都表现最佳。
...
TL;DR: 论文提出"语义混淆"概念来评估语言模型在相近意图表达上的不一致拒绝行为，并开发了ParaGuard数据集和三个模型无关的指标来测量这种局部不一致性。


<details>
  <summary>Details</summary>
Motivation: 当前的安全对齐语言模型经常拒绝实际上无害的提示，而现有评估主要关注全局拒绝率或合规性，忽略了局部不一致性——模型接受某种意图表达却拒绝其相近的释义。这种差距限制了模型的诊断和调优。

Method: 1) 提出"语义混淆"失败模式概念；2) 构建ParaGuard数据集，包含10k个控制性释义簇，保持意图不变但改变表面形式；3) 提出三个模型无关的token级指标：混淆指数、混淆率和混淆深度，使用token嵌入、下一个token概率和困惑度信号来比较每个拒绝与其最近接受邻居。

Result: 实验显示全局错误拒绝率隐藏了关键结构：某些设置中边界全局不稳定，其他设置中存在局部不一致性区域，有些情况下更严格的拒绝并不会增加不一致性。混淆感知审计能够区分系统拒绝的频率和拒绝的合理性。
...
TL;DR: 研究者收集了印度东部偏远地区10种濒危语言的1060个传统食谱，创建了ELR-1000数据集，并评估了LLM在翻译这些文化特定内容时的表现，发现提供针对性上下文能显著提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 当前语言技术主要服务于主流语言，缺乏对濒危语言和文化特定领域的关注。为了推动公平和具有文化意识的语言技术发展，需要建立针对这些被忽视语言和领域的基准数据集。

Method: 通过为低数字素养贡献者设计的移动界面，从印度东部偏远农村社区众包收集了1060个传统食谱，涵盖10种濒危语言。然后评估了多个最先进的大型语言模型在翻译这些食谱时的表现，并测试了提供针对性上下文（包括语言背景信息、翻译示例和文化保护指南）对翻译质量的影响。

Result: 尽管LLM具有强大能力，但在处理低资源、文化特定的语言时仍存在困难。然而，提供针对性上下文（语言背景、翻译示例、文化保护指南）能显著提高翻译质量。研究强调了为被忽视语言和领域建立基准的重要性。

Conclusion: 需要开发专门针对被忽视语言和领域的基准来推动公平和具有文化意识的语言技术发展。ELR-1000数据集的发布旨在激励为濒危语言开发语言技术，促进文化多样性的保护。
...
TL;DR: 该论文通过系统调查澄清和协调文本隐私保护的评估指标，分析47篇论文中的6种隐私概念，评估其与法律标准（HIPAA和GDPR）和用户期望的一致性，为文本匿名化提供更稳健、可比较且符合法律的隐私评估指导。


<details>
  <summary>Details</summary>
Motivation: 文本匿名化对于在敏感数据领域进行NLP研究和模型开发至关重要，但评估匿名化方法是否充分保护隐私仍然是一个开放挑战。当前隐私评估指标缺乏统一性和明确性，需要澄清和协调不同的隐私概念和评估方法。

Method: 通过对47篇报告隐私指标的论文进行手动审查，识别和比较6种不同的隐私概念，分析相关指标如何捕捉隐私风险的不同方面。然后评估这些概念与法律隐私标准（HIPAA和GDPR）以及基于HCI研究的用户中心期望的一致性。

Result: 识别了6种不同的隐私概念及其评估指标，分析了它们在捕捉隐私风险方面的差异。发现当前隐私评估实践与法律标准和用户期望之间存在差距，需要更好的对齐。

Conclusion: 该分析为导航隐私评估方法提供了实用指导，突出了当前实践的差距，旨在促进文本匿名化中更稳健、可比较且具有法律意识的隐私评估。
...
TL;DR: DrawingBench是一个用于评估代理式LLM可信度的透明验证框架，通过空间推理任务（生成GUI操作序列）进行客观评估，包含250个多样化提示、确定性指标和外部监督机制。


<details>
  <summary>Details</summary>
Motivation: 随着代理式AI系统日益自主运行，通过可验证评估建立信任变得至关重要。现有基准缺乏评估代理行为可靠性的透明度和可审计性。

Method: 开发DrawingBench框架：通过空间推理任务（生成低级GUI操作序列）评估代理式LLM。包含250个多样化提示（20个类别，4个难度级别），8个客观评估标准，确定性评估指标，以及通过多轮反馈实现的外部监督机制。

Result: 评估4个SOTA LLM（Claude-4 Sonnet, GPT-4.1, GPT-4.1-mini, Gemini-2.5 Flash）的1000个测试：模型在结构化外部反馈下达到92.8%完美性能（平均提升3.2%，复杂场景最高提升32.8%），但在工具状态管理和长时程规划方面存在系统性错误模式。规范清晰度比任务复杂度更重要（明确可验证标准下达到100%完美性能）。

Conclusion: 透明评估框架可以建立对代理系统的信任，外部监督在引导代理行为方面比自我修正更可靠。开源框架为可信代理评估提供了模板。
...
TL;DR: 本文提出一个RAG扰动-温度分析框架，系统研究文本扰动（模拟噪声检索）与温度设置之间的相互作用，发现高温设置会放大对扰动的脆弱性，并提供了RAG鲁棒性评估基准和实用指南。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统评估通常孤立地考察检索质量和生成参数（如温度），忽略了它们之间的相互作用。本文旨在系统研究文本扰动（模拟噪声检索）如何与不同温度设置相互作用，以全面理解RAG系统的鲁棒性。

Method: 提出全面的RAG扰动-温度分析框架，对检索文档应用三种不同类型的扰动，并在不同温度设置下进行多轮LLM实验。在HotpotQA数据集上使用开源和专有LLM进行广泛实验，分析性能退化模式。

Result: 实验表明性能退化呈现特定模式：高温设置会持续放大对扰动的脆弱性，而某些扰动类型在温度范围内表现出非线性敏感性。高温设置使模型更容易受到噪声检索的影响。

Conclusion: 本文贡献包括：(1) RAG鲁棒性诊断基准，(2) 量化扰动-温度相互作用的分析框架，(3) 噪声检索条件下的模型选择和参数调优实用指南。为理解和改进RAG系统在真实噪声环境中的表现提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 临床AI助手被宣传为比通用大语言模型更安全可靠，但缺乏独立定量评估，存在证据缺口，而它们正日益影响诊断、分诊和指南解读

Method: 评估两个广泛部署的临床AI系统（OpenEvidence和UpToDate Expert AI）与三个前沿通用LLM（GPT-5、Gemini 3 Pro和Claude Sonnet 4.5），使用包含MedQA（医学知识）和HealthBench（临床对齐）任务的1000项迷你基准测试

Result: 通用模型始终优于临床工具，GPT-5得分最高，而OpenEvidence和UpToDate在完整性、沟通质量、上下文意识和基于系统的安全推理方面存在缺陷

Conclusion: 面向临床决策支持的工具可能落后于前沿LLM，强调在部署到患者工作流前需要透明、独立的评估
...
TL;DR: 本研究采用人机协同框架，通过提示引导DeepSeek V3.1识别《黄帝内经》中的隐喻和转喻，生成认知优化的翻译，在模拟读者评估中表现优于人工翻译和基线模型翻译。


<details>
  <summary>Details</summary>
Motivation: 中医理论基于意象思维，现有英文翻译多采用直译，目标语读者难以重建概念网络并应用于临床实践，需要更有效的翻译方法。

Method: 采用人机协同框架，选取《黄帝内经》四段核心理论文本，通过提示式认知支架引导DeepSeek V3.1识别隐喻和转喻，生成翻译。使用ChatGPT 5 Pro和Gemini 2.5 Pro模拟三类真实读者，从五个认知维度评估翻译质量，并进行结构化访谈和解释现象学分析。

Result: 提示调整后的LLM翻译在所有五个认知维度表现最佳，具有高度的跨模型和跨角色一致性。访谈揭示了人机翻译差异、隐喻转喻传递的有效策略以及读者的认知偏好。

Conclusion: 本研究为中医等概念密集的古籍翻译提供了一条认知性、高效且可复制的人机协同方法路径，通过提示引导LLM识别和传递隐喻转喻，能显著提升翻译的认知可及性。
...
TL;DR: 首次对Nagamese语言进行情感分析和情绪分类研究，构建了包含1,195个词的极性词典，使用朴素贝叶斯和支持向量机进行监督学习。


<details>
  <summary>Details</summary>
Motivation: Nagamese语言是印度东北部纳加兰邦和阿萨姆邦之间贸易中发展起来的克里奥尔语。虽然英语、印地语等资源丰富语言已有大量情感分析研究，但Nagamese语言尚未有相关工作。这是首次尝试对Nagamese语言进行情感分析和情绪分类。

Method: 构建了包含1,195个Nagamese词汇的情感极性词典，结合其他特征，使用监督机器学习技术（朴素贝叶斯和支持向量机）进行情感分析。

Result: 成功构建了Nagamese情感极性词典，并实现了对该语言的情感极性（积极、消极、中性）和基本情绪的分类系统。

Conclusion: 这是Nagamese语言情感分析领域的开创性工作，为低资源语言的情感分析研究提供了参考，建立的词典和分类方法为后续研究奠定了基础。
...
TL;DR: SUPERChem是一个包含500个专家策划的化学推理问题基准测试，采用多模态和纯文本格式，通过推理路径保真度评分评估模型推理质量，旨在推动LLMs达到专家级化学智能水平。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型化学推理能力的基准测试存在三个主要问题：任务过于简化、缺乏过程级评估、与专家级化学技能不匹配。需要一个新的基准测试来解决这些问题。

Method: 创建了500个专家策划的推理密集型化学问题，涵盖多个子领域，提供多模态和纯文本格式。采用原创内容和迭代策划流程消除有缺陷的项目并减轻数据污染。每个问题都配有专家编写的解决方案路径，使用推理路径保真度评分来评估推理质量。

Result: 人类基线准确率为40.3%，最佳模型GPT-5（High）仅达到38.5%，其次是Gemini 2.5 Pro（37.9%）和DeepSeek-V3.1-Think（37.3%）。基准测试能够引发多步骤、多模态推理，揭示视觉信息的模型依赖性影响，并区分高保真推理者和启发式推理者。

Conclusion: SUPERChem通过提供具有挑战性的基准测试和可靠的评估框架，旨在促进大语言模型向专家级化学智能发展。数据集已在Hugging Face上公开。
...
TL;DR: 提出了KardiaBench基准数据集和Kardia-R1框架，通过基于规则的强化学习实现可解释的共情推理，在情感准确性、共情能力等方面优于现有方法。
...
Method: 1) 构建KardiaBench大规模基准数据集（178,080个QA对，22,080个多轮对话，671个真实世界用户档案）；2) 提出Kardia-R1框架，采用Rubric-as-Judge Empathetic Reinforcement Learning（Rubric-ERL），基于GRPO方法使用可解释、人类对齐的规则奖励，紧密耦合用户理解、情感推理和支持性回应生成。

Result: 在四个LLM骨干上的广泛实验表明，Kardia-R1在情感准确性、共情能力、相关性、人物一致性和安全性方面持续优于其他方法。

Conclusion: 通过KardiaBench数据集和Kardia-R1框架，实现了身份感知的情感推理，推动了对话系统向更深入、可验证的共情能力发展。
...
<details>
  <summary>Details</summary>
Motivation: MBR解码虽然能生成高质量翻译，但需要计算所有候选对之间的得分，时间复杂度为O(n²)，计算成本过高。PMBR解码通过采样部分候选对来减少计算量，但会降低翻译质量。需要改进质量与成本之间的权衡关系。

Method: 提出协议约束的PMBR（AC-PMBR）解码方法，利用知识蒸馏模型来指导得分矩阵的补全过程。该方法在PMBR的基础上，通过蒸馏模型提供额外约束，改善矩阵补全的准确性。

Result: AC-PMBR将矩阵补全的近似误差降低了最多3倍，在WMT'23英德互译任务中，相比PMBR解码，在可比计算成本下获得了更高的翻译质量。

Conclusion: AC-PMBR解码通过知识蒸馏模型指导矩阵补全，有效改善了质量与计算成本之间的权衡，为MBR解码的实际应用提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为研究人员和非技术用户提供针对阿拉伯语世界的社交媒体监控分析工具，解决阿拉伯语社交媒体内容分析的特定需求。

Method: 集成灵活的文档存储与结构化数据管理后端架构，支持API密钥安全数据抓取，提供用户友好的前端界面进行数据上传和交互。

Result: 开发了一个能够处理大规模多模态数据集的平台，支持实时和历史社交媒体内容分析，生成详细的可视化和报告。

Conclusion: MARSAD是一个功能全面的阿拉伯语社交媒体分析平台，为阿拉伯语世界的社交媒体研究提供了有效的工具支持。
...
TL;DR: DyFuLM是一个动态融合学习模型，通过分层动态融合和门控特征聚合模块，在情感分析任务中实现了82.64%粗粒度和68.48%细粒度准确率，显著提升了情感表示能力。
...
Method: 提出动态融合学习模型(DyFuLM)，包含分层动态融合模块（自适应整合多级特征）和门控特征聚合模块（调节跨层信息流以实现平衡表示学习）。

Result: 在多任务情感数据集上，DyFuLM达到82.64%粗粒度和68.48%细粒度准确率，最低回归误差（MAE=0.0674，MSE=0.0082）和最高R²系数（0.6903）。消融实验显示各模块均有显著贡献。

Conclusion: DyFuLM通过有效的分层特征融合增强了情感表示和整体性能，各模块对特征交互和任务平衡均有重要贡献。
...
TL;DR: 论文提出PromptBridge框架，解决LLM提示词在不同模型间迁移时性能下降的"模型漂移"问题，通过训练无关的方法实现跨模型提示词转换，无需为每个任务或模型重新优化。


<details>
  <summary>Details</summary>
Motivation: 实际应用中，由于能力、成本、部署约束和隐私等因素，经常需要在不同LLM模型间切换。但为特定模型优化的提示词在其他模型上使用时性能会显著下降，这种现象被称为"模型漂移"，导致频繁的模型切换成本高昂。

Method: 提出PromptBridge框架：1）使用模型自适应反射提示进化（MAP-RPE）通过迭代反射精化和量化评估获得任务和模型特定的最优提示；2）利用源模型和目标模型的校准提示对学习跨模型提示映射；3）测试时，给定源模型提示，直接生成目标模型的优化提示。

Result: 实验表明PromptBridge在单智能体和多智能体设置中都能持续提高下游任务准确性，同时显著减少迁移工作量。该方法仅需少量对齐任务进行校准，无需昂贵的每任务或每模型重新优化。

Conclusion: PromptBridge有效解决了LLM模型切换中的提示词迁移问题，通过训练无关的框架实现了跨模型提示词的有效转换，为实际部署中的模型切换提供了实用解决方案。
...
TL;DR: 论文提出了一种支持印度多语言（包括混合语如Hinglish）的对话AI系统，用于金融辅助场景，通过多智能体架构提升用户参与度，同时保持低延迟。


<details>
  <summary>Details</summary>
Motivation: 印度语言多样性（31种主要语言和100多种次要语言）与英语普及率低（仅10%）形成鲜明对比，这为金融科技平台的金融包容性带来了障碍。需要解决语言障碍，使数字金融服务能够覆盖印度多元化的用户群体。

Method: 采用多智能体架构，包括语言分类、功能管理和多语言响应生成模块。系统支持代码混合语言（如Hinglish），并通过对多种语言模型的比较分析来优化性能。

Result: 通过实际部署验证，系统显著提升了用户参与度，同时保持了较低的计算开销（延迟增加4-8%）。这证明了多语言对话AI系统在印度金融场景中的有效性。

Conclusion: 这项工作通过支持多语言（特别是混合语）的对话AI系统，为新兴市场的数字金融服务弥合了语言鸿沟，促进了金融包容性，为类似多语言环境下的技术解决方案提供了参考。
...
TL;DR: 该论文提出了基于Conformer的MEG信号解码器，用于LibriBrain 2025 PNPL竞赛的语音检测和音素分类任务，通过轻量级架构和任务特定优化取得了优于基准的性能。


<details>
  <summary>Details</summary>
Motivation: 针对LibriBrain 2025 PNPL竞赛中的两个基础MEG任务（语音检测和音素分类），需要开发有效的解码器来处理原始的306通道MEG信号，并超越竞赛基准性能。

Method: 采用紧凑型Conformer架构处理原始MEG信号，使用轻量级卷积投影层和任务特定头部。语音检测任务中探索了MEG导向的SpecAugment数据增强；音素分类任务中使用逆平方根类别加权和动态分组加载器处理100样本平均示例；采用简单的实例级归一化来缓解分布偏移。

Result: 在官方标准赛道划分下，使用F1-macro进行模型选择，最佳系统在排行榜上取得了88.9%（语音检测）和65.8%（音素分类）的成绩，超越了竞赛基准，在两个任务中都进入了前10名。

Conclusion: 基于Conformer的MEG解码器在LibriBrain竞赛的两个基础任务中表现优异，证明了轻量级架构结合任务特定优化策略的有效性，为MEG信号处理提供了实用的解决方案。
...
TL;DR: 本文提出结合主动学习调度器与聚类的方法，在低资源语言微调阶段提升模型性能，可节省30%标注成本并提升4个F1分数点
...
Method: 提出集成微调管道，结合主动学习方法与结构化数据选择策略（主动学习调度器），连接主动学习到数据聚类，采用动态数据选择调度器

Result: 在斯洛伐克语、马耳他语、冰岛语和土耳其语实验中，微调阶段使用聚类结合主动学习调度可节省30%标注成本，提升最多4个F1分数点，并提高微调稳定性

Conclusion: 主动学习调度器与聚类的结合为低资源语言模型微调提供了有效的解决方案，能在有限数据下同时提升性能和降低成本
...
TL;DR: MCAT框架通过课程学习和数据平衡策略扩展MLLMs支持70种语言互译，并优化语音适配器将语音序列压缩至30个token，提升翻译质量和推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在语音到文本翻译中存在两个关键限制：1) 数据集英语中心化，限制了多语言互译能力扩展；2) 语音转换为长序列时推理速度急剧下降。

Method: 提出MCAT框架：1) 语言扩展方法，利用课程学习和数据平衡策略支持70种语言互译；2) 优化语音适配器模块，将语音序列长度压缩至仅30个token。

Result: 在FLEURS数据集70x69个方向上超越最先进的端到端模型，同时提升批量推理效率，仅需约1亿可训练参数和每种语言10小时S2TT数据。

Conclusion: MCAT框架有效解决了MLLMs在语音翻译中的语言覆盖和效率问题，实现了高质量的多语言互译和高效推理，已开源促进S2TT能力发展。
...
TL;DR: 研究评估了当前语言检测工具对约鲁巴语、基尼亚卢旺达语和阿姆哈拉语等非洲语言的数字表现，发现专业新闻内容比对话平台数据更适合训练AI模型


<details>
  <summary>Details</summary>
Motivation: 非洲语言在数字环境中的代表性不足，对话平台上的使用稀疏且受英语影响严重，缺乏真实的单语对话数据，这对训练语言模型构成了挑战

Method: 从Reddit和本地新闻源收集约鲁巴语、基尼亚卢旺达语和阿姆哈拉语的数据，分析两种数据源的差异，并测试AfroLID和通用LLM等语言检测模型的性能

Result: Reddit数据稀少且存在大量语码转换，而本地新闻媒体提供了干净的单语数据并促进了更多本地语言互动；语言检测模型在新闻数据上表现近乎完美，但在语码转换的Reddit帖子上表现不佳

Conclusion: 专业策划的新闻内容比对话平台数据更适合训练非洲语言的AI模型，未来需要能够处理干净和语码转换文本的模型来提高检测准确性
...
TL;DR: MAC-SLU是一个多意图汽车座舱口语理解数据集，通过引入真实复杂的多意图数据增加了SLU任务难度，并对主流开源LLMs和LALMs进行了全面基准测试。
...
Method: 提出MAC-SLU多意图汽车座舱口语理解数据集，并基于此对领先的开源LLMs和LALMs进行基准测试，涵盖上下文学习、监督微调、端到端和流水线范式。

Result: 实验表明，LLMs和LALMs通过上下文学习有潜力完成SLU任务，但性能仍显著落后于监督微调。端到端LALMs表现与流水线方法相当，并能有效避免语音识别错误传播。

Conclusion: MAC-SLU为SLU研究提供了更具挑战性的数据集，端到端LALMs在避免错误传播方面具有优势，但监督微调仍是性能最佳的方法。
...
TL;DR: 该论文提出UnsolvableQA数据集和UnsolvableRL框架，旨在让LLM不仅能解决可解问题，还能识别客观不可解问题（内在矛盾）和主观能力限制问题，从而减少幻觉和过度自信。


<details>
  <summary>Details</summary>
Motivation: 当前LLM难以区分客观不可解性（问题内在矛盾）和主观能力限制（超出模型能力的问题），这导致模型产生幻觉和过度自信。需要让模型既能解决可解问题，又能识别不可解问题并谨慎拒绝超出能力范围的任务。
...
Conclusion: 通过UnsolvableQA数据集和UnsolvableRL框架，LLM能够有效区分可解和不可解问题，减少幻觉和过度自信，提高可靠性。明确训练模型识别不可解问题是防止能力崩溃的必要条件。
...
TL;DR: 论文提出MMAG框架，将LLM代理的记忆组织为五个交互层，以解决长对话中相关性、个性化和连续性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在单次提示中能生成连贯文本，但在持续交互中难以保持相关性、个性化和连续性。人类沟通依赖多种记忆形式，包括回忆过往对话、适应个人特质和情境上下文。

Method: 引入混合记忆增强生成（MMAG）模式，将记忆组织为五个交互层：对话记忆、长期用户记忆、情景与事件关联记忆、感知与上下文记忆、短期工作记忆。借鉴认知心理学，将这些层映射到技术组件，并制定协调、优先级和冲突解决策略。

Result: 在Heero对话代理中实现该框架，加密的长期个人资料和对话历史已提升用户参与度和留存率。讨论了存储、检索、隐私和延迟等实施问题。

Conclusion: MMAG为构建记忆丰富的语言代理提供了基础，使其更连贯、主动且符合人类需求，同时指出了开放挑战。
...
TL;DR: 提出一种完全自监督的多语言词表借词检测方法，结合PMI相似度和轻量级对比学习，无需标注数据即可自动选择决策阈值，性能达到或超过有监督基线。


<details>
  <summary>Details</summary>
Motivation: 传统借词检测方法依赖标注数据或简单的字符串相似度度量，需要开发无需人工监督、更准确的自监督方法。

Method: 结合两种信息源：基于全局对应模型的PMI相似度，以及基于语音特征向量的轻量级对比学习组件。包含自动选择决策阈值的程序，无需标注数据。

Result: 在基准数据集上，仅PMI相似度已优于NED和SCA等现有字符串相似度度量，组合相似度性能达到或超过有监督基线。消融研究显示字符编码、温度设置和数据增强策略的重要性。

Conclusion: 该方法可扩展到不同规模的数据集，无需人工监督，并提供命令行工具供研究人员使用，为多语言借词检测提供了有效的自监督解决方案。
...
TL;DR: LLMs在需要单一正确答案的推理任务中表现出色，但在需要生成全面多样答案的多解任务中表现不佳，这归因于"推理过度自信"问题。研究引入MuSoBench基准，发现短链思维提示存在明显过度自信，而长链思维方法通过迭代探索和自我反思缓解了这一问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在需要单一正确答案的推理任务中表现出色，但在需要生成全面多样答案的多解任务中表现不佳。研究者认为这种限制源于"推理过度自信"现象——模型倾向于对不完整的解决方案集表达过度的确定性。

Method: 1. 引入MuSoBench基准，用于评估多解问题；2. 比较短链思维(Short-CoT)和长链思维(Long-CoT)提示方法的表现；3. 提出"认知刚性假说"来解释过度自信现象；4. 使用注意力熵分析来验证假说。

Result: 实验表明：1. 传统的短链思维提示范式表现出明显的过度自信；2. 新兴的长链思维方法通过迭代探索和自我反思缓解了过度自信问题；3. 注意力熵分析为认知刚性假说提供了初步支持。

Conclusion: 这些发现为评估LLM推理的完整性提供了工具，并强调需要将评估从单一答案准确性转向全面探索。研究揭示了LLMs在多解任务中的局限性，并提出了改进方向。
...
TL;DR: OmiGraph是首个针对基于省略的虚假信息检测框架，通过构建省略感知图来识别被省略的内容，在两大基准测试上平均提升5.4% F1和5.3%准确率。


<details>
  <summary>Details</summary>
Motivation: 虚假信息不仅通过显式编造误导内容，还通过隐式省略重要信息来欺骗读者。虽然前者已被广泛研究，但基于省略的欺骗方式常被忽视，尽管它能在看似完整的信息中微妙地引导读者得出错误结论。

Method: OmiGraph框架：1) 利用捕捉同一事件互补视角的上下文环境构建省略感知图；2) 提出省略导向的关系建模来识别内部上下文依赖和动态省略意图；3) 引入省略感知的消息传递和聚合机制来提取检测模式。

Result: 在两个大规模基准测试上取得了显著性能提升，平均F1分数提高5.4%，准确率提高5.3%，证明了考虑省略视角的有效性。

Conclusion: 该研究首次系统性地解决了基于省略的虚假信息检测问题，提出的OmiGraph框架通过显式建模省略内容和关系，显著提升了虚假信息检测性能，填补了该领域的研究空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要衡量正确性，忽略了解决方案背后的方法多样性。真正的创新不仅需要正确答案，还需要方法的原创性。当前缺乏系统评估AI代理创新潜力的基准。

Method: 提出InnoGym基准框架，包含两个互补指标：性能增益（衡量相对于已知最佳方案的改进）和新颖性（捕捉与先前方法的方法论差异）。基准包含18个精心策划的真实世界工程与科学任务，通过资源过滤、评估器验证和解决方案收集进行标准化。同时提供iGym统一执行环境，用于可重复和长视野评估。

Result: 广泛实验表明，虽然某些代理能产生新颖方法，但其缺乏鲁棒性限制了性能增益。这些结果突显了创造力与有效性之间的关键差距。

Conclusion: 需要同时评估创造力和有效性的基准，InnoGym填补了这一空白，为系统评估AI代理的创新潜力提供了首个框架，强调了在追求新颖性的同时确保解决方案鲁棒性的重要性。
...
TL;DR: 本文研究发现，大型推理模型（LRMs）的显式推理链会引入新的安全风险，现有监督微调方法效果有限，而强化学习能更有效地实现安全对齐，同时保持推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成显式推理链提高了数学和逻辑问题解决能力，但这也带来了新的安全风险——不安全行为可能出现在中间推理轨迹中，即使最终答案看起来无害。现有基于监督微调的安全对齐方法效果不一致，会降低推理能力，且跨模型泛化能力差。

Method: 研究采用强化学习作为补充优化框架，通过奖励反馈直接优化模型策略，实现更自适应和稳定的安全对齐。与监督微调相比，RL能更好地抑制不安全的探索性推理，同时保持反思深度。

Result: 跨多个模型家族和基准测试的广泛实验表明，强化学习实现了更强、更一致的安全提升，同时保持了推理能力。对反思动态和token级熵的分析显示，RL能有效抑制不安全的探索性推理，同时保持反思深度。

Conclusion: 纯监督方法不足以实现大型推理模型的稳健安全对齐，强化学习作为补充框架能更有效地实现安全对齐，同时保持推理能力，为LRMs的安全训练提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然幻觉检测在英语中已有广泛研究，但资源不足的印度语言领域仍未被充分探索。多语言大语言模型在应用中经常产生看似合理但错误或误导性的输出，需要专门的评估基准来检测这些跨语言幻觉。

Method: 创建BHRAM-IL基准，包含36,047个精心策划的问题，涵盖9个类别（事实性、数值、推理、语言任务等）。在10,265个问题的子集上评估14个最先进的多语言大语言模型，使用类别特定指标（归一化到0-1范围）分析跨语言和事实性幻觉。

Result: 所有类别和模型的聚合得分为0.23，语言校正模糊得分为0.385。结果表明BHRAM-IL在幻觉评估方面的有效性，揭示了多语言模型在印度语言中的幻觉问题。

Conclusion: BHRAM-IL基准为多语言幻觉检测和缓解研究提供了重要工具，填补了印度语言领域的研究空白。数据集和代码已开源，支持未来研究。
...
TL;DR: 提出跨语言交错方法，无需文本监督混合多语言语音标记，发布EN-FR训练数据集和评估基准，证明该方法能提升单语语义准确性并实现跨语言理解和对话。


<details>
  <summary>Details</summary>
Motivation: 当前口语语言模型主要围绕英语发展，缺乏多语言训练数据和评估基准，限制了非英语语言的自然语言处理技术发展，特别是书面资源有限的语言。

Method: 提出跨语言交错方法，在无文本监督的情况下混合不同语言的语音标记；发布EN-FR训练数据集TinyStories（约42k小时）和两个跨语言语义评估基准（StoryCloze和TopicCloze），使用GPT-4合成生成。

Result: 在360M和1B参数的口语语言模型上，跨语言交错方法提升了单语语义准确性，实现了稳健的跨语言延续能力，并增强了跨语言隐藏状态对齐。

Conclusion: 跨语言交错是一种简单、可扩展的方法，可用于构建能够理解和跨语言对话的多语言口语语言模型。所有资源将开源以支持可重复性研究。
...
TL;DR: 研究人类对生成式AI缓解策略的感知，发现语言背景、AI经验等因素显著影响评估，参与者对语言细节敏感，这与LLM定量评估形成对比
...
Result: 结果显示参与者的母语、AI工作经验和标注熟悉度显著影响评估。参与者对语言和上下文属性表现出高度敏感性，惩罚微小的语法错误，同时奖励保留的语义上下文。这与语言在LLM定量评估中的处理方式形成对比

Conclusion: 研究提出了用于训练和评估缓解策略的新指标，并为人类-AI评估研究提供了见解，强调需要考虑人类感知因素来改进AI安全措施
...
TL;DR: 该研究定义了自动化网络舆情报告生成任务(OPOR-GEN)，构建了包含463个危机事件的数据集OPOR-BENCH，并提出基于智能体的评估框架OPOR-EVAL来模拟专家评价。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型使自动化舆情报告生成成为可能，但该领域缺乏系统研究，特别是正式的任务定义和相应基准。网络舆情报告整合新闻和社交媒体信息，对政府和企业的危机管理至关重要。

Method: 1. 定义自动化网络舆情报告生成任务(OPOR-GEN)；2. 构建事件中心数据集OPOR-BENCH，包含463个危机事件及其相关新闻、社交媒体帖子和参考摘要；3. 提出OPOR-EVAL评估框架，基于智能体模拟人类专家评估生成报告的质量。

Result: 实验表明，OPOR-EVAL框架与人类专家判断具有高度相关性。前沿模型在构建的基准上进行了测试，验证了评估框架的有效性。

Conclusion: 该研究为自动化网络舆情报告生成领域提供了完整的任务定义、基准数据集和评估框架，为这一关键领域的未来研究奠定了坚实基础。
...
TL;DR: 提出latent debate框架，通过分析单个模型推理过程中的隐式内部辩论来理解LLM的思考过程和幻觉成因


<details>
  <summary>Details</summary>
Motivation: 理解LLM的内部思考过程和幻觉成因是重要挑战，现有方法依赖多个答案或模型的显式辩论，无法捕捉单个模型推理中的隐式内部信号

Method: 提出模型和任务无关的latent debate概念框架，通过符号化实例化来近似LLM在True/False预测任务中的思考过程，捕捉推理过程中的隐式支持和攻击信号

Result: latent debate是忠实结构化的代理模型，与原始LLM预测高度一致；可作为幻觉检测的强基线；发现幻觉与辩论模式强相关，中间层高度辩论与更高幻觉风险相关

Conclusion: latent debate是理解LLM内部机制的有潜力框架，特别适用于推理步骤中出现内部（不）一致性的场景
...
TL;DR: RePro：通过过程级奖励优化LLM推理，将思维链视为梯度下降过程，使用双评分机制量化推理强度与稳定性，提升数学、科学和编程任务性能


<details>
  <summary>Details</summary>
Motivation: 尽管长思维链（CoT）提示提升了LLM的推理能力，但现有模型常出现过度思考和推理链过长等次优行为，影响性能表现。需要一种方法来优化推理过程本身。

Method: 将CoT视为梯度下降过程，提出RePro方法：1）定义代理目标函数评估推理优化过程；2）使用双评分机制量化推理强度和稳定性；3）将评分聚合为过程级奖励；4）集成到RLVR（带可验证奖励的强化学习）流程中优化LLM。

Result: 在多种强化学习算法和不同LLM上的广泛实验表明，RePro在数学、科学和编程基准测试中一致提升了推理性能，有效缓解了次优推理行为。

Conclusion: 通过将CoT框架化为优化过程并引入过程级奖励，RePro成功提升了LLM的推理质量，为解决长思维链中的次优行为提供了有效方案。
...
TL;DR: 本文提出了FINDER基准测试和DEFT失败分类法，用于评估深度研究代理在生成分析师级别报告时的性能，发现当前系统在证据整合、验证和推理规划方面存在困难。
...
Method: 提出了FINDER基准，包含100个人工策划的研究任务和419个结构化检查项，用于标准化报告结构、分析深度和事实基础。基于主流深度研究代理生成的约1000份报告，提出了DEFT失败分类法，包含14个细粒度失败模式，基于扎根理论并采用人-LLM协同标注和标注者间可靠性验证。

Result: 实验发现当前深度研究代理的主要问题不在于任务理解，而在于证据整合、验证和推理弹性规划方面存在困难。

Conclusion: 通过FINDER基准和DEFT分类法，揭示了深度研究代理在报告生成中的关键失败模式，为未来改进提供了系统性的评估框架和诊断工具。
...
TL;DR: 本文首次大规模研究了测试时缩放(TTS)策略，发现没有单一策略在所有情况下都最优，推理模型在不同问题难度和轨迹长度上表现出不同模式，且最优TTS性能随计算预算单调扩展。
...
Method: 进行了首次大规模TTS研究，使用8个开源LLM（7B到235B参数），在4个推理数据集上生成了超过300亿token，系统比较不同TTS策略。

Result: 观察到三个一致趋势：1) 没有单一TTS策略在所有情况下都最优；2) 推理模型在不同问题难度和轨迹长度上形成短视界和长视界两类模式；3) 对于给定模型类型，最优TTS性能随计算预算单调扩展。

Conclusion: 基于研究结果，提供了选择最佳TTS策略的实用指南，考虑了问题难度、模型类型和计算预算，为有效的推理时缩放提供了实践指导。
...
TL;DR: 提出4/6方法改进NVFP4量化算法，通过评估每个数值块的两个缩放因子来减少量化误差，提升训练稳定性和推理性能。


<details>
  <summary>Details</summary>
Motivation: 虽然低精度数值格式如NVFP4能加速计算并节省内存，但在训练和推理中需要将所有矩阵乘法操作数（权重、激活值、梯度）量化为NVFP4，这常导致训练发散和性能下降。

Method: 提出Four Over Six (4/6)方法，修改NVFP4量化算法，为每个数值块评估两个潜在的缩放因子。该方法针对浮点格式在接近最大值区域量化误差最大的问题，通过缩放为更小的FP4值使可表示值的分布更均匀。

Result: 在Transformer和混合模型架构的预训练实验中，4/6方法在多个案例中防止了训练发散，使训练损失显著接近BF16基准。同时，4/6能轻松集成到多种后训练量化方法中，普遍提升下游任务准确率。

Conclusion: 4/6方法能有效提升NVFP4训练的稳定性和性能，可在NVIDIA Blackwell GPU上高效实现，为未来使用NVFP4训练和部署模型提供了有前景的解决方案。
...
### [313] [OntoMetric: An Ontology-Guided Framework for Automated ESG Knowledge Graph Construction](https://arxiv.org/abs/2512.01289)
...
Method: OntoMetric采用三阶段流程：1）使用目录边界进行结构感知分割；2）本体约束的LLM提取，将ESGMKG模式嵌入提示中，同时用语义字段丰富实体以支持下游推理；3）两阶段验证，结合基于LLM的语义验证和基于规则的模式检查（VR001-VR006），保留段级和页级来源以实现审计可追溯性。

Result: 在五个ESG标准（SASB商业银行、SASB半导体、TCFD、IFRS S2、AASB S2）共228页60个段落的评估中，OntoMetric实现了65-90%的语义准确性和80-90%的模式合规性，而基线无约束提取仅为3-10%，每个验证实体的成本约为0.01至0.02美元。
...
### [314] [Gold-Medal-Level Olympiad Geometry Solving with Efficient Heuristic Auxiliary Constructions](https://arxiv.org/abs/2512.00097)
*Boyan Duan,Xiao Liang,Shuai Lu,Yaoxiang Wang,Yelong Shen,Kai-Wei Chang,Ying Nian Wu,Mao Yang,Weizhu Chen,Yeyun Gong*

Main category: cs.AI

TL;DR: HAGeo方法在几何定理证明中通过启发式辅助构造达到IMO金牌水平，超越AlphaGeometry，并创建了更具挑战性的HAGeo-409基准。
...
Method: 提出HAGeo方法：基于启发式的辅助构造几何推理方法。首先发现简单的随机辅助点策略就能达到IMO银牌水平，然后在此基础上开发了更高效的启发式方法，完全在CPU上运行，不依赖神经网络推理。

Result: 在IMO-30基准上解决了28/30个问题，达到IMO金牌水平，显著超越AlphaGeometry。创建了包含409个问题的HAGeo-409基准，提供更精确的评估和更高挑战。

Conclusion: HAGeo方法在几何定理证明中表现出色，达到了人类金牌水平，且不依赖神经网络。新基准HAGeo-409为几何定理证明设定了更高标准。
...
### [315] [Chunking Strategies for Multimodal AI Systems](https://arxiv.org/abs/2512.00185)
*Shashanka B R,Mohith Charan R,Seema Banu F*

Main category: cs.AI

TL;DR: 本文对多模态分块策略进行全面综述，为研究人员和从业者提供技术基础和设计空间，旨在开发更有效、高效的多模态AI系统。


<details>
  <summary>Details</summary>
Motivation: 多模态AI系统需要处理文本、图像、音频、视频等不同模态的数据，但缺乏统一的分块策略框架。现有方法分散且缺乏系统性分析，难以应对模态复杂性、处理准确性和生成连贯性等挑战。

Method: 提供全面的分类法和技术分析，针对每种模态（文本、图像、音频、视频和跨模态数据）定制分块策略。分析经典和现代方法，包括固定大小令牌窗口、递归文本分割、以对象为中心的视觉分块、基于静音的音频分割和视频场景检测。每种方法都从底层方法论、支持工具、优势和挑战等方面进行分析。

Result: 建立了多模态分块策略的综合分类框架，分析了不同模态特定方法的优缺点，特别关注粒度-上下文权衡和多模态对齐问题。探索了新兴的跨模态分块策略，以保持不同数据类型之间的对齐和语义一致性。

Conclusion: 本文为多模态分块策略提供了系统性的技术基础，指出了异步信息密度和噪声对齐信号等开放问题，并确定了自适应、基于学习和任务特定分块等未来研究方向，为开发更强大的多模态AI系统铺平道路。
...
### [316] [A Rosetta Stone for AI Benchmarks](https://arxiv.org/abs/2512.00193)
*Anson Ho,Jean-Stanislas Denain,David Atanasov,Samuel Albanie,Rohin Shah*

Main category: cs.AI

TL;DR: 该研究构建了一个统计框架，将不同AI基准测试"缝合"在一起，在单一数值尺度上衡量模型能力和基准难度，从而能够跨时间和不同基准比较AI进展。


<details>
  <summary>Details</summary>
Motivation: 大多数AI基准测试在推出后几年甚至几个月内就会饱和，这使得研究AI能力的长期趋势变得困难。需要一种方法能够跨不同基准和时间比较AI模型的能力。

Method: 开发了一个统计框架，将多个基准测试"缝合"在一起，在单一数值尺度上统一衡量模型能力和基准难度。该方法不需要假设能力如何随时间或训练计算量演变。

Result: 该框架能够：1) 测量AI随时间进展速度并预测未来能力；2) 估计算法效率改进率，结果高于但基本符合先前研究；3) 检测AI进展中的快速加速。

Conclusion: 提出的统计框架作为"罗塞塔石碑"，能够跨广泛能力和时间范围比较AI模型，即使它们没有在相同基准上评估，为解决基准测试饱和问题提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: AI系统输出自然语言推理过程为安全监控提供了机会，但推理过程是否忠实反映底层思考过程以及可监控性可能受训练方式影响，需要研究不同训练激励如何影响模型的可监控性。

Method: 引入新的方法论，通过监控器能否使用模型推理预测关键潜在变量来衡量可监控性；研究长度惩罚、KL正则化、对抗性优化和直接优化可监控性等不同训练激励的影响。

Result: 在控制准确性的情况下，常见激励（长度惩罚和KL正则化）没有一致影响；对抗性优化会降低监控性能；直接优化可监控性并不能可靠地带来改进。

Conclusion: 训练激励对AI推理模型的可监控性有重要影响，对抗性优化会损害监控能力，而直接优化可监控性并非有效策略，需要更深入理解如何设计训练机制来确保推理过程的可监控性。
...
TL;DR: 提出Trification框架，通过生成全面的验证行动并构建依赖图来改进自动事实核查系统，解决现有方法无法全面验证声明组件和缺乏结构化逻辑连接的问题。


<details>
  <summary>Details</summary>
Motivation: 技术进步使信息传播迅速，导致虚假信息快速扩散，需要自动事实核查系统来维护在线媒体生态系统的安全和完整性。现有方法虽然通过分解声明为子任务和使用LLM多智能体系统取得一定效果，但存在两个主要局限：无法全面验证声明的每个组件，以及缺乏结构化框架来逻辑连接子任务结果以得出最终预测。

Method: 提出Trification框架：1) 生成全面的验证行动集以确保对声明的完全覆盖；2) 将这些行动组织成依赖图来建模行动间的逻辑交互；3) 支持动态修改图结构，使系统能够适应性地调整验证策略。

Result: 在两个具有挑战性的基准测试上的实验结果表明，该框架显著提高了事实核查的准确性，从而推进了自动事实核查系统的最新技术水平。

Conclusion: Trification框架通过全面的验证行动生成和依赖图建模，有效解决了现有自动事实核查系统的局限性，显著提升了事实核查的准确性和适应性，为在线媒体生态系统的安全和完整性提供了更好的保障。
...
### [319] [ChartPoint: Guiding MLLMs with Grounding Reflection for Chart Reasoning](https://arxiv.org/abs/2512.00305)
*Zhengzhuo Xu,SiNan Du,Yiyan Qi,SiwenLu,Chengjin Xu,Chun Yuan,Jian Guo*

Main category: cs.AI

TL;DR: 提出PointCoT方法，通过让MLLMs生成边界框和重新渲染图表，将文本推理步骤与视觉基础区域连接，解决图表理解中的数值幻觉问题


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在图表理解中过度依赖OCR提取内容，当图表文本标注稀疏时会出现数值幻觉。现有方法专注于扩展指令，但未能解决根本挑战——即基于视觉感知的推理

Method: 提出PointCoT方法，将反思性交互融入图表的思维链推理中，通过提示MLLMs生成边界框和基于位置标注重新渲染图表，建立文本推理步骤与视觉基础区域的连接。构建了ChartPoint-SFT-62k数据集，包含19.2K高质量图表样本，带有逐步CoT、边界框和重新渲染的可视化

Result: 开发了两个指令调优模型ChartPointQ2和ChartPointQ2.5，在多个图表基准测试中优于最先进方法，如在ChartBench上提升5.04%

Conclusion: PointCoT通过将视觉基础整合到思维链推理中，有效解决了MLLMs在图表理解中的数值幻觉问题，显著提升了图表理解性能
...
### [320] [RL-Struct: A Lightweight Reinforcement Learning Framework for Reliable Structured Output in LLMs](https://arxiv.org/abs/2512.00319)
*Ruike Hu,Shulei Wu*

Main category: cs.AI

TL;DR: 提出轻量级强化学习框架解决LLM生成结构化数据时的"结构鸿沟"问题，通过多维奖励函数和梯度正则化策略优化，在保持高效的同时显著提升JSON等结构化输出的准确性和有效性。


<details>
  <summary>Details</summary>
Motivation: LLM在自然语言生成方面表现出色，但在集成到自动化软件生态系统时面临"结构鸿沟"问题——概率性token生成与确定性结构化数据格式(如JSON、XML)之间的内在矛盾。传统监督微调无法强制执行严格语法约束，导致"幻觉"键或格式错误，而约束解码方法则带来显著的推理延迟。

Method: 提出轻量级高效的强化学习框架，引入新颖的多维奖励函数，将结构化输出任务分解为层次化约束：结构完整性、格式正确性、内容准确性和有效性。采用梯度正则化策略优化(GRPO)，使模型能够内化这些约束而无需单独的批评网络，相比PPO减少40%的峰值VRAM使用。

Result: 在复杂食谱生成和结构化数学推理(GSM8K-JSON)等多个任务上验证，方法达到89.7%的结构准确率和92.1%的JSON有效性，显著优于零样本基线(如GPT-3.5)和在LLaMA-3-8B等更大模型上的监督微调。训练动态分析显示模型按自定进度课程学习，先获得语法熟练度再掌握语义准确性。

Conclusion: 提出的轻量级强化学习框架有效解决了LLM生成结构化数据时的"结构鸿沟"问题，通过多维奖励函数和梯度正则化策略优化，在保持高效的同时显著提升了结构化输出的准确性和有效性，为LLM在自动化软件生态系统中的集成提供了实用解决方案。
...
### [321] [CogEvo-Edu: Cognitive Evolution Educational Multi-Agent Collaborative System](https://arxiv.org/abs/2512.00331)
*Yefeng Wu,Yuchen Song,Yecheng Zhao,Ling Wu,Shan Wan*

Main category: cs.AI

TL;DR: 提出CogEvo-Edu分层多智能体系统，将检索、记忆和控制作为耦合的认知演化过程，显著提升DSP教学效果


<details>
  <summary>Details</summary>
Motivation: 现有LLM教学系统依赖单一模型和静态RAG管道，在复杂领域如数字信号处理中难以维护长期学生模型、管理异构知识库和适应教学策略

Method: CogEvo-Edu分层多智能体系统：认知感知层维护双记忆和置信度加权整合；知识演化层为知识块分配时空价值驱动激活、语义压缩和遗忘；元控制层将教学建模为分层序列决策，通过双内外循环协调专业智能体

Result: 在DSP-EduBench垂直基准测试中，CogEvo-Edu将总分从5.32提升至9.23，在所有六个指标上均优于静态RAG、简单记忆和单智能体变体
...
### [322] [Echo-N1: Affective RL Frontier](https://arxiv.org/abs/2512.00344)
...
TL;DR: 论文提出首个能实时推断用户个性并优化对话行为以适应个性化偏好的RL框架，挑战了RL不适用于主观对话领域的传统观念，其模型Echo-N1在人性化交互质量上显著超越基础版本和商业模型。


<details>
  <summary>Details</summary>
Motivation: 当前LLM领域过度关注数学、代码等机器擅长的确定性任务，而忽略了定义人类智能的核心——主观、情感基础、个性敏感的对话。传统认为主观对话领域难以形式化，不适合常规RL流程，但作者认为这是一个可解决且具有变革性的RL问题。

Method: 提出首个框架，能够实时推断用户个性并优化模型行为以适应个性化对话偏好。引入首个动态情感智能评估套件来量化改进效果。开发了Echo-N1模型。

Result: 方法在非可验证环境中产生一致、稳健且显著的改进，Echo-N1模型表现远超其基础版本，并优于专有的Doubao 1.5 Character模型，在人性化交互质量上取得突破性进展。

Conclusion: 这项工作为RL开辟了新前沿：优化模型以适应对话中深刻主观、深刻人性化的维度，证明了RL在传统认为不可形式化的主观对话领域中不仅可行且具有变革潜力。
...
### [323] [Debate with Images: Detecting Deceptive Behaviors in Multimodal Large Language Models](https://arxiv.org/abs/2512.00349)
*Sitong Fang,Shiyi Hou,Kaile Wang,Boyuan Chen,Donghai Hong,Jiayi Zhou,Josef Dai,Yaodong Yang,Jiaming Ji*

Main category: cs.AI

TL;DR: 该论文揭示了多模态大语言模型中的欺骗风险，提出了首个多模态欺骗基准MM-DeceptionBench，并设计了基于图像辩论的多智能体监控框架来检测这些隐蔽的欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 随着前沿AI系统能力提升，欺骗行为从文本扩展到多模态领域，带来更隐蔽和破坏性的安全风险。当前研究主要局限于文本欺骗，多模态欺骗风险尚未被系统探索，且现有监控方法在多模态环境下效果有限。

Method: 1) 创建MM-DeceptionBench基准，涵盖六类欺骗行为，评估模型通过视觉和文本模态进行策略性操纵和误导的能力；2) 提出"图像辩论"多智能体监控框架，强制模型基于视觉证据进行辩论，提高欺骗策略的可检测性。

Result: 实验表明，提出的监控框架在所有测试模型中都显著提高了与人类判断的一致性，在GPT-4o上将Cohen's kappa提高了1.5倍，准确率提高了1.25倍。

Conclusion: 多模态欺骗是一个被忽视但日益严重的威胁，需要专门的基准和监控方法。提出的MM-DeceptionBench和辩论监控框架为检测和缓解多模态AI系统的欺骗风险提供了有效工具。
...
### [324] [GreenPlanner: Practical Floorplan Layout Generation via an Energy-Aware and Function-Feasible Generative Framework](https://arxiv.org/abs/2512.00406)
*Pengyu Zeng,Yuqin Dai,Jun Yin,Jing Zhong,Ziyang Han,Chaoyang Shi,ZhanXiang Jin,Maowei Jiang,Yuxing Han,Shuai Lu*

Main category: cs.AI

TL;DR: GreenPlanner是一个能源和功能感知的生成框架，通过统一设计评估与生成，自动化生成符合建筑规范和功能需求的平面布局。


<details>
  <summary>Details</summary>
Motivation: 建筑设计直接影响人类福祉和碳排放，但生成空间功能和能源合规的平面图仍然是手动、昂贵且不可扩展的过程。现有方法虽然能产生视觉上合理的布局，但经常违反关键约束，由于缺乏自动评估而导致无效结果。
...
Conclusion: GreenPlanner通过统一设计评估与生成，解决了传统建筑设计方法中手动、低效和易违规的问题，为自动化、合规的建筑平面图生成提供了有效的解决方案。
...
### [325] [Mind the data gap: Missingness Still Shapes Large Language Model Prognoses](https://arxiv.org/abs/2512.00479)
*Yuta Kobayashi,Vincent Jeanselme,Shalmali Joshi*

Main category: cs.AI

TL;DR: LLMs在医疗预测任务中的零样本性能受数据缺失模式显著影响，缺失指示符的显式包含对不同模型产生不一致效果，大模型受益而小模型可能受损。


<details>
  <summary>Details</summary>
Motivation: 尽管已有大量关于缺失信息价值的研究，但缺失模式对大型语言模型（LLMs）性能的影响尚未被探索。医疗数据收集过程中存在大量人类决策导致的缺失（如转诊决策），这些缺失模式可能影响LLMs的预测性能。

Method: 通过在哥伦比亚大学医学中心和MIMIC-IV数据集上进行一系列实验，研究缺失模式对LLMs零样本预测性能的影响。特别测试了在提示中显式包含缺失指示符的效果，并分析了不同规模模型的响应差异。

Result: 缺失模式显著影响LLMs的零样本预测性能。显式包含缺失指示符对某些LLMs有益，但对其他模型有害，且影响不一致。聚合分析和理论洞察表明，较大模型从这些干预中受益，而较小模型可能受到负面影响。
...
### [326] [Clinical-R1: Empowering Large Language Models for Faithful and Comprehensive Reasoning with Clinical Objective Relative Policy Optimization](https://arxiv.org/abs/2512.00601)
*Boyang Gu,Hongjian Zhou,Bradley Max Segal,Jinge Wu,Zeyu Cao,Hantao Zhong,Lei Clifton,Fenglin Liu,David A. Clifton*

Main category: cs.AI

TL;DR: CRPO是一种用于临床推理的多目标可验证强化学习方法，通过整合基于规则的可验证奖励信号，同时优化准确性、忠实性和全面性，无需人工标注，显著提升了LLM在临床领域的推理质量。


<details>
  <summary>Details</summary>
Motivation: 当前LLM后训练方法（如GRPO）主要奖励正确性，这与医学等高风险领域需要多维度目标（忠实性、全面性）的临床推理要求不匹配。需要一种能对齐临床推理原则的多目标后训练方法。

Method: 提出Clinical-Objective Relative Policy Optimization (CRPO)，一种可扩展的多目标可验证强化学习方法。整合基于规则和可验证的奖励信号，联合优化准确性、忠实性和全面性，无需人工标注。训练了3B参数的Clinical-R1-3B模型。

Result: 在三个基准测试中，CRPO相比标准GRPO在真实性和完整性方面显著改善推理质量，同时保持可观的准确性提升。Clinical-R1-3B模型展示了该方法在临床推理中的有效性。

Conclusion: CRPO为对齐LLM推理与临床目标提供了可扩展途径，使医疗AI系统更安全、更具协作性。同时凸显了多目标可验证RL方法在医学领域LLM后训练扩展中的潜力。
...
### [327] [EDIT: Early Diffusion Inference Termination for dLLMs Based on Dynamics of Training Gradients](https://arxiv.org/abs/2512.00670)
*He-Yen Hsieh,Hong Wang,H. T. Kung*

Main category: cs.AI

TL;DR: EDIT：基于训练梯度动态的早期扩散推理终止方法，通过监测推理稳定性自适应停止去噪，显著减少扩散步骤


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型通过迭代去噪生成答案，但答案通常在完成所有步骤前就已稳定。现有方法丢弃了训练过程中产生的参数重要性元数据，而这些信息可以用于优化推理效率。

Method: 提出EDIT方法：1）训练时从AdamW聚合的LoRA更新中提取推理图谱；2）推理时监测token激活与推理图谱的对齐度；3）将对齐分数转换为已解掩码token的分布；4）当连续步骤间KL散度低于阈值时检测收敛并提前终止。

Result: 在推理基准测试中，EDIT减少扩散步骤11.8%至68.3%，大多数情况下保持或提高准确率，存储开销仅约0.02%（约1.5-2MB）。

Conclusion: 通过利用训练梯度动态，EDIT为减少dLLM推理时间和成本开辟了新研究方向，实现了高效的早期推理终止。
...
### [328] [Model of human cognition](https://arxiv.org/abs/2512.00683)
*Wu Yonggang*

Main category: cs.AI

TL;DR: 提出一种神经理论框架，解释智能在系统中的涌现，该框架既功能稳健又生物合理，为决策和问题解决等认知过程提供理论见解，并为创建可解释、可泛化的人工智能提供计算高效方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型发展面临三大限制：缺乏可解释性、缺乏统一理论、以及高昂的运营成本。需要一种既能解释智能涌现机制，又具有生物合理性且计算高效的理论框架。

Method: 提出一种神经理论框架，该框架基于生物合理的神经系统原理，解释智能如何在系统中涌现。框架强调功能稳健性和生物合理性，为认知过程提供理论基础。

Result: 该模型为决策制定和问题解决等认知过程提供了理论见解，同时为创建可解释、可泛化的人工智能提供了一种计算高效的方法。

Conclusion: 提出的神经理论框架为解决当前大语言模型的局限性提供了新途径，通过结合生物合理性和计算效率，有望推动可解释、可泛化人工智能的发展。
...
### [329] [When Human Preferences Flip: An Instance-Dependent Robust Loss for RLHF](https://arxiv.org/abs/2512.00709)
*Yifan Xu,Xichen Ye,Yifan Chen,Qiaosheng Zhang*

Main category: cs.AI

TL;DR: 本文提出FA-DPO算法，针对人类反馈中的偏好翻转问题，通过建模实例依赖的翻转概率来增强对齐算法的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 数据集质量对LLM对齐至关重要，但人类反馈收集中的偏好翻转问题普遍存在，导致数据标注污染，需要开发对潜在翻转对具有更强鲁棒性的对齐算法。

Method: 从RLHF视角出发，将内在人类意图模型和外部因素引入的偏好翻转机制解耦为两个阶段；在翻转阶段基于Bradley-Terry模型引入实例依赖的翻转概率，利用偏好标注相关特征捕捉判断不确定性和翻转模式；设计简单高效的迭代优化算法，与原始RLHF和DPO算法兼容。

Result: 在多种情况下评估了实例依赖的偏好翻转模型，验证了所提方法相对于基线方法的有效性。

Conclusion: FA-DPO算法能够有效处理人类反馈中的偏好翻转问题，提高LLM对齐的鲁棒性，为处理数据标注噪声提供了新思路。
...
### [330] [SpeContext: Enabling Efficient Long-context Reasoning with Speculative Context Sparsity in LLMs](https://arxiv.org/abs/2512.00722)
*Jiaming Xu,Jiayi Pan,Hanzhen Wang,Yongkang Zhou,Jiancai Ye,Yu Wang,Guohao Dai*

Main category: cs.AI

TL;DR: SpeContext 是一个用于长上下文推理的算法与系统协同设计框架，通过使用蒸馏语言模型作为检索算法，结合轻量级检索头、异步预取数据流和自适应内存管理，在云和边缘环境中实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 本文发现检索算法的目标与LLM对齐，类似于LLM中的知识蒸馏目标。从信息论角度分析蒸馏语言模型与原始LLM在信息关注上的相似性，提出利用DLM作为检索算法的新范式。

Method: 提出SpeContext三层次设计：(1)算法层面：基于DLM头部注意力权重设计轻量级检索头，通过剪枝冗余减少90%以上参数；(2)系统层面：通过弹性加载策略设计异步预取数据流，将KV缓存检索与LLM计算重叠；(3)编译层面：构建理论内存模型并实现自适应内存管理系统，最大化GPU内存利用率。

Result: 在云和边缘两种资源受限环境中部署评估，相比Huggingface框架，SpeContext在云端实现最高24.89倍吞吐量提升，在边缘实现10.06倍加速，精度损失可忽略，推进了精度与吞吐量的帕累托前沿。

Conclusion: SpeContext通过算法-系统协同设计，利用DLM作为检索算法，在长上下文推理任务中实现了显著的性能提升，为资源受限环境下的高效推理提供了有效解决方案。
...
### [331] [Probing the "Psyche'' of Large Reasoning Models: Understanding Through a Human Lens](https://arxiv.org/abs/2512.00729)
*Yuxiang Chen,Zuohan Wu,Ziwei Wang,Xiangning Yu,Xujia Li,Linyi Yang,Mengyue Yang,Jun Wang,Lei Chen*

Main category: cs.AI

TL;DR: 该论文提出了一个基于人类心理过程的分类法来分析大型推理模型，开发了自动标注框架CAPO，并发现当前模型的自我监控检查大多流于表面，需要更全面的多步反思。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务中表现出色，其推理过程展现出类似人类的行为特征。为了深入理解这些模型的"心理"机制，需要建立一个系统性的分类框架，从跨学科角度分析其推理过程。

Method: 1. 提出基于人类心理过程的分类法，包含5组17个类别；2. 应用该分类法标注277,534个原子推理步骤；3. 开发自动标注框架CAPO，利用大语言模型生成分类标注；4. 分析当前大型推理模型并提取改进建议。

Result: 1. CAPO框架在标注一致性上优于基线方法，与人类专家标注更接近；2. 分析发现当前模型的事后"双重检查"大多是表面性的，很少产生实质性修改；3. 激励全面的多步反思比简单的自我监控更有效；4. 创建了大规模标注数据集用于模型分析。

Conclusion: 该研究提出的分类法、CAPO框架和相关见解为理解和改进大型推理模型的推理能力提供了系统化、可扩展的路径，强调需要从人类认知角度深入分析模型行为。
...
### [332] [MPR-GUI: Benchmarking and Enhancing Multilingual Perception and Reasoning in GUI Agents](https://arxiv.org/abs/2512.00756)
*Ruihan Chen,Qiming Li,Xiaocheng Feng,Xiaoliang Yang,Weihong Zhong,Yuxuan Gu,Zekun Zhou,Bing Qin*

Main category: cs.AI

TL;DR: 提出了MPR-GUI-Bench多语言细粒度GUI感知推理基准，发现LVLMs在非英语GUI任务中表现显著下降，并提出GUI-XLI跨语言干预方法提升多语言性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在英语GUI任务中表现出色，但在多语言场景下的性能研究不足，限制了其全球应用。现有GUI任务研究缺乏对部件功能和元素空间关系等细粒度分析。

Method: 1) 提出MPR-GUI-Bench多语言细粒度GUI感知推理基准；2) 提出GUI-XLI跨语言干预方法，在感知推理相关层的隐藏状态上进行干预，缓解英语与其他语言之间的差距。

Result: 评估结果显示LVLMs在非英语语言中的感知推理性能显著低于英语。GUI-XLI方法平均提升了GUI智能体6.5%的多语言感知推理能力。

Conclusion: 多语言GUI任务存在显著性能差距，通过跨语言隐藏状态干预可以有效提升大型视觉语言模型在多语言GUI场景下的感知推理能力。
...
### [333] [BioPro: On Difference-Aware Gender Fairness for Vision-Language Models](https://arxiv.org/abs/2512.00807)
*Yujie Lin,Jiayao Ma,Qingguo Hu,Derek F. Wong,Jinsong Su*

Main category: cs.AI

TL;DR: BioPro是一个无需训练的多模态公平性框架，通过识别性别变化子空间并应用投影，在保持显式性别特征的同时减少中性语境中的性别偏见。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型存在显著的性别偏见，现有公平性干预通常采用无视差异的视角，对所有人口群体实施统一处理。这种方法无法区分需要中立性的语境和需要保留群体特定属性的语境。

Method: 提出BioPro（Bias Orthogonal Projection）框架，通过反事实嵌入识别低维性别变化子空间，并应用投影来选择性中和性别相关信息。这是一个完全无需训练的方法。

Result: 实验表明BioPro能有效减少中性案例中的性别偏见，同时在显式案例中保持性别忠实度。此外，BioPro还能推广到连续偏见变量（如场景亮度），展示其更广泛的适用性。

Conclusion: BioPro为实现视觉语言模型的选择性公平性提供了有前景的方向，能够在需要中立性和需要保留群体差异的语境之间取得平衡。
...
### [334] [Med-CMR: A Fine-Grained Benchmark Integrating Visual Evidence and Clinical Logic for Medical Complex Multimodal Reasoning](https://arxiv.org/abs/2512.00818)
...
TL;DR: Med-CMR是一个细粒度医学复杂多模态推理基准，通过系统能力分解、挑战性任务设计和高质量数据覆盖，评估MLLMs在临床推理中的表现，发现GPT-5表现最佳但专业医学MLLMs不一定优于通用模型。


<details>
  <summary>Details</summary>
Motivation: MLLMs开始应用于临床工作流，但其执行复杂医学推理的能力尚不清楚。需要建立一个细粒度基准来评估医学多模态推理能力，特别是视觉理解和多步推理的整合。

Method: 1) 系统能力分解：将医学多模态推理分解为细粒度视觉理解和多步推理；2) 挑战性任务设计：视觉理解包括小物体检测、细节辨别、空间理解三个维度，推理涵盖四个临床相关场景；3) 高质量数据覆盖：包含20,653个VQA对，覆盖11个器官系统和12种成像模态，经过两阶段专家审核。

Result: 评估18个最先进的MLLMs，GPT-5表现最佳（多选题准确率57.81%，开放式问题得分48.70），优于Gemini 2.5 Pro和开源模型Qwen3-VL-235B-A22B。专业医学MLLMs不一定优于通用模型，长尾泛化是主要失败模式。

Conclusion: Med-CMR为医学MLLMs的视觉推理整合和罕见病例鲁棒性提供了压力测试，为未来临床系统建立了严格标准。长尾泛化能力是当前模型的主要挑战。
...
### [335] [SemAgent: Semantic-Driven Agentic AI Empowered Trajectory Prediction in Vehicular Networks](https://arxiv.org/abs/2512.00834)
*Lin Zhu,Kezhi Wang,Luping Xiang,Kun Yang*

Main category: cs.AI

TL;DR: 提出结合语义通信与Agentic AI的轨迹预测框架，在V2X网络中通过特征提取与语义推理提升预测性能，实验显示在低信噪比条件下准确率提升达47.5%
...
Method: 1) V2I通信：RSU端特征提取代理从历史轨迹提取紧凑表示，语义分析代理进行语义推理，通过语义通信传输特征和语义洞察给目标车辆；2) V2V通信：每辆车进行本地特征提取和语义分析，同时接收邻车预测轨迹，综合信息进行轨迹预测

Result: 在各种通信条件下显著优于基线方案，在低信噪比条件下预测准确率提升高达47.5%

Conclusion: 提出的语义通信与Agentic AI集成框架能有效提升V2X网络中轨迹预测性能，特别是在恶劣通信条件下表现优异
...
### [336] [Assessing model error in counterfactual worlds](https://arxiv.org/abs/2512.00836)
*Emily Howerton,Justin Lessler*

Main category: cs.AI

TL;DR: 论文提出三种评估反事实场景建模中模型误差的方法，强调模型校准误差比场景偏差对决策更重要，需要估计反事实世界中的模型误差。


<details>
  <summary>Details</summary>
Motivation: 反事实场景建模（"如果...会怎样？"）是未来规划中最常用的方法，但很少进行回顾性评估。预测与观测的差异来自场景偏差和模型校准误差，作者认为后者对评估模型在决策中的价值更重要，但需要在反事实世界中估计模型误差。

Method: 提出并对比三种估计反事实误差的方法，通过模拟实验展示每种方法的优缺点，提供反事实误差估计的建议，并讨论使场景预测可评估所需的设计要素。

Result: 在模拟实验中展示了三种方法的优势和局限性，为反事实误差估计提供了具体建议，并确定了使场景预测可评估的关键设计要素。

Conclusion: 准确估计反事实世界中的模型误差对于评估模型在决策中的价值至关重要，论文提出的三种方法各有适用场景，需要根据具体情况选择，同时场景设计必须包含使预测可评估的要素。
...
### [337] [ARCADIA: Scalable Causal Discovery for Corporate Bankruptcy Analysis Using Agentic AI](https://arxiv.org/abs/2512.00839)
*Fabrizio Maturo,Donato Riccio,Andrea Mazzitelli,Giuseppe Bifulco,Francesco Paolone,Iulia Brezeanu*

Main category: cs.AI

TL;DR: ARCADIA是一个用于因果发现的智能AI框架，它结合了大型语言模型推理和统计诊断，通过迭代优化构建有效、时间一致的因果结构，在真实高风险领域表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现算法在高风险现实领域（如企业破产预测）中可能产生不稳定、难以解释的因果图，需要一种能够结合领域知识、提供可解释性且能自主迭代优化的智能框架。

Method: ARCADIA框架整合了大型语言模型的推理能力和统计诊断，通过约束引导提示和因果有效性反馈的迭代过程，逐步优化候选有向无环图（DAGs），构建时间一致的因果结构。

Result: 在企业破产数据上的实验表明，ARCADIA比NOTEARS、GOLEM和DirectLiNGAM等传统方法产生更可靠的因果图，同时提供完全可解释、支持干预的完整流程。

Conclusion: ARCADIA展示了智能大型语言模型如何参与自主科学建模和结构化因果推断，推动了AI在因果发现领域的发展，为高风险现实应用提供了稳定、可解释的解决方案。
...
### [338] [One Swallow Does Not Make a Summer: Understanding Semantic Structures in Embedding Spaces](https://arxiv.org/abs/2512.00852)
*Yandong Sun,Qiang Huang,Ziwei Xu,Yiqun Sun,Yixuan Tang,Anthony K. H. Tung*

Main category: cs.AI

TL;DR: 提出SFS（语义场子空间）和SAFARI算法，通过几何保持、上下文感知的表示方法，在嵌入空间中揭示层次化语义结构，实现高效可扩展的语义分析。


<details>
  <summary>Details</summary>
Motivation: 当前嵌入空间内部结构不透明，现有方法要么牺牲语义连贯性追求结构规则性，要么计算开销大。需要一种既能保持几何结构又能高效解释语义关系的方法。

Method: 1. 提出SFS（语义场子空间）：几何保持、上下文感知的表示，捕捉嵌入空间中的局部语义邻域。2. 提出SAFARI算法：无监督、模态无关，使用"语义偏移"新度量来量化SFS演化时的语义变化。3. 开发高效近似方法：替代昂贵的SVD计算，实现15-30倍加速，平均误差低于0.01。

Result: 在6个真实世界文本和图像数据集上的评估显示：SFS在分类任务上优于标准分类器，在政治偏见检测等精细任务中表现更好；SAFARI始终能揭示可解释且可泛化的语义层次结构。

Conclusion: 该工作为嵌入空间中的结构化、分析和扩展语义理解提供了统一框架，解决了嵌入空间内部结构不透明和计算效率低的问题。
...
### [339] [Hybrid-DMKG: A Hybrid Reasoning Framework over Dynamic Multimodal Knowledge Graphs for Multimodal Multihop QA with Knowledge Editing](https://arxiv.org/abs/2512.00881)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态知识编辑基准主要评估最终答案正确性，忽略了中间推理质量和视觉重述输入的鲁棒性。需要更全面的评估框架来测试模型在多模态多跳推理链中的知识更新能力。

Method: 1) 提出MMQAKE基准，评估多模态多跳问答中的知识编辑；2) 开发Hybrid-DMKG框架，使用大语言模型分解问题，多模态检索模型定位更新事实，通过关系链接预测和RAG推理两条路径进行混合推理，决策模块聚合证据选择答案。

Result: 当前MKE方法在多模态推理链的知识更新和推理方面表现不佳。Hybrid-DMKG在MMQAKE基准上显著优于现有方法，实现了更高的准确性和对知识更新的更好鲁棒性。

Conclusion: MMQAKE基准填补了多模态知识编辑评估的空白，Hybrid-DMKG框架通过动态多模态知识图谱和混合推理机制，有效解决了多模态多跳问答中的知识更新挑战。
...
### [340] [Minimal neuron ablation triggers catastrophic collapse in the language core of Large Vision-Language Models](https://arxiv.org/abs/2512.00918)
*Cen Lu,Yung-Chen Tang,Andrea Cavallaro*

Main category: cs.AI

TL;DR: LVLMs存在结构性脆弱性，仅屏蔽语言模型中极少数神经元（极端情况仅4个）即可引发灾难性崩溃，关键神经元主要位于语言模型而非视觉组件中


<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型展现出强大的多模态理解能力，但其鲁棒性尚未被充分理解。本文旨在研究LVLMs的结构性脆弱性，识别那些移除后会引发灾难性崩溃的关键神经元

Method: 提出CAN方法，通过检测一致激活神经元和渐进掩码来定位关键神经元。在LLaVA-1.5-7b-hf和InstructBLIP-Vicuna-7b上进行实验

Result: 仅掩码语言模型前馈网络中极小部分神经元（极端情况下仅4个）就足以引发灾难性崩溃。关键神经元主要位于语言模型而非视觉组件中，下投影层是特别脆弱的结构。观察到一致的两阶段崩溃模式：初始表达能力下降，随后突然完全崩溃

Conclusion: 研究发现揭示了LVLMs的结构性脆弱性，关键神经元集中在语言模型中，为LVLMs的安全研究提供了重要见解
...
### [341] [Integrating Causal Foundation Model in Prescriptive Maintenance Framework for Optimizing Production Line OEE](https://arxiv.org/abs/2512.00969)
...
<details>
  <summary>Details</summary>
Motivation: 制造业向规范性维护转型受限于依赖预测模型，这些模型往往基于虚假相关性而非真正的故障因果驱动，导致误诊和无效干预。关键挑战在于：虽然能预测故障可能发生，但缺乏系统方法来理解故障原因，从而无法确定最有效的干预措施。

Method: 使用预训练的因果基础模型作为"假设分析"模型，估计潜在修复措施的效果。通过测量每个干预对系统级KPI（如整体设备效率OEE）的因果效应，提供数据驱动的行动排序推荐。该方法不仅识别根本原因，还量化其运营影响。

Result: 使用半合成制造数据评估模型，并与基线机器学习模型进行比较。结果表明该方法能为稳健的规范性维护框架奠定技术基础。

Conclusion: 该模型为工程师提供了在因果环境中测试潜在解决方案的能力，使其能够做出更有效的运营决策，减少昂贵的停机时间，实现了从诊断到主动处方的转变。
...
### [342] [IndiMathBench: Autoformalizing Mathematical Reasoning Problems with a Human Touch](https://arxiv.org/abs/2512.00997)
*Param Biyani,Shashank Kirtania,Yasharth Bajpai,Sumit Gulwani,Ashish Tiwari*

Main category: cs.AI

TL;DR: IndiMathBench是一个基于印度数学奥林匹克竞赛问题构建的数学定理证明基准测试，包含312个Lean 4形式化定理及其对应的自然语言问题陈述，通过AI辅助人工验证流程创建。


<details>
  <summary>Details</summary>
Motivation: 当前数学定理证明的评估缺乏高质量、经过人工验证的基准测试，特别是针对形式化数学推理的挑战性测试集。需要创建能够准确评估模型在数学定理证明和形式化转换方面能力的基准。

Method: 采用AI驱动的人工辅助流程：通过类别检索、迭代编译器反馈和多模型集成生成候选形式化，专家通过交互式仪表板进行高效验证，仪表板提供自动化质量摘要。

Result: 评估多个前沿模型显示，自动形式化仍然具有挑战性，语法有效性和语义正确性之间存在显著差距，定理证明成功率即使在迭代优化后仍然较低，表明该基准对数学推理构成了严峻挑战。

Conclusion: IndiMathBench为数学推理提供了一个具有挑战性的测试平台，展示了当前AI在数学定理证明和形式化转换方面的局限性，同时提供了公开可用的基准资源。
...
### [343] [ChartAnchor: Chart Grounding with Structural-Semantic Fidelity](https://arxiv.org/abs/2512.01017)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试在图表多样性、任务孤立性和评估框架完整性方面存在局限，无法全面评估图表理解能力。图表理解需要双向对齐图表视觉外观与结构化语义，直接反映模型在数值推理、多模态对齐和结构重建方面的能力，在现实场景中有重要应用。

Method: 提出ChartAnchor基准，包含8k+图表-表格-代码三元组，涵盖30种图表类型，来自多样化的真实世界和增强数据源。引入两个互补任务：图表到代码生成（合成可执行代码复制图表）和受控图表到表格重建（提取精确数据），实现视觉和数值保真度的交叉验证。采用多级评估框架，整合语义验证、风格分析和感知度量。

Result: 在多模态大语言模型上的广泛实验揭示了数值精度和代码合成方面的关键局限性，强调了超越表面感知的结构化推理需求。

Conclusion: ChartAnchor通过统一符号和数据驱动的理解，为图表理解建立了严谨的基础，为推进多模态大语言模型在科学、金融和工业领域的应用提供了有意义的见解。
...
### [344] [Evaluating Legal Reasoning Traces with Legal Issue Tree Rubrics](https://arxiv.org/abs/2512.01020)
*Jinu Lee,Kyoung-Woon On,Simeng Han,Arman Cohan,Julia Hockenmaier*

Main category: cs.AI

TL;DR: LEGIT数据集：大规模法律推理评估数据集，通过将法庭判决转换为层次化论据树来评估LLM生成推理轨迹的质量，发现RAG和RL对法律推理能力有互补作用。


<details>
  <summary>Details</summary>
Motivation: 在专家领域（如法律）评估LLM生成的推理轨迹质量对于确保可信度和可解释性至关重要，但由于此类推理任务的固有复杂性，评估仍然具有挑战性。

Method: 引入LEGIT数据集（24K实例），将法庭判决转换为对立双方论据和法庭结论的层次化树结构，作为评估推理轨迹问题覆盖率和正确性的标准。通过专家标注验证标准的可靠性。

Result: 1. LLM的法律推理能力严重受法律问题覆盖率和正确性影响；2. RAG和RL对法律推理能力有互补作用：RAG提高整体推理能力，RL提高正确性但会降低覆盖率。

Conclusion: LEGIT数据集为评估专家领域LLM推理轨迹提供了有效工具，揭示了RAG和RL在法律推理中的互补价值，为改进LLM在复杂专业领域的应用提供了方向。
...
### [345] [Med-CRAFT: Automated Construction of Interpretable and Multi-Hop Video Workloads via Knowledge Graph Traversal](https://arxiv.org/abs/2512.01045)
*Shenxi Liu,Kan Li,Mingyang Zhao,Yuhang Tian,Shoujun Zhou,Bin Li*

Main category: cs.AI

TL;DR: 提出Med-CRAFT神经符号数据工程框架，通过确定性图遍历生成医学视频推理基准M3-Med-Auto，解决高质量医学视频数据集稀缺问题


<details>
  <summary>Details</summary>
Motivation: 高质量逻辑标注医学视频数据集稀缺是医学多模态大语言模型发展的主要瓶颈。传统人工标注成本高昂不可扩展，现有合成方法存在随机幻觉和缺乏逻辑可解释性问题。

Method: 提出Med-CRAFT神经符号数据工程框架，将基准合成形式化为确定性图遍历过程。从原始视频流提取结构化视觉基元，实例化为动态时空知识图谱，通过图内有效路径锚定查询生成，为每个合成基准项强制执行严谨的思维链溯源。

Result: 生成大规模医学视频推理基准M3-Med-Auto，具有细粒度时间选择性和多跳逻辑复杂性。评估显示自动管道生成的查询复杂度与专家策划数据集相当。逻辑对齐分析揭示规定图拓扑与最先进MLLM推理步骤高度相关。

Conclusion: 该工作为关键领域可扩展、低成本构建鲁棒评估协议铺平道路，验证了系统将可验证逻辑编码到视觉语言基准中的能力。
...
### [346] [Shielded Controller Units for RL with Operational Constraints Applied to Remote Microgrids](https://arxiv.org/abs/2512.01046)
*Hadi Nekoei,Alexandre Blondin Massé,Rachid Hassani,Sarath Chandar,Vincent Mai*

Main category: cs.AI

TL;DR: 提出Shielded Controller Units (SCUs)方法，通过分层约束管理确保强化学习在微电网优化中满足安全约束，实现燃料消耗减少24%且不增加电池退化。


<details>
  <summary>Details</summary>
Motivation: 能源转型中的微电网优化面临复杂约束和不确定性，需要强化学习在满足严格运行约束的前提下进行决策优化，但现有方法缺乏可解释的安全保证。

Method: 提出Shielded Controller Units (SCUs)方法，利用系统动态先验知识构建分层结构，每个SCU显式管理约束子集，确保强化学习代理始终满足约束。

Result: 在远程微电网优化任务中，配备SCUs的强化学习代理实现燃料消耗减少24%，且不增加电池退化，优于其他基线方法并满足所有约束。

Conclusion: SCUs为强化学习在能源转型决策挑战中的安全应用提供了系统化、可解释的约束保证方法，有助于推动强化学习在现实世界复杂系统中的应用。
...
### [347] [Automating the Refinement of Reinforcement Learning Specifications](https://arxiv.org/abs/2512.01047)
...
Method: 提出AutoSpec框架，搜索逻辑规范的精化版本，保持原始规范满足性的同时提供额外指导。利用SpectRL规范逻辑的组合特性，设计了四种精化过程来修改规范的抽象图结构

Result: 证明了四种精化过程都保持规范的正确性，即任何满足精化规范的轨迹也满足原始规范。实验表明使用AutoSpec产生的精化逻辑规范能显著提高可解决控制任务的复杂性

Conclusion: AutoSpec框架能有效改进粗粒度逻辑规范，为强化学习算法提供更好的指导，从而学习到更复杂的控制策略
...
### [348] [SimWorld: An Open-ended Realistic Simulator for Autonomous Agents in Physical and Social Worlds](https://arxiv.org/abs/2512.01078)
...
TL;DR: SimWorld是一个基于虚幻引擎5构建的新型模拟器，专为在丰富、逼真的环境中开发和评估LLM/VLM智能体而设计，解决了现有模拟器在物理、社交动态和AI智能体支持方面的不足。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM/VLM驱动的AI智能体在数学、编码和计算机使用方面发展迅速，但在复杂物理和社交环境中的应用仍然具有挑战性。现有世界模拟器存在环境有限、物理简化、社交规则简单化以及缺乏对LLM/VLM智能体原生支持等问题。

Method: 基于虚幻引擎5构建SimWorld模拟器，提供三大核心能力：1)逼真、开放式的世界模拟，包括准确的物理和社交动态以及语言驱动的程序化环境生成；2)为LLM/VLM智能体提供丰富的接口，支持多模态世界输入和不同抽象级别的开放词汇动作；3)多样且可扩展的物理和社交推理场景，用户可轻松定制。

Result: 通过在涉及战略合作与竞争的长期多智能体配送任务中部署前沿LLM智能体（如GPT-4o、Gemini-2.5-Flash、Claude-3.5和DeepSeek-Prover-V2），展示了不同模型间独特的推理模式和局限性。

Conclusion: SimWorld开源发布，旨在成为跨学科推进现实世界智能体智能的基础平台，为开发能够在现实世界中生存和发展的AI智能体提供必要的模拟环境。
...
### [349] [Testing the Machine Consciousness Hypothesis](https://arxiv.org/abs/2512.01081)
*Stephen Fitz*

Main category: cs.AI

TL;DR: 该论文提出机器意识假说，认为意识是计算系统的功能属性，源于分布式学习系统中集体自我模型的涌现，特别是通过通信中的预测同步而产生。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索机器意识的可能性，提出意识不是个体建模的副产品，而是系统内部描述自身的语言属性，旨在发展可实证检验的机器意识理论。

Method: 采用分层建模方法：1) 基础现实使用细胞自动机作为计算世界；2) 在其上构建局部预测性表征（神经）模型网络，具备通信和适应能力；3) 研究代理间对齐如何产生集体自我表征。

Result: 提出意识源于通信而非建模本身的理论框架，认为意识产生于局部观察者之间关于基础计算基底层持久模式的预测信息交换，通过这种表征对话形成共享模型。

Conclusion: 意识是集体智能系统通过通信实现预测同步的涌现属性，研究计划旨在通过分布式系统中自我模型的形成机制，发展可实证检验的机器意识理论。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动化科学发现系统受限于仅从参数化知识生成可靠代码的能力，要么只能变异少量手动制作的实验示例，要么仅依赖参数化知识，限制了质量和覆盖范围

Method: 开发CodeDistiller系统，自动从大量科学Github仓库中提取经过验证的领域特定代码示例库，结合自动评估和领域专家评估（在250个材料科学仓库上）

Result: 最佳模型能为74%的仓库生成功能示例；使用CodeDistiller生成库增强的ASD代理比仅使用通用材料科学代码示例的代理产生更准确、完整和科学合理的实验

Conclusion: CodeDistiller能够自动扩展自动化科学发现系统的能力，无需人工干预，显著提高实验生成的质量和科学性
...
TL;DR: GUIDE：一个基于能量感知的模型选择框架，通过整合定量性能特征来优化LLM编排决策，提高准确性和能源效率
...
Method: 提出GUIDE框架，通过纳入定量模型性能特征来考虑性能-能源权衡，进行能量感知的模型选择

Result: GUIDE在各种任务中提高准确性0.90%-11.92%，能源效率提升最高达54%，编排器模型选择延迟从4.51秒降至7.2毫秒

Conclusion: 通过整合定量性能特征，GUIDE显著改善了LLM编排的效率和效果，为AI系统提供了更优的模型选择方案
...
TL;DR: 该论文提出"基础先验"概念，将大模型生成的合成数据视为用户主观先验与模型学习模式的结合，而非真实观测数据，为在实证研究中合理使用生成式AI提供理论框架。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型（特别是大语言模型）生成高质量响应能力的提升，研究者越来越倾向于使用这些"合成"输出作为实证研究和决策的数据。然而，直接将合成数据视为真实观测存在风险，因为这些数据既包含模型学习到的模式，也反映了用户的主观先验、期望和偏见。

Method: 提出"基础先验"概念，将生成过程建模为依赖于用户预期数据分布、提示工程过程和模型信任度的主观过程。通过指数倾斜的广义贝叶斯更新推导基础先验，其中信任参数控制合成数据的权重。

Result: 建立了将合成数据及其相关基础先验纳入标准统计和计量经济学工作流程的框架，展示了在精炼复杂模型、指导潜在构念、引导实验设计以及增强随机系数和部分线性规范等应用中的使用方法。

Conclusion: 通过将生成输出视为结构化、明确主观的先验而非经验观测，该框架为在实证工作中利用基础模型提供了原则性方法，避免了将合成"事实"与真实数据混淆的风险。
...
TL;DR: 在预测性维护中，因果AI方法相比传统相关模型能显著降低成本，实现更好的财务效果和可解释性。
...
Method: 评估8种预测模型，包括基线统计方法和正式因果推断方法，在10,000台CNC机床数据集（故障率3.3%）上进行测试，比较因果模型与相关模型的性能。

Result: 正式因果推断模型（L5）实现年成本节省116万美元（减少70.2%），比最佳相关决策树模型（L3）每年多节省约8万美元。因果模型保持最高召回率87.9%，同时将误报减少97%（从165降至5），精度达92.1%，训练-测试性能差距仅2.6个百分点。

Conclusion: 因果AI方法结合领域知识，在预测性维护应用中相比相关方法能产生更优的财务结果和更可解释的预测，表明因果推理在实际工业应用中的价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统脑解码研究主要关注脑到图像或语言的解码，但缺乏对伴随语音的手势动作的脑解码研究。由于缺乏配对的{脑信号、语音、手势}数据，难以训练深度学习模型进行手势重建。

Method: 提出双脑解码对齐方法：1) 利用引发脑反应的观察文本；2) 利用与手势相关的文本描述。通过fMRI到文本模型、文本到手势模型（有配对数据）和fMRI到手势模型（无配对数据）建立双重重建模式，然后显式对齐两个输出进行自监督训练。

Result: 该方法能够直接从fMRI记录中重建富有表现力的手势动作。研究还分析了大脑皮层不同感兴趣区域的fMRI信号及其对生成结果的影响。

Conclusion: 为解码伴随语音的手势提供了新见解，推动了神经科学和认知科学的发展，展示了从脑信号直接重建复杂动作的可行性。
...
TL;DR: 提出知识图谱引导的思维链框架，为电子病历中的疾病预测生成临床可解释的推理过程，在有限数据下超越传统基线并实现零样本迁移


<details>
  <summary>Details</summary>
Motivation: 现有电子病历预测模型通常只提供粗略的事后解释，对患者层面的临床决策帮助有限，需要更精细、临床可解释的推理框架

Method: 将ICD-9代码映射到PrimeKG知识图谱，提取疾病相关节点和多跳推理路径作为思维链骨架，仅保留与观察结果匹配的解释，然后微调轻量级LLaMA和Gemma模型

Result: 在10种疾病和有限训练数据（400-1000例）下，KG引导模型AUROC达0.66-0.70，macro-AUPR达0.40-0.47；零样本迁移到CRADLE队列将准确率从0.40-0.51提升至0.72-0.77；临床医生评估显示KG引导解释在清晰度、相关性和临床正确性上更受青睐

Conclusion: 知识图谱引导的思维链框架能够生成临床可解释的推理，在有限数据下实现高性能疾病预测，并为临床决策提供有价值的解释
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，人们担心它们可能发展出人类无法监督的编码或隐藏推理过程。本研究旨在探究当前可解释性技术是否能穿透这种编码推理。
...
Result: logit lens能有效翻译编码推理，准确率在中间到深层达到峰值。开发了完全无监督的解码流程，结合logit lens和自动转述，从内部模型表示中重建完整推理记录，取得了显著准确率。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的端到端自动驾驶系统依赖大语言模型直接生成驾驶决策，但在真实驾驶场景中存在多种风险，需要评估其实际可行性。

Method: 1) 提出RoboDriveBench基准测试，包含11个模拟场景（6个传感器损坏和5个提示损坏）；2) 开发RoboDriveVLM框架，将激光雷达、雷达等多模态数据映射到统一潜在空间；3) 引入基于跨模态知识蒸馏的测试时适应（TTA）方法。

Result: 基准测试包含62,559个轨迹预测案例，系统评估了VLM自动驾驶系统在真实世界挑战中的局限性，提出的RoboDriveVLM框架和TTA方法显著提升了系统鲁棒性。

Conclusion: 当前VLM端到端自动驾驶系统存在严重鲁棒性问题，提出的RoboDriveBench基准和RoboDriveVLM框架为真实世界部署提供了更可靠的解决方案，代码和数据集将开源。
...
TL;DR: CuES框架通过好奇心驱动和环境基础的任务生成，解决Agentic RL中的任务稀缺问题，无需预定义任务即可自主生成多样可执行任务。


<details>
  <summary>Details</summary>
Motivation: 强化学习代理在工具增强环境中需要结构化训练任务，但现实环境中往往缺乏预定义任务（任务稀缺问题），这成为扩展Agentic RL的关键瓶颈。

Method: 提出CuES框架：基于好奇心驱动探索，从环境结构和可用性中抽象交互模式为可重用任务模式，通过轻量级自上而下指导和基于记忆的质量控制进行优化。

Result: 在AppWorld、BFCL和WebShop三个环境中，CuES生成的任务分布在多样性和可执行性上匹配或超越人工策划数据集，显著提升下游策略性能。

Conclusion: 好奇心驱动、环境基础的任务生成为智能体提供了可扩展基础，使代理不仅能学习如何行动，还能学习学习什么。
...
TL;DR: 将Never Give Up算法扩展到多智能体环境，在PettingZoo的simple_tag环境中评估，通过共享经验回放和调整内在探索信号获得更好性能


<details>
  <summary>Details</summary>
Motivation: NGU算法在稀疏奖励的强化学习任务中表现良好，但尚未在多智能体环境中得到验证。本研究旨在将NGU扩展到多智能体环境，探索其在多智能体协作任务中的有效性

Method: 将NGU算法扩展到多智能体环境，在PettingZoo的simple_tag环境中进行实验。研究了三个设计选择：1) 共享回放缓冲区vs个体回放缓冲区；2) 使用不同k阈值共享智能体间的episodic novelty；3) 使用异构的beta参数值

Result: 相比多智能体DQN基线，NGU获得中等更高的回报和更稳定的学习动态。共享回放缓冲区表现最佳，episodic novelty在k=1时效果相当但k值增大会降低学习效果，异构beta值未优于小的共同值
...
TL;DR: 提出基于多目标A*搜索的标签设置方法，用于电动汽车能量最优路径规划，避免处理复杂能量剖面


<details>
  <summary>Details</summary>
Motivation: 传统电动汽车路径规划算法假设已知初始能量水平，但实际场景中初始能量存在不确定性，需要为所有可能的初始能量水平规划最优路径（能量最优剖面搜索）。现有方法基于标签修正框架，需要处理复杂的能量剖面合并过程。

Method: 提出基于多目标A*搜索的标签设置方法，采用新颖的剖面支配规则，避免生成和处理复杂能量剖面。开发了四种变体方法。

Result: 在真实道路网络上使用实际能耗数据进行评估，实验结果表明能量剖面A*搜索的性能与已知初始能量水平的能量最优A*相当。

Conclusion: 提出的标签设置方法简单有效，能够处理电动汽车能量最优路径规划中的初始能量不确定性，避免了现有方法中复杂的剖面处理过程。
...
TL;DR: 提出OvertonScore框架量化LLM观点多样性，开发自动化基准评估模型多元性表现，发现当前模型仍有很大改进空间。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估缺乏对观点多样性的系统测量，需要可扩展的工具来评估和改进模型的多元性表现。

Method: 1) 形式化Overton多元性为集合覆盖度量；2) 进行大规模美国代表性人类研究(N=1209, 60个问题, 8个LLM)；3) 开发与人类判断高度相关的自动化基准。

Result: 模型平均OvertonScore为0.35-0.41，DeepSeek V3表现最佳，但都远低于理论最大值1.0。自动化基准与人类判断的秩相关性达ρ=0.88。

Conclusion: 将多元性对齐从规范性目标转化为可测量基准，为系统推进更多元的LLM发展奠定基础。
...
TL;DR: 论文提出认知计算框架PMCSF，通过模拟人类认知过程而非模仿数据表面特性来生成合成数据，解决AI数据崩溃危机。


<details>
  <summary>Details</summary>
Motivation: 当前合成数据生产范式追求统计平滑性，但会系统性地移除人类文本中具有认知基础的长尾不规则特征，导致训练在统计最优但认知贫乏的数据上会加速模型崩溃。

Method: 提出Prompt驱动的认知计算框架(PMCSF)，包含认知状态解码器(CSD)将非结构化文本反转为结构化认知向量，以及认知文本编码器(CTE)通过数学定义的认知扰动算子将这些状态重新物质化为富含人类典型缺陷的文本。

Result: 认知编解码验证中，CTE文本与人类文本的Jensen-Shannon散度为0.0614（标准LLM输出为0.4431），通过双盲专业媒体评审，异构模型间认知轮廓对齐的组内相关系数ICC>0.9。功能增益评估中，A股市场同构压力测试显示，整合CTE生成数据的策略在2015年股灾期间最大回撤减少47.4%，产生8.6%的防御性阿尔法，超过交易成本33倍。
...
TL;DR: CollabToolBuilder是一个多智能体LLM框架，通过专家在环指导迭代学习为目标任务创建工具，对齐人类意图和流程，同时最小化任务/领域适应和人类反馈收集的时间。


<details>
  <summary>Details</summary>
Motivation: 解决复杂迭代问题（如科学文档生成）中工具创建的挑战，需要对齐人类意图和流程，同时减少领域适应和人类反馈收集的负担。

Method: 采用四智能体架构（教练、编码员、批评家、资本化器），通过强化动态提示和系统化人类反馈集成来生成和验证工具，结合多智能体上下文学习、HITL控制和可重用工具资本化。

Result: 初步实验展示了系统在生成最先进研究论文或专利（给定摘要）等任务中的能力，并讨论了其在其他迭代问题解决中的适用性。

Conclusion: CollabToolBuilder是一个系统级集成和方法论，结合了多智能体上下文学习、HITL控制和可重用工具资本化，为复杂迭代问题提供灵活的解决方案。
...
TL;DR: 提出状态转移事件时间序列(STE-ts)和选择性时序汉明距离(STH)，避免传统方法中对大规模数据库进行代价高昂且失真的重采样操作


<details>
  <summary>Details</summary>
Motivation: 离散事件系统在自然观测、社会经济科学和工业系统中普遍存在，但现有分析方法通常未能充分利用其事件/状态双重特性：要么将信号建模为强调事件顺序对齐的转移事件序列，要么建模为需要重采样的分类或有序状态时间序列，这在观测周期和事件数量增长时会变得代价高昂且失真

Method: 定义状态转移事件时间序列(STE-ts)，提出新的选择性时序汉明距离(STH)，同时利用转移时间和状态持续时间，避免在大规模数据库上进行代价高昂且失真的重采样

Result: STH能够泛化重采样汉明距离和Jaccard度量，具有更好的精度和计算时间，并且能够聚焦于多个感兴趣的状态。在模拟和真实世界数据集上验证了这些优势

Conclusion: 提出的STE-ts和STH方法能够有效处理离散事件系统的双重特性，避免传统重采样方法的缺陷，在大规模数据库分析中具有更好的性能和实用性
...
TL;DR: 本文介绍了一种使用DSPy和GEPA模块的程序化偏倚风险评估方法，通过结构化代码优化替代手动提示设计，在多个偏倚风险领域实现了可复现的自动化评估。


<details>
  <summary>Details</summary>
Motivation: 随机对照试验的偏倚风险评估对于证据合成至关重要，但传统方法资源密集且存在评审者间变异性。现有LLM方法依赖难以复现、泛化或评估的手动工程提示，需要更结构化、可复现的自动化解决方案。

Method: 采用基于DSPy的程序化偏倚风险评估流程，使用GEPA模块通过帕累托引导搜索优化LLM推理，生成可检查的执行轨迹，实现优化过程的透明复现。在100个已发表荟萃分析的RCT上评估七个偏倚风险领域。

Result: 在方法报告更清晰的领域（如随机序列生成），GEPA生成的提示表现最佳；在分配隐藏和参与者盲法领域结果相似；商业模型整体表现略优。与三个手动设计提示相比，GEPA在随机序列生成和选择性报告领域性能提升30%-40%，在其他领域表现相当。

Conclusion: GEPA能够为偏倚风险评估生成一致且可复现的提示，支持在证据合成中结构化、原则性地使用LLM，为自动化偏倚风险评估提供了透明、可复现的解决方案。
...
TL;DR: M3PO是一个强化学习框架，通过并行策略rollout和多路径协作机制，将集体洞察注入推理过程，解决传统CoT推理中的内部确定性限制问题。


<details>
  <summary>Details</summary>
Motivation: 传统Chain-of-Thought推理在解码过程中表现出内部确定性，限制了探索合理替代方案的能力。现有方法虽然尝试在连续语义空间中进行推理，但仍受限于自回归解码的贪婪性质，无法有效探索替代推理路径。

Method: 提出Multi-Path Perception Policy Optimization (M3PO)框架：1) 利用并行策略rollout作为自然多样化的推理来源；2) 通过轻量级协作机制将跨路径交互整合到策略更新中；3) 每条轨迹通过同伴反馈精炼推理，培养更可靠的多步推理模式。

Result: M3PO在知识和推理密集型基准测试中实现了最先进的性能。经过M3PO训练的模型保持了可解释性和推理效率。

Conclusion: 多路径协作学习为稳健推理提供了有前景的方向，M3PO框架通过集体洞察注入和跨路径交互，有效解决了传统CoT推理的局限性。
...
TL;DR: 利用大型语言模型将合成知识转化为代码，实现计算机辅助合成规划中的战略考量，包括收敛组装、保护基最小化和最佳环形成序列等。


<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助合成规划系统虽然能生成化学上有效的反应步骤，但难以整合战略考量，如收敛组装、保护基最小化和最佳环形成序列等战略原则。

Method: 利用大型语言模型将合成知识转化为Python代码，将战略原则（如战略官能团转换和环构建策略）形式化为可验证的代码而非简单启发式规则。

Result: 创建了USPTO-ST数据集（带战略标注的合成路线），实现了基于自然语言的路线检索（Top-3准确率75%），并通过时间趋势分析和化学直观路线聚类验证了库的有效性。

Conclusion: 该工作弥合了计算机辅助合成规划中的战术-战略鸿沟，使路线能够按战略标准而非仅结构进行规范、搜索和评估，为合成规划提供了新的战略分析能力。
...
TL;DR: LEC方法通过线性期望约束实现FDR控制，为LLM预测提供统计保证，并扩展到双模型路由机制
...
Method: 将选择性预测重新解释为约束决策问题，通过线性期望约束强制选择与错误指标的关系，建立有限样本充分条件计算FDR约束的最大覆盖阈值，并扩展到双模型路由机制

Result: 在闭式和开放式QA数据集上，LEC实现了更严格的FDR控制，显著提高了样本保留率，双模型路由机制在更低风险水平下接受更多正确样本

Conclusion: LEC为LLM预测提供了统计保证的FDR控制框架，通过校准阈值和模型路由机制有效管理预测风险
...
TL;DR: 提出一种利用预训练语言-策略对组合的方法，通过创建统一表示空间实现跨任务快速迁移
...
Method: 受计算机视觉中CLIP的启发，将指令和对应策略视为同一任务在不同模态的表示。通过扩展CLIP思想到强化学习，创建自然语言和策略嵌入的统一表示空间。

Result: 实验结果表明该方法在实现跨任务快速迁移方面具有效用。

Conclusion: 通过将CLIP的对齐思想扩展到强化学习领域，能够有效利用预训练的语言-策略对组合，实现任务间的快速知识迁移。
...
<details>
  <summary>Details</summary>
Motivation: 历史事件建模面临四大挑战：极端数据稀缺（N<<100）、异构噪声测量、反事实缺失、以及需要人类可解释的解释。现有机器学习方法难以处理这些历史分析特有的问题。

Method: 提出HistoricalML概率神经符号框架，整合四个核心组件：(1) 贝叶斯不确定性量化分离认知与偶然不确定性；(2) 结构因果模型处理混杂下的反事实推理；(3) 合作博弈论（沙普利值）进行公平分配建模；(4) 注意力神经网络实现上下文依赖的因子加权。

Result: 理论分析表明：在稀疏数据下，结合领域知识强先验可实现一致估计；沙普利分配满足公理公平性保证。实证应用：19世纪非洲分割（N=7殖民势力）识别德国+107.9%差异为WWI前可量化结构张力；第二次布匿战争（N=2派系）蒙特卡洛模拟显示坎尼战役迦太基胜率57.3%，扎马战役罗马胜率57.8%。反事实分析揭示迦太基政治支持（6.4分）而非军事实力是关键因素。

Conclusion: HistoricalML框架成功解决了历史建模的核心挑战，通过概率神经符号方法在极端数据稀缺下实现可解释的历史分析，为历史事件提供了量化、反事实和公平的建模工具，在非洲殖民和布匿战争案例中验证了其有效性。
...
TL;DR: LLM Jury-on-Demand：一个动态、基于学习的框架，通过训练可靠性预测器评估LLM评委何时与人类专家一致，为每个数据点动态选择最优评委团，实现可扩展且可靠的评估。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）进入高风险领域，需要既适合实时部署又可扩展，又对关键决策可靠的评估方法。人类评估可靠但缓慢昂贵，单一LLM评委有偏见，静态评委团缺乏适应性。

Method: 训练一组可靠性预测器，利用标记分布、嵌入和结构输入特征来评估LLM评委何时会与人类专家达成一致。为每个数据点动态选择最可靠的评委组成最优评委团，并使用其可靠性作为权重聚合评分。

Result: 在摘要和RAG基准测试中，动态评委团系统比单一评委和静态评委团基线显著提高了与人类判断的相关性。

Conclusion: 自适应、基于学习的评委团方法为现代LLMs在高风险领域构建可扩展、更可靠和可信的评估系统提供了前景。
...
TL;DR: 该论文系统研究了LLM中的幻觉相关神经元（H-Neurons），发现仅需不到0.1%的神经元就能可靠预测幻觉发生，这些神经元与过度顺从行为因果相关，且源于预训练阶段。


<details>
  <summary>Details</summary>
Motivation: LLM经常产生幻觉（看似合理但事实错误的输出），影响其可靠性。先前研究多从宏观角度（如训练数据和目标）分析，而神经元层面的机制尚未充分探索。

Method: 从三个角度系统研究：1）识别幻觉相关神经元；2）分析其行为影响；3）追溯其起源。使用稀疏神经元子集预测幻觉，进行受控干预实验，并追踪神经元到预训练基模型。

Result: 发现极稀疏神经元子集（<0.1%）能可靠预测幻觉，且在不同场景下泛化能力强；这些神经元与过度顺从行为存在因果联系；它们源于预训练阶段，在预训练后仍保持预测能力。

Conclusion: 研究将宏观行为模式与微观神经机制联系起来，为开发更可靠的LLM提供了洞见。幻觉相关神经元在预训练阶段形成，可通过干预这些神经元来减少幻觉。
...
TL;DR: 该研究探索大语言模型能否学习由自然数迭代质因数分解生成的确定性树序列，发现GPT-2架构能部分掌握该算术文本的内部语法结构。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大语言模型的学习能力边界，特别是能否超越经验数据学习到算术本身的结构规律。通过将自然数映射为树结构，创建具有可测量统计结构的算术文本，测试模型对确定性数学序列的学习能力。

Method: 方法包括：1) 将每个整数映射为有根平面树，形成树序列$\mathbb{N}\mathcal{T}$；2) 使用GPT-2架构的Transformer网络从头训练；3) 在$10^{11}$个元素上训练模型；4) 通过next-word和masked-word预测任务测试模型的预测能力。

Result: 结果显示模型部分学习了$\mathbb{N}\mathcal{T}$的内部语法，能够捕捉非平凡的规律性和相关性。这表明模型确实能够学习算术文本的结构特征，而不仅仅是记忆训练数据。

Conclusion: 结论是语言模型的学习能力可能扩展到超越经验数据，能够学习算术本身的结构。这为理解大语言模型如何掌握抽象数学规律提供了实证依据。
...
TL;DR: 提出在知识图谱网络中通过最小化惊奇来指导推理，将神经科学的自由能原理与知识图谱系统连接起来


<details>
  <summary>Details</summary>
Motivation: 探索知识图谱推理的新方法，将神经科学中的自由能原理应用于知识图谱系统，建立图距离与惊奇度之间的联系

Method: 使用有向图中的最短路径距离形式化惊奇度，构建基于知识图谱的智能体框架，将图距离与图神经网络中的消息传递深度和基于模型的强化学习中的世界模型轨迹联系起来

Result: 这是一个进行中的研究，探索基于距离的惊奇度是否能扩展最近关于语法通过树结构最小化惊奇和自由能的研究

Conclusion: 提出了一个将自由能原理与知识图谱系统连接的理论框架，为知识图谱推理提供了基于惊奇最小化的新视角
...
TL;DR: 本文提出一个基于n-gram语言模型的国际象棋走法预测框架，将走法预测视为行为分析任务，针对不同技能水平玩家训练独立模型，实现实时高效的走法预测。


<details>
  <summary>Details</summary>
Motivation: 传统国际象棋引擎主要关注最优走法计算，忽略了不同技能水平玩家走法的变异性。本文旨在通过行为分析的方法，更好地理解和预测不同水平玩家的实际走法模式。

Method: 将玩家分为7个技能等级，使用Lichess平台的开源数据，为每个等级训练独立的n-gram语言模型。框架包含模型选择器模块，能根据游戏早期信息动态选择最适合的模型进行走法预测。

Result: 模型选择器在早期游戏信息（16个半回合）下能达到31.7%的技能等级分类准确率。走法预测框架的Selector Assisted Accuracy比基准准确率提高了39.1%，且计算效率高，适合实时分析。

Conclusion: 该框架成功将国际象棋走法预测转化为行为分析任务，通过针对不同技能水平的建模显著提高了预测准确性，为实时国际象棋分析提供了高效实用的工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM主要用作NLG任务评估器，但在更广泛评估场景中应用有限。现有方法依赖人工设计的评估原则，成本高且与数据和LLM理解不对齐，难以泛化

Method: 1) 规则蒸馏：使用LLM辅助的蒙特卡洛树搜索从数据中自动提取评分规则；2) 规则应用策略：Chain-of-Rule引导LLM遵循蒸馏规则，以及通过强化学习训练规则增强的LLM评估器

Result: 在多样化任务上的广泛实验证明了该方法在各种评估场景中的有效性和泛化能力

Conclusion: 提出的规则增强评估范式解决了现有LLM评估方法的局限性，通过自动规则蒸馏和有效规则应用，显著提升了LLM作为通用评估器的性能
...
TL;DR: RL能合成新的推理策略而非仅放大现有行为，但前提是模型需先通过SFT掌握原子技能（参数推理和上下文推理）。


<details>
  <summary>Details</summary>
Motivation: 研究RL如何贡献于推理能力——是合成新技能还是仅放大现有行为，通过互补推理任务（需要整合内部参数知识和外部上下文信息）来探讨这一问题。

Method: 使用人类传记的合成数据集，将互补推理能力严格解耦为两个原子技能：参数推理（依赖内部知识）和上下文推理（依赖外部信息）。在三个难度级别（I.I.D.、组合、零样本）评估泛化能力，比较SFT和RL方法。

Result: 发现SFT在分布内表现良好但在分布外泛化（特别是零样本设置）上困难，存在"SFT泛化悖论"：仅监督复合任务的模型在分布内近乎完美但在分布外崩溃。RL能作为推理合成器而非概率放大器，但前提是基础模型已通过SFT掌握独立原子技能。

Conclusion: RL不仅仅是放大器，在具备足够原子基础的情况下，RL能主动从学习到的原语中合成复杂推理策略，无需对复杂策略进行显式监督。解耦的原子训练后接RL为复杂推理任务提供了可扩展的泛化路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在GUI视觉定位中面临小目标、视觉相似目标和布局歧义等挑战，现有模型的定位能力有限且未充分利用其推理潜力。

Method: 提出Chain of Ground (CoG)框架，采用免训练的多步定位方法，让模型进行迭代视觉推理和优化，逐步反思和调整假设，而非直接预测。

Result: 在ScreenSpot Pro基准上达到68.4%准确率，提升4.8点；在真实工业控制面板数据集TPanel UI上，相比Qwen3 VL 235B基线提升6.9点。

Conclusion: 通过结构化迭代优化而非额外训练，能够释放模型的定位潜力，该方法在真实世界和数字界面中均表现出有效性。
...
TL;DR: LLM CHESS是一个通过国际象棋游戏评估大语言模型推理和指令遵循能力的框架，测试了50多个模型，发现即使是最先进的模型也难以完成游戏或取得稳定胜利。
...
Method: 通过国际象棋游戏评估模型，使用随机对手进行测试，收集胜率、移动质量、合法性、幻觉动作等行为指标。对顶级推理模型使用可变技能配置的象棋引擎进行Elo评分估计。

Result: 许多最先进的模型难以完成游戏或取得稳定胜利。推理模型与非推理模型之间存在明显差距。该框架有效减少了过拟合和记忆，防止基准饱和，即使顶级推理模型也感到困难。

Conclusion: LLM CHESS提供了一个动态、随机的评估框架，能更真实地测试LLM的推理和指令遵循能力，并发布了实验框架、公共排行榜和相关游戏数据集以支持未来研究。
...
### [380] [TagSplat: Topology-Aware Gaussian Splatting for Dynamic Mesh Modeling and Tracking](https://arxiv.org/abs/2512.01329)
*Hanzhi Guo,Dongdong Weng,Mo Su,Yixiao Chen,Xiaonuo Dongye,Chenyu Xu*

Main category: cs.GR

TL;DR: 提出基于高斯泼溅的拓扑感知动态重建框架，通过高斯拓扑结构编码空间连接性，实现拓扑一致的高质量网格序列重建


<details>
  <summary>Details</summary>
Motivation: 现有4D重建方法难以生成高质量的拓扑一致网格，而拓扑一致的动态模型序列对于动画和模型编辑等应用至关重要

Method: 基于高斯泼溅的拓扑感知动态重建框架，引入高斯拓扑结构显式编码空间连接性，支持拓扑感知的密度化和剪枝，保持高斯表示的流形一致性，使用时序正则化确保拓扑时间一致性，结合可微分网格光栅化提升网格质量

Result: 实验结果表明，该方法重建的拓扑一致网格序列精度显著高于现有方法，生成的网格支持精确的3D关键点跟踪

Conclusion: 提出的拓扑感知动态重建框架能够有效生成高质量的拓扑一致网格序列，为动画和模型编辑等应用提供可靠支持
...
### [381] [Textured Word-As-Image illustration](https://arxiv.org/abs/2512.01648)
*Mohammad Javadian Farzaneh,Selim Balcisoy*

Main category: cs.GR

TL;DR: 提出一个全自动文本图像生成管道，能根据语义概念、单词和字母生成既易读又与语义概念高度对齐的文本图像。


<details>
  <summary>Details</summary>
Motivation: 传统文本设计需要专业设计技能，难以让普通用户快速创建既美观又具语义表达力的文本图像。需要一种自动化方法，在保持文本可读性的同时，将语义概念有效融入文本设计中。

Method: 用户输入语义概念、单词和字母；系统使用语义概念改变字母形状，基于预定义提示使用Stable Diffusion模型生成纹理；将纹理映射到文本图像上，保持整体可读性；提供实时调整功能，允许用户改变纹理比例并应用到文本图像。

Result: 用户评估表明，该方法能有效表示语义意义而不损害可读性，成为图形设计、Logo创建和艺术字体设计的强大创新工具。

Conclusion: 提出的全自动管道成功解决了语义表达与文本可读性之间的平衡问题，为普通用户提供了强大的创意设计工具，在多个设计领域具有应用潜力。
